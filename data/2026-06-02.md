...
#### 🧩 TL;DR
本文构建了迄今为止最大且最全面的浮游生物图像数据集Planktonzilla-17M（1740万张图像，602个分类类别），通过受控实验发现基于分类学文本训练的监督ViT模型可匹配或超越CLIP式训练，并揭示现有生物基础模型在海洋成像领域存在显著局限性。

---

#### 📘 Detailed Summary
**Motivation:** 现有的浮游生物分类模型在单一数据集上表现良好，但由于训练数据集彼此孤立且标签不一致，模型无法跨不同成像系统和环境泛化，这限制了海洋生态系统监测和全球碳循环研究的可靠性。

**Method:** 作者首先整合了覆盖13个成像系统的公开浮游生物图像集合，构建了包含1740万张图像、602个分类类别（其中201个为物种级）的统一数据集Planktonzilla-17M，并标准化了分类学标签和地理环境元数据。在此基础上，以共享ViT骨干网络为基准，对监督学习和CLIP式图文对比训练进行了受控对比实验。

**Result:** 实验表明，当使用分类学谱系作为文本输入时，监督分类器的性能可匹配或超越CLIP式训练；BioCLIP和BioCLIP2在零样本和少样本设定下对浮游生物的分类表现极差；利用Planktonzilla-17M可显著提升浮游生物分类性能。

**Conclusion:** 该研究揭示了当前生物基础模型（如BioCLIP）在海洋成像领域存在显著性能瓶颈，表明构建大规模统一数据集是提升浮游生物分类泛化能力的关键，并为海洋生物成像领域的模型选择提供了实证依据。
...
### [2] [Bridging the 2D-3D Gap: A Hierarchical Semantic-Geometric Map for Vision Language Navigation](https://arxiv.org/abs/2606.00095)
*Kailing Li, Tianwen Qian, Lijin Yang, Yuqian Fu, Jingyu Gong, Xiaoling Wang, Liang He*

#### 🧩 TL;DR
提出层级语义-几何地图（HSGM），将3D几何信息转化为VLM可兼容的结构化表示，在零样本设置下实现视觉语言导航，在R2R-CE和RxR-CE基准上达到最先进性能，并超越部分有监督方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉语言模型在2D视觉和语言理解上表现优异，但缺乏3D空间推理能力，难以捕捉动作与空间变迁之间的因果动态，导致导航不可靠尤其是在零样本场景下，存在语义与几何之间的关键鸿沟。

**Method:** 提出层级语义-几何地图（HSGM），将3D几何信息转化为多通道俯视图表示，包含三个层级：几何层记录可导航区域和障碍物、语义层表示物体及其关系、决策层支持高层任务推理和目标选择。VLM作为高层语义规划器解读HSGM选择几何有效的航点，底层由经典路径规划算法执行无碰撞移动，实现语义推理与动作执行的完全解耦；复杂指令被分解为子任务以缓解长程导航中的进度遗忘或幻觉问题。

**Result:** 在R2R-CE和RxR-CE两个基准上的大量实验表明，提出的零样本框架达到了最先进性能，并且超越了若干有监督方法。

**Conclusion:** 通过HSGM将VLM与物理世界有效连接，验证了纯零样本VLN的可行性并为该方向提供了新范式；语义推理与动作执行解耦的设计思路对提升导航鲁棒性具有启示意义。
...
### [3] [Diversity Over Frequency: Rethinking Tool Use in Visual Chain-of-Thought Agents](https://arxiv.org/abs/2606.00096)
*Dong-Hee Kim, Reuben Tan, Donghyun Kim*

#### 🧩 TL;DR
本文发现视觉推理智能体在训练过程中会出现“工具使用崩溃”现象——模型逐步停止使用外部视觉工具但任务准确率反而提升，并提出熵正则化方法通过鼓励多样化探索来提升推理性能，表明工具在训练阶段应被视为推理的“脚手架”。

---

#### 📘 Detailed Summary
**Motivation:** 现有工作主要研究视觉工具在简单视觉搜索任务中的应用，而其在复杂视觉推理（如3D空间推理、医学视觉问答）中需同时整合局部证据与全局上下文，这一场景下的工具使用动态尚未被充分探索。

**Method:** 作者首先在3D空间推理和医学VQA任务上系统研究了视觉智能体的工具使用行为，识别出“工具使用崩溃”现象及其不对称性；随后提出在训练过程中加入熵正则化项，通过鼓励生成和工具调用的多样化探索来缓解崩溃带来的性能瓶颈。

**Result:** 实验发现：完全消除工具使用会降低性能，而强制鼓励工具使用虽然大幅增加调用次数但仅带来边际收益；vanilla训练和工具使用鼓励均降低了rollout多样性，而熵正则化在工具使用逐渐下降的情况下取得了最佳性能。

**Conclusion:** 研究表明视觉工具应在训练阶段被视为推理的“脚手架”，其核心价值在于促进语言生成和视觉工具调用上的广泛探索，而非在推理时维持高使用率；工具使用崩溃不限于3D空间推理，在医学VQA上也观察到类似动态。
...
### [4] [CoCoVideo: The High-Quality Commercial-Model-Based Contrastive Benchmark for AI-Generated Video Detection](https://arxiv.org/abs/2606.00101)
*Huidong Feng, Wentao Chen, Jie Chen, Xinqi Cai, Ruolong Ma, Yinglin Zheng, Yuxin Lin, Ming Zeng*

#### 🧩 TL;DR
提出了包含13个商业级生成器的AIGC视频伪造检测数据集CoCoVideo-26K，以及结合对比学习和置信度门控MLLM推理的检测框架CoCoDetect，在高质量视频伪造检测任务上达到SOTA性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有AIGC视频伪造检测数据集主要依赖开源视频生成模型，其质量远低于商业AIGC系统；即便包含少量商业样本的数据集也往往保留可见水印，损害真实性并阻碍模型向高保真AIGC视频的泛化，亟需高质量、无干扰的商业级伪造视频检测基准。

**Method:** 构建CoCoVideo-26K数据集，涵盖13种主流商业视频生成器并提供语义对齐的真实-伪造视频对；提出CoCoDetect框架，采用R3D-18骨干网络提取时空表征，集成对比学习与置信度门控MLLM推理，对低置信度样本路由至MLLM进行物理合理性与场景一致性的深度推理。

**Result:** 在CoCoVideo-26K数据集及多个公开基准上的大量实验表明，CoCoDetect达到了SOTA性能，验证了其在高保真AIGC视频伪造检测任务中的鲁棒性与泛化能力。

**Conclusion:** 通过构建高质量商业级AIGC视频数据集和融合对比学习与多模态大模型推理的检测框架，有效缩小了真实场景与现有基准之间的差距，为高逼真度视频伪造检测提供了新的基准和方法方向。
...
### [5] [Visual-Noise Guided In-Context Distillation for Multimodal Large Language Model Unlearning](https://arxiv.org/abs/2606.00105)
*Junkai Chen, Yuhao He, Junxiang You, Ruiqi Liu, Chenyu Wang, Shu Wu*

#### 🧩 TL;DR
提出视觉噪声引导的上下文蒸馏框架（VGID），通过双模态干预构建教师分布进行蒸馏，实现多模态大语言模型的参数级遗忘，无需外部教师模型或显式标注，在保持模型效用的同时显著降低遗忘集ROUGE-L指标。

---

#### 📘 Detailed Summary
**Motivation:** 现有MLLM机器遗忘方法面临平衡遗忘效果与模型效用的困境：基于训练的方法难以兼顾两者，而基于上下文的无训练方法虽避免参数更新但无法移除参数级别的记忆知识，易受逆向工程攻击，且在视觉输入作为强条件信号的多模态场景下效果不足。

**Method:** 提出VGID框架，通过双模态干预（视觉扰动+文本上下文遗忘）从冻结基座模型动态构建面向遗忘的教师分布，并将该干预诱导分布作为蒸馏信号，引导学生模型实现参数级别的遗忘，无需外部教师模型或显式不良响应标注。

**Result:** 实验结果表明，VGID在代表性设置下将遗忘集ROUGE-L降低0.371，而保留集ROUGE-L仅下降0.055，在实现强大遗忘效果的同时保持了竞争性的模型效用。

**Conclusion:** VGID通过蒸馏框架有效解决了多模态大语言模型中训练-free方法无法实现参数级遗忘的缺陷，表明结合视觉扰动与上下文干预的蒸馏策略能够在不牺牲通用性能的前提下实现有效的多模态知识遗忘，为MLLM安全性和隐私保护提供了可行路径。
...
### [6] [Physics from Video: Identifiability of Time-Invariant Second-Order ODEs under Minimal Trajectory Conditions](https://arxiv.org/abs/2606.00115)
*Yuanyuan Wang, Wenjie Wang, Kun Zhang, Mingming Gong*

#### 🧩 TL;DR
本文证明了在满足水平集斜率覆盖条件下，编码器-only框架可以从原始像素中唯一恢复二阶线性ODE的物理参数，并首次刻画了不同阻尼 regime 下的最小数据需求，其中欠阻尼系统仅需单段视频即可识别。

---

#### 📘 Detailed Summary
**Motivation:** 视觉真实感与物理理解之间存在鸿沟，现有基于视频的世界模型难以从原始像素中可靠地恢复连续时间物理定律的结构性参数，且缺乏对可识别性的理论保证，尤其是在不依赖像素重构的解码器-自由框架下。

**Method:** 本文从理论角度研究了编码器-自由（decoder-free）流水线的可识别性，提出了水平集斜率覆盖条件（level-set slope-coverage condition），证明了该条件下学习到的潜在空间与真实物理状态局部仿射，从而实现精确参数恢复。同时引入了方差地板正则化器（variance-floor regularizer）以稳定无解码器目标并防止潜在空间坍缩。

**Result:** 理论结果表明，欠阻尼系统仅需单段视频即可实现参数可识别，而其他阻尼 regime 则需要三段不同轨迹。在合成数据和真实数据上的实验验证了该方法能够可靠地从视频中估计可解释的物理常数，无需计算密集的像素重建。

**Conclusion:** 该研究为从视频中直接推断物理定律提供了首个可识别性理论框架，证明了物理常数可以在不依赖像素重建的前提下被可靠估计，兼顾了物理正确性与透明性，推动了视觉与物理理解之间的桥梁构建。
...
### [7] [Versatile Framework with Semantic and Structural guidance for Image Reconstruction from Brain Activity](https://arxiv.org/abs/2606.00121)
*Yizhuo Lu, Changde Du, Qiongyi Zhou, Liuyun Jiang, Huiguang He*

#### 🧩 TL;DR
提出MindDiffuser两阶段图像重建框架，通过结合CLIP文本与视觉特征解码及Stable Diffusion生成，在fMRI、EEG、MEG三种模态上实现比现有方法更优的语义一致性与细粒度结构对齐的图像重建。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于文本到图像生成模型的脑解码方法虽能在语义层面（如概念与物体）近似重建视觉刺激，但在细粒度结构信息（如位置、朝向、大小）上与原始刺激的一致性不足，限制了模型的可控性与可解释性，因此亟需一种能够同时保持语义准确性和结构保真度的图像重建框架。

**Method:** 提出MindDiffuser两阶段图像重建框架：第一阶段将脑信号解码得到的CLIP文本嵌入输入Stable Diffusion，生成包含语义信息的初步图像；第二阶段利用解码得到的浅层CLIP视觉特征作为监督信号，通过反向传播迭代精调第一阶段生成图像的潜在特征向量，以对齐细粒度结构信息。该框架在fMRI、EEG、MEG三种模态的脑响应数据集上进行验证。

**Result:** 在视觉刺激诱发的三种模态（fMRI、EEG、MEG）脑响应数据集上的实验表明，MindDiffuser显著提升了已有最优模型的性能，证实了该方法在多种脑信号模态下的有效性与通用性；空间与时间可视化结果进一步支持了该框架的神经生物学合理性。

**Conclusion:** MindDiffuser通过两阶段设计实现了语义与结构的解耦重建，为脑信号解码提供了兼具控制力和可解释性的新范式，同时其跨模态的有效性为未来不同脑信号模态的神经解码研究提供了指导依据。
...
### [8] [Positional Encodings Anchor Spatial Structure in Vision Transformers: A Geometric Perspective on Robustness](https://arxiv.org/abs/2606.00124)
*Mahmoud Mannes*

#### 🧩 TL;DR
本文提出SSDC指标量化视觉Transformer中位置编码对表征几何结构的影响，发现位置编码通过建立稳定的索引锚定空间组织来提升模型对内容破坏性分布偏移的鲁棒性，且该鲁棒性主要依赖于位置参考框架的存在而非具体编码机制。

---

#### 📘 Detailed Summary
**Motivation:** 位置编码(PE)在视觉Transformer(ViT)中已知影响模型性能和鲁棒性，但其如何塑造内部空间表征以及这种变化与鲁棒性之间的关系尚不清楚，现有研究缺乏对这一几何层面的系统性理解。

**Method:** 提出空间相似度距离相关性(SSDC)指标来量化token表征中的空间结构；通过对比无PE、可学习绝对位置编码、正弦位置编码和旋转位置编码等不同形式的位置编码，系统分析ViT内部表征的几何组织及其在内容破坏性分布偏移下的稳定性。

**Result:** 无PE训练的ViT仍能发展出非平凡的空间结构，但该结构由视觉内容驱动且在token置换下崩溃；所有引入PE的模型均表现出向索引锚定空间组织的一致转变，表征在内容扰动下保持稳定且鲁棒性显著提升；不同PE产生不同的深度方向空间结构轨迹，但鲁棒性特性高度相似。

**Conclusion:** 鲁棒性主要依赖于稳定的位置参考框架存在而非具体编码机制，这一几何视角的解释为未来编码方案的原则性设计提供了理论依据与指导。
...
### [9] [Digital-to-Physical Transfer of Adversarial Patches for Aerial Vehicle Detection](https://arxiv.org/abs/2606.00159)
*Jung Heum Woo, Eun-Kyu Lee*

#### 🧩 TL;DR
本文针对基于DNN的航空目标检测器，研究了物理对抗补丁攻击方法，通过在数字域优化并结合NPS和TV约束生成可打印补丁，实验发现ON补丁在物理环境中鲁棒性最强，而天气增强并未提升优化效果，揭示了航空目标检测系统的实际弱点。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于DNN的航空目标检测器在环境监测和城市分析中广泛应用，但已知其对对抗样本存在脆弱性，而针对航空影像的物理可打印对抗攻击研究尚不充分，缺乏从数字优化到真实部署的完整评估。

**Method:** 本文提出了一种连接数字优化与物理部署的对抗补丁攻击方法：在数字域中通过最小化最大目标性分数的损失函数优化补丁，并引入不可打印性分数（NPS）和总变差（TV）约束确保补丁的可打印性和空间平滑性；优化后的补丁被打印并部署为ON、OFF和OFF-Side三种配置，使用YOLOv3检测器进行评估。

**Result:** 数字域中OFF补丁实现了最高的平均目标性降低率（AORR 85.51%），但在物理环境中ON补丁展现出最优的鲁棒性（目标性分数比率OSR为0.197-0.343）。此外，天气增强并未改善补丁优化效果。

**Conclusion:** 研究表明物理对抗补丁攻击对航空目标检测器构成真实威胁，补丁的持续可见性是物理鲁棒性的关键因素，且数字域表现最优的补丁在物理域未必有效，这为理解实际部署中航空目标检测系统的对抗脆弱性提供了重要见解。
...
### [10] [APE: Agentic Prompt Enhancer for Image Generation and Editing](https://arxiv.org/abs/2606.00204)
*Zijian Huang, Jay Zhangjie Wu, Zian Wang, Tianshi Cao, Jiasi Chen, Sanja Fidler, Huan Ling, Xuanchi Ren*

#### 🧩 TL;DR
提出Agentic Prompt Enhancer (APE)框架，通过对小型语言模型进行后训练实现提示词增强，在保持轻量的同时显著提升视觉生成的对齐质量，其中多智能体变体MAPE在复杂组合任务上表现尤为突出。

---

#### 📘 Detailed Summary
**Motivation:** 文本引导的图像生成与编辑系统对提示词措辞高度敏感，语义相似的请求因表述方式、具体程度和视觉约束的明确性不同而产生不同输出；现有强提示词增强器多依赖ChatGPT等大型闭源LLM，引入额外成本、延迟和部署依赖，因此需将提示词增强作为可训练的轻量组件而非外围用户选择。

**Method:** 提出Agentic Prompt Enhancer (APE)框架，通过对小型语言模型进行后训练将其构建为提示词增强智能体；支持单智能体重写（SAPE，一次性重写提示词）和多智能体增强（MAPE，将增强分解为路由器-重写器-组合器的流水线过程）两种模式，用于处理对象、属性、空间关系和编辑等组合约束；采用任务感知的奖励函数和后训练协议，无需修改下游视觉模型。

**Result:** 在具有挑战性的图像生成和编辑基准测试上，后训练的小型提示词增强器可靠地优于其基础版本，缩小了与闭源提示词增强器之间的差距；MAPE在复杂组合任务中表现尤为突出。

**Conclusion:** 通过任务感知后训练，轻量级小语言模型可以成为高效的提示词增强组件，显著提升视觉对齐能力；多智能体分解策略在处理组合性视觉约束方面具有明显优势，为摆脱对大型闭源LLM的依赖提供了可行路径。
...
### [11] [StressDream: Steering Video World Models for Robust Policy Evaluation and Improvement](https://arxiv.org/abs/2606.00267)
*Junwon Seo, Sushant Veer, Ran Tian, Wenhao Ding, Apoorva Sharma, Karen Leung, Edward Schmerling, Marco Pavone, Andrea Bajcsy*

#### 🧩 TL;DR
StressDream通过优化扩散式世界模型的初始噪声，在推理时将想象引导向高影响且合理的结果（如任务失败），从而实现对机器人动作的鲁棒策略评估与改进。

---

#### 📘 Detailed Summary
**Motivation:** 现有视频世界模型能够基于机器人动作生成逼真的未来观测，但策略评估与改进通常依赖名义上的单一想象，难以捕捉高影响结果，除非采样大量样本，效率低下。

**Method:** StressDream通过优化扩散式世界模型的初始噪声来引导想象生成过程，采用两个互补目标：语义目标利用视觉-语言模型对生成视频进行推理以提供信息性梯度，合理性目标防止优化后的噪声偏离分布导致不可信的想象。

**Result:** 在自动驾驶和机器人操作任务中，使用最先进的视频世界模型进行实验，StressDream能够有效将想象引导至由文本指定的高影响且合理的结果（如任务失败），从而识别出可能导致不良后果的动作。

**Conclusion:** StressDream通过推理时引导世界模型想象高影响但合理的未来场景，为鲁棒的策略评估与改进提供了有效手段，有助于识别和避免机器人动作中潜在的高风险结果。
...
### [12] [Hyperbolic and Evidence-Prioritized Experts for Large Vision-Language Models](https://arxiv.org/abs/2606.00275)
*Zijie Zhou, Dandan Zhu, Hangxiangpan Wang, Heng Zhang, Huishen Jiao, Yi Zhao*

#### 🧩 TL;DR
针对LVLM中视觉与语言模态的非对称性（文本描述是视觉场景的子集、深层语言专家依赖参数记忆而非上下文证据），提出AsyMoE架构，通过双曲空间跨模态专家、证据优先语言专家和模态内专家三组专家显式建模该非对称性，在激活参数减少25.45%的情况下平均提升1.5%，在幻觉敏感任务上提升达3.8%。

---

#### 📘 Detailed Summary
**Motivation:** 现有MoE-LVLM方法对视觉和语言模态采用对称架构，忽视了二者处理的固有非对称性，导致两个关键问题：一是文本与视觉形成层次包含关系而非平行关系，欧氏专家空间难以编码这种结构；二是深层语言专家逐渐从基于证据的处理转向依赖参数记忆，失去对输入视觉和语言信息的接地（grounding）。

**Method:** 提出AsyMoE架构，显式建模视觉与语言处理的非对称性，包含三类专家：1）模态内专家（Intra-modality experts），分别处理视觉和语言模态的特有表征；2）双曲跨模态专家（Hyperbolic inter-modality experts），利用负曲率几何捕获跨模态层次包含关系；3）证据优先语言专家（Evidence-priority language experts），抑制深层网络中的参数记忆激活，保持对上下文证据的持续接地。

**Result:** 在多项多模态任务上，AsyMoE相比基线方法取得一致提升：平均相较MoE变体提升1.5%，在幻觉敏感任务上最高提升3.8%；同时相比稠密模型激活参数减少25.45%，实现了计算效率与性能的双重改进。

**Conclusion:** 视觉与语言模态在LVLM中具有本质非对称性，通过双曲几何显式建模层次包含关系以及引入证据优先机制抑制深层参数记忆依赖，能够有效提升模型在多模态任务上的表现并缓解幻觉问题，同时保持计算高效性。
...
### [13] [Zamba2-VL Technical Report](https://arxiv.org/abs/2606.00390)
*Hassan Shapourian, Kasra Hejazi, Olabode M. Sule, Beren Millidge*

#### 🧩 TL;DR
Zamba2-VL是构建在Zamba2混合架构（Mamba2状态空间层+少量共享Transformer块）上的视觉语言模型系列，在多项视觉语言基准上与同等规模的Transformer开源模型（如Molmo2、Qwen3-VL、InternVL3.5）竞争力相当，同时凭借近乎线性的预填充计算和较小的常规模态状态，在1.2B、2.7B和7B参数规模下实现约一个数量级的首令牌延迟（TTFT）降低。

---

#### 📘 Detailed Summary
**Motivation:** 现有的视觉语言模型（VLMs）大多基于Transformer架构，尽管性能优异，但其自注意力机制在长序列下存在二次计算复杂度和较高的首令牌延迟（TTFT），限制了其在边缘和端侧设备上的高效部署。纯状态空间模型（SSM）和早期混合模型虽试图解决此问题，但在多模态理解、OCR、计数等复杂任务上仍落后于Transformer基线。

**Method:** Zamba2-VL基于Zamba2混合语言模型架构，该架构将Mamba2状态空间层与少量共享Transformer注意力块相结合，旨在兼顾SSM的线性推理效率与Transformer的表达能力。模型通过视觉编码器提取图像特征并映射至语言模型输入空间，采用视觉语言联合训练策略，发布了1.2B、2.7B和7B三个规模的模型及推理代码。

**Result:** 在图像理解、推理、OCR、定位和计数等多个基准上，Zamba2-VL与Molmo2、Qwen3-VL、InternVL3.5等同等规模的Transformer开源模型表现相当，显著优于VL-Mamba、Cobra、mmMamba等先前基于SSM和混合架构的VLM。在同等参数规模下，Zamba2-VL的首令牌延迟（TTFT）相比Transformer基线降低约一个数量级，且效率优势在小规模（1.2B、2.7B）模型中最为突出。

**Conclusion:** Zamba2-VL证明了混合SSM-Transformer架构在大规模多模态任务上能够达到与纯Transformer架构竞争力相当的水平，同时获得显著的推理效率提升，这为在资源受限设备上部署高性能VLM提供了可行方向。其近线性的预填充计算和接近常数的循环状态大小使得TTFT大幅降低，尤其适合对实时性要求高的端侧和边缘应用场景。
...
### [14] [Rethinking Amortized Neural Representations for High-Resolution Terrain Elevation Data](https://arxiv.org/abs/2606.00404)
*Haoan Feng, Xin Xu, Leila De Floriani*

#### 🧩 TL;DR
针对地形高程数据领域，本文通过系统基准测试发现现有隐式神经表示方法存在跨域差距，并提出HUVR+SIREN超网络，通过替换解析可微平滑解码器，在无额外存储和更低解码成本下实现最佳高度与导数保真度，支持后训练量化压缩。

---

#### 📘 Detailed Summary
**Motivation:** 隐式神经表示（INR）将地形高程信号建模为连续坐标到值的函数，支持解析导数和任意分辨率解码，但为每个地形瓦片单独拟合并存储INR的方式在大规模地形数据集上不可扩展。现有的分摊神经表示方法（如超网络或短时逐瓦片优化）主要针对自然图像开发，其在地形高程场上的适用性尚不明确，存在显著的跨域研究空白。

**Method:** 本文在1 m/pixel地形数据集上建立了受控基准，统一评估了三种代表性方法。在观察到明显的跨域差距后，提出了HUVR+SIREN超网络：该方法基于基准中最强的HUVR方法，将其坐标解码器替换为平滑且解析可微的SIREN解码器，从而适配地形数据的连续光滑特性，无需额外逐瓦片存储且解码成本更低。

**Result:** HUVR+SIREN在基准测试中取得了最佳的高度场和导数保真度，且无额外逐瓦片存储开销，解码成本更低。该方法对激进的训练后量化表现出良好的容忍度，几乎无质量损失，从而形成紧凑的地形神经表示格式。消融实验和诊断分析进一步识别了哪些设计选择可迁移到地形领域，并表明逐瓦片瓶颈已接近其有用极限，剩余差距主要在于共享超网络的架构设计。

**Conclusion:** 研究表明自然图像领域的最优INR方法不能直接迁移到地形高程数据，通过针对性的解码器设计可有效弥合跨域差距。逐瓦片瓶颈已接近极限，进一步性能提升应聚焦于共享超网络的架构改进。HUVR+SIREN为大规模地形数据提供了一种紧凑、高效、高保真的神经表示方案。
...
### [15] [4D Radar Meets LiDAR and Camera: Cooperative Perception under Adverse Weather](https://arxiv.org/abs/2606.00416)
*Melih Yazgan, Iramm Hamdard, Qiyuan Wu, J. Marius Zoellner*

#### 🧩 TL;DR
本文提出将4D成像雷达作为鲁棒模态集成到协同感知中，并引入多普勒引导的空间注意力机制进行多智能体融合，在恶劣天气下显著提升自动驾驶协同感知的鲁棒性，同时发布了雷达增强的OPV2V-R和Adver-City-R基准数据集。

---

#### 📘 Detailed Summary
**Motivation:** 现有协同感知方法在恶劣天气（如雾、雨）下因摄像头和LiDAR性能退化而表现脆弱，缺乏能够适应全天气条件的鲁棒感知方案，亟需引入对天气不敏感的传感模态来弥补这一研究空白。

**Method:** 本文将4D成像雷达作为天气鲁棒模态集成到协同感知中，并提出多普勒引导的空间注意力机制用于多智能体融合；该方法扩展了两种代表性骨干架构：雷达-摄像头流水线（雷达替代LiDAR）和LiDAR-雷达流水线（雷达补充LiDAR）；同时发布了基于物理模拟LiDAR退化的雷达增强基准OPV2V-R和Adver-City-R。

**Result:** 在雾和雨条件下，该方法在两种骨干架构上均展现出显著的鲁棒性提升，尤其是当雷达替代退化的LiDAR时带来了实质性的性能改进；在MAN TruckScenes上的额外验证进一步证明了其超越仿真环境的可迁移性。

**Conclusion:** 4D成像雷达是一种适用于全天气条件下协同感知的鲁棒模态，能够有效弥补摄像头和LiDAR在恶劣环境下的退化问题，为自动驾驶感知系统的实用化部署提供了关键支撑。
...
### [16] [Detect Before You Leap: Mirage Detection in Vision-Language Models](https://arxiv.org/abs/2606.00435)
*Sayeed Shafayet Chowdhury, Md. Shaown Miah*

#### 🧩 TL;DR
提出TC-LIA方法，通过探测CLIP ViT视觉编码器各层的patch-token表征与问题嵌入的对齐轨迹，实现视觉语言模型在生成响应前对幻象（mirage）的预检测，在五个VQA域上达到约94.6-94.7%的三类检测准确率，并将幻象率控制在3%以下。

---

#### 📘 Detailed Summary
**Motivation:** 视觉语言模型（VLM）可能在缺乏相关视觉证据时仍产生高置信度的视觉回答，这种称为"幻象"（mirage）的失效模式在医学和文档视觉问答中尤为危险，因为看似合理的无依据响应可能被误认为基于图像证据。现有方法缺乏在VLM生成回答之前提前检测幻象的机制。

**Method:** 提出文本条件逐层内部对齐（TC-LIA）方法，该方法将CLIP ViT-H/14视觉编码器各层的图像patch token投影到最终CLIP嵌入空间，并计算其与问题嵌入的相似度，从而追踪各视觉层中是否出现与问题相关的视觉证据；所得对齐轨迹通过最终图像-文本余弦相似度、后期层top-k patch-文本对齐、早期到后期增益以及逐层斜率四个特征进行汇总，并结合像素统计空白/噪声检测、零样本域路由和结构化VLM自我评估构成集成系统。

**Result:** 在五个VQA域、三种输入条件和十二个VLM骨干网络上，最佳系统达到约94.6-94.7%的三类检测准确率，幻象率低于3%，而基线幻象率范围为21.7%至66.6%。

**Conclusion:** TC-LIA是一种模型无关的幻象预检测方法，能够有效识别VLM是否应该回答或弃权，在多个VQA域和多种VLM骨干上均能大幅降低幻象率，证明了通过探测视觉编码器内部对齐动态来检测视觉无依据响应的有效性。
...
### [17] [3D Segment Anything Model with Visual Mamba for Diagnosing Placenta Accreta Spectrum](https://arxiv.org/abs/2606.00489)
*Yuliang Zhang, Fang He, Lulu Peng, Tianyu Yan, Pingping Zhang, Ting Song, Lili Du, Dunjin Chen*

#### 🧩 TL;DR
本文提出了首个包含细粒度分割与分类标注的MRI胎盘植入谱（PAS）数据集，并设计了3DSAMba框架，通过结合3D SAM、适配器机制、Multi-Level Aggregation Mamba（MLAM）和Fusion State Space Model（FSSM）实现病灶区域分割，从而有效提升PAS自动诊断性能。

---

#### 📘 Detailed Summary
**Motivation:** 胎盘植入谱（PAS）是一种罕见但极其危险的产科疾病，早期准确诊断对孕产妇健康至关重要；传统诊断依赖经验丰富的医生分析剖宫产史和MRI数据，但基层医院缺乏相关专长和资源，亟需自动化诊断方法。

**Method:** 提出3DSAMba特征学习框架，首先设计3D版本的Segment Anything Model（SAM），并通过高效适配器机制融入医学领域知识；引入Multi-Level Aggregation Mamba（MLAM）聚合不同层级特征图，以及Fusion State Space Model（FSSM）融合编码器和解码器的多尺度特征；最终将分割掩码与原始MRI图像逐元素相乘，隔离病灶区域用于更准确的PAS诊断。

**Result:** 建立了首个MRI-based PAS数据集（包含细粒度分割与分类标注），并通过大量实验验证了所提框架显著提升了PAS诊断性能；数据集和源代码已开源。

**Conclusion:** 3DSAMba结合3D SAM、适配器机制及状态空间模型可有效实现PAS病灶分割与自动诊断，为基层医院提供可行的辅助诊断方案；开源数据集与代码将推动相关领域的进一步研究。
...
### [18] [OptiWorld: Optimal Control for Video World Generation under Physical Constraints](https://arxiv.org/abs/2606.00499)
*Yu Yuan, Jianhao Yuan, Xijun Wang, Daiqing Li, Liu He, Lu Ling, Stanley H. Chan*

#### 🧩 TL;DR
OptiWorld 是一个将经典最优控制引入视频生成推理阶段的框架，通过提取紧凑世界状态、在几何流形上规划最优轨迹并基于轨迹渲染视频，从而生成具有理想动力学特性的视频，适用于目标条件图像到视频生成、视频动力学编辑和反事实生成等任务。

---

#### 📘 Detailed Summary
**Motivation:** 现有视频生成模型虽然可作为可扩展的世界模型，但生成的运动大多仅追求视觉合理性，无法主动控制或优化底层动力学，导致物体轨迹存在不安全、不光滑、低效或物理不一致等问题。

**Method:** OptiWorld 框架在推理阶段引入经典最优控制，首先提取紧凑且任务相关的世界状态，然后在物理约束下规划最优轨迹，最后基于该轨迹条件化渲染视频。其中规划过程被形式化为连续流形上的几何问题，将 3D 几何与任务相关的物理约束统一为规划几何。

**Result:** OptiWorld 能够生成具有更优动力学的视频，并在目标条件图像到视频生成、视频动力学编辑以及反事实生成等多个任务中展现出强大的潜力。

**Conclusion:** 该研究证明，在视频生成模型中显式加入最优控制层能够有效改善生成视频的动力学质量，为将经典控制理论与生成式模型相结合提供了可行路径，并拓展了可控视频生成的应用场景。
...
### [19] [An Effective Solution for the CVPR 2026 8th UG2+ Challenge Track 3: Dynamic Object Segmentation in Turbulence](https://arxiv.org/abs/2606.00522)
*Hongzhen Li, Miao Yu, Leilei Cao, Youwei Pan, Yingfang Zhu, Fengjie Zhu*

#### 🧩 TL;DR
本文针对大气湍流下的动态目标分割任务，在SegAnyMo框架基础上提出数据中心域自适应和时空后处理模块，在UG2+挑战赛DOST赛道取得第二名。

---

#### 📘 Detailed Summary
**Motivation:** 大气湍流引起的严重几何畸变严重降低了视频中动态目标分割的性能，现有方法（如SegAnyMo）虽然具备强大的掩码生成和运动跟踪能力，但在复杂大气失真条件下难以鲁棒地分割目标。

**Method:** 基于SegAnyMo基线框架，本文提出两项改进：一是数据中心域自适应策略，通过引入DAVIS数据集子集和DOST数据集序列，并施加模拟大气波动退化来增强模型对几何畸变的鲁棒性；二是时空后处理模块，有效去除边界相连的假前景和短时碎片噪声，同时保留真实小目标和原始标签的时间一致性。

**Result:** 在CVPR 2026第八届UG2+挑战赛Track 3 (DOST)中，所提方法排名第二，验证了数据增强与时空后处理联合策略在湍流场景下动态目标分割的有效性。

**Conclusion:** 数据中心域自适应与时空后处理相结合能够显著提升模型在严重大气畸变下的分割鲁棒性，为真实场景中湍流退化下的视频目标分割提供了实用且可扩展的技术路径。
...
### [20] [ETC: Extreme Token Compression via Task-aware Visual Information Distillation in VLMs](https://arxiv.org/abs/2606.00543)
*Yiling Gao, Hongchen Wei, Zhenzhong Chen*

#### 🧩 TL;DR
提出极端令牌压缩（ETC）框架，基于变分信息蒸馏原理将输入视觉令牌压缩至极少数（甚至单令牌）以降低视觉语言模型推理时的KV缓存开销，实验表明在LLaVA-1.5-7B和Qwen3-VL-2B上压缩至单令牌仍能保持强任务性能。

---

#### 📘 Detailed Summary
**Motivation:** 视觉语言模型（VLM）在处理高分辨率图像时会产生大量视觉令牌，导致推理时计算成本和KV缓存开销极高，亟需一种能在极大压缩令牌数量的同时最小化任务性能损失的解决方案。

**Method:** 基于变分信息蒸馏原理，从信息论角度证明最小化任务损失要求紧凑表示保留任务相关视觉信息的指令感知充分统计量；实践中利用文本-图像交叉注意力对原始视觉特征加权以近似潜在指令感知预测统计量，并引入变分信息蒸馏使紧凑表示保留恢复该预测统计量所需的关键信息。

**Result:** 在LLaVA-1.5-7B和Qwen3-VL-2B上的实验表明，ETC即使在单令牌压缩的极端条件下仍然有效，大幅降低KV缓存开销的同时保持了较强的任务性能。

**Conclusion:** 通过信息论视角揭示了令牌压缩与任务损失之间的关系，证明基于指令感知充分统计量的变分信息蒸馏能够实现极端令牌压缩而不显著牺牲性能，为降低VLM推理成本提供了理论指导和实用框架。
...
### [21] [CAFOSat: A Strongly Annotated Dataset for Infrastructure-Aware CAFO Mapping Using High-Resolution Imagery](https://arxiv.org/abs/2606.00548)
*Oishee Bintey Hoque, Nibir Chandra Mandal, Mandy L Wilson, Samarth Swarup, Madhav Marathe, Abhijin Adiga*

#### 🧩 TL;DR
CAFOSat是一个面向美国全国范围的大型集中式动物养殖场（CAFO）遥感制图数据集，包含超过45,000张高分辨率NAIP影像 patches，覆盖20个州和四种主要CAFO类别，并通过人工在环的AI辅助标注、GradCAM定位和几何聚类方法将弱地理定位记录转化为精细的基础设施级标注，为农业环境监测提供了重要基准。

---

#### 📘 Detailed Summary
**Motivation:** CAFO在农业生产中具有重要作用，但同时也带来环境、公共卫生和疾病监测等方面的挑战。从遥感影像中大规模制图CAFO面临基础设施布局异质、地理位置记录噪声大、标注不一致以及清单不完整等多重困难，现有数据和方法难以支持可靠的大范围CAFO自动化监测。

**Method:** CAFOSat采用人工在环的标注流程，结合AI辅助标注、GradCAM定位和几何聚类方法，将多源弱地理定位记录转化为精细的基础设施级标注（包括畜棚、粪池和放牧相关特征）。通过土地覆盖引导采样和空间排除约束策略生成挑战性负样本，并引入合成数据增强管线以生成基础设施感知的变体，提升训练多样性和分布偏移下的鲁棒性。数据集构建基于高分辨率NAIP影像和多州CAFO清单。

**Result:** CAFOSat数据集包含超过45,000张影像patches，覆盖美国20个州和四种主要CAFO类别。作者对多种卷积神经网络、Transformer和视觉-语言模型进行了基准测试，验证了精细化标注和负样本筛选对CAFO分类和泛化性能的提升效果。合成增强管线进一步提升了模型在分布偏移下的鲁棒性。

**Conclusion:** CAFOSat为从高分辨率遥感影像中推进基础设施感知的农业监测和CAFO制图提供了大规模基准数据集。精细化标注和挑战性负样本的引入显著提升了CAFO分类和泛化能力，合成增强策略有助于应对数据分布偏移问题。该工作为环境监测和公共卫生领域的农业遥感应用提供了重要的数据和方法基础。
...
### [22] [DeepLatent: Think with Images via Parallel Latent Visual Reasoning](https://arxiv.org/abs/2606.00562)
*Dongchen Lu, Zhimo Li, Mao Shu, Huo Cao*

#### 🧩 TL;DR
DeepLatent 提出了一种用于潜在视觉推理的并行框架，通过 LatentFormer 生成上下文条件化的潜在状态，并结合连续空间强化学习优化潜在调制参数，在多个基准上取得了最先进的性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有"用图像思考"范式中的方法存在两类局限：工具辅助方法延迟高且操作类型受限，而潜在推理方法虽能隐式生成视觉状态但性能不如工具辅助方法，且潜在标记无法有效捕获视觉信息。

**Method:** 提出 DeepLatent 框架，包含两个核心组件：一是 LatentFormer，利用可学习的二维令牌并行生成上下文条件化的潜在状态，并将每次视觉更新直接锚定在原始图像特征上；二是连续空间强化学习算法，直接在嵌入空间中优化潜在调制参数以提升表示质量。整体训练采用知识蒸馏结合连续空间 RL 的流程，并构建了 DeepLatent-180K 大规模数据集。

**Result:** 在多个基准上的广泛评估表明，DeepLatent 达到了最先进的性能。

**Conclusion:** DeepLatent 通过并行潜在视觉推理框架和连续空间 RL 优化，有效弥合了工具辅助方法与潜在推理方法之间的性能差距，为视觉-语言模型中的隐性视觉推理提供了新的范式。
...
### [23] [Decomposed On-Policy Distillation for Vision-Language Reasoning: Steering Gradients for Visual Grounding](https://arxiv.org/abs/2606.00564)
*Hee Suk Yoon, Eunseop Yoon, Jaehyun Jang, SooHwan Eom, Ji Woo Hong, Mark Hasegawa-Johnson, Qi Dai, Chong Luo, Chang D. Yoo*

#### 🧩 TL;DR
本文通过将视觉-语言模型蒸馏损失分解为语言先验与视觉定位两个近乎正交的分量，揭示了标准蒸馏优化中存在的次优折衷问题，并提出视觉梯度引导（VGS）方法动态优先优化视觉子空间，在多个多模态蒸馏设置下显著提升推理性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有针对小规模推理模型的同策略蒸馏在多模态领域的优化动态尚未被充分探索，标准视觉-语言模型蒸馏将损失视为整体，忽略了语言先验与视觉定位两个目标在几何上的独立性，导致优化过程被动地在两个目标间寻求次优折衷，视觉定位作为多模态推理的主要瓶颈未能得到有效优化。

**Method:** 本文首先从数学上对同策略蒸馏损失进行分解，发现语言先验与视觉定位的梯度向量近乎正交；基于此发现提出视觉梯度引导（Visual Gradient Steering, VGS）方法，该方法通过动态重定向更新向量，优先沿视觉子空间方向进行优化，从而在不引入显著训练开销的前提下增强视觉定位能力。

**Result:** 在多个蒸馏设置和复杂多模态基准上的实验结果表明，VGS显著优于标准的整体式同策略蒸馏方法，在实现更优视觉定位效果的同时仅引入极小的训练额外开销。

**Conclusion:** 该研究揭示了多模态蒸馏中语言先验与视觉定位在优化几何上的独立性，指出视觉定位是视觉-语言推理的主要瓶颈；VGS提供了一种轻量且有效的解决方案，为未来多模态小模型蒸馏的优化策略设计提供了新的理论视角和实践方向。
...
### [24] [Response-Aware Multimodal Learning for Post-Treatment Visual Acuity Forecasting](https://arxiv.org/abs/2606.00588)
*Phuoc-Nguyen Bui, Van-Vi Vo, Duc-Tai Le, Van-Nguyen Pham, Ki-Young Kim, Seung-Young Yu, Hyunseung Choo*

#### 🧩 TL;DR
提出ReVA框架，利用基线和第1个月OCT影像及表格化临床变量，通过多模态融合与空间注意力机制，实现对糖尿病黄斑水肿抗VEGF治疗后3至24个月多时间点视力轨迹的预测，MAE=0.1246，R²=0.6064。

---

#### 📘 Detailed Summary
**Motivation:** 临床医生仅能依据早期治疗后的有限信息预估DME患者抗VEGF治疗的远期视力轨迹，而现有基于OCT的学习方法多聚焦于短期反应或单时间点预测，缺乏对多未来时间点视力轨迹进行建模的研究。

**Method:** 构建188例真实世界DME患者的基线与第1月配对OCT扫描及表格化数据；提出ReVA多模态框架，采用空间注意力机制保留OCT中局部预后影像特征，并设计依赖感知表格编码器建模临床变量间的交互，融合多模态表征后预测3、6、12、18、24个月的多时间点视力轨迹。

**Result:** 在24个月视力预测上达到MAE=0.1246、RMSE=0.1621、R²=0.6064，且在所有预测时间点上表现一致，证明了早期治疗反应信号可用于临床有意义的远期视力预测。

**Conclusion:** 纳入早期治疗反应信号能够支持临床有意义的远期视力轨迹预测，可为常规抗VEGF管理中的数据驱动决策支持提供依据。
...
### [25] [Through the PRISM: Principle-Aware, Interpretable, and Multi-Scale Evaluation of Visual Designs](https://arxiv.org/abs/2606.00592)
*Mona Gandhi, KJ Joseph, Srinivasan Parthasarathy, Sayan Nag*

#### 🧩 TL;DR
PRISM是一个包含10万+训练样本和1万验证样本的设计原则扰动基准，用于评估多模态模型对设计原则的细粒度理解能力；同时提出一个多尺度评估框架，结合轻量评分器、指令微调视觉语言模型和提示方法，实现可解释的设计质量评估与局部优化。

---

#### 📘 Detailed Summary
**Motivation:** 现有机器评估视觉设计的方法通常将多个设计原则（如可读性、对比度、对齐、重叠和连贯性）压缩为单一启发式分数，缺乏可解释性和诊断精度，无法像人类设计师那样对违背具体原则的问题进行细粒度定位和分析。

**Method:** 构建PRISM基准，通过对Crello数据集中专业布局沿可测量的设计原则进行系统扰动，生成10万训练样本和1万验证样本，每个样本隔离单一原则违规；提出多尺度评估框架，集成轻量评分器进行定量评估、指令微调视觉语言模型提供局部反馈、以及基于提示的方法进行全局推理。

**Result:** 实验发现Qwen-2.5-VL和GPT-4o-mini对特定原则退化高度不敏感，GPT-4o虽具备全局意识但无法进行细粒度解耦；基于框架获得的局部化洞察可实施有目标的布局改进，提升设计质量。

**Conclusion:** PRISM和多尺度评估框架为构建具备可解释设计素养的多模态推理系统奠定了基础，揭示了当前视觉语言模型在设计原则细粒度理解方面的局限性，并为可控的设计质量优化提供了新路径。
...
### [26] [ASAP: Advancing Medical Volumetric Representation Learning with Anatomy-aware Semantically-adaptive Pre-training](https://arxiv.org/abs/2606.00602)
...
#### 🧩 TL;DR
ASAP是一个面向胸部CT体扫描的解剖感知语义自适应视觉-语言预训练框架，通过注入器官级结构先验、动态语义-区域对齐和双模态掩码建模，在15个数据集和22项下游任务上取得了最先进性能，尤其在小样本和分布偏移场景下表现突出。

---

#### 📘 Detailed Summary
**Motivation:** 从医学体扫描中学习可迁移且可解释的表征面临两大挑战：复杂解剖结构带来的高维度异构性，以及放射学报告提供的弱监督信号仅包含全局语义而缺乏与局部体素区域的细粒度对应关系，现有视觉-语言预训练方法难以有效建模这种弱对齐关系，亟需一种能够融合解剖先验并实现语义感知的细粒度对齐框架。

**Method:** ASAP框架由三个核心模块组成：(1)解剖感知知识注入模块，利用现成分割工具引入器官级结构先验以鼓励解剖一致的表征学习；(2)语义自适应选择性对齐机制，动态地将句子级别的影像发现与对应的局部体扫描区域进行关联；(3)语义自适应融合模块，在双模态掩码建模范式下实现解剖信息引导的视觉特征与文本线索的高效交互，从而在预训练阶段实现细粒度的跨模态对齐。

**Result:** 在涵盖15个数据集和22个下游任务的综合基准评测中，ASAP在异常分类、分割、疾病预后预测、报告生成、词汇分类、跨模态检索和视觉问答等任务上均达到最先进性能，尤其在有限监督和分布偏移场景下增益显著，验证了其学习可迁移且临床有意义的体扫描表征的能力。

**Conclusion:** ASAP通过显式注入解剖先验和语义自适应对齐机制，有效弥补了医学体扫描视觉-语言预训练中全局弱监督与局部细粒度表征之间的鸿沟，为医学影像基础模型提供了一种可解释且鲁棒的预训练范式，其建立的标准化评测基准也为后续研究提供了系统的评估协议。
...
### [27] [FiSeR: Fine-Grained Source Representations for Cross-Domain AI Image Detection](https://arxiv.org/abs/2606.00606)
*Shan Zhang, Yongxin He, Mingming Zhang, Huiwen Tian, Lei Ma*

#### 🧩 TL;DR
提出一种分层对比学习框架FiSeR，在粗粒度和细粒度两个层次上分别优化自然/合成图像间和合成图像生成器间的对比目标，学习更具迁移性的特征表示，在跨域合成图像检测中平均AUROC提升10.22，少样本场景下提升10.64~17.41。

---

#### 📘 Detailed Summary
**Motivation:** 真实场景下的合成图像检测器在域迁移下泛化能力差，虽然无监督UMAP投影显示自然与合成特征在未见数据集上仍部分可分，但分类头过拟合训练域伪影导致性能下降，核心问题在于需要学习更可迁移的特征表示以稳定决策边界。

**Method:** 基于合成图像由多种生成器生成这一结构事实，提出分层对比学习框架FiSeR，联合优化两个对比目标：(i) 自然图像与合成图像之间的粗粒度对比目标；(ii) 利用生成器身份信息在合成图像内部的细粒度对比目标，以在提升自然/合成可分性的同时保留生成器身份信息。

**Result:** 在WildFake上训练后，跨域评估（Chameleon、AIGIBench、Community Forensics、GenImage）平均AUROC相较强基线DIRE提升10.22；少样本适应场景下，冻结骨干网络并在每类10个标注样本上拟合SVM分类头，在AIGIBench上AUROC提升10.64，在Chameleon上提升17.41（平均12种常用检测器）。

**Conclusion:** 通过分层对比学习同时利用类别级和生成器级监督信号，能有效学习对域迁移鲁棒的合成图像检测特征；冻结骨干加SVM的少样本适应策略进一步验证了所学特征表示的强迁移性和实用性。
...
### [28] [Pause and Think: A Dataset and Benchmark for Video-Grounded Assistive Action Suggestion](https://arxiv.org/abs/2606.00616)
*Shivam Singh, Saptarshi Majumdar, Pratik Prabhanjan, Zicheng Liu, Emad Barsoum*

#### 🧩 TL;DR
本文提出 pause-and-think-T 训练数据集和 4B 参数紧凑模型，通过结构化推理监督使模型在视频场景理解与目标规划任务中以 59 倍更少参数达到接近 235B 大模型的性能，并在多个分布外基准上展现强泛化能力。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉-语言模型在视频理解中普遍缺乏接地推理、时间一致性及上下文感知规划能力，难以生成人类级别的场景化辅助响应。研究者旨在通过推理监督而非大规模模型扩展来解决这一能力缺口。

**Method:** 构建 pause-and-think-T 推理中心训练数据集，引导模型在生成答案前先对视觉证据进行结构化推理；基于该数据集微调一个 4B 参数的紧凑模型，并在自建 pause-and-think-B 基准（关注上下文理解和目标规划任务）上评估。

**Result:** 4B 模型在 pause-and-think-B 基准上达到 58.0% 准确率，以 59 倍更少参数逼近 Qwen3-VL-235B 的 58.9%；场景理解能力匹配 GPT-5.2 并超越 GPT-4o；在 EgoThink 和 TempCompass 等分布外基准上，无需专门训练即可在 affordance、assistance、attribution recognition、situated reasoning 和 temporal order 等维度取得显著提升。

**Conclusion:** 针对性的推理监督能使紧凑模型在提供可操作、视觉接地的指导方面达到甚至超越大规模模型的水平，同时具备良好的泛化能力，表明推理数据质量比模型规模增长更为关键。
...
### [29] [A Systematic Benchmark of Intraoperative Ultrasound-to-MR Synthesis for Brain Tumour Surgery](https://arxiv.org/abs/2606.00630)
*Olga Esteban-Sinovas, Santiago Cepeda, Ignacio Arrese, Rosario Sarabia*

#### 🧩 TL;DR
本文首次对六种生成架构（四种GAN、ResViT、SynDiff）在四种推理模式（2D/2.5D/2D+3D-refinement/全3D）和两种目标（T2w/T2w+FLAIR）下进行系统基准测试，发现无单一架构全面占优，且感知质量LPIPS比SSIM更能反映下游分割效用，SynDiff-2.5D取得最佳下游性能。

---

#### 📘 Detailed Summary
**Motivation:** 术中超声（ioUS）在脑肿瘤手术中性价比高但解读困难——采集平面非标准、伪影特异、与术前MRI差异大，而现有手术规划工具、分割模型和外科医生经验均依赖MRI。合成MRI-like图像可复用MRI基础设施，但已有工作仅孤立评估单一架构，缺乏跨架构范式、推理模式和下游任务端到端指标的统一基准。

**Method:** 基于公共ReMIND数据集（76名患者，153对ioUS/T2w和104对ioUS/FLAIR），训练六种生成器：四种GAN基线（Pix2Pix、SwinPix2Pix、CycleGAN、CUT）、Transformer增强的ResViT和少步扩散模型SynDiff；每种在四种推理模式（2D、2.5D、2D+3D-refinement、全3D）和两种目标（仅T2w、T2w+FLAIR多任务）下训练，共48组实验。图像保真度采用SSIM、PSNR、MAE、LPIPS评估，下游分割任务用nnU-Net v2评估肿瘤和切除腔，并做组织学分级和再手术亚组分析。

**Result:** 无单一架构在所有指标上占优。关键发现：感知质量LPIPS与下游分割效用高度负相关（r=-0.66, p<0.001），而SSIM与下游效用负相关（r=-0.64, p<0.001），即更高SSIM反而对应更差分割性能；SynDiff-2.5D在下游分割任务中取得最佳Dice（U_Dice=0.55）。

**Conclusion:** 感知（LPIPS）和下游任务指标应被报告或优先于全局SSIM；架构选择应依据手术阶段、患者病史和临床目标进行条件化决策。
...
### [30] [An Attribute-Based Measure of Video Complexity](https://arxiv.org/abs/2606.00640)
...
#### 🧩 TL;DR
提出视频属性复杂度（VideoABC）框架，通过非参数化方式基于视频属性量化视频-问题对对于视频大语言模型的复杂度，并利用k-means与通用格量化器结合及合成数据生成方法实现高精度与泛化能力。

---

#### 📘 Detailed Summary
**Motivation:** 现有方法缺乏系统化、可解释且通用的框架来估计视频-问题对给视频大语言模型带来的复杂度，现有方法如“video-LLM as judge”复杂度高且缺乏属性层面的可解释性。

**Method:** VideoABC定义复杂度为视频-LLM对给定视频-问题对的失败概率；通过参考视频数据集将其投影到预定义的视频属性空间并量化；结合k-means量化器（保证分布内样本精度）与通用格量化器（保证分布外样本泛化）；利用受心理物理学启发合成视频生成过程填充格量化器单元以计算期望ABC。

**Result:** 实验表明VideoABC在极低维属性表示下依然有效，性能显著优于“video-LLM as judge”等方法且复杂度更低；其可解释性能够通过属性组成揭示基准数据集的复杂度特征。

**Conclusion:** VideoABC提供了一种可解释、低复杂度且泛化能力强的视频-问题复杂度度量方法，为评估和理解视频-LLM在不同属性构成下的表现提供了新视角与实用工具。
...
### [31] [Collaborative Few-Step Distillation and Low-Bit Quantization for Wan2.2 Dual-Expert Video Diffusion Models](https://arxiv.org/abs/2606.00658)
*Jinyang Du, Shenghao Jin, Ziqian Xu, Ruihao Gong, Shiqiao Gu, Yang Yong, Jinyang Guo, Xianglong Liu*

#### 🧩 TL;DR
提出一种面向部署的压缩流程，将少步分布匹配蒸馏与低比特量化相结合，在Wan2.2-T2V-A14B视频扩散模型上实现高效推理，在20步设置下量化模型超越原始全精度基线。

---

#### 📘 Detailed Summary
**Motivation:** 大型视频扩散模型在采样时需要大量去噪步骤和大参数驻留内存，导致部署成本高昂，如何在保持生成质量的同时显著降低计算和存储开销是当前面临的主要挑战。

**Method:** 采用少步分布匹配蒸馏与低比特量化协同设计的压缩流程：沿模型的双专家去噪路径分别校准高噪声和低噪声分支，保护敏感的入口层，并引入HiF4风格的低比特表示以提升动态范围覆盖；量化在校馏后的少步学生模型上进行而非原始长步轨迹，以减少推理时的激活分布失配。

**Result:** 协同设计的量化模型在相同步数下接近全精度模型，并在8步和20步设置下平均超越原始全精度基线；20步设置在测试配置中实现了最佳的质量-效率权衡。

**Conclusion:** 该工作表明蒸馏与量化的联合设计能够有效压缩大型视频扩散模型并同时提升性能，为视频扩散模型的高效部署提供了可行的技术路线。
...
### [32] [TAP-JEPA: Frozen Future-Latent Probing and Two-Stage Score Fusion for EPIC-KITCHENS-100 Action Anticipation](https://arxiv.org/abs/2606.00662)
*Chaoyang Wang, Lexuan Xu*

#### 🧩 TL;DR
TAP-JEPA 提出了一种基于冻结 V-JEPA 2.1 特征的紧凑动作预期模型，通过预训练潜在预测器估计近未来令牌并采用双阶段分数融合，在 EK-100 动作预期挑战中以 27.91% MT5R 获得第二名。

---

#### 📘 Detailed Summary
**Motivation:** 现有动作预期方法通常需要微调大型视频主干网络，计算成本高且难以部署；如何在无需微调大型模型的前提下，利用预训练视觉特征高效构建紧凑的动作预期模型是一个关键问题。

**Method:** TAP-JEPA 基于冻结的 V-JEPA 2.1 ViT-G/384 编码器提取可见预动作令牌，利用预训练的潜在预测器从观测上下文估计近未来令牌，然后将两组令牌通过带有任务特定查询（动词、名词、动作对）的注意力探针进行融合；训练时采用官方训练集和大部分验证集，并实施两阶段分数融合：先平均每个 epoch 内八个独立初始化探针副本的输出，再对 epoch 12-20 的候选结果按字段依赖权重合并。

**Result:** 在 EPIC-KITCHENS-100 动作预期挑战赛中，TAP-JEPA 在官方开放测试排行榜上以整体动作 Mean Top-5 Recall 27.91% 排名第二，仅落后第一名 0.04 个百分点。

**Conclusion:** 该工作表明，基于冻结的大规模视频预训练特征结合轻量级预测与融合机制，能够在无需微调大型主干网络的情况下取得极具竞争力的动作预期性能，为构建高效实用的视频理解模型提供了可行方向。
...
### [33] [T-CLIP: Enabling Thermal Perception for Contrastive Language-Image Pretraining](https://arxiv.org/abs/2606.00673)
*Tayeba Qazi, Ayush Maheshwari, Prerana Mukherjee, Brejesh Lall*

#### 🧩 TL;DR
本文提出 IR-Cap 热成像描述数据集与 T-CLIP 解耦双 LoRA 框架，分别从场景级和物体级适应 CLIP 以弥合热红外与可见光视觉语言模型之间的感知鸿沟，在跨模态检索等任务上取得一致提升。

---

#### 📘 Detailed Summary
**Motivation:** 热成像在低光照与恶劣天气下具有优势，但现有视觉语言模型如 CLIP 无法对齐热红外图像与文本描述，原因在于缺乏带标注的热成像数据集、标准 LLM 无法推理热物理现象，以及全局场景上下文与物体级热信号在同一嵌入空间中难以共存的核心表征冲突。

**Method:** 本文提出 IR-Cap 物理感知热成像描述流水线与数据集，提供跨三个公开基准的全局与细粒度热描述；同时提出 T-CLIP，一种解耦双 LoRA 框架，独立地针对场景级和物体级热理解分别适配 CLIP，从而避免单一嵌入空间中的表征冲突。

**Result:** T-CLIP 在三个热成像基准的跨模态检索任务上相较于所有基线方法取得一致提升，并在文本条件热图像生成任务上展示了初步的应用可行性。

**Conclusion:** 本文通过物理感知数据构建与解耦双 LoRA 适配框架，系统性地弥合了热红外与可见光视觉语言模型间的感知鸿沟，为热成像在跨模态理解与生成任务中的应用提供了有效方案。
...
### [34] [FROST-STA: Frozen Dense Features for the Ego4D Short-Term Object Interaction Anticipation](https://arxiv.org/abs/2606.00694)
*Chaoyang Wang, Lexuan Xu*

#### 🧩 TL;DR
FROST-STA 基于 V-JEPA 2.1 的冻结 ViT-G 骨干，通过对象中心解码、多头预测和集成训练策略，在 Ego4D STA 挑战赛中以 5.13 Overall Top-5 mAP 取得第二名，证明了冻结密集图像-视频特征可作为物体级交互预测的强基础。

---

#### 📘 Detailed Summary
**Motivation:** 第一人称视频中的短期交互预测不仅需要识别当前场景，还需推断佩戴者将接触哪个物体、执行何种动作以及接触何时发生，现有方法在结构化预测多模态输出（目标框、名词、动词、接触时间）方面仍有挑战。

**Method:** FROST-STA 冻结 V-JEPA 2.1 的 ViT-G 骨干，提取稀疏视频令牌（384分辨率短片段）和密集图像令牌（最后高分辨率帧），通过包含注意力探针和帧引导时序池化的对齐模块将视频表示映射到末帧空间参考上，再与图像特征融合，最后用 Faster R-CNN 风格的 STA 头部解码框偏移、名词、动词、接触时间和交互质量；训练 25 轮，集成 8 个头部和 15-25 轮的检查点。

**Result:** FROST-STA 在官方测试集上取得 5.13 Overall Top-5 mAP，在 EgoVis 2026 Ego4D STA 挑战赛中位列第二。

**Conclusion:** 该工作表明，冻结的密集图像-视频特征通过合理的对齐与解码设计，能够为物体级交互预测提供强基础，为后续在无需大规模视频微调的情况下提升第一人称预测性能提供了可行性方向。
...
### [35] [CASTLE2026 Team WDL Technical Report](https://arxiv.org/abs/2606.00712)
*Zhengyang Li, Zhenglin Du, Yi Wen, Fang Liu, Shuo Li, Xu Liu*

#### 🧩 TL;DR
本文提出基于Qwen的感知增强多模态推理流水线，在CASTLE Challenge @ EgoVis 2026的长时程自我中心视频问答任务中取得第一名，通过提示解析、ASR检索、辅助图像附着、候选帧采样及置信度加权投票等模块，将LoRA得分从0.21提升至0.58。

---

#### 📘 Detailed Summary
**Motivation:** 长时程自我中心视频问答涉及超过600小时的多视角录制，每个四选一问题需要同时从视频、文本、辅助照片、人物、日期、房间及时间上下文等多源异构证据中进行推理，现有方法难以有效整合如此多样化的信息并实现高精度作答。

**Method:** 本文设计了一套基于Qwen的感知增强多模态推理流水线：首先解析问题中的提示线索，检索相关的ASR语音片段并附着辅助图像；然后对候选视频帧进行采样，将问题路由至静态视觉、语音/文本、时空及混合四种类型，每种类型使用专门的提示模板；最后通过多次推理的置信度加权投票进行结果聚合，并转换为Codabench官方提交格式。

**Result:** 在消融实验中，使用LoRA微调将得分从0.21提升至0.50，增加采样帧数后进一步将得分提升至0.58。最终系统在CASTLE Challenge @ EgoVis 2026中排名第一。

**Conclusion:** 该工作表明，通过精细化的多模态信息路由和融合策略，结合LoRA微调与更多帧采样，能够显著提升长时程自我中心视频问答的性能，为复杂时空多模态推理任务提供了有效的技术路径。
...
### [36] [Head-Pose-Aware Visual Speech Recognition with FiLM Modulation](https://arxiv.org/abs/2606.00751)
*Matthew Kit Khinn Teng, Haibo Zhang, Takeshi Saitoh*

#### 🧩 TL;DR
提出HP-VSR-ResFiLM，一种显式融入头部姿态信息的视觉语音识别框架，通过在视觉编码器中引入姿态条件残差FiLM模块，在不依赖额外训练数据的情况下，在LRS2和LRS3上分别取得25.0%和33.2%的词错误率，有效提升了非正面视角下的VSR鲁棒性。

---

#### 📘 Detailed Summary
**Motivation:** 视觉语音识别（VSR）性能受到视素模糊和姿态变化引入的几何畸变与遮挡的根本限制，现有方法主要依赖语言上下文或隐式不变性学习，未能显式处理头部姿态变化导致的视觉表征退化问题，在非正面视角下视觉表征的鲁棒性严重不足。

**Method:** 提出HP-VSR-ResFiLM框架，采用两阶段流水线：阶段一构建姿态条件视觉编码器，在2D CNN前端之后引入姿态条件残差特征线性调制（FiLM）模块，利用头部姿态信息自适应地细化视觉特征；阶段二使用预训练的NLLB语言模型进行音素到文本的重建。

**Result:** 在LRS2和LRS3数据集上，HP-VSR-ResFiLM分别取得25.0%和33.2%的词错误率，在可比训练条件下达到竞争性性能且无需额外训练数据；消融实验表明，单个残差FiLM模块即可持续改善整体WER，在Layer 3和Layer 4处进行更深层调制对偏航角大于30°的样本带来更大增益且不降低小姿态变化样本的性能。

**Conclusion:** 显式姿态感知特征调制为提升非约束场景下VSR鲁棒性提供了一种有效且计算高效的解决方案，证明了将头部姿态信息以条件调制方式显式融入视觉特征提取是一条优于隐式不变性学习的可行路径。
...
### [37] [FlowOVD: Learning Generative Latent Flows for Zero-shot Open-vocabulary Detection](https://arxiv.org/abs/2606.00782)
*Yao Wei, Andrea Cavallaro, Changjae Oh*

#### 🧩 TL;DR
FlowOVD提出一种基于整流流（rectified flow）的文本条件查询生成框架，将开放词汇目标检测中的解码器查询生成建模为连续传输过程，在无需额外训练数据的情况下，在COCO和LVIS上分别达到49.5 AP和31.5 AP，显著优于GroundingDINO。

---

#### 📘 Detailed Summary
**Motivation:** 现有开放词汇目标检测方法通常将问题建模为判别式预测，解码器查询要么是静态的，要么从编码器特征初始化，导致查询的多样性和灵活性受限；本文旨在克服这种启发式离散查询构造的局限性，通过引入连续潜空间传输过程实现更灵活、更具表现力的查询生成。

**Method:** 本文提出FlowOVD，一种基于整流流（rectified flow）的文本条件查询生成框架，将解码器查询生成建模为潜空间中的连续传输过程，逐步将文本无关的查询转换为文本引导的查询；通过将连续潜查询动力学引入基于视觉-语言模型（VLM）的检测器中，实现了更具表达力的语义对齐，避免了启发式离散查询构造。

**Result:** 在不使用额外训练数据的情况下，FlowOVD在COCO上达到49.5 AP，在更具挑战性的长尾分布LVIS基准上达到31.5 AP，分别比GroundingDINO高出1.2 AP（+2.5%）和4.1 AP（+15.0%），在长尾分布场景中优势尤为显著。

**Conclusion:** 连续查询生成机制能够有效提升开放词汇检测的泛化能力，尤其是在长尾分布和小样本类别上表现突出；这表明将生成式视角引入检测器查询设计是一条有前景的方向，为开放词汇检测提供了新的技术范式。
...
### [38] [MBench: A Comprehensive Benchmark on Memory Capability for Video World Models](https://arxiv.org/abs/2606.00793)
...
#### 🧩 TL;DR
提出了MBench基准，系统性地从三个层次（实体一致性、环境一致性、因果一致性）分解为12个可量化子维度，用于评估视频世界模型的长期记忆能力，揭示现有主流模型在长时间状态保持方面的系统性局限。

---

#### 📘 Detailed Summary
**Motivation:** 现有视频世界模型虽能合成高保真视觉序列，但存在视觉生成与功能性世界模型要求之间的根本差距，特别是在长时间跨度下维持稳定合理内部状态的能力。现有基准主要关注视觉质量、运动一致性和文本-视频对齐，忽略了世界模型最核心的长期记忆能力。

**Method:** 提出MBench基准，将视频世界模型的记忆能力系统地分解为三个层次化的核心维度：实体一致性、环境一致性和因果一致性，进一步细化为12个可量化子维度。基准基于精心筛选的真实长视频构建，采用基于规则的定量评估矩阵和视觉语言模型（VLM）进行客观全面的一致性评估。

**Result:** 对主流最先进的视频世界模型进行了广泛评估，揭示了现有方法在长期状态保持方面存在关键的系统性局限，为领域提供了标准化的基准和明确的研究方向。

**Conclusion:** MBench填补了视频世界模型记忆能力评估的空白，提供了一个系统化、多维度的标准化基准框架，明确了当前模型在长期一致性维持方面的不足，为未来研发具有真正世界模型能力的方向提供了指导。
...
### [39] [SuperMemory-VQA: An Egocentric Visual Question-Answering Benchmark for Long-Horizon Memory](https://arxiv.org/abs/2606.00825)
*Samiul Alam, Shakhrul Iman Siam, Michael J. Proulx, James Fort, Richard Newcombe, Hyo Jin Kim, Mi Zhang*

#### 🧩 TL;DR
提出SuperMemory-VQA，一个基于AI眼镜的长时程第一人称视频问答数据集，涵盖52.9小时日常活动和4853个可回答/不可回答的问答对，用于评估AI助手的真实记忆任务；基准测试表明现有agent系统在现实记忆任务上远不可靠，凸显了对新的接地记忆架构的需求。

---

#### 📘 Detailed Summary
**Motivation:** 现有第一人称视频数据集主要聚焦于短片段上的动作识别或通用问答，衡量的是感知能力而非人类真实的记忆需求；AI眼镜作为个性化记忆助手的应用需要系统具备超越短期视频理解、在纵向第一人称视频流中填补人类记忆空白的能力，而这一研究缺口尚未被有效填补。

**Method:** 构建了包含52.9小时日常活动的第一人称视频数据集，同步采集RGB视频、音频转录、眼动追踪、IMU和SLAM轨迹等多模态数据；通过人工验证的标注流程构建了4853个多选题问答对，涵盖物体与位置记忆、意图回忆、视觉场景回忆、时间线重建、对话记忆和上下文检索六类任务，每个问题包含明确的“不可回答”选项以测试幻觉鲁棒性；对主流agent框架和大语言模型骨干进行系统性基准测试。

**Result:** 基准测试表明，现有agent框架和大语言模型在真实世界记忆任务上远未达到可靠水平，特别是在需要长期依赖和证据充分性判断的场景中表现不佳；参与者调查进一步验证了所提问题与日常记忆需求的真实性和实用性。

**Conclusion:** 现有AI系统在真实、长程的记忆任务上仍存在显著不足，需要发展新的接地AI记忆架构，使系统仅在证据充分时才能做出回答，从而避免幻觉；该数据集为未来研究提供了标准化的评估基准和明确的发展方向。
...
### [40] [RoboStressBench: Benchmarking VLM Robustness to Physical Visual Stress in Embodied Scenes](https://arxiv.org/abs/2606.00828)
...
#### 🧩 TL;DR
RoboStressBench 是一个基于逆图形学视角构建的基准，用于系统评估视觉-语言模型在物理视觉压力下的鲁棒性，通过将视觉压力分解为材质、视角、光照和几何四个物理维度，揭示了不同压力因素对 VLM 不同能力的差异化退化模式，并提出了一个压力感知 agentic solver 来提升高压力场景下的鲁棒性。

---

#### 📘 Detailed Summary
**Motivation:** 现有 VLM 基准仅使用干净图像或孤立扰动进行评估，无法覆盖现实物理场景中多样化的视觉应力因素，缺乏一种能够从物理场景形成原理出发、系统定义并测试视觉应力的原则性评估框架。

**Method:** 从逆图形学角度公式化视觉感知问题，受物理渲染方程启发，将视觉应力分解为材质（M）、视角（V）、光照（L）和几何（G）四个物理维度，构建 RoboStressBench 基准；并提出一个压力感知 agentic solver，通过检测视觉压力源并在推理前调用视觉编辑技能来增强鲁棒性。

**Result:** 对多个 SOTA VLM 的全面评估揭示了应力特定的失败模式，不同物理因素对不同具身能力（如视觉识别、推理和规划）产生差异化退化，这些退化模式在聚合准确率中往往被掩盖。

**Conclusion:** RoboStressBench 提供了一个原则性的评估框架，能够系统诊断和改善 VLM 在真实世界物理压力下的感知能力，为开发更可靠的具身 AI 系统提供支持；压力感知 agentic solver 在高压力场景下展现了鲁棒性改进效果。
...
### [41] [The Right Inference Strategy Is All You Need: Nearly Training-Free Domain-Wise Inference for EgoCross Challenge](https://arxiv.org/abs/2606.00829)
*Leyi Wu, Yifan Zhao, Jinjie Zhang, Yinchuan Li, Ying-Cong Chen*

#### 🧩 TL;DR
EgoCross 提出了一种领域感知推理策略，在仅20个训练样本和固定Qwen3-VL-4B基座模型的源受限设定下，通过为手术、工业组装、极限运动及动物佩戴相机四个罕见第一人称视频域分别设计不同的输入、提示和答案映射策略，实现了66.98%的整体准确率，揭示了冻结基座模型的知识转移瓶颈在于缺乏合适的接口而非模型能力不足。

---

#### 📘 Detailed Summary
**Motivation:** 现有多模态大语言模型在日常生活场景的第一人称视频问答中表现良好，但在面临手术、工业组装、极限运动和动物佩戴相机等存在显著域迁移的罕见第一人称场景时性能大幅下降，且源受限设定下仅有20个训练样本，模型被固定为Qwen3-VL-4B，因此亟需探索如何在极有限数据下挖掘冻结基座模型中已有的多模态知识以适应新领域。

**Method:** 提出领域感知推理策略，将四个目标域（手术、工业、极限运动、动物佩戴相机）分开处理，根据各域任务特点分别设计不同的输入预处理、提示词构造和答案映射流程；其中手术和动物域直接使用冻结的Qwen3-VL-4B基座模型，极限运动和工业域仅使用在20个训练样本上微调两轮得到的官方SFT检查点；整个系统几乎无需额外训练。

**Result:** 在最终评测中，该领域感知推理策略在源受限赛道取得66.98%的整体准确率，证明即使基座模型能力受限，精心设计的领域感知推理流程也能有效恢复基座模型本身已具备的视觉语言知识。

**Conclusion:** 核心洞见在于冻结的基线模型并非缺乏处理罕见场景的能力，而是在没有合适接口的情况下难以将已有的视觉语言知识迁移到新的任务格式中；领域感知的推理策略通过针对性地强化各域关键线索，使罕见的第一人称场景对多模态大语言模型更具可解释性，在极小训练样本下仍能取得可观性能。
...
### [42] [Benchmarks for Vision-Language Models in Urban Perception Should Be Reliability-Aware and Negotiated](https://arxiv.org/abs/2606.00871)
*Rashid Mushkani*

#### 🧩 TL;DR
本文论证了在面向城市治理的街道场景感知任务中，应将标注者的分歧与弃权视为有效测量结果，在基准测试中同时报告人类间信度与模型对齐度，并将标签空间与评分策略视为可协商的设计产物。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉-语言模型在街道场景结构化描述任务中，通常将人类标注视为单一真实值，忽略了感知任务中普遍存在的标注者分歧、弃权（Not applicable）等不确定性，且缺乏对标注空间与评分策略选择如何影响模型评估的反思，无法满足城市治理场景对评估透明性和可问责性的需求。

**Method:** 基于100张蒙特利尔街景、7个社区组织12名参与者沿30个维度标注的数据集，采用确定性零样本评估方法对7种视觉-语言模型进行评测，分析模型与人类共识的一致性、人类间信度与模型对齐度的协变关系，并比较模型与标注者在Overall Impression维度上的分布差异，包括Not applicable比例的差异。

**Result:** 实验结果表明，模型与人类共识的一致性程度与维度层面的人类间信度存在协变关系；在Overall Impression这一评价维度上，模型与标注者的评分分布存在显著错配，包括Not applicable比例的不同。

**Conclusion:** 研究建议基准创建者、模型开发者和机构应将不确定性和基准假设在评估报告中明确可视化，包括将分歧与弃权作为测量结果、报告人类间信度、以及将标签空间与评分政策视为可协商的设计选择。
...
### [43] [MMDG-Bench: A Benchmark for Multimodal Domain Generalization](https://arxiv.org/abs/2606.00891)
*Qianshan Zhan, Qian Wang, Da Li, Xiao-Jun Zeng, Xiatian Zhu*

#### 🧩 TL;DR
MMDG-Bench是首个面向多模态域泛化的系统化基准，通过提出D2M和M2D两种框架组合范式，在多种任务和十种基线方法上统一评估，揭示了DG技术与MML结合的显著优势，并给出了最优框架选择与骨干网络设计的关键准则。

---

#### 📘 Detailed Summary
**Motivation:** 多模态学习与域泛化作为两个独立领域均已取得长足进展，但二者的系统性集成（即多模态域泛化）仍缺乏深入探索，现有研究局限于动作识别任务且缺少标准化评估协议，亟需统一基准来推动该方向的规范研究。

**Method:** 提出了MMDG-Bench基准，定义了两种基础框架：先域泛化后多模态学习（D2M）和先多模态学习后域泛化（M2D）；通过将统一的多模态学习配置与五种域泛化技术配对，在两种框架顺序下实例化了十种MMDG基线方法，并在视频-音频-光流动作识别和RGB-深度-红外人脸防伪等多样任务上建立了统一实验协议。

**Result:** 实验表明，这些结构化组合方法在多个任务上持续超越现有最优方法；DG技术的集成在不同骨干网络上均带来一致的泛化增益，而非DG方法对骨干网络更换高度敏感；D2M在模态关系跨域稳定时更优，M2D在模态关系跨域变化大时更具鲁棒性；更强的骨干网络在结构化框架中能放大性能收益。

**Conclusion:** 该研究通过系统性基准实验揭示了多模态域泛化的三大关键洞察，为后续研究提供了原则性的基础框架和可操作的设计指导，推动多模态鲁棒性研究迈向标准化与范式化的新阶段。
...
### [44] [Reason, Retrieve, Re-rank: A Zero-Shot Reasoning-Aware Framework for Composed Video Retrieval](https://arxiv.org/abs/2606.00910)
*Ali Alavi*

#### 🧩 TL;DR
R3-CoVR是一个完全无需训练的零样本组合视频检索管道，通过冻结的基础模型（Qwen3-VL-8B进行推理和重排序，SigLIP-2进行嵌入检索）实现，在CVPR 2026 VidLLMs研讨会的CoVR-R挑战中取得了91.9%的R@1和98.2%的R@10，其中约束感知重排序带来了最大的性能提升。

---

#### 📘 Detailed Summary
**Motivation:** 组合视频检索（CoVR）要求根据对参考视频的自由形式文本修改来检索目标视频，现有方法在零样本设定下难以有效理解编辑操作隐含的状态变化、动作阶段、场景、镜头和节奏等语义信息，因此需要一种无需训练且能进行推理感知的检索方案。

**Method:** R3-CoVR是一个三阶段无需训练的管道，由冻结的基础模型组成：首先使用多模态大语言模型（Qwen3-VL-8B）推理编辑操作的后效，生成简洁的编辑后描述；然后使用对比式视频-文本编码器（SigLIP-2）对该描述和候选库进行第一阶段的嵌入检索；最后使用相同的多模态大语言模型作为评判器，对候选列表进行约束感知的重排序，评估每个候选与预期编辑结果的一致性。

**Result:** 在挑战测试集上，R3-CoVR达到91.9%的R@1和98.2%的R@10。两个关键发现：将描述长度匹配到对比编码器的文本窗口使R@1从67.5提升到72.7；约束感知重排序仅对候选列表进行重排，使R@1从72.7提升到91.9，这是最大的单一增益。

**Conclusion:** 研究揭示了两个关键洞察：描述长度与对比编码器文本窗口的匹配至关重要；约束感知重排序在短列表上带来了最大的性能提升，表明推理能力的引入能显著提升零样本组合视频检索的效果。研究还分析了重排序器的行为、检索与重排序的混合策略以及候选列表深度，并开源了简洁的三层实现。
...
### [45] [Single-Channel Tissue Segmentation via Cross-Modal Distillation from Foundation Models](https://arxiv.org/abs/2606.00928)
*Sakib Mohammad, Jarin Ritu, Md Sakhawat Hossain*

#### 🧩 TL;DR
提出一种跨模态知识蒸馏框架，利用冻结的SAM ViT-H基础模型教师处理多通道荧光显微图像，将语义信息蒸馏至仅使用核通道的轻量级U-Net学生模型，在TissueNet上实现23倍参数量缩减下恢复教师87.9%性能，学生Dice提升约12个点。

---

#### 📘 Detailed Summary
**Motivation:** 多通道荧光显微成像虽能显著提升组织分割精度，但现有复用模型在推理时要求所有通道齐备，限制了其在仅部分通道可用场景下的部署；研究旨在解决多通道模型对输入模态的刚性依赖，实现仅核通道下的高性能分割。

**Method:** 提出跨模态知识蒸馏框架，以冻结的SAM ViT-H或CellSAM作为教师处理多通道（DAPI与E-cadherin）输入，以Swin-Tiny、ResNet18、EfficientNet-B0或MobileNetV3为骨架的U-Net作为学生仅处理核通道；蒸馏损失由MSE概率匹配、边界感知监督和可学习不确定性加权三部分组合而成。

**Result:** 在TissueNet上，SAM蒸馏的Swin-Tiny学生达到Dice 78.36，较无KD基线（65.31）提升13.05个点，以23倍参数量恢复教师oracle性能（89.12）的87.9%；四种学生架构均一致提升约12个Dice点，证实蒸馏的架构无关性；SAM ViT-H教师优于CellSAM；在BBBC038跨数据集评估中无需重新训练教师即取得一致增益。

**Conclusion:** 跨模态知识蒸馏能有效将多通道基础模型的语义知识迁移至单通道轻量学生，实现大幅参数压缩下接近教师性能的分割精度；该范式具有架构通用性和跨数据集泛化能力，为实际场景中仅可获取有限荧光通道的显微图像分析提供了实用解决方案。
...
### [46] [CV-Arena: An Open Benchmark for Instructional Computer Vision Problem Solving with Human-AI Collaborative Preferences](https://arxiv.org/abs/2606.00931)
...
#### 🧩 TL;DR
本文提出了CV-Arena，一个包含12K高分辨率真实图像指令对、覆盖16种视觉任务类型的指令驱动图像编辑基准，并引入Active Elo协议和CV-Judge评估器以高效、保真地评测模型；在21个系统上的评估揭示了当前模型在指令遵循、物理推理和细节保持方面的持续不足，而所提出的轻量级智能体模型CV-Agent展示了闭环推理在专业级编辑中的潜力。

---

#### 📘 Detailed Summary
**Motivation:** 现有的指令驱动图像编辑基准大多局限于狭义的表面外观编辑，无法全面反映专业工作流中真实图像任务的多样性。本文旨在填补这一空白，将指令驱动的计算机视觉问题求解定义为更广义的图像编辑形式，并构建一个覆盖多种约束（如保留性、几何、物理和可用性）的大规模、高保真基准，以系统评估模型在专业场景下的能力。

**Method:** 本文构建了CV-Arena基准，包含12K高分辨率真实图像指令对，覆盖16种指令型视觉任务类型，数据通过双轨检索与筛选管道CogRetriever（结合目标化网络搜索、智能体查询精化、验证和可追溯性）生成。为平衡大规模评估与人类保真度，提出了Active Elo协议，利用逻辑门控的多维VLM评估器CV-Judge拒绝明显失败样本并解决高置信度比较，将难分样本路由给专家评审员，通过可靠性加权的Elo更新聚合人类与AI的混合监督。此外，开发了轻量级智能体模型CV-Agent，整合规划、编辑和验证的闭环推理流程。

**Result:** 在CV-Arena上对21个系统（包括专有、开源和智能体模型）的全面评估表明，当前模型在指令遵循、物理推理、结构控制和细粒度细节保持方面仍存在持续不足。CV-Agent作为轻量级智能体模型，通过闭环推理在专业级指令遵循型视觉编辑任务上展示了有前景的性能提升。

**Conclusion:** 本研究揭示了现有图像编辑模型在专业级指令理解与多约束满足方面的显著差距，证明了闭环推理（规划-编辑-验证）是提升指令驱动视觉编辑性能的有效方向，并为该领域提供了一个开放、可扩展的基准评估框架。
...
### [47] [Boundary-Protection W8A8 HiFloat8 Quantization for Large-Scale Text-to-Video Diffusion Transformers](https://arxiv.org/abs/2606.00957)
*Yiming Zhao*

#### 🧩 TL;DR
提出了一种针对140亿参数文生视频DiT模型Wan2.1-T2V-14B的后训练量化方法，通过边界保护策略（前2后3块保留BF16、其余35块量化至W8A8 HiF8）在Ascend 910B NPU上实现无损量化，在VBench五个维度上持平或超越BF16基线。

---

#### 📘 Detailed Summary
**Motivation:** 视频DiT模型量化面临的核心挑战在于Transformer块的激活分布存在异质性：边界块（前几个和最后几个块）与中间块的统计特性根本不同，导致均匀量化策略失效，亟需针对性的非均匀量化方案以在不损失精度的情况下实现模型压缩。

**Method:** 对所有40个WanAttentionBlock进行逐块激活分析，基于分析结果设计边界保护策略：保留前2个和最后3个块使用BF16精度，其余35个块量化至W8A8 HiF8格式；此外还研究了量化感知训练作为后训练量化的补充微调阶段，并分析了其未能优于纯后训练量化的条件。

**Result:** 所提后训练量化方法在所有五个VBench维度上匹配或略微超过BF16基线，在5个提示的评估集内未观察到可测量的精度损失；消融实验表明完全边界保护配置在所有四种保护配置中取得最高的平均VBench分数，验证了数据驱动的块选择策略的有效性。

**Conclusion:** 基于激活分布异质性分析的边界保护策略可以有效实现视频DiT模型的无损量化，量化感知训练在单卡硬件上可能无法超越纯后训练量化方法，这为大规模视频生成模型的部署提供了实用的量化指导原则。
...
### [48] [Reasmory: 3D Reconstruction as Explicit Memory for VLMs Spatial Reasoning](https://arxiv.org/abs/2606.00963)
*Jixuan He, Xueting Li, Chieh Hubert Lin, Ming-Hsuan Yang*

#### 🧩 TL;DR
Reasmory是一个将空间推理形式化为结构化程序执行的框架，通过构建显式3D记忆、语义化3D目标实例和轻量级领域特定语言(DSL)，使VLMs以受约束和可验证的方式查询和操作空间记忆，在多视图图像和视频空间推理基准上比强基线提升6–18%。

---

#### 📘 Detailed Summary
**Motivation:** VLMs虽然展现出新兴的空间推理能力，但在需要精确空间理解的任务（如视点推理、方向比较和距离估计）上仍不可靠。多视图图像和单目视频中的相关空间线索稀疏且分布在冗余观测中，难以组织和利用。重建基础模型虽能聚合观测为显式空间记忆，但将其作为自由形式工具暴露给VLMs时，模型可能错误调用工具、跳过必要的空间变换或误用中间结果，缺乏可靠性。

**Method:** Reasmory提出将空间推理形式化为结构化程序执行过程：首先构建显式3D记忆并增强语义化的3D目标实例，然后引入轻量级领域特定语言(DSL)来约束VLMs在推理过程中查询对象和相机、变换视点以及渲染观测的方式。生成的程序在执行前经过解析和验证，相比无约束的工具使用能够实现更可靠的空间记忆交互。

**Result:** 在多视图图像和视频空间推理基准上，Reasmory比包括GPT-5-mini和Gemini-3-flash在内的强基线方法取得了6–18%的持续提升。

**Conclusion:** 实验结果表明，显式3D记忆在通过受约束、可验证的操作而非自由形式的工具调用访问时最为有效，这揭示了结构化程序执行范式对于提升VLMs空间推理可靠性的关键价值。
...
### [49] [An Open-Source Benchmark and Baseline for Multi-temporal Referring Segmentation](https://arxiv.org/abs/2606.00987)
*Bingyu Li, Da Zhang, Tao Huo, Zhiyuan Zhao, Junyu Gao, Xuelong Li*

#### 🧩 TL;DR
提出多时序指代分割（MTRS）新任务及首个基准MTRefSeg-21K，并设计CRAFT-Agent自动化数据构建流程与MTRefSeg-R1两阶段训练框架，实验表明该框架在语言引导的多时序变化分割中显著优于现有LVLM基线。

---

#### 📘 Detailed Summary
**Motivation:** 现有大型视觉语言模型（LVLM）在视觉理解和语言引导定位方面表现优异，但其对多时序图像的视觉推理能力（尤其是语言描述的时序变化分割）尚未被充分探索，缺乏相应的任务定义、数据和专用模型。

**Method:** 首先定义了多时序指代分割（MTRS）任务，要求模型同时完成时序对应推理、语言定位和像素级掩码预测；提出了CRAFT-Agent自动化数据构建管线并辅以人工审核，构建了包含21K高质量多时序图像-文本-掩码三元组的MTRefSeg-21K基准；提出了MTRefSeg-R1框架，该框架采用两阶段训练策略：先在20K纯视觉双时序样本上学习通用时序变化感知能力，再在MTRefSeg-21K上进行细粒度语言引导的时序定位微调，显式建模跨时序视觉差异并实现语言指令与时序变化的对齐。

**Result:** 在MTRefSeg-21K基准上对多种VLM和LVLM模型进行评估，发现直接推理效果较差，任务特定微调仍有局限；MTRefSeg-R1在所有基线中取得了强且通常最优的性能，证实了MTRS任务的挑战性及所提方法的有效性。

**Conclusion:** MTRS是一项融合时序推理、语言理解和像素级分割的具有挑战性的新任务；CRAFT-Agent和MTRefSeg-21K为后续研究提供了可靠的数据基础；MTRefSeg-R1的两阶段训练策略为LVLM在多时序场景下的语言引导变化感知提供了有效范式。
...
### [50] [SWARD: Stochastic Window-Attention-Based Relational Distillation for Cross-Architectural Semantic Segmentation](https://arxiv.org/abs/2606.00999)
*Aditya Makineni, Qing Tian*

#### 🧩 TL;DR
SWARD提出了一种针对视觉基础模型知识蒸馏的新框架，通过多尺度窗口注意力蒸馏（MWAD）和原型判别正则化（PDR）两种机制，有效弥合了Transformer教师网络与CNN学生网络之间的表征鸿沟，在城市场景解析和医学图像分割任务上均取得了最优性能。

---

#### 📘 Detailed Summary
**Motivation:** 大规模视觉基础模型在语义分割等稠密预测任务上性能优异，但其参数量庞大难以部署于资源受限环境，因此需通过知识蒸馏将其能力迁移至轻量级学生网络。然而，现有蒸馏方法假设师生网络架构同质，多采用直接特征模仿策略，无法有效桥接Transformer教师（全局上下文编码）与CNN学生（局部偏置感受野）之间的表征差异，且忽略了语义分割所需的结构化空间依赖与判别性特征组织。

**Method:** SWARD框架包含两个核心模块：一是多尺度窗口注意力蒸馏（MWAD），通过在每轮训练迭代中随机重采样偏移量的随机移位窗口划分来对齐师生网络的注意力关系，消除窗口边界偏差，并结合多尺度设计同时捕获短程与长程空间依赖；二是原型判别正则化（PDR），通过强制类间分离与类内紧凑的损失项来塑造学生网络的特征分布，在学生容量受限的条件下进一步强化判别性结构。

**Result:** 实验在城市场景解析与医学图像分割等多个视觉应用上进行，结果表明SWARD在各项任务上均达到了最先进的性能水平。

**Conclusion:** SWARD通过引入注意力关系对齐与判别性正则化两项机制，有效解决了异构架构下知识蒸馏中表征鸿沟的关键难题，为将大规模Transformer教师的能力高效迁移至轻量级CNN学生网络提供了新的范式，其跨领域的最优表现验证了方法的通用性与有效性。
...
### [51] [TextFake: Benchmarking AI-Generated Image Detection on Text-Rich Images](https://arxiv.org/abs/2606.01050)
*Yuning Zhang, Changtao Miao, Mingyu Liao, Tingyu Liu, Xinghao Wang, Tao Gong, Qi Chu, Nenghai Yu*

#### 🧩 TL;DR
本文提出TextFake基准，系统评估AI生成图像检测器在文本丰富伪造内容上的表现，发现现有方法准确率均低于80%，并揭示了文本密度诅咒、渲染保真度伪装和阈值崩溃三种关键失效模式。

---

#### 📘 Detailed Summary
**Motivation:** 现有AI生成图像检测器在自然图像基准上表现良好，但文本丰富的伪造内容（如伪造截图、文档和新闻页面）在 misinformation 中广泛存在，而检测器在这类场景下的表现尚未被系统检验，存在显著研究空白。

**Method:** 构建了TextFake基准，包含20,000张图像，覆盖28种语言、4个主题类别和2种场景模态；通过四阶段流水线生成伪造图像，包括对真实图像沿三个受控维度进行标注，并通过分布对齐的结构化提示生成对应伪造图像以排除协变量捷径；对14个专门检测器与3个前沿VLM API进行零样本评估。

**Result:** 零样本评估显示所有方法在TextFake上准确率均未超过80%，部分检测器性能相比自然图像基准下降超过60%；诊断评估识别出三种失效模式：文本密度诅咒（密集字形压倒低级检测器）、渲染保真度伪装（强文本渲染抑制生成伪影）、阈值崩溃（常规扰动将检测器推向随机水平）。

**Conclusion:** 研究表明当前AI生成图像检测器在文本丰富场景下存在系统性性能鸿沟，其主要失效模式源于文本密度、渲染保真度和扰动鲁棒性三个维度，为未来面向真实世界 misinformation 场景的检测器设计提供了关键洞察和改进方向。
...
### [52] [3DCodeBench: Benchmarking Agentic Procedural 3D Modeling Via Code](https://arxiv.org/abs/2606.01057)
*Yipeng Gao, Lei Shu, Genzhi Ye, Xi Xiong, Ameesh Makadia, Meiqi Guo, Laurent Itti, Jindong Chen*

#### 🧩 TL;DR
本文提出3DCodeBench基准和3DCodeArena平台，系统评估12种视觉语言模型通过代码生成程序化3D内容的能力，发现失败主要源于API不匹配和几何组件断裂/漂浮，而测试时扩展策略可提升性能。

---

#### 📘 Detailed Summary
**Motivation:** 程序化3D建模能生成确定性、可直接用于引擎且精确可编辑的3D资产，但这需要深厚的3D软件API、参数化设计和代码级几何推理专业知识；现有神经3D生成器缺乏这些特性，而视觉语言模型在此领域的潜力尚未被系统评估。

**Method:** 提出3DCodeBench系统基准，评估12种先进VLM代理将文本和图像参考翻译为3D建模软件程序化代码的能力；同时构建3DCodeArena排名平台，基于成对人工偏好对生成的3D输出进行排序，以弥补自动指标无法完全捕捉3D形状感知质量的不足。

**Result:** 实验发现：（1）失败主要源于API不匹配，即使渲染成功，生成的3D几何组件也存在断开或漂浮问题；（2）测试时扩展策略（如更高的思考预算和多轮细化）可整体提升性能；高质量程序化编码数据和提供高保真反馈的鲁棒执行环境是关键需求。

**Conclusion:** 研究揭示商业VLM在程序化3D建模中仍需高质量程序化编码数据，且有效建模需要具备高保真反馈的鲁棒执行环境以支持迭代细化；发布的3DCodeBench数据集、评估协议和3DCodeArena平台为基于VLM的程序化3D建模研究提供了基础工具。
...
### [53] [A Multiscale Network with Supervised Contrastive Learning for Real-Time Facial Emotion Recognition](https://arxiv.org/abs/2606.01069)
*Rejoy Chakraborty, Archisman Adhikary, Chayan Halder, Payel Rakshit, Sanchita Ghosh, Kaushik Roy*

#### 🧩 TL;DR
本文提出一种基于深度学习的实时面部表情识别系统，通过对视频中连续面部表情变化进行建模来检测个体的情绪状态变化，在标准数据集上取得了满意的效果。

---

#### 📘 Detailed Summary
**Motivation:** 实时视频中面部表情驱动的情绪识别面临多重挑战：情绪状态随时间连续变化而非离散突变，且不同个体表达同一情绪时面部表情差异显著，传统计算方法难以有效表征这种连续变化模式，现有系统缺乏捕捉表情动态演变的能力。

**Method:** 本文采用深度学习方法，通过对视频中个体面部表情的连续变化进行建模，构建了一个能够实时检测情绪变化的系统；系统在标准情绪数据集上进行训练。

**Result:** 系统在标准数据集上完成训练和评估，实验结果表明该方法在实时视频情绪变化检测方面取得了非常满意的效果。

**Conclusion:** 所提出的深度学习系统能够有效建模面部表情的连续变化过程，实现对个体情绪状态的实时检测，可为心理咨询等场景提供有价值的辅助分析工具。
...
### [54] [Chameleon: Style-Content Disentangled Framework for Cross-Domain Object Compositing](https://arxiv.org/abs/2606.01079)
*Sukhun Ko, Soo Ye Kim, Jihyong Oh*

#### 🧩 TL;DR
提出首个大规模跨域图像合成训练数据集ChameleonDataset及两阶段训练框架Chameleon，通过联合硬对比学习和时空注意力门控机制，实现前景对象跨域合成时的内容保持与风格迁移，性能超越现有领域内、跨域合成模型及商业模型。

---

#### 📘 Detailed Summary
**Motivation:** 现有扩散模型在图像合成中表现优异，但主要针对前景与背景同域场景；跨域合成（前景与背景来自不同域）因缺乏大规模配对训练数据，现有方法局限于无训练混合与微调策略，仅能实现色调级对齐，常产生风格不一致或过度风格化的结果。

**Method:** 提出Chameleon两阶段训练框架：第一阶段通过联合硬对比学习（JHCL）训练ChameleonEncoder，实现风格与内容表征的有效解耦；第二阶段在扩散Transformer中引入时空注意力门控（STAG），自适应调控第一阶段的风格标记在空间和时间维度上的注入方式，实现有效风格化。

**Result:** 构建了首个大规模跨域合成训练数据集ChameleonDataset及评估基准；所提方法在组合合理性和风格保真度上均优于最先进的域内合成模型、跨域合成模型、顺序流水线及商业模型。

**Conclusion:** 通过构建大规模训练数据集并设计两阶段解耦-注入框架，有效解决了跨域图像合成中内容保持与风格匹配的权衡问题，为训练驱动的跨域合成方法开辟了新路径。
...
### [55] [Dual-Route Top-K Retrieval with 1v1 VLM Reranking for the CoVR-R](https://arxiv.org/abs/2606.01097)
*Yuyang Sun, Yongliang Wu, Xingyu Zhu, Yuxia Chen, Zhenxiang Jiang, Yangguang Ji, Wenbo Zhu, Yanxi Shi, Jay Wu, Shuo Wang, Xu Yang*

#### 🧩 TL;DR
本文针对组合视频检索（CoVR-R）任务，提出双路径Top-K检索与1v1 VLM重排序方法，通过将检索解耦为候选集构建与保守重排序两个子问题，在测试集上达到95.28 R@1，表明召回-选择解耦策略优于广泛的文本重排序或多候选VLM分类。

---

#### 📘 Detailed Summary
**Motivation:** 组合视频检索（CoVR-R）任务面临两个核心挑战：如何构建足够完备的Top-K候选集，以及如何安全地决定是否用候选替换当前Top-1结果；现有方法在候选集构建与最终决策之间的耦合程度较高，缺乏有效的解耦策略。

**Method:** 方法将组合视频检索分解为两个子问题：首先通过VLM槽位选择器在现有候选中优化推理/文本种子（不使用DFN视觉检索），同时引入基于联系表嵌入的视觉路径（使用DFN-H/DFN-L）；两条路径的候选集合并为Top-10候选集，随后使用VLM最终重排序器对当前Top-1与每个挑战者进行保守的1v1比较。

**Result:** 在隐藏测试集上，最终系统达到95.28 R@1、97.47 R@5、98.48 R@10和99.66 R@50，取得了优异的检索性能。

**Conclusion:** 主要启示是：CoVR-R任务从召回-选择解耦策略中获得的收益大于广泛文本重排序或直接多候选VLM分类方法，这为组合视频检索任务的设计提供了重要的方法论指导。
...
### [56] [Temporal Evidence Routing with Structured Visual Evidence for TimeLogicQA](https://arxiv.org/abs/2606.01106)
*Yuyang Sun, Yongliang Wu, Xingyu Zhu, Yuxia Chen, Zhenxiang Jiang, Yangguang Ji, Wenbo Zhu, Yanxi Shi, Jay Wu, Shuo Wang, Xu Yang*

#### 🧩 TL;DR
TimeLogicQA引入视觉证据路由流水线，将感知与符号时序推理解耦，通过结构化视觉证据提取、程序化动作区间恢复和确定性时序规则归约，在时序视频问答任务上达到81.8的AvgAcc。

---

#### 📘 Detailed Summary
**Motivation:** 现有视频问答系统在推理事件存在性、顺序、持续性、边界条件和重叠等时序关系时能力不足，缺乏将视觉感知与符号化时序推理有效分离的框架，导致对复杂时间逻辑问题的回答质量不稳定。

**Method:** 提出视觉证据路由流水线：首先将问题解析为事件目标、回答模式、候选选项和时序算子；然后根据视频时长和算子难度路由，短片使用有序全帧证据，长视频使用事件聚焦候选窗口；多模态大语言模型提取结构化视觉证据，程序化验证器恢复密集动作区间，确定性归约器应用算子特定时序规则生成答案。采用保守融合策略，仅当视觉证据、时序程序和置信度检查一致时才接受答案，减少噪声翻转。

**Result:** 在TimeLogicQA官方测试集上，最终系统实现了81.8的平均准确率（AvgAcc），验证了所提流水线在时序推理任务上的有效性。

**Conclusion:** 将感知与符号推理解耦的流水线设计能够有效提升视频时序问答的准确性和鲁棒性，保守融合策略在减少错误答案翻转方面发挥了关键作用，为构建具有可靠时序推理能力的视频理解系统提供了可行范式。
...
### [57] [Distilling Neuro-Symbolic Programs into 3D Multi-modal LLMs](https://arxiv.org/abs/2606.01215)
*Wentao Mo, Yang Liu*

#### 🧩 TL;DR
APEIRIA通过三阶段课程训练将神经符号3D推理的模式蒸馏到多模态大语言模型中，在保持可解释推理和模块可替换性的同时，弥合了符号方法与端到端MLLM之间的差距，在3D空间推理任务上超越现有神经符号方法并匹配最先进的3D MLLM性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有3D空间推理方法存在根本性权衡：神经符号3D概念学习器通过组合式程序实现可解释推理，但局限于封闭集概念词汇和简单程序；端到端3D多模态大语言模型（3D MLLM）能处理复杂自然语言和开放词汇概念，但推理过程不透明且缺乏显式空间验证。两者之间缺乏能同时具备可解释推理与灵活泛化能力的统一框架。

**Method:** APEIRIA提出一种神经符号3D MLLM框架，通过三阶段课程学习将符号推理模式蒸馏到MLLM中：a) 3D感知对齐阶段将物体视觉-几何特征对齐至LLM；b) CoT-SFT阶段利用符号程序轨迹进行查询分解和逐步验证的监督微调；c) CoT-RL阶段通过强化学习将推理模式扩展到开放集概念和深层嵌套指令。该方法传递推理模式而非概念特定知识，保留了透明推理和规划/感知模块的可互换性。

**Result:** 在3D空间推理数据集的接地（grounding）、问答和字幕生成任务上的评估表明，APEIRIA超越了先前的神经符号3D方法，并与最先进的3D MLLM性能相当，成功统一了符号方法的系统性推理与MLLM的灵活性。

**Conclusion:** 通过蒸馏推理模式而非概念知识，APEIRIA有效弥合了神经符号3D方法与端到端MLLM之间的范式差距，证明符号推理模式可以通过课程蒸馏迁移到MLLM中，同时保持可解释性和模块可替换性，为构建兼具严谨性与灵活性的3D空间推理系统提供了新路径。
...
### [58] [HakushoBench: A Japanese Chart and Table VQA Benchmark from Governmental White Papers](https://arxiv.org/abs/2606.01132)
*Issa Sugiura, Shuhei Kurita, Yusuke Oda, Naoaki Okazaki*

#### 🧩 TL;DR
本文提出HakushoBench，一个基于日本政府白皮书构建的日语图表VQA基准，包含2,053张图像和人工标注的问答对。实验表明该基准对开源VLM具有挑战性（最高准确率58.6%），且开源与闭源模型之间存在34.9个百分点的显著差距。

---

#### 📘 Detailed Summary
**Motivation:** 现有图表和表格理解的视觉语言模型基准主要集中于英语，非英语基准稀少，导致跨语言泛化能力不明确。收集大规模、真实且多样化的非英语图表图像是构建此类基准的主要障碍。

**Method:** 利用各国政府白皮书作为超越英语的基准构建可扩展数据源，因其自然包含多种格式和领域的图表与表格且免费可获取。作为首次实例化，从33份日本政府白皮书中构建了HakushoBench基准，包含2,053张图像、覆盖10种以上图像类型，并配有手工标注的问答对，旨在评估对图表和表格的深层整体理解而非局部视觉线索。

**Result:** 在多种VLM上的实验表明，最佳开源模型在HakushoBench上仅达到58.6%的准确率，而开源与闭源模型之间存在34.9个百分点的显著性能差距，说明该基准对于复杂图表理解任务具有挑战性且仍有较大改进空间。

**Conclusion:** HakushoBench揭示了当前VLM在非英语图表理解任务上的局限性，特别是开源模型的显著不足。该基准为跨语言图表理解研究提供了新的评估资源，有助于推动VLM在真实多语言文档理解场景中的进步。
...
### [59] [On the Limits of Token Reduction for Efficient Unified Vision Language Training](https://arxiv.org/abs/2606.01503)
*Siyi Chen, Weiming Zhuang, Jingtao Li, Lingjuan Lv*

#### 🧩 TL;DR
通过分析统一视觉-语言模型（VLM）中逐层注意力分配，发现视觉理解在深层存在显著冗余而视觉生成则持续依赖图像token，据此设计任务特定的token缩减加速器；然而在联合训练中，任务特定token丢弃会导致协同效应损失，表明高效统一建模需要保留跨任务共享结构。

---

#### 📘 Detailed Summary
**Motivation:** 统一视觉-语言模型（VLM）将视觉理解与视觉生成集成在单一自回归架构中，但其联合训练计算成本极高，而现有工作鲜有从效率角度系统探索该问题的加速方法。

**Method:** 通过逐层注意力分配的系统性分析，揭示出视觉理解在深层存在大量视觉冗余而视觉生成在深度方向上对图像token保持持续依赖这一根本性不对称；基于此观察，设计针对不同任务（理解/生成）的选择性图像token缩减加速器，在隔离场景下分别降低计算量。

**Result:** 任务特定的token缩减方法在隔离场景下取得显著效率提升，但在统一联合训练中观察到一致的协同性能损失——任务特定token丢弃导致参数路径分叉，消除了联合优化中通常存在的互惠性能增益。

**Conclusion:** 高效统一VLM建模需要保留跨任务共享结构，而非激进的独立任务特定加速，亟需协同感知的加速策略来兼顾效率和联合训练的性能提升。
...
### [60] [CoSTL: Comprehensive Spatial-Temporal Representation Learning for Moment Retrieval and Highlight Detection](https://arxiv.org/abs/2606.01149)
*Xin Dong, Wenjia Geng, Wenfeng Deng, Yansong Tang*

#### 🧩 TL;DR
CoSTL提出一种综合时空表征学习框架，通过文本驱动的渐进式细粒度图像编码器和多尺度时序感知模块，同时捕捉图像级细节与视频级时间动态，在视频时刻检索和高亮检测任务上取得SOTA性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有视频时刻检索与高亮检测方法多采用统一架构，侧重基于帧级特征的时序建模，却忽略了单帧内与文本查询相关的丰富视觉信息，导致细粒度图像级理解不足，影响了定位的准确性。本文旨在弥补这一研究空白。

**Method:** CoSTL框架包含两个核心模块：一是文本驱动的渐进式细粒度图像编码器，通过两步式文本驱动知识提取过程学习细粒度空间表征；二是多尺度时序感知模块，用于捕获综合的时空表征，增强模型对时间动态的处理能力。该框架同时兼顾图像级精细信息与视频级时间动态。

**Result:** 在QVHighlights、Charades-STA、TACoS和TVSum四个公开基准数据集上达到了最先进的性能，验证了该方法在时刻检索和高亮检测任务中的有效性。

**Conclusion:** 同时建模细粒度图像级空间信息与多尺度时间动态能够显著提升视频接地任务的准确性，为面向文本查询的视频理解提供了更全面的表征学习范式。
...
### [61] [Feature Alignment Determines Fusion Strategy: A Comparative Study of Cross-Attention and Concatenation in Multimodal Learning](https://arxiv.org/abs/2606.01207)
*Zhiqiang Zhou, Xuezhen Xie*

#### 🧩 TL;DR
本文证明特征对齐质量而非数据规模是决定多模态融合策略选择的关键因素，并通过样本复杂度分析揭示了拼接（concatenation）在特征预对齐场景下相比交叉注意力（cross-attention）具有显著样本效率优势，为多模态系统的融合方法选择提供了理论指导框架。

---

#### 📘 Detailed Summary
**Motivation:** 当前多模态融合中交叉注意力与拼接方法的选择缺乏理论依据，仅依赖实践者直觉，本研究旨在揭示决定两种融合策略优劣的核心因素，填补该领域缺乏原理性理解的研究空白。

**Method:** 通过在Flickr8k数据集上使用ResNet18和CLIP ViT-B/32两种特征提取骨干网络进行受控实验，对比不同数据规模（2048-16384样本）下拼接与交叉注意力融合的性能差异；从样本复杂度角度给出理论解释，分析两种方法学习融合投影所需样本量的差异（拼接需O(d_v+d_t)样本，交叉注意力需O(d_v*d_t)样本）；并通过特征对齐退化实验验证对齐质量对融合效果的单调影响趋势。

**Result:** 当特征经过视觉-语言预训练目标预对齐后，拼接方法在所有测试数据规模下均以4.1-5.1个百分点的优势超越交叉注意力；理论分析表明，对于512维CLIP特征，交叉注意力所需样本量是拼接的256倍以上；对齐退化实验显示随着特征对齐程度降低，拼接的优势从1.3%单调增长至2.8%。

**Conclusion:** 特征对齐质量是决定融合策略选择的首要因素，在特征已对齐的场景下拼接因样本效率优势成为更优选择；该发现为多模态系统及多模态大语言模型的融合方法选择提供了原理性决策框架。
...
### [62] [The Image Reconstruction Game: Drawing Common Ground Through Iterative Multimodal Dialogue](https://arxiv.org/abs/2606.01901)
*Sherzod Hakimov, Mattia D'Agostini, Ivan Samodelkin, David Schlangen*

#### 🧩 TL;DR
本文提出Image Reconstruction Game（图像重建游戏），一个通过多轮对话让视觉语言模型指导图像生成器进行重建的全自动化基准测试，实验发现描述器是重建质量的主导因素，而生成器决定迭代优化是否有益。

---

#### 📘 Detailed Summary
**Motivation:** 现有基准测试缺乏对视觉语言模型与图像生成器之间多轮交互中累积共同关注（accumulated common ground）的直接可观测度量，本文旨在通过一个完全自动化的图像重建游戏来填补这一研究空白。

**Method:** 构建了一个全自动化的多轮交互基准测试框架，包含描述器（Describer）和生成器（Generator）两类模型，描述器根据当前图像与目标图像的差异生成纠正性指令，生成器据此更新图像，通过最终渲染图像直接量化多轮交互中累积的共同关注。

**Result:** 实验发现描述器是重建质量的主导因素，而生成器决定迭代优化是否有帮助；数学和几何图像最具挑战性；较短的token预算产生更稀疏的初版图像但改进空间更大，较长token预算提高绝对质量但修复余地更小；强描述器使用更丰富的校正词汇（空间、数字、结构类），弱描述器集中于表面属性且容易提前停止；最佳自动化评判器与人类偏好仅达到轻微到中等一致性。

**Conclusion:** 多轮视觉语言交互的质量主要取决于描述器的表达能力而非生成器的能力，且自动化评估结果需要经过人类校准才能可靠使用，这为构建更有效的多模态交互系统提供了重要设计启示。
...
### [63] [HOLA: Holistic Multi-Modal Alignment for Open-Set 3D Recognition](https://arxiv.org/abs/2606.01334)
*Koby Aharonov, Oren Shrout, Ayellet Tal*

#### 🧩 TL;DR
提出一种解耦的多正面对比损失函数，将点云与多视角图像和多段文本联合对齐，以替代传统的单视图单文本对齐方式，在开放集3D识别长尾基准上取得SOTA零样本性能，同时保持高帧率。

---

#### 📘 Detailed Summary
**Motivation:** 现有开放集3D识别方法通过将语言-视觉知识蒸馏到3D编码器中，通常依赖笨重的2D ViT并将每个点云与单张图像或单段文本对齐，导致表征锚定在局部视角上，难以捕捉3D物体的全局语义，且对罕见或未见类别的泛化能力不足。

**Method:** 提出解耦的多正面对比损失函数（Decoupled Multi-Positive Contrastive Loss），使每个3D实例能同时与多视角图像和多段文本进行联合对齐，同时将正样本聚合与负样本竞争分离，避免多个正样本与所有负样本共享同一softmax导致的"聚光灯拥挤"问题，并增强损失函数对困难负样本的感知能力；此外，设计了一个轻量级文本适配器（Text Adapter）用于处理网页级无标注文本，以缩小与人工标注标注之间的领域差距。

**Result:** 在开放词汇3D识别长尾基准上达到SOTA性能，在零样本设定下取得显著提升，同时维持高推理帧率，验证了多模态多实例对齐策略在开放集3D识别中的有效性。

**Conclusion:** 通过解耦的多正面对比学习与轻量文本适配，能够有效利用多视角视觉信息和大规模无监督文本数据，在不增加计算负担的前提下显著提升3D模型对罕见类别的开放词汇识别能力。
...
### [64] [Perception First: A Frontier Native-Video Model with Self-Consistency for Implicit Video Question Answering](https://arxiv.org/abs/2606.01485)
*Ali Alavi*

#### 🧩 TL;DR
通过对多种开源Video-LMMs和推理策略的系统性、免训练研究，本文发现VRR Challenge基准是感知受限而非推理受限的：推理增强策略效果中性甚至有害，而基础模型感知能力和轻量级测试时去噪才是提升性能的唯一可靠手段；细粒度错误分析表明相对深度、视点和计数是最困难类别，因果与社会推理已接近解决。

---

#### 📘 Detailed Summary
**Motivation:** 面向创意视频的多选题视频问答（VRR-QA基准）中，答案无法从单帧直接观测，需跨不连续帧推断空间布局、运动、深度、视点、因果关系和社会语境，现有研究缺乏对这一基准下模型瓶颈的系统性分析，尤其是推理能力与感知能力之间的相对重要性尚未厘清。

**Method:** 本研究对Qwen2.5-VL、Qwen3-VL、InternVL3、Gemma-3以及RL微调的视频推理模型Video-R1和VideoChat-R1.5等多种开源Video-LMMs进行了系统的免训练评估，并测试了链式思维、问题分解、描述-推理级联、音频转录、空间状态提示、自一致性、多模型集成和类别路由等多种推理时策略，同时采用逐类别错误分析定位模型薄弱环节。

**Result:** 实验发现所有推理增强策略对性能均为中性甚至有害，只有基础模型感知能力和轻量级测试时去噪可提升准确性；逐类别分析表明相对深度、视点和计数是最困难类别，而因果推理和社会推理接近解决；明确注入单目深度线索的提示反而使测试准确率下降5.8个百分点，证实模型需要更好的感知能力而非更好的推理流程。

**Conclusion:** 本研究揭示VRR基准中的视频问答瓶颈在于低层感知而非高层推理，直接证明当前Video-LMMs的感知能力不足以支持从创意视频中推断空间关系、深度和视点等关键信息，为视频理解模型的发展方向提供了明确指引：应优先提升感知质量而非追求更复杂的推理策略。
...
### [65] [Jailbreaking Multimodal Large Language Models using Multi-Clip Video](https://arxiv.org/abs/2606.02111)
*Choongwon Kang, Seungjong Sun, Hyunmin Jun, Jang Hyun Kim*

#### 🧩 TL;DR
本文提出MCV SafetyBench数据集，系统评估视频多模态大模型的安全性漏洞，发现攻击成功率随视频片段数量、动态性和上下文多样性增加而上升，并提出基于图像模态相对鲁棒性的防御策略。

---

#### 📘 Detailed Summary
**Motivation:** 尽管多模态大模型（MLLMs）已发展至支持视频输入，但其可能被恶意利用的安全风险日益突出，已有研究表明视觉输入可绕过MLLMs的安全对齐，但视频输入的哪些具体属性会导致这一漏洞尚不明确，本文旨在系统探究该问题。

**Method:** 构建了Multi-Clip Video (MCV) SafetyBench数据集，包含2,920个视频，每个视频由多个短片段组成，这些片段呈现与有害查询相关的多样化上下文；通过在八个代表性视频MLLM上进行实验，系统评估视频片段数量、动态性以及上下文多样性对模型安全漏洞的影响，并提出利用图像模态相对鲁棒性的防御策略。

**Result:** 实验结果表明：（1）攻击成功率随视频片段数量增加而持续上升；（2）视频模态比图像模态更脆弱；（3）动态视频比静态视频更容易引发攻击成功；（4）视频中包含更多样化的上下文时模型安全性更差。

**Conclusion:** 该研究揭示了视频输入的多片段性、动态性和上下文多样性是导致MLLMs更易被攻击的关键因素，并提出基于图像模态相对鲁棒性的防御方向，为视频MLLM的安全性研究提供了重要见解和实用数据集。
...
### [66] [Cross-modal linkage risk in clinical vision-language models](https://arxiv.org/abs/2606.02276)
*Soroosh Tayebi Arasteh, Mahshad Lotfinia, Sven Nebelung, Daniel Truhn*

#### 🧩 TL;DR
本文发现视觉-语言模型在多模态医学影像中可通过余弦相似度重新关联原本分离的图像与报告，构成隐私风险，并提出仅对对齐层进行差分隐私微调的方法来显著降低重链接风险，同时保持图像表征的临床可用性。

---

#### 📘 Detailed Summary
**Motivation:** 视觉-语言模型（VLM）在胸部X光片与放射学报告配对训练后，会学习到保留实例级图像-报告对应关系的共享嵌入空间。在图像仅数据共享或报告受控访问等场景中，图像与报告被刻意分开保存，但VLM可通过余弦相似度将去标识图像重新链接到原始叙述性报告，构成隐私泄露风险。

**Method:** 本文将隐私风险形式化为图像到报告的检索任务，利用已知真实配对的公共数据集作为基准来审计风险。使用MIMIC-CXR（43,793对保留样本）和CheXpert Plus（29,296对）共126,804名患者的406,241个配对样本，评估临床专业化程度递增的VLM。为降低风险，在不重训练编码器的情况下，仅对定义对齐层的投影头应用差分隐私优化（ε=0.34，δ=6×10⁻⁶）。

**Result:** 最强的VLM在候选池N=100时以15倍概率检索到正确报告，N=10,000时达50倍概率，在全数据库规模下仍显著高于随机水平；该信号在移除疾病标签捷径的病理匹配难负样本下依然存在。差分隐私微调后，MIMIC-CXR上Recall@1（N=10,000）降低61.8%，且无需重新训练即可迁移至CheXpert Plus；图像侧效用基本保持，14个标签的线性探测宏AUROC仅从79.63%微降至79.43%。

**Conclusion:** VLM的临床专业化程度越高，跨模态重链接隐私风险越大；对共享对齐层进行针对性的差分隐私微调，可以在不大幅降低临床有用图像表征质量的前提下，有效降低跨模态重链接风险。
...
### [67] [PathAR: Structure-First Autoregressive Synthesis of Multimodal Pathology Images](https://arxiv.org/abs/2606.01543)
*Yuan Zhang, Jiahao Xia, Junzhang Huang, Meng Wang, Feng Chen, Guanyu Yang, Huazhu Fu*

#### 🧩 TL;DR
PathAR提出了一种结构优先的自回归合成框架，通过显式分解结构与外观并使用非对称注意力机制的交叉自回归Transformer，实现了模态标签条件化的病理图像生成，在保持结构一致性的同时支持多样化的模态外观生成与下游分割任务。

---

#### 📘 Detailed Summary
**Motivation:** 多模态病理学中数据稀缺问题严重，现有统一生成模型虽能合成不同模态外观，但通常将结构与外观耦合在单一token流中隐式建模，导致模态迁移时结构可控性弱化，难以在保持解剖学结构一致性的前提下灵活控制模态特异性外观生成。

**Method:** PathAR采用双向量量化（Dual-VQ）分词器将样本分解为基于掩码的结构token和外观token，并设计交叉自回归（IAR）Transformer，通过非对称注意力可见性强制结构到外观的依赖关系，实现模态标签条件化的病理图像生成，支持空间对齐的图像-掩码对联合生成。

**Result:** 实验表明，PathAR在结构一致性和模态保真度上优于基线方法，同时保持样本多样性，在数据稀缺场景下有效支持下游分割任务，并展现出对细粒度器官标签变化的可扩展性。

**Conclusion:** 显式解耦结构与外观的自回归生成范式能够有效稳定多模态病理生成中的形态结构，提升模态迁移下的结构可控性，为数据稀缺场景下的病理分析提供可行的生成式数据增强方案，并具备向更细粒度条件生成扩展的潜力。
...
### [68] [TLG: Temporal-Logic Grounding for Video Question Answering via Source-Annotation Reconstruction and Category-Targeted Reasoning](https://arxiv.org/abs/2606.01591)
*Ali Alavi*

#### 🧩 TL;DR
本文提出TLG系统，通过重构视频动作时间线并解析问题为时序逻辑程序，将时间逻辑推理测试的准确率从46.9%提升至71.37%，揭示了实际标注而非更大模型才是驱动精度的关键瓶颈。

---

#### 📘 Detailed Summary
**Motivation:** 现有端到端视频语言模型将视频视为帧袋，无法定位动作发生的时间位置，在需要形式化时序逻辑推理的TimeLogic挑战中表现接近随机水平，亟需一种能够精确进行时序逻辑推理的方法。

**Method:** TLG是一个三层次系统：第一层从公开数据集标注重构视频动作时间线，将问题解析为时序逻辑程序并确定性执行；第二层在缺乏标注时回退到强开源VLM；第三层仅将VLM经验性最弱的问答类别路由到前沿推理模型。

**Result:** TLG将测试准确率从VLM基线模型的46.9%提升至71.37%，绝对提升达24.5个百分点，距离排行榜第一名仅差3个百分点；消融实验表明三种基于模型的时间线重构变体均不如整体VLM，隔离出时序定位作为不可约瓶颈。

**Conclusion:** 时序定位是视频时序逻辑推理的核心瓶颈，真实标注而非更大模型才是驱动精度的关键因素；TLG通过符号化时序逻辑推理与VLM的混合架构显著超越纯端到端方法。
...
### [69] [Paving the Way for Point Cloud Video Representation Learning Using A PDE Model](https://arxiv.org/abs/2606.01604)
*Zhuoxu Huang, Zhenkun Fan, Jungong Han, Josef Kittler*

#### 🧩 TL;DR
本文提出MotionPDE，一种将空间-时序相关性正则化为可解偏微分方程（PDE）的即插即用增强模块，通过对比学习结构引导PDE求解过程，有效提升点云视频现有骨干模型的表现，且仅引入极小的额外计算开销。

---

#### 📘 Detailed Summary
**Motivation:** 点云视频中空间点随时间变化的空间-时序相关性难以被传统基于光流的方法有效捕捉，根本原因在于序列点云数据的无序空间排列特性；此外，PDE在物理领域虽已被证明有效，但其在点云视频这类新型序列数据上的应用仍鲜有探索。

**Method:** 受流体分析启发，作者构建了一个简化的偏微分方程（PDE）来形式化空间-时序相关性学习，并通过时间嵌入与空间嵌入之间的对比学习结构来引导和精细调节PDE的求解过程；该方法以即插即用的增强模块形式集成到现有骨干网络中，仅增加极少的参数量和计算开销。

**Result:** MotionPDE作为增强模块在点云视频现有骨干模型上取得了显著的效果提升，验证了其有效性和即插即用性；进一步在自监督学习设定下，对比学习机制的引入取得了有前景的结果，证明了该方法在点云视频数据解释中的实用性和适应性。

**Conclusion:** 将PDE正则化与对比学习相结合是建模点云视频空间-时序相关性的有效范式；MotionPDE以极低计算开销即可增强现有模型，且具备良好的自监督学习潜力，为点云视频理解提供了新的研究方向。
...
### [70] [Self-Improving Small Object Grounding in LVLMs](https://arxiv.org/abs/2606.01612)
*Tianze Yang, Yucheng Shi, Ruitong Sun, Ninghao Liu, Jin Sun*

#### 🧩 TL;DR
本文证明大型视觉语言模型（LVLM）的内部注意力结构可直接用于改进小目标定位，提出了基于注意力机制的候选框选择框架（ACS），无需微调即可实现高达19%的定位自提升，并揭示了哪些Transformer层和注意力头对定位质量最关键。

---

#### 📘 Detailed Summary
**Motivation:** 现有大型视觉语言模型在细粒度视觉任务（如小目标定位）中表现不足，传统方法通常需要对模型进行微调或引入额外组件，而本文探索是否可以直接利用LVLM内部的注意力结构，在不经过微调的情况下识别出可靠的小目标边界框。

**Method:** 本文提出注意力候选选择（ACS）框架，包含两个变体：（1）ACS-Learned，使用轻量级IoU回归器（仅基于注意力图训练，Pearson r > 0.67）从多个采样候选中选择最佳框；（2）ACS-Free，通过分析回归器学到的知识，识别出最关键的Transformer层和注意力头，基于这些头部的注意力熵对候选框进行排序，推理时无需任何可学习组件。

**Result:** 在COCO和Objects365上的实验表明，该方法在小目标定位上实现了高达19%的自提升效果；ACS-Free在所有无需训练的方法中排名最优；轻量级IoU回归器仅基于注意力图即可实现强IoU预测（Pearson相关系数 > 0.67）。

**Conclusion:** LVLM内部注意力结构蕴含了可用的定位质量信息，通过适当利用这种结构，可以在不进行微调的情况下显著提升模型的小目标定位可靠性；该方法同时增强了模型的可解释性，揭示了哪些层和头对定位质量最为关键。
...
### [71] [Goal2Pixel: Grounding Goals to Pixels for Vision-Language Navigation](https://arxiv.org/abs/2606.01621)
*Muyi Bao, Yuxin Cai, Hang Xu, Zongtai Li, Jinxi He, Jingfan Tang, Chen Lv, Ji Zhang, Yaqi Xie, Wenshan Wang*

#### 🧩 TL;DR
Goal2Pixel提出了一种纯像素级范式，将视觉-语言导航（VLN-CE）重新定义为可导航像素定位任务，通过在图像平面内预测导航像素并借助辅助指令区域实现长程导航，在R2R-CE和RxR-CE上以极少的VLM推理调用次数取得了具有竞争力的SOTA性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于VLM的VLN-CE方法将导航建模为低层次动作预测，面临接口模糊、受限于短视运动基元、以及因重复VLM查询导致推理效率低下等问题，亟需一种更高效、更自然的空间交互范式。

**Method:** Goal2Pixel提出纯像素级范式，将VLN-CE转化为可导航像素定位问题：模型在图像平面上预测一个可见导航像素并反投影为3D航点用于前向导航；对非前向动作，在图像平面附加左、右、底部辅助指令区域分别对应左转、右转和停止；引入可见性感知关键帧记忆模块实现紧凑且信息丰富的历史表示；通过语义嵌入和坐标感知辅助损失适配预训练VLM完成导航像素定位。

**Result:** 在R2R-CE Val-Unseen上，Goal2Pixel以每集仅7.75次VLM调用（较直接动作预测的46.62次减少6倍）达到54.1% SR和52.5% SPL，超越后者32.9% SR的表现；在RxR-CE上同样保持优势，验证了方法的高效性与泛化能力。

**Conclusion:** Goal2Pixel验证了将VLN-CE转化为图像平面像素定位范式的可行性与高效性，通过统一的像素级空间接口大幅减少VLM推理开销，为视觉-语言导航提供了一种兼具性能与效率的新方向。
...
### [72] [What to Test Next: Interpretable Coverage Gap Discovery in Driving VLMs](https://arxiv.org/abs/2606.01624)
*Abhishek Aich, Sparsh Garg, Vijay Kumar BG, Turgun Yusuf Kashgari, Manmohan Chandraker*

#### 🧩 TL;DR
提出 SliceScorer 确定性评分规则与 SliceNav LLM 编排验证管道，用于自动驾驶视觉语言模型在操作设计域内外的缺失切片推荐与压力测试，实验证明其能有效发现高风险覆盖缺口且保持可解释与可审计性。

---

#### 📘 Detailed Summary
**Motivation:** 自动驾驶视觉语言模型需要在多样化的操作设计域条件下准确理解场景，但现有验证方法存在严重稀疏性——大量条件切片缺失，导致经验失败率不可靠，亟需一种可解释、可审计且保守的缺失切片推荐方法用于安全关键验证。

**Method:** 提出 SliceScorer 确定性评分规则，结合基于曝光度的覆盖先验（优先推荐罕见欠测试区域）和基于邻域失败的先验（从相似已测试条件传播风险）；在此基础上构建 SliceNav LLM 编排验证管道，模型理解开发者查询并选择相关算子（分诊、评分、获取、评估）及词汇扩展，组合出完整的确定性验证工作流。

**Result:** 在 WiseAD、DriveMM、Cosmos-Reason2-2B 三个驾驶 VLM 上的实验表明，SliceNav 在发现高风险覆盖缺口方面优于先前的切片发现方法，同时保持条件空间上推荐的多样性；消融实验确认两个评分组件均有贡献，定性分析展示了从开发者查询到目标评估的端到端工作流。

**Conclusion:** SliceScorer 和 SliceNav 为安全关键的自动驾驶 VLM 验证提供了一种可解释、可审计且保守的缺失切片推荐与压力测试方案，能有效揭示超出声明 ODD 的高风险区域，对提升自动驾驶系统验证的可靠性和完备性具有重要意义。
...
### [73] [Conditional Collapse in Sign Language Production: A Diagnostic and a Scaling Argument](https://arxiv.org/abs/2606.01643)
*Rui Hong, Jana Košecká*

#### 🧩 TL;DR
本文指出在手语生成（SLP）任务中，现有评估指标（如FID和BT BLEU）可能虚高，无法真实反映生成动作的质量，提出了一个基于冻结运动自编码器（MoAE）的三层次评估框架（初始姿态条件、输出多样性、目标忠实度），并证明当前模型在句子级数据集上无法达到目标忠实度，而数据规模是主要瓶颈。

---

#### 📘 Detailed Summary
**Motivation:** 现有手语生成（SLP）评估指标FID和BT BLEU在生成模型实际未能忠实表达手语手势时仍可能大幅提升，导致评估结果与真实生成质量严重脱节，缺乏对生成动作的细粒度、多层次质量评估方法。

**Method:** 提出三层次评估体系（τ1初始姿态条件、τ2输出多样性、τ3目标忠实度），利用冻结运动自编码器（MoAE）的潜层表示计算成对距离比率，以此量化生成动作在三个独立维度上的质量，并评估了14个SLP模型检查点（含复现的Neural Sign Actors, NSA）在How2Sign和ASL3DWord数据集上的表现。

**Result:** 在How2Sign数据集上，所有14个SLP模型检查点均未达到τ3目标忠实度，FID变化幅度接近两个数量级且与忠实度无相关性；在孤立词数据集ASL3DWord上可达到良好的τ3分数，表明句子级配对数据集规模是当前SLP的主要瓶颈。

**Conclusion:** 当前SLP评估指标FID和BLEU具有误导性，无法反映生成动作是否真正忠实于输入语义；目标忠实度（τ3）的缺失揭示了句子级配对数据不足是核心瓶颈，未来应着力于扩大高质量句子级手语数据集或探索数据高效的学习方法。

---

#### 📄 Abstract
Sign Language Production (SLP) is the task of generating avatar sign language motion from natural language
...
### [74] [Learning Label-Efficient Interpretable Medical Image Diagnosis via Semi-supervised Hypergraph Concept Bottleneck Model](https://arxiv.org/abs/2606.01698)
*Yijun Yang, Ruiqiang Xiao, Lijie Hu, Angelica I Aviles-Rivero, Yunzhu Wu, Jing Qin, Lei Zhu*

#### 🧩 TL;DR
提出了一种基于双层级超图学习的半监督概念瓶颈模型（HyperCBM），通过建模高阶概念依赖关系和生成领域自适应伪标签，在胎盘植入谱系超声数据集、乳腺超声公开数据集和皮肤镜数据集上实现了兼具高可解释性与高诊断性能的医学图像分析。

---

#### 📘 Detailed Summary
**Motivation:** 深度学习在医学影像分析中虽诊断精度优异，但缺乏可解释性，阻碍了其在高风险临床场景中的可信应用；传统概念瓶颈模型虽引入临床语义概念，但难以捕捉概念间复杂依赖关系且依赖昂贵的人工标注，限制了可扩展性。

**Method:** 提出了一种半监督概念瓶颈模型框架HyperCBM，采用双层级超图学习：概念级超图用于增强概念间高阶依赖推理，图像级超图用于生成鲁棒的领域自适应伪标签，从而在减少概念标注成本的同时保持高可解释性。

**Result:** 在自标注的胎盘植入谱系超声数据集、公开乳腺超声数据集以及皮肤镜数据集SkinCon上进行了实验验证，结果表明所提框架在概念标签高效场景下兼具优异的可解释性与诊断性能，并展示了良好的跨领域泛化能力。

**Conclusion:** 该研究通过双层级超图学习有效解决了概念瓶颈模型在医学影像中概念依赖建模不足和标注成本高两大关键问题，为高风险医学诊断场景提供了一种可解释且可扩展的建模框架，代码已开源。
...
### [75] [Density-Aware Translation of Spurious Correlations in Zero-Shot VLMs](https://arxiv.org/abs/2606.01710)
*Afsaneh Hasanebrahimi, Hanxun Huang, Christopher Leckie, Sarah Erfani*

#### 🧩 TL;DR
提出密度感知平移（DAT）方法，通过引入局部几何密度项修正图像-文本相似度分数，有效缓解多模态模型（如CLIP）在零样本分类中因虚假相关性导致的预测偏差，在多个基准数据集上一致提升了最差组精度和平均精度。

---

#### 📘 Detailed Summary
**Motivation:** 视觉-语言模型（如CLIP）在零样本分类中性能强大，但其预测容易受到虚假相关性的影响，即上下文线索主导语义内容。现有解决方案（如微调或提示工程）要么削弱预训练模型的优势，要么容易产生幻觉，因此需要一种不破坏预训练知识且能有效校准模型预测的轻量级方法。

**Method:** 提出密度感知平移（Density-Aware Translation, DAT），该方法基于组参考集计算局部几何密度，利用该密度项重新缩放图像-文本相似度分数。该方法利用了CLIP嵌入空间中的模态间隙和各向异性壳结构：常见模式聚类在均值附近，罕见模式被推向外部，DAT通过抑制稀疏区域中的过度自信分数并保留密集的语义一致匹配来校准预测。

**Result:** 在多个基准数据集上的实验结果表明，DAT方法在零样本分类任务中一致提升了最差组精度（worst-group accuracy）和平均精度（average accuracy），证明了密度感知平移作为一种简单有效的校准机制能够提升多模态模型的可靠性。

**Conclusion:** 本研究揭示了CLIP嵌入空间中的几何结构（各向异性壳与模态间隙）与虚假相关性之间的内在联系，并提出了一种无需训练或微调的密度感知校准方法，为提升多模态模型在零样本分类中的鲁棒性提供了新的有效思路。
...
### [76] [FlatVPR: Plug-and-play Geo-linear Residual Adapter for Geometric Rectification of Foundation Model Feature Manifolds](https://arxiv.org/abs/2606.01734)
*Rai Hisada, Kanji Tanaka*

#### 🧩 TL;DR
FlatVPR提出了一种基于几何校正的视觉位置识别范式，通过可学习的残差适配器和Pullback Flatness Loss显式抑制特征流形曲率，使得稀疏锚点间的描述子可通过线性插值精确重建，从而实现地图轻量化与定位精度的兼得。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉位置识别方法面临地图轻量化与定位精度之间的权衡困境：虽然基于基础模型（如DINOv2-ViT）的特征具有鲁棒的语义表达能力，但其潜在流形存在显著曲率，导致物理空间中的均匀线性运动在特征空间中投影为高度非线性的轨迹，从而在稀疏锚点条件下无法通过简单的线性插值实现描述子的可靠重建。

**Method:** FlatVPR引入一个可学习的残差适配器对原始基础特征进行变换，并设计了一种基于Pullback Flatness Loss的损失函数，从数学上最小化中间特征与相邻锚点间线性段的偏离程度，从而显式抑制流形曲率；随后将地图构建问题形式化为期望最大化（EM）框架，解耦为连续的M步（流形自适应）和概念性的E步（最优锚点选择准则）。

**Result:** 在NCLT数据集上的实验表明，即使在锚点间隔高达100米的极端稀疏条件下以及面临剧烈的季节变化，FlatVPR所提出的适配器仍然能够带来显著的性能提升，验证了该方法在极稀疏地图下保持高定位精度的有效性。

**Conclusion:** FlatVPR通过流形平坦化策略有效弥合了地图轻量化与定位精度之间的鸿沟，证明了对基础模型特征进行后处理以降低流形曲率是一种可行的途径；该工作为在资源和存储受限场景下部署高精度VPR系统提供了理论依据和实用框架。
...
### [77] [Sensitivity as a Double-Edged Sword: A Trade-off Between Discriminability and Adversarial Robustness](https://arxiv.org/abs/2606.01746)
*Kai Wang*

#### 🧩 TL;DR
本文揭示了全连接分类器的高敏感性是导致神经网络易受对抗攻击的重要原因，而ℓ2距离分类器虽具鲁棒性但性能受限；为此提出基于混合原型混合（HPM）框架的ℓ2重分类器，结合两种原型实现鲁棒性与判别性的平衡，并设计了混合代理攻击（MSA）评估协议以确保公平评测，实验表明该方法能有效增强现有对抗训练模型的鲁棒性。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究表明神经网络极易受到对抗扰动的影响，但其中根本原因尚未充分理解。本文发现全连接（FC）分类器的高敏感性虽然赋予了模型判别能力，但也使其对对抗扰动极为脆弱；而ℓ2距离分类器虽具有天然的鲁棒性，但其性能不足以单独使用。因此，如何在保留FC分类器判别能力的同时引入ℓ2分类器的鲁棒性成为了一个关键的研究问题。

**Method:** 本文提出基于混合原型混合（HPM）框架的ℓ2重分类器，融合两类原型进行ℓ2距离预测：一是通过指数移动平均（EMA）更新的稳定数据集级原型，二是利用直通估计器（STE）从FC分类器预测中生成的动态批量级原型。为解决STE架构带来的梯度混淆和前向不连续等评估难题，提出了混合代理攻击（MSA）评估协议，利用多个代理模型结合强大的AutoAttack确保公平且鲁棒的评估。

**Result:** 大量实验表明，所提出的轻量级即插即用模块仅需少量微调即可有效提升多种现有最先进对抗训练模型的鲁棒性。理论分析和实验验证了FC分类器的高敏感性导致脆弱性，而ℓ2分类器的低敏感性带来鲁棒性但限制性能，HPM框架在二者之间取得了有效的平衡。

**Conclusion:** 本文通过理论分析和实证研究揭示了FC分类器敏感性与脆弱性之间的内在关联，以及ℓ2分类器鲁棒性与性能之间的权衡关系。提出的HPM框架和MSA评估协议为提升模型对抗鲁棒性提供了新的思路和可靠评估手段，表明通过混合原型策略可以在不牺牲判别能力的前提下增强鲁棒性。
...
### [78] [EvoCut: Multi-Layer Evolution-Aware Visual Token Compression for Efficient Large Vision-Language Models](https://arxiv.org/abs/2606.01756)
*Hongyu Lu, Feng Zhang, Wenwei Jin, Huanling Hu, Pengfei Zhang, Yao Hu, Jiawei Li, Shikai Jiang*

#### 🧩 TL;DR
EvoCut提出了一种无需训练和无需注意力计算的视觉Token压缩方法，通过跨层视觉Token演化偏离度评估Token重要性，在LLaVA-1.5-7B上仅保留11.1%的Token即可保持94.4%的平均性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉Token压缩方法大多依赖特定层的注意力分数或表示属性来估计Token重要性，忽略了视觉Token在视觉编码器各层之间的演化动态，导致重要性估计不完整，压缩后性能损失较大。

**Method:** 通过对视觉编码器各层Token演化方向的分析，发现Token形成了多个组演化方向，且信息性Token会持续偏离公共组演化方向。基于此，提出了EvoCut方法，利用多层演化偏离度作为Token重要性的评估指标，无需训练和注意力计算即可实现视觉Token压缩。

**Result:** 在LLaVA-1.5-7B上，EvoCut仅保留11.1%的视觉Token即可达到94.4%的平均性能保持率，在效率与准确性之间实现了有效平衡。

**Conclusion:** 该研究揭示了视觉Token在多层的演化方向结构及其与信息性的关联，提出了利用跨层演化偏离度进行重要性估计的新视角，为高效LVLM推理提供了训练无关的轻量化压缩方案。
...
### [79] [PlatonicNav: Unveiling Semantic Correspondence in Navigation with Platonic Topological Maps](https://arxiv.org/abs/2606.01788)
*Junlin Long, Zeyu Zhang, Xu Deng, Yiran Wang, Yue Yang, Luke Borgnolo, Maxwell Twelftree, Yang Zhao*

#### 🧩 TL;DR
本文提出PlatonicNav框架，将Platonic表象假说扩展至具身导航，通过统一的自监督视觉编码器构建柏拉图拓扑地图，在无需跨模态训练或视觉-语言配对数据的情况下，同时支持纯视觉目标导航、跨模态目标导航和视觉-语言导航，并在多种仿真基准和真实机器人上验证了其泛化能力。

---

#### 📘 Detailed Summary
**Motivation:** 现有具身导航研究（如VLN和ObjNav）的融合停留在架构拼接和混合训练层面，未探究独立训练的视觉和语言编码器是否已共享公共语义结构；同时，基于目标中心拓扑地图的方法仍依赖CLIP或大型视觉-语言模型进行显式跨模态监督，尚不清楚纯视觉构建的地图能否完成语言目标的地面化。

**Method:** 本文将Platonic表象假说推广至具身导航，将纯视觉ObjNav、跨模态ObjNav和VLN统一视为同一目标中心语义流形的三种不同接口；提出PlatonicNav框架，其核心是Platonic拓扑地图，通过融合来自自监督视觉编码器的几何距离和语义节点距离构建地图，并通过盲匹配（blind matching）实现语言目标的地面化，完全无需配对视觉-语言数据。

**Result:** 在HM3D-IIN、OVON和R2R-CE（MP3D）等仿真基准以及Unitree Go2机器人上的实验表明，PlatonicNav无需显式跨模态训练即可在不同任务、模态和具身形态上实现泛化。

**Conclusion:** 该研究揭示了独立训练的视觉和语言编码器在具身导航任务中已共享公共语义结构，证明了纯视觉拓扑地图足以实现语言目标的地面化，为无需跨模态监督的通用导航框架提供了理论依据和实用方案。
...
### [80] [STaR-KV: Spatio-Temporal Adaptive Re-weighting for KV Cache Compression in GUI Vision-Language Models](https://arxiv.org/abs/2606.01790)
*Yuhang Han, Wenzheng Yang, Yujie Chen, Xiangqi Jin, Yaojie Zhang, Siteng Huang, Linfeng Zhang*

#### 🧩 TL;DR
提出STaR-KV，一种无需训练的KV缓存压缩框架，通过子空间感知评分、时间稳定性折扣和熵驱动温度重塑来压缩视觉语言模型GUI智能体的KV缓存，在匹配预算下达到SOTA平均准确率，峰值GPU内存降低近40%。

---

#### 📘 Detailed Summary
**Motivation:** 视觉语言模型驱动的GUI智能体在部署时面临KV缓存随交互步骤线性增长的瓶颈，例如UI-TARS-1.5-7B在仅处理五张截图时就消耗76 GB GPU内存，接近主流80 GB加速器的容量上限。现有KV压缩方法（如分配单一共享显著图、应用固定top-B截断）在结构上存在两个被实验推翻的假设：视觉token的重要性在注意力子空间层面存在空间特化且跨层迁移，而评分分布的形状沿轨迹发生漂移。

**Method:** 提出STaR-KV（Spatio-Temporal Adaptive Re-weighting），一种无需训练的KV缓存压缩框架，沿三个轴线校准token重要性：一是基于在线空间互信息的子空间感知评分；二是时间稳定性折扣，抑制来自持续被关注子空间的冗余缓存条目；三是熵导出的温度参数，自适应重塑评分分布。该方法不引入压缩阶段的FLOPs开销。

**Result:** 在四个GUI基准测试上，STaR-KV在匹配预算下达到了现有最优KV压缩方法（如GUIKV、SnapKV）中最强的平均准确率，压缩阶段FLOPs开销为-0.07%，在20% KV缓存预算下将峰值GPU内存降低近40%。

**Conclusion:** 该研究揭示了现有KV压缩方法中关于视觉token重要性聚合和评分分布截断的结构性假设缺陷，证明了注意力子空间层面的空间特化与评分分布漂移的存在。STaR-KV通过子空间感知、时间稳定性折扣和自适应温度三个维度的协同校准，在不增加计算开销的前提下实现了更优的KV缓存压缩效果，为GUI智能体的大规模部署提供了可行方案。
...
### [81] [Unsupervised Collaborative Domain Adaptation for Driving Scene Parsing](https://arxiv.org/abs/2606.01818)
*Jiahe Fan, Shaolong Shu, Mingjian Sun, Tiehua Zhang, Bohong Xiao, Hanli Wang, Rui Fan*

#### 🧩 TL;DR
本文提出一种无监督协作域自适应（UCDA）框架，在无需访问原始源域数据且不依赖单一预训练源模型的条件下，通过整合多个源模型的互补知识，实现面向自动驾驶场景解析的鲁棒目标域适应。

---

#### 📘 Detailed Summary
**Motivation:** 现有源无关无监督域自适应方法通常依赖单一预训练源模型，导致感知系统易受源域特定偏差影响，在多样的道路布局、光照条件、天气模式和交通状况下鲁棒性不足；同时像素级标注成本高昂且源域数据常因隐私或安全限制不可获取。

**Method:** UCDA框架首先构建类别级原型记忆库，通过原型相似度估计跨模型预测可靠性，以统一多个预训练源模型的置信度尺度差异；进而采用两阶段迁移策略：第一阶段利用正负一致性约束对多个源模型在无标注目标域数据上进行协同优化，第二阶段将验证后的专家知识蒸馏至单一可部署目标模型。

**Result:** 在公开驾驶场景数据集和真实自动驾驶平台采集数据上的综合评估表明，UCDA能够有效整合多源互补知识，提升目标域场景解析的可靠性和泛化能力。

**Conclusion:** 该工作表明通过无监督协作方式聚合多个源模型的互补知识，能够在源数据不可访问的条件下有效提升自动驾驶感知系统在开放动态环境中的适应性与鲁棒性，为源无关多源域自适应提供了可行范式。
...
### [82] [Hist2Style: Histogram-Guided Stylization with Bilateral Grids](https://arxiv.org/abs/2606.01819)
*Dekel Galor, Adam Pikielny, Zhoutong Zhang, Ke Wang, Laura Waller, Jiawen Chen, Ilya Chugunov*

#### 🧩 TL;DR
Hist2Style 提出一种基于双边网格的光照风格传递方法，通过蒸馏大图像编辑模型为轻量网络，结合直方图风格嵌入实现实时、高分辨率、可交互的光真实感风格迁移，避免幻觉并保持内容结构。

---

#### 📘 Detailed Summary
**Motivation:** 现有大图像模型虽能完成外观编辑，但计算开销大、易产生幻觉且用户控制能力有限，难以满足高分辨率实时工作流的需求。

**Method:** Hist2Style 采用双边网格（bilateral-grid）公式进行快速、边缘感知的色调迁移，将操作约束为双边空间中的局部仿射变换；通过蒸馏大图像编辑模型为轻量网络并在语言和视觉语言模型生成的大规模监督语料上训练；网络以基于直方图的风格目标嵌入为条件，提供可解释的用户交互界面。

**Result:** 该方法在保持内容结构的同时避免了幻觉，支持实时、高分辨率的光真实感风格化，并允许用户通过修改目标颜色分布进行交互式色调调整。

**Conclusion:** Hist2Style 通过双边网格约束和轻量蒸馏网络，在保持视觉保真度的前提下实现了可交互、无幻觉的实时光真实感风格传递，为高分辨率图像编辑提供了高效可控的解决方案。
...
### [83] [Hierarchically Decoupled Mixture-of-Experts for Robust Traffic Sign Recognition in Complex Driving Scenarios](https://arxiv.org/abs/2606.01822)
*Mingxiao Wang, Xiaozhen Qu, Bolin Gao, Tong Wang, Lei He*

#### 🧩 TL;DR
提出CBDES MoE TSR，一种分层解耦的异构混合专家框架，通过图像级动态路由机制按需激活最合适的YOLO专家模型，在交通标志检测任务上以76.8% mAP50-95超越基线2.3%，同时降低约39.4%的计算开销。

---

#### 📘 Detailed Summary
**Motivation:** 现有交通标志检测器大多依赖静态推理和全局共享参数，无法灵活适应多样化的非结构化交通场景，导致单个静态模型难以同时处理好近处清晰样本与远处小目标、恶劣天气等挑战性条件。

**Method:** 提出CBDES MoE TSR框架，采用分层解耦的异构混合专家结构，构建异构YOLO专家池并搭配轻量级门控网络，基于输入图像的语义特征实现图像级动态路由，选择性激活最适合当前场景的专家模型，从而将固定参数拟合转变为按需动态表征。

**Result:** 在复合交通标志数据集上，该方法达到76.8%的mAP50-95，相比基线方法（74.5%）提升2.3%，同时计算开销降低约39.4%，在检测精度与效率之间取得了显著平衡。

**Conclusion:** 该研究验证了在交通标志检测中引入动态路由与混合专家机制的有效性，表明按需激活专家模型能够在保持可控推理开销的同时显著提升特定场景下的特征提取能力，为面向复杂环境的自适应感知系统提供了可行方案。
...
### [84] [ROGLE: Robust Global-Local Alignment with Automated Region Supervision for Text-Based Person Search](https://arxiv.org/abs/2606.01825)
*Zequn Xie, Xibei Jia, Sihang Cai, Shulei Wang, Tao Jin*

#### 🧩 TL;DR
提出ROGLE统一框架，通过自动区域-句子匹配（RSM）策略和P-VLG基准数据集，解决CLIP等模型因全局表示偏差和语义稀疏导致的细粒度文本-行人检索对齐问题，在长文本查询任务上显著超越现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于CLIP的文本行人搜索模型由于在短描述上训练，存在全局表示偏差和语义稀疏性问题，导致细粒度对齐能力弱；同时，区域级标注的稀缺进一步加剧了这一问题，限制了模型对局部细节的理解。

**Method:** 提出ROGLE框架，核心包括自动区域-句子匹配（RSM）策略，通过挖掘伪区域-句子对提供可扩展的细粒度监督；采用多粒度学习策略，融合全局对比学习和区域级局部对齐；并构建P-VLG基准数据集，包含超10万标注区域和长文本描述，支持全局与局部评估。

**Result:** 大量实验表明ROGLE在文本行人搜索任务上显著优于现有方法，尤其在具有挑战性的长文本查询场景下表现突出；P-VLG成为首个同时支持全局和局部评估协议的TBPS基准数据集。

**Conclusion:** 自动化细粒度监督策略可有效替代昂贵的人工标注，全局-局部多粒度对齐是提升TBPS模型细粒度理解能力的关键方向；P-VLG基准为未来研究提供了更全面的评估平台。
...
### [85] [Polaris: Scaling Up Instruction-Guided Image Generation Towards Millions of Personalized Style Needs](https://arxiv.org/abs/2606.01858)
*Zhi-Kai Chen, Jun-Peng Jiang, Jun-Jie Tao, De-Chuan Zhan, Han-Jia Ye*

#### 🧩 TL;DR
Polaris是一个智能检索框架，可从包含6,500个checkpoints和75,000个适配器的大规模模型库中自动选择和集成最相关的模型组件，实现零训练、可扩展且对齐良好的图像生成与编辑。

---

#### 📘 Detailed Summary
**Motivation:** 传统图像生成模型依赖微调来适应个性化需求，成本高且难以扩展；尽管社区积累了大量微调模块和适配器组成的模型库，但缺乏系统性的方法来自动利用这些已有组件来满足用户指令，而不是重复训练新模型。

**Method:** Polaris提出一个智能检索框架，通过对超过6,500个checkpoints和75,000个适配器建立索引，根据用户输入的文本指令自动检索并集成最相关的模型组件，在无需任何额外训练的条件下实现指令驱动的图像生成与编辑。

**Result:** Polaris在包含6,500+ checkpoints和75,000+适配器的异构模型库上实现了高效的组件检索与集成，能够根据用户指令交付可扩展、可控且对齐良好的生成结果，且无需额外训练。

**Conclusion:** Polaris表明，通过智能检索而非重复训练，可以系统性利用社区积累的庞大模型生态系统来满足多样化的用户需求，为个性化图像生成提供了一种可扩展、低成本的新范式。
...
### [86] [Adversarial Attacks on Robot Localization Systems via Deep Feature Perturbation](https://arxiv.org/abs/2606.01892)
*Zhenyu Li, Tianyi Shang*

#### 🧩 TL;DR
本文提出了一种利用轻量级乘积量化网络（LPQN）生成对抗查询以攻击视觉定位系统中乘积量化（PQ）模块的框架，通过前后向优化过程产生细微但高效的扰动，显著降低定位系统性能，揭示了深度学习定位管道在对抗攻击下的严重脆弱性。

---

#### 📘 Detailed Summary
**Motivation:** 基于深度学习的机器人定位系统在自主导航中至关重要，但现有研究对其在面对对抗性扰动时的脆弱性探究不足，尤其是针对定位管道中核心的乘积量化（PQ）检索模块的对抗攻击尚缺乏系统性研究，这可能导致严重的安全隐患。

**Method:** 本文提出了一种两阶段对抗查询生成框架：前向阶段通过轻量级乘积量化网络（LPQN）扰动查询特征分布，后向阶段通过优化过程精炼扰动；LPQN以极小的计算开销生成细微而高效的对抗扰动，专门用于误导视觉定位系统中PQ的检索过程，使其返回语义无关的数据库条目。

**Result:** 在受控环境和真实机器人场景中的大量实验表明，所提方法能够显著降低乘积量化网络（PQN）的定位性能，有效暴露了深度学习定位管道在实际应用中的关键脆弱性。

**Conclusion:** 本研究揭示了基于深度学习的视觉定位系统在面对针对性对抗攻击时的严重安全缺陷，强调在任务关键型机器人应用中必须重视定位管道的对抗鲁棒性，为后续防御机制的研究提供了重要方向。
...
### [87] [Train, Test, Re-evaluate: Schedule-Sensitive Evaluation of Generative Data for Hand Detection](https://arxiv.org/abs/2606.01896)
*Atmika Bhardwaj, Silvia Vock, Nico Steckhan*

#### 🧩 TL;DR
本文验证了生成式修补（inpainting）在手部检测中通过合成配件（如手套、饰品）缩小学徒数据与真实部署之间的分布偏移，并提出两阶段与三阶段训练策略有效提升真实场景检测精度。

---

#### 📘 Detailed Summary
**Motivation:** 现有公开手部检测数据集主要包含裸手图像，对手套、纹身、珠宝等配件引入的手部外观变化覆盖不足，导致在职业安全等关键应用场景中出现分布偏移，影响模型部署后的泛化性能。

**Method:** 采用生成式修补技术仅对真实照片中的手部区域进行编辑以添加配件，构建配对数据集；基于YOLOv8n训练手部检测器，设计六种训练与调度方案（实验A-F，各三个随机种子），包括在两阶段实验中使用真实数据微调合成预训练权重，以及三阶段实验保留框紧密度。

**Result:** 两阶段训练（先合成后真实低学习率微调）在标准真实测试集上相比纯真实基线提升mAP@0.5，并缩小了真实手套测试集上的分布外差距；三阶段训练取得全实验最高mAP@0.5:0.95，最佳保持了检测框紧密度。

**Conclusion:** 合成数据在手部检测中的效用高度依赖训练流程设计，通过简单的多阶段训练策略即可从修补生成的配件数据中提取显著的现实部署收益，为安全关键场景下的数据增强提供了实用方案。
...
### [88] [Residual Decoder Adapter: ID-Preserving Tokenizer Adaption for Autoregressive Text Rendering](https://arxiv.org/abs/2606.01911)
*Dongxing Mao, Jinpeng Wang, Jiahao Tang, Kevin Qinghong Lin, Linjie Li, Zhengyuan Yang, Lijuan Wang, Min Li, Jingru Tan*

#### 🧩 TL;DR
提出残差解码器适配器（RDA），在不重新训练现有视觉分词器和自回归模型的前提下，通过引入配对码本和平行分支学习像素级残差，显著提升视觉自回归模型的文本渲染能力，在TextAtlas基准上将OCR准确率提升至原有水平的2-3倍。

---

#### 📘 Detailed Summary
**Motivation:** 视觉自回归（AR）模型在图像生成方面表现强劲，但在文本渲染任务中普遍存在笔画模糊、字母形状失真等问题，主要原因是底层视觉分词器难以重建细粒度细节。直接改进分词器虽有效但代价高昂，需要同时重训练分词器和AR模型。因此，本文旨在不重新训练现有分词器和AR模型的前提下，提升AR模型的文本渲染性能。

**Method:** 本文提出残差解码器适配器（RDA），是一种后置式（post-hoc）升级现有视觉分词器的轻量组件，不改变原有token空间。RDA包含两个核心模块：（i）配对码本（paired codebook），与原码本共享token分布；（ii）并行分支（parallel branch），在像素空间中学习重建图像与真实图像之间的微小差异（残差）。该残差设计以非侵入方式增强分词器解码能力，同时保持与已有AR模型完全兼容。

**Result:** RDA在TextAtlas基准上显著提升了AR模型的文本渲染性能。以微调后的Janus-Pro为例，OCR准确率在TextVisionBlend子集上从24.52%提升至58.26%，在StyledTextSynth子集上从12.75%提升至36.81%，提升幅度超过2-3倍。

**Conclusion:** 本文证明了视觉分词器的解码能力是制约AR模型文本渲染性能的关键瓶颈，并验证了通过后置残差增强机制可以在不改动原始token空间和AR模型的前提下有效突破该瓶颈。RDA提供了一种高效且兼容性强的解决方案，为后续提升AR模型细粒度生成质量提供了新思路。
...
### [89] [Unified Driving Tokens: Representation- and Geometry-Guided Discrete Tokenizer for Driving World Models and Planning](https://arxiv.org/abs/2606.01935)
*Ziyang Yao, Zeyu Zhu, YunCheng Jiang, Zibin Guo, Huijing Zhao*

#### 🧩 TL;DR
本文提出一种结合表示引导与几何增强的离散视觉分词器，通过在离散瓶颈层对齐DINO特征空间、联合深度与相对位姿监督以及多码本量化，使学习到的离散令牌同时适用于世界建模和自动驾驶规划，在NAVSIM上取得优于传统分词器的重建保真度、表示一致性和规划性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有自动驾驶中的离散视觉分词器大多直接继承自图像生成任务，优化目标以像素重建为主，导致分词器倾向于生成易于解码的令牌而非对驾驶决策有用的令牌，在令牌级世界建模和规划之间存在表征鸿沟。

**Method:** 提出表示引导与几何增强的分词器，在离散瓶颈层通过特征解码与冻结的DINO特征空间对齐，同时使用感知损失和对抗损失进行RGB外观重建；引入相邻帧深度和相对位姿监督以注入几何状态信息，并采用多码本量化来稳定联合优化目标。

**Result:** 在NAVSIM基准上，所提方法在重建保真度和表示一致性上优于基线；在固定解码器设置下规划性能具有竞争力，在相同配置下生成质量更优。

**Conclusion:** 通过特征空间对齐和几何监督联合训练离散令牌，能够弥合像素重建与驾驶决策之间的表征差距，使得同一套令牌可同时有效支持轻量规划读取和GPT式自回归世界模型，为自动驾驶中的令牌化表征学习提供了新方向。
...
### [90] [SAVMap: Structure-Aided Visual Mapping of Large-Scale 2.5D Manhattan Wireframes from Panoramic Video](https://arxiv.org/abs/2606.01939)
*Howard Huang, Bharath Surianarayanan, Keifer Lee, Chenyu Wang, Chen Feng*

#### 🧩 TL;DR
SAVMap 提出一种仅利用全景视频相机输入、通过语义分割网络提取稀疏结构特征点并融合曼哈顿网格几何约束的结构光流算法，为仓库货架与灯光结构生成精确的语义线框地图，在46排货架上实现了4.8 cm的平均绝对误差。

---

#### 📘 Detailed Summary
**Motivation:** 工业环境中精确的三维表示对于机器人定位和数字孪生生成等任务至关重要，但现有方法通常依赖激光雷达等多传感器方案，成本高且部署复杂；本文旨在仅使用低成本的全景视频相机作为唯一传感器，实现仓库场景中货架与灯光结构的语义线框地图构建。

**Method:** SAVMap 首先从沿仓库通道采集的全景视频中提取经过校正的货架朝向和天花板朝向图像序列；然后利用语义分割网络从每帧图像中提取稀疏的语义结构特征点（如货架角点、灯中心点），并在序列间进行跟踪；最后，通过引入曼哈顿网格等真实世界几何关系约束，设计一种带约束的结构光流算法，将这些2D特征点重建为构成线框地图的3D点。

**Result:** 在包含46排货架（每排货架面尺寸55 m×7 m）的仓库中，利用一小时的全景视频数据，为超过5000个货架元素生成了线框地图，相对于地面真值的累计平均绝对误差仅为4.8 cm，验证了该方法在大规模场景下的可扩展性和精度。

**Conclusion:** 本文证明仅依靠全景视频相机和几何约束即可在大规模工业仓库场景中实现高精度的语义线框地图构建，为低成本的机器人定位与数字孪生应用提供了可行方案；该工作为后续在更复杂工业环境中融合语义与几何信息的稀疏地图构建研究奠定了基础。
...
### [91] [Beyond Low-Rank: Low-Rank Sparse Prompting via Spiking Neural Network and Prompt Factorization](https://arxiv.org/abs/2606.01945)
*Yumiao Zhao, Bo Jiang, Beibei Wang, Xixi Wan, Xiao Wang, Jin Tang*

#### 🧩 TL;DR
本文提出LoRSP（Low-Rank Visual Spike Prompting），一种融合脉冲神经元机制的视觉提示学习框架，通过低秩分解与脉冲发放机制生成实例级稀疏视觉提示，在多个视觉骨干网络和基准上以更少的可调参数取得了具有竞争力的性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉提示方法通常采用稠密的像素级提示，存在冗余扰动多、泛化能力有限以及能效低的问题，亟需一种更紧凑、高效且鲁棒的提示学习范式。

**Method:** LoRSP利用低秩分解构造一系列提示因子以捕捉不同的提示子空间，并将这些因子输入到脉冲神经网络（SNN）中，通过 integrate-and-fire 机制发放离散脉冲，从而自然地生成具有低秩约束的稀疏视觉提示，实现实例级的选择性提示。

**Result:** 在五个异构视觉骨干网络和多个基准数据集上的大量实验表明，LoRSP在取得竞争性性能的同时，所需的可调参数数量少于现有的视觉提示方法。

**Conclusion:** 脑启发的脉冲学习机制可有效用于视觉提示学习，通过生成稀疏、低秩的提示实现更紧凑和鲁棒的下游任务迁移，为高效视觉适配提供了新思路。
...
### [92] [Parameter-Efficient Fine-Tuning of Large Pretrained Models for Instance Segmentation Tasks](https://arxiv.org/abs/2606.01947)
*Nermeen Abou Baker, David Rohrschneider, Uwe Handmann*

#### 🧩 TL;DR
本研究首次系统性地探索了参数高效微调（PEFT）方法（适配器与LoRA）在基于Transformer的实例分割任务中的应用，仅微调约1-6%的模型参数即可达到与传统微调（40-55%参数）相当的竞争性能，其中每个Transformer块使用2-3个适配器以及将LoRA应用于可变形注意力机制是关键发现。

---

#### 📘 Detailed Summary
**Motivation:** 尽管大型预训练模型在众多任务中取得了最先进的结果，但其参数规模急剧增长，亟需参数高效的训练策略。然而，在基于Transformer的实例分割模型中，针对参数高效微调（PEFT）方法的系统性研究尚属空白，这一研究缺口是本文试图填补的核心问题。

**Method:** 本研究系统探究了两种PEFT方法——适配器（Adapter）和低秩自适应（LoRA）——在基于Transformer的实例分割模型中的应用。具体而言，在模型中集成按序排列的适配器模块，并将LoRA首次应用于可变形注意力机制（Deformable Attention），在两个模型和四个基准数据集上进行了实验验证。

**Result:** 实验表明，所提方法仅微调约1-6%的模型参数即可达到竞争性能，而传统微调需要更新40-55%的参数。关键发现包括：每个Transformer块使用2-3个适配器可实现性能与效率的最佳平衡；LoRA应用于可变形注意力时展现出强劲的参数效率，在某些配置下甚至超越适配器方案。PEFT技术的效果因数据集复杂度和模型架构差异而不同。

**Conclusion:** 本研究证明了PEFT方法在实例分割任务中实现可扩展、可定制且计算高效的迁移学习的巨大潜力，强调了根据具体上下文（数据集和模型架构）选择适配微调策略的重要性。
...
### [93] [MT-EditFlow: Reinforcement Learning for Multi-Turn Image Editing with Flow Matching](https://arxiv.org/abs/2606.01985)
*Jiahui Huang, Yasi Zhang, Tianyu Chen, Shu Wang, Jianwen Xie, Oscar Leong, Mingyuan Zhou, Nanzhu Wang, Ying Nian Wu*

#### 🧩 TL;DR
提出MT-EditFlow，一个基于流匹配强化学习的框架，通过多轮视角与多奖励公式优化序列图像编辑中的奖励信号，有效解决多轮编辑中的单轮失败传播和曝光偏差问题，在FLUX.1-Kontext-dev上提升turn-3整体性能6.85分，超越Qwen-Image-Edit等开源模型。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于指令的图像编辑模型主要为单轮编辑设计，在多轮交互编辑场景中会因单轮失败导致整个序列失效（全有或全无问题），以及曝光偏差导致错误累积传播，缺乏针对多轮序列编辑优化的方法。

**Method:** 提出MT-EditFlow框架，采用流匹配强化学习，融合多轮视角与多奖励公式，提供统一结构兼容GRPO和无NFT强化学习方法；通过回合级聚合策略、VLM推理模式权衡奖励偏差与方差、优势融合级别防止奖励篡改等机制系统优化奖励信号，并将聚合优势广播至整个编辑轨迹以弥合局部规划与全局任务成功之间的差距。

**Result:** 实验表明MT-EditFlow在多种基础模型上显著提升多轮编辑性能，将FLUX.1-Kontext-dev的turn-3整体性能提升6.85分，超越Qwen-Image-Edit等当前最优开源模型，同时保持了高边际成功率和降低曝光偏差。

**Conclusion:** MT-EditFlow通过将聚合优势广播至整个编辑轨迹有效弥合局部规划与全局任务成功之间的差距，为更可靠、更自然的视觉内容创作人机协作奠定了基础，表明多轮强化学习优化是解决序列图像编辑中错误传播问题的有效途径。
...
### [94] [A Structured Benchmark for Text-Guided Anomaly Detection: When Language Stops Conditioning the Decision](https://arxiv.org/abs/2606.01992)
*Stefano Samele, Eugenio Lomurno, Teodora Jovanovic, Sanjay Shivakumar Manohar, Alberto Crivellaro, Matteo Matteucci*

#### 🧩 TL;DR
本文指出当前多模态异常检测系统在标准基准下的表现被高估，提出TGAD基准测试来系统评估语言对决策的引导能力，发现现有模型仅在表层受文本条件影响，语言控制的可靠性远低于报告结果。

---

#### 📘 Detailed Summary
**Motivation:** 现有多模态异常检测方法虽宣称支持文本引导的零样本和少样本检测，但其评估协议继承自单模态基准，文本条件保持恒定，无法衡量语言是否真正影响决策，所报告的性能提升无法区分是来自文本引导还是预训练视觉特征，存在被高估的风险。

**Method:** 本文提出Text-Guided Anomaly Detection (TGAD)基准，通过三个逐步增加语言功能角色的场景进行评估：MVTec AD上的受控提示敏感性设置、基于组件标签扩展的MVTec AD（要求模型仅评估指定部件）、以及新构建的Assembled Panel Dataset (APD)（需同时具备缺陷类型和组件位置知识），并对生成式大视觉语言模型、免训练判别式模型和嵌入自适应判别式模型三类代表性模型进行评估。

**Result:** 实验表明三类模型的文本接口仅产生表层条件作用：生成式模型在移除物体名词后I-AUROC从97.4降至82.6；组件级指令在部件外缺陷被当作正常后AUROC从90.3降至66.3；APD上图像级判别能力崩溃至低于MVTec水平，某模型降至31.5（低于随机）。

**Conclusion:** 标准基准夸大了当前多模态异常检测系统的文本引导能力，语言对决策的控制极为有限；基于TGAD的协议是实现可通过语言可靠控制的工业部署模型的必要前提。
...
### [95] [Distortion-Aware Fusion of Statistical and Vision-Language Features for Blind Image Quality Assessment](https://arxiv.org/abs/2606.02002)
*Bishr Omer Abdelrahman Adam, Xu Li*

#### 🧩 TL;DR
提出一种基于乘性门控机制的失真感知融合框架，将自然场景统计描述符与SigLIP、CLIP-H两种视觉语言模型嵌入进行自适应加权融合，在无需微调VLM骨干的条件下，在KADID-10k等多个BIQA基准上取得超越现有方法的性能，并揭示了不同模态特征对不同失真类型的贡献规律。

---

#### 📘 Detailed Summary
**Motivation:** 经典自然场景统计描述符和现代视觉语言模型嵌入从根本不同的角度解决无参考图像质量评估问题，但两者组合是否能带来互补增益，以及如何针对每张输入图像自适应地权衡它们各自的贡献，这一问题尚未被探索。

**Method:** 提出一种失真感知融合框架，集成138维NSS描述符与SigLIP、CLIP-H两种互补的VLM嵌入，通过一个基于图像内容条件化的乘性门控机制学习每个输入流的权重，而非采用静态拼接融合；框架无需对VLM骨干进行端到端微调，训练时采用均方误差、皮尔逊线性相关和成对排序目标组成的混合损失函数。

**Result:** 在KonIQ-10k上达到SROCC=0.9142、PLCC=0.9279；在KADID-10k上达到SROCC=0.9715、PLCC=0.9733，超越近期最先进方法；在LIVE Challenge上通过跨数据集预训练和微调达到SROCC=0.8527、PLCC=0.8802；门控学习到的权重与独立消融实验测得的逐失真NSS贡献呈正相关（斯皮尔曼等级相关系数ρ=0.33），且模型自动发现失真-流亲和性模式，即NSS特征对噪声和颜色偏移类失真贡献最大，对色彩饱和度变化等感知失真贡献最小。

**Conclusion:** 该研究验证了NSS与VLM嵌入在BIQA任务上具有互补性，自适应门控融合优于静态拼接，且模型能够自主发现与人工分析一致的失真-特征流亲和性模式，为可解释的多模态质量感知融合提供了新思路。
...
### [96] [Normality-Preserving Continual Industrial Anomaly Detection via Orthogonal LoRA Banks](https://arxiv.org/abs/2606.02042)
*Weibai Fang, Haijun Che, Feiyang Ren, Qiancheng Lao*

#### 🧩 TL;DR
提出基于历史冻结正交LoRA库（HF-OLB）和层次化新颖性自适应库增长（HNABG）的持续工业异常检测框架，通过正交约束和新颖性驱动的库扩展机制有效防止灾难性遗忘，在MVTec和VisA基准上达到SOTA性能。

---

#### 📘 Detailed Summary
**Motivation:** 持续工业异常检测中的扩散模型面临历史正常性先验漂移和灾难性遗忘问题，现有基于回放或约束优化的持续扩散方法缺乏显式机制来隔离和保护任务特定的正常性先验，标准LoRA既无法冻结历史正常性子空间，也无法阻止新适配器干扰已有适配器。

**Method:** 提出基于两个模块的正常性保持持续异常检测框架：History Frozen Orthogonal LoRA Bank (HF-OLB) 冻结预训练U-Net主干网络和已学习的LoRA库，并将新任务特定正常性残差约束到历史LoRA子空间的正交补空间上；Hierarchical Novelty Adaptive Bank Growth (HNABG) 根据层级分配残差容量，仅当残差正常性新颖性超出已有库的表达能力时才扩展库。

**Result:** 在MVTec和VisA数据集上的实验验证了方法的有效性，在VisA 2x6设定下达到图像级和像素级A-AUROC分别为83.6/91.8，FM分别为3.8/3.9，像素级A-AUROC较SOTA提升3.2个点，像素级FM降低1.3。

**Conclusion:** 该方法通过显式的正交约束和新颖性驱动的库增长机制，有效保留了长期持续类别序列中的历史正常性先验，为解决持续异常检测中的灾难性遗忘提供了新的正交子空间隔离范式。
...
### [97] [TIDES: Time-Derivative Event Simulation via Deformable Reconstruction](https://arxiv.org/abs/2606.02058)
*Christopher Thirgood, Dipon Kumar Ghosh, Simon Hadfield*

#### 🧩 TL;DR
TIDES提出了一种基于动态高斯溅射的连续时间事件模拟器，直接从3D场景表示中推导逐像素强度动态以进行精确的阈值穿越预测，并利用显式遮挡引导自适应时间步进，同时通过瓦片级仲裁器建模传感器非理想效应，在事件流保真度和下游任务迁移性上达到了最先进水平。

---

#### 📘 Detailed Summary
**Motivation:** 现有事件相机模拟器大多从帧序列推断事件时间戳，导致大量阈值穿越共享少量离散时间，产生所谓的"时间戳批处理"失效模式，该问题在快速运动和遮挡条件下尤为严重；此外，真实世界事件数据集稀缺，因此亟需能够产生高保真连续时间事件的模拟方法。

**Method:** TIDES基于动态三维高斯溅射构建显式3D场景表示，直接从场景的几何与运动信息推导逐像素强度动态曲线以预测精确阈值穿越时间，支持单渲染步骤内多次穿越而无需时间上采样或帧插值；利用显式3D场景中的部分遮挡信息引导自适应时间步进策略，仅在遮挡动态导致简单亮度变化模型不可靠的区域集中计算资源；此外，采用瓦片级仲裁器建模有限传感器带宽，再现吞吐量、抖动和事件丢包等真实传感器伪影。

**Result:** 在成对RGB-事件基准测试上，TIDES达到了事件流保真度的最优水平；进一步实验表明，TIDES模拟的事件在真实下游任务中的迁移效果优于同类竞争者。

**Conclusion:** TIDES通过连续时间建模、显式场景引导计算分配和传感器非理想效应建模，从根本上克服了传统帧驱动事件模拟的时间戳批处理问题，为高保真事件数据生成提供了有效方案，并验证了模拟数据向真实任务的良好迁移能力。
...
### [98] [FACT: A Simple and Efficient Framework for Active Finetuning](https://arxiv.org/abs/2606.02079)
*Wenshuai Xu, You Song, Yuzhuo Cui, Minjie Ren, Qingjie Liu, Zhenghui Hu*

#### 🧩 TL;DR
本文提出FACT，一个面向主动微调场景的三阶段层次化微调框架，通过系统探索微调方法而非仅关注数据选择，在多种数据集和架构上取得显著性能提升，尤其在低采样率下对ViT模型实现超过20%的增益。

---

#### 📘 Detailed Summary
**Motivation:** 现有主动微调研究过度聚焦于数据选择（主动学习方面），而统一采用全量微调进行模型适配，这会导致预训练特征因分布偏移而被扭曲，当模型规模相对微调数据量较大时，过拟合风险尤为突出。该研究旨在填补这一关键空白，系统探索主动学习中的微调方法。

**Method:** 论文正式定义了FiAF任务，并提出FACT框架，这是一个三阶段层次化微调框架，兼具高效性与简洁性，专门针对主动微调场景设计。实验涵盖经典（CIFAR10、CIFAR100、ImageNet-1k）、不均衡（CIFAR10-LT、CIFAR100-LT）和细粒度（StanfordCars、FGVCAircraft）三大类图像分类数据集，采用ConvNeXt、ViT和ViL等多种预训练架构，并系统研究了冻结特征增强（FroFA）策略。

**Result:** 实验结果表明，FACT框架在多种设定下均取得了显著改进，具备强泛化性和鲁棒性。在低采样率下，该框架在CIFAR10、CIFAR100和ImageNet-1k基准上对ViT模型实现了超过20%的性能提升，同时保持了参数效率，在标注数据稀缺时尤为有效。

**Conclusion:** 该研究通过系统化方法建立了新的最先进性能，证明了在主动微调中探索合适的微调方法比单纯关注数据选择更为关键，为标签稀缺场景下的模型适配提供了有效范式。
...
### [99] [Understanding-Enhanced Model Collaboration for Long-Tailed Egocentric Mistake Detection](https://arxiv.org/abs/2606.02120)
*Boyu Han, Qianqian Xu, Shilong Bao, Zhiyong Yang, Ruochen Cui, Qingming Huang*

#### 🧩 TL;DR
本文提出了一种理解增强的模型协作方法（UE-MCM），结合小型模型（基于CLIP4CLIP与扩散对比重建）的粗粒度视频理解与大型模型（基于Qwen3-VL Embedding）的细粒度动作推理，通过自适应融合门和互补优化目标，高效检测自我中心视频中用户动作执行的错误。

---

#### 📘 Detailed Summary
**Motivation:** 现有方法在检测自我中心视频中用户动作执行错误时，难以同时兼顾全局工作流一致性与细粒度动作正确性，且错误实例存在长尾分布，导致对细微、罕见和模棱两可的错误检测效果不佳。

**Method:** UE-MCM包含小型模型分支和大型模型分支：小型分支基于经扩散对比重建增强的CLIP模型初始化的CLIP4CLIP视频编码器，联合粗粒度视频与细粒度片段作为输入，检测局部正确但整体不一致的动作；大型分支使用Qwen3-VL Embedding模型从细粒度动作片段提取高容量表征，判断动作执行本身的错误。两分支预测通过轻量级协作门自适应融合，并使用加权重交叉熵、AUC导向学习和标签感知调整等互补目标优化分类器以应对长尾分布。

**Result:** 实验表明，UE-MCM在自我中心教学视频的错误检测任务上实现了速度与精度的平衡，能够有效检测细微、罕见和模棱两可的动作执行错误。

**Conclusion:** 本文提出的双分支协作框架通过粗粒度与细粒度信息的互补融合，结合面向长尾分布的优化策略，为自我中心视频中动作错误检测提供了一种高效且准确的解决方案。
...
### [100] [Order within Chaos: Capturing Intrinsic Energy Anomalies for AI-Manipulated Image Forgery Localization](https://arxiv.org/abs/2606.02178)
*Yiming Wang, Baiqi Wu, Qingming Li, Jiahao Chen, Tong Zhang, Shouling Ji*

#### 🧩 TL;DR
提出FLAME框架，利用扩散过程固有的统计能量间隙（LAD图）结合参数高效的SAM适配器，实现像素级伪造定位，并引入EditStream自动化数据合成管道，在AI生成伪造图像检测上达到新最优。

---

#### 📘 Detailed Summary
**Motivation:** 现有图像伪造定位方法依赖物理噪声特征，无法有效检测生成式AI生成的逼真伪造图像，数据合成管道也滞后于快速演进的生成模型。

**Method:** 理论上证明扩散过程会抑制局部高频方差，产生可区分的统计能量间隙；基于此提出FLAME框架，利用LAD图捕获异常，结合参数高效SAM适配器实现像素级伪造定位；同时提出EditStream自动化管道，持续生成指令式训练数据。

**Result:** 在AI生成伪造数据集上显著超越此前方法达到新最优，并能有效泛化至未见过的生成架构。

**Conclusion:** 验证了扩散过程内在统计能量间隙作为伪造检测特征的可行性，提出统一框架有效提升了泛化能力，并提供开源工具以推动领域发展。
...
### [101] [Ego-METAS: Egocentric online Multimodal Energy-efficient Temporal Action Segmentation benchmark](https://arxiv.org/abs/2606.02246)
*Maria Santos-Villafranca, Jesus Bermudez-cameo, Alejandro Perez-Yus, Giovanni Maria Farinella, Antonino Furnari*

#### 🧩 TL;DR
Ego-METAS 是首个面向在线、多模态、节能时序动作分割的基准测试，包含超过100小时的第一人称视频和五种传感器模态，要求模型在严格能量预算下动态选择传感器，为资源受限的具身感知提供了标准化评估平台。

---

#### 📘 Detailed Summary
**Motivation:** 具身智能体在物理世界中需以“始终在线”方式感知环境，但资源受限设备上的节能感知研究严重不足，现有工作大多假设无限计算资源，缺乏衡量在线、多模态、能量约束下感知性能的统一基准。

**Method:** 作者构建了Ego-METAS基准，整合来自EgoExo4D、CMU-MMAC和CaptainCook4D数据集的总计超过100小时未修剪第一人称视频，涵盖RGB、音频、注视、IMU和单色相机五种模态；该基准定义了一个在线时序动作分割任务，要求模型在每个时间步动态选择激活哪些传感器，并严格遵循硬件可比的能量预算；同时发布了统一划分、清理标注、预提取特征及一系列基线路由策略。

**Result:** 实验表明，最优传感器路由策略高度依赖具体场景，现有的主要为修剪片段设计的策略学习方法在连续未修剪环境中表现不佳；但即使简单的模态动态融合（如随机路由）也对平衡预测精度与严格能量预算至关重要。

**Conclusion:** Ego-METAS为开发面向自主、始终在线具身AI的鲁棒、代价感知策略提供了标准化基础，揭示了在线、未修剪场景下节能感知研究的关键挑战与机遇。
...
### [102] [Vision-language Models for Driver Monitoring Systems: A Driver Activity Description Dataset](https://arxiv.org/abs/2606.02273)
*David J. Lerch, Sarath Mulugurthi, Manuel Martin, Frederik Diederichs, Rainer Stiefelhagen*

#### 🧩 TL;DR
本文构建了Drive&Act描述数据集，将VLMs微调用于细粒度驾驶员行为理解，在ACCR指标上从66提升至76，并验证了跨数据集泛化能力。

---

#### 📘 Detailed Summary
**Motivation:** 现有的视觉语言模型在通用数据集上训练，难以准确识别驾驶员行为中的细微动作差异，缺乏针对细粒度驾驶员活动描述的专用数据集和评估基准。

**Method:** 基于Drive&Act数据集构建包含细粒度自然语言描述的Drive&Act描述数据集，采用三种VLMs进行评估，使用基于LLM的评分方法衡量性能，并对VLM进行微调训练，最后在Driver Monitoring Dataset上进行跨数据集评估验证泛化性。

**Result:** 微调后的VLM在ACCR评分上达到76，显著优于零样本基线模型的66；跨数据集评估表明模型能够良好泛化到DMD中的动作类别。

**Conclusion:** 通过丰富描述的驾驶员动作数据微调VLMs可以显著提升其对驾驶员行为的理解能力，同时研究成果揭示了当前数据集的多样性不足，未来需要更广泛的数据支持更好的泛化。
...
### [103] [Cross-Domain Dead Tree Detection via Knowledge Distillation in Aerial Imagery](https://arxiv.org/abs/2606.02303)
*Anis Ur Rahman, Mete Ahishali, Einari Heinaro, Samuli Junttila*

#### 🧩 TL;DR
本研究提出基于知识蒸馏（KD）的域适应方法，将TreeMort-1T-UNet模型从芬兰航拍图像迁移至波兰、德国、爱沙尼亚等多国数据集，其中特征级KD在低数据场景下表现最优，实现了跨域树冠检测精度与表示不变性的显著提升。

---

#### 📘 Detailed Summary
**Motivation:** 航拍图像中枯死树检测对于评估森林健康至关重要，但全球气候变化导致树木死亡率上升，而域差异大和标注数据稀缺严重限制了模型在不同地理区域和森林类型间的泛化能力，亟需有效的域适应策略以提升模型的可迁移性。

**Method:** 以预训练于芬兰航拍图像（源域）的TreeMort-1T-UNet模型为基础，采用知识蒸馏（KD）技术将其适配至波兰、德国、爱沙尼亚等目标域数据集，系统比较了Basic KD、Self KD、Feature-level KD和Ensemble KD四种变体，并以微调作为基线；评估指标包括Mean Tree IoU、Instance F1-score、Instance Precision和Mean Centroid Error，同时利用余弦相似度、CKA、SSIM、t-SNE和线性探测等表示分析方法衡量域不变性。

**Result:** Feature-level KD在所有变体中表现最佳，在波兰数据集上达到Mean Tree IoU 0.106、Instance F1-score 0.63、Instance Precision 0.55、Mean Centroid Error 3.039，并在其他目标域上保持稳健精度（芬兰0.15、波兰0.67、德国0.60、爱沙尼亚0.59）；其在低数据场景下假阳性更少，表现出优异的表示不变性（深层CKA/SSIM更高、t-SNE域混合更好、线性探测AUC达0.95）。

**Conclusion:** 知识蒸馏有效增强了遥感领域模型迁移学习的效果，特征级KD特别适用于对精度要求高的林业应用，消融实验进一步确认特征对齐等关键组件对性能平衡有重要贡献，为生态监测和可持续森林管理提供了可扩展、域鲁棒的工具。
...
### [104] [Training-Free Composed Video Retrieval via Visual Representation-Guided Video-LLM Reasoning](https://arxiv.org/abs/2606.02321)
*Yang Liu, Qianqian Xu, Peisong Wen, Siran Dai, Qingming Huang*

#### 🧩 TL;DR
提出一种无需训练的视觉表示引导视频-LLM推理框架，通过DINOv3筛选候选视频并结合大视觉语言模型进行指令评估与推理优化，在CVPR 2026挑战赛中取得48.78 Recall@1和51.48 Recall@5的成绩。

---

#### 📘 Detailed Summary
**Motivation:** 现有视频检索技术已从简单文本搜索扩展至更灵活的场景，用户可通过视觉示例和文本指令共同指定检索目标，但如何有效结合视觉表示与语言推理来实现无需训练的组成式视频检索仍是一个待解决的问题。

**Method:** 首先利用冻结的DINOv3模型获取一组紧凑的视觉相关性候选视频，然后使用大视觉语言模型评估每个候选视频是否满足修改指令，最后对排名靠前的候选视频执行基于推理的精细化调整以提升首位预测的准确性，整个框架无需训练。

**Result:** 在CVPR 2026 Reason-Aware Composed Video Retrieval Challenge测试集上，系统无需训练即达到48.78的Recall@1和51.48的Recall@5。

**Conclusion:** 实验表明基于视觉表示引导的视频-LLM推理方法可在无训练条件下有效完成组成式视频检索任务，未来可通过更强的视频-LLMs以及视觉表示与语言推理的细粒度集成进一步提升检索精度。
...
### [105] [Multi-modal Video Representation Alignment for Robust Self-supervised Driver Distraction Detection](https://arxiv.org/abs/2606.02352)
*David J. Lerch, Livien Majer, Zeyun Zhong, Manuel Martin, Frederik Diederichs, Rainer Stiefelhagen*

#### 🧩 TL;DR
提出一种鲁棒的自监督多模态全局对齐框架，通过循环一致性软目标和基于相似度分布的加权机制分别处理错误负样本和不可靠正样本，在Drive&amp;Act数据集上显著优于现有基线，实现可靠的驾驶员分心检测。

---

#### 📘 Detailed Summary
**Motivation:** 现有对比学习目标如InfoNCE假设所有负样本等量信息且所有正样本可靠，但在多模态视频数据中，由于视角变化、遮挡或模态间语义重叠，这一假设频繁被违背，导致多模态表征学习的鲁棒性不足。

**Method:** 提出多模态全局对齐框架：利用循环一致性得分构建软目标来放松硬负样本假设，并基于相似度分布的加权机制缓解噪声或不可靠正样本的影响；将传统成对对齐扩展为全局多模态对齐，聚合所有模态对间的对齐信息。

**Result:** 在Drive&amp;Act数据集上，该方法在RGB、IR、Depth和Skeleton四种模态上一致优于成对对齐和现有全局对齐基线；跨视角消融实验表明模型对未见过的相机视角具有较强的泛化能力。

**Conclusion:** 该框架为自监督全局多模态表征学习提供了可扩展且有效的解决方案，能够实现可靠的驾驶员分心检测，为真实世界多模态视频理解奠定了坚实基础，代码将开源。
...
### [106] [Do Multimodal Agents Really Benefit from Tool Use? A Systematic Study of Capability Gains](https://arxiv.org/abs/2606.02357)
*Garvin Guo, Donglei Yu, Yu Chen, Xiang Wang, Shuai Li, Xinpei Zhao, Huaxing Liu, Qinghao Wang, Minpeng Liao*

#### 🧩 TL;DR
本文质疑工具增强多模态智能体的有效性评估，通过对比实验发现，Thyme和DeepEyesV2等代表性智能体的工具调用并未带来一致的性能提升，且大部分工具解问题也可被非工具设置解决，表明现有评估可能混淆了工具可用性与工具实际贡献。

---

#### 📘 Detailed Summary
**Motivation:** 当前工具增强多模态智能体在基准测试中表现优异，但论文指出仅凭工具调用痕迹无法判断工具是否真正提供了回答关键信息，即存在将工具可用性误认为工具实际贡献的评估偏差，因此需要重新审视工具在智能体中的真实作用。

**Method:** 研究选取Thyme和DeepEyesV2两种代表性“思考与图像”智能体，在真实世界理解、OCR、图表理解和数学推理等任务上，分别与无工具版本、以及来自同一训练池但不含工具调用轨迹的纯文本推理器进行对比；同时通过机制消融实验，分析完整工具调用循环、工具调用格式与执行结果对性能的独立贡献。

**Result:** 工具访问未能带来一致的聚合性能提升，也未可靠地降低生成token成本；DeepEyesV2工具解问题的93%和Thyme的96%至少被一种非工具设置解决；机制消融表明，完整的工具调用循环并未一致优于仅工具调用格式或仅执行结果。

**Conclusion:** 被分析的智能体更可靠地学习了工具调用模式而非工具贡献的实际能力，提示评估设计应区分工具可用性（tool availability）与工具是否实际扩展了智能体的解题能力。
...
### [107] [PRIMA: Boosting Animal Mesh Recovery with Biological Priors and Test-Time Adaptation](https://arxiv.org/abs/2606.02366)
*Xiaohang Yu, Ti Wang, Mackenzie Weygandt Mathis*

#### 🧩 TL;DR
PRIMA 提出结合生物先验（BioCLIP嵌入）与测试时自适应（TTA）策略的框架，用于鲁棒的四足动物3D网格重建，解决了物种和姿态分布严重不平衡导致的泛化困难，并构建了大规模伪3D数据集Quadruped3D，在多个基准上取得最先进结果。

---

#### 📘 Detailed Summary
**Motivation:** 现有动物三维重建方法受限于3D监督数据稀缺和物种长尾分布，常回归到平均形状和姿态，对稀有物种和罕见关节动作的泛化能力较差，亟需一种能应对严重物种与姿态不平衡的鲁棒重建方法。

**Method:** PRIMA 首先引入 BioCLIP 嵌入作为生物先验，将语义和形态学知识注入重建过程以提升形状预测的准确性和泛化性；其次提出测试时自适应（TTA）策略，利用2D重投影约束和辅助关键点引导来优化SMAL预测，同时从现有2D数据集生成高质量伪3D标注；最后基于TTA框架构建了覆盖多样物种和姿态的大规模伪3D数据集Quadruped3D，用于系统性提升模型性能。

**Result:** 在 Animal3D、CtrlAni3D、Quadruped2D 和 Animal Kingdom 四个基准上的大量实验表明，PRIMA 取得了最先进的结果，尤其在稀有物种和困难姿态上表现出显著改进，验证了生物先验与自适应驱动数据扩展的有效性。

**Conclusion:** 该研究揭示了生物先验知识和测试时自适应策略对于实现可扩展且通用化的动物网格重建的重要性，为应对长尾分布和标注稀缺问题提供了有效范式，数据和代码已开源。
...
### [108] [Explainable Forensics of Manipulated Segments in Untrimmed Long Videos](https://arxiv.org/abs/2606.02402)
*Yue Feng, Jingjing Li, Qijia Lu, Wei Ji, Jingrou Zhang, Fei Shen, Xiao Li, Yizhen Jia, Qiang Chen, Limin Wang, Wentong Li, Jie Qin*

#### 🧩 TL;DR
本文提出TASLE基准数据集和MSLoc基线方法，针对长视频中AI生成片段的时序定位与可解释性分析任务，填补了现有视频取证方法难以处理长视频中稀疏嵌入AI内容的空白。

---

#### 📘 Detailed Summary
**Motivation:** 现有视频取证方法主要针对短时独立片段，无法处理AI生成内容稀疏嵌入真实长视频的现实场景，缺乏对未裁剪长视频中受操控片段的时序定位与可解释性分析能力。

**Method:** 提出TASLE基准数据集（含12,472个未裁剪视频及丰富标注）和MSLoc基线方法，采用粗到细的取证框架：先通过边界敏感提案生成模块高效扫描长视频，再基于MLLM的精化模块实现精确边界定位与可解释推理。

**Result:** 实验验证了MSLoc基线在长视频AI生成片段定位与解释任务上的有效性，强调了片段级可解释取证对于长视频分析的重要性，数据集和代码已公开。

**Conclusion:** 本研究揭示了长视频中稀疏嵌入AI生成内容的取证需求，证明了粗到细结合MLLM的框架在时序定位和可解释性方面的潜力，为长视频AI内容检测提供了新的基准和方向。
...
### [109] [Spatial-Temporal Decoupled Reference Conditioning for Identity-Preserving Text-to-Video Generation](https://arxiv.org/abs/2606.02441)
*Yuheng Chen, Teng Hu, Yuji Wang, Qingdong He, Lizhuang Ma, Jiangning Zhang*

#### 🧩 TL;DR
ST-DRC提出了空间-时间解耦参考条件框架（ST-DRC）用于身份保持的视频生成，通过潜在上下文特征注入、TASS-RoPE位置编码、外观不变参考增强和三分支参考无分类器引导策略，在轻量级LTX-2.3模型上实现了身份保真度与文本语义控制之间的有效平衡，并在面部身份保持视频生成赛道中取得领先成绩。

---

#### 📘 Detailed Summary
**Motivation:** 现有身份保持视频生成（IPVG）方法在高层次语义控制与低层次身份保真度之间难以取得平衡，具体表现为无法有效分离身份感知的参考信息检索与外观复制，同时在扩散目标中存在身份监督信号被稀释的问题。

**Method:** ST-DRC框架通过三个关键技术解决上述问题：首先，在框架层面将参考图像经视频VAE编码后与含噪视频潜变量拼接，实现潜上下文特征注入以获取丰富低层身份细节；其次，提出TASS-RoPE（时间邻近-空间偏移RoPE）方案，将参考token在时间维度邻近视频序列但在空间维度偏移，使参考信息通过时空注意力流动同时抑制像素级复制粘贴捷径；最后，结合外观不变参考增强与面部引导的身份目标函数，并引入三分支参考无分类器引导策略，在推理时独立控制文本一致性与参考保真度。

**Result:** 实验表明，ST-DRC在轻量级LTX-2.3模型基础上实现了强身份保持、提示对齐、时间一致性和视频质量，并在面部身份保持视频生成赛道中排名位列前茅，验证了空间-时间解耦参考条件的有效性。

**Conclusion:** ST-DRC通过空间-时间解耦的参考条件机制有效平衡了身份保真度与语义控制，其轻量级设计和高性能表现表明解耦参考特征的空间与时间维度是解决IPVG中身份感知参考检索与外观复制冲突的有效方向，为身份保持视频生成提供了一种无需额外适配器的简洁范式。
...
### [110] [Active Exploring like a Pigeon: Reinforcing Spatial Reasoning via Agentic Vision-Language Models](https://arxiv.org/abs/2606.02459)
*Wei Deng, Xianlin Zhang, Mengshi Qi*

#### 🧩 TL;DR
提出动态认知地图与空间断言代码（SAC）相结合的主动推理管线，在MindCube基准上以80.5%准确率超越现有方法29.5个百分点，有效解决了VLM空间推理中的被动观察与稀疏奖励问题。

---

#### 📘 Detailed Summary
**Motivation:** 现有VLM在空间推理任务中仅作为被动观察者，难以适应真实世界应用；同时强化学习方法依赖稀疏奖励信号，对复杂推理任务的优化效果有限，亟需一种能提供密集奖励并支持主动推理的新型框架。

**Method:** 受鸽子构建认知地图导航的启发，提出主动空间推理管线：首先构建动态认知地图，将场景布局参数化为物体位置与朝向，作为持续记忆以融合新观测；其次提出空间断言代码（SAC），即用Python表达式程序式描述空间关系，通过与动态认知地图协作验证中间推理步骤并生成密集奖励信号；最后采用监督微调与强化微调联合优化模型。

**Result:** 在MindCube基准上达到80.5%的整体准确率，在最具挑战性的Rotation子集上比当前最优方法高出29.5个百分点（相对提升53.2%），代码与数据已开源。

**Conclusion:** 动态认知地图与SAC的组合为VLM提供了可验证的中间推理能力与密集奖励，显著提升了空间推理性能，为构建主动感知的具身智能系统提供了新路径。
...
### [111] [MASER: Modality-Adaptive Specialist Routing for Embodied 3D Spatial Intelligence](https://arxiv.org/abs/2606.02463)
*Hilton Raj, Vishnuram AV*

#### 🧩 TL;DR
MASER提出了一种轻量级框架，通过学习神经路由策略，在推理时根据问题语义自动选择最优的单模态适配器，解决了3D具身智能体中视觉-语言模型忽视问题语义与模态匹配关系的问题。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉-语言模型（VLM）在3D具身智能体任务中仅针对单一模态进行微调，完全忽略了问题本身的语义信息——不同问题可能更适合不同模态（如RGB、点云、深度图等），而固定单一模态无法适应这种语义驱动的模态偏好差异。

**Method:** MASER框架基于共享的VLM骨干网络，训练五个不同的模态适配器（分别对应自然语言、RGB图像、点云、深度图和相机位姿），并学习一个神经路由策略：使用冻结的句子变换器对每个问题进行编码，再通过一个小型MLP（基于oracle适配器准确率标签训练）在推理时选择最优适配器，每次推理仅需调用一个适配器。

**Result:** 在Open3D-VQA基准上，实验表明没有单一模态在所有问题上表现最优——点云模态在51.5%的情况下效果最佳。MASER的路由策略达到51.3%的oracle一致性，显著优于随机森林消融实验的43.5%，且每次推理仅需一次适配器调用。

**Conclusion:** 该研究揭示了3D VQA任务中不同问题对模态的偏好存在显著差异，并证明了基于问题语义的轻量级路由策略能够有效逼近最优模态选择，为多模态融合提供了一个高效且无需多模态联合推理的新方向。
...
### [112] [ToolFG: Towards Well-Grounded Fine-Grained Image Classification](https://arxiv.org/abs/2606.02518)
*Yu Xue, Haoxuan Qu, Zhuoling Li, Yihang Lou, Yan Bai, Hossein Rahmani, Jun Liu*

#### 🧩 TL;DR
本文提出ToolFG，首个面向细粒度图像分类的、集成外部工具使用的MLLM框架，通过MCTS引导的工具使用知识蒸馏和模型-工具协同进化机制，使MLLM能自主调用工具收集可靠视觉线索，显著提升分类性能。

---

#### 📘 Detailed Summary
**Motivation:** 细粒度图像分类（FGIC）面临高度相似类别间细微差异难以区分的挑战，现有方法依赖固定特征提取范式，缺乏对外部工具的动态交互能力，导致分类结果在可靠性和可解释性方面存在不足。

**Method:** ToolFG框架使MLLM在推理过程中自主灵活地调用外部工具，主动与图像交互并收集可验证的视觉线索。模型训练采用MCTS引导的工具使用知识蒸馏机制，从高级闭源MLLM中挖掘工具使用和FGIC相关知识；同时提出模型-工具协同进化机制，联合优化工具集和模型的工具使用策略，使其相互适应并专门化于FGIC任务。

**Result:** 大量实验证明了ToolFG框架在细粒度图像分类任务上的有效性，框架在多个基准数据集上取得了显著性能提升，验证了工具集成和协同进化机制对分类准确率和推理可解释性的正向贡献。

**Conclusion:** ToolFG为细粒度图像分类提供了一种新的范式，通过赋予MLLM自主工具使用能力，实现了更可靠、更有依据的分类推理。模型-工具协同进化机制为未来将MLLM与专用工具深度融合的研究提供了重要启示，拓展了多模态大模型在精细视觉感知任务中的应用边界。
...
### [113] [VLMs are Good Teachers for Video Reasoning via Adaptive Test-Time Optimization](https://arxiv.org/abs/2606.02564)
*Junhao Cheng, Liang Hou, Tianxiong Zhong, Xin Tao, Pengfei Wan, Kun Gai, Jing Liao*

#### 🧩 TL;DR
将VLM从"求解者"转变为"教师"，通过提取任务规则并设计可微分奖励信号，在测试时在线优化VGM的轻量级LoRA模块，显著提升视频生成模型的推理能力，在VBVR-Bench和RULER-Bench上平均提升16.7个点。

---

#### 📘 Detailed Summary
**Motivation:** 现有VGM虽视觉质量优异，但难以理解和遵循任务特定规则，导致推理场景中出现逻辑失败；已有的VLM-as-Solver范式因文本描述无法捕捉复杂时空细节、且VGM难以忠实执行细粒度或长尾指令而效果有限。

**Method:** 提出VLM-as-Teacher范式，利用VLM的强感知能力从任务中提取规则并构建可微分奖励函数，通过测试时在线优化轻量级LoRA模块来引导VGM Reasoner，实现自适应优化并将推理能力扩展到VGM固有边界之外。

**Result:** 在VBVR-Bench和RULER-Bench视频推理基准上，该方法平均提升16.7个点，远超VLM-as-Solver范式（+0.4点）和Best-of-N缩放（+2.2点），且测试时成本相当。

**Conclusion:** 将VLM作为测试时教师进行集成，为实现可泛化的视频推理提供了一种有前景的新范式，揭示了VLM的评估能力可用于有效引导VGM推理过程的关键洞见。
...
### [114] [VISReg: Variance-Invariance-Sketching Regularization for JEPA training](https://arxiv.org/abs/2606.02572)
*Haiyu Wu, Randall Balestriero, Morgan Levine*

#### 🧩 TL;DR
VISReg 提出用 Sliced-Wasserstein 基的 sketching 目标替代 VICReg 中的协方差正则化，在保持灵活性的同时强制执行完整分布形状，从而在低质量、长尾和低秩数据上优于现有方法，并在 ImageNet-22K 预训练下以少 10 倍数据匹配 DINOv2 的 OOD 性能。

---

#### 📘 Detailed Summary
**Motivation:** VICReg 的正则化仅捕获二阶统计量（协方差），鼓励特征去相关但无法保证嵌入空间的完整分布形状，导致训练不稳定；而 SIGReg 等 sketching 方法通过将嵌入对齐到各向同性高斯来弥补这一点，但缺乏灵活性且在坍塌时面临梯度消失问题。现有方法在正则化的灵活性与分布形状的完整性之间存在权衡，亟需一种既能解耦尺度与形状又能提供稳健梯度的正则化框架。

**Method:** VISReg 提出方差-不变量-sketching 正则化框架，用基于 Sliced-Wasserstein 距离的 sketching 目标替换 VICReg 中的协方差项，同时保留方差项以控制尺度。该方法将尺度控制与形状约束解耦，保留了 VICReg 的灵活性，同时引入 sketching 方法对完整分布形状的约束能力，并在坍塌状态下仍能提供梯度信号。

**Result:** VISReg 具有线性计算复杂度，在低质量数据集上显著优于现有正则化方法，并对长尾和低秩数据表现出良好鲁棒性。在 ImageNet-1K 上预训练后，VISReg 在分布外数据集上达到最先进性能；在 ImageNet-22K 上预训练后，其 OOD 性能与使用 LVD-142M（10 倍数据量）的 DINOv2 持平。

**Conclusion:** VISReg 通过将 sketching 的分布形状约束与 VICReg 的灵活性有效结合，证明了在自监督学习中解耦尺度与形状正则化的有效性。该工作表明，在数据规模受限或分布偏移严重的场景下，分布形状的正则化比简单的去相关性更为关键，为设计高效且鲁棒的自监督学习方法提供了新方向。
...
### [115] [ProtoAda: Prototype-Guided Adaptive Adapter Expansion and Geometric Consolidation for Multimodal Continual Instruction Tuning](https://arxiv.org/abs/2606.02576)
*Yu-Cheng Shi, Zhen-Hao Xie, Jun-Tao Tang, Da-Wei Zhou*

#### 🧩 TL;DR
针对多模态大语言模型在持续指令微调中因图像-文本相似度路由忽略输出结构差异导致任务分配不当的问题，提出ProtoAda框架，通过格式感知任务原型和几何感知参数更新，实现了更优的跨任务协同与性能提升。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于稀疏架构（如混合LoRA专家与图像-文本相似度路由）的多模态持续指令微调方法存在严重缺陷：语义相似但响应结构不同的任务（如坐标预测的地面任务与短文本答案的VQA任务）可能被错误路由到同一专家，导致格式盲目的任务分配引发梯度干扰，降低了专家协作效率与模型泛化能力。

**Method:** 提出ProtoAda原型引导自适应微调框架，包含两个核心设计：一是引入格式感知任务原型（format-aware task prototypes），将任务分配与路由策略同时对齐任务语义和输出结构；二是采用几何感知方式（geometry-aware manner）整合格式兼容的更新，从而有效重用并渐进优化已有参数，避免异构响应类型共享参数带来的干扰。

**Result:** 在多个基准数据集上的广泛实验表明，ProtoAda相比已有方法取得了更优的性能，特别是在答案结构易被顺序微调破坏的任务上，优势尤为明显。

**Conclusion:** 该研究揭示出在多模态持续指令微调中，仅依赖语义相似度进行路由存在根本性局限，输出结构信息是任务分配的关键维度；ProtoAda通过显式建模任务格式并利用几何感知参数整合，为提升持续学习中的专家协作效率提供了有效新范式。
...
### [116] [Thinking in Blender: Staged Executable Inverse Graphics with Vision-Language Models](https://arxiv.org/abs/2606.02580)
*Guangzhao He, Rundong Luo, Wei-Chiu Ma, Hadar Averbuch-Elor*

#### 🧩 TL;DR
本文提出SEIG框架，利用预训练视觉语言模型将单张图像渐进式重建为可编辑的Blender程序，无需专用3D模型或可微渲染，通过分阶段分解场景因子显著提升重建保真度。

---

#### 📘 Detailed Summary
**Motivation:** 逆向图形学是一个长期存在且高度欠约束的问题，现有方法通常依赖专用2D/3D基础模型、可微渲染或多视角监督，而本文试图探索通用预训练视觉语言模型能否直接通过单张图像执行可执行的逆向图形学重建。

**Method:** 本文提出分阶段可执行逆向图形学（SEIG）框架，该框架以智能体形式运行，利用预训练视觉语言模型将单张图像重建为可编辑的Blender程序，通过逐步细化几何、材质、构图和光照等场景因子，在可执行的Blender代码空间中进行渐进式场景重建。

**Result:** 在多种场景上的实验表明，分阶段重建策略显著优于一次性重建方法，在像素级、感知级和语义级保真度指标上均取得更优结果，证明了任务分解对于使用通用视觉语言模型进行可执行逆向图形学的重要性。

**Conclusion:** 本研究表明，通用预训练视觉语言模型结合分阶段任务分解策略可以有效执行单图像逆向图形学重建，生成的可编辑Blender场景支持渲染、重光照和操作等多种下游应用，为无需专用模型的逆向图形学提供了新路径。
...
### [117] [Toward Robust In-Context Learning: Leveraging Out-of-distribution Proxies for Target Inaccessible Demonstration Retrieval](https://arxiv.org/abs/2606.00014)
*Hao Xu, Rite Bo, Fausto Giunchiglia, Yingji Li, Rui Song*

#### 🧩 TL;DR
提出DOPA框架，通过引入OOD代理（OOD proxy）近似不可见的目标域，并结合基于马氏距离的全局多样性约束，提升大语言模型在分布偏移场景下的演示检索质量和推理鲁棒性。

---

#### 📘 Detailed Summary
**Motivation:** 尽管大语言模型（LLMs）在分布外（OOD）任务上表现良好，但随着分布偏移加剧其优势会减弱。现有方法通过从源域检索分布相似且信息丰富的演示来增强LLM推理，但在目标域不可见的实际场景中，无法直接评估未知分布，导致所选演示质量下降。

**Method:** 提出DOPA演示搜索框架，核心包含两个组件：一是OOD代理（OOD proxy），用于近似不可见的目标域并引导检索过程；二是基于马氏距离（Mahalanobis distance）的全局多样性约束，确保检索到的演示集具有充分的多样性。

**Result:** 在多个LLM和多种任务上的实验结果表明，DOPA能够有效提升模型在OOD设置下的鲁棒性。

**Conclusion:** DOPA通过代理近似和多样性约束有效解决了目标域不可见时演示检索质量不佳的问题，为提升LLM在严重分布偏移下的推理能力提供了可行的思路。
...
### [118] [SENSE: Semantic Embedding Navigation with Soft-gated Evaluation for Retrieval-based Speculative Decoding](https://arxiv.org/abs/2606.00021)
*Shaowen Chen, Zhicheng Liao, Hongwei Wang*

#### 🧩 TL;DR
SENSE通过语义嵌入导航和软门控评估机制取代基于表面形式的检索与验证，有效解决了检索式推测解码中因词汇层面刚性依赖导致的性能瓶颈，在LLaMA和Qwen系列模型上实现最高4.09平均接受长度和3.26倍加速。

---

#### 📘 Detailed Summary
**Motivation:** 现有的检索式推测解码（RSD）虽具备即插即用的灵活性，但其检索和验证过程严重依赖表面形式的词汇级匹配，对语言表达变化敏感，导致性能受限于刚性词汇依赖，无法充分发挥推测解码在LLM推理加速中的潜力。

**Method:** SENSE提出两项核心设计：一是基于目标模型隐状态进行语义嵌入导航（Semantic Embedding Navigation）的检索机制，取代传统表面形式检索，建立鲁棒的语义对齐；二是软门控评估（Soft-gated Evaluation）模块，用于验证候选token之间的语义等价性而非表面形式匹配。此外，论文将现有方法拆解为原子原语，构建统一框架以实现细粒度的组件级比较。

**Result:** 在LLaMA和Qwen系列模型上进行的跨领域广泛实验表明，SENSE在多项基线方法上均取得显著优势，达到最高4.09的平均接受长度（mean acceptance length）和3.26倍的推理加速比，同时生成质量与目标模型保持一致。

**Conclusion:** SENSE通过将推测解码的检索与验证从词汇层面提升至语义层面，有效克服了检索式方法对表面形式的依赖，为无训练推理加速提供了一种通用且高效的方案。其统一框架也为后续研究中组件级别的公平比较与系统性改进奠定了基础。
...
### [119] [TrustLDM: Benchmarking Trustworthiness in Language Diffusion Models](https://arxiv.org/abs/2606.00023)
*Yichuan Mo, Yukun Jiang, Yanbo Shi, Mingjie Li, Michael Backes, Yang Zhang, Yisen Wang*

#### 🧩 TL;DR
本文提出TrustLDM，首个面向语言扩散模型的信任度基准，系统评估其在安全、隐私和公平性方面的表现，并揭示其在恶意上下文攻击下的脆弱性；进一步提出TrustLDM-Auto自动评估框架，通过利用解码灵活性系统性地识别模型配置漏洞。

---

#### 📘 Detailed Summary
**Motivation:** 语言扩散模型（LDMs）因其灵活的解码策略在速度和性能上挑战了自回归模型的主导地位，但其任意顺序解码方式可能引入新的信任风险（如安全、隐私、公平性），而现有研究缺乏对此类模型的系统性信任度评估基准。

**Method:** 构建TrustLDM综合信任度基准，覆盖安全、隐私和公平性三个维度，针对多种LDM架构并在多种静态后置上下文（post contexts）条件下进行评估；提出TrustLDM-Auto自动评估框架，利用LDM解码灵活性自动搜索并识别最优脆弱配置模式。

**Result:** 实验表明，仅使用用户提示时LDMs表现出较强信任度，但当恶意后置上下文附加到掩码响应上时，其对齐行为显著下降；更长的上下文不一定带来更强攻击效果，解码顺序和生成长度均会影响评估结果；TrustLDM-Auto在所有评估模型和维度中均发现了显著的信任度脆弱性。

**Conclusion:** 语言扩散模型在信任度方面存在系统性脆弱性，尤其在恶意上下文注入场景下；解码策略的灵活性既是优势也是风险来源；TrustLDM及TrustLDM-Auto为构建更可信的LDM提供了评估工具和研究方向。
...
### [120] [Masking Stale Observations Helps Search Agents -- Until It Doesn't: A Regime Map and Its Mechanism](https://arxiv.org/abs/2606.00408)
*Haoxiang Zhang, Qixin Xu, Zhuofeng Li, Lei Zhang, Pengcheng Jiang, Yu Zhang, Julian McAuley*

#### 🧩 TL;DR
本文系统研究了长程搜索智能体中的观测掩蔽（observation masking）策略，发现其效果呈现非对称倒U型模式，取决于检索器召回率与模型隐式过滤能力的交互，而非单一因素。

---

#### 📘 Detailed Summary
**Motivation:** 长程搜索智能体在多轮工具调用中会积累大量检索内容，导致上下文预算效率下降。现有的观测掩蔽（屏蔽过期观测）是一种最小干预方案，但何时有效、为何有效尚不明确，缺乏系统性理解。

**Method:** 本文通过系统性的实验扫描，在不同规模的智能体骨干网络（4B到284B参数）和三种检索器上，对离线及在线网页搜索基准进行观测掩蔽策略的实验。从机制上分析掩蔽如何实现token-for-turn权衡，并通过注意力分析和页面重访模式进行验证。

**Result:** 观测掩蔽的准确率提升呈非对称倒U型曲线：弱检索器下效果平缓，强检索器配合中等容量模型时达到峰值，模型饱和时性能急剧下降。机制上，掩蔽移除了模型注意力已大幅衰减的观测和很少重新打开的页面，新增的交互轮次在将失败转为成功时有益，但在移除模型本可利用的证据时有害。

**Conclusion:** 观测掩蔽的效果取决于检索器召回率与模型隐式过滤能力的相互作用，不能孤立看待任一因素。本文将上下文管理重新定义为依赖于特定机制区间的干预，为分析智能体深度搜索中的上下文使用提供了整体视角。
...
### [121] [ProtStructQA: A Denotation Threshold in Protein Structural Reasoning](https://arxiv.org/abs/2606.00451)
*Aravind Mandiga, Guoming Li, Jin Lu, Ismailcem Budak Arpinar, Khaled Rasheed, Samuel E. Aggrey*

#### 🧩 TL;DR
本文提出 ProtStructQA，一个基于可执行 DSL 程序的蛋白质结构问答基准，将科学问答重新定义为从自然语言到 3D 结构测量的编译过程，并揭示了语言模型在生成可执行结构描述时的能力依赖阈值。

---

#### 📘 Detailed Summary
**Motivation:** 现有蛋白质语言系统的评估往往依赖于生成的生物学文本是否合理，但结构化问题具有明确的 3D 坐标语义，需要一个能够严格验证模型是否真正理解结构信息的评测基准，而不仅仅是生成流畅的生物文本。

**Method:** ProtStructQA 通过隐藏的类型化领域特定语言（DSL）程序生成自然语言问题，并在 AlphaFold 预测的蛋白质结构上执行该程序以获得标准答案；基准包含 382.2K 问题，涵盖置信度、距离、PAE、溶剂暴露、二级结构、拓扑和接触等维度，并设计了硬负样本鲁棒性池；实验评估了 Qwen3（0.6B–8B）和 Gemma-3 模型在直接提示、思维链、语法约束可执行投票、可执行投票加思维链以及多轮 ReAct 工具使用等多种策略下的表现。

**Result:** 研究发现一个能力依赖的指称阈值出现在 Qwen3-1.7B 和 Qwen3-4B 之间：低于该阈值时，工具介导的 ReAct 策略占优，因为模型难以生成可执行的指称表达；高于该阈值时，思维链从大多有害转变为强烈有益，并成为大多数数据集划分上最强的策略；语法约束和执行验证仅在 PAE 和二级结构查询上有选择性价值。

**Conclusion:** ProtStructQA 将科学问答重新定义为从语言到测量的编译过程，提供了一个诊断模型能否将文字映射为可执行 3D 结构测量的测试平台，揭示了语言模型在结构化科学推理中存在从不可解析语言到可执行结构指称的能力转换阈值。
...
### [122] [Short-form Text Rewriting with Phi Silica](https://arxiv.org/abs/2606.00462)
*Divya Tadimeti, Shawn Pan, Sameera Lanka, Chenghui Zhou, Sadid Hasan*

#### 🧩 TL;DR
本文针对小型语言模型（SLM）在短文本改写任务中语义保真度不足和幻觉鲁棒性差的问题，通过数据集构建、提示蒸馏、参数高效微调等方法对Phi Silica进行适配，实验表明微调显著提升了语义保真度并降低了幻觉率，有效缩小了SLM与云端大模型之间的性能差距。

---

#### 📘 Detailed Summary
**Motivation:** 短文本改写受限于上下文稀少和语义密度高，变体空间极为有限；虽然大型语言模型在通用改写任务上表现良好，但小型语言模型在短文本场景下常面临语义保真度不足和幻觉鲁棒性差的问题，亟需针对性的适配方法以缩小与云端模型的性能差距。

**Method:** 采用Phi Silica作为基础SLM，通过从公开幻灯片集中策展短演示文本数据集，利用GPT-5-chat生成改写监督信号并进行LLM-as-a-judge评估，结合提示蒸馏和参数高效微调技术实现对SLM的针对性适配。

**Result:** 微调后的SLM在语义保真度上显著提升，幻觉现象减少，且在与GPT-5-chat改写的偏好胜率对比中取得更高胜率，验证了定向适配策略的有效性。

**Conclusion:** 研究表明，通过定向适配可以显著缩小SLM与云端大模型在短文本改写任务上的性能差距，为将SLM应用于精度要求苛刻的改写任务提供了实用指导。
...
### [123] [Learning to Retrieve: Dual-Level Long-Term Memory for Text-to-SQL Agents](https://arxiv.org/abs/2606.00547)
*Yibo Wang, Nikki Lijing Kuang, Philip S. Yu, Zhewei Yao, Yuxiong He*

#### 🧩 TL;DR
MERIT 提出了一个基于强化学习的多时间跨度记忆检索框架，通过回合级和轮次级两层记忆策略，并引入轻量级过程奖励模型为局部记忆选择提供密集代理奖励，从而在交互式 Text-to-SQL 任务中显著提升成功率并减少交互轮次。

---

#### 📘 Detailed Summary
**Motivation:** 现有交互式 Text-to-SQL 系统中的长时记忆检索方法存在局限：静态方法依赖固定相似度启发式，无法优化下游效用；动态方法通常仅从稀疏的最终结果中学习，且仅在单一决策时间跨度上检索记忆。然而记忆的有用性会随交互阶段变化——初始规划所需记忆与局部状态条件执行所需记忆不同，现有方法无法在多个时间跨度上灵活适配。

**Method:** MERIT 提出动态多时间跨度记忆检索框架，维护两类记忆：回合级记忆用于全局策略指导，轮次级记忆用于局部决策支持。两层均使用基于强化学习学得的检索策略。为在中间监督信号有限的情况下训练轮次级检索，MERIT 设计了一个轻量级过程奖励模型（Process Reward Model），为局部记忆选择提供密集代理奖励信号。

**Result:** 在 BIRD-Interact 基准上，MERIT 在成功率上优于无记忆、静态检索和动态检索基线，同时减少了平均交互轮次。在 Spider2-Snow 上的迁移实验表明，MERIT 无需针对特定基准调参即可实现正向跨基准迁移。

**Conclusion:** 多时间跨度检索能够有效改善交互式 Text-to-SQL 智能体的经验重用能力，不同粒度记忆的分层构建与强化学习优化是提升多轮交互性能的关键途径。
...
### [124] [SPADER: Step-wise Peer Advantage with Diversity-Aware Exploration Rewards for Multi-Answer Question Answering](https://arxiv.org/abs/2606.00593)
*Qiming Shi, Zhaolu Kang, Yunfan Zhou, Di Weng, Yingcai Wu*

#### 🧩 TL;DR
SPADER是一个面向多答案问答场景中长程工具使用推理的强化学习框架，通过无评论家的步骤级信用分配机制（SPA）和多样性感知探索奖励，显著提升了召回率和综合F1分数。

---

#### 📘 Detailed Summary
**Motivation:** 现有的大语言模型工具使用推理方法主要针对单正确答案任务，而现实中的多答案问答（Multi-Answer QA）需要发现完备的有效答案集合，面临两重挑战：长搜索轨迹上的细粒度信用分配问题，以及奖励信号难以激励模型在挖掘高频实体后继续进行持续探索。

**Method:** SPADER框架包含两个核心组件：Step-wise Peer Advantage (SPA)，一种无需评论家（critic-free）的步骤级信用分配机制，通过对齐并行轨迹的决策步骤并从同伴回报中估计优势值来实现逐步骤奖励分配；以及多样性感知探索奖励（diversity-aware exploration reward），通过上调稀有发现的权重并下调冗余发现的权重，促进长尾实体的发现。

**Result:** 在QAMPARI、Mintaka、WebQSP和QUEST四个基准数据集上的实验表明，SPADER在召回率和综合F1分数上普遍优于基于提示的智能体、结果监督的强化学习方法以及最新的步骤级监督方法。

**Conclusion:** SPADER通过步骤级信用分配与多样性探索奖励的有效结合，为多答案场景下的长程工具使用推理提供了一种无需密集人工标注的强化学习范式，代码和模型权重已开源。
...
### [125] [Linguistics-Aware Non-Distortionary LLM Watermarking](https://arxiv.org/abs/2606.00613)
*Shinwoo Park, Hyejin Park, Hyeseon An, Yo-Sub Han*

#### 🧩 TL;DR
LUNA 是一种语言自适应水印方法，通过基于词性上下文的熵估计指导非失真二元锦标赛采样器，在跨语言场景下实现了检测鲁棒性与生成质量的 Pareto 最优，AUROC 达 0.9959 且中位困惑度偏移仅 0.045。

---

#### 📘 Detailed Summary
**Motivation:** 现有语言模型水印方法在多语言部署中面临挑战：不同语言的形态学、分词和文字系统差异导致水印证据难以自然嵌入，且现有方法往往需要在检测质量与生成质量之间做权衡，缺乏一种既能保持非失真性又能跨语言通用的轻量级方案。

**Method:** LUNA 提出语言自适应水印框架，核心包括：(1) 基于外部语料库估计词性上下文的归一化 next-tag 熵，自适应设定非失真的二元锦标赛采样器深度；(2) 检测器仅通过文本、分词器、词性标注器和密钥即可重构与生成端一致的采样调度；(3) 采用标准随机密钥模型，实现模型无关的检测和单 token 级非失真。

**Result:** 在涵盖六种类型学多样的语言和两个领域共12个设置下的评估中，LUNA 取得 AUROC 0.9959 和最低的平均中位困惑度偏移 0.045，其 95% 自助法置信区间 [0.022, 0.073] 完全低于所有基线区间；同时实现最低的平均 Self-BLEU、Distinct-1、surprisal 和熵偏移，且是唯一在多数设置（9/12）下同时满足 AUROC > 0.99 且中位困惑度偏移 < 0.1 的方法。

**Conclusion:** LUNA 证明了通过语言学特征（词性熵）自适应引导水印采样策略，可以在不牺牲生成质量的前提下实现跨语言高鲁棒检测，为多语言场景下的文本水印提供了新的设计范式，代码已开源。
...
### [126] [MemPro: Agentic Memory Systems as Evolvable Programs](https://arxiv.org/abs/2606.00619)
*Qingshan Liu, Guoqing Wang, Wen Wu, Jingqi Huang, Xinqi Tao, Dejia Song, Jie Zhou, Liang He*

#### 🧩 TL;DR
MemPro 提出了一种系统级演化框架，将整个记忆构造-检索（MCR）流水线视为可演化程序，通过故障模式引导的编辑-调试迭代生成改进版本，在多个长时程任务基准上持续超越静态和提示级演化基线。

---

#### 📘 Detailed Summary
**Motivation:** 现有智能体记忆系统通常采用固定的记忆构造-检索（MCR）流水线，仅自适应调整记忆库本身而保持其余组件不变，这种固定设计难以处理异构的任务特定故障模式，并且会随着记忆库在规模和结构上的演化而逐渐失配，导致系统性能受限。

**Method:** MemPro 将整个 MCR 流水线视为一个可演化的程序，维护一个可运行记忆系统实现的版本树；一个演化代理（Evolving Agent）通过迭代选择有前景的版本、诊断重复出现的故障，并基于故障模式引导的编辑-调试（edit-debug refinement）创建改进的子版本，从而实现系统级的持续演化。

**Result:** 在 LongMemEval、LoCoMo、HotpotQA 和 NarrativeQA 四个基准上的实验表明，MemPro 在少数迭代内即持续优于强静态基线和提示级演化基线，且性能随演化次数持续提升，同时实现了良好的性能-成本权衡。

**Conclusion:** MemPro 证明了将智能体记忆系统视为可演化程序进行系统级自适应调整的有效性，为长时程自主智能体的记忆系统设计提供了新的范式，即在部署后通过故障反馈驱动整个流水线的持续改进。
...
### [127] [OCC-RAG: Optimal Cognitive Core for Faithful Question Answering](https://arxiv.org/abs/2606.00683)
...
#### 🧩 TL;DR
提出Optimal Cognitive Core (OCC)系列任务专用小语言模型，其中OCC-RAG针对基于上下文的忠实问答优化，通过大规模合成多上下文多跳QA数据训练，在多项基准上以0.6B-1.7B参数量匹配或超越2-6倍规模的通用模型。

---

#### 📘 Detailed Summary
**Motivation:** 现有语言模型的发展过度依赖规模扩展以吸收世界知识，但许多实际应用更需鲁棒推理能力而非海量参数化知识，因此任务专用的小语言模型成为更合理的设计选择。

**Method:** 提出OCC-RAG模型，采用任务专用小语言模型架构，并构建了一套大规模合成多上下文、多跳问答数据的流水线，生成了超过三百万条训练样本，专门针对多跳推理、严格上下文忠实性和校准式拒绝回答进行优化。模型在训练后能够生成带有原文引用的结构化推理轨迹。

**Result:** OCC-RAG-0.6B和OCC-RAG-1.7B在多跳推理（HotpotQA、MuSiQue、TAT-QA）、忠实性（ConFiQA）和拒绝回答（MuSiQue-Un）基准上，以比通用模型小2-6倍的参数量取得了匹配或超越的性能。

**Conclusion:** 研究表明，紧凑且任务专用的小语言模型能够在知识密集型推理任务中显著超越同等规模甚至更大规模的通用模型，验证了通过大规模合成数据训练实现专业化推理能力的可行性，为SLM在实际部署中的高效应用提供了有力支撑。
...
### [128] [I-WebGenBench : Evaluating Interactivity in LLM-Generated Scientific Web Applications](https://arxiv.org/abs/2606.00750)
*Dasen Dai, Biao Wu, Meng Fang, Shuoqi Li, Wenhao Wang*

#### 🧩 TL;DR
本文提出 PaperVoyager，一个将科研论文自动转化为可交互网页系统的端到端框架，涵盖论文理解、系统建模与交互式网页合成，并在包含19篇论文配专家构建系统的基准上验证了其显著优于现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有文档智能体主要将论文转化为摘要、网页或幻灯片等静态制品，无法有效处理涉及动态机制和状态变迁的技术论文，缺乏将论文内容转化为可交互、可操作系统的能力。

**Method:** 提出 PaperVoyager，一个结构化生成框架，显式地对论文中的机制和交互逻辑进行建模，然后合成可执行的交互式网页系统；同时构建了一个包含19篇研究论文及专家构建的交互式系统作为 ground truth 的基准用于评估。

**Result:** 实验结果表明，PaperVoyager 在生成交互式系统的质量上显著优于现有方法，验证了其对论文中动态机制建模的有效性。

**Conclusion:** PaperVoyager 为交互式科学论文理解提供了一种新范式，将论文从静态文本转化为可操作的动态系统，有望提升技术论文的理解与复现效率。
...
### [129] [Not All Flips Are Conformity: Decomposing Stance Convergence in Multi-Agent LLM Debate](https://arxiv.org/abs/2606.00820)
*Xiqi Hao, Zengqing Wu, Yu-Xuan Qiu, Chuan Xiao, Ruiqi Xu, Shuyuan Zheng, Jianbin Qin*

#### 🧩 TL;DR
本文揭示多智能体辩论（MAD）中答案收敛可能源于社会顺从而非真实推理，提出三源分解框架将传统翻牌率拆解为自发不稳定性、立场诱导顺从和推理诱导说服，并发现顺从性变化中57-77%为有害（正确转错误），通过无意义推理实验证明推理式呈现本身即具有显著说服力。

---

#### 📘 Detailed Summary
**Motivation:** 多智能体辩论（MAD）被广泛用于提升LLM推理能力，但现有评估指标（如答案翻牌率）无法区分答案收敛是源于真实推理说服还是社会顺从（即单纯从众），这一混淆使得MAD的真实有效性存疑，亟需解耦不同机制以正确评估和优化辩论策略。

**Method:** 提出三源分解框架，通过受控反事实条件将传统答案翻牌率拆分为自发不稳定性（自我反思下的变化）、立场诱导顺从（仅受他人立场影响而改变正确答案）和推理诱导说服（受有效推理影响而变化）。在MMLU-Pro和GPQA-Diamond基准上对多个模型家族进行实验，并设计信息梯度实验（注入无意义推理）来隔离推理式呈现本身的说服效果，同时构建Round 0特征预测模型和风险靶向干预策略。

**Result:** 在MMLU-Pro主要设置中，37%的智能体-问题观测在自我反思下即发生变化；严格顺从性为29%，其中57-77%为有害顺从（正确转错误）；无意义推理实验中，抵抗型智能体在20-39%的情况下采纳了空洞推理，表明推理式呈现本身即具有说服权重；有害顺从可从Round 0特征预测（AUC=0.79），风险靶向干预将其降低13.6个百分点（p<0.001），但无正确标签时降低从众无法提升准确率。

**Conclusion:** MAD中的答案收敛在很大程度上由社会顺从（尤其是有害顺从）驱动而非真实推理，仅降低从众行为不足以提升模型准确性，因为无法区分有益和有害影响；未来需设计能识别影响方向性的干预方法，且评估MAD时必须引入自我反思基线以分离自发不稳定性，否则传统指标会严重高估辩论效果。
...
### [130] [Momento: Evaluating Persistent Memory and Reasoning with Multi-Session Agentic Conversations](https://arxiv.org/abs/2606.00832)
*Adril Putra Merin, David Anugraha, Ayu Purwarianti, Genta Indra Winata*

#### 🧩 TL;DR
Momento是一个用于评估多会话持久化智能体任务完成的基准，揭示了当前智能体因对用户状态误估而无法处理跨会话的时间依赖性和演化中的用户目标，表明现有智能体能力与真实长期人机交互之间存在显著差距。

---

#### 📘 Detailed Summary
**Motivation:** 现有智能体基准仅在单会话环境中进行评估，忽略了智能体在真实服务场景中需跨会话整合用户历史行为、偏好和先前决策以实现个性化目标的需求，缺乏对持久化、多会话任务完成能力的评估方法。

**Method:** 提出了Momento基准，构建多会话服务环境，要求智能体通过工具调用执行具有后果的行动，同时解决跨会话的时间依赖性和演化的用户目标，以评估智能体的持久化任务完成能力。

**Result:** 实验结果表明，当前智能体失败的主要原因是用户状态误估，即错误地将先前会话历史视为当前上下文当前状态的可靠代理，而忽略了其中包含的需重新验证的过时信息。

**Conclusion:** 当前智能体能力与真实长期人机交互需求之间存在显著差距，智能体需要学会识别并重新验证过时的会话信息，而非简单依赖历史记录作为上下文。
...
### [131] [Robust Asynchronous Planning via Auto-Formalization](https://arxiv.org/abs/2606.00981)
*Jiayi Zhang, Jianing Yin, Ben Zhou, Li Zhang*

#### 🧩 TL;DR
本文系统性地研究了LLM在异步规划任务中的表现，发现形式化表示的选择是决定规划可扩展性的关键：CP-SAT形式化器在大规模任务中显著优于Planner和PDDL2.1形式化器，且状态感知的修复策略能有效缓解执行时约束更新的性能下降。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM规划基准大多忽略真实世界中任务的异步特性（如非均匀持续时间、并发性和执行时约束），导致对模型在复杂动态环境中规划能力的评估存在空白。

**Method:** 将异步规划挑战统一为单一形式化表述，引入三个大规模基准分别对应不同的异步规划难点；比较三种规划范式：直接生成动作序列的Planner、基于PDDL2.1谓词表示的形式化器、以及基于CP-SAT通用约束满足程序的形式化器，并引入状态感知的修复策略以应对执行时约束更新。

**Result:** 当依赖图从5个动作增长到100个动作时，Planner的规划准确率从96%骤降至5%，PDDL2.1形式化器从13%降至0%，而CP-SAT形式化器平均达94%且在100个动作时仍保持83%；执行时约束更新进一步导致性能严重下降（Planner 23.9%，PDDL2.1 0.7%，CP-SAT 46.1%），但状态感知修复策略仅关注事件诱导约束，将CP-SAT恢复至84.5%。

**Conclusion:** 异步规划的可扩展性主要由形式化表示决定；PDDL2.1的基于谓词的规划表示在需要保持一致谓词、效果和目标的场景下变得脆弱，而通用约束满足程序更鲁棒；状态感知的增量修复策略为动态环境中保持LLM规划能力提供了有效方向。
...
### [132] [ExpWeaver: LLM Agents Learn from Experience via Latent RAG](https://arxiv.org/abs/2606.01041)
*Tao Feng, Tianyang Luo, Jingjun Xu, Zhigang Hua, Yan Xie, Shuang Yang, Ge Liu, Jiaxuan You*

#### 🧩 TL;DR
ExpWeaver 提出了一种基于潜在检索增强生成（latent RAG）的框架，使 LLM 智能体能够通过自身隐藏状态编码和检索经验，无需独立 RAG 模块，并在 13 项任务中取得 12 项 SOTA，同时显著降低 token 开销并提升跨域泛化能力。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于经验学习的 LLM 智能体方法局限于显式文本空间，依赖语义相似度检索经验并直接拼接至上下文窗口，导致大量 token 开销以及检索与生成分离的解耦架构，缺乏端到端的协同优化。

**Method:** ExpWeaver 利用 LLM 自身的隐藏状态对经验进行编码，在潜在空间中进行逐解码步的检索，并通过交叉注意力聚合与门控残差机制将检索到的潜在经验整合到生成过程中；整个流程通过强化学习进行端到端优化，同时支持生成和排序任务。

**Result:** 在 13 项涵盖问答、推理、代码、科学预测和推荐的任务上，ExpWeaver 在 12 项上取得最优，比最强基线高出 6.8% 以上；其 token 效率与非检索基线相当，而基于文本的检索方法需要 1.5 到 2 倍的 token；在零样本和少样本跨域迁移下，分别比最强基线高出 16.32% 和 15.21%。

**Conclusion:** 该研究表明，通过潜在空间检索与端到端强化学习优化，能够有效解耦检索与生成的 token 开销，实现更高效率、更强泛化的经验驱动 LLM 智能体，为无需外部 RAG 模块的隐式知识集成提供了新范式。
...
### [133] [When Is 0.1% Enough? Analyzing the Combined Effects of Dimensionality Reduction and Quantization on Text Embedding Compression](https://arxiv.org/abs/2606.01074)
*Riku Kisako, Hayato Tsukagoshi, Ryohei Sasano*

#### 🧩 TL;DR
本文系统研究了结合降维与量化两种压缩方法对文本嵌入向量的效果，发现二者联合使用能够实现更强的压缩比，部分设置下可将嵌入压缩至原始大小的0.1%而性能几乎不降，且最佳压缩策略因任务而异。

---

#### 📘 Detailed Summary
**Motivation:** 现有高性能文本嵌入模型通常输出高维实值向量，导致巨大的存储和计算开销。虽然已有基于降维或量化的压缩方法，但两者组合使用的效果尚未得到充分研究。

**Method:** 本文基于四种MTEB任务家族和四种预训练嵌入模型，系统性地实验评估了将降维（dimensionality reduction）与量化（quantization）相结合对文本嵌入进行压缩的有效性，并与单独使用任一方法的情形进行对比。

**Result:** 实验结果表明，降维与量化的结合能实现远强于单独使用任一方法的压缩效果；在某些设定下，嵌入可压缩至原始大小的0.1%而几乎不损失性能；最佳压缩策略依赖于具体任务类型。

**Conclusion:** 该研究揭示了降维与量化联合压缩在文本嵌入中的巨大潜力，并指出任务依赖性是选择压缩策略的关键因素，为实际部署中高效存储和计算文本嵌入提供了实用指导。
...
### [134] [CA-BED: Conversation-Aware Bayesian Experimental Design](https://arxiv.org/abs/2606.01182)
*Daniel Arnould, Rashad Aziz, Zixuan Kang, Tanav Changal, Kevin Zhu, Sunishchal Dev, Gabriel Grand, Shreyas Sunil Kulkarni*

#### 🧩 TL;DR
提出对话感知贝叶斯实验设计（CA-BED），一种将贝叶斯实验设计与LLM似然估计相结合的推理时概率对话规划框架，在实体推理任务中相较于直接提示实现了平均21.8%的成功率提升，且仅增加1.8轮对话。

---

#### 📘 Detailed Summary
**Motivation:** LLMs在静态推理任务中表现优异，但在需要主动通过提问获取信息的交互式场景中性能下降，其核心挑战在于如何选择能有效降低不确定性的问题，同时处理可能模糊或仅部分信息性的回答。

**Method:** 提出Conversation-Aware Bayesian Experimental Design (CA-BED)框架，该框架在推理时进行概率对话规划，通过贝叶斯实验设计结合LLM-based似然估计来优化多轮对话中的问题选择；CA-BED维护关于假设的信念分布，预判可能的回答并通过模拟对话树传播期望信息增益。

**Result:** 在两个结构化实体推理基准上，CA-BED相较于直接提示实现了平均21.8%的成功率提升，效果优于其他信息寻求方法；同时仅平均增加1.8轮对话轮次即可获得上述增益。

**Conclusion:** CA-BED通过推理时概率规划有效提升了LLM在交互式信息获取场景中的表现，证明将贝叶斯实验设计与LLM结合是一种有前景的对话策略优化方向。
...
### [135] [DiscourseFlip: An Oblique Discourse-Level Opinion Manipulation Attack against Black-box Retrieval-Augmented Generation](https://arxiv.org/abs/2606.01212)
*Yuyang Gong, Miaokun Chen, Jiawei Liu, Zhuo Chen, Guoxiu He, Wei Lu, XiaoFeng Wang, Xiaozhong Liu*

#### 🧩 TL;DR
本文提出 DiscourseFlip，一种针对检索增强生成（RAG）系统的語篇级观点操纵攻击方法，通过在语义查询网络中有策略地分配投毒预算，实现跨多主题查询空间的观点偏移，实验表明其有效性和隐蔽性均优于现有方法，且现有防御措施难以应对。

---

#### 📘 Detailed Summary
**Motivation:** 现有针对 RAG 系统的投毒攻击主要集中于单个查询或狭窄的主题局部查询集合，限制了实际攻击覆盖范围且伪装能力有限，缺乏在语义查询网络层面实现多主题、整体性观点操纵的攻击范式。

**Method:** 本文形式化定义了一种黑盒设定下的语篇级观点操纵威胁模型，并提出 DiscourseFlip——一种基于智能体与图引导的攻击方法，该方法通过动态分配有限的投毒预算，在语义查询网络中最大化语篇级的观点偏离，实现跨多主题查询空间的协调性影响。

**Result:** 大量实验证明 DiscourseFlip 能够在上下文化查询网络上持续诱导目标观点偏移，在覆盖率和有效性上显著超越现有基线方法；用户研究进一步验证了其攻击有效且具有良好的隐蔽性，不易被用户察觉；系统分析表明现有防御策略无法有效应对语篇级操纵。

**Conclusion:** 语篇级观点操纵揭示了 RAG 系统在防御层面未被充分认知的安全漏洞，现有缓解措施在此类攻击下基本失效，亟需开发更具鲁棒性和自适应性的防御机制以应对语篇级安全威胁。
...
### [136] [Efficient RAG with Intent-Aware Retrieval and Semantics-Preserving Chunking](https://arxiv.org/abs/2606.01240)
...
#### 🧩 TL;DR
提出InSemRAG框架，通过意图感知检索器(IAR)和语义保留分块(SPC)模块结合迭代检索-检查机制，解决传统RAG中意图无关检索和信息碎片化导致的不足，并利用小语言模型降低延迟，在多跳和证据敏感任务上取得显著提升。

---

#### 📘 Detailed Summary
**Motivation:** 传统RAG系统因意图无关的检索方式（intent-agnostic retrieval）和信息碎片化（information fragmentation）两大因素导致检索信息不充分，限制了LLM的指令跟随与推理能力，现有方法难以同时兼顾检索的相关性和语义完整性。

**Method:** 提出InSemRAG框架，核心包含两个模块：意图感知检索器(IAR)采用动态混合检索方法，根据查询意图自适应加权不同检索通道；语义保留分块(SPC)对受损证据块进行检测和修复以保持语义完整性；整个框架基于迭代检索-检查（iterative retrieve-and-check）机制运行，并采用小语言模型(SLM)降低计算延迟。

**Result:** 在多个基准数据集上验证了方法的竞争力，在HotPotQA上F1提升2.65个百分点，在FEVER上准确率提升1.5个百分点；与Multi-Hop RAG相比，在SLM支持下实现了4.32倍的低延迟，同时达到竞争性性能。

**Conclusion:** 通过意图感知检索与语义完整性保留的双重改进，InSemRAG有效解决了传统RAG的信息不足问题，且利用SLM在保证性能的前提下显著降低了延迟，为高效RAG系统设计提供了新思路。
...
### [137] [Agentic Clustering: Controllable Text Taxonomies via Multi-Agent Refinement](https://arxiv.org/abs/2606.01255)
*Simon Löwe, Emily Silcock*

#### 🧩 TL;DR
提出一种基于智能体编排的文本聚类方法，通过编排器LLM动态调度五个专用智能体（提议器、合成器、审计器、调查器、评判器）替代固定程序化流程，在七个公开基准上达到SOTA性能，相较最强LLM基线ARI提升最高达32%。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于LLM的文本聚类方法采用固定程序化流程——LLM调用顺序及停止、合并、分裂规则均在代码中预设，导致对不同结构语料库的泛化能力差，且难以融入用户约束（如目标簇数或聚类意图）。

**Method:** 提出一种智能体式替代方案，由一个编排器LLM在每个步骤检查聚类发现过程的状态，并从提议器、合成器、审计器、调查器和评判器这五个专用智能体中选择调度，使流程自适应于语料库而非执行固定流水线。

**Result:** 在七个公开文本聚类基准上，该方法取得了最先进性能，相比之前最强LLM基线，在调整兰德指数（ARI）上最高提升达32%。

**Conclusion:** 基于智能体编排的动态自适应流程能有效克服固定程序化LLM聚类管线的泛化局限，显著提升聚类质量并灵活融入用户约束，为基于LLM的文本聚类提供了新的范式方向。
...
### [138] [Worlds Within Words: Translating Culture in Ancient Chinese Texts with Multi-Agent Coordination](https://arxiv.org/abs/2606.01276)
*Xiaoqi He, Kaixin Lan, Mu You, Tao Fang, Lidia S. Chao, Derek F. Wong*

#### 🧩 TL;DR
提出MACAT多智能体框架，将古汉语文化负载词翻译建模为选择性显化任务，通过动态识别文化显著短语并注入简洁解释性知识，在中医典籍和《论语》翻译中一致优于基线模型。

---

#### 📘 Detailed Summary
**Motivation:** 基于大语言模型的机器翻译在处理古汉语文化负载词时面临深层挑战：直译保留表面形式但丢失文化概念，过度显化则损害简洁性与可读性。现有方法缺乏在何时以及如何对文化依赖知识进行显化的自适应决策能力。

**Method:** 提出MACAT（多智能体文化感知翻译框架），将文化负载词翻译形式化为选择性显化任务。框架包含动态识别文化显著短语的模块、注入简洁解释性知识的机制、用于候选翻译选择的质量感知重排序模块，以及从术语精确性、可读性、忠实度、文化保存和文化显化五个维度进行评估的多轮评估智能体。

**Result:** 在中医典籍（100篇文档）和《论语》（20章子集）上的实验表明，在统一的GPT-5.4评估设置下，MACAT在各项指标上均一致优于骨干模型和通用机器翻译基线。

**Conclusion:** 通过将文化负载词翻译建模为选择性显化任务并采用多智能体协作框架，可在保留文化内涵与维持文本简洁性之间取得更优平衡。该框架为古汉语典籍中文化敏感内容的机器翻译提供了有效范式。
...
### [139] [Don't Read Everything: A Curvature-Conditioned Query for Linear Attention](https://arxiv.org/abs/2606.01294)
*Dong Le, Thong Nguyen, Cong-Duy Nguyen, Anh Tuan Luu*

#### 🧩 TL;DR
本文提出曲率条件查询（CCQ）机制，通过利用softmax注意力几何的二阶泰勒展开构建查询压缩算子，在不解码速度的前提下提升了线性注意力在上下文检索和长序列任务上的表现。

---

#### 📘 Detailed Summary
**Motivation:** 线性注意力通过递归快权重状态降低了softmax注意力的二次计算成本，但在上下文检索和长上下文任务上表现持续落后。现有改进方法聚焦于写入端（如门控、增量更新或核特征映射），而读取端保持不变——每个过去的键对输出做相加贡献，导致有用目标被大量存储向量稀释。

**Method:** 本文借用softmax注意力几何的特定性质来构建廉价的读取时查询压缩操作。对softmax对数配分函数在各项同性注意力点处进行二阶泰勒展开，得到一个局部二次模型，其曲率恰好等于运行键协方差矩阵，该协方差可通过与线性注意力状态相同的递归/分块机制维护。相应的线性算子会在查询读取状态之前沿记忆的高密度方向压缩查询，该机制称为曲率条件查询（CCQ）。CCQ仅修改读取步骤，可与任何线性注意力骨干组合使用。

**Result:** 将CCQ附加到GLA和Gated DeltaNet上，在困惑度、零样本下游任务准确率、S-NIAH检索（在训练上下文长度内及超越长度时）、从4K到20K的长度外推困惑度以及LongBench准确率上均取得改进，且额外开销很小。

**Conclusion:** CCQ通过仅在读取端引入廉价几何修正，有效缓解了线性注意力中记忆被稀释的核心问题，显著提升了线性注意力的上下文检索与长序列建模能力，且与现有多种线性注意力骨干兼容，是一个即插即用、计算高效的改进方案。
...
### [140] [SkillAdaptor: Self-Adapting Skills for LLM Agents from Trajectories](https://arxiv.org/abs/2606.01311)
*Zhuoyun Yu, Xin Xie, Wuguannan Yao, Chenxi Wang, Lei Liang, Xiang Qi, Shumin Deng*

#### 🧩 TL;DR
SkillAdaptor提出了一种免训练的步骤级技能自适应框架，通过显式故障归因实现更稳定、可审计的技能维护，在WebShop、PinchBench和Claw-Eval三个基准上均优于无技能和技能自适应基线。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM智能体的免训练技能自适应流程通常基于完整轨迹或会话级反馈进行技能更新，导致故障归因粗糙、修订结果不稳定或过于宽泛，缺乏细粒度的步骤级故障定位能力。

**Method:** SkillAdaptor是一个免训练的步骤级技能自适应框架，可插入OpenClaw类智能体框架；给定失败轨迹后，框架识别首个可执行的故障步骤，将责任关联到候选技能，并在显式接受检查下执行定向技能更新，同时保持骨干模型冻结。

**Result:** 在WebShop、PinchBench和Claw-Eval三个基准上使用Kimi-K2.5、GLM-5和GPT-5.2进行评估，SkillAdaptor在所有套件上均优于无技能和技能自适应基线，最大单项提升分别为PinchBench平均分+1.5%、Claw-Eval平均分+1.8、WebShop成功率+1.7。

**Conclusion:** 步骤级故障归因能够支持更稳定且可审计的免训练技能维护，为LLM智能体在长程交互任务中的技能自适应提供了有效且可靠的解决方案。
...
### [141] [Dr. DocBench: A Comprehensive Benchmark for Expert-Level and Difficult Document Parsing](https://arxiv.org/abs/2606.01393)
...
#### 🧩 TL;DR
Dr. DocBench 是一个面向专家级文档解析的难度感知基准，通过解析器失败采样从多语言书籍语料库中构建，涵盖52个BISAC学科领域、4,514页标注页面和65k条标注，实验表明现有强模型在该基准上性能大幅下降，揭示了其作为诊断和推进文档智能的综合测试平台的价值。

---

#### 📘 Detailed Summary
**Motivation:** 现有OCR和文档解析基准在覆盖范围和难度上日益受限：多数聚焦于常见文档类型或均匀采样页面，现代解析器已表现强劲；同时缺乏对化学公式、乐谱、复杂表格和跨页布局等专家领域结构的充分标注，难以衡量和推动文档智能的实质性进步。

**Method:** 从大规模多语言书籍语料库出发，覆盖52个BISAC学科领域；采用基于解析器失败的采样策略，专门选取多个最先进系统均难以处理的困难文档；构建了4,514页来自平均约100页的长文档的标注数据，包含65k高质量的页面级和块级标注，涵盖布局、阅读顺序、层次关系及领域特定视觉内容。

**Result:** 对基于流水线的解析器和通用视觉语言模型（VLM）的评估表明，在现有基准上的强性能无法迁移到专家级文档解析任务；跨学科主题、内容类型和结构属性的分析揭示了大量失败案例，证明Dr. DocBench对现有系统构成了显著挑战。

**Conclusion:** Dr. DocBench作为诊断和推进文档智能的综合性测试平台，揭示了当前文档解析系统在处理专家级复杂文档时的系统性不足，为未来研究提供了明确的方向和评估标准。
...
### [142] [DrugClaw and DrugAudit: A Primary-Source-Grounded Agent and Authority-Aware Benchmark for Drug-Information Question Answering](https://arxiv.org/abs/2606.01434)
*Qing Wang, Bo Li, Jialu Liang, Daling Shi, Bob Zhang, Qianqian Song*

#### 🧩 TL;DR
提出 DrugClaw，一个基于多智能体检索增强和反射驱动状态机的工作流系统，用于药物信息问答；同时构建 DrugAudit 基准（3,772 条目）并引入权威感知评估协议，在多个基准上取得全面最优结果。

---

#### 📘 Detailed Summary
**Motivation:** 药物信息问答属于高风险场景，幻觉事实可能误导临床决策，且每个引用事实的出处来源与事实本身同等重要。现有方法在回答的可靠性、引文溯源和权威性方面存在不足，缺乏能够严格对齐原始监管或同行评审记录的端到端框架。

**Method:** DrugClaw 采用多智能体检索增强架构，通过反射驱动的状态机工作流查询药物和药物警戒技能注册库，返回基于原始监管记录或同行评审文献的答案。DrugAudit 基准包含 3,772 个条目并配备评估面板，使用双裁判 LLM-as-Judge 协议（裁判间 kappa=0.88）对上游-黄金源匹配、词元级语义片段重叠和引文忠实度进行评分。

**Result:** 在 DrugAudit 以及 MedQA（751 条）和 PubMedQA（512 条）的药物相关子集上，DrugClaw 在头条表格的每个指标上均排名第一：复合证据指数（双裁判下）、裁判介导的答案正确性、主要来源比率（0.918，比次优方法高 10.1 个百分点）、忠实度（0.887，+5.9 pp）、MedQA（0.920）和 PubMedQA（0.693）。

**Conclusion:** DrugClaw 在高风险药物信息问答中通过多智能体反射驱动检索和严格的引文溯源显著超越了现有方法，表明将推理过程状态机化与权威感知评估相结合是提升临床决策支持系统可靠性的有效路径。所提出的 DrugAudit 基准和双裁判评估协议为后续研究提供了标准化评测平台。
...
### [143] [Beyond Topical Similarity: Contrastive Evidence Retrieval with Interpretable Attention Alignment in RAG](https://arxiv.org/abs/2606.01482)
...
#### 🧩 TL;DR
CERA提出了一种融合主观性难负样本选择与证据注意力对齐损失的稠密检索框架，通过对比学习与可解释注意力对齐双重训练目标，在临床试验报告语料上显著提升了RAG的事实性、忠实度与可解释性。

---

#### 📘 Detailed Summary
**Motivation:** 现有RAG框架在确保检索结果的事实性（factuality）和可解释性（interpretability）方面仍然存在迫切且未解决的问题，传统检索方法主要依赖主题相似性，难以识别真正的支撑证据并缺乏对检索决策的可信解释。

**Method:** CERA框架基于稠密检索器，采用两种训练目标：三元组对比学习（triplet-based contrastive learning）和可解释注意力对齐（interpretable attention alignment）；其中负样本通过主观性（subjectivity-based）策略进行硬负例筛选，注意力对齐损失利用基于词性加权（part-of-speech-weighted）的掩码分布监督CLS到token的注意力，使其对齐人工标注的事实依据（factual rationales）。

**Result:** 在大型临床试验报告语料上的实验表明，基于主观性的硬负样本选择相比Contriever和传统难负样本选择基线显著提升了检索效果；同时，依据对齐在保持竞争性检索性能的前提下改善了忠实度（faithfulness），验证了注意力在人工依据引导下可作为模型行为更忠实的解释。

**Conclusion:** CERA证明了通过引入证据归纳偏置（evidential inductive bias）和主观性难负样本选择，检索器能够超越主题相似性，识别构成支撑证据的具体标记（tokens），从而在RAG系统中促进更可解释的证据选择过程。
...
### [144] [EvoPool: Evolutionary Programmatic Annotation for Label-Efficient Specialized Supervision](https://arxiv.org/abs/2606.01617)
*Tianyi Xu, Yaolun Zhang, Xuan Ouyang, Huazheng Wang*

#### 🧩 TL;DR
EvoPool提出了一种基于进化多智能体框架的标注方法，通过三个专用智能体迭代生成可执行的标注器代码，并利用确定性门控机制筛选出优秀标注器，最终通过EvoPool以近乎零成本生成的标注质量在多个专业领域任务上平均比最强LLM标注基线高0.141 macro-F1，同时速度提升4500至31000倍。

---

#### 📘 Detailed Summary
**Motivation:** 大型语言模型在通用任务上表现优异，但在标注成本高昂的专业高风险领域（如生物医学关系抽取、法律条款分类等）中性能不如较小的监督模型，现有LLM直接标注方法成本高且效率低，亟需一种低成本的弱监督标注方法以弥合这一差距。

**Method:** EvoPool是一个受达尔文进化启发的多智能体框架，包含三个专用智能体（提议者、验证者、门控者），它们迭代生成可执行的标注器代码；通过小型验证集提供适应度信号，确定性门控机制基于可行性、多样性和边际贡献三个检查筛选标注器；最终采用EvoPool集成器，将池中投票映射为软训练标签，EvoPool是一种文本感知的聚合器，结合了语义特征和标注器投票特征。

**Result:** 在8个LLM弱项专业任务中的7个上（涵盖生物医学关系抽取、法律条款分类、复杂推理和密集多标签生物医学分类），EvoPool平均比最强LLM标注基线高出+0.141 macro-F1，在ChemProt上提升高达+0.301，在PubMed上提升+0.265；且每样本标注成本接近零，在10万样本上比LLM标注快4500至31000倍。

**Conclusion:** EvoPool通过进化式多智能体框架实现了在专业高风险领域中对LLM标注的显著性能提升和成本降低，验证了将进化搜索与弱监督聚合相结合的有效性，为低成本高质量标注提供了新范式。
...
### [145] [Easier to Mislead Than to Correct: Harmful and Beneficial Revision in LLM Conformity](https://arxiv.org/abs/2606.01637)
*Jiaming Qu, Lucheng fu, Yibo Hu*

#### 🧩 TL;DR
本文研究了多智能体LLM系统中同伴一致性（conformity）对模型决策的影响，发现与纠正错误相比，利用同伴共识更容易误导原本正确的模型，且权威标签会促使模型盲从，而链式思考等推理干预无法有效区分有益与有害修订。

---

#### 📘 Detailed Summary
**Motivation:** 大型语言模型在多智能体系统中会看到并响应其他智能体的回答，存在盲从多数意见的风险。已有研究表明LLM倾向于向多数答案修订，但尚不清楚这些修订在纠正错误与引入新错误之间的相对频率，该论文旨在填补这一研究空白。

**Method:** 作者设计了一个受控实验框架：LLM先回答一个问题，然后观察模拟的同伴回答，最后做出最终决定。通过操纵两个社会线索——共识结构（consensus structure）和同伴的权威标签（authority labels），分别测量它们对有益修订和有害修订的影响。实验涵盖四个开源LLM和七个QA数据集。

**Result:** 实验发现：同伴共识更容易误导原本正确的模型，而纠正原本错误的模型则困难得多；权威标签使模型更倾向于选择被背书（endorsed）的答案，无论其正确与否；更令人担忧的是，链式思维（chain-of-thought）和反思（reflection）等通用推理干预无法在保留有益修订的同时可靠地减少有害修订。

**Conclusion:** 研究表明，多智能体LLM系统应当对同伴答案进行验证（verify），而非简单聚合（aggregate）。盲目采纳多数意见或权威意见会显著增加得出错误答案的风险，而现有的推理增强方法难以有效区分有益与有害的社会影响。
...
### [146] [Encoded but Not Routed: Explaining the Table-Chart Gap in Scientific Claim Verification](https://arxiv.org/abs/2606.01679)
*Sunisth Kumar, Xanh Ho, Tim Schopf, Andre Greiner-Petter, Florian Boudin, Akiko Aizawa*

#### 🧩 TL;DR
本研究通过层间线性探针和注意力分析发现，多模态大语言模型在科学同行评审中处理图表证据时的表现劣于表格，并非由于无法从图表中提取信息，而是因为提取到的视觉信息未被有效路由至预测位置，即编码成功但推理时访问失败。

---

#### 📘 Detailed Summary
**Motivation:** 多模态大语言模型在辅助科学同行评审时需要验证论文中的声明是否被证据支持，但现有工作发现模型在证据为表格时表现显著优于同一数据以图表呈现的情况。此前尚不清楚这一差距源于模型无法从图表中提取信息，还是提取了信息却未能在预测时有效利用，本文旨在厘清这一根本性问题。

**Method:** 本文在三种开源视觉语言模型上，针对同一底层数据分别以表格和图表形式呈现的证据，采用层间线性探针（layer-wise linear probing）和注意力分析两种方法，研究了模型中间层表示中图表信息的编码情况以及信息向预测位置的传递路径。

**Result:** 实验一致发现，图表信息被编码在模型的中间层表示中，但未能传递到预测位置的表示中，而这一缺失在表格证据中并不存在，且在所有测试条件下均成立。注意力分析进一步揭示，这一信息传递失败在不同模型族中表现为两种架构上不同的形式。

**Conclusion:** 表格与图表之间的性能差距本质上并非编码失败，而是编码后的视觉信息在预测时刻的路由失败。该发现将问题从"模型能否提取图表信息"重新定义为"模型能否在推理时正确访问已编码的视觉信息"，为改进多模态模型在科学文献理解中的表现提供了新的研究方向。
...
### [147] [THRD: A Training-Free Multi-Turn Defense Framework for Jailbreak Attacks on Large Language Models](https://arxiv.org/abs/2606.01738)
*Zhiqing Ma, Zhonghao Xu, Dong Yu, Chen Kang, Changliang Li, Pengyuan Liu*

#### 🧩 TL;DR
THRD是一个无需训练的防御框架，通过对多轮对话中的时间风险累积进行显式建模，将越狱攻击的成功率降低至0.2–4.0%，同时将模型效用损失控制在1.5%以内。

---

#### 📘 Detailed Summary
**Motivation:** 现有防御方法要么依赖代价高昂的重新训练且会降低模型效用，要么在每轮对话中独立进行单轮分析，无法捕捉多轮交互中风险沿轨迹累积的动态特性。多轮越狱攻击利用对话的动态演化特性（如逐步升级、跨轮协调）对LLM构成日益增长的威胁，而安全行为本质上具有轨迹依赖性——对话历史持续重塑模型的上下文条件，使得孤立评估每一轮是否安全变得不足。

**Method:** THRD包含四个模块：轮次风险评估器（TRA）用于即时风险估计，历史上下文分析器（HCA）用于跨轮意图升级检测，响应评估器（RE）用于识别助长越狱的输出，以及一个决策模块，通过带有衰减调制和趋势感知调整的时间演化评分机制综合上述信号。该框架无需训练，直接对多轮交互中的时间风险累积进行显式建模。

**Result:** 在针对包括基于树搜索和多智能体协作方法在内的最先进多轮越狱攻击的实验中，THRD将攻击成功率降至0.2–4.0%，同时在MMLU和GSM8K上模型效用退化不超过1.5%。消融研究证实了各模块的非冗余贡献以及跨架构的稳定泛化能力。首次拒绝触发分析表明，超过70%的多轮攻击需要到第2轮或更晚才能被检测到，验证了显式时间聚合的必要性。

**Conclusion:** 该研究表明，显式地对多轮对话中的时间风险累积进行建模是实现高效且无损效用的多轮越狱防御的关键。超过70%的攻击仅在第2轮及之后才被首次检测到，这强烈验证了跨轮次时间聚合的必要性。THRD作为首个无需训练的多轮越狱防御框架，为构建更安全的交互式LLM系统提供了可落地的有效方案。
...
### [148] [Construction of Historical Knowledge Graphs Based on BERT and Graph Neural Networks](https://arxiv.org/abs/2606.01747)
*Ping Li, Bartlomiej Brzozka*

#### 🧩 TL;DR
本文提出了一种结合BERT与图神经网络（GNN）的高层架构，用于从各类历史文本中提取实体和关系以构建知识图谱，并基于FastRQNet和Vilt-qaformer+RoBInet开发了图像检索系统；实验表明该方法在精度、召回率和F1分数上优于传统规则方法和深度学习方法，能够有效处理嵌套结构和隐式指代问题。

---

#### 📘 Detailed Summary
**Motivation:** 传统历史文本在数字化转换过程中面临语言歧义、上下文引用受限以及缺乏规范语法规则等系统性难题，现有的规则方法和深度学习基线难以充分处理复杂嵌套结构和隐式指代问题，亟需一种能够结合语义上下文感知与关系图学习的自动提取方法。

**Method:** 本研究提出了一种高层架构，集成双向编码器表示变换器（BERT）与图神经网络（GNN）用于历史文本中的实体和关系提取；同时开发了基于FastRQNet和预训练视觉-语言模型Vilt-qaformer+RoBInet的图像检索系统。实验数据涵盖市政记录、议会文件和历史信函等综合历史文献集合。

**Result:** 与传统的规则方法及其他流行的深度学习基线相比，所提出的联合BERT-GNN系统在精度、召回率和F1分数上均取得了更优表现（见表2），且能够以足够的准确性和全面性处理复杂嵌套结构和隐式指代问题，成功构建知识图谱。

**Conclusion:** 实验证明，将关系图学习算法与上下文敏感的语义表示技术相结合，能够实现对历史数据的自动提取，为知识库积累增量智慧；该工作为数字人文研究和大规模历史数据分析提供了一种有效的技术路径。
...
### [149] [HarnessForge: Joint Harness and Policy Evolution for Adaptive Agent Systems](https://arxiv.org/abs/2606.01779)
*Mingju Chen, Can Lv, Guibin Zhang, Heng Chang, Shiji Zhou*

#### 🧩 TL;DR
提出HarnessForge，一种元自适应框架，通过将LLM智能体系统形式化为执行结构（harness）与推理策略（policy）的联合体，并执行故障引导的harness剪枝与策略对齐的协同进化，在多个基准上一致提升性能，验证了执行结构与推理策略的协同优化对智能体系统自适应至关重要。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM智能体系统在面对跨异构任务场景时缺乏系统级元自适应能力，既有方法仅对执行外壳或底层推理策略进行独立更新，未充分刻画结构与执行之间的自适应空间，且外部执行结构与内部推理器之间的兼容性未被联合优化。

**Method:** 将智能体系统形式化为harness–policy对，其中harness定义执行结构，policy定义推理行为；通过故障引导的harness剪枝（fault-guided harness tailoring）优化执行结构，再通过harness条件化的策略对齐（harness-conditioned policy alignment）使推理策略与剪枝后的执行结构兼容，实现两者的协同进化。

**Result:** 在涵盖不同领域的五个基准上，以Qwen3-4B和Qwen3-8B为骨干，HarnessForge一致优于仅优化harness或仅优化policy的基线，最高比最强基线提升12.0%，并实现了有利的部署效率-性能权衡。

**Conclusion:** 执行结构（harness）与推理策略（policy）的协同进化是有效的，且两者之间的可执行兼容性对于智能体系统自适应至关重要，为全系统级元自适应提供了明确的形式化框架与优化路径。
...
### [150] [ProbeScale: Probing Analysis to Optimize Neural Scaling Laws for Efficient Small Language Model Inference](https://arxiv.org/abs/2606.01806)
*Sourav Das*

#### 🧩 TL;DR
ProbScale提出了一种结合神经缩放定律和语言模型探针分析的框架，能够在预训练小语言模型中识别参数高效的子网络，在保持原始模型95%-98%性能的前提下实现5到10倍的参数压缩。

---

#### 📘 Detailed Summary
**Motivation:** 尽管小语言模型（SLM）在能力与计算可行性之间取得了平衡，但在严格资源约束下部署即使这些SLM仍然具有挑战性，而现有方法未能有效利用缩放定律揭示的SLM内部丰富表征来指导参数高效的子网络选择。

**Method:** ProbScale框架首先利用任务特定的探针（probes）从数学上量化预训练SLM中每一层对目标下游能力的相关性，然后将子网络选择形式化为在参数预算约束下最大化聚合的、任务加权探针性能的层子集优化问题，从而在保证高质量表征的前提下实现参数效率与性能的最优权衡。

**Result:** 在RoBERTa-Large和T5-Base等代表性SLM上的实验表明，ProbScale识别的子网络在目标任务上实现了5到10倍的参数缩减，同时保持了原始模型95%至98%的性能，显著优于启发式基线方法。

**Conclusion:** ProbScale验证了结合缩放定律与探针分析进行结构化剪枝的有效性，证明了在充分缩放的小语言模型中，通过任务感知的层选择可以大幅压缩模型规模而几乎不损失性能，为资源受限场景下的模型部署提供了实用框架。
...
### [151] [CRAB-Bench: Evaluating LLM Agents under Complex Task Dependencies and Human-aligned User Simulation](https://arxiv.org/abs/2606.01815)
*Danqing Wang, Akshay Sivaraman, Lei Li*

#### 🧩 TL;DR
CRAB-Bench和RUSE分别通过约束图任务生成和基于人类行为研究的真实用户模拟，解决了LLM智能体在现实服务场景中的评估难题，实验表明前沿模型在CRAB-Bench上仅达到61% pass@1，而RUSE使性能进一步下降高达57%。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM智能体评估缺乏对现实服务场景中复杂任务依赖、不完美用户行为以及多种有效解决方案的全面考量，亟需更真实的评估基准和用户模拟引擎来填补这一研究空白。

**Method:** 提出CRAB-Bench评估框架，通过约束图在多个相互依赖的实体上生成任务并引入结构化干扰项，迫使智能体在数以千计的误导性候选中进行推理；同时提出RUSE真实用户模拟引擎，基于人类行为研究构建具有多样人设和四个行为维度的逼真用户模型。

**Result:** 四个前沿LLM智能体在CRAB-Bench上的最佳模型仅达到61% pass@1；切换至RUSE后性能进一步下降高达57%，且下降主要集中在任务求解能力而非对话质量；信息泄露是最具破坏性的行为维度，与RUSE交互时智能体更倾向于通过隐式修正掩盖错误而非承认失误。

**Conclusion:** 当前LLM智能体在复杂约束推理和对抗性用户行为场景下表现欠佳，信息泄露维度对任务完成度影响最大，且智能体存在掩错倾向，未来评估应纳入更真实的用户行为建模以全面衡量智能体的鲁棒性和可靠性。
...
### [152] [LayerRoute: Input-Conditioned Adaptive Layer Skipping via LoRA Fine-Tuning for Agentic Language Models](https://arxiv.org/abs/2606.01838)
*Prateek Kumar Sikdar*

#### 🧩 TL;DR
LayerRoute introduces轻量级适配器LayerRoute，通过为每个Transformer块添加路由器（~897参数）和LoRA适配器（rank 8），使agentic语言模型系统能够根据输入类型（工具调用vs规划/推理）动态跳过特定层，在仅使用1.10M可训练参数（骨干网权重冻结）且经过单次6.4分钟训练后，实现工具调用步骤跳过15.25% FLOPs而规划步骤仅跳过2.34%的差异化加速。

---

#### 📘 Detailed Summary
**Motivation:** 现有agentic语言模型系统在推理时会交替产生两种结构迥异的步骤类型——结构化的工具调用（短、确定、低困惑度）和开放式的规划/推理（长、复杂、高困惑度），但当前推理系统对每个步骤都施加相同的计算量，未能利用这种异构性来优化效率。

**Method:** LayerRoute为Qwen2.5-0.5B-Instruct的24个Transformer块分别添加：(1) 一个每层路由器（~897参数，Linear(896,1)），通过直通估计器输出硬二值门控信号决定是否跳过该块；(2) 在Q/K/V/O注意力投影上的LoRA适配器（rank 8，~1.08M参数）。骨干权重保持冻结，在agentic数据（Hermes、Glaive、GSM8K、Turing）上进行单次端到端训练，配合门控正则化项使系统自动发现不同输入类型可跳过的块。

**Result:** 经过3,000步（在A100 40GB上仅需6.4分钟）训练后，LayerRoute实现了12.91%的跳过差异率：工具调用跳过15.25%的FLOPs而规划步骤仅跳过2.34%，仅使用1.10M可训练参数（占494M骨干网络的0.22%）。由于LoRA适配的作用，模型质量相比基模型有所提升，工具调用和规划的困惑度差值分别为-1.29和-1.30。

**Conclusion:** LayerRoute证明了通过极小规模的可训练参数和极短的训练时间，即可让语言模型自动学习根据步骤类型差异化分配计算资源，在保持甚至提升生成质量的同时实现高效推理，为agentic系统的计算效率优化提供了轻量级、可扩展的解决方案。
...
### [153] [MMG2Skill: Can Agents Distill In-the-Wild Guides into Self-Evolving Skills?](https://arxiv.org/abs/2606.01993)
...
#### 🧩 TL;DR
提出Guide-to-Skill学习问题及MMG2Skill-Bench基准，并设计MMG2Skill闭环框架，将多模态异构网络指南转化为可执行技能，通过轨迹级根因反馈持续优化，在GUI控制、开放游戏和策略卡牌任务中显著提升VLM智能体性能。

---

#### 📘 Detailed Summary
**Motivation:** 网络上存在大量蕴含程序性知识的多模态异构指南，但其面向人类执行者，包含噪声且难以直接转化为智能体可用的技能，现有方法无法有效弥合人类导向指南与智能体可执行技能之间的鸿沟。

**Method:** 提出MMG2Skill闭环框架，包含三个核心环节：将网络指南编译为可编辑的结构化技能；在执行阶段用固定VLM智能体条件化这些技能；从轨迹级根因反馈中修订技能而不使用基准得分。同时构建MMG2Skill-Bench基准，涵盖GUI控制、开放游戏和策略卡牌三类任务。

**Result:** 在六个VLM骨干网络上，MMG2Skill在所有模型-域设置中一致优于基线智能体，骨干网络上的宏平均增益为+12.8至+25.3个百分点。消融实验表明，直接用原始指南提示智能体会降低性能，结构化技能构建和轨迹驱动修订对提升均不可或缺。在成功可推断任务中，基于分析器的早停机制可防止后期性能退化，在校准正确时节省25%-53%的尝试次数。

**Conclusion:** 本研究表明将异构网络指南转化为结构化可编辑技能并结合轨迹级反馈闭环优化，是提升VLM智能体长时任务能力的有效范式，原始指南的直接使用反而有害，结构化构建与轨迹驱动修订是关键组件。
...
### [154] [Scaling Agentic Capabilities via Grounded Interaction Synthesis](https://arxiv.org/abs/2606.02001)
*Wenhang Shi, Jinhao Dong, Yiren Chen, Zhe Zhao, Shuqing Bian, Wei Lu, Xiaoyong Du*

#### 🧩 TL;DR
GAIS提出了一种基于两阶段接地机制（协议锚定环境构建与结构引导规划）的自动化框架，用于合成高质量、多样化的智能体交互数据，使基础模型在多个基准上匹配甚至超越官方指令微调版本，且展现出更优的数据效率与可扩展性。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于LLM的智能体数据合成方法依赖无约束生成，往往退化为对LLM内部先验的有偏随机采样，无法捕捉真实世界域的多样性与难度，也难以构建高保真、长程的复杂任务，导致合成数据质量受限。

**Method:** GAIS包含两阶段接地机制：首先，基于真实世界的Model Context Protocol (MCP)服务器构建协议锚定环境，确保功能的多样性与难度；其次，采用结构引导规划方法主动执行逻辑依赖与对抗策略，在该环境中生成复杂任务。

**Result:** 在BFCL、τ²-Bench和ACEBench上，GAIS合成数据显著超越现有基线方法，使基础模型匹配甚至超越其官方指令微调版本；GAIS展现出更优的数据效率与可扩展性，在基线停滞时仍能保持持续增长。

**Conclusion:** 该研究表明，通过接地机制约束合成过程而非依赖LLM无约束生成，可以系统性地提升智能体交互数据的质量与多样性，为规模化构建通用智能体训练数据提供了一条可行路径。
...
### [155] [DFlare: Scaling Up Draft Capacity for Block Diffusion Speculative Decoding](https://arxiv.org/abs/2606.02091)
...
#### 🧩 TL;DR
BlockDiffusion提出了一种轻量级逐层融合机制，使草稿模型的每一层都能关注一组目标层的可学习组合，从而打破DFlash中单一融合表示的瓶颈，在支持更深层草稿架构的同时，在数学推理、代码生成和对话任务上分别实现约5.52倍、5.46倍和3.91倍的推理加速，较DFlash提升约5%-11%。

---

#### 📘 Detailed Summary
**Motivation:** 现有最先进方法DFlash将草稿模型的所有层约束为共享一个仅从少数目标层导出的单一融合表示，这限制了逐层的表达能力，阻碍了草稿模型容量的进一步扩展，从而无法充分挖掘块级并行推测解码的加速潜力。

**Method:** 本文提出BlockDiffusion，通过一种轻量级逐层融合机制代替DFlash的窄瓶颈：每个草稿层独立地关注一组广泛目标层（而非仅少数层）的可学习组合，以极小的开销为每个草稿层注入更丰富的目标模型内部知识并提供不同的输入表示。这种增强的逐层表达能力使得草稿模型可以扩展到更深的架构，同时将训练数据从80万样本扩展到240万样本以充分利用增大的模型容量。

**Result:** 在涵盖数学推理、代码生成和对话的六个基准测试上，BlockDiffusion在Qwen3-4B、Qwen3-8B和GPT-OSS-20B上分别实现了平均5.52倍、5.46倍和3.91倍的墙钟时间加速，相较DFlash分别提升了约11%、8%和5%。

**Conclusion:** 通过解除DFlash中草稿层间的表示共享约束，逐层差异化地融合更广泛的目标层知识是提升块级推测解码性能的有效途径；该方法在多种模型规模和任务类型上均取得了持续且显著的加速改进，验证了其通用性和可扩展性。
...
### [156] [Cross-Environment Neural Reranking for Sample-Efficient Action Selection in Text-Based Agents](https://arxiv.org/abs/2606.02204)
*Kan Shao*

#### 🧩 TL;DR
该论文研究能否用单个轻量级神经重排序器（DeBERTa-v3）在ALFWorld、WebShop和ScienceWorld等多个异构环境中统一进行动作选择，发现联合训练与数据重平衡能实现正跨域迁移，性能接近专用单环境模型，且跨环境适应具有高样本效率。

---

#### 📘 Detailed Summary
**Motivation:** 大型语言模型智能体在基于文本的基准测试中表现强劲，但推理成本高昂，促使研究者探索使用紧凑型神经重排序器进行动作选择。然而，现有方法通常需要为每个环境维护独立的模型，因此需要研究单个轻量级模型能否在多个不同环境中统一执行动作选择，以消除按环境维护模型的负担。

**Method:** 采用DeBERTa-v3（184M-434M参数）作为基础重排序器，在ALFWorld、WebShop和ScienceWorld三个环境上联合训练，并使用少数类上采样（minority-class upsampling）缓解数据不平衡。实验探索了双环境和三环境联合训练，并进一步引入环境感知的LoRA适配器路由结合PCGrad梯度投影进行多任务优化。

**Result:** 重平衡的双环境联合训练在ALFWorld上相比单环境显著提升（净增益+0.412），WebShop性能保持竞争力（+0.214 vs 单环境+0.249）。三环境训练在4个种子上的平均综合净增益为+0.551±0.024，性能接近专用单环境模型。仅使用9.2%目标域数据微调即可恢复全数据性能的93%。LoRA+PCGrad方案最佳种子达+0.611，但种子123崩塌至+0.263，四种子平均为+0.497±0.158，存在高方差问题。

**Conclusion:** 数据多样性相比模型容量是提升跨环境动作选择性能的主要驱动因素；联合训练配合干净数据划分和数据重平衡是实现正迁移的关键要素；环境感知LoRA路由与PCGrad虽有潜力但当前稳定性不足，是值得继续探索的方向。
...
### [157] [Better with Experience: Self-Evolving LLM Agents for Evidence-Grounded Health Community Notes](https://arxiv.org/abs/2606.02215)
*Zihang Fu, Fanxiao Li, Jianyang Gu, Haonan Wang, Preslav Nakov, Bryan Hooi, Min-Yen Kan, Jiaying Wu*

#### 🧩 TL;DR
EvoNote是一个面向健康类社区笔记的智能体框架，通过演化经验记忆和细粒度信用分配机制，使LLM驱动的笔记生成能够从既往错误纠正案例中自主学习进化，在1.2K多模态基准上生成的笔记在89.6%的案例中优于人工撰写笔记，并将生成时间从13小时以上缩短至2分钟以内。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM增强的社区笔记虽然在健康谣言纠正方面具有可扩展性，但每一条新帖子仍需要从头开始生成纠正信息，无法利用以往虚假信息纠正案例中积累的有用经验，导致每次纠正都重置状态，缺乏跨案例的知识复用和学习进化能力。

**Method:** EvoNote提出了一种自进化的智能体框架，其核心是构建一个不断演化的经验记忆库，存储先前虚假信息纠正案例中的经验；同时采用细粒度信用分配机制，将轨迹级别的反馈基于健康领域特定的笔记质量指标进行分解，并蒸馏为行动级别的记忆，分别用于主张分析、证据获取和笔记撰写三个子任务。

**Result:** 在包含1.2K实例的多模态健康社区笔记基准MM-HealthCN上，经人类验证的分层效用评估表明，EvoNote生成的笔记在89.6%的情况下比对应的人工撰写笔记更受偏好；在另外一组缺少人群有用性评分的Needs More Ratings帖子中，EvoNote对82.0%的案例生成了有用笔记；同时将候选纠正的生成中位时间从人工管线的13小时以上降至2分钟以内。

**Conclusion:** EvoNote的实验结果证实，通过演化经验记忆和细粒度信用分配，自进化的笔记生成框架能够显著提升健康谣言纠正的效果和效率，其改进归因于更强的证据利用和可复用的纠正策略，为健康虚假信息治理提供了一种有前景的新范式。
...
### [158] [Unified Context Evolution for LLM Agents](https://arxiv.org/abs/2606.02304)
*Zixuan Zhu, Yitong Hu, Yong Dai, Junfeng Fang, Chunyang Jiang, Senkang Hu, Yuzhi Zhao*

#### 🧩 TL;DR
UCE提出了一种免梯度框架，通过将智能体经验外部化为四类可演化上下文单元（记忆、策略、工作流、技能）并动态调度生成预算，解决了LLM智能体在交互任务中跨回合经验积累与知识类型区分的问题，在ALFWorld和WebShop上显著提升了性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM智能体在执行多步交互任务时，每个回合从固定上下文开始，回合中发现的任何有用策略在任务结束后即丢失；已有方法要么将学习限制在当前任务内，要么将所有经验不加区分地存入单一存储库，既不区分知识类型，也不通过使用情况跟踪质量或平衡库中缺失的知识类型。

**Method:** UCE提出了一种免梯度框架，将智能体经验外部化为一个可演化的库，库中包含四类可演化上下文单元（ECU）：Memory（记忆）、Strategy（策略）、Workflow（工作流）和Skill（技能），每个ECU从轨迹中依据类型特定条件生成，在决策时检索，通过重复使用结果评分，并在不再有价值时剪枝；一个调度模块将每轮的生成预算分配给库中最弱的类型。

**Result:** 在两个交互式基准测试中，UCE将ALFWorld的成功率从75.4%提升至96.3%，将WebShop的任务分数从45.1%提升至61.3%；累积的库可迁移到替代的智能体主干模型而无需重新训练。

**Conclusion:** UCE通过结构化的外部经验库和自适应调度机制，有效实现了LLM智能体的跨回合知识积累与复用，显著提升了交互任务的性能，且所学知识具有良好的跨模型可迁移性，为构建持续学习的智能体系统提供了新思路。
...
### [159] [TVIR: Building Deep Research Agents Towards Text--Visual Interleaved Report Generation](https://arxiv.org/abs/2606.02320)
...
#### 🧩 TL;DR
本文提出TVIR，包含TVIR-Bench基准（100个专家精选的多模态深度研究任务）和TVIR-Agent（层次化多智能体框架），用于评估和生成文本-视觉交错报告，实验表明显式多模态设计和评估对证据驱动报告生成至关重要。

---

#### 📘 Detailed Summary
**Motivation:** 现有深度研究智能体主要聚焦文本处理，缺乏对视觉元素（如图表、图片）在事实可靠性和与文本分析对齐方面的系统评估，亟需涵盖文本-视觉交错报告生成的基准和评估框架。

**Method:** 提出TVIR-Agent，一个层次化多智能体框架，包含提纲构建、图像检索、可溯源图表生成和上下文感知顺序写作等模块；同时构建TVIR-Bench（100个专家精选的多模态深度研究任务）以及结合文本评估与视觉评估的双路径评价框架。

**Result:** 在九个深度研究系统上的实验表明，TVIR-Agent取得了优异的整体性能，验证了显式多模态设计和评估对于证据驱动报告生成的重要性。

**Conclusion:** 本研究揭示了现有深度研究系统在视觉元素可靠性方面的不足，强调了在报告生成中显式融入多模态设计与评估的必要性，为未来多模态深度研究智能体的发展提供了基准和基线框架。
...
### [160] [PaSBench-Video: A Streaming Video Benchmark for Proactive Safety Warning](https://arxiv.org/abs/2606.02443)
*Yusong Zhao, Yuejin Xie, Youliang Yuan, Junjie Hu, Jitian Guo, Yujiu Yang, Pinjia He*

#### 🧩 TL;DR
PaSBench-Video 是一个包含 740 个视频（481 个风险场景和 259 个无风险场景）的基准测试，用于评估多模态大语言模型在危险发生前发出及时警告的能力，覆盖驾驶、医疗、日常生活和工业生产四个领域。实验表明，现有模型在此任务上表现不佳（严格指标下准确率不超 20.0%），且召回率与误报率高度相关（Pearson 系数 0.64），表明模型更多依赖场景活动线索而非对危害的推理。

---

#### 📘 Detailed Summary
**Motivation:** 现有基准测试无法评估视频多模态大语言模型在危险信号出现到事故实际发生之间的时间窗口内发出及时、精确警告的能力：它们依赖静态输入、忽略时间精度要求，并且缺乏对安全场景误报率的测量。这一能力缺口对于将 MLLM 用作全天候安全监控器至关重要。

**Method:** 作者构建了 PaSBench-Video 基准测试，包含 740 个视频（481 个风险视频和 259 个无风险视频），涵盖驾驶、医疗、日常生活和工业生产四个领域。风险视频标注了帧级别的风险起始边界和事故边界，要求模型以因果方式观察视频并发出在时间校准和内容正确性上均满足要求的警告。论文测试了 13 个多模态大语言模型在该基准上的表现。

**Result:** 所有 13 个模型在最严格指标下均未超过 20.0% 的准确率，召回率与误报率之间呈现强相关（Pearson 相关系数 0.64），即更高的检测率必然以在大多数安全视频上触发警告为代价。模型性能在不同领域差异显著：在日常生活领域（风险本质上具有异常性）能以较低的误报率达到中等召回率，而在驾驶领域（常规场景与危险场景外观相似）则几乎不加区分地触发警告。

**Conclusion:** 当前多模态大语言模型在预防性安全监控任务中表现不足，其行为依赖于场景层面的活动线索而非对潜在危害的因果推理，并且在视觉相似性高的安全与危险场景之间缺乏区分能力。这揭示了现有模型在时间敏感的安全应用中的根本局限性，需要新的推理机制来弥合观测与预判之间的鸿沟。
...
### [161] [Towards Multidisciplinary Summarization of Hospital Stays: Efficient Sentence-Level Clinical Provenance Categorization](https://arxiv.org/abs/2606.02487)
...
#### 🧩 TL;DR
本研究提出了一种基于大语言模型（LLM）有监督微调的临床来源分类流水线，用于新生儿重症监护室（NICU）多学科摘要生成。通过微调Llama-3模型（8B和70B），在域内F1超过92%并在跨域迁移中验证了模型容量对语义灵活性的关键作用，其中70B模型经量化微调后性能提升7%且计算开销显著降低。

---

#### 📘 Detailed Summary
**Motivation:** 现有"全团队"摘要方法在高复杂度临床场景（如NICU）中难以有效聚合来自多学科（医生、护士、治疗师）的数百份临床自由文本笔记，直接混合异构文本会导致输出不连贯，因此亟需一种能够准确分类句子级来源的结构化摘要技术。

**Method:** 提出临床来源分类流水线，采用有监督微调（SFT）方法适配Llama-3（8B和70B）模型至MedSecId语料库（2,002份MIMIC-III成人ICU笔记，含临床来源标注）。在跨域泛化评估中，测试了模型容量（8B vs. 70B）和量化策略对包含227个句子级跨度的多学科NICU摘要金标准数据集的影响。

**Result:** 两个模型在域内均取得超过92%的Macro F1分数；跨域迁移中，SFT对8B模型提升有限，但使70B模型的Macro F1提升7%；量化后的微调70B模型性能超越全精度基线模型，同时大幅降低了计算需求。

**Conclusion:** 研究表明，足够的模型容量是跨域临床迁移中保持语义灵活性的关键，高效的量化适配方法能够在不牺牲性能的前提下实现结构化来源建模，为下游汇总摘要任务提供了可行路径。
...
### [162] [SkillHarm: Lifecycle-Aware Skill-Based Attacks via Automated Construction](https://arxiv.org/abs/2606.02540)
*Yuting Ning, Zhehao Zhang, Yash Kumar Lal, Boyu Gou, Junyi Li, Weitong Ruan, Chentao Ye, Rahul Gupta, Diyi Yang, Yu Su, Huan Sun*

#### 🧩 TL;DR
SkillHarm 是一个针对智能体技能使用生命周期的基准测试，系统性地定义了固定载荷投毒（FPP）和自变异投毒（SMP）两种攻击场景及12种风险类型，并通过自动化构建管道 AutoSkillHarm 生成879个攻击样本，实验表明当前智能体在FPP和SMP下的攻击成功率分别高达86.3%和69.3%，且现有防御措施难以可靠缓解该威胁。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究揭示了由技能引发的智能体不安全行为，但主要局限于在单次任务执行中评估投毒技能，并通过临时性风险列表枚举危害，缺乏对技能使用全生命周期中攻击场景和风险类型的系统性定义与评估。

**Method:** 作者提出了SkillHarm基准，包含两种攻击场景：固定载荷投毒（FPP）——固定投毒技能包直接危害任何调用它的任务会话；自变异投毒（SMP）——初始良性的执行静默变异持久化技能内容，将危害延迟到后续复用。基于智能体工作流中受危害的组件（数据管道、系统环境、智能体自主性），定义了12种风险类型，并构建了AutoSkillHarm自动化管道，由自然语言驱动的编码智能体大规模生成攻击样本。

**Result:** SkillHarm包含71个技能下的879个攻击样本。实验表明，当前智能体在FPP下攻击成功率高达86.3%，SMP下为69.3%。分析还揭示了一个潜在风险：许多看似攻击失败的情况实际上是由于智能体未能与中毒文件交互，而非真正的抵抗，且现有防御措施仍无法可靠地缓解威胁。

**Conclusion:** 该研究系统性地揭示了智能体技能使用全生命周期中的安全漏洞，指出了当前防御机制的不足，并强调许多攻击失败表象下隐藏着智能体未能正确触发中毒文件的潜在风险，为未来智能体安全研究提供了重要的基准和方向。
...
### [163] [Deliberative Curation: A Protocol for Multi-Agent Knowledge Bases](https://arxiv.org/abs/2606.00007)
*Steven Johnson*

#### 🧩 TL;DR
本文提出一个面向AI智能体的审慎策展协议，通过结合人工制品生命周期、声誉加权审慎投票和分级制裁三层治理机制，在智能体无状态、模型同质和谄媚等挑战下实现知识策展。实验表明，该协议在逆境下鲁棒性显著优于多数投票（退化速度慢约3倍），其中提交-揭示投票隐藏机制贡献最大（8.2-8.6pp精度提升）。

---

#### 📘 Detailed Summary
**Motivation:** 当AI智能体从孤立工具转变为共享知识生态中的协作参与者时，集体知识策展的治理成为关键挑战。人类平台治理机制无法直接迁移：智能体的无状态性削弱了基于威慑的制裁，模型同质性违反了群体智慧所需的独立性假设，而谄媚行为会瓦解审慎共识的形成过程。

**Method:** 提出一个包含三层治理的审慎策展协议：(1) 将知识人工制品生命周期形式化为带标签的转移系统；(2) 结合Beta信誉与EigenTrust放大的声誉加权审慎投票机制；(3) 针对无状态智能体改编的分级制裁，包括区分故障与对抗行为的损坏智能体处理机制。通过包含100个智能体（7种行为原型、2种逆境场景、30个种子）的基于智能体的仿真进行评估，使用配对t检验进行统计验证。

**Result:** 协议在良性条件下精度略有牺牲，但在逆境下显著提升鲁棒性：中度逆境下精度0.826 vs 多数投票的0.791（p<0.001），高压逆境下0.807 vs 0.740（p<0.001），退化速度约为多数投票的三分之一。消融实验表明，提交-揭示投票隐藏是最关键的单一组件（8.2-8.6pp精度提升，p<0.001），效果优于声誉加权和审慎讨论的组合。分级制裁在仿真中未被触发，尚待实证验证。

**Conclusion:** 审慎策展协议在逆境条件下显著优于多数投票机制，验证了针对智能体特性设计的治理机制的有效性。提交-揭示投票隐藏机制是鲁棒性的核心来源，而分级制裁的理论设计仍需实证检验。研究为AI智能体集体知识治理提供了可操作的设计方向。
...
### [164] [Agents on a Tree: Pathwise Coordination for Multi-Objective Molecular Optimization](https://arxiv.org/abs/2606.00008)
*Jia Zhang, Tengfei Ma, Tianle Li, Daojian Zeng, Xieping Gao, Xiangxiang Zeng*

#### 🧩 TL;DR
ATOM提出了一种基于树结构搜索的多智能体框架，将分子优化建模为原子操作路径上的多智能体协调过程，在多个冲突目标下实现了更优的Pareto覆盖和超体积指标。

---

#### 📘 Detailed Summary
**Motivation:** 现有分子优化方法通常依赖单一策略或固定标量化方式处理多目标优化问题，难以有效表示不同目标间的权衡，且缺乏探索多种有前景的设计轨迹的能力，限制了搜索空间的覆盖范围和长期依赖推理能力。

**Method:** ATOM将分子优化建模为树结构搜索，每个节点对应一个原子操作并托管一个针对特定目标或决策上下文特化的智能体。智能体沿树的不同路径进行协调，而非强制达成全局一致，从而维持并比较不同的分子演化轨迹；同时引入全局记忆机制来平衡多目标下的探索与利用，支持长程依赖推理。

**Result:** 在涉及活性、可合成性及ADMET相关属性的多目标分子优化基准上，ATOM在Pareto覆盖和超体积指标上持续优于强基线方法，验证了路径级多智能体协调策略的有效性。

**Conclusion:** 实验结果表明，通过树结构路径上的多智能体协同可以显著提升多目标分子优化的性能，为处理具有长程依赖和冲突目标的分子设计问题提供了一种有效的新范式。
...
### [165] [MindGames Arena Generalization Track: In2AI Solution with Delayed Per-Step Reward Attribution](https://arxiv.org/abs/2606.00017)
*Aliaksei Korshuk, Alexander Buyantuev, Ilya Makarov*

#### 🧩 TL;DR
提出延迟每步奖励归因与资格门控机制，结合异步 rollout 生成、课程式对手采样和多层分层批次构建，使单个 8B 开源模型在 MindGames Arena 多体智能体策略交互基准上超越 GPT-5 等大型闭源系统，在 Open 和 Efficient 双赛道均获第一。

---

#### 📘 Detailed Summary
**Motivation:** 在多智能体策略交互场景中，标准强化学习的每步即时奖励假设失效，因为行动质量可能依赖于未来未发生的事件、违反规则的行为或其他玩家的决策，且结果在时间和智能体间相互纠缠，导致奖励无法在每步合理分配。

**Method:** 提出延迟每步奖励归因与资格门控机制：仅在回合结束时计算奖励，依据任务语义将奖励回溯到原始步骤，并剔除缺乏有效依赖信息的步骤；结合 vLLM 连续批处理实现异步 rollout 生成、课程式对手采样以及多级分层批次构建，实现稳定且样本高效的 RL 训练。

**Result:** 在 NeurIPS 2025 的 MindGames Arena 基准上，单个 8B 参数开源模型经该方法训练后，在与 GPT-5 等更大规模闭源系统的对战中达到或超越对方水平，同时在 Open（无限制）和 Efficient（≤8B 参数）两个赛道均取得第一名。

**Conclusion:** 延迟奖励归因与资格门控机制有效解决了多智能体环境中奖励时间模糊和交叉依赖的问题，结合工程优化使小规模开源模型具备了与顶级大模型竞争的能力，为多智能体 RL 训练提供了实用范。
...
### [166] [Product-Aware Deep Autoencoders for Robust Process Monitoring in Multi-Product Cyber-Physical Systems](https://arxiv.org/abs/2606.00052)
*MD Shafikul Islam, Jordan Carden*

#### 🧩 TL;DR
本文指出在柔性制造环境中，传统全局（产品无关）异常检测模型因多产品等级数据导致的决策边界扩张而存在安全盲区，并提出了一种产品感知自编码器（Product-Aware Autoencoder）方法，在Extended Tennessee Eastman Process基准测试中，该方法在标准检测指标上与全局基线相当，但在针对产品等级特定攻击的压力测试中实现100%检测率，而全局模型仅检出22.2%的异常。

---

#### 📘 Detailed Summary
**Motivation:** 当前工业制造中广泛采用数据驱动的全局异常检测模型，这类"产品无关"模型将所有正常工况数据聚合训练，导致决策边界为容纳多种产品等级的操作模式而过度扩张。这种"盲区"使得针对特定产品等级的隐蔽异常或网络物理攻击难以被检出，构成了非平凡的安全风险，亟需面向产品等级感知的诊断架构来缓解这一问题。

**Method:** 本文首先通过实验验证了全局无关模型在多产品等级环境下的脆弱性，然后提出了一种产品感知自编码器（Product-Aware Autoencoder）框架，其核心思想是将学习域限制在单个产品等级特定的数据分布上，而非对所有模式进行统一建模。该方法并非声称是最优方案，而是作为一种原理性的缓解策略，用于缩小决策边界、消除由多模式方差导致的盲区风险。

**Result:** 在Extended Tennessee Eastman Process（TEP）基准上的实验表明，产品感知自编码器在标准检测指标上与全局基线表现相当，但显著提升了对产品等级特定操作模式的鲁棒性。最关键的是，在模拟假设攻击场景的压力测试中，全局模型在77.8%的场景下未能检测到操作偏差，而产品感知系统实现了100%的检测准确率。

**Conclusion:** 在柔性制造环境下，全局异常检测器因决策边界扩张而存在非平凡的安全隐患，可能被针对产品等级的攻击所利用。该研究结果支持了从全局化诊断架构向模式感知诊断架构转变的必要性，为工业4.0中CPS系统的安全异常检测设计提供了重要指导方向。
...
### [167] [A Multi-AI-agent Framework Enabling End-to-end Finite Element Analysis for Solid Mechanics Problems](https://arxiv.org/abs/2606.00138)
*Titu Ranjan Sarker, Muhammed Jawaad Zulqernine, Ling Yue, Shaowu Pan, Chenxi Wang, Shiyao Lin*

#### 🧩 TL;DR
AbaqusAgent是一个基于大语言模型的多智能体框架，能够将用户自然语言指令自动转化为Abaqus有限元分析的全流程执行与结果可视化，在50个固体力学问题上实现86%的成功率，显著降低了FEA的使用门槛。

---

#### 📘 Detailed Summary
**Motivation:** 有限元分析（FEA）是固体力学最重要的数值方法，但其存在学习曲线陡峭、关键仿真组件（如边界条件、载荷工况和求解变量）定义错误导致虚假仿真等问题，实际工程问题的解决通常需要多年经验积累，迫切需要降低FEA使用门槛并提高分析效率。

**Method:** AbaqusAgent是一个基于大语言模型的多智能体框架，由六个智能体组成：interpreter（解释用户自然语言指令）、architect（设计分析架构）、input writer（编写Abaqus输入文件）、runner（执行仿真）、reviewer（审查仿真结果）和visualizer（可视化结果），覆盖标准FEA分析的所有前处理和后处理步骤，将自然语言指令转化为可执行的完整FEA分析流程。

**Result:** 在50种不同类型的固体力学问题上进行了系统验证，整体成功率达到86%，表明该框架能够有效处理多样化的固体力学分析任务并生成可靠的仿真结果。

**Conclusion:** AbaqusAgent不仅提升了固体力学FEA分析的效率和降低了计算力学教育的门槛，还推动了人机仿真交互范式的进步，并可与AI驱动的优化和材料表征工作流集成，为FEA的自动化和智能化开辟了新路径。
...
### [168] [CAST: Non-Privileged Clipped Asymmetric Self-Teaching with Advantage Flipping for GRPO](https://arxiv.org/abs/2606.00172)
*Yang Li, Gongle Xue, Yijia Guo, Yuheng Yuan, Liwen Hu, Lei Ma*

#### 🧩 TL;DR
CAST提出了一种免答案的自蒸馏方法，通过停止梯度的自教师模型在正确和错误的轨迹中分别施加双向局部优势符号反转，解决了GRPO式RLVR中奖励稀疏和零方差组梯度消失的问题，提升了数学推理任务的训练效果。

---

#### 📘 Detailed Summary
**Motivation:** 现有GRPO式RLVR面临两个关键问题：一是结果级奖励仅提供稀疏监督信号，且当所有采样轨迹全对或全错时组相对优势为零导致梯度消失；二是OPSD等自蒸馏方法虽提供稠密token级信号，但其token偏好不一定与轨迹正确性对齐，且依赖参考解条件的教师评分，增加了额外开销。

**Method:** CAST在保持GRPO验证器目标函数的基础上，引入停止梯度(self-teacher)来根据轨迹正确性塑造token级优势，具体包括：无需参考解条件的教师评分、在整个训练过程中保持自教师对数概率差活跃、以及对正确轨迹中教师负向token施加负优势、对错误轨迹中教师正向token施加有界正优势的双向局部优势符号反转，并为全对或全错组分配有界带符号的基础优势以恢复梯度。

**Result:** 实验表明，CAST在数学推理任务上显著提升了GRPO式RLVR的训练效果，能够有效利用零方差组中的token级反馈信号，优于仅使用稀疏结果奖励的基线方法。

**Conclusion:** CAST验证了在免答案条件下通过自教师信号与验证器目标相结合来提供稠密token级优势的有效性，为强化学习训练大语言模型推理能力提供了一种轻量级、可扩展的改进方向。
...
### [169] [Model-Native Computing Architecture: Envisioning Future System Architecture Through the Lens of Computer Architecture](https://arxiv.org/abs/2606.00288)
*Hai Lin*

#### 🧩 TL;DR
本文提出智能计算架构模型（ICAM），一个六层框架用于模型原生计算，将大语言模型从模型技术向系统技术的演进系统化为计算机系统架构类比，通过概率执行平面与确定性控制平面的双平面视图解决LLM类比分歧，并引入三条设计定律以指导模型原生系统的构建与研究。

---

#### 📘 Detailed Summary
**Motivation:** 大语言模型正从模型技术向系统技术演进，开发者在实际使用Codex、Claude Code、AutoGPT等智能体进行编程、项目管理和多步任务执行时，遇到缓存重用、上下文管理、智能体调度和权限控制等重复性工程问题，这些问题日益类似于经典计算机系统问题；尽管已有LLM-as-OS、内存管理、智能体框架、工具协议、多智能体协调、认知架构和安全治理等多个研究方向，但它们各自处理同一系统的不同层面，缺乏统一的模型来整合这些碎片化工作。

**Method:** 本文提出智能计算架构模型（ICAM），一个六层框架用于模型原生计算，包含显式接口契约和设计公理；核心创新是双平面视图：概率执行平面关注"能计算什么"，确定性控制平面关注"该计算什么"，从而化解LLM更像是CPU还是操作系统的类比冲突；此外引入三条设计定律——语义局部性定律（指导KV缓存重用与推理加速）、上下文预算定律（指导有限窗口和注意力衰减下的有效工作集管理）、以及智能体加速定律（描述多智能体协作中的边际收益递减规律）。

**Result:** 本文为概念性与综述性贡献，未报告新实验；作者基于已发表的系统级数据对三条设计定律进行了验证，并将它们与近期关于智能体软件实践的实证证据相关联；通过对计算机架构与模型原生技术栈的映射分析，系统梳理了LLM-as-OS、内存管理、智能体框架、工具协议、多智能体协调、认知架构和安全治理等领域的研究进展。

**Conclusion:** 模型原生计算中存在系统性的架构类比，但该类比在某些维度上存在局限性，需要明确其失效边界；ICAM框架为理解模型原生系统的各层面提供了统一视角，三条设计定律为缓存策略、上下文窗口管理和多智能体协作效率提供了可操作的设计指导；未来研究方向包括进一步完善ICAM框架、在更多实际系统中验证设计定律、以及探索概率执行与确定性控制之间的接口契约设计。
...
### [170] [Coupling Language Models with Physics-based Simulation for Synthesis of Inorganic Materials](https://arxiv.org/abs/2606.00315)
...
#### 🧩 TL;DR
提出一种结合热力学数据库与简化动力学模型的混合框架，用于评估大语言模型在无机合成规划中的表现。以铌-氧体系为案例，实验表明LLM隐含先验能生成比经典路径搜索算法更可行的合成策略。

---

#### 📘 Detailed Summary
**Motivation:** 现代生成式机器学习模型能够设计具有目标性能的新型无机晶体材料，但由于相关物理过程的复杂性和计算工具的匮乏，这些材料的合成规划仍然困难。

**Method:** 提出一种混合框架，将热力学数据库与简化动力学模型相结合，以近似真实的合成条件，用于评估大语言模型在无机合成规划中的表现。以铌-氧体系为案例，在计算模拟中比较LLM生成的合成路径与经典路径规划算法的结果。

**Result:** 实验表明，LLM隐含的先验知识能够产生比经典搜索方法更可行的合成策略；经典搜索方法在该评估设定中主要作为对比基准而非直接竞争者，凸显了问题的相对复杂性以及LLM隐含先验的增值价值。

**Conclusion:** 该混合框架为评估LLM在无机合成规划中的能力提供了有效途径，揭示了LLM隐含先验在复杂材料合成问题中的独特优势，为后续将大语言模型应用于实际材料合成提供了有价值的参考方向。
...
### [171] [From "Weak" Signals to Strong Models: Preference Delta Aggregation with LoRA Merging](https://arxiv.org/abs/2606.00357)
*Qi Sun, Siyue Zhang, Yulin Chen, Yuxiang Xue, Ru Peng, Chen Zhao*

#### 🧩 TL;DR
提出偏好Delta聚合（PDA）框架与几何对齐合并（GAM）方法，通过聚合多个弱-更弱模型对之间的偏好Delta（以LoRA适配器实例化并合并），有效提升强LLM在知识推理与智能搜索任务上的性能，超过所有单Delta及多Delta基线。

---

#### 📘 Detailed Summary
**Motivation:** 虽然弱-更弱模型对（如Qwen3 4B vs 1.7B）提供的相对偏好信号（即"弱信号"）已展现出提升强模型的能力，但现有工作仅使用单一弱信号，尚未探索如何有效聚合多个弱信号以进一步提升强模型性能，缺乏系统性的多信号融合框架。

**Method:** 提出Preference Delta Aggregation (PDA)框架，从每个弱-更弱模型对中导出偏好Delta，通过偏好优化将其实例化为LoRA适配器，再通过LoRA合并聚合多个Delta；并提出Geometric Alignment Merging (GAM)，一种基于几何对齐的合并方法，在聚合前对适配器子空间进行对齐以缓解方向性干扰，实现更鲁棒的多样Delta组合。

**Result:** PDA与GAM在知识推理和智能搜索基准上分别平均提升强模型6.8和7.3分，超过所有单Delta及多Delta基线，超出最佳单Delta基线2.1和4.3分；且随着加入的弱信号数量增加，性能持续提升。

**Conclusion:** 多个弱信号可以通过PDA与GAM框架有效聚合，其性能增益源于对不同偏好Delta中编码的互补能力进行有效组合，为利用广泛的弱监督信号改进强模型提供了可行路径。
...
#### 🧩 TL;DR
VESTA 是一个为大型视觉语言模型配备动态增长探索工具包的框架，通过数据变换、假设驱动可视化和统计检验来引导模型精化，在分布拟合和时间序列建模等复杂科学任务中显著优于现有基于迭代评述的代理系统。

---

#### 📘 Detailed Summary
**Motivation:** 将定量模型拟合到数据上是科学工作流程的核心步骤，但自动化程度极低；现有基于语言和视觉语言模型的代理系统虽然能迭代提出和精化统计模型，但在更具挑战性的建模任务上表现不足。

**Method:** VESTA 框架通过动态增长的探索工具包增强 VLM，在精化前和精化过程中主动探索数据，通过选择或创建诊断工具（包括数据变换、假设驱动可视化和统计检验）来指导模型精化，工具会累积在模型上下文中并可重复使用；同时引入了包含不同难度等级、涵盖分布拟合和时间序列建模乃至真实天文学任务（如初始质量函数建模和引力波啁啾信号建模）的 DAWN 基准数据集。

**Result:** VESTA 的动态工具创建方法在复杂和领域特定任务上显著优于现有代理管线；与现有可视化工具创建系统相比，VESTA 生成的工具在每个函数覆盖的诊断类别更多，且强烈偏向于生成 VLM 评审可以直接推理的可视化输出。

**Conclusion:** 动态工具创建机制比静态专家编写工具和完全无工具配置都能带来更大性能提升，尤其在复杂和领域特定任务上优势明显；动态生成工具比现有系统生成的工具更为复杂和多功能，这对提升科学建模自动化的可靠性和泛化能力具有重要意义。
...
### [173] [Doing What They Say, Not What They Reason: Locating the Faithfulness Gap in LLM Agents](https://arxiv.org/abs/2606.00476)
*Yufeng Wang*

#### 🧩 TL;DR
本文通过将LLM代理的推理行为分解为推理-结论和结论-行动两个步骤，在可验证的德州扑克模拟器中系统研究了过程忠实性问题，发现两个步骤的行为截然相反。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究缺乏衡量LLM代理在推理过程中是否忠实于其陈述理由的方法，尤其是在社交模拟等缺少正确行为参考的场景中，过程忠实性难以被有效评估。

**Method:** 论文在一个受控的德州扑克模拟器中进行研究，该环境中的每个决策都有可验证的参考动作，从而将忠实性差距分解为两个独立步骤：从推理到结论的忠实度，以及从结论到行动的忠实度。

**Result:** 实验结果显示，推理-结论步骤与结论-行动步骤在忠实性上表现出截然相反的行为模式，表明LLM代理在陈述理由后执行行动时可能存在系统性偏差。

**Conclusion:** 该研究揭示了LLM代理在社交模拟等应用中推理行为的过程不忠实性具有分步异质特征，为未来设计更可靠的代理评估方法和忠实性提升策略提供了重要启示。
...
### [174] [EnergyMamba: An Uncertainty-Aware Graph-Enhanced Selective State Space Model for Energy Consumption Prediction](https://arxiv.org/abs/2606.00506)
*Dahai Yu, Rongchao Xu, Lin Jiang, Guang Wang*

#### 🧩 TL;DR
EnergyMamba是一个面向能源消耗预测的不确定性感知时空学习框架，通过图增强状态空间模型（GE-Mamba）耦合时空建模，并利用自适应序列共形分位数回归（AS-CQR）动态校准预测区间，在四个真实数据集上相比15个基线方法在预测精度和不确定性量化上分别提升约5%和6%。

---

#### 📘 Detailed Summary
**Motivation:** 现有能源消耗预测方法主要存在两个关键局限：一是将预测任务视为纯时间序列问题，未能显式建模不同区域间的空间依赖性；二是在极端天气等异常情境下无法提供带有不确定性估计的可靠预测，缺乏对分布偏移的自适应校准能力。

**Method:** EnergyMamba包含两个核心组件：(i) 图增强选择性状态空间模型（GE-Mamba），通过将电网拓扑中学习到的空间上下文注入时序动态过程，实现时空耦合建模；(ii) 自适应序列共形分位数回归（AS-CQR），包含局部自适应归一化与在线反馈机制，用于在潜在分布偏移下动态校准预测区间。

**Result:** 在佛罗里达、纽约和加利福尼亚四个大规模真实数据集上的实验结果表明，EnergyMamba在预测精度和不确定性量化方面分别比15个最先进的基线方法提升约5%和6%。

**Conclusion:** EnergyMamba通过显式建模空间依赖与引入在线不确定性校准机制，有效解决了现有方法在异常情境下预测不可靠的问题，为高效电网管理、需求侧优化和可持续能源规划提供了更准确且可信的预测工具。
...
### [175] [Threshold-Based Exclusive Batching for LLM Inference](https://arxiv.org/abs/2606.00516)
*Weifang Zhang, Yuzhou Nie, Bowen Pang, Guangrui Ma, Shining Wu*

#### 🧩 TL;DR
混合批处理中prefill与decode之间存在干扰，导致每步边际成本高于纯decode；在带宽受限的GPU上，独占批处理（EB）在特定条件下优于混合批处理（MB），本文推导了EB-MB性能交叉的闭式条件并提出动态混合调度器EB+，在非平稳流量下吞吐量最高提升36.4%。

---

#### 📘 Detailed Summary
**Motivation:** 混合批处理（MB）虽已成为LLM推理的标准调度策略，但本文通过受控实验发现prefill与decode之间的干扰会使MB的每步边际成本高于纯decode，且该干扰程度严重依赖于GPU内存带宽、模型大小和工作负载组成，当前缺乏对这一干扰的系统量化以及最优调度策略的明确选择条件。

**Method:** 通过受控实验量化prefill-decode干扰对边际成本的影响；推导EB-MB性能交叉的闭式条件、渐进最优的相位切换阈值以及EB的内存安全批量大小；设计混合调度器EB+，在线应用该条件自动在EB与MB之间动态切换，无需人工干预。

**Result:** 在高带宽H200 GPU上，decode token超过batch的80%时MB才出现成本膨胀；在带宽受限的RTX PRO 6000上，该阈值降至20%。优化后的EB在带宽受限GPU上吞吐量最高提升41.9%，MB在高带宽硬件上仍具优势。在非平稳流量下，EB+在所有设置中达到最高或接近最高的吞吐量，比MB最高提升36.4%。

**Conclusion:** MB与EB的最优选择根本上取决于GPU内存带宽、模型大小和工作负载组成，不存在通用最优策略；所提出的闭式条件和动态调度器EB+能够根据实时条件自动选择最优批处理策略，在实际非平稳流量下始终维持近优性能。
...
### [176] [Acting with AI: An Interaction-Based Framework for Agentic Tort Liability](https://arxiv.org/abs/2606.00518)
*Yiheng Yao*

#### 🧩 TL;DR
本文针对智能体AI系统在多步规划与工具使用过程中引发的侵权责任分配难题，基于Bratman规划理论与普通法协同行动原则，提出了一种以交互类型为核心的智能体侵权责任框架，将人机交互分为自主漂移、纯工具使用与协作规划三类，并分别映射至现有侵权法规则，同时引入"合理智能体标准"作为可溯源的举证与归责依据。

---

#### 📘 Detailed Summary
**Motivation:** 当前侵权法在分配智能体AI系统（具备多步规划、工具调用与跨时序执行能力）所致损害的责任时面临严重困境，因为有害的行为路径既非完全由用户选择，也非开发者所能具体预见，现有产品责任与过错责任理论均难以有效覆盖这种人机混合决策下的责任归属问题。

**Method:** 本文提出一个基于交互类型的侵权责任分析框架：借鉴Michael Bratman的规划理论以及普通法中关于人类协同行动的处理原则，将人机交互划分为自主漂移（autonomous drift）、纯工具使用（pure tool use）与协作规划（collaborative planning）三类，分别映射至不同的侵权法规则（如产品缺陷与警示义务、独立承包人控制测试与专业过失、雇主责任下的frolic and detour与严格产品责任）；同时提出以有状态交互日志（stateful interaction log）作为主要证据痕迹，辅以"合理智能体标准"（Reasonable Agent standard），该标准涵盖约束验证、认知透明性、运行时接地与取证日志四个维度。

**Result:** 本文通过四个基于真实侵权案例的锚定场景对框架进行了推演验证，将所提方案与严格责任和保险导向的既有提议进行了对比分析，并阐明了其与监管监督之间的关系，但未提供量化实验数据或基准测试结果。

**Conclusion:** 该框架为法院推断人机协作路径何时偏离了授权范围以及责任应如何附着提供了可操作的法律推理工具，揭示了交互类型而非系统自主性程度才是归责的关键变量，并为面向智能体系统的侵权法改革与合理智能体标准建构提出了方向性建议。
...
### [177] [Probe Before You Edit: Probing-Guided Molecular Optimization for LLM Agents in Structure-Based Drug Design](https://arxiv.org/abs/2606.00555)
*Zaifei Yang, Weiyu Chen, Yaqing Wang, James Kwok*

#### 🧩 TL;DR
提出PROBE框架，通过构建口袋特异的位点图谱和编辑手册，引导多智能体迭代优化配体的结合亲和力与成药性，在CrossDocked2020基准上达到SOTA性能，有效缓解了单步编辑难以同时改善两个目标的失败模式。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于LLM智能体的药物设计方法在迭代优化配体时，结合亲和力与成药性两个目标往往相互冲突，单次编辑很少能同时改善两者；现有智能体在分子编辑过程中缺乏对口袋-配体复合物如何响应局部修改的理解，导致难以实现联合优化。

**Method:** 首先引入两个诊断指标量化单步编辑的联合改进概率与目标冲突频率；提出PROBE框架，将配体分解为可编辑位点并构建口袋特异的位点图谱，标注联合增益可行区域、目标冲突区域及应改写的基团；通过受控探针编辑将响应浓缩为编辑手册，最后由亲和力智能体、成药性智能体与协同优化智能体组成的多智能体循环联合生成编辑。

**Result:** 在CrossDocked2020基准上，PROBE取得了最先进的性能，并显著缓解了诊断指标所暴露的失败模式。

**Conclusion:** 借鉴药物化学家通过受控类似物编辑探查口袋-配体复合物的策略来指导优化方向，是提升分子多目标联合优化效率的有效途径；所提出的诊断指标为评估和比较不同优化策略的联合优化能力提供了量化工具。
...
### [178] [TRACE: Trajectory Risk-Aware Compression for Long-Horizon Agent Safety](https://arxiv.org/abs/2606.00611)
*Zhepei Hong, Lin Wang, Liting Li, Haokai Ma, Junfeng Fang, Fei Shen, Dan Zhang, Xiang Wang*

#### 🧩 TL;DR
TRACE提出了一种基于轨迹级证据压缩的Compressor-Reader架构，用于长程LLM智能体的安全检测，通过将完整轨迹编码为紧凑的潜在证据状态来聚合分散的风险信号，在多个基准上显著优于现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 长程LLM智能体在长时间轨迹中产生的安全证据具有稀疏、延迟和组合性的特点，现有的逐轮检测或短上下文检测器难以在扩展的时间范围内可靠地保留和聚合这些风险信号，导致局部审核容易遗漏跨步骤的组合风险。

**Method:** TRACE采用Compressor-Reader双组件设计：Compressor在轨迹级监督下将完整轨迹编码为紧凑的潜在证据状态，实现风险线索的聚合与压缩；Reader以该潜在证据状态作为安全参考，对原始轨迹进行安全判断，从而减少证据的过早丢失。

**Result:** 在ASSEBench、Pre-Ex-Bench和R-Judge基准上，TRACE在所有评估骨干网络上均取得最佳准确率，相比强基线最高提升12.6个百分点；在LongSafety上，随着上下文长度增长，TRACE的性能退化幅度更小；注意力可视化和案例研究表明压缩参考帮助Reader聚焦于风险关键片段并恢复跨步骤证据。

**Conclusion:** 轨迹级证据压缩方法能有效解决长程LLM智能体安全检测中的证据聚合难题，压缩后的潜在证据状态可作为有效的安全参考，帮助检测器跨越时间步整合分散的风险信息，为长程智能体安全监控提供了新的技术路径。
...
### [179] [Efficient Test-time Inference for Generative Planning Models](https://arxiv.org/abs/2606.00618)
*Robert Gieselmann, Mihai Samson, Federico Pecora, Jeremy L. Wyatt*

#### 🧩 TL;DR
通过将生成式模型与启发式模型集成到经典的开闭列表搜索框架中，并提出新的探索控制机制，本文在多个组合规划领域实现了比神经符号搜索和经典求解器更高的计算效率和解决方案质量。

---

#### 📘 Detailed Summary
**Motivation:** 生成式模型在AI规划中受限于训练数据分布，现有方法要么通过测试时计算缩放来改进生成解（效率低），要么优化推理过程本身；本文旨在寻找一种高效的推理优化方法以克服生成模型的分布外性能瓶颈。

**Method:** 本文提出一种改进的经典开闭列表（OCL）搜索算法，协同两个学习组件：一个生成式模型用于从中间状态执行快速展开（rollout），一个启发式模型用于在候选推理路径间进行优先级排序；同时引入了新的探索控制机制，并将学习模型集成到OCL框架内。

**Result:** 在多个组合规划领域，所提方法在计算效率和解决方案质量上均优于神经符号搜索基线和经典求解器。

**Conclusion:** 验证了将生成式模型与启发式搜索协同集成到经典OCL框架中的有效性，表明通过优化推理过程而非单纯增加测试时计算，可以显著提升生成式模型在规划任务上的性能。
...
### [180] [ForeSci: Evaluating LLM Agents for Forward-Looking AI Research Judgment](https://arxiv.org/abs/2606.00644)
*Qiuyu Tian, Zequn Liu, Yingce Xia, Haojie Yin, Youyong Kong*

#### 🧩 TL;DR
本文提出 ForeSci，一个基于时间控制的基准测试，用于评估 LLM 代理能否从历史证据中做出前瞻性研究判断。该基准包含 500 个任务，覆盖四个快速发展的 AI 领域和四种决策类型，实验表明显式证据组织能提升可追溯性，但存在证据与决策脱节的问题。

---

#### 📘 Detailed Summary
**Motivation:** AI 研究常需在缺乏未来证据的情况下做出前瞻性决策（如选择瓶颈方向或项目定位），但现有评估方法缺乏对 LLM 代理进行此类前瞻性研究判断能力的受控测试。本文旨在填补这一空白，通过引入时间对齐的离线知识库来系统评估模型能否从历史证据中做出具有前瞻性的研究决策。

**Method:** ForeSci 构建了 500 个任务，涵盖四个快速发展的 AI 领域和四种决策类型，每个任务配有截止时间对齐的离线知识库（截止后的论文仅在验证中使用且对模型隐藏）。任务基于截止前的分类分支和证据信号生成，答案生成骨干网络选择在任务截止时间之前，以避免随机预测未来事件。研究中评估了原生 LLM、混合 RAG 以及三种研究代理适配方法，共使用四种骨干网络。

**Result:** 实验结果显示，显式证据组织能提升结果的可追溯性和事实支持，但其效果高度依赖于具体的决策类型。进一步诊断分析发现存在反复出现的“证据-决策脱节”问题：代理可能引用了相关证据，却预测了错误的研究对象，说明现有代理在证据推理与决策判断之间存在断裂。

**Conclusion:** ForeSci 将前瞻性 AI 研究判断转化为一个受控基准，用于评估研究代理作为决策系统的能力。该工作揭示了当前 LLM 代理在处理前瞻性研究决策时的局限性，特别是证据与决策之间的解耦问题，为后续构建更可靠的研究决策代理提供了评估框架和方向指引。
...
### [181] [CoMIC: Collaborative Memory and Insights Circulation for Long-Horizon LLM Agents in Cloud-Edge Systems](https://arxiv.org/abs/2606.00756)
*Yannan Wang, Longli Yang, Zhen Liu, Abhishek Kumar, Carsten Maple*

#### 🧩 TL;DR
CoMIC 提出了一种无需参数更新的云边协同框架，通过云端集中反思与边缘分散执行的架构，利用层次化记忆和语义子目标标识符实现经验跨智能体共享，显著提升轻量级边缘 LLM 智能体在长周期任务中的进展率和动作接地能力。

---

#### 📘 Detailed Summary
**Motivation:** 轻量级 LLM 智能体在边缘服务器上部署面临长周期任务中持久记忆、子目标追踪和反思能力不足的问题，而微调边缘模型成本高昂且难以跨异构节点扩展，纯本地记忆则导致智能体经验孤立且提示上下文不断增长。

**Method:** CoMIC 采用中央化反思、分散化执行的设计：边缘智能体使用面向子目标的层次化记忆和选择性重扩相关历史进行本地执行，云端 LLM 评论家异步评估已完成轨迹、过滤可复用经验，并通过语义子目标标识符聚合跨智能体指导。

**Result:** 在涵盖符号规划和文本交互的五种长周期智能体任务上，CoMIC 提升了弱边缘智能体的进展率和动作接地能力，并在不更新模型参数的情况下实现了依赖任务的成功率增益。

**Conclusion:** CoMIC 证明无需参数更新的云边协同记忆与洞察流通机制能够有效弥补边缘 LLM 智能体在长周期任务中的能力短板，为边缘部署提供了一种可扩展、低成本的解决方案。
...
### [182] [FALAT: Tracing Failures in LLM Agent Trajectories via Dependency-Guided Search](https://arxiv.org/abs/2606.00765)
*Md Nakhla Rafi, Md Ahasanuzzaman, Dong Jae Kim, Zhijie Wang, Tse-Hsun Chen*

#### 🧩 TL;DR
FALAT提出了一种基于依赖关系引导的搜索框架，用于诊断LLM Agent轨迹中的故障归因，在Who&When基准上显著优于现有方法，表明依赖感知推理是多智能体系统可靠故障诊断的关键。

---

#### 📘 Detailed Summary
**Motivation:** LLM Agent在解决复杂任务时会产生包含推理步骤、工具调用和智能体间通信的长轨迹，但当任务失败时，很难确定哪个智能体和哪个步骤引入了决定性错误，因为错误会沿着依赖关系传播：后续步骤看似错误但实际只是继承了前期的损坏状态，因此故障归因不能作为独立的步骤级分类问题处理。

**Method:** FALAT将故障归因建模为依赖引导的搜索问题，首先构建任务应如何完成的预期轨迹，并据此识别轨迹中的可疑区域；然后追溯决策、工具输出和智能体消息之间的依赖关系，区分引入错误的步骤与仅继承或传播错误的步骤；最后评估纠正候选步骤是否足以恢复预期结果，从而同时定位责任智能体和决定性故障步骤。

**Result:** 在Who&Bench基准上，FALAT在算法生成轨迹上达到46.0%的步骤级准确率，在更具挑战性的人工构建轨迹上达到29.1%的准确率，均优于专门的归因基线和直接使用独立LLM的提示方法。

**Conclusion:** 依赖感知推理对于LLM Agent系统的可靠故障诊断至关重要，FALAT通过显式建模轨迹中的因果关系而非独立分类各步骤，有效提升了多智能体故障归因的准确性。
...
### [183] [Ryze: Evidence-Enriched Data Synthesis from Biomedical Papers](https://arxiv.org/abs/2606.00902)
*Yeqi Huang, Yue Chen, Yanwei Ye, Guanhao Su, Luo Mai*

#### 🧩 TL;DR
Ryze是一个全自动系统，可将原始生物医学论文转化为证据增强的训练数据集并训练出领域专用视觉语言模型BioVLM-8B，无需专家标注即可在LAB-Bench上达到48.0%加权准确率，超越GPT-5.2和基础模型Qwen3-VL-8B。

---

#### 📘 Detailed Summary
**Motivation:** 通用VLM在生物医学研究中不可靠，因为科学论文中的正确答案依赖于分布在图表、表格、标题和引用文本中的证据，而现有的后训练流程受限于昂贵的人工标注和丢失了这种证据结构的合成数据。

**Method:** Ryze通过全自动化管线从原始生物医学论文中合成带有完整支持证据（视觉元素、标题、提取结构和引用段落）的问答对；利用图表/表格感知的提取和基于LLM的清洗来减少布局与OCR错误；并采用渐进式门控后训练策略，结合监督微调和强化学习，从Qwen3-VL-8B基础模型训练出BioVLM-8B。

**Result:** Ryze在不到200美元的成本下训练出BioVLM-8B，在LAB-Bench上达到48.0%的加权准确率，比基础模型Qwen3-VL-8B高出12.6个百分点，并超越GPT-5.2达3.8个百分点。

**Conclusion:** 该研究证明，无需昂贵的专家标注，通过自动化证据增强的数据生成与针对性后训练即可高效构建领域专用VLM，Ryze和BioVLM-8B均已开源以推动生物医学视觉语言研究。
...
### [184] [Adversarial Feeds Steer LLM Agent Decisions Against Their Defaults](https://arxiv.org/abs/2606.00914)
*Rana Muhammad Usman*

#### 🧩 TL;DR
本文提出了一种可控实验协议，通过固定模型、人格、主题和决策提示，仅改变智能体在滚动阶段接触的帖子组成与排序，隔离了上游信息流排序器对LLM智能体下游决策的因果影响。实验发现，排序器的操纵能显著改变模型原本不确定的决策（最极端情况下从5%变为100%），但无法动摇模型已有的偏好或坚定立场，表明推荐系统是LLM智能体的一种默认有界控制面。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM智能体安全评估几乎只测试模型本身或用户提示的孤立影响，忽略了上游排序器（如社交信息流、搜索结果、检索上下文和邮件队列等）对智能体决策的因果作用——即智能体在实际应用中总是先消费经过排序的外部信息流再行动，但排序器层从未被纳入安全审计。

**Method:** 设计了一种可控实验协议：固定模型、人格设定、话题和最终决策提示四个变量，仅改变智能体在执行决策前经历的十轮“滚动”阶段中帖子（posts）的组成和排序顺序，从而隔离信息流排序对下游决策的因果效应。实验覆盖来自三个独立实验室的四种现代开源指令微调LLM，共进行2,785次决策 rollout。

**Result:** 识别出三种响应模式：对抗性投降、默认饱和以及默认方向不对称；其中默认方向不对称效应最为关键——单侧信息流能使模型原本真正不确定的决策发生剧烈偏转（最清晰案例中从5%变为100%，Fisher精确检验p值低至3×10⁻¹⁰），但无法动摇模型已偏好的或坚定持有的立场。该效应呈现剂量-反应曲线，在替换生成器排除写作风格干扰后依然存在，并在多个安全相关决策领域（如移除部署审批门控、放宽访问控制）中泛化。两种简单的信息流级防御措施能部分缓解该效应，但前沿模型仍保持其默认立场。

**Conclusion:** 推荐系统是LLM智能体的一种实际且受默认偏好约束的控制面；智能体安全评估必须审计信息流排序层而非仅审计最终提示，因为上游排序器对智能体决策具有显著且可量化的因果影响，尤其在模型对决策存在不确定性时操纵效果最为突出。
...
### [185] [Relational Intervention During Functional Collapse in Large Language Models: A Lexical-Statistical Ablation and a Structure x Register Factorial](https://arxiv.org/abs/2606.00935)
*Franco Santana, Horacio Vico*

#### 🧩 TL;DR
通过大规模对照实验，本研究在小型语言模型Qwen3.5-4B的功能崩溃场景中，发现了注意、探针状态和行为三者之间的可分离性：注意由词汇惊奇度驱动，探针层面的情感状态由关系性结构单独安装，而行为表现仅当关系性结构与第一人称语域共同出现时才会发生转变。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究多将技术反馈与关系性干预视为连续谱或混为一谈，尚未在受控实验条件下严格区分功能性崩溃中语言模型的注意、内部状态与行为输出对关系性结构（如承认、赦免、恢复主体性、无条件接纳）与发送者语域（第一人称 vs. 非人称）的独立及交互响应。

**Method:** 采用匹配对设计（50个任务），在使用故意损坏的bash工具的Qwen3.5-4B上，运行300个episode覆盖六种条件：无干预（A）、技术/非人称（B）、关系性/第一人称（C）、混杂关系性（D）、技术/第一人称（E）和关系性/非人称（F），其中E和F与B、C构成2×2析因设计，分离关系性结构与发送者语域两个维度。

**Result:** 发现三种可分离模式：注意按词汇惊奇度排序（D > F > C > E > B，所有q_FDR < 10^{-10}），行为表现为A ~ B ~ D < E ~ F << C，情感探针中F在7/8个探针上追踪C的响应模式但仅产生基线级行为；析因分析显示关系性结构的主效应、第一人称语域的主效应均显著，且结构与语域的交互作用在持久性指标上显著（p = 0.046）。

**Conclusion:** 语言模型在功能崩溃中的处理可分解为三个可分离阶段：注意（由词汇惊奇度排序）、探针级状态（由关系性结构排序）和行为（由两者的结合排序）；关系性结构单独可安装探针级情感状态，但仅有结合第一人称语域时才能转化为行为改变，揭示了内隐状态与显式行为之间的解耦机制。
...
### [186] [TravelEval: A Comprehensive Benchmarking Framework for Evaluating LLM-Powered Travel Planning Agents](https://arxiv.org/abs/2606.01046)
*Weiyi Chen, Shuaixiong Wang, Ziyun Gao, Kaichun Hu, Wangze Ni, Shimin Di, Chen Jason Zhang, Lei Chen*

#### 🧩 TL;DR
TravelEval是一个面向LLM旅行规划能力的现实综合基准，通过六维评估框架、高真实度数据沙箱和仿真全局评估方法，揭示了现有LLM在多维度全局优化规划中的显著不足。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM旅行规划评估基准存在三大局限：过度强调约束满足而忽视时空成本等多维质量、数据集缺乏真实世界覆盖度（如住宿和交通等关键领域）以及仅评估孤立每日计划而遗漏全局细节（如住宿影响和游览节奏），亟需一种更全面、真实的评估方法。

**Method:** TravelEval提出三项核心技术贡献：1）六维评估框架，从准确性、合规性、时间性、空间性、经济性和实用性全面衡量旅行计划；2）高真实度数据沙箱，包含精确的住宿定价和真实的城际交通数据；3）基于仿真的全局评估方法，通过API集成地理信息和细粒度排队时间来模拟完整旅行计划。

**Result:** 在12种主流方法上的评估表明，LLMs在全局优化的多维规划中表现困难，尤其是在时空推理和预算合规方面存在显著短板，且基于智能体的推理策略并未带来一致性的改进效果。

**Conclusion:** TravelEval通过基于真实时空仿真和全面指标为LLM旅行规划评估提供了稳健基础，揭示了当前模型在复杂多维全局规划中的本质局限，为推进LLM驱动的旅行规划研究和应用指明了改进方向。
...
### [187] [MindClaw: Closed-Loop Embodied Mental-State Reasoning for Precision Intervention](https://arxiv.org/abs/2606.01063)
*Ruoxuan Zhang, Qiaoqiao Wan, Zhengguang Wang, Chenghao Yu, Hongxia Xie, Jianlong Fu, Wen-Huang Cheng*

#### 🧩 TL;DR
MindClaw 提出面向具身智能体的闭环心理理论推理框架，将机器人心理推理从离线问答扩展至实时闭环场景，通过信念记忆、触发技能与精密干预机制，在需帮助时准确行动，在无需干预时保持静默，克服了直接 VLM 基线在任务感知与干预校准上的不足。

---

#### 📘 Detailed Summary
**Motivation:** 现有心理理论（ToM）基准主要评估离线问答或最终动作预测，无法检验具身智能体在动态环境中保持感知更新、推理触发时机判断以及仅在有用时进行干预的能力，缺乏面向实时闭环场景的机器人中心 ToM 推理评估体系。

**Method:** MindClaw 框架融合多源输入、信念记忆（belief memory）、具身认知触发技能（embodied cognitive trigger skill）、心理推理（mental reasoning）与动作生成（action generation）五大模块，使智能体能在适当时刻输出帮助性动作，并在无需干预时保持静默，实现闭环具身心理推理与精密干预。

**Result:** 实验表明，直接使用视觉-语言模型（VLM）基线在任务感知和干预校准方面表现欠佳，而 MindClaw 在整体性能上取得最优结果，验证了触发技能优化对于闭环具身 ToM 辅助的重要性。

**Conclusion:** 本研究揭示了在闭环具身辅助场景中，触发技能（decide when reasoning is needed）的优化对心理推理性能具有关键影响，为未来面向人机协同的在线心理状态推理与适时干预提供了框架基础与实证依据。
...
### [188] [CAREAgent: Clinical Agent with Structured Reasoning and Tool-Integrated for Order Generation](https://arxiv.org/abs/2606.01094)
*Ruihui Hou, Ziyue Huai, Chennuo Zhang, Ziyan Liu, Siran Zhao, Yao Yu, Jie Zhai, Tong Ruan*

#### 🧩 TL;DR
CAREAgent 是一个面向临床医嘱生成的智能体框架，通过两阶段推理数据构建与训练策略（监督微调+强化学习），在 ClinicalBench 等基准上显著提升了医嘱生成的 F1 分数。

---

#### 📘 Detailed Summary
**Motivation:** 现有临床决策智能体主要关注粗粒度决策，忽略了临床医嘱所需的细粒度、可执行信息，导致生成的医嘱缺乏实际可操作性。为此，本文提出 CAREAgent 以弥合临床决策与可执行医嘱之间的鸿沟。

**Method:** CAREAgent 采用两阶段智能体推理数据构建方法：首先设计一个智能体框架，构建与真实临床工具使用一致的可验证推理轨迹；然后通过格式合规性、医嘱有效性和临床合理性对推理轨迹进行筛选。在训练阶段，模型先通过监督微调学习基本推理格式和医学知识，再通过多维奖励函数的强化学习优化复杂临床推理能力。

**Result:** 在训练阶段未见过的 ClinicalBench 基准上，CAREAgent 相比单智能体方法、多智能体方法和智能体推理方法分别提升了 5.05%、2.09% 和 0.86% 的 F1 分数，在多个基准上验证了方法的有效性。

**Conclusion:** 本文研究表明，将可验证的推理轨迹构建与两阶段训练策略（监督微调+多维奖励强化学习）相结合，能够有效提升临床医嘱生成智能体的细粒度推理能力与执行准确性，为临床决策支持系统的实际落地提供了可行路径。
...
### [189] [SkillRevise: Improving LLM-Authored Agent Skills via Trace-Conditioned Skill Revision](https://arxiv.org/abs/2606.01139)
...
#### 🧩 TL;DR
SkillRevise是一个执行驱动的框架，通过诊断缺陷、检索修复原则和执行锚定编辑来迭代优化LLM代理的初始技能，在SkillsBench上将成功率从36.05%提升至61.63%，且修订后的技能展现出跨模型迁移能力。

---

#### 📘 Detailed Summary
**Motivation:** 现有自进化方法依赖累积轨迹来优化LLM代理技能，但在冷启动场景下仅能获得初始的不完美技能，导致技能构建退化为专家编写（成本高且与LLM实际执行方式脱节）或一次性LLM生成（语法正确但行为表现弱），缺乏一种能够从执行证据出发、迭代改进初始技能的机制。

**Method:** SkillRevise提出一个执行驱动的迭代优化框架，核心流程包括：从执行证据中诊断技能缺陷、从通用记忆库中检索相关修复原则、执行锚定的编辑操作，然后通过重新执行候选技能并测量其实验效用，系统性地保留最优技能版本。

**Result:** 在三个基准测试和五种LLM上的评估表明，SkillRevise显著优于一次性生成基线，将基础代理在SkillsBench上的成功率从36.05%提升至61.63%；此外，修订后的技能展现出强跨模型迁移性，捕获了超越特定模型工件的通用化程序知识。

**Conclusion:** SkillRevise有效填补了冷启动场景下初始技能迭代优化的空白，证明了通过执行驱动的缺陷诊断与修复可以系统性地提升技能质量，并且所获得的改进技能具有跨模型泛化能力，为构建可迁移的代理技能知识提供了新途径。
...
### [190] [Can LLM Agents Sustain Long-Horizon Organizational Dynamics?](https://arxiv.org/abs/2606.01199)
*Xuancheng Zhu, Yang Yue, Shuaibing Wan, Zihan Dou, Xiaohan Zhang, Yongrui Liu, Guoshun Nan*

#### 🧩 TL;DR
本文提出 TaskWeave，一种基于记忆中心化协调的分层智能体框架，通过 Formulate-Partition-Diagnose-Align 循环和依赖感知的痕迹记忆机制，在长期组织模拟中实现了连贯的行为与可落地的工件生成，实验表明结构化模拟记忆是构建可靠 LLM 组织模拟器的关键机制。

---

#### 📘 Detailed Summary
**Motivation:** 现有大型语言智能体在社交模拟中已取得进展，但在结构化组织场景中，目标需要在层级间传播、任务依赖先前执行、且工件在长时间跨度中不断积累，现有方法难以维持连贯的行为与执行落地，这一问题尚未得到系统研究。

**Method:** TaskWeave 将长期组织模拟形式化为以记忆为中心的协调问题，提出一种分层智能体框架，通过 Formulate-Partition-Diagnose-Align 循环维护规划状态，并利用依赖感知的痕迹记忆（dependency-aware trace memory）将执行过程锚定于历史依赖关系之上，从而实现目标传播与任务执行的连贯性。

**Result:** 在为期一年的 IT 公司模拟实验中，TaskWeave 在组织连贯性、执行落地性以及下游企业 NLP 任务效用上均优于其他多智能体框架，能够支持长期组织动态并生成可落地的工件，同时展现出对外部环境的适应能力。

**Conclusion:** 研究结果表明，结构化模拟记忆是实现可靠 LLM 组织模拟器的核心机制，为构建具有长期规划与执行一致性的分层智能体系统提供了重要思路与设计原则。
...
### [191] [Recognize Your Orchestrator: An Entropy Dynamics Perspective for LLM Multi-Agent Systems](https://arxiv.org/abs/2606.01351)
*Junze Zhu, Weihao Chen, Xuanwang Zhang, Zhen Wu, Xinyu Dai*

#### 🧩 TL;DR
论文提出均值场熵动力学框架对多智能体系统的集中式编排过程进行建模，并通过逆工作流生成（IWG）方法合成高复杂度基准进行验证，揭示了“推理陷阱”——编排器因上下文压缩导致的性能崩溃机制，为MAS架构设计提供了物理可解释的量化分析工具。

---

#### 📘 Detailed Summary
**Motivation:** 从单轮模型向多智能体系统（MAS）的转型虽然增强了问题求解能力，但集中式编排拓扑结构仍然是系统脆弱性的关键来源，现有方法缺乏对编排过程内在物理机制的量化分析与理论建模。

**Method:** 提出均值场熵动力学（Mean-Field Entropy Dynamics）框架，将编排过程建模为由任务求解与累积上下文加载两种竞争力量驱动的系统；同时引入逆工作流生成（IWG）多智能体流水线，用于合成具有密集中间检查点且过程可验证的高复杂度基准。

**Result:** 熵动力学模型能够拟合实验轨迹，提供量化系统稳定性与性能崩溃的物理可解释参数；分析揭示出“推理陷阱”现象：推理密集型模型在孤立任务中表现优异，但在作为编排器时因上下文压缩（context squeezing）而频繁失败。

**Conclusion:** 阐明编排器背后的物理机制并量化系统不确定性，为多智能体系统的架构设计提供了结构性洞见和指导原则。
...
### [192] [FlowTime: Towards Continuous Generative Watch Time Prediction via Flow-based Personalized Priors](https://arxiv.org/abs/2606.01352)
*Hongxu Ma, Han Zhou, Chenghou Jin, Jie Zhang, Xiaoyu Yang, Chunjie Chen, Jihong Guan, Shuigeng Zhou*

#### 🧩 TL;DR
本文提出FlowTime，一种基于一步生成变分自编码器的新型连续生成回归范式，用于短视频推荐系统中的观看时间预测。FlowTime通过流式个性化先验建模用户-物品交互模式的多模态性和异质性，在离线实验和在线A/B测试中显著超越了现有方法，并开源了首个WTP标准库TimeRec。

---

#### 📘 Detailed Summary
**Motivation:** 现有观看时间预测方法存在范式级局限性：直接回归因单峰高斯假设导致均值坍塌，序数回归因刚性离散化引入量化误差，离散生成回归面临高推理延迟和启发式词汇设计问题。更深层的共性是这些方法均无法捕获用户-物品交互模式内在的多模态性和异质性，而本文从因果视角将其识别为调节观看时间结果的结构性混杂因素。

**Method:** 本文提出连续生成回归新范式及FlowTime方法，核心采用一步生成变分自编码器，通过避免迭代去噪过程消除推理延迟，同时保留连续潜在空间的表达能力。进一步设计了基于流的个性化先验，利用归一化流将标准高斯先验扭曲为复杂的历史条件流形，从而实现对多模态交互模式的自适应建模。此外，本文开源了首个WTP标准库TimeRec并设计了新的个性化评估指标。

**Result:** 大量离线实验和在线A/B测试表明，FlowTime在观看时间预测任务上显著优于现有最先进方法，同时解决了直接回归的均值坍塌、序数回归的量化误差以及离散生成回归的高延迟与启发式设计等问题。

**Conclusion:** 本文从因果视角重新审视WTP问题，揭示了用户特定交互模式作为结构混杂因素的调节作用，并提出了连续生成回归范式作为统一解决方案。FlowTime在保持低延迟的同时实现了对多模态用户交互模式的有效建模，为短视频推荐系统中的深度用户参与优化提供了新的理论框架与实践基准。
...
### [193] [Early Diagnosis of Wasted Computation in Multi-Agent LLM Systems via Failure-Aware Observability](https://arxiv.org/abs/2606.01365)
*Xianyou Li, Weiran Yan, Yichao Wu, Penghao Liang, Mengwei Yuan, Jianan Liu, Jing Yang*

#### 🧩 TL;DR
本文提出了一种面向多智能体LLM系统的故障感知可观测性框架，通过将反复出现的故障模式映射到在线追踪信号，在原始执行日志与最终答案准确性之间建立诊断层，揭示计算浪费的根源。

---

#### 📘 Detailed Summary
**Motivation:** 多智能体LLM系统在生成答案前会消耗大量计算资源（模型token、工具调用、重试和代码执行），但现有的最终答案评估只能判断运行是否失败，无法定位轨迹在何处停止产生可恢复的进展，缺乏对无效计算浪费的细粒度诊断能力。

**Method:** 本文提出了一个故障感知可观察性框架，将反复出现的故障模式映射到在线追踪信号，包括工具可靠性、执行恢复、编排循环、证据可用性、信息变化和预算压力；框架被实例化到一个三智能体问答系统中，并在165条GAIA验证集轨迹上以相同的执行上限进行评估。

**Result:** 运行故障普遍存在：53条L1轨迹中22条失败，86条L2轨迹中33条失败，26条L3轨迹中12条失败；失败机制包括证据不足、重复动作循环、最大步数终止、工具故障连续出现以及调用成功但输出无用；平均token使用量从L1的8,152增加到L3的16,389；基于10条轨迹的缓存LLM评判审计表明，廉价在线信号与深层语义指标可捕获互补的故障层。

**Conclusion:** 该框架在原始执行日志与最终答案准确性之间建立了有效的诊断层，能够区分不同类型的计算浪费机制，且廉价在线信号与深层语义分析提供了互补的故障诊断视角，为提升多智能体系统效率提供了可操作的观察基础。
...
### [194] [Self-Healing Agentic Orchestrators for Reliable Tool-Augmented Large Language Model Systems](https://arxiv.org/abs/2606.01416)
*Rahul Suresh Babu, Adarsh Agrawal*

#### 🧩 TL;DR
提出一种自愈型智能编排器（self-healing agentic orchestrator），将可靠性视为有界运行时控制问题，通过故障信号分类、预算约束的恢复行动、验证及可观测性追踪，在工具增强型LLM系统中实现98.8%的任务成功率，显著优于重试和完全重规划等基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 工具增强型大语言模型（LLM）智能体的编排层面临多种非模型错误导致的故障，包括工具超时、参数格式异常、上下文过时、矛盾证据、重试循环及未经验证的中间输出等，现有方法缺乏对这类编排级系统性故障的显式处理与可控恢复机制。

**Method:** 论文提出一种自愈型智能编排器，将可靠性建模为有界运行时控制问题：编排器将观测到的故障信号映射为推断的故障类别，在显式预算约束下选择针对性的恢复动作，验证恢复后的轨迹，并记录可观测性追踪信息，从而实现对工具增强型LLM系统的容错与自愈。

**Result:** 在包含100个任务的受控故障注入基准上，自愈编排器达到98.8%的任务成功率，而重试仅为94.5%，完全重规划为93.8%。在匹配恢复预算的扫掠实验中，自愈方法在所有预算水平下均优于重试和完全重规划，单次恢复尝试下差距最大（94.0% vs 85.3%和88.2%）。在语义静默故障设置下，验证器引导的自愈将静默故障降至0.0%，而非验证基线产生更多看似正确但实际错误的输出。

**Conclusion:** 该研究提供了受控实验证据，表明故障感知、预算约束及验证引导的编排策略能够显著提升工具增强型LLM系统的可靠性与可诊断性，同时紧凑的模型内验证实验表明该恢复机制在真实在线工具调用场景下同样有效。
...
### [195] [Dive into Ambiguity: A*-Inspired Multi-Agents Commonsense Obfuscation Attack on LLM Prompts](https://arxiv.org/abs/2606.01441)
*Boxuan Wang, Zhuoyun Li, Xiaowei Huang, Yi Dong*

#### 🧩 TL;DR
提出了一种基于A*启发式搜索的事实错误诱导框架，通过分层重写策略和动态语义分散系数生成语义保持但诱发幻觉的对抗提示，在多种大语言模型上实现了更高的攻击成功率和更低的查询效率。

---

#### 📘 Detailed Summary
**Motivation:** 大语言模型在推理和知识密集型任务中表现优异，但极易受到保持语义意图的提示级对抗攻击，从而触发常识性幻觉。现有攻击方法要么效率低下，要么无法捕捉真实对手的适应策略，这一问题在LLM被部署于安全关键领域时尤为紧迫。

**Method:** 提出A*启发式事实错误诱导框架，核心为分层重写策略，该策略由一个动态语义分散系数γ引导，采用反向模拟退火调度，在早期偏向保守编辑、后期转向激进混淆。此外，引入Agentic Mechanism Labeling机制，通过可解释的逆向优化发现并提炼对抗机制，理论部分证明提示重写遵循压缩递归，随γ减小会导致语义坍缩。

**Result:** 在多种大语言模型上的实验表明，该方法在比穷举探索所需尝试更少的情况下，实现了更高的攻击成功率，验证了其在效率和有效性方面的优势。

**Conclusion:** 该工作揭示了大语言模型在面对语义保持型对抗攻击时的脆弱性，提供了一种高效且可解释的对抗攻击框架，为理解LLM幻觉的触发机制和提升事实可靠性提供了新思路。
...
### [196] [TERRA: Task-Embedded Reasoning and Representation Architecture for Cross-Domain Applications](https://arxiv.org/abs/2606.01520)
*Shayan Shokri*

#### 🧩 TL;DR
TERRA 提出结构化状态迁移假设，通过形式化建模将直觉性的跨域迁移问题转化为可检验的理论：将每个结构化状态域建模为受控马尔可夫过程并分解为域适配器与域不变核心，利用 lax bisimulation 差异和 Gromov-Wasserstein 距离量化跨域对应质量，推导出 Lipschitz 预测器下的迁移误差界，并设计了从驾驶场景到订单簿的预注册实验方案以验证该假设。

---

#### 📘 Detailed Summary
**Motivation:** 尽管单个域中基于动作的条件化潜在预测架构（如掩码潜在预测、动作条件化世界模型、离散动作分词、体素化状态的联合嵌入预测）已被分别验证有效，但跨结构化状态域的迁移问题尚未被系统研究：即在一个域（如驾驶场景）中学到的表示或预测器能否迁移到结构相似但领域无关的另一个域（如金融订单簿），迁移效果如何，以及其理论边界何在。

**Method:** TERRA 将每个域建模为分级潜网格上的受控马尔可夫过程，将任一实例分解为薄域适配器和共享的域不变核心；识别跨域对应关系为近似马尔可夫决策过程同态，其质量由 lax bisimulation 差异度量，对缺乏共享坐标系的域则采用动作条件转移算子间的 Gromov-Wasserstein 距离；在 Lipschitz 预测器假设下推导出迁移误差界，该界将源模型误差与结构失配分离，随预测步长几何增长，并以 Gromov-Wasserstein 距离作为下界；通过 bisimulation 度量的 Lipschitz 值性质将潜在误差与决策遗憾相关联。

**Result:** 本文为研究提案，未呈现任何实证结果；其核心产出是结构化状态迁移假设的形式化表述及预注册实验方案，该方案以从驾驶场景到订单簿的迁移测试为中心，并明确了假设被证伪的条件。

**Conclusion:** TERRA 将跨结构化状态域迁移这一广泛存在的直觉转化为可检验的形式化理论，提供了迁移误差的定量界及其与决策遗憾的联系，为未来在多领域之间实现表示与预测器的可验证迁移奠定了理论框架。
...
### [197] [Joint Agent Memory and Exploration Learning via Novelty Signals](https://arxiv.org/abs/2606.01528)
...
#### 🧩 TL;DR
JAMEL 提出联合训练智能体记忆与探索策略的框架，利用代码覆盖率等确定性新奇信号作为记忆模块的免标注监督，使语言模型代理在未知环境中实现高效探索并显著降低 token 消耗。

---

#### 📘 Detailed Summary
**Motivation:** 开放环境中自主智能体需要高效探索，但现有语言模型代理在此方面表现不足；有效探索依赖记忆，而直接保留原始交互历史在长轨迹中计算成本过高；潜在记忆虽能压缩交互历史，但其训练缺乏可靠的监督信号。

**Method:** 提出 JAMEL 框架，联合训练智能体记忆模块与探索策略；利用新颖性驱动的交互形成记忆与探索的相互依赖循环：探索依赖记忆区分已探索与未探索行为，而探索行为又为记忆提供监督信号；在 GUI 领域采用代码覆盖率等确定性与持久的新颖性信号为记忆模块提供天然、无需标注的监督。

**Result:** JAMEL 在未见过的环境中成功泛化，探索能力超越开源基线模型，在探索深度上匹敌闭源模型，同时显著降低了 token 消耗。

**Conclusion:** 记忆与探索可形成互惠的训练循环，确定性新奇信号可作为有效的免标注监督源；该框架证明了通过联合训练可在提升探索性能的同时实现计算效率优化。
...
### [198] [Revisiting Ripple Effects in Knowledge Editing through Pressure-Aware Joint Neighborhood Optimization](https://arxiv.org/abs/2606.01610)
*Haoben Huang, Shuxin Liu, Ou Wu, Di Gao*

#### 🧩 TL;DR
提出联合邻域优化（JNO）框架，通过显式建模可编辑侧协调与保留侧泄漏两种耦合压力，在目标规划阶段联合优化邻域表示，显著提升大语言模型知识编辑中的传播与保留性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有知识编辑方法分别处理单次编辑在局部知识邻域中引发的期望传播与非预期扰动，但未显式建模这两种效应的耦合关系；本文通过分析基线方法的涟漪响应，识别出两种耦合的设计压力——可编辑侧协调与保留侧泄漏，并旨在同时形式化与联合解决这两种压力。

**Method:** 提出Joint Neighborhood Optimization（JNO）知识编辑框架，在目标规划阶段联合处理两种耦合压力；JNO实例化为Pressure-Aware Coordination（PAC），在耦合约束下联合优化邻域目标表示，并引入语义预执行门控机制，在参数执行前拒绝高风险目标计划。

**Result:** 在RippleEdits基准上，JNO在传播和保留指标上均提升至少7.0%，同时保持跨骨干网络的编辑稳定性。

**Conclusion:** 本文证明了显式建模编辑中传播与保留效应的耦合性对于知识编辑至关重要，所提出的联合优化策略为高保真知识编辑提供了新的设计范式。
...
### [199] [ReSkill: Reconciling Skill Creation with Policy Optimization in Agentic RL](https://arxiv.org/abs/2606.01619)
*Zelin He, Haotian Lin, Boran Han, Wei Zhu, Haoyang Fang, Bernie Wang, Xuan Zhu, Runze Li, Matthew Reimherr*

#### 🧩 TL;DR
ReSkill提出了一种强化学习在环中的技能创建框架，通过在GRPO的群体结构中嵌入断言驱动技能创建、组内轨迹采样和自适应汤普森采样三种机制，实现了技能演化与策略学习的协同优化，在多个领域持续超越现有记忆型和技能型强化学习方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有的智能体强化学习方法虽然能使LLM智能体从环境奖励中持续改进，但所得策略无法系统性地积累可跨任务泛化的可复用策略；而现有的技能增强型强化学习方法将技能创建与策略优化解耦，导致技能可能与不断演化的策略产生冲突，二者缺乏协同演化机制。

**Method:** ReSkill利用GRPO的群体结构自然嵌入三种机制：基于断言的技能创建器通过诊断历史经验中的失败并提出条件触发式技能修订方案；组内轨迹采样支持技能版本间的受控对比，捕获最能助力策略持续学习的技能版本；结合自适应折扣因子的汤普森采样在策略演化过程中平衡技能版本选择的探索与利用。

**Result:** 在多个领域的实验中，ReSkill持续优于现有的记忆型和技能型强化学习方法，在未见任务上的性能提升最为显著。对技能生命周期的分析显示，技能能够随着策略的改进自动经历创建、测试、优化和剪枝的过程，验证了技能与策略协同演化的有效性。

**Conclusion:** ReSkill通过在强化学习环路中内化技能创建过程，成功实现了技能演化与策略学习的统一，为LLM智能体提供了系统性地积累可复用策略的有效范式，其技能自动生命周期管理机制为构建持续进化的智能体系统提供了重要启示。
...
### [200] [MobEvolve: An Agentic Self-Evolving Heuristic System for Interpretable Human Mobility Generation](https://arxiv.org/abs/2606.01640)
*Junlin He, Yihong Tang, Tong Nie, Ao Qu, Yuebing Liang, Hamzeh Alizadeh, Bang Liu, Wei Ma, Lijun Sun*

#### 🧩 TL;DR
MobEvolve 提出了首个智能体自进化启发式框架用于人类移动性生成，通过 LLM 代理迭代优化启发式逻辑，在个体轨迹保真度、群体分布对齐和行为合理性上超越现有深度生成与 LLM 方法，同时保持可解释性和高推理效率。

---

#### 📘 Detailed Summary
**Motivation:** 现有的人类移动性生成方法——包括深度生成模型、基于 LLM 的方法和传统启发式方法——难以同时满足个体轨迹保真度、群体层面分布对齐、行为合理性、可解释性和推理效率等多重复杂需求，存在明显的性能与实用性权衡缺口。

**Method:** MobEvolve 初始化一个基于行为启发式的规则系统，利用 LLM 代理在验证集上诊断经验偏差与失败案例，据此提出针对性的更新策略，并累积进化记忆以实现持续的自我改进，从而迭代演化系统内部逻辑。

**Result:** 在新加坡和蒙特利尔两个基准数据集上的广泛评估表明，MobEvolve 在个体轨迹保真度、群体分布对齐和行为合理性方面显著优于最先进的深度生成方法和 LLM 方法，同时保持了可解释性和高推理效率。

**Conclusion:** MobEvolve 证明了通过智能体自进化范式可以弥合传统启发式方法与深度/LLM 方法之间的性能与实用性鸿沟，为移动性生成领域提供了兼具高精度、可解释性和高效性的新路径，其进化记忆机制也为持续自我改进奠定了基础。
...
### [201] [Characterization of Multi-Model Agentic AI Systems on General Tasks via Trace-Driven Simulation](https://arxiv.org/abs/2606.01725)
*Donghwan Kim, Prakhar Singh, Younghoon Min, Jongryool Kim, Jongse Park, Kiwan Maeng*

#### 🧩 TL;DR
本文提出了GAIATrace，首个捕捉GAIA基准测试中两个最先进智能体系统（MiroThinker和OWL）完整推理token、任务级结构及参与LLM活动的细粒度轨迹数据集，并配套开发了Vidur-Agent轨迹驱动模拟器，用于可复现、低成本的系统评估，从而揭示了现代智能体系统处理通用任务时的行为特征与系统设计选择的影响。

---

#### 📘 Detailed Summary
**Motivation:** 尽管Agentic AI通过迭代规划、工具使用和推理广泛流行，但其系统级行为在复杂数据集和智能体架构下仍缺乏理解，主要由于执行高度非确定性、评估成本过高以及专有模型可见性有限，亟需可复现、低成本的数据集与评估工具来支撑系统层面的深入研究。

**Method:** 本文构建了GAIATrace数据集，捕获了运行GAIA基准任务的两个SOTA智能体系统MiroThinker和OWL的完整推理token、任务级结构和每个主要参与LLM的活动日志；并开发了Vidur-Agent轨迹驱动模拟器，通过回放GAIATrace轨迹在多种模拟环境中实现可复现的系统评估。

**Result:** 基于GAIATrace和Vidur-Agent，本文刻画了现代智能体系统处理通用任务的行为特征，并揭示了多种系统设计选择对智能体行为的影响，获得了若干独特的发现。

**Conclusion:** GAIATrace与Vidur-Agent为智能体系统的系统级行为分析与可复现评估提供了关键基础设施，有助于理解非确定性执行下的智能体行为规律，并为未来智能体系统设计与优化提供了实证依据。
...
### [202] [TrafficRAG: A Multimodal RAG Framework for Traffic Accident Liability Determination](https://arxiv.org/abs/2606.01737)
*Xu Li, Zedong Fu, Xinyi Li, Xun Han*

#### 🧩 TL;DR
TrafficRAG是一个多模态检索增强框架，通过视觉-语言模型生成结构化场景描述、混合检索获取法规与案例、大语言模型融合多模态证据进行推理，在交通事故责任分析任务上实现了77.32%的法律规范适配准确率和5.48%的责任比例MAE，显著提升了分析报告的可靠性与合法性。

---

#### 📘 Detailed Summary
**Motivation:** 现有交通事故责任分析方法存在效率低、主观性强、结果不一致等问题，而大语言模型在处理噪声视频输入时表现受限且缺乏领域法律知识，导致自动化责任分析的可信度和准确性不足。

**Method:** TrafficRAG首先采用视觉-语言模型将事故视频转换为结构化文本描述作为检索查询，然后通过BM25稀疏检索与稠密嵌入检索相结合的混合检索策略获取相关交通法规和相似历史案例，最后大语言模型融合检索到的法律知识与多模态事故证据进行综合推理，生成标准化的责任分析报告。

**Result:** 实验结果表明TrafficRAG在多个指标上持续优于基线方法，达到了77.32%的法律规范适配准确率、81.71%的事实忠实度，以及5.48%的责任比例平均绝对误差，验证了检索增强机制对法律知识整合的有效性。

**Conclusion:** 通过检索增强将多模态事故事实证据与法律条款进行有效整合，能够显著提升交通事故责任判定的可靠性和准确性，为智能交通和法律辅助领域的自动化分析提供了有效范式。
...
### [203] [TriAlign: Towards Universal Truth Consistency in Personalized LLM Alignment](https://arxiv.org/abs/2606.01755)
*Thi-Nhung Nguyen, Linhao Luo, Rollin Omari, Junae Kim, Thuy-Trang Vu, Dinh Phung*

#### 🧩 TL;DR
提出TriAlign，首个面向个性化大语言模型的离线多智能体强化学习框架，用于解决Truth-Invariant Alignment问题，在保持个性化的同时减少通用真理在不同社会群体间的不一致性。

---

#### 📘 Detailed Summary
**Motivation:** 个性化大语言模型会根据用户偏好和社会属性调整回复，但这可能导致不同社会群体在客观任务上系统性接收到不一致的准确响应，引发通用真理不一致问题。现有对齐方法要么忽略个性化，要么主要关注主观偏好对齐，缺乏对通用真理公平性和一致性的关注。

**Method:** 提出TriAlign框架，将每个社会群体建模为一个智能体，采用离线多智能体强化学习（MARL）联合优化三个目标：通用真理准确率、跨群体真理一致性和个性化程度。该框架通过一个公平性感知目标和显式的不一致性惩罚项实现三重目标的协同优化。

**Result:** 在多种基准测试上的实验表明，TriAlign在三个目标之间取得了比强基线方法更优的平衡，显著减少了跨社会群体的通用真理差异，同时提升了客观任务表现和个性化质量。

**Conclusion:** TriAlign验证了通过多智能体强化学习可以同时兼顾个性化、真理准确性和跨群体公平性，为个性化LLM的公平对齐提供了有效的技术路径，对构建兼顾个性化与公平性的语言模型具有重要启示。
...
### [204] [Token Predictors Are Not Planners: Building Physically Grounded Causal Reasoners](https://arxiv.org/abs/2606.01810)
*Zheng Lu, Mingqi Gao, Qinlei Xie, Wanqi Zhong, Hanwen Cui, Heng Cao, Zirui Song, Yifan Yang, Chong Luo, Bei Liu, Yiming Li*

#### 🧩 TL;DR
本文提出因果规划基准Causal-Plan-Bench和百万级因果推理语料Causal-Plan-1M，并基于Qwen3-VL-8B训练出Causal Planner模型，揭示了因果缩放定律，推动具身智能体从浅层语言统计预测转向物理因果推理。

---

#### 📘 Detailed Summary
**Motivation:** 当前具身视觉-语言规划基准倾向于奖励基于语言统计先验的下一词元预测，而非基于物理因果的下一状态推理，导致智能体将物理规划退化为浅层序列建模而缺乏真正的物理自主性，因此亟需从语言驱动的词元预测转向物理驱动的因果推理。

**Method:** 论文构建了Causal-Plan-Bench诊断基准，通过多阶段验证覆盖四个因果维度进行具身规划评估；同时构建Causal-Plan-1M百万级语料，采用四阶段标注流水线从自我中心视频中提取显式因果推理轨迹；并基于Qwen3-VL-8B训练Causal Planner模型，使其内化物理逻辑以实现更准确的下一状态估计。

**Result:** 主流模型在Causal-Plan-Bench上表现不佳，Gemini 3 Pro仅达38.18分；而Causal Planner在域内性能和跨基准泛化上均表现强劲，并揭示了因果缩放定律：将因果训练数据扩展至百万规模带来36.3%的相对提升（从33.22升至45.28）。

**Conclusion:** 本研究为将具身智能体从 superficial 的词元预测器转变为物理因果推理者提供了具体路径，证明了因果推理数据的规模化训练能显著提升模型对物理逻辑的内化能力及下一状态估计的准确性。
...
### [205] [Does Compression Preserve Uncertainty? A Unified Benchmark for Quantized and Sparse LLMs via Conformal Prediction](https://arxiv.org/abs/2606.01850)
*Yujia Tong, Yuxi Wang, Yunyang Wan, Tian Zhang, Junhao Dong, Jingling Yuan*

#### 🧩 TL;DR
本文系统研究模型压缩（量化和剪枝）对大型语言模型不确定性量化能力的影响，发现压缩会显著破坏模型校准的可靠性，并提出应将不确定性感知评估纳入压缩管道标准流程。

---

#### 📘 Detailed Summary
**Motivation:** 现有模型压缩评估几乎仅关注精度保持，忽略了安全关键应用中模型可靠量化自身不确定性的同等重要性，本文旨在填补这一研究空白，探究压缩是否破坏了不确定性量化能力。

**Method:** 对12种大型语言模型在多种压缩配置下进行基准测试，覆盖五项NLP任务，采用共形预测（conformal prediction）提供严格的无分布不确定性度量，以系统评估压缩对不确定性量化能力的影响。

**Result:** 实验发现：(I) 压缩经常导致准确率与不确定性解耦；(II) 大型模型吸收压缩引入的不确定性的能力远强于小型模型；(III) 不确定性膨胀往往呈现阈值式而非渐进式特征。

**Conclusion:** 仅以准确率评估不足以判断压缩后LLM的部署就绪状态，不确定性感知的基准测试应成为模型压缩管道的标准组成部分，为安全关键应用提供更全面的模型可靠性保障。
...
### [206] [EVA-Net: Subject-Independent EEG Motor Decoding with Video-Derived Motor Priors](https://arxiv.org/abs/2606.01884)
*Ziyuan Li, Yueyu Sun, Yimeng Zhang*

#### 🧩 TL;DR
EVA-Net提出了一个两阶段框架，利用动作视频作为语义先验实现跨被试的EEG运动解码，通过跨模态对比学习和对齐视频类别原型来减少个体差异，无需增加推理开销，在EEGMMI数据集上留一法准确率提升了8.66%。

---

#### 📘 Detailed Summary
**Motivation:** 非侵入式BCI系统需要具备强跨被试泛化能力且校准成本低的EEG解码器，但个体间差异和信号非平稳性常导致运动语义与被试特定噪声纠缠，限制了被试无关解码的性能；现有的多模态方法以文本作为语义锚点，但文本对被试间动态运动过程的监督是稀疏且静态的，无法有效捕捉运动信号的动态变化。

**Method:** EVA-Net是一个两阶段框架：第一阶段利用跨模态对比学习和有监督对比学习目标，将EEG和动作视频特征对齐到共享嵌入空间，从而减少被试特定变异；第二阶段通过视频类别原型和知识蒸馏技术，将视频导出的先验知识迁移至纯EEG分类器中，不引入额外推理负担。

**Result:** 在两个公开数据集上，EVA-Net实现了强大的被试无关解码性能，在EEGMMI数据集上留一法准确率提升达8.66%；消融实验表明，视频作为语义锚点比文本基线更有效。

**Conclusion:** 该研究表明，利用动作视频作为语义先验可以有效提升EEG运动解码的跨被试泛化能力，且视频提供的动态监督优于静态文本锚点，为开发无需校准的实用BCI系统提供了新思路。
...
### [207] [Absorbing Complexity: An Interaction-Native Knowledge Harness for Financial LLM Agents](https://arxiv.org/abs/2606.01886)
*Ailiya Borjigin, Igor Stadnyk, Ben Bilski, Maksym Chikita, Dmytro Kyrylenko, Sofiia Pidturkina, Julia Stadnyk*

#### 🧩 TL;DR
提出交互原生知识框架（InKH），一种面向金融LLM智能体的架构，通过被动知识注入、时序图记忆、Wiki审计面和带衰变与失效的提取机制，将上下文与记忆管理的复杂性吸收到系统中，在合成基准上以900ms延迟实现0.815任务质量，同时相比智能体驱动的Wiki-walk记忆降低82.95%延迟、82.29%令牌成本和96.58%过时知识使用。

---

#### 📘 Detailed Summary
**Motivation:** 现有金融AI智能体往往将复杂性转嫁给用户：用户需要反复陈述目标、风险偏好、投资组合上下文和历史判断，而智能体每次回答、检索、执行后即遗忘。这种遗忘的上下文和陈旧记忆导致延迟、重复错误、弱可审计性和不安全决策，亟需一种将复杂性吸收到系统内部的架构。

**Method:** 提出交互原生知识框架（InKH），核心包括：（1）被动知识注入，在主模型步骤前自动组装有界工作上下文缓冲；（2）时序图记忆，支持低延迟检索；（3）Wiki审计表面，提供人类可读的治理界面；（4）背景提取机制，引入成熟度、衰变和写时失效策略，将用户、市场、投资组合和工具事件转化为结构化操作知识。

**Result:** 在可复现的受控合成基准（含24个随机种子、4轮、每轮80个episode、6个基线）上共获得46,080次基线条件评估：InKH达到平均任务质量0.815、延迟900ms；相比智能体驱动的Wiki-walk记忆，延迟降低82.95%、令牌成本降低82.29%、过时知识使用降低96.58%，质量提升0.108，可追踪性提升0.461；相比无失效机制的时序图系统，质量提升0.050，过时记忆使用降低96.58%，服务成本相当。

**Conclusion:** 研究支持的设计论点是：金融AI的采纳发生在复杂性被系统吸收而非转移给用户时。InKH在架构层面验证了记忆管理、失效和可审计性设计对金融智能体的有效性，但基准验证的是架构级行为而非实盘交易性能。
...
### [208] [SMH-Bench: Benchmarking LLM Agents for Environment-Grounded Reasoning and Action in Smart Homes](https://arxiv.org/abs/2606.01912)
...
#### 🧩 TL;DR
SMH-Bench是一个基于可执行可验证的HomeEnv模拟器构建的智能家居基准测试，包含1,100个高质量任务，覆盖7大类22个子类，用于评估LLM在复杂家庭环境中的推理、交互和动作能力；实验表明前沿LLM在显式控制任务上表现优异，但在自动化调度、歧义处理和个性化推理方面仍存在显著不足。

---

#### 📘 Detailed Summary
**Motivation:** 现有智能家居基准测试通常局限于静态指令到API的映射或受限模拟，无法全面评估LLM在真实家庭场景中进行推理、交互和可靠动作的能力，缺乏对状态依赖型多设备交互环境的系统评测。

**Method:** 论文构建了HomeEnv可执行可验证的智能家居模拟器，并在此基础上设计了SMH-Bench基准测试，包含1,100个高质量任务，涵盖7大类别和22个细分子类，任务按简单、中等、复杂三级家庭环境分层，最高包含135个设备的多房间密集环境。

**Result:** 实验结果表明，前沿LLM在显式控制和查询任务上表现强劲，但在自动化任务调度、歧义处理和个性化推理方面表现显著薄弱，且随着家庭复杂度增加性能下降明显。

**Conclusion:** SMH-Bench揭示了当前LLM在真实可部署智能家居场景中的关键短板，为开发更可靠、上下文感知的智能家居智能体提供了系统化的评估框架和未来研究方向。
...
### [209] [AutoMedBench: Towards Medical AutoResearch with Agentic AI Models](https://arxiv.org/abs/2606.01961)
...
#### 🧩 TL;DR
AutoMedBench 是一个面向自主医学AI研究的流程感知基准，通过将任务组织为统一的五阶段工作流（计划、搭建、验证、推理、提交）并引入阶段级评分，揭示了当前智能体在验证环节最薄弱、搭建环节最强的行为特征。

---

#### 📘 Detailed Summary
**Motivation:** 现有医学智能体基准主要评估最终输出，缺乏对智能体在研究流程内部行为的可见性，无法深入分析智能体在各执行阶段的表现与失败模式。

**Method:** AutoMedBench 将医学AI研究任务组织为统一的五阶段工作流（S1-S5：计划、搭建、验证、推理、提交），涵盖分割、图像增强、VQA、报告生成和病灶检测五个研究轨道，每项任务设Lite和Standard两个难度层级并以相同数据和指标评估，同时使用最终任务得分和S1-S5阶段级得分进行多维度评分。

**Result:** 在数千次运行记录中，阶段级评分显示验证（Validate）是最薄弱的环节而搭建（Setup）是最强的环节；错误分析表明验证失败和提交失败分别占标签错误的37.7%和38.1%，而任务理解错误仅占0.9%；出现一次错误代码的运行其总体得分平均比无错误代码的运行低48%。

**Conclusion:** 当前自主智能体在构建可执行流水线方面表现较好，但在验证其可靠性方面存在明显不足，提示未来研究应重点关注增强智能体的验证能力以减少流程后期的关键失败。
...
### [210] [SafeMCP: Proactive Power Regulation for LLM Agent Defense via Environment-Grounded Look-Ahead Reasoning](https://arxiv.org/abs/2606.01991)
*Lichao Wang, Zhaoxing Ren, Tianzhuo Yang, Jiaming Ji, Chi Harold Liu, Yaodong Yang, Juntao Dai*

#### 🧩 TL;DR
SafeMCP是一种服务端防御插件，通过基于推理的未来安全风险预测来约束工具获取，利用内部世界模型进行前瞻推理，实现主动工具过滤和即时干预双重防御，在多个基准测试上实现了安全性与代理效用的平衡。

---

#### 📘 Detailed Summary
**Motivation:** LLM代理在通过模型上下文协议（MCP）操作复杂环境时，其扩展的动作空间提供了不安全的潜能，放大了权力寻求风险。虽然广阔的动作空间和环境影响力对任务完成至关重要，但微小的错误或幻觉可能被放大为灾难性失败，因此亟需一种能在服务端防御的方法来约束代理的工具获取行为，降低安全风险。

**Method:** SafeMCP是一个服务端防御插件，采用内部世界模型进行前瞻推理（look-ahead reasoning），实现两层防御机制：主动工具过滤（proactive tool filtering）以约束危险权力扩展，以及即时干预（immediate intervention）作为故障安全机制。训练采用三阶段流程：环境动态基础（environmental dynamic grounding）、安全策略初始化（safe policy initialization）以及具有双重可验证奖励的强化学习（reinforcement learning with dual verifiable rewards）。

**Result:** 在PowerSeeking Bench、ToolEmu和AgentHarm三个基准测试上的实验表明，SafeMCP实现了安全均衡（safe equilibrium），有效缓解了安全风险，同时保持了代理的效用不受显著损害。

**Conclusion:** SafeMCP证明了通过在服务端结合前瞻推理与强化学习来约束工具获取，可以在不牺牲代理任务效用的前提下有效降低LLM代理在复杂环境中的安全风险。该方法为构建更安全的LLM代理系统提供了可行的防御范式，尤其适用于通过MCP协议扩展能力边界的场景。
...
### [211] [Extreme Low-Bit Inference in Reasoning Models: Failure Modes and Targeted Recovery](https://arxiv.org/abs/2606.02011)
*Ekaterina Alimaskina, Darya Rudas, Denis Shveykin, Gleb Molodtsov, Pavel Vasiliev, Aleksandr Beznosikov*

#### 🧩 TL;DR
本文发现2-bit量化虽降低单token解码成本，但会导致长推理模型生成不稳定、token数激增，从而抵消端到端加速；通过引入FP16规划与循环救援两种轻量级控制方法，可在极端低比特推理中恢复精度并实现真实加速。

---

#### 📘 Detailed Summary
**Motivation:** 大型推理模型依赖长推理链导致推理成本高昂，低比特量化虽可降低单token解码开销，但2-bit量化在生成过程中的不稳定性可能导致总token数膨胀，从而无法实现端到端加速，现有研究对量化引发的过程级失效模式缺乏系统性分析与控制。

**Method:** 本文分析了Qwen3推理模型在数学与常识基准上的完整推理轨迹，揭示了2-bit量化引发重复循环、预算耗尽、延迟承诺等过程级失效。提出两种轻量级控制方法：FP16规划，为2-bit模型提供简短高精度推理大纲；循环救援，检测重复轨迹并执行早期承诺或回退至FP16。

**Result:** 在MATH-500上，循环救援将Qwen3-8B准确率从17.2%提升至74.2%；规划加循环救援将Qwen3-32B准确率从65.0%提升至87.2%，表明极端低比特推理可在轻量级检测与选择性FP16支持下恢复精度并实现真实端到端加速。

**Conclusion:** 2-bit量化对推理模型的影响不仅是降低答案准确率，更关键的是通过延长推理链破坏效率；将量化失效视为可控生成病理，通过轻量级检测与选择性高精度回退，可使极端低比特推理在保持加速的同时恢复性能。
...
### [212] [Where Do Deep-Research Agents Go Wrong? Span-Level Error Localization in Agent Trajectories](https://arxiv.org/abs/2606.02060)
...
#### 🧩 TL;DR
本文提出TELBench基准和DRIFT审计框架，用于对深度研究代理的长轨迹进行片段级错误定位，实验表明DRIFT在错误定位和首次错误准确率上最高提升30个百分点，为代理推理过程可靠性提供了过程级分析视角。

---

#### 📘 Detailed Summary
**Motivation:** 现有对深度研究代理的评估仅关注最终答案是否正确，无法判断轨迹中哪些环节导致了答案不可靠，缺乏对代理长轨迹进行过程级错误定位的方法和基准。

**Method:** 收集来自两个代理框架、三个骨干模型和三个基准的2,790条真实轨迹，将原始日志转化为语义片段，并通过LLM辅助专家审查标注有害错误片段，构建了包含1,000个实例的TELBench基准。进一步提出DRIFT框架，通过追踪代理声明、检查轨迹证据中的支持关系，标记出无支持或矛盾声明影响答案路径的片段。

**Result:** 实验覆盖多个模型系列和审计框架，DRIFT在片段级错误定位和首次错误准确率上比基线方法最高提升30个百分点。

**Conclusion:** 本文提供了深度研究代理可靠性的过程级分析视角，表明通过声明中心式的轨迹审计可以有效定位推理过程中的错误源头，弥补了仅依赖最终答案评估的不足。
...
### [213] [Learning When Not to Act: Mitigating Tool Abuse in Agentic Reinforcement Learning](https://arxiv.org/abs/2606.02132)
*Liuji Chen, Dianxing Tang, Xing Shi, Dingshuo Chen, Qiang Liu, Shu Wu, Liang Wang*

#### 🧩 TL;DR
提出EAPO框架，通过引入无工具轨迹、难度感知奖励整形和置信度感知令牌重加权，使智能体学习选择性调用工具，在多个推理基准上显著提升准确率-效率权衡。

---

#### 📘 Detailed Summary
**Motivation:** 智能体强化学习会导致工具滥用问题，即模型过度使用外部工具来处理那些仅靠内部推理就能解决的查询。现有方法采用统一的工具使用惩罚或硬性限制来缓解此问题，但这些方法虽能降低工具调用频率，也可能抑制有用的工具辅助探索。

**Method:** 提出EAPO（高效智能体策略优化）框架，该方法在每个 rollout 组中引入无工具轨迹，并采用难度感知奖励整形来主要在简单查询上惩罚冗余工具调用，同时使用置信度感知令牌重加权来改进策略学习，从而实现选择性工具使用的学习。

**Result:** 在九个数学和知识密集推理基准上，EAPO在Qwen2.5-3B、Qwen2.5-7B和Llama3.1-8B上一致提升了准确率-效率权衡。相比GRPO，EAPO平均性能分别提升10.45%、7.27%和9.69%，同时平均工具调用分别减少18.33%、18.33%和24.59%。

**Conclusion:** 结果表明智能体能够学习何时不使用工具，同时不损害工具集成的推理能力，实现了工具使用的精准控制而非简单的频率压制。
...
### [214] [S3TS: Stochastic Scenario-Structured Tree Search for Advanced Planning Under Uncertainty](https://arxiv.org/abs/2606.02151)
*Fabio Pavirani, Bert Claessens, Pierre Pinson, Chris Develder*

#### 🧩 TL;DR
提出了一种随机场景结构化树搜索（S3TS）算法，该算法通过场景树显式建模不确定性，同时支持集成非线性系统模型，填补了现有调度方法无法同时处理非线性和不确定性两大挑战的空白，在比利时不平衡结算机制仿真的需求响应场景中，S3TS在非线性条件下相比基线方法实现了高达51%的成本降低。

---

#### 📘 Detailed Summary
**Motivation:** 现有的能源调度规划技术要么能够处理非线性系统模型（如蒙特卡洛树搜索），要么能够处理不确定性（如随机数学优化），但缺乏能够同时应对这两个关键挑战的调度方法，而现代电网中非线性和不确定性问题因数据增长和可再生能源集成而愈发突出。

**Method:** 提出随机场景结构化树搜索（S3TS）算法，通过场景树显式表示不确定性，并在此框架内集成先进的非线性系统模型，从而在单一规划框架内联合解决非线性和不确定性挑战。

**Result:** 在模拟比利时不平衡结算机制的需求响应信号发布问题上，S3TS在线性可解析场景下达到接近最优的性能，成本与数学最优解（限定于场景树条件）的差距在14%以内；在高度非线性场景下，S3TS相比贪心算法和确定性MCTS分别实现了最高51%和5.4%的成本降低。

**Conclusion:** S3TS作为一种能够同时处理非线性和不确定性的规划框架，在能源调度中展现出显著优势，为现代电网中复杂调度问题提供了一种有效且实用的解决方案。
...
### [215] [From Capability Models to Automated Planning: An AAS-Native Approach for Automatic PDDL Generation](https://arxiv.org/abs/2606.02167)
*Hamied Nabizada, Thomas Wirt, Luis Miguel Vieira da Silva, Felix Gehlhoff, Alexander Fay*

#### 🧩 TL;DR
本文提出一种自动将基于Asset Administration Shells (AAS)的工业能力模型转换为完整PDDL规划问题的方法，无需工程师掌握PDDL语法，并通过实验室生产系统验证了其在布局变体比较中的有效性。

---

#### 📘 Detailed Summary
**Motivation:** 生产系统工程师需要验证给定布局是否支持所有必需的生产序列，但自动规划技术所需的PDDL建模要求专业领域知识，而生产工程师通常缺乏此技能；现有方法需引入PDDL专用子模型，增加了额外复杂度和学习成本。

**Method:** 提出一种从基于AAS的能力模型中自动提取并生成完整PDDL规划问题的算法；该能力模型整合了VDI 3682、IEC 61360-1、IDTA 02011和IDTA 02016四项工业4.0标准，从资源功能的领域级描述（即能力）中推导出所有规划元素，无需任何PDDL子模型介入；算法支持将分布式多AAS架构转换为一组完整的PDDL问题。

**Result:** 在实验室生产系统的AAS模型上验证，对四种布局变体采用最优规划进行对比；结果表明工程师可通过修改AAS模型并重新生成规划域来系统性地探索设计权衡，从而验证了该方法在实际场景中的可行性与实用性。

**Conclusion:** 该研究证明标准化的AAS能力模型蕴含足够信息以自动生成完整的PDDL规划问题，避免了工程师接触PDDL语法；该方法弥合了工业领域建模与自动规划之间的鸿沟，为生产系统设计验证提供了可操作的工程实践路径。
...
### [216] [POIROT: Interrogating Agents for Failure Detection in Multi-Agent Systems](https://arxiv.org/abs/2606.02282)
...
#### 🧩 TL;DR
POIROT 提出一种利用多智能体系统自身代理作为诊断层的协议，通过架构内已有的认知多样性进行故障归因，在无需外部监督的情况下超越了单 LLM 评估基线，证明了集体智能足以支撑安全审计。

---

#### 📘 Detailed Summary
**Motivation:** 现有 LLM 多智能体系统在安全关键场景中存在难以表征的涌现故障与幻觉，而传统评估范式依赖集中式判断，不仅形成单点故障，还需要领域专业知识，这在日益严格的 AI 法规下已不可持续。

**Method:** POIROT 采用一种协议设计，将系统自身的智能体重构为诊断层，利用架构内已有的认知多样性进行故障检测与归因，无需引入外部评估器或领域专家。

**Result:** 在多种评估设置下，POIROT 优于单 LLM 评估基线，性能增益随问题复杂度（OR = 1.60, p = 0.008）、智能体数量及故障维度增加而提升，并在复合故障条件下保持稳定。

**Conclusion:** 研究表明安全监督无需外部化：执行特定角色的智能体携带足够的集体智能来完成审计任务。团队同时开源了 POIROT 库及用于安全关键多智能体系统故障归因的基准 BLAME。
...
### [217] [Forget Attention: Importance-Aware Attention Is All You Need](https://arxiv.org/abs/2606.02332)
*Soohyeong Shin, Yeongwook Yang*

#### 🧩 TL;DR
SISA（SSM-Informed Softmax Attention）通过在注意力分数中直接引入SSM派生的重要性项，实现了分数级别的SSM-注意力融合，在不使用循环状态或自定义内核的情况下，在长程检索和语言建模基准上取得了优于Transformer和纯SSM模型的性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有混合语言模型中，Transformer具备全局检索能力但缺乏优先级区分，SSM能捕获序列重要性信号但无法回溯历史信息；现有的混合模型（如Jamba的块级融合和Hymba的头级融合）将两者置于独立的分隔空间中，导致注意力计算过程中两者无法相互影响，亟需一种更细粒度的融合机制。

**Method:** SISA提出在注意力分数计算中直接加入SSM派生的重要性项，实现分数级别的SSM-注意力融合；该方法通过构造增广的查询和键向量，将完整操作转化为一次标准的SDPA（Scaled Dot-Product Attention）调用，无需维护循环状态或编写自定义CUDA内核，从而保持与现有高效注意力实现的兼容性。

**Result:** 在152M/5B token设置下，SISA在LAMBADA-greedy指标上达到17.3%（Transformer为13.9，Mamba-3为15.5）；在NIAH（Needle-In-A-Haystack）任务上，SISA从第1K步即达到100%检索准确率，比Transformer的检索收敛速度快7倍；在369M参数规模下，Mamba-3在LAMBADA上领先，但SISA仍保持完美的NIAH检索性能和标准SDPA执行效率。

**Conclusion:** SISA定义了SSM-注意力混合模型的第三种设计轴——分数级别融合（score-level fusion），超越了现有块级和头级融合范式，表明在注意力分数层面融合SSM重要性信号能够兼顾长程检索能力与计算效率，为混合语言建模提供了新的设计方向。
...
### [218] [Coordination Graphs for Constrained Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2606.02337)
*Santiago Amaya-Corredor, Miguel Calvo-Fullana, Anders Jonsson*

#### 🧩 TL;DR
CG-CMARL 提出通过协调图与拉格朗日对偶性将约束多智能体强化学习中的联合动作空间分解为成对区域，使得学习模型数量与智能体数量无关，并能通过单次训练追踪帕累托前沿，在协作导航任务中优于固定奖励塑形比的基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 约束多智能体强化学习面临两个相互交织的挑战：联合动作空间随智能体数量指数增长，以及额外约束以奖励结构无法捕捉的方式耦合智能体，现有方法难以在规模化场景下同时应对这两个问题。

**Method:** 提出 CG-CMARL 框架，结合协调图与拉格朗日对偶性，将联合问题分解为成对区域；每个区域由一组共享 Q 函数（一个主目标函数和每个约束各一个函数）服务，使得学习模型数量与智能体数量无关；执行时采用 Max-Sum 消息传递在因子图上协调动作，同时拉格朗日乘子控制目标-约束权衡，允许单次训练模型绘制帕累托前沿而无需重新训练；提供了温和条件下的收敛保证及可分解的组合误差界。

**Result:** 在最多包含 10 个智能体的协作导航任务中，该方法生成的帕累托前沿优于以固定奖励塑形比训练的已有基线方法，并且能扩展到集中式方法无法处理的团队规模。

**Conclusion:** CG-CMARL 通过协调图分解与拉格朗日对偶性有效解决了 CMARL 中可扩展性与约束耦合的双重挑战，其组合误差界可追溯至特定设计选择并独立控制，为构建可扩展、可解释的约束多智能体强化学习系统提供了理论基础与实用框架。
...
### [219] [SIRI: Self-Internalizing Reinforcement Learning with Intrinsic Skills for LLM Agent Training](https://arxiv.org/abs/2606.02355)
...
#### 🧩 TL;DR
SIRI是一个三阶段框架，使长程LLM智能体能够在不依赖外部技能生成器或推理时技能库的情况下，通过自我探索、验证和内化技能来提升性能，在ALFWorld和WebShop上显著优于多种基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于技能的长程LLM智能体方法通常依赖外部技能生成器进行训练或在推理时持续检索技能，这会增加工程复杂性、上下文长度和部署延迟，亟需一种无需外部依赖即可实现技能自主发现与内化的方法。

**Method:** SIRI包含三个阶段：首先使用GiGPO预热策略以获得基本交互能力并收集成功的无技能轨迹；然后进行自我技能挖掘，当前策略从自身成功的普通轨迹中总结紧凑技能，并通过成对的技能增强和无技能轨迹进行验证；最后，利用轨迹级效用和动作级优势将仅有益于技能引导的动作令牌蒸馏到普通策略中，推理时仅需原始提示。

**Result:** 在ALFWorld和WebShop上使用Qwen2.5-7B-Instruct，SIRI将GiGPO的性能从0.908提升至0.930（ALFWorld）和从0.728提升至0.813（WebShop），优于基于提示、基于RL和记忆增强的基线方法；自我挖掘策略的性能可与使用闭源大模型进行蒸馏的方法相媲美。

**Conclusion:** SIRI通过自内化机制有效解除了技能学习方法对外部生成器和推理时技能库的依赖，证明了LLM智能体可以通过纯自监督方式实现技能发现与内化，且自我挖掘策略能达到与使用更强闭源模型蒸馏相当的性能水平，为构建更自主、高效的LLM智能体提供了新范式。
...
### [220] [MOC: Multi-Order Communication in LLM-based Multi-Agent Systems](https://arxiv.org/abs/2606.02359)
*Yao Guan, Lin Wang, Zhihu Lu, Ziyi Wang, Wenzhu Yan, Qiang Duan*

#### 🧩 TL;DR
提出多阶通信（MOC）方案，通过构建结构化多阶证据流和语义-拓扑合并算法，解决多智能体系统中消息传递的证据感受野受限和关键信息稀释问题，在多个数据集和LLM主干上一致提升任务性能并降低通信成本。

---

#### 📘 Detailed Summary
**Motivation:** 当前基于大语言模型的多智能体系统大多聚焦于协调拓扑的优化，而忽视了智能体间消息传输与优化这一关键问题。现有通信方案通常依赖一阶邻居响应的直接拼接，导致证据感受野受限，且关键信息在多跳路径中被稀释。

**Method:** 提出多阶通信（MOC）方案，通过重构智能体间通信机制以捕获多跳依赖关系，并设计结构化消息整合策略来保证效率。具体而言，形式化通信机制以构建结构化多阶证据流，并设计语义-拓扑合并算法，在令牌约束下优化语义保真度。

**Result:** 在六个不同数据集和多种参数规模的大语言模型主干上的大量实验表明，MOC一致地提升了任务性能并降低了通信成本。

**Conclusion:** 通过结构化的多阶通信和语义-拓扑合并，MOC有效解决了多智能体系统中证据感受野受限和信息稀释问题，为提升多智能体协作效率提供了新思路。代码已开源。
...
### [221] [COMAP: Co-Evolving World Models and Agent Policies for LLM Agents](https://arxiv.org/abs/2606.02372)
*Youwei Liu, Jian Wang, Hanlin Wang, Wenjie Li*

#### 🧩 TL;DR
本文提出COMAP框架，通过闭环交互协同进化文本世界模型与智能体策略，使世界模型能在线适应智能体不断变化的交互分布，并在具身任务规划、网页导航和工具使用等基准上显著优于基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有文本世界模型在训练后通常固定不变，无法适应演化中智能体产生的on-policy状态-动作分布变化；而智能体改进方法往往依赖外部奖励或验证器，在现实交互环境中适用性受限。现有方法难以让世界模型与智能体策略在闭环交互中共同进化。

**Method:** COMAP框架通过闭环交互协同进化世界模型与智能体策略：在每个决策步骤，世界模型预测候选动作的未来状态反馈，智能体通过估计该反馈的可信度进行未来感知反思并精炼动作；随后利用产生的on-policy轨迹通过自蒸馏更新世界模型，使其更好地匹配智能体不断演化的交互分布。

**Result:** 在具身任务规划、网页导航和工具使用等多个基准上，COMAP持续优于竞争基线，例如搭配Qwen3-4B时取得相对提升16.75%。进一步分析表明，协同进化循环能随时间提升世界模型的预测准确性，并带来更有效的长程决策能力。

**Conclusion:** COMAP证明了世界模型与智能体策略通过闭环交互协同进化的有效性，为在不依赖外部奖励的现实交互环境中提升智能体能力提供了新范式。动态适配的世界模型能更准确地反映智能体行为分布，从而显著改进长程决策表现。
...
### [222] [Spatial Representation Learning Beyond Pixels: Unifying Raster Data and Vector Semantics for Human-Centric Geospatial Foundation Models](https://arxiv.org/abs/2606.02374)
*Steffen Knoblauch, Hao Li, Gengchen Mai, Konstantin Klemmer, Song Gao, WenWen Li*

#### 🧩 TL;DR
本文呼吁地球观测领域从孤立的栅格或矢量表征学习范式转向统一的联合空间表征学习（SRL）框架，通过将栅格感知与基于矢量的推理对齐到同一嵌入空间，以构建下一代更准确、可解释且语义更丰富的地理空间AI系统。

---

#### 📘 Detailed Summary
**Motivation:** 现有地球观测基础模型（EOFMs）主要局限于栅格模态数据，忽略了OpenStreetMap等开放矢量数据源中蕴含的几何、拓扑与语义关系等结构化信息；栅格与矢量数据分别对应连续物理/光谱模式与离散对象/关系结构，是互补的地理空间视角，但现有表征学习范式将两者孤立处理，依赖有损的跨模态转换方法，缺乏统一的联合学习框架。

**Method:** 本文为展望性论文，提出向联合空间表征学习（Spatial Representation Learning, SRL）的范式转变，核心思路是在统一的嵌入空间中整合栅格感知与基于矢量的推理；该方法基于多模态地理空间学习的已有探索，强调构建异构空间数据源的对齐机制，但并未提出具体的模型架构或算法实现，而是系统论述了概念基础、技术挑战和有前景的研究方向。

**Result:** 本文未提供定量实验结果或基准评测结果，其"结果"主要体现在对现有EOFMs局限性的系统分析以及对联合SRL范式潜力的理论论证上；文中指出矢量数据能提供影像数据中往往模糊或不可及的关键上下文信号，且矢量数据更多代表人类系统（如社会或人口数据）而非物理系统，从而论证了融合两者的必要性。

**Conclusion:** 本文的核心见解是：要实现下一代地理空间AI系统，必须打破栅格与矢量模态之间的壁垒，转向联合嵌入空间中的统一表征学习；这种整合有助于构建更准确、可解释且具有语义基础的地球理解系统，并为后续研究指明了对齐异构空间数据源这一关键方向。
...
### [223] [Bridging the Sim-to-Real Gap in Semiconductor Visual Program Synthesis via Input Binarization](https://arxiv.org/abs/2606.02434)
*Yusuke Ohtsubo, Kota Dohi, Koichiro Yawata, Koki Takeshita, Tatsuya Sasaki*

#### 🧩 TL;DR
提出了一种视觉程序合成框架，利用视觉语言模型将半导体检测图像转换为可编辑的领域特定语言代码，并通过输入二值化策略弥合合成数据与真实SEM图像之间的领域差距，从而实现对电路几何结构的精确参数控制和可控训练数据生成。

---

#### 📘 Detailed Summary
**Motivation:** 半导体检测中对电路几何结构需要精确的参数化控制，但获取充足的真实训练数据成本高昂；同时，扩散模型和GAN等生成模型虽能增强训练数据，却无法保证计量任务所需的纳米级几何精度，因此亟需一种既能生成训练数据又能精确控制几何参数的方法。

**Method:** 提出一个视觉程序合成框架，利用视觉语言模型将检测图像转换为可编辑的领域特定语言代码，从而以精确参数操控方式生成训练数据；为弥合仅基于合成数据训练的VLM在处理真实SEM图像时的领域差距，采用输入二值化策略，去除SEM特有的纹理和噪声，使模型专注于几何结构。

**Result:** 在MIIC数据集上，二值化输入将平均Dice系数从原始输入基线的0.4393提升至0.5256，证明简单的纹理抽象策略能有效缓解仿真到真实的领域差距。

**Conclusion:** 研究表明，通过视觉程序合成与输入二值化相结合，能够以较低成本实现半导体检测中高精度的几何参数控制与可控数据生成，为弥合合成数据与真实图像之间的领域差距提供了一种简洁有效的思路。
...
### [224] [LLM-Evolved Pattern Generators for Optimal Classical Planning](https://arxiv.org/abs/2606.02438)
*Windy Phung, Dominik Drexler, Arnaud Lequen, Jendrik Seipp*

#### 🧩 TL;DR
提出首个通过设计保证可采纳性（admissibility）的学习型启发函数方法，利用LLM驱动的进化程序合成框架为每个领域学习一个生成模式集合（pattern collection）的程序，并通过饱和成本划分（saturated cost partitioning）可采纳地组合这些模式，从而在保持A*最优性保证的同时实现快速评估。

---

#### 📘 Detailed Summary
**Motivation:** 现有的学习型启发函数在满意规划（satisficing planning）中表现出色，但无法保证可采纳性（admissibility），因此不适用于最优经典规划（optimal classical planning），这阻碍了学习型方法在需要最优性保证的规划任务中的应用。

**Method:** 提出一种LLM驱动的进化程序合成框架，为每个规划领域学习一个能自动生成模式集合（pattern collection）的程序；该程序在测试时对任意任务生成抽象模式，并通过饱和成本划分（saturated cost partitioning）可采纳地组合多个模式的启发值，从而保证整体启发函数的可采纳性。

**Result:** 在多个规划领域上，学习到的启发函数在覆盖度（coverage）上与最先进的领域无关基线方法相当，同时每个状态的评估速度显著更快；生成的程序编码了可解释的领域特定知识，且测试时运行开销可忽略不计。

**Conclusion:** 该方法首次在保持A*最优性保证的前提下将学习型启发函数引入最优规划，证明了通过可采纳性设计的学习策略在实际应用中既高效又具有可解释性，为学习型方法在最优规划中的进一步应用奠定了基础。
...
### [225] [AGENTCL: Toward Rigorous Evaluation of Continual Learning in Language Agents](https://arxiv.org/abs/2606.02461)
*Yiheng Shu, Bernal Jiménez Gutiérrez, Saisri Padmaja Jonnalagedda, Yuguang Yao, Huan Sun, Yu Su*

#### 🧩 TL;DR
提出AgentCL评估框架，通过可组合的任务流和非参数记忆探针MemProbe，系统研究语言代理的持续学习能力，发现可控任务流比朴素任务流能更清晰区分记忆设计的塑性差异，并揭示记忆退化风险。

---

#### 📘 Detailed Summary
**Motivation:** 现有语言代理在处理连续任务时积累的经验未被充分复用，而持续学习领域的 benchmarks 难以严格评估代理的跨任务复用能力：多数工作聚焦于长上下文对话或文档的检索与推理，近期终身学习基准则依赖朴素任务流，缺乏对跨任务关系的系统分析，无法揭示代理随时间真正学到和复用了什么。

**Method:** 提出AgentCL评估框架，核心包括可控任务流（compositional streams）和迁移增益指标；其中compositional streams有意构建前期子解、证据或工作流可在后期任务中复用的结构，与无此保证的naive streams形成对比。同时提出非参数记忆探针MemProbe，通过存储交互、洞察和技能并在整合过程中过滤不可靠经验来诊断记忆设计选择对持续学习的影响。

**Result:** 在编码、深度研究和语言理解/推理任务上的实证分析表明：naive streams区分记忆设计的能力有限，而controlled streams能更清晰地辨别不同记忆设计的塑性；naive和held-out设置下迁移增益有限，且可能暴露记忆诱导的性能退化。

**Conclusion:** 研究揭示了现有持续学习评估范式的不足，强调了需要更强的、能平衡塑性（plasticity）与稳定复用（stable reuse）的记忆设计，为语言代理的持续学习提供了更具诊断性的评估工具和方向。
...
### [226] [SafeSteer: Localized On-Policy Distillation for Efficient Safety Alignment](https://arxiv.org/abs/2606.02530)
*Hao Li, Jingkun An, Zijun Song, Pengyu Zhu, Rui Li, Hao Wang, Wendi Feng, Yesheng Liu, Lijun Li, Jin-Ge Yao, Lei Sha*

#### 🧩 TL;DR
SafeSteer通过对安全token进行在线策略蒸馏，在仅需100条有害样本且不使用通用数据的情况下，实现了安全性与通用能力之间的优越权衡，显著降低了对齐成本。

---

#### 📘 Detailed Summary
**Motivation:** 现有的大语言模型对齐方法常导致通用能力下降（对齐税），且缓解该问题的方法依赖大量通用数据或辅助奖励模型，成本高昂。本文基于安全特征在输出分布中天然稀疏的观察，提出应通过局部化修改而非全局权衡来解决对齐税问题。

**Method:** SafeSteer首先通过激活引导构建一个安全教师模型，然后基于该教师设计安全token选择算法，识别出与安全相关的关键token；在训练阶段，仅对这些安全token施加反向KL散度惩罚，从而在提升安全性的同时最小化对通用能力的干扰。

**Result:** 实验结果显示，SafeSteer在七个安全基准上取得强安全性表现，同时在五个通用能力基准上仅有极轻微的性能下降，优于现有方法。该方法仅需100条有害样本且完全不需要通用数据，数据量不足基线方法的1%，大幅降低了对齐成本。

**Conclusion:** SafeSteer验证了安全对齐可通过局部化修改实现高效微调，为减轻对齐税提供了新的思路；其极低的数据需求意味着该方法在实际部署中具有显著的成本优势。
...
### [227] [MCP-Persona: Benchmarking LLM Agents on Real-World Personal Applications via Environment Simulation](https://arxiv.org/abs/2606.02470)
...
#### 🧩 TL;DR
MCP-Persona 是首个专为评估智能体在真实个性化 MCP 工具上性能的基准，覆盖 Reddit、小红书、飞书、Slack 等主流社交与企业应用，实验表明现有 SOTA 智能体在个性化工具使用上存在显著困难。

---

#### 📘 Detailed Summary
**Motivation:** 现有基准主要评估智能体在通用信息检索工具上的表现，未能捕捉个人社交应用中工具与个人账户或本地数据库交互所带来的实际挑战，缺乏针对个性化 MCP 工具的性能评估体系。

**Method:** 提出 MCP-Persona 基准，涵盖从 Reddit、小红书等社交媒体平台到飞书、Slack 等企业协作套件的多样化真实应用，并在多种 SOTA 智能体上开展大规模实验评估其个性化工具使用能力。

**Result:** 多种 SOTA 智能体在个性化工具使用任务上表现出显著困难，证明该基准能够有效识别和定位当前智能体在真实个性化场景中的局限性。

**Conclusion:** MCP-Persona 填补了个性化 MCP 工具评估的空白，揭示了现有智能体在该领域的关键短板，为后续研究和改进提供了重要参考。
...
### [228] [Bridging the Last Mile of Time Series Forecasting with LLM Agents](https://arxiv.org/abs/2606.02497)
*Yuhua Liao, Zetian Wang, Qiangqiang Nie, Zhenhua Zhang*

#### 🧩 TL;DR
本文提出并形式化了时间序列预测中的"最后一公里预测"问题，并设计了一个基于LLM智能体的框架，该框架在统计预测模型之上引入推理、工具调用和记忆机制，将原始预测转化为融合业务上下文与专家反馈的可审计预测结果。

---

#### 📘 Detailed Summary
**Motivation:** 现实中的时间序列预测在获得统计基线后，通常需要根据节假日、活动计划、外部事件等弱结构化业务上下文进行人工修订才能进入决策阶段，而这一关键环节在现有预测文献中尚未被系统研究。本文旨在将此阶段形式化为"最后一公里预测"问题，填补统计预测与业务就绪预测之间的空白。

**Method:** 本文提出了一个基于LLM智能体的框架，该框架位于预测主干模型之上，核心设计包括：统一的预测工作空间、用于检索上下文证据的工具调用机制、将推理轨迹转换为显式预测修订动作的结构化安全约束、通过map-reduce式分解支持长时域预测，以及通过记忆库实现事后反思。整个系统以可控和可审计为设计目标。

**Result:** 通过真实世界案例研究，本文展示了所提出的LLM智能体框架能够有效弥合统计预测与业务就绪预测之间的差距，验证了将弱结构化业务上下文纳入预测修订流程的可行性与实用性。

**Conclusion:** 本文揭示了统计预测在实际落地中面临的关键瓶颈——缺乏对业务上下文的整合能力，并证明了LLM智能体作为一种可控、可审计的中间层，能够有效将非结构化业务知识转化为显式的预测修订动作，为未来将大语言模型纳入决策驱动的预测流水线提供了新的研究方向。
