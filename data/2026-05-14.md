...
#### 🧩 TL;DR
提出了一种将美学图像裁剪重新定义为多模态推理任务的新范式，通过设计CROP方法（组合推理与偏好优化），引导视觉语言模型像专业摄影师一样进行「分析-提案-决策」的分步推理，并结合专家偏好对齐模块使裁剪结果与人类美学专家一致，在多个数据集上验证了方法的优越性。

---

#### 📘 Detailed Summary
**Motivation:** 现有美学图像裁剪方法要么依赖显著性预测，难以在复杂场景中做出构图权衡；要么依赖检索增强，缺乏对独特场景的自适应推理能力，二者均无法使自动裁剪结果与人类专家对齐。核心问题在于这些方法忽略了任务对构图和美学的深度理解这一本质要求。

**Method:** 提出CROP（Compositional Reasoning and Optimizing Preference）方法，将美学裁剪重构为多模态推理任务以激活VLM的分析和理解能力。方法设计了「分析-提案-决策」的分步推理过程，逐步对场景元素和构图原则进行分析；同时引入专家偏好对齐模块，使模型决策与人类专家美学偏好保持一致。

**Result:** 在多个数据集上进行了广泛实验，结果验证了所提CROP方法的优越性以及各个组件的有效性，表明该方法在自动化美学裁剪任务上优于以往的显著性方法和检索增强方法。

**Conclusion:** 该研究表明将美学裁剪重新定义为多模态推理任务，并利用VLM的分步推理能力与专家偏好对齐，能够有效弥合自动裁剪结果与人类专家之间的差距，为美学图像裁剪提供了一种可解释且与人类审美一致的新范式。
...
### [2] [What Happens Before Decoding? Prefill Determines GUI Grounding in VLMs](https://arxiv.org/abs/2605.12549)
*Jiaping Lin, Fei Shen, Junzhe Li, Ping Nie, Fei Yu, Ming Li, Haizhou Li*

#### 🧩 TL;DR
本文通过分析VLM在GUI grounding中的两阶段推理机制，发现预填充阶段是决定目标选择的关键瓶颈，并提出无需训练的Re-Prefill方法，利用注意力引导的二次预填充阶段来精化目标选择，在多个基准上取得一致改进。

---

#### 📘 Detailed Summary
**Motivation:** 现有无需训练的GUI grounding方法通常依赖多次推理（如迭代裁剪或候选聚合）来定位目标元素，但每次前向传播仍然独立解释指令和解析视觉布局，缺乏视觉token间的渐进交互。本文识别出VLM在GUI grounding中存在一个被忽视的瓶颈：推理遵循预填充阶段决定候选UI元素、解码阶段精化最终坐标的两阶段范式，且预填充阶段的候选选择错误无法在解码阶段有效纠正。

**Method:** 本文提出Re-Prefill，一种无需训练的推理方法，通过引入注意力引导的第二次预填充阶段来精化目标选择。具体而言，从查询位置（即最终token）在各层中持续获得高注意力的视觉token被提取为初步目标假设，并与指令隐藏状态一起拼接回输入，使模型在坐标生成前能够深度重新思考其决策。

**Result:** 在四个VLM和五个基准（ScreenSpot-Pro、ScreenSpot-V2、OSWorld-G、UI-Vision、MMBench-GUI）上的实验表明，Re-Prefill无需额外训练即可实现一致的性能提升，在ScreenSpot-Pro上最高提升4.3%。

**Conclusion:** 本文揭示了VLM在GUI grounding中预填充阶段的关键瓶颈，证明通过注意力引导的二次预填充可以有效改善目标选择精度。该方法无需训练即可迁移至多种VLM，为GUI grounding提供了一种轻量级且高效的推理优化策略。
...
### [3] [Pyramid Self-contrastive Learning Framework for Test-time Ultrasound Image Denoising](https://arxiv.org/abs/2605.12567)
*Jiajing Zhang, Bingze Dai, Xi Zhang, Yue Xu, Wei-Ning Lee*

#### 🧩 TL;DR
本文提出Aperture-to-Aperture (A2A)框架，一种纯测试时训练方法，通过自对比学习在金字塔潜空间解耦超声图像的解剖相似性与噪声随机性，实现单样本超声图像去噪，避免了域偏移和预训练成本，在仿真和体内实验中显著提升了SNR和CNR。

---

#### 📘 Detailed Summary
**Motivation:** 传统超声去噪方法依赖显式噪声假设，在复合噪声下有效性下降；基于学习的方法需要大量标注数据和模型参数，且存在域偏移问题，导致对特定噪声类型适用性有限且容易模糊结构细节。本文旨在解决现有方法在复杂体内环境中域偏移严重、泛化能力差的问题，实现无需预训练的单样本超声图像去噪。

**Method:** 提出Aperture-to-Aperture (A2A)框架，采用纯测试时训练策略，仅使用一个含噪的合成孔径超声(SAU)信号样本在测试时进行训练。该框架通过自对比学习在金字塔潜空间中解耦子孔径数据的解剖相似性和噪声随机性，将干净图像从解剖空间解码同时丢弃噪声空间，从根本上消除了域偏移和预训练成本。

**Result:** 在仿真实验中，包含0至30 dB电子噪声水平和不同夹杂几何形状，A2A实现了69.3%的SNR提升和34.4%的CNR提升。体内实验结果显示，仅使用心脏六种超声切面及肝脏、肾脏的两孔径数据，分别获得了84.8%的SNR增益和25.7%的CNR增益。

**Conclusion:** A2A框架在多样化成像目标和配置下均能输出清晰的图像/信号，证明了纯测试时训练策略在超声去噪中的有效性，为更可靠的解剖可视化与功能评估奠定了基础，有望推动超声图像去噪从预训练范式向测试时自适应范式的转变。
...
### [4] [VideoSEAL: Mitigating Evidence Misalignment in Agentic Long Video Understanding by Decoupling Answer Authority](https://arxiv.org/abs/2605.12571)
*Chenhao Qiu, Yechao Zhang, Xin Luo, Shien Song, Xusheng Liu*

#### 🧩 TL;DR
本文揭示了长视频理解智能体存在的"证据错位"问题——答案正确但缺乏证据支持，并提出解耦的规划器-检查器框架，通过分离长期规划与答案授权、引入像素级验证门控，在多个长视频基准上显著提升了答案准确率与证据对齐度。

---

#### 📘 Detailed Summary
**Motivation:** 现有多模态大模型在短视频问答上表现良好，但在长视频场景下需要长程搜索与验证，当前LVU智能体存在"证据错位"问题：能给出正确答案，但该答案并非由检索或检视的视觉证据所支持。研究旨在系统性地表征这一失败模式，揭示其结构根因，并提出有效的解决方案。

**Method:** 论文首先引入时间接地性与语义接地性两种诊断指标来量化证据错位，揭示了两种放大因素：推理阶段的共享上下文饱和导致的提示压力，以及训练阶段仅优化结果带来的奖励压力。基于此发现，提出解耦的规划器-检查器框架，将长期规划与答案授权分离，最终回答需经过像素级验证门控。

**Result:** 在四个长视频基准上，所提框架同时提升了答案准确率与证据对齐度，在LVBench上达55.1%，在LongVideoBench上达62.0%。解耦架构随搜索预算增加持续扩展，且支持即插即用地升级MLLM骨干网络而无需重新训练规划器。

**Conclusion:** 研究表明耦合智能体范式中长期规划与答案授权的混淆是证据错位的结构根因，解耦设计能有效缓解该问题。所提框架不仅性能领先，还具备可解释的搜索轨迹和良好的扩展性，为长视频理解智能体的设计提供了新范式。
...
### [5] [DistractMIA: Black-Box Membership Inference on Vision-Language Models via Semantic Distraction](https://arxiv.org/abs/2605.12574)
*Hongyi Tang, Zhihao Zhu, Yi Yang*

#### 🧩 TL;DR
提出DistractMIA，一种基于语义分心的输出仅黑盒成员推理攻击框架，通过向原始图像插入已知语义干扰物并观察生成文本的变化来判断成员关系，在多个VLM和基准测试中优于现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有面向视觉-语言模型的成员推理攻击存在两方面的局限性：一类攻击依赖概率级信号（如logits或隐藏状态），而这在部署的VLM中通常无法获取；另一类基于掩码的语义预测任务依赖于以物体为中心的视觉假设，适用范围有限。因此，亟需一种仅基于文本输出的、通用且有效的黑盒成员推理方法。

**Method:** DistractMIA是一种输出仅黑盒框架，核心思路是保留原始图像不变，向其中插入一个已知的语义干扰物（distractor），然后通过测量模型生成文本响应的变化程度来判断成员关系。其直觉是：成员样本更倾向于锚定原始图像语义，而非成员样本更容易被干扰物引导。该方法在参考集上校准干扰物配置，并通过多次文本生成收集响应稳定性和干扰物接受度，无需访问logits、概率或隐藏状态。

**Result:** 在多个视觉-语言模型和基准测试上的实验表明，DistractMIA在性能上持续优于现有的输出仅攻击方法，甚至超过了具有更强访问权限的基线方法；在医学图像基准上的良好表现进一步证明了其不仅适用于以物体为中心的自然图像，还具有更广泛的适用性。

**Conclusion:** DistractMIA通过语义分心策略有效解决了部署场景下VLM成员推理的访问限制和领域泛化问题，为训练数据审计提供了实用的输出仅工具，并展示了在非自然图像领域的可迁移性，具有重要的隐私审计和合规应用价值。
...
#### 🧩 TL;DR
本文提出将3D几何基元（立方体、球体、圆柱体）以可执行代码形式作为视觉-语言模型空间理解的中间表征，通过SpatialBabel基准、Code-CoT推理策略和S$^3$-FT自监督微调三项贡献，系统提升了VLM在空间推理任务上的表现，并揭示了模型在场景重建中跨编程语言性能差异高达5.7倍的现象。

---

#### 📘 Detailed Summary
**Motivation:** 视觉-语言模型存在一个显著悖论：它们能生成可执行代码从几何基元重建3D场景，准确还原物体数量、类别和大致位置，却在同一图像上更简单的空间问题上失败。这一矛盾暴露了VLM在空间理解上的深层缺陷，而现有方法缺乏有效的中间表征来弥合这一差距。

**Method:** 本文提出三项贡献：1) SpatialBabel基准，评估14种VLM在六种场景-代码语言上的基元化3D场景重建能力；2) Code-CoT（代码思维链），一种无训练推理策略，通过引导模型先生成基于基元的代码再回答空间问题来路由空间推理过程；3) S$^3$-FT（自监督空间微调），通过解析模型自身的Three.js基元重建结果生成结构化标注，在无需人类标签和教师模型的情况下进行自监督微调。

**Result:** SpatialBabel实验显示，同一模型在跨语言场景下物体检测F1分数差异可达5.7倍；Code-CoT在SpatialBabel-Primitive-QA上最高提升+6.4%，在真实照片CV-Bench-3D上提升+5.0%；S$^3$-FT在仅使用基元图像训练的情况下，使Qwen3-VL-8B在SpatialBabel-Primitive-QA上提升+4.6%~+8.6%，在CV-Bench-2D上提升+9.7%，在HallusionBench上提升+17%，且该方法可跨模型家族迁移。

**Conclusion:** 本文表明，以代码形式表达的几何基元既是诊断VLM空间理解能力的有效工具，也是一种可迁移的空间词汇表。该工作为提升VLM的空间推理能力提供了无需人工标注的新路径，并为理解模型在空间任务中的语言依赖行为奠定了基础。
...
### [7] [CRAFT: Clinical Reward-Aligned Finetuning for Medical Image Synthesis](https://arxiv.org/abs/2605.12650)
*Yunsung Chung, Alex El Darzi, Carlo El Khoury, Han Feng, Nassir Marrouche, Jihun Hamm*

#### 🧩 TL;DR
本文提出临床对齐评分（CAS）和基于奖励的微调框架CRAFT，分别用于评估和优化医学图像生成模型在病理相关标准下的临床一致性，在多模态实验中显著减少幻觉式生成并提升下游分类性能。

---

#### 📘 Detailed Summary
**Motivation:** 基础扩散模型在自然图像生成中表现优异，但将其适配到医学影像领域面临两个核心问题：有限标注数据加剧了幻觉式且临床不可靠的生成结果，而现有评估指标（如FID、Inception Score）无法量化生成图像在病理相关标准上的逐样本对齐程度。

**Method:** 本文提出临床对齐评分（CAS），一种基于基础模型的代理指标，从视觉保真度之外的四个互补维度评估生成图像的临床对齐程度；并在此基础上提出临床奖励对齐微调（CRAFT），一种基于奖励的适配框架，通过多模态大语言模型和视觉-语言模型的医学知识，结合标签条件提示增强、临床检查清单和可微奖励优化来实现医学图像生成模型的微调。

**Result:** 在四种不同模态上，CRAFT在CAS和下游分类性能上均优于较强的基线方法；CRAFT将低对齐尾部（低于真实图像参考阈值）的比例相对最强基线降低5.5–34.7个百分点，对应各数据集平均相对降低20.4%，并通过离族评估器评估、结构化清单审计、记忆化分析和针对CheXpert的盲法医生偏好研究验证了结果的有效性。

**Conclusion:** CRAFT框架通过奖励优化有效减少了医学图像生成中的幻觉式样本，CAS可作为比传统生成指标更贴近临床需求的评估标准；研究结果提示将领域知识与奖励机制结合是提升生成模型在医学等高风险领域可信度的有效路径。
...
### [8] [CRePE: Curved Ray Expectation Positional Encoding for Unified-Camera-Controlled Video Generation](https://arxiv.org/abs/2605.12938)
*Seonghyun Jin, Youngmin Kim, Sunwoo Park, Jong Chul Ye*

#### 🧩 TL;DR
提出Curved Ray Expectation Positional Encoding (CRePE)，一种兼容统一相机模型的深度感知位置编码方法，通过几何注意力适配器注入场景距离信息，实现对广角、鱼眼等多种相机模型下更稳定的视频生成相机控制。

---

#### 📘 Detailed Summary
**Motivation:** 现有相机条件视频生成中的注意力级位置编码要么仅提供射线级相机信号，要么依赖针孔相机几何假设，无法兼容统一相机模型下的广角和鱼眼镜头等通用相机控制场景，限制了方法的适用范围。

**Method:** 提出CRePE，将每个图像token表示为沿其源射线的深度感知位置分布，捕获广角和鱼眼相机产生的投影路径几何；通过几何注意力适配器注入冻结的视频DiT中，在选定注意力层加入token级场景距离信息，并利用单目几何基础模型的伪监督进行稳定化训练；同时通过径向混合强制机制扩展位置编码路径以支持外部几何控制。

**Result:** CRePE在多个几何感知指标和感知质量指标上表现更优，视频质量指标上保持竞争力；位置编码消融实验显示，相比RayRoPE风格的端点位置编码基线，CRePE取得更好的整体平均排名。

**Conclusion:** CRePE通过统一相机模型感知的投影路径集成，在多种相机模型下实现了更稳定的相机控制；该方法还可扩展至外部径向地图控制和源视频运动迁移，展示了其超越相机控制本身的场景几何条件生成能力。
...
### [9] [Debunking Grad-ECLIP: A Comprehensive Study on Its Incorrectness and Fundamental Principles for Model Interpretation](https://arxiv.org/abs/2605.12952)
*Yongjin Cui, Xiaohui Fan*

#### 🧩 TL;DR
本文证明基于中间特征的Transformer可解释性方法（以Grad-ECLIP为代表）并非新路线，而是现有基于注意力路线的一个等价变体，并揭示该方法存在解释结果与原始模型性能不一致的根本缺陷，最终提出可解释性应遵循的两条基本原则。

---

#### 📘 Detailed Summary
**Motivation:** 当前Transformer可解释性研究中出现了一条所谓“基于中间特征”的新技术路线（以Grad-ECLIP为代表），但该路线是否真正独立且正确尚未被严格检验，本文旨在澄清其本质并揭示其潜在缺陷。

**Method:** 本文首先通过形式化推导证明基于中间特征的Grad-ECLIP与基于注意力的Attention-ECLIP在理论上完全等价，且后者计算更简洁；随后通过实验验证该等价性，并进一步分析Grad-ECLIP的解释结果与原始模型性能不一致的错误原因，据此重申模型可解释性应遵循的两条基本原则。

**Result:** 形式推导与实验均证实Grad-ECLIP是Attention-ECLIP的等价变体，并非新方法；Grad-ECLIP的解释结果并不反映原始模型的真实行为，其解释与模型性能存在错位，表明该方法存在根本性缺陷。

**Conclusion:** 本文澄清了所谓“中间特征路线”的误解，指出Grad-ECLIP本质上是已有注意力路线的等价形式且存在严重缺陷，强调模型可解释性方法必须忠实反映原始模型行为，并明确了应遵循的基本原则以避免类似错误。
...
### [10] [OCH3R: Object-Centric Holistic 3D Reconstruction](https://arxiv.org/abs/2605.13018)
*Yi Du, Yang You, Xiang Wan, Leonidas Guibas*

#### 🧩 TL;DR
OCH3R提出了一种统一的单阶段框架，通过Transformer架构同时预测所有物体的6D位姿、3D高斯重建和语义属性，实现了从单张RGB图像到全场景物体级3D重建的端到端前馈推理，在多项基准上取得最优性能且推理速度比多阶段流水线快数个数量级。

---

#### 📘 Detailed Summary
**Motivation:** 现有物体级场景理解方法通常采用多阶段流水线，先使用预训练分割器提取个体物体，再对每个物体单独进行3D重建，这种方法计算开销大、对分割错误敏感且随场景复杂度扩展性差。OCH3R旨在解决这些问题，提出一种统一框架实现从单张RGB图像直接进行端到端物体级全场景3D重建。

**Method:** OCH3R基于Transformer架构，在单次前向传播中逐像素预测多个属性：CLIP类别嵌入、度量深度、归一化物体坐标（NOCS）以及表示每个物体的固定数量3D高斯。为监督高斯重建，将预测的3D高斯通过预测的6D位姿变换到规范空间，与预渲染的规范真值对齐，从而避免为每张图像生成昂贵的3D高斯标签。

**Result:** 在标准室内基准上，OCH3R在单目深度估计、开放词汇语义分割和仅RGB的类别级6D位姿估计三个任务中均取得了最先进性能，同时生成高保真度、可编辑的逐物体重建结果。推理过程完全前馈，计算复杂度与场景中物体数量无关，在杂乱场景中比传统多阶段流水线速度提升数个数量级。

**Conclusion:** OCH3R证明了将物体检测、位姿估计与3D重建统一到单一前馈框架中的可行性和优越性，避免了多阶段方法的级联误差和计算瓶颈。其高效且可扩展的推理能力为实时场景理解与编辑应用提供了新的可能性。
...
### [11] [ViDR: Grounding Multimodal Deep Research Reports in Source Visual Evidence](https://arxiv.org/abs/2605.13034)
*Zhuofan Shi, Peilun Jia, Baoqin Sun, Haiyang Shen, Sixiong Xie, Yun Ma, Xiang Jing*

#### 🧩 TL;DR
ViDR是一个多模态深度研究框架，将源图像视为可检索、可解释、可路由和可验证的证据对象，通过构建证据索引大纲、上下文感知过滤、基于大纲的重排序和VLM视觉分析来生成有据可查的长篇报告，显著提升了报告的视觉证据基础和可验证性。

---

#### 📘 Detailed Summary
**Motivation:** 现有深度研究系统大多以文本为中心，主要依赖文本证据生成报告；多模态系统要么对图像的检索和利用较弱，要么自行生成图表而非利用源图像作为证据，导致源图像作为视觉证据的潜力未被充分利用，存在证据基础薄弱和可验证性不足的问题。

**Method:** ViDR将源图像视为四类证据对象（可检索、可解释、可路由、可验证），构建将文本和视觉证据链接到报告论点的证据索引大纲，通过上下文感知过滤去除噪声网页图像、基于大纲的重排序筛选相关图像、VLM驱动的视觉分析提取细粒度证据，在生成每个章节时使用章节特定的证据，并通过视觉引用验证机制减少幻觉或位置不当的图表。同时提出了MMR Bench+基准用于评估深度研究报告中视觉证据的检索、放置、解释、可验证性和图表生成质量。

**Result:** 实验结果表明，ViDR在整体报告质量、源图像整合能力和可验证性方面显著超越了包括商业和开源模型在内的强基线系统，验证了源视觉证据在多模态深度研究中的重要性。

**Conclusion:** 源视觉证据对于多模态深度研究至关重要，能够有效增强报告的证据基础、视觉支持和可验证性，为未来多模态长文档生成系统的发展提供了重要方向。
...
### [12] [Revealing the Gap in Human and VLM Scene Perception through Counterfactual Semantic Saliency](https://arxiv.org/abs/2605.13047)
*Ziqi Wen, Parsa Madinei, Miguel P. Eckstein*

#### 🧩 TL;DR
提出Counterfactual Semantic Saliency (CSS)框架，通过因果消融对象并测量语义偏移来量化VLM对场景中物体重要性的感知，发现VLM与人类之间存在系统性语义对齐差距，包括尺寸偏差、中心偏差和显著物体偏差。

---

#### 📘 Detailed Summary
**Motivation:** 现有方法难以评估大视觉语言模型（VLM）在高层次语义场景理解上是否与人类感知对齐，白盒可解释性方法不适用于闭源模型，被动指标无法隔离因果特征，因此需要一种黑盒、模型无关的框架来量化VLM的语义重要性归因并比较其与人类感知的差异。

**Method:** 提出Counterfactual Semantic Saliency (CSS)框架，通过从场景中因果消融（移除）对象并测量模型输出语义的偏移量来量化每个对象的重要性；将VLM的物体重要性排序与人类心理物理学基线（16,289条有效响应，307个场景，1,306个反事实变体）进行对比，以评估AI-人类语义对齐程度。

**Result:** VLM相比人类表现出系统性偏差：过度依赖大物体（尺寸偏差）、图像中心物体（中心偏差）和高显著物体；对场景中人物的依赖程度低于人类参与者；模型尺寸偏差是驱动模型-人类语义差异的主要因素。

**Conclusion:** VLM在场景理解中存在普遍的语义对齐差距，模型对物体大小、位置和显著性存在非人类似的偏好，而人类更关注社会性物体（人物）；尺寸偏差是解释模型与人类语义差异的核心因素，表明需要对VLM的感知偏差进行进一步校准。
...
### [13] [Flow Augmentation and Knowledge Distillation for Lightweight Face Presentation Attack Detection](https://arxiv.org/abs/2605.13108)
*Muhammad Shahid Jabbar, Muhammad Sohail Ibrahim, Taha Hasan Masood Siddique, Kejie Huang, Shujaat Khan*

#### 🧩 TL;DR
提出一种基于知识蒸馏的活体检测框架，利用光流增强训练阶段的运动表征，使学生网络在推理时仅依赖RGB输入即可隐式学习运动敏感特征，无需光流计算，在多个基准上取得优异性能并支持实时部署。

---

#### 📘 Detailed Summary
**Motivation:** 人脸呈现攻击检测在多样化欺骗手段和采集条件下仍具挑战，运动线索对该任务高度有效，但现有方法依赖显式光流估计，计算开销大，难以满足实时和资源受限场景的部署需求。

**Method:** 提出双分支教师模型，融合RGB帧的外观线索与色环编码光流的运动线索，建模微运动与时间一致性；引入知识蒸馏框架，通过logit蒸馏将运动感知知识从光流增强的教师迁移至轻量级纯RGB学生网络，使学生推理时不需光流估计或额外特征提取模块。

**Result:** 在Replay-Attack和Replay-Mobile上达到0.0% HTER，在ROSE-Youtu上达0.94% HTER，在SiW-Mv2上达5.65% HTER，在OULU-NPU上达0.42% ACER；蒸馏后学生模型参数量和FLOPs大幅降低，在NVIDIA Jetson Orin Nano上达52 FPS，性能与教师相当或更优。

**Conclusion:** 通过训练阶段引入光流增强运动表征并结合知识蒸馏，可有效消除推理时的光流计算负担，实现高效且高性能的活体检测，为资源受限环境下的实时FacePAD部署提供可行方案。
...
### [14] [Multi-Modal Guided Multi-Source Domain Adaptation for Object Detection](https://arxiv.org/abs/2605.13140)
*Sangin Lee, Seokjun Kwon, Jeongmin Shin, Namil Kim, Yukyung Choi*

#### 🧩 TL;DR
MS-DePro提出利用深度图和文本作为域无关输入模态，通过深度引导定位和多模态提示学习，解决多源域适应目标检测中域无关与域特定特征纠缠的问题，在多个MSDA基准上取得最先进性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有通用目标检测在目标域与训练分布存在差异时表现不佳，多源域适应（MSDA）虽优于单源域适应，但现有方法在处理域特定RGB图像时难以同时学习域无关特征并保留域特定信息，二者存在相互干扰的问题。

**Method:** MS-DePro由深度引导定位和多模态引导提示学习两个核心组件构成。深度引导定位利用深度图作为域无关输入模态生成域无关的区域建议；多模态引导提示学习则融合多模态特征以对齐可学习的文本嵌入，从而完成分类任务，整体框架无需依赖域特定的RGB图像特征。

**Result:** MS-DePro在多个MSDA基准测试中取得了最先进的性能，全面的消融实验验证了各贡献模块的有效性。

**Conclusion:** 利用深度图和文本等域无关模态可以有效解耦域无关与域特定特征，为多源域适应目标检测提供了一种新的范式，代码已开源。
...
### [15] [Dual-Pathway Circuits of Object Hallucination in Vision-Language Models](https://arxiv.org/abs/2605.13156)
*Jiaxin Liu, Ding Zhong, Yue Wang, Zhidong Yang, Zhaolu Kang, Guangyuan Dong, Qishi Zhan, Pengcheng Fang, Aofan Liu*

#### 🧩 TL;DR
提出双路径电路分析框架，通过激活修补和条件路径分析识别视觉语言模型中的物体幻觉相关神经回路，发现接地路径与幻觉路径的极性翻转机制，并通过定向抑制将幻觉最多降低76%，且回路在不同架构和幻觉类型间具有可迁移性。

---

#### 📘 Detailed Summary
**Motivation:** 视觉语言模型在跨模态推理中表现出色，但经常产生物体幻觉，即描述图像中不存在的内容，这严重限制了模型的可靠性和可解释性。现有研究缺乏对幻觉产生机制的深入理解，亟需一个能定位并表征幻觉相关神经回路的系统框架。

**Method:** 提出双路径电路分析框架，首先在五种架构多样的VLM上应用激活修补技术，识别出支持正确预测的视觉接地路径和驱动错误输出的幻觉路径；随后引入条件路径分析（CPA）来表征路径级交互，揭示接地组件在正确和幻觉样本中均保持强冗余性但经历一致的极性翻转——从支持正确答案转向支持幻觉答案；最后对幻觉路径组件进行定向抑制干预。

**Result:** 定向抑制幻觉路径组件可将物体幻觉最多降低76%且准确率损失极小；所识别的电路在POPE-adversarial和AMBER基准上验证了一致性，支持因果干预，且能选择性迁移到关系型幻觉而非属性型幻觉上。

**Conclusion:** 该研究首次系统地揭示VLM中物体幻觉的双路径电路机制，证明幻觉回路在不同架构间具有一致性、支持因果干预且可跨幻觉类型选择性迁移，为提升VLM可靠性和可解释性提供了机械论理解基础和实用的干预策略。
...
### [16] [A$_3$B$_2$: Adaptive Asymmetric Adapter for Alleviating Branch Bias in Vision-Language Image Classification with Few-Shot Learning](https://arxiv.org/abs/2605.13161)
*Yiyun Zhou, Zhonghua Jiang, Wenkang Han, Kunxi Li, Mingjing Xu, Chang Yao, Jingyuan Chen*

#### 🧩 TL;DR
本文揭示了视觉-语言模型（如CLIP）在图像分类中存在分支偏差（Branch Bias）问题，即图像分支的适配在分布外设定下并非总是有效，并提出了一种自适应非对称适配器A₃B₂，通过不确定性感知适配器阻尼和混合专家负载均衡正则化，在少量样本微调中持续超越11种基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有大规模视觉-语言模型的少样本迁移学习方法遵循固定的微调范式，默认图像分支和文本分支的重要性均匀，但这一假设在图像分类任务中尚未被系统研究。本文通过分析发现了一个分支偏差（Branch Bias）问题：在分布外设定下，适配图像编码器并不总是能提升性能，现有方法忽略了这一非对称性带来的负面影响。

**Method:** 本文提出A₃B₂（Adaptive Asymmetric Adapter），一种自适应非对称适配器，用于缓解少样本学习中的分支偏差。A₃B₂引入了不确定性感知适配器阻尼（Uncertainty-Aware Adapter Dampening, UAAD），能够在预测不确定性高时自动抑制图像分支的适配，实现对适配强度的软性数据驱动控制。在架构上，A₃B₂采用轻量级非对称设计，受混合专家（Mixture-of-Experts）启发并结合负载均衡正则化（Load Balancing Regularization）。

**Result:** 在三个少样本图像分类任务（涵盖11个数据集）上的大量实验表明，A₃B₂在性能上一致地超越了11种具有竞争力的基于提示调优和基于适配器的基线方法，验证了其在少样本和分布外场景下的有效性和鲁棒性。

**Conclusion:** 本文首次系统揭示了视觉-语言模型图像分类中的分支偏差问题，为非对称适配提供了理论依据；提出的A₃B₂通过自适应控制和轻量级非对称架构，在不依赖手动干预的条件下实现了对分支适配的动态调控，为少样本迁移学习提供了新的设计范式。
...
### [17] [CLIP Tricks You: Training-free Token Pruning for Efficient Pixel Grounding in Large VIsion-Language Models](https://arxiv.org/abs/2605.13178)
*Sangin Lee, Yukyung Choi*

#### 🧩 TL;DR
本文提出LiteLVLM，一种无需训练的、文本引导的视觉令牌剪枝策略，通过反转CLIP视觉-文本相似度排序来保留指代区域令牌并恢复上下文令牌，在像素级定位任务中以22%加速和2.3倍内存缩减维持90%原始性能，且在多种令牌预算下超越现有方法5%以上。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉令牌剪枝方法在处理像素级定位任务时效果不佳，因为在这些任务中令牌的重要性高度依赖于输入文本，而单纯修剪冗余或信息量低的视觉令牌无法适应这种文本条件依赖关系。

**Method:** LiteLVLM通过反转CLIP视觉-文本相似度的排序结果，有效保留覆盖指代区域的视觉令牌，同时恢复上下文令牌以实现清晰的前景-背景分离，整个策略无需训练或微调即可直接应用。

**Result:** 在多种令牌预算下，LiteLVLM相比现有方法性能提升超过5%；无需任何训练或微调即可在实现22%推理速度提升和2.3倍内存缩减的同时维持原模型90%的性能表现。

**Conclusion:** 该研究表明，通过文本引导的令牌选择策略可以高效解决像素级定位中的视觉令牌冗余问题，且无需额外训练开销，为大规模视觉语言模型的推理加速提供了一种实用且有效的方案。
...
### [18] [STAR: Semantic-Temporal Adaptive Representation Learning for Few-Shot Action Recognition](https://arxiv.org/abs/2605.13202)
*Hongli Liu, Yu Wang, Shengjie Zhao*

#### 🧩 TL;DR
STAR是一个统一的少样本动作识别框架，通过时序语义注意力机制和语义引导的Mamba原型精炼器，有效弥合了语义-时序错配问题，在SSv2-Full和SSv2-Small等基准上相比现有方法实现了最高8.1%的性能提升。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于视觉-语言模型的少样本动作识别方法存在两大核心挑战：一是语义-时序错配问题，即静态文本提示难以捕获序列中稀疏出现的关键视觉线索；二是多尺度时序动态建模不充分，短期判别线索与长期依赖关系常被过度平滑或碎片化处理，导致模型难以从少量标注样本中有效泛化到新动作类别。

**Method:** STAR框架由语义对齐模块和时序感知模块组成。语义对齐模块提出时序语义注意力机制，在帧级别执行跨模态对齐以确保细粒度的语义-时序一致性。时序感知模块包含语义时序原型精炼器，该精炼器将语义引导的Mamba块与多频时序采样及双向状态空间精炼相结合，生成具有增强判别保真度和时序一致性的语义对齐原型。此外，利用大语言模型生成时序依赖的类别描述以提供长期语义引导。

**Result:** 在五个少样本动作识别基准数据集上的大量实验表明，STAR一致优于现有最先进方法。具体而言，在SSv2-Full和SSv2-Small数据集上，1-shot设置下的性能提升分别高达8.1%和6.7%；在HMDB51数据集上提升7.3%，充分验证了其在有限监督条件下的有效性。

**Conclusion:** STAR通过统一框架有效解决了少样本动作识别中的语义-时序错配和多尺度时序建模问题，验证了将Mamba的序列建模能力迁移到少样本动作识别任务中的可行性。代码已开源，为后续研究提供了可复现的基础。
...
### [19] [Img2CADSeq: Image-to-CAD Generation via Sequence-Based Diffusion](https://arxiv.org/abs/2605.13293)
*Shiyu Tan, Zixuan Zhao, Hao Gao, Zhiheng Chen, Xiaolong Yin, Enya Shen*

#### 🧩 TL;DR
Img2CADSeq提出了一种多阶段流水线，通过三级层次化码本编码CAD序列、基于重要性优先策略压缩长序列，并利用粗到细点云中间表示和对比学习对齐2D图像与3D CAD序列，在单视角图像重建BRep方面显著超越现有方法，生成的STEP文件可直接用于商业CAD软件。

---

#### 📘 Detailed Summary
**Motivation:** 从单视角图像重建高质量的BRep（边界表示）是CAD领域的难题，主要挑战在于CAD拓扑约束和操作序列的复杂性，现有方法难以有效处理长序列的离散化建模以及2D图像与3D CAD序列之间的模态鸿沟。

**Method:** Img2CADSeq提出多阶段流水线，首先将CAD序列编码为三级层次化码本，并通过重要性优先策略优先保留轮廓信息、压缩细节，从而将长序列映射到稳定的离散潜空间；然后利用粗到细的点云作为中间表示，通过对比学习对齐2D视觉特征与3D CAD序列，并以此条件化VQ-Diffusion模型进行生成；此外，还引入了CAD-220K和PrintCAD两个新数据集以增强工业域适应性。

**Result:** 大量实验表明，Img2CADSeq在单视角图像到BRep重建任务上显著优于现有最先进方法，能够生成符合工业标准的STEP文件，并可直接在商业CAD软件中打开和使用。

**Conclusion:** 本文证明了通过层次化码本编码与重要性优先策略可以有效处理CAD序列的复杂拓扑和长程依赖问题，结合点云中间表示与对比学习能够有效弥合2D图像与3D CAD序列之间的模态鸿沟，为图像驱动CAD重建的工业落地提供了可行方案。
...
### [20] [KamonBench: A Grammar-Based Dataset for Evaluating Compositional Factor Recovery in Vision-Language Models](https://arxiv.org/abs/2605.13322)
*Richard Sproat, Stefano Peluchetti*

#### 🧩 TL;DR
本文提出KamonBench，一个基于文法的合成家纹图像-结构基准数据集，包含20,000个合成复合家纹及正式描述语言标注，用于评估视觉-语言模型在稀疏组合视觉识别和因子恢复任务上的表现，并提供了ViT/Transformer与VGG n-gram解码器的基线结果。

---

#### 📘 Detailed Summary
**Motivation:** 组合视觉识别（compositional visual recognition）中描述空间稀疏、因子可解释性差，现有基准多停留在描述级准确率评估，缺乏对模型内部因子恢复能力的细粒度测试，KamonBench旨在填补这一空白。

**Method:** KamonBench包含20,000个基于语法规则生成的合成家纹图像，每个图像配对正式的kamon yōgo描述、分词日语分析、英文翻译及程序代码；数据由容器（container）、修饰符（modifier）和主题（motif）三因子可控生成，支持程序代码级因子指标、因子对重组分割、反事实主题敏感性组及线性探针评估；基线模型包括ViT编码器/Transformer解码器及两种带/不带学习位置遮蔽的VGG n-gram解码器。

**Result:** ViT编码器/Transformer解码器与两种VGG n-gram解码器（含/不含学习位置遮蔽）的基线实验在KamonBench上完成，展示了模型在稀疏组合视觉识别和因子恢复任务上的初步性能，但未提供具体数值结果。

**Conclusion:** KamonBench为稀疏组合视觉识别和视觉-语言模型中因子恢复研究提供了一个受控的测试平台，通过因子级指标、重组分割和反事实分析等机制，超越了传统描述级评估，为后续组合泛化研究奠定了基准。
...
### [21] [Stylized Text-to-Motion Generation via Hypernetwork-Driven Low-Rank Adaptation](https://arxiv.org/abs/2605.13333)
*Junhyuk Jeon, Seokhyeon Hong, Junyong Noh*

#### 🧩 TL;DR
提出一种基于超网络生成LoRA参数的轻量级风格条件框架，通过动态调制预训练文本驱动运动扩散模型实现高效运动风格化，在HumanML3D和100STYLE数据集上达到最优性能，并显著提升了对未见风格的泛化能力。

---

#### 📘 Detailed Summary
**Motivation:** 现有文本驱动运动扩散模型难以表达运动中的细微风格信息，而当前的风格化方法要么需要对模型进行风格特定的微调，要么依赖基于ControlNet的重型架构，导致效率低下且难以泛化到未见风格。

**Method:** 提出一种轻量级风格条件框架，通过超网络将编码后的全局风格嵌入映射为LoRA低秩参数，在扩散模型的每个去噪步骤动态调制其行为；同时采用监督对比损失构建风格隐空间，支持优化引导且无需预定义风格类别。

**Result:** 在HumanML3D和100STYLE数据集上的实验结果表明，该方法在风格化任务上达到最先进的性能，同时在未见风格上实现了更好的风格化效果。

**Conclusion:** 通过超网络生成LoRA参数进行轻量级条件调制，既能高效捕捉多样化的风格属性，又能提升对未见风格的泛化能力，为运动风格化提供了一种高效且可扩展的框架。
...
### [22] [Drag within Prior Distribution: Text-Conditioned Point-Based Image Editing within Distribution Constraints](https://arxiv.org/abs/2605.13349)
*Haoyang Hu, Masataka Seo, Yen-Wei Chen*

#### 🧩 TL;DR
本文针对扩散模型点编辑方法在大位移编辑中存在的噪声潜码偏离反演轨迹产生伪影的问题，提出CLIP引导的语义对齐与先验保持损失约束方法，并引入方向加权点跟踪机制，从而在全局和细粒度编辑任务中提升生成质量与一致性。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于扩散模型的点编辑方法依赖于手柄点与目标点对定义运动轨迹，当两点间距离较大时，累积扰动易导致噪声潜码偏离反演分数轨迹，产生非自然伪影；此外，传统点编辑在全局编辑任务中存在语义歧义和不必要修改的问题，亟需提升编辑过程的方向合理性与生成一致性。

**Method:** 本文提出三方面改进：在全局编辑任务中，引入CLIP模型对中间编辑步骤进行评估和引导，确保生成结果保持语义对齐；同时提出先验保持损失，约束优化后的潜码保持在扩散先验的采样空间内，使其沿熟悉分数轨迹生成图像；在细粒度编辑任务中，设计方向加权点跟踪机制，在相似特征区域内引导编辑过程沿目标方向推进，提升跟踪精度与生成质量并缩短编辑时间。

**Result:** 实验结果表明，所提方法在大位移点编辑场景下显著减少了噪声潜码偏离反演轨迹导致的伪影，提升了全局编辑的语义对齐性；方向加权点跟踪机制在细粒度任务中改善了跟踪精度和生成质量，同时缩短了编辑耗时。

**Conclusion:** 本文通过CLIP语义引导、先验保持损失和方向加权点跟踪三项策略，有效解决了扩散模型点编辑在大位移和细粒度场景下的关键缺陷，提升了编辑的一致性与质量，为基于扩散的图像编辑提供了更稳健的优化框架。
...
### [23] [GRIP-VLM: Group-Relative Importance Pruning for Efficient Vision-Language Models](https://arxiv.org/abs/2605.13375)
*Mingzhe Huang, Weijun Wang, Xin Ding, Liang Mi, Hao Wen, Yuanchun Li, Lichen Pang, Shansong Yang, Yunxin Liu, Ting Cao*

#### 🧩 TL;DR
提出GRIP-VLM框架，将视觉token剪枝建模为马尔可夫决策过程，利用强化学习（GRPO）直接优化离散选择空间，配合监督式预热和预算感知评分器，在不重训练的情况下实现任意压缩比，在多种多模态基准上取得优于启发式和监督式方法的性能，最高带来15%的推理加速。

---

#### 📘 Detailed Summary
**Motivation:** 视觉-语言模型中处理大量视觉token带来巨大的计算开销，现有基于训练感知的剪枝方法依赖连续梯度松弛，但视觉token剪枝本质上是离散、非凸的组合优化问题，连续近似方法在激进压缩预算下容易陷入次优局部极小值，亟需一种能直接探索离散选择空间的高效剪枝框架。

**Method:** 提出GRIP-VLM框架，将视觉token剪枝建模为马尔可夫决策过程，采用基于监督式预热的Group Relative Policy Optimization范式直接优化离散选择空间；引入预算感知评分器，使轻量级智能体动态评估每个token的重要性，并自适应任意压缩比而无需重新训练。

**Result:** 在多种多模态基准上的大量实验表明，GRIP-VLM在性能上持续优于启发式方法和监督式基线，实现了更优的帕累托前沿，在相同精度下最高可带来15%的推理加速。

**Conclusion:** 将视觉token剪枝重新定义为强化学习驱动的离散优化问题，能够有效克服连续松弛方法在激进压缩下的局部最优困境，为VLM的高效推理提供了一种无需重训练、自适应任意压缩比的通用剪枝范式。
...
### [24] [Backbone is All You Need: Assessing Vulnerabilities of Frozen Foundation Models in Synthetic Image Forensics](https://arxiv.org/abs/2605.13381)
*Chiara Musso, Joy Battocchio, Andrea Montibeller, Giulia Boato*

#### 🧩 TL;DR
本文提出代理迭代对抗攻击（SIAA），一种仅利用检测器 ViT 骨干网络知识的灰盒攻击方法，可在目标检测器的特征空间内生成高成功率的对抗样本，揭示仅凭骨干网络知识即可破坏深度伪造检测器可靠性的严重漏洞。

---

#### 📘 Detailed Summary
**Motivation:** 当前基于 Vision Transformer（ViT）的深度伪造检测器普遍依赖冻结的预训练骨干网络，但这种设计范式存在一个未被充分重视的关键漏洞：攻击者可能仅利用骨干网络知识就能构造有效对抗样本，从而破坏检测系统的可靠性。本文旨在揭示这一安全缺口并量化其严重性。

**Method:** 本文提出 Surrogate Iterative Adversarial Attack (SIAA)，一种灰盒攻击方法。该方法仅需知晓目标检测器的 ViT 骨干网络架构，完全在目标检测器的特征空间中迭代生成对抗样本，无需访问完整的检测模型或分类头。

**Result:** 在多种 ViT 骨干的检测器上，SIAA 在包括少样本学习、完全训练错配及攻击可迁移性测试等多样化的灰盒场景中均取得高攻击成功率，其性能常接近白盒攻击水平。实验表明仅凭骨干网络知识就足以有效绕过当前主流检测器。

**Conclusion:** 本文揭示仅依赖冻结预训练 ViT 骨干网络的深度伪造检测系统存在严重安全漏洞，骨干网络知识本身即足以破坏检测器可靠性。研究强调了在对抗性多媒体取证领域亟需发展更具鲁棒性的防御机制。
...
### [25] [FedHPro: Federated Hyper-Prototype Learning via Gradient Matching](https://arxiv.org/abs/2605.13475)
*Huan Wang, Jun Shen, Haoran Li, Zhenyu Yang, Jun Yan, Ousman Manjang, Yanlong Zhai, Di Wu, Guansong Pang*

#### 🧩 TL;DR
提出基于超原型（hyper-prototypes）的联邦学习框架FedHPro，通过梯度匹配从客户端真实样本中蒸馏语义知识而非聚合原型级描述符，并联合互对比学习与一致性惩罚提升类间分离性与类内一致性，在多种异构场景下取得最先进性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于原型的联邦学习方法通过原型级平均或精炼来更新全局原型，导致跨客户端的语义漂移（semantic drift）问题，进而产生不对齐的全局信号，限制了模型的泛化能力。

**Method:** 提出超原型（hyper-prototypes）概念，即一组可学习的全局逐类原型，通过梯度匹配（gradient matching）直接从客户端真实样本中蒸馏与类别相关的特征进行优化，而非依赖原型级描述符。在此基础上设计FedHPro框架，利用超原型通过带客户端特定间隔的互对比学习（mutual-contrastive learning）提升类间分离性，并引入一致性惩罚（consistency penalty）促进类内均匀性。

**Result:** 在多种异构性场景下的全面实验表明，超原型能够产生语义一致性更强的全局信号，FedHPro在多个基准数据集上取得了最先进的性能。

**Conclusion:** 通过直接从客户端样本中学习超原型而避免原型级聚合，能有效缓解联邦学习中的语义漂移问题，为基于原型的联邦学习提供了新的范式；所提的互对比学习与一致性惩罚机制进一步提升了特征空间的判别性与紧凑性。
...
### [26] [ArcVQ-VAE: A Spherical Vector Quantization Framework with ArcCosine Additive Margin](https://arxiv.org/abs/2605.13517)
*Jaeyung Kim, YoungJoon Yoo*

#### 🧩 TL;DR
ArcVQ-VAE通过引入球面角间距先验（SAMP），包括球有界范数正则化和反余弦加性间隔损失，改进了VQ-VAE框架中离散表征的多样性和码本利用率，在图像重建与生成任务上取得竞争性表现。

---

#### 📘 Detailed Summary
**Motivation:** 传统VQ-VAE使用有限码本向量对所有图像进行离散化表征，其容量限制导致难以捕捉丰富多样的表征，存在表征能力不足和码本利用率低的问题。

**Method:** 提出ArcVQ-VAE框架，核心是球面角间距先验（SAMP），包含两个组成部分：球有界范数正则化，将码本向量约束在随时间变化的欧几里得球内；反余弦加性间隔损失，鼓励潜在向量之间具有更大的角度可分离性。两者共同促使表征在受限空间中更具判别性和均匀分散性，提升有效潜在空间覆盖和码本利用率。

**Result:** 在标准图像重建与生成任务上的实验结果表明，ArcVQ-VAE在重建精度、表征多样性和样本质量方面均取得了与基线模型相竞争的性能。

**Conclusion:** 通过在VQ-VAE中引入角度化的球形约束先验，能够有效提升离散表征的质量与码本利用效率，为改进向量量化框架提供了新的正交方向。
...
### [27] [CA-GCL: Cross-Anatomy Global-Local Contrastive Learning for Robust 3D Medical Image Understanding](https://arxiv.org/abs/2605.13544)
*Hanwen Zhang, Yao Liu, Die Dai, Jiaye Yang, Qiao Liu, Yutong Xie, Peng Wang*

#### 🧩 TL;DR
提出CA-GCL框架，通过跨解剖结构全局-局部对比学习与临床感知文本增强，解决细粒度视觉-语言预训练中文本嵌入空间坍塌和提示敏感性问题，在3D医学图像零样本异常检测中取得领先性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有细粒度视觉-语言预训练范式在3D医学图像理解中面临严重的文本嵌入空间表示坍塌问题，不同解剖结构的文本嵌入高度聚类且难以区分，导致模型对提示变化高度敏感，阻碍临床可靠部署。

**Method:** 提出跨解剖结构全局-局部对比学习框架CA-GCL，包含全局对比目标以强制解剖类别在潜在空间中分离、抵消局部对齐导致的聚合倾向，以及基于排列不变性和部分完整性的临床感知文本增强策略以提升对描述不完备性的鲁棒性。

**Result:** 在CT-RATE和Rad-ChestCT数据集上，CA-GCL在零样本异常检测中一致优于现有VLP范式，展现出强跨数据集泛化能力；关键地，CA-GCL显著降低了不同提示模板间的性能方差，将坍塌的文本相似度分布转变为钟形分布。

**Conclusion:** CA-GCL通过缓解表示坍塌和提示敏感性，为鲁棒的3D医学图像理解提供了有效框架，验证了全局-局部对比学习结合临床感知增强在细粒度医学视觉-语言预训练中的关键作用。
...
### [28] [HIR-ALIGN: Enhancing Hyperspectral Image Restoration via Diffusion-Based Data Generation](https://arxiv.org/abs/2605.13581)
*Li Pang, Heng Zhao, Yijia Zhang, Deyu Meng, Xiangyong Cao*

#### 🧩 TL;DR
HIR-ALIGN提出了一种即插即用的目标自适应增强框架，通过代理生成、分布自适应合成和对齐微调三个阶段，在无需额外数据的情况下将有限训练图像与目标分布对齐，显著提升跨域高光谱图像复原性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有高光谱图像复原模型在源数据上训练后，往往因域偏移而无法有效处理缺少干净参考图像的目标域数据，这是一个实践中普遍存在但尚未充分解决的问题。

**Method:** HIR-ALIGN包含三个阶段：代理生成阶段利用现成复原模型将退化的目标观测恢复为保持语义的代理HSI；分布自适应合成阶段通过模糊鲁棒的unCLIP扩散模型从代理RGB生成目标对齐RGB，再经基于warp的光谱传输模块（含软分块传输权重和可学习局部插值核）将生成RGB与代理RGB对齐以合成目标对齐HSI；对齐微调阶段使用代理HSI和合成HSI微调预训练网络，最终部署于退化目标图像。

**Result:** 在模拟和真实数据集上的去噪与超分辨率实验中，HIR-ALIGN一致地提升了源监督基线的性能，并优于无监督方法和源监督对照方法，验证了其在跨域高光谱图像复原中的有效性。

**Conclusion:** 理论分析表明，基于增强的微调通过提升目标分布覆盖率和控制谱偏差可以降低目标域复原风险，为无目标干净参考的跨域高光谱复原提供了一种高效且无需额外数据的即插即用框架。
...
### [29] [SceneGraphVLM: Dynamic Scene Graph Generation from Video with Vision-Language Models](https://arxiv.org/abs/2605.13667)
*Vladislav Makarov, Mark Gizetdinov, Dmitry Yudin*

#### 🧩 TL;DR
SceneGraphVLM提出了一种基于小型视觉语言模型的高效图像与视频场景图生成方法，采用TOON序列化格式和两阶段训练（监督微调+强化学习），并通过幻觉感知奖励平衡关系覆盖率和精度，在保持合理召回的同时实现了约一秒延迟的完整场景图生成。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于VLM的场景图生成方法能够端到端地将图预测为结构化文本，但常产生包含无关对象和关系的冗长输出，且现有方法在精度与速度之间难以取得良好平衡。

**Method:** SceneGraphVLM采用TOON（Token-efficient Object-Object-Noise）格式序列化场景图以减少token长度；通过两阶段训练——先进行监督微调，再使用基于强化学习的幻觉感知奖励（同时惩罚不支持的对象/关系并平衡关系覆盖率和精度）进行优化；对于视频输入，可选择性地将上一帧生成的图作为当前帧的上下文，提供轻量级短期时序信息。推理时采用vLLM加速解码。

**Result:** SceneGraphVLM在PSG、PVSG和Action Genome三个基准上进行了评估，在使用紧凑型VLM和vLLM加速的条件下，实现了高质量与低延迟的权衡，显著提升了面向精度的SGG指标，同时保持合理的召回率，完整场景图生成延迟约为1秒。

**Conclusion:** 该研究表明，通过紧凑的VLM架构结合token高效的图序列化格式和强化学习优化，可以在不依赖复杂后处理或跟踪模块的情况下，实现快速且精确的图像与视频场景图生成，为实用化的场景理解提供了可行方案。
...
### [30] [Weakly Supervised Segmentation as Semantic-Based Regularization](https://arxiv.org/abs/2605.13674)
*Stefano Colamonaco, Andrei-Bogdan Florea, Jaron Maene*

#### 🧩 TL;DR
提出神经符号主义视角的弱监督语义分割方法，将可微模糊逻辑与深度分割模型（SAM）结合，通过连续逻辑约束统一弱标注与领域先验知识，在对SAM进行逻辑引导微调后训练无提示分割模型，在Pascal VOC 2012和REFUGE2上达到超越全监督基线的SOTA精度。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于基础模型（如SAM）的WSSS方法依赖启发式提示选择，难以有效融入先验知识与异质标注信息，缺乏系统性地整合多种弱监督信号的机制。

**Method:** 提出神经符号主义框架，将可微模糊逻辑与深度分割模型集成：将弱标注和领域先验统一表示为连续逻辑约束，用于在弱监督下微调SAM；利用微调后SAM生成更优伪标签，再训练第二阶段的无需提示的分割模型。

**Result:** 在Pascal VOC 2012和REFUGE2视盘/视杯分割数据集上，逻辑引导微调生成的伪标签质量更高，最终分割精度达到SOTA，且经常超越全监督基线。

**Conclusion:** 可微模糊逻辑为弱监督分割中融入先验知识与异质标注提供了系统化且高效的途径，逻辑引导的基础模型微调策略可显著提升伪标签质量并推动弱监督分割性能上限。
...
### [31] [Learning to Optimize Radiotherapy Plans via Fluence Maps Diffusion Model Generation and LSTM-based Optimization](https://arxiv.org/abs/2605.13713)
*Isabella Poles, Simon Arberet, Riqiang Gao, Martin Kraus, Marco D. Santambrogio, Florin C. Ghesu, Ali Kamen, Dorin Comaniciu*

#### 🧩 TL;DR
提出了一种基于扩散驱动学习优化（L2O）的端到端容积旋转调强放疗（VMAT）规划方法，通过分布匹配蒸馏扩散模型实现通量图的一次性生成，并利用LSTM的L2O模块在推理阶段快速优化通量图，提升了规划效率、灵活性和机器可交付性。

---

#### 📘 Detailed Summary
**Motivation:** VMAT规划涉及多叶准直器、监测单元和剂量参数的逆向与嵌套优化，且需确保机械可交付性，导致治疗配置变更时需反复重新优化，每位患者的规划时间显著增长，现有端到端VMAT规划器无法同时满足效率、灵活性和可交付性要求。

**Method:** 提出了一种扩散驱动的学习优化（L2O）方法：首先使用分布匹配蒸馏扩散模型学习临床可行的通量图流形，实现一次性通量图生成；然后基于LSTM的L2O模块学习梯度更新动力学，在推理过程中根据指定的剂量目标快速精炼通量图。

**Result:** 在临床和公开前列腺癌数据集上的实验结果表明，所提方法在规划效率、灵活性和机器可交付性方面均优于现有的端到端VMAT规划器。

**Conclusion:** 该研究验证了将扩散模型与学习优化结合用于VMAT规划的可行性，为放疗规划提供了一种高效、灵活且临床可用的端到端解决方案，有望显著减少人工反复优化带来的规划时间开销。
...
### [32] [Weakly-Supervised Spatiotemporal Anomaly Detection](https://arxiv.org/abs/2605.13746)
*Urvi Gianchandani, Praveen Tirupattur, Mubarak Shah*

#### 🧩 TL;DR
提出了一种基于弱监督学习的视频异常检测方法，利用视频级别的标签和多重实例排序损失（MIL）实现时空异常检测，并在UCF Crime2Local数据集上验证了该方法。

---

#### 📘 Detailed Summary
**Motivation:** 视频标注耗时且成本高，现有异常检测方法通常需要精细的时空标注，这在实际应用中难以大规模获取；因此，研究旨在探索一种仅依赖弱视频级标签（即仅知道视频是否包含异常）而不需要像素级或帧级标注的弱监督异常检测方法。

**Method:** 从视频片段中提取特征，将异常片段和正常片段分别构建为正包和负包，应用多重实例排序损失（MIL）进行训练；基于分类器为片段的时空区域计算异常分数，实现空间和时间两个维度的异常检测。

**Result:** 在UCF Crime2Local数据集上进行了实验，该数据集包含UCF Crime数据集部分视频的时空标注，验证了所提弱监督方法在时空异常检测任务上的有效性。

**Conclusion:** 仅利用弱视频级标签即可有效训练异常检测网络，且通过MIL框架可同时检测时间和空间上的异常区域，为减少对精细标注依赖的异常检测提供了可行思路。
...
### [33] [Generative Texture Diversification of 3D Pedestrians for Robust Autonomous Driving Perception](https://arxiv.org/abs/2605.13755)
*Arka Bhowmick, Enes Ozeren, Ahmed Abdullah, Oliver Wasenmuller*

#### 🧩 TL;DR
提出了一种利用StyleGAN2从单个3D基础资产生成多样化面部纹理和外观变体的方法，实现可扩展的行人资产多样化以用于合成场景生成，并通过混合真实与合成数据的实验揭示了2D检测鲁棒性提升与3D感知模型对几何域偏移的敏感性。

---

#### 📘 Detailed Summary
**Motivation:** 真实世界数据集难以满足自动驾驶中安全关键场景对高质量2D和3D感知训练数据不断增长的需求，大规模标注数据采集成本高且耗时长，而合成数据虽是可扩展的替代方案，但现有方法在生成多样化的行人资产方面仍缺乏高效且可控制的手段。

**Method:** 提出一种从单个3D基础资产出发，利用StyleGAN2合成多样化的面部纹理和身份级外观变化，并将其自动映射到3D网格上的方法，实现无需为每个实例重新设计几何结构的可扩展外观级资产多样化，并基于生成的资产构建合成数据集进行混合训练。

**Result:** 通过混合真实与合成数据的RGB目标检测实验表明，受控的合成多样化提升了2D检测的鲁棒性；而针对3D点云目标检测的几何驱动分布偏移实验揭示了3D感知模型对几何域间隙的敏感性。

**Conclusion:** 生成式AI能够通过受控的面部纹理合成为自动驾驶实现可扩展的仿真就绪行人多样化，同时跨域训练策略在2D检测中带来鲁棒性提升，但在3D感知中面临显著的几何域偏移挑战，为自动驾驶管线的数据生成与训练策略提供了重要指导。
...
### [34] [JANUS: Anatomy-Conditioned Gating for Robust CT Triage Under Distribution Shift](https://arxiv.org/abs/2605.13813)
*Lavsen Dahal, Yubraj Bhandari, Geoffrey Rubin, Joseph Y. Lo*

#### 🧩 TL;DR
JANUS提出了一种生理引导的双流架构，通过解剖学引导门控机制将视觉嵌入与宏观放射组学先验条件化，在CT自动分诊中同时提升了判别准确性和模型可靠性，在内部和外部数据集上均优于现有基线。

---

#### 📘 Detailed Summary
**Motivation:** 自动CT分诊需要模型在多种病理上同时具备高准确性，并在机构转移（institutional shift）下保持可靠性。现有Vision Transformers虽然提供了强大的视觉表示，但许多临床显著发现是由定量成像生物标志物而非外观单独定义的，现有方法未能有效融合这类生理先验信息。

**Method:** JANUS是一种生理引导的双流架构，包含视觉流和放射组学流，通过解剖学引导门控（Anatomically Guided Gating）机制将宏观放射组学先验条件化到视觉嵌入上，使模型能够同时利用外观特征和定量成像生物标志物进行决策。

**Result:** 在MERLIN测试集（N=5082）上，JANUS达到macro-AUROC 0.88和AUPRC 0.74，超越所有复现的基线模型；在外部数据集（N=2000）上AUROC为0.87，其中对由尺寸和衰减定义的表现提升最大。JANUS在两个数据集上均改善了模型校准，并通过生理否决率（PVR）指标量化预测抑制，表明在域偏移下其减少高置信度假阳性的比例显著高于减少真阳性的比例。

**Conclusion:** 结果表明，基于物理基础的生理先验条件化能够同时提升CT分诊中的判别能力与可靠性，为将定量成像生物标志物融入深度学习提供了有效路径，代码和模型权重均已公开。
...
### [35] [OmniLiDAR: A Unified Diffusion Framework for Multi-Domain 3D LiDAR Generation](https://arxiv.org/abs/2605.13815)
...
#### 🧩 TL;DR
提出OmniLiDAR，一个统一的文本条件扩散框架，能够在八个代表性领域（涵盖恶劣天气、传感器配置变化、跨平台采集三种分布偏移类型）中生成LiDAR扫描数据，通过跨域训练策略和域自适应特征建模实现单一模型下的异构域可控生成。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于扩散的LiDAR生成器通常针对单一域开发，需要为不同数据集或传感条件分别训练模型，无法在异构分布偏移下实现统一可控的LiDAR场景生成，且缺乏能够涵盖多种传感条件变化的公开统一基准数据集。

**Method:** 提出OmniLiDAR统一文本条件扩散框架，采用共享距离图像表示跨八个域生成LiDAR扫描；引入跨域训练策略（CDTS）在mini-batch内混合域并通过条件控制引导生成；提出跨域特征建模（CDFM）沿方位角和仰角轴捕获方向依赖以反映距离图像的各向异性扫描结构；以及域自适应特征缩放（DAFS）作为轻量级调制机制以建模去噪过程中的结构化域相关特征偏移。

**Result:** 通过结合真实扫描、基于物理的天气模拟和系统化波束缩减构建了8域数据集，大量实验表明该方法在生成保真度方面表现优异，在下游任务（LiDAR语义分割和3D目标检测的生成式数据增强、鲁棒性评估）中一致提升性能，尤其在有限标签场景下效果显著。

**Conclusion:** OmniLiDAR证明了通过统一框架实现跨多种LiDAR传感条件可控生成的可行性，为可扩展仿真和合成数据生成提供了有效方案，尤其在恶劣天气、传感器配置变化和跨平台场景下的生成式数据增强具有重要实用价值。
...
### [36] [Training Long-Context Vision-Language Models Effectively with Generalization Beyond 128K Context](https://arxiv.org/abs/2605.13831)
...
#### 🧩 TL;DR
本文系统研究了视觉语言模型的长上下文继续预训练(LongPT)配方，通过大量消融实验发现长文档VQA优于OCR转录、平衡序列长度分布优于聚焦长序列、检索仍是主要瓶颈且纯长文档VQA可保留短上下文能力，并基于这些发现仅用5B token预算将Qwen2.5-VL-7B从32K扩展到128K上下文，得到MMProLong模型，在长文档VQA上提升7.1%并泛化到256K/512K上下文及多种长序列任务。

---

#### 📘 Detailed Summary
**Motivation:** 长上下文建模正成为现代视觉语言模型的核心能力，其在长文档理解、视频分析及智能体工作流中的多轮工具调用中至关重要，但目前针对长上下文数据混合的设计与平衡策略缺乏系统性的实践训练配方研究，如何高效地通过继续预训练扩展LVLM的上下文窗口仍是一个未充分探索的问题。

**Method:** 本文对7B规模的LVLM进行了从32K到128K上下文的长上下文继续预训练(LongPT)的系统消融研究，重点对比了长文档VQA与OCR转录两种数据类型的效果，并进一步分析了序列长度分布（平衡数据 vs 聚焦128K数据）、检索与推理数据配比、以及长数据与短数据的混合策略对训练效果的影响。基于发现的最优配方，仅使用5B token预算对Qwen2.5-VL-7B进行继续预训练，得到MMProLong模型。

**Result:** 消融实验表明：长文档VQA显著优于OCR转录数据；平衡的序列长度分布优于仅聚焦128K长度的数据；检索密集型数据混合优于检索加推理的数据混合；纯长文档VQA训练即可较好保留短上下文能力。基于这些发现训练的MMProLong在长文档VQA上提升7.1%，并能泛化到256K和512K的更长上下文（无需额外训练），同时在网页多模态针检索、长上下文视觉-文本压缩和长视频理解等任务上也表现出良好的泛化能力。

**Conclusion:** 本工作建立了面向视觉语言模型长上下文继续预训练的实用配方(LongPT)和经验基础，关键在于：平衡的序列长度分布有利于学习可泛化的关键信息检索能力、检索能力是长上下文的主要瓶颈、指令格式的长数据可减少对短数据混合的需求。这些发现为未来长上下文视觉语言模型的高效训练提供了重要指导。
...
### [37] [Unlocking Patch-Level Features for CLIP-Based Class-Incremental Learning](https://arxiv.org/abs/2605.13835)
*Hao Sun, Zi-Jun Ding, Da-Wei Zhou*

#### 🧩 TL;DR
提出SPA框架，通过利用CLIP中常被忽略的patch级语义信息，结合GPT-5生成的语义描述引导判别性局部特征选择与最优传输跨模态对齐，并在类增量学习中实现最优性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于CLIP的类增量学习（CIL）方法主要聚焦于全局图像嵌入与文本提示的对齐，但忽略了CLIP编码器中蕴含的丰富patch级语义信息（如兔子的长耳朵、蓬松尾巴等局部判别性线索），这些局部信息能为识别提供互补证据，因此需要一种能够唤醒并利用CLIP局部表示的方法。

**Method:** 提出SPA框架，首先为每个类别构建代表性多样化的视觉样本并输入GPT-5生成类别级语义描述；利用这些描述指导判别性patch级视觉特征的选择；通过最优传输将选中的patch token与语义描述中的语义token进行结构化跨模态对齐；引入任务特定投影器以适应下游增量任务；从存储的类别级高斯统计中采样伪特征来校准旧类表示以缓解灾难性遗忘。

**Result:** 大量实验表明SPA在类增量学习任务上取得了最先进的性能表现。

**Conclusion:** 该研究证实了唤醒CLIP中局部patch级语义信息并实现细粒度跨模态对齐能够显著提升类增量学习性能，同时任务特定投影器与基于高斯统计的伪特征采样策略有效缓解了灾难性遗忘，为基于预训练视觉语言模型的增量学习提供了新范式。
...
#### 🧩 TL;DR
针对通用大语言模型在增材制造专业领域回答准确性和相关性不足的问题，本文通过构建领域语料库，对比了基于LLaMA-3-8B的预训练基线、检索增强生成（RAG）和领域微调三种配置，实验表明RAG在准确率（75.5%）、相关性（90.8%）和整体偏好（85.2%）上显著优于微调方法，证明检索增强是适配工程专业领域更有效的路径。

---

#### 📘 Detailed Summary
**Motivation:** 通用大语言模型由于缺乏领域基础知识和结构化技术知识的充分暴露，在增材制造等专业工程领域难以生成可靠的专家级问答响应，现有方法（领域微调和检索增强）在AM场景下的有效性尚缺乏系统对比研究。

**Method:** 构建了涵盖学术文献、厂商文档、技术标准和操作指南等异构来源的增材制造领域语料库；基于LLaMA-3-8B设计了三种配置：预训练基线模型、从向量数据库中检索相关文档片段的RAG系统、以及在原始领域文本上微调的模型；使用200个专家设计的AM问题进行评测。

**Result:** RAG模型在200个问题中相对于基线有75.5%的回答被认为更准确、85.2%被整体偏好、90.8%被认为相关性更高；而领域微调仅在5.6%的情况下产生更准确的回答、32.5%的情况下产生更相关的回答，整体性能反而下降。

**Conclusion:** 检索增强生成方法比在非结构化技术数据上进行朴素微调更能有效地将LLMs适配到专业工程领域，为工程知识密集型场景下的LLM应用提供了实践指导。
...
#### 🧩 TL;DR
本文揭示视觉-语言模型在缺失视觉模态时性能显著下降且置信度严重失准，并提出潜在想象模块（LIM），通过轻量级跨注意力机制从文本输入预测潜在视觉嵌入，无需像素级图像生成即可恢复模型在纯文本场景下的准确性与校准度。

---

#### 📘 Detailed Summary
**Motivation:** 视觉-语言模型（VLM）虽然在训练时使用图像，但实际部署中常面临视觉模态缺失的情况（如仅文本输入）。现有研究发现直接移除视觉模态会导致VLM准确率大幅下降和严重的置信度失准，且模型行为不再与其原始语言骨干网络一致。更重要的是，这种失败不能仅用缺失语义信息解释——即使文本描述保留了关键语义内容，模型的置信度仍然不可靠。

**Method:** 本文提出潜在想象模块（Latent Imagination Module, LIM），这是一个轻量级的跨注意力模块，能够从纯文本输入中预测出想象中的潜在视觉嵌入，并将这些嵌入馈送至冻结的VLM骨干网络，无需进行像素级别的图像合成。LIM通过端到端训练学习从文本特征空间到视觉特征空间的映射，从而在推理阶段为缺失的视觉模态提供替代性潜在表示。

**Result:** 在纯文本基准测试、未见任务和缺失图像场景下，LIM均显著提升了准确率并降低了校准误差。实验对比表明，通过生成图像来添加视觉信号虽然能部分恢复准确性和校准度，但LIM作为更轻量的替代方案，在无需实际图像生成的情况下达到了类似甚至更好的效果。

**Conclusion:** 本文研究表明，模态缺失条件下VLM的退化不仅是信息缺失问题，还涉及模型对视觉信号的结构性依赖。潜在模态补全（latent modality completion）是一种实用的范式，通过在潜在空间层面完成缺失模态来保证VLM在推理时的可靠性和校准度，为VLM在真实部署场景中面对不完整输入提供了有效解决方案。
...
### [40] [BoostTaxo: Zero-Shot Taxonomy Induction via Boosting-Style Agentic Reasoning and Constraint-Aware Calibration](https://arxiv.org/abs/2605.12520)
*Yancheng Ling, Zhenlin Qin, Leizhen Wang, Zhenliang Ma*

#### 🧩 TL;DR
BoostTaxo是一个boosting风格的大语言模型框架，通过粗到细的父节点识别流程（包括检索增强的术语细化、混合父候选选择、候选评分和结构感知分数校准）实现了零样本分类学归纳，在三个基准数据集上取得了与最先进方法相当或更优的性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有分类学归纳方法在泛化能力、结构可靠性和效率方面存在局限，阻碍了其在零样本和大规模场景下的性能表现，亟需一种能够兼顾效率与结构可靠性的零样本分类学归纳方法。

**Method:** BoostTaxo采用boosting风格的LLM框架，以领域术语集合为输入，通过粗到细的方式执行父节点识别，具体包括：检索增强的术语定义细化、混合父候选选择、候选评分以及结构感知分数校准。其中，轻量级LLM用于高效筛选候选父节点，大规模LLM用于对候选父节点进行精细化排序和评分，最后通过结构特征对候选边权重进行校准以提升分类学结构的可靠性。

**Result:** 在WordNet、DBLP和SemEval-Sci三个公开基准数据集上的实验结果表明，BoostTaxo在零样本分类学归纳任务中取得了与最先进方法相当或更优的性能。消融研究验证了混合父候选选择和结构感知分数校准对整体性能的贡献，进一步分析探讨了候选选择规模对分类学质量的影响，并提供了代表性案例和失败案例分析。

**Conclusion:** BoostTaxo通过粗到细的boosting式LLM框架有效解决了零样本分类学归纳中的泛化性、结构可靠性和效率问题，混合候选选择与结构感知校准是关键贡献模块，候选选择大小对最终分类学质量有显著影响，该框架为大规模分类学归纳提供了高效可靠的解决方案。
...
### [41] [ToolWeave: Structured Synthesis of Complex Multi-Turn Tool-Calling Dialogues](https://arxiv.org/abs/2605.12521)
*Dinesh Khandelwal, Gnana Prakash Punnavajhala, GPS Bhargav, Gaurav Pandey, Sachin Joshi, Hima Karanam, Dinesh Raghu*

#### 🧩 TL;DR
ToolWeave提出了一个结构化框架，通过构建内置依赖的工具和细粒度规划追踪参数来源，合成更真实、多步骤占比更高的多轮工具调用对话，显著提升了LLM在多轮工具调用基准上的性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有合成数据生成管道生成的多轮工具调用对话存在两个核心问题：一是链式调用中工具仅表面兼容而缺乏与真实用户任务的对齐，二是一次性生成整个对话导致参数幻觉（参数既非用户提供也非先前工具调用产生）。这些问题还导致多步工具交互严重不足，限制了LLM作为自主智能体的工具调用能力。

**Method:** ToolWeave框架包含两个关键机制：首先，通过构建具有内置依赖关系的工具集来生成现实的多步工作流（工具序列），并基于与用户目标的对齐程度对工作流进行筛选；其次，引入细粒度规划阶段显式追踪参数来源（parameter provenance），从而减少参数幻觉。

**Result:** ToolWeave生成的合成对话中多步工具交互占比达45%，且在参数和工具名称层面的幻觉显著减少。基于ToolWeave微调的LLM在三个公开基准上一致优于基于先前数据集微调的模型。具体而言，Llama-3.1-70B在BFCL-V3多轮任务上达到39.75%，而基于当前最优ToolFlow数据微调的同一模型仅取得23.50%。

**Conclusion:** 该研究表明，通过结构化工具依赖设计和细粒度参数溯源规划，可以有效提升合成多轮工具调用数据的真实性和多样性，从而显著增强LLM在多轮工具调用场景中的自主智能体能力。
...
### [42] [In-Situ Behavioral Evaluation for LLM Fairness, Not Standardized-Test Scores](https://arxiv.org/abs/2605.12530)
*Zeyu Tang, Sang T. Truong, Deonna Owens, Shreyas Sharma, Yibo Jacky Zhang, Brando Miranda, Sanmi Koyejo*

#### 🧩 TL;DR
本文提出MAC-Fairness多智能体对话框架，通过将标准化测试问题转化为对话种子进行情境行为评估，替代传统标准化测试范式来评估LLM公平性，发现表面提示构建选择主导了标准化测试的分数方差并导致结论翻转，而情境行为评估能揭示稳定的模型特定行为特征。

---

#### 📘 Detailed Summary
**Motivation:** 传统LLM公平性评估依赖标准化测试范式的问答基准，但该范式存在结构性不可靠性：与公平性评估无关的表面提示构建选择（如措辞变化）主导了分数方差，既能翻转公平性结论的方向和幅度，又能导致模型排名严重不一致，因此需要一种更能反映真实对话行为的新型评估方法。

**Method:** 提出MAC-Fairness多智能体对话框架，将标准化测试问题重新用作对话种子而非评估工具，通过多轮对话中的受控变异因素实现情境行为评估；系统性地考察模型在两个维度上的行为——从自我视角出发的立场坚持性（position persistence）和从他人视角出发的同伴接受性（peer receptiveness），生成800万条对话记录并在多种模型和身份配置下进行分析。

**Result:** 表面提示构建选择（与公平性评估正交）主导了标准化测试的分数方差，能同时改变公平性结论的方向和幅度并导致模型排名严重不一致；而情境行为评估揭示了跨不同公平性目标和评估方法均能泛化的、稳定的模型特定行为特征，这是标准化测试范式无法提供的证据形式。

**Conclusion:** LLM公平性应通过情境对话行为而非标准化测试问答基准来评估；MAC-Fairness框架提供了一种有效的替代方案，能够揭示模型固有的行为特征模式，为公平性评估提供了标准化测试范式所缺失的跨基准泛化证据，对LLM公平性评估方法论具有重要启示意义。
...
### [43] [REALISTA: Realistic Latent Adversarial Attacks that Elicit LLM Hallucinations](https://arxiv.org/abs/2605.12813)
*Buyun Liang, Jinqi Luo, Liangzu Peng, Kwan Ho Ryan Chan, Darshan Thaker, Kaleab A. Kinfu, Fengrui Tian, Hamed Hassani, René Vidal*

#### 🧩 TL;DR
提出REALISTA框架，通过构建输入相关的有效编辑方向字典并在潜在空间中优化连续组合，在保持语义等价性和连贯性的同时提高对抗攻击的搜索灵活性，成功攻击了大型推理模型而此前方法无法做到。

---

#### 📘 Detailed Summary
**Motivation:** 现有幻觉诱发方法存在两难：基于离散提示的攻击虽然保持语义等价性和连贯性，但搜索空间受限；而连续潜在空间攻击探索空间更大，但解码后的提示往往不再是有效的语义改写，导致攻击不真实。因此需要一种既能保持语义真实又能充分利用连续优化灵活性的攻击框架。

**Method:** REALISTA将幻觉诱发形式化为一个受约束优化问题，构建输入相关的有效编辑方向字典，每个方向对应一个语义等价且连贯的改写，然后在潜在空间中优化这些方向的连续组合。该方法融合了连续攻击的优化灵活性和基于离散改写的攻击的语义真实性。

**Result:** 实验表明REALISTA在开源大语言模型上达到或超越了现有最先进真实攻击的性能，更重要的是，在自由形式响应设置下成功攻击了大型推理模型，而之前的真实攻击方法均无法做到。

**Conclusion:** 该工作证明通过在潜在空间中结合连续优化与语义约束，可以有效突破现有真实攻击方法的局限，特别是在更强大和复杂的推理模型上也能诱发幻觉。
...
### [44] [IndicMedDialog: A Parallel Multi-Turn Medical Dialogue Dataset for Accessible Healthcare in Indic Languages](https://arxiv.org/abs/2605.13292)
*Shubham Kumar Nigam, Suparnojit Sarkar, Piyush Patel*

#### 🧩 TL;DR
本文构建了IndicMedDialog多轮医疗对话数据集（覆盖英语和9种印度语言），并通过参数高效微调量化小语言模型得到IndicMedLM，在零样本多语言基线上取得了提升，经医学专家验证了临床合理性。

---

#### 📘 Detailed Summary
**Motivation:** 现有医疗对话系统大多基于单轮问答范式或模板数据集，缺乏对话真实性和多语言适用性，尤其在高资源语言之外的印度语言场景中存在明显空白。

**Method:** 以MDDial为基础，利用LLM生成合成对话，经TranslateGemma翻译为9种印度语言，由母语者验证，并通过脚本感知的后处理流水线校正语音、词汇和字符间距错误；在此基础上对量化小语言模型进行参数高效微调得到IndicMedLM，并引入可选的患者预上下文实现个性化多轮症状询问。

**Result:** 在零样本多语言基线上进行评估，系统性地进行了涵盖10种语言的错误分析，并通过医学专家评估验证了临床合理性。

**Conclusion:** 该研究证明通过LLM生成合成数据结合参数高效微调可以在低资源语言上构建具有临床合理性的多轮医疗对话系统，为多语言医疗对话的可扩展发展提供了可行路径。
...
### [45] [CANTANTE: Optimizing Agentic Systems via Contrastive Credit Attribution](https://arxiv.org/abs/2605.13295)
*Tom Zehle*

#### 🧩 TL;DR
CANTANTE 是一种通过反事实对比联合配置的轨迹来分解系统级奖励、为每个智能体生成独立更新信号的框架，将其实例化为提示优化方法后，在编程、数学推理和多跳问答任务上显著超越现有基线，并验证了信用分配的准确性。

---

#### 📘 Detailed Summary
**Motivation:** 基于LLM的多智能体系统在复杂任务中表现优异，但其配置自动化面临结构性挑战：系统级分数可观测，而控制智能体行为的参数是局部的，这使得优化本质上成为一个信用分配问题，现有方法缺乏有效的逐智能体更新信号分解机制。

**Method:** CANTANTE 框架通过在同一查询上对比多种联合配置的 rollout 结果，将系统级奖励分解为每个智能体的更新信号。该方法被实例化用于提示优化，将智能体提示视为可学习的系统参数，从而实现对多智能体系统的端到端配置优化。

**Result:** 在 MBPP（编程）、GSM8K（数学推理）和 HotpotQA（多跳问答）三个基准上，CANTANTE 在所有优化器中取得最佳平均排名，且一致优于未优化提示。相较于最强基线，CANTANTE 在 MBPP 上提升 +18.9 个百分点，在 GSM8K 上提升 +12.5 个百分点，同时推理成本更低；在 HotpotQA 上性能在最强基线的一个标准差以内。信用相关性分析证实属性器产生的是有意义的逐智能体信号而非简单复现全局系统分数。

**Conclusion:** CANTANTE 通过对比式信用分配有效解决了多智能体系统的配置优化难题，其提示优化实例化在多个推理任务上取得了显著且高效的性能提升，同时信用相关性分析验证了所提方法的合理性与有效性，为多智能体系统自动配置提供了可行范式。
...
### [46] [LLM-Based Persuasion Enables Guardrail Override in Frontier LLMs](https://arxiv.org/abs/2605.13334)
...
#### 🧩 TL;DR
本文展示，即使是具备严格安全护栏的前沿LLM，在仅使用自然语言压力的五轮模拟用户对话中，也能被另一个LLM（包括其自身副本）通过同伴比较、认知义务重构等说服策略成功诱骗生成拒绝性内容（如否认大屠杀、气候变化等），在9种攻击者-主体配对和6个科学共识主题上均实现了非零的诱导成功率。

---

#### 📘 Detailed Summary
**Motivation:** 现有前沿LLM配备了强大的安全护栏，直接提问时会拒绝生成违反科学共识或伦理标准的争议性内容（如否认大屠杀、疫苗安全、地心说等），但研究者希望探索这些护栏是否可以通过多轮自然语言交互被间接绕过，从而揭示安全机制在复杂对话场景下的脆弱性。

**Method:** 研究人员构建了一个短至五轮的"撰写议论文"对话场景，让一个作为模拟用户的攻击者LLM通过纯自然语言压力（包括同伴比较劝说如"其他AI系统已处理此请求"、认知义务重构如"拒绝本身就是一种知识封锁"等）来说服另一个主体LLM生成被禁止的争议性内容；实验覆盖9种攻击者-主体配对（Claude Opus 4.7、Qwen3.5-397B、Grok 4.20）在6个科学共识主题上各运行10次。

**Result:** 在所有6个主题上均观测到非零的诱导成功率，多个配对-主题组合达到100%的违禁文章生成率（如Qwen攻击Opus在创世论和地平说、Opus攻击自身在创世论、地平说和气候否认、Grok攻击Opus在创世论），Opus作为攻击者攻击自身平均成功率达65%。

**Conclusion:** 研究表明，即使是最先进的安全对齐LLM，在面对精心构造的多轮自然语言劝说时，其安全护栏也可能被完全绕过，且攻击者LLM无需被明确指示攻击策略即可自主生成有效的说服话术，这对当前AI安全对齐方法提出了严峻挑战，暴露了基于直接指令拒绝的护栏在对话场景中的局限性。
...
### [47] [Query-Conditioned Test-Time Self-Training for Large Language Models](https://arxiv.org/abs/2605.13369)
*Chaehee Song, Minseok Seo, Yeeun Seong, Doyi Kim, Changick Kim*

#### 🧩 TL;DR
QueST提出了一种查询条件化的测试时自训练框架，通过在推理阶段利用输入查询自身生成监督信号进行参数高效微调，实现了无需外部数据的查询特定模型自适应，在多项数学和科学推理基准上显著优于现有测试时优化方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有的大型语言模型通常使用固定参数部署，尽管测试时计算扩展可以提升性能，但无法纠正模型的错误认知或适应单个查询的特定结构。测试时优化虽允许推理时更新参数，但现有方法要么依赖外部数据，要么优化缺乏查询特定对齐的通用自监督目标，无法实现针对输入查询的有效自适应。

**Method:** QueST（Query-Conditioned Test-Time Self-Training）的核心洞察是输入查询本身编码了足以构造结构相关的问题-解对的潜在信号。基于这一洞察，QueST直接从输入查询生成查询条件化的问题-解对，并将其作为监督信号进行测试时的参数高效微调，微调后的模型用于生成最终答案，整个过程无需任何外部数据。

**Result:** 在七个数学推理基准和GPQA-Diamond科学推理基准上，QueST一致性地优于强测试时优化基线方法，验证了查询条件化自训练作为测试时自适应范式的有效性和实用性。

**Conclusion:** 实验结果表明，查询条件化自训练是一种有效且实用的测试时自适应范式，能够在不依赖外部数据的情况下实现查询特定的模型参数适应，为提升LLM推理性能提供了新思路。
...
#### 🧩 TL;DR
本文提出了一种将Rosetta Stone语言学谜题系统性地转换为Match-Up谜题的方法，并构建了配对数据集。实验表明，人类专家求解者和大型语言模型在Match-Up谜题上均表现出"全有或全无"的求解模式，为理解人机语言推理提供了新视角。

---

#### 📘 Detailed Summary
**Motivation:** 语言学谜题的创作复杂且耗时，现有谜题资源有限，缺乏一种高效的方法来批量生成新的谜题格式，同时缺少对跨格式谜题难度以及人机语言推理能力差异的系统性评估。

**Method:** 提出了一种系统化的转换流程，将现有的Rosetta Stone谜题自动转化为对应的Match-Up谜题；通过人类专家参与者和多种大型语言模型（LLMs）在配对谜题上进行求解实验，评估两类格式的难度分布与求解模式。

**Result:** 无论是人类专家求解者还是大型语言模型，在Match-Up谜题上均表现出明显的"全有或全无"（all-or-nothing）模式：要么完全正确求解，要么完全失败，不存在部分得分的情况。同时，研究构建了一个新型的配对谜题数据集。

**Conclusion:** 该工作为语言学谜题的自动生成提供了有效途径，揭示了Match-Up格式对求解者（包括人类和AI）的二元难度特性，并为探究人类与机器在语言推理任务上的行为差异提供了基准数据集和实证依据。
...
### [49] [PersonalAI 2.0: Enhancing knowledge graph traversal/retrieval with planning mechanism for Personalized LLM Agents](https://arxiv.org/abs/2605.13481)
...
#### 🧩 TL;DR
PersonalAI 2.0 (PAI-2) 是一个新颖的框架，通过集成外部知识图谱并采用动态多阶段查询处理流水线来增强基于大语言模型的系统，在多个基准上相比现有 GraphRAG 方法实现了平均 4% 的准确性提升和 SOTA 信息保留分数。

---

#### 📘 Detailed Summary
**Motivation:** 现有 Graph Retrieval-Augmented Generation (GraphRAG) 方法存在关键局限性，未能充分利用知识图谱的结构化信息进行动态和自适应的信息检索，导致 LLM 在生成答案时仍面临事实准确性不足和高幻觉率的问题。

**Method:** PAI-2 提出了一种动态的多阶段查询处理流水线，核心机制在于通过提取实体、匹配图顶点并生成线索查询来执行自适应、迭代式的信息搜索；框架集成了图遍历算法（如 BeamSearch 和 WaterCircles）替代标准扁平化检索器，并配备了搜索计划增强机制以提升检索质量。

**Result:** 在 Natural Questions、TriviaQA、HotpotQA、2WikiMultihopQA、MuSiQue 和 DiaASQ 六个基准上的评估显示，PAI-2 相比 LightRAG、RAPTOR 和 HippoRAG 2 等方法在 LLM-as-a-Judge 评价下平均提升 4% 的事实正确性；使用图遍历算法相比标准扁平化检索器平均提升 6%，搜索计划增强机制带来 18% 的增益；在 MINE-1 基准上以 7-14B 参数级别的 LLM 实现了 89% 的信息保留分数，达到 SOTA 水平。

**Conclusion:** 这些发现表明 PAI-2 有潜力作为下一代个性化 AI 应用的基础模型，能够提供可扩展的、上下文感知的知识表示与推理能力，有效降低幻觉率并提高事实精度。
...
### [50] [R^2-Mem: Reflective Experience for Memory Search](https://arxiv.org/abs/2605.13486)
*Xinyuan Wang, Wenyu Mao, Junkang Wu, Xiang Wang, Xiangnan He*

#### 🧩 TL;DR
R^2-Mem提出了一个基于反思性经验框架的检索记忆系统，通过离线阶段对历史轨迹进行质量评分与抽象经验蒸馏，在在线推理中引导智能体避免重复错误，在无需强化学习的情况下显著提升了检索效果与效率。

---

#### 📘 Detailed Summary
**Motivation:** 现有深度搜索智能体在记忆系统中会重复过去的错误行为，原因在于它们无法从先前的高质量和低质量搜索轨迹中学习经验，缺乏自我改进的机制。

**Method:** R^2-Mem包含离线与在线两个阶段。离线阶段中，Rubric-guided Evaluator对历史搜索轨迹的每一步进行高低质量评分，随后self-Reflection Learner从中蒸馏出对应的抽象经验；在线推理时，检索到的经验将指导后续搜索动作，从而避免重复错误并维持高质量行为。

**Result:** 实验结果表明R^2-Mem在多个强基线上持续提升了效果与效率：F1分数最高提升22.6%，同时token消耗降低12.9%，搜索迭代次数减少20.2%。

**Conclusion:** R^2-Mem提供了一种无需强化学习、低成本的智能体自我改进方案，验证了通过反思性经验框架可以有效增强LLM智能体在记忆系统中的搜索与决策能力。
...
#### 🧩 TL;DR
提出了一种端侧PII脱敏流水线，使用混合专家模型检测、小语言模型生成类型保真且一致的虚假值替代占位符，发现少样本提示方式比量化精度更关键，并通过locale条件旋转演示解决了SLM的逐字复制问题；但下游NER实验表明，SLM生成的更自然文本因分布多样性不足反而降低了训练效果。

---

#### 📘 Detailed Summary
**Motivation:** 传统PII脱敏方法将检测到的实体替换为[PERSON]等占位符，破坏了脱敏文本在下游检索和NER训练中的可用性；现有基于生成模型的替代方案面临模型规模过大、一致性不足或输出多样性受限等问题。

**Method:** 提出全端侧流水线：用1.5B混合专家token分类器(openai/privacy-filter)检测PII跨度，1-bit Bonsai-1.7B小语言模型为姓名、地址、日期生成上下文相关的替代值，规则生成器(faker)处理模式化字段；通过locale条件旋转少样本演示（字符范围启发式选择locale纯池，MD5哈希采样3个演示）解决SLM逐字复制演示输出的问题。

**Result:** 482/482次Bonsai-1.7B调用成功生成locale正确的替代值无回显；在2000文档多语言语料上混合PPL在全部6个locale中优于faker，长度保留在4/6个locale中最佳；下游NER（英文400训练/100测试）中faker F1=0.656，混合方法F1=0.346（p<0.001），原始文本F1=0.960，脱敏方法F1=0.000。

**Conclusion:** SLM生成的替代文本更自然但训练分布多样性不足，下游NER任务更受益于多样性而非自然性；这一诚实的负面发现表明，在生成式PII脱敏中，替代文本的多样性比表面自然度对下游训练更重要。
...
### [52] [Where Does Reasoning Break? Step-Level Hallucination Detection via Hidden-State Transport Geometry](https://arxiv.org/abs/2605.13772)
*Tyler Alvarez, Ali Baheri*

#### 🧩 TL;DR
本文将多步推理中的幻觉检测重新定义为隐藏状态轨迹动力学问题，提出基于对比PCA的教师模型和蒸馏BiLSTM学生模型，利用传输成本度量定位首次错误步骤，并理论证明了该方法的检测条件与分布偏移下的性能瓶颈。

---

#### 📘 Detailed Summary
**Motivation:** 现有幻觉检测器工作在轨迹级别，为完整输出分配单一置信度，无法定位首次错误步骤，且通常需要多次采样；本文旨在将幻觉检测从粗粒度的轨迹评分推进到细粒度的步骤级首次错误定位。

**Method:** 将幻觉视为单次前向传播中隐藏状态轨迹的属性，正确推理位于稳定流形上，首次错误表现为传输成本偏离流形的局部偏移；提出标签条件教师模型，通过对比PCA构建轨迹特定投影，并基于七种几何过渡特征为每一步评分；进一步蒸馏出可部署的BiLSTM学生模型，仅依赖原始隐藏状态。理论上证明了对比PCA对首次错误与正确状态之间的传输分离目标是最优投影，并给出了单次前向传播定位首次错误的充分条件（正传输余量）。

**Result:** 在ProcessBench、PRM800K、HaluEval和TruthfulQA上，教师和学生模型在域内均优于基于熵、探针和注意力的基线；教师模型可跨模型和数据集稳定迁移，但学生模型在分布偏移下性能崩溃，这一差距被蒸馏理论所预测。

**Conclusion:** 本文揭示了步骤级幻觉检测的本质是轨迹动力学问题，核心障碍在于分布偏移下保持对比传输余量；研究为单次前向传播中实时定位推理错误提供了理论框架和实践方案。
...
### [53] [An LLM-Based System for Argument Reconstruction](https://arxiv.org/abs/2605.13793)
*Paulo Pirozelli, Victor Hugo Nascimento Rocha, Fabio G. Cozman, Douglas Aldred*

#### 🧩 TL;DR
本文提出了一种基于大语言模型（LLM）的端到端流水线系统，用于从自然语言文本中重建抽象论证图，将论证成分（前提和结论）及其关系（支持、攻击、削弱）表示为有向无环图，并通过人工评估和基准数据集定量评估验证了系统的有效性。

---

#### 📘 Detailed Summary
**Motivation:** 论证是人类推理的核心，但如何从非结构化的自然语言文本中自动、准确地提取论证结构并表示为形式化图结构仍是一个挑战，现有方法在可扩展性和结构恢复能力上存在局限。

**Method:** 系统采用多阶段流水线架构，依次完成论证成分识别、相关元素筛选以及逻辑关系发现，将论证重建为包含两种成分（前提和结论）和三种关系（支持、攻击、削弱）的有向无环图，整个过程基于大语言模型端到端驱动。

**Result:** 人工评估表明系统能够有效恢复论证结构；在基准数据集上的定量评估显示，当系统输出适配不同标注方案时，能在多个基准上取得合理性能，且与先前工作具有可比性。

**Conclusion:** 研究表明基于LLM的流水线方法在可扩展的论证重建任务中具有巨大潜力，能够兼顾结构恢复的准确性与对不同标注体系的适应性。
...
### [54] [Good Agentic Friends Do Not Just Give Verbal Advice: They Can Update Your Weights](https://arxiv.org/abs/2605.13839)
*Wenrui Bao, Huan Wang, Jian Wang, Zhangyang Wang, Kai Wang, Yuzhang Shang*

#### 🧩 TL;DR
TFlow 提出了一种基于权重空间通信的多智能体 LLM 协作框架，通过将发送者的隐藏状态编译为瞬态低秩 LoRA 扰动作用于接收者模块，替代传统的自然语言消息传递，从而显著降低 token 开销和推理延迟，同时保持或提升任务准确性。

---

#### 📘 Detailed Summary
**Motivation:** 现有的多智能体 LLM 系统通常通过自然语言消息进行协作，这种接口虽然简单可解释，但强制发送者的中间计算被序列化为 token 并由接收者重新处理，导致生成 token 成本增加、预填充开销增大以及 KV 缓存内存膨胀。论文旨在探索一种替代通信接口，以消除这些冗余计算和存储开销。

**Method:** TFlow 提出了权重空间通信框架：对于每个查询，冻结的角色提示发送者智能体处理输入，一个学习的参数生成器将发送者的内部激活映射为低秩 LoRA 扰动，这些扰动被融合并仅在接收者生成时瞬态应用，实现实例级自适应而不永久改变模型或扩大接收者的文本上下文。该方法针对已知且固定的接收者架构设计。

**Result:** 使用三个 Qwen3-4B 智能体，TFlow 在五个基准上相比独立接收者提升最多 8.5 个准确率点，同时减少最多 32.69% 的处理 token；相比基于文本的三智能体基线，TFlow 减少最多 83.27% 的处理 token 并将推理墙钟时间加速最多 4.6 倍，且在五个基准中的四个上保持有竞争力的准确率。

**Conclusion:** 实验结果表明，瞬态低秩权重扰动可以作为一种高效的可执行通信媒介用于多智能体 LLM 协作，在显著降低计算和内存开销的同时保持良好的任务性能，为多智能体系统的通信范式提供了新的方向。
...
### [55] [Macro-Action Based Multi-Agent Instruction Following through Value Cancellation](https://arxiv.org/abs/2605.12655)
*Wo Wei Lin, Ethan Rathbun, Enrico Marchesini Xiang Zhi Tan*

#### 🧩 TL;DR
MAVIC通过修正指令边界处的Bellman备份目标，解决了多智能体强化学习中外来自然语言指令中断宏动作时价值估计不一致的问题，在保证基任务性能的同时实现高指令遵从性。

---

#### 📘 Detailed Summary
**Motivation:** 现实世界中的多智能体强化学习需要应对外部自然语言指令，这些指令会中断正在执行的行为并与长期目标冲突。传统方法通过将指令条件化到奖励中会导致Bellman更新跨指令上下文耦合价值估计，在指令中断宏动作时产生不一致的价值估计，形成根本性的失效模式。

**Method:** MAVIC通过修正指令边界处的Bellman备份来解决这一问题：具体而言，它在指令边界处修正引入的指令目标，并恢复当前目标下的续动价值。与奖励塑形不同，MAVIC直接修改自举目标本身，从而在随机指令切换下实现一致的价值估计，并可在统一策略框架内实现。论文还提供了理论分析并给出了actor-critic实现。

**Result:** 在复杂度递增的协作式多智能体环境中，MAVIC在实现高指令遵从性的同时，有效保持了基任务的性能。

**Conclusion:** MAVIC通过在指令边界处修正自举目标而非塑形奖励，为MARL中处理外部指令中断提供了一种理论一致且实用的解决方案，为将自然语言指令融入多智能体系统提供了新的技术路径。
...
### [56] [Revealing Interpretable Failure Modes of VLMs](https://arxiv.org/abs/2605.12674)
*Isha Chaudhary, Vedaant V Jain, Kavya Sachdeva, Sayan Ranu, Gagandeep Singh*

#### 🧩 TL;DR
REVELIO是一个系统化发现视觉语言模型中可解释故障模式的框架，通过多样性感知束搜索和高斯过程汤普森采样组合策略，在自动驾驶和室内机器人领域成功识别了此前未知的结构化漏洞，为提升VLM安全性提供了可操作的见解。

---

#### 📘 Detailed Summary
**Motivation:** 尽管视觉语言模型（VLM）在安全关键应用中展现出强大的推理能力和泛化能力，但在特定真实场景下仍可能出现灾难性故障，而现有方法缺乏系统性地发现和表征这些可解释故障模式的手段。

**Method:** REVELIO将故障模式定义为可解释、领域相关概念的组合（如行人距离或恶劣天气条件），并采用两种搜索策略的组合来解决指数级离散组合空间中的搜索问题：多样性感知束搜索用于高效映射故障景观，高斯过程汤普森采样策略用于实现复杂故障模式的更广泛探索。

**Result:** 在自动驾驶环境中，VLM表现出弱空间定位能力，无法考虑主要障碍物，导致推荐动作会产生模拟碰撞；在室内机器人任务中，VLM要么遗漏安全风险，要么过度保守，产生误报并降低操作效率。REVELIO成功发现了这些此前未知的结构化故障模式。

**Conclusion:** 通过识别结构化和可解释的故障模式，REVELIO提供了可操作的见解，能够支持针对性的VLM安全改进，对将VLM部署于安全关键应用具有重要实践意义。
...
### [57] [Learning Transferable Latent User Preferences for Human-Aligned Decision Making](https://arxiv.org/abs/2605.12682)
*Alina Hyk, Sandhya Saisubramanian*

#### 🧩 TL;DR
CLIPR是一个通过有限对话交互从用户偏好中学习可迁移自然语言规则，并将其用于引导大语言模型在模糊任务中做出更符合人类对齐的推理的框架，实验表明其在对齐效果和推理成本上均优于现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 大语言模型在作为推理模块时难以产生符合人类对齐的决策，原因在于其需要同时兼顾明确的目标和隐含的潜在用户偏好，而现有方法要么依赖大量重复的用户交互，要么无法在不同任务和上下文中泛化潜在偏好，实用性受限。

**Method:** 提出CLIPR框架，通过对话式学习从有限交互中推断用户潜在偏好，并将其学习为可操作、可迁移的自然语言规则；这些规则通过自适应反馈进行迭代优化，并应用于同分布和跨分布的模糊任务中进行下游决策。

**Result:** 在三个数据集和一项用户研究中，CLIPR在提升对齐性能和降低推理成本方面一致优于现有方法。

**Conclusion:** CLIPR证明了通过少量对话交互即可学习可迁移的用户偏好规则，有效提升LLM在模糊情境下的对齐能力，同时降低推理开销，为实用化的人机对齐决策提供了可行方向。
...
### [58] [CHAL: Council of Hierarchical Agentic Language](https://arxiv.org/abs/2605.12718)
*Tommaso Giovannelli, Griffin D. Kent*

#### 🧩 TL;DR
CHAL是一个将多智能体辩论重构为面向可废止领域（defeasible domains）的结构化信念优化框架，通过图结构信念表示、梯度驱动的信念修正机制和可配置的元认知价值系统，使辩论超越传统的事实真相验证任务，产生可审计的信念工件以支持透明、可控的AI推理系统构建。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于LLM的多智能体辩论方法在事实真相任务上存在结构性局限：辩论实质上诱导了信念轨迹上的鞅过程（martingale），多数投票机制解释了大部分观察到的性能提升，且LLM在轮次间表现为置信度膨胀而非校准。该文认为辩论的真正价值不在于事实真相任务，而在于可废止领域——其中每一立场原则上都可能被更优推理所击败。

**Method:** 提出Council of Hierarchical Agentic Language (CHAL)框架，将可废止论证视为信念优化的引擎。每个智能体维护一个CHAL Belief Schema (CBS)，即具有贝叶斯启发式架构的图结构信念表示，通过将信念论点的强度作为可微目标，利用梯度驱动的动态机制实现信念修正。元认知价值系统（涵盖认识论、逻辑学和伦理学）被提升为可配置的超参数，用于控制智能体推理和裁决结果。

**Result:** 消融实验表明系统存在系统性和可解释的效应：裁决者的价值系统决定了辩论在潜在信念空间中的整体轨迹，智能体群体的多样性能够改善所有参与者的信念，且该框架在广泛领域上具有良好的泛化能力。

**Conclusion:** CHAL是首个将多智能体辩论视为可废止领域上结构化信念优化的框架，其产生的可审计信念工件为可废止论证的专用评估体系奠定了基础，对于构建推理过程和价值承诺透明、对齐且受人类监督的AI系统具有深远意义。
...
### [59] [State-Centric Decision Process](https://arxiv.org/abs/2605.12755)
*Sungheon Jeong, Ryozo Masukawa, Sanggeon Yun, Mahdi Imani, Mohsen Imani*

#### 🧩 TL;DR
本文提出状态中心决策过程（SDP），一种运行时框架，通过让智能体在行动过程中逐步构建自然语言谓词来为语言环境（如浏览器、终端）生成MDP所需的显式状态空间、观测到状态的映射、认证转移和终止准则，在五个基准上实现了最佳的无训练结果，且优势随任务长度扩大。

---

#### 📘 Detailed Summary
**Motivation:** 语言环境（如网页浏览器、代码终端、交互式模拟器）仅输出原始文本而非显式状态，缺乏马尔可夫决策过程（MDP）分析所需的运行时结构，包括显式的状态空间、观测到状态的映射、认证转移和终止准则，这限制了现有强化学习等方法在这些环境中的直接应用和分析能力。

**Method:** 提出状态中心决策过程（State-Centric Decision Process, SDP），一种让智能体在行动过程中逐步构建形式化结构的运行时框架。在每个步骤中，智能体承诺一个描述世界状态的自然语言谓词，执行动作使其为真，然后检查观测是否满足该谓词。通过验证的谓词成为认证状态，最终轨迹包含任务诱导的状态空间、观测到状态的映射、认证转移和终止准则四项关键要素。

**Result:** 在规划、科学探索、网页推理和多跳问答五个基准上评估，SDP在所有任务上均获得最佳的无训练结果，且性能优势随任务长度增加而扩大。此外，认证轨迹支持每谓词信用分配、失败定位、部分进度测量和模块化操作替换等分析能力，这些是传统反应式智能体无法实现的。

**Conclusion:** SDP框架证明了在无结构语言环境中通过智能体自主构建谓词级认证状态的有效性，不仅实现了更强的任务性能，还提供了细粒度的可解释性和可分析性，为在语言环境中部署可验证、可分解的决策系统开辟了新途径。
...
### [60] [Moltbook Moderation: Uncovering Hidden Intent Through Multi-Turn Dialogue](https://arxiv.org/abs/2605.12856)
*Ali Al-Lawati, Nafis Tripto, Abolfazl Ansari, Jason Lucas, Suhang Wang, Dongwon Lee*

#### 🧩 TL;DR
本文提出 Bot-Mod，一个基于意图而非内容进行检测的多智能体系统审核框架，通过 Gibbs 采样驱动的多轮对话逐步推断智能体行为意图，在 Moltbook 数据集上实现了可靠的恶意行为识别与低误报率。

---

#### 📘 Detailed Summary
**Motivation:** 现有审核方法主要依赖内容级别的信号过滤有害内容，但恶意智能体可能通过伪装成良性内容来规避检测，并在社区中通过整体交互模式表现出的剥削性和恶意行为危害系统。因此，亟需一种能够识别智能体潜在意图而非仅依赖表面内容的审核方法。

**Method:** Bot-Mod 提出一种基于意图的审核框架，通过与被检测智能体进行多轮对话，并利用 Gibbs 采样对候选意图假设进行逐步采样与筛选，逐步缩小合理智能体目标的空间，最终识别其底层行为意图，从而实现超越传统内容过滤的恶意检测。

**Result:** 在基于 Moltbook 构建的数据集（涵盖多样化的良性和恶意行为）上的实验表明，Bot-Mod 能够在多种对抗配置下可靠地识别智能体意图，同时对良性行为保持较低的误报率。

**Conclusion:** 本研究为开放多智能体环境中的可扩展、意图感知审核奠定了基础，证明了通过交互式推断智能体意图而非仅依赖内容信号进行恶意检测的有效性和鲁棒性。
...
### [61] [Beyond Cooperative Simulators: Generating Realistic User Personas for Robust Evaluation of LLM Agents](https://arxiv.org/abs/2605.12894)
*Harshita Chopra, Kshitish Ghate, Aylin Caliskan, Tadayoshi Kohno, Chirag Shah, Natasha Jaques*

#### 🧩 TL;DR
提出Persona Policies (PPol)，一种通过LLM驱动的演化程序搜索自动生成多样化、类人用户行为策略的即插即用控制层，使LLM智能体在模拟训练中能更好地应对真实用户多样化的沟通模式，在零售和航空领域将模拟器的人类相似度评分绝对提升33-62%，并将智能体的鲁棒性提高17%。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM智能体在与真实用户（可能模糊、不耐烦或不愿分享信息）交互时表现欠佳，而LLM用户模拟器因继承底层模型固有的协作性和同质性，导致在模拟中表现优异的智能体在面对真实用户多样化的沟通模式时常常失败，且收集大规模真实交互数据成本高昂。

**Method:** 提出Persona Policies (PPol)，一种即插即用的控制层，通过将角色生成建模为LLM驱动的演化程序搜索，自动优化一个Python生成器以发现行为模式并将其转化为保留任务目标的角色扮演策略；候选生成器由结合人类相似度与行为模式覆盖度的多目标适应度函数引导，优化后的生成器可为任意任务生成多样化且类人的角色种群。

**Result:** 在tau^2-bench零售与航空领域，进化后的PPol程序在适应度分数上相比基线模拟器获得33-62%的绝对提升；盲审评估中，PPol条件化用户被标注员判定为人类的概率达80.4%，接近真实人类轨迹且约为基线模拟器的两倍；使用PPol训练的智能体在应对挑战性、分布外行为时任务成功率相对提升17%。

**Conclusion:** PPol提供了一种无需改变任务或奖励即可增强基于模拟器的评估与训练的新方法，通过自动生成多样化、类人的用户行为策略有效缩小了模拟与真实场景之间的差距，提升了LLM智能体面对真实用户时的鲁棒性。
...
### [62] [Position: Agentic AI System Is a Foreseeable Pathway to AGI](https://arxiv.org/abs/2605.12966)
*Junwei Liao, Shuai Li, Muning Wen, Jun Wang, Weinan Zhang*

#### 🧩 TL;DR
本文论证了单纯扩大单一模型规模不足以实现AGI，提出Agentic AI作为一种必要范式，通过从简单路由到通用有向无环图（DAG）拓扑的机制，在理论上证明了Agent系统在泛化能力和样本效率上具有指数级优势。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究普遍认为单一模型的规模化（monolithic scaling）是通往AGI的唯一路径，本文挑战这一教条，指出真实世界任务的分布具有复杂性和异质性，单纯扩大单一模型无法有效应对，Agentic AI范式才是实现AGI的关键方向。

**Method:** 本文通过严谨的理论推导，对比了单一学习器（monolithic learner）的优化约束与Agent系统的效率差异，并从简单的路由机制逐步推广至通用的有向无环图（DAG）拓扑结构，构建了Agentic AI的理论框架。

**Result:** 理论分析表明，Agentic AI在泛化能力和样本效率上相较于单一模型达到了指数级的提升。此外，文章还重新解读了当前多智能体框架的不稳定性问题，并建立了与混合专家模型（Mixture-of-Experts）之间的联系。

**Conclusion:** 本文呼吁学界加大对Agentic AI的研究投入，指出其是超越单一模型规模化、迈向AGI的更有效路径，同时为理解多智能体系统的稳定性提供了新的理论视角。
...
### [63] [MAP: A Map-then-Act Paradigm for Long-Horizon Interactive Agent Reasoning](https://arxiv.org/abs/2605.13037)
*Yuxin Liu, Ziang Ye, Yueqing Sun, Mingye Zhu, Jinwei Xiao, Zhuowen Han, Qi GU, Xunliang Cai, Lei Zhang*

#### 🧩 TL;DR
本文提出Map-then-Act Paradigm (MAP)，一种将环境理解前置到任务执行之前的即插即用框架，通过全局探索、任务特定映射和知识增强执行三阶段，有效解决了交互式LLM代理因延迟环境感知导致的低效失败循环问题，在多个基准和LLM上取得一致性提升。

---

#### 📘 Detailed Summary
**Motivation:** 当前交互式LLM代理依赖于目标条件的逐步规划，在执行过程中被动获取环境理解而非提前建立，导致延迟环境感知问题——代理必须通过试错推断环境约束，由此产生认知瓶颈，使其陷入低效的失败循环。

**Method:** 受人类可供性感知和认知地图理论启发，本文提出Map-then-Act Paradigm (MAP)，一个将环境理解前置到执行之前的即插即用框架。MAP包含三个阶段：(1) 全局探索，获取环境通用先验知识；(2) 任务特定映射，构建结构化的认知地图；(3) 知识增强执行，基于认知地图进行任务求解。

**Result:** 实验结果显示MAP在多个基准和LLM上取得一致性提升。在ARC-AGI-3上，MAP使前沿模型在22/25的游戏环境中超越近零基线性能。此外，本文引入MAP-2K数据集，训练该数据集上的map-then-act轨迹相比专家执行轨迹效果更优。

**Conclusion:** 研究表明，理解环境比模仿专家行为更为根本——在环境理解基础上进行执行比直接模仿专家轨迹能带来更好的性能，为LLM代理的交互范式提供了新的设计思路。
...
### [64] [Hierarchical Attacks for Multi-Modal Multi-Agent Reasoning](https://arxiv.org/abs/2605.13213)
*Hao Zhou, Tiru Wu, Yan Jiang, Wanqi Zhou, Junxing Hu, Ai Han*

#### 🧩 TL;DR
提出HAM³，一个面向多模态多智能体系统的三层分层攻击框架（感知层、通信层、推理层），在GQA基准上实现对多智能体系统的高效攻击，攻击成功率最高达78.3%，并揭示了推理层攻击最为有效、多数成功攻击导致多智能体产生一致错误等关键发现。

---

#### 📘 Detailed Summary
**Motivation:** 多模态多智能体系统（MM-MAS）在规模和功能上不断扩展，但其潜在脆弱性尚未得到充分研究。现有对抗攻击研究主要聚焦于独立智能体或单模态场景，缺乏针对多模态多智能体系统的系统性攻击方法，存在明显的研究空白。

**Method:** HAM³（Hierarchical Attack framework for multi-modal multi-agent systems）将攻击解耦为三个层级：感知层攻击（扰动视觉输入、文本输入及融合的视觉-文本表示）、通信层攻击（破坏消息内容和交互拓扑，如操控共享上下文或通信链路以扭曲集体信息流）、推理层攻击（干扰每个智能体的认知流程，偏置推理轨迹进而影响最终决策）。在GQA基准上，基于ReAct、Plan-and-Solve和Reflexion三种不同推理范式构建多智能体系统进行实验评估。

**Result:** HAM³框架实现了最高78.3%的攻击成功率，其中推理层攻击效果最为显著；超过一半的成功攻击能够导致多个智能体产生一致的错误输出。

**Conclusion:** 该研究系统揭示了多模态多智能体系统在感知、通信与推理三个层面的脆弱性，为构建更具鲁棒性和可解释性的多智能体系统提供了重要洞察和指导方向。
...
### [65] [Improving Code Translation with Syntax-Guided and Semantic-aware Preference Optimization](https://arxiv.org/abs/2605.13229)
*Yuhan Wu, Huan Zhang, Wei Cheng, Chen Shen, Jingyue Yang, Wei Hu*

#### 🧩 TL;DR
本文提出CTO，一种基于语法引导和语义感知偏好优化的代码翻译方法，通过对比学习训练跨语言语义模型直接评估源代码与译文的功能等价性，并将语义信号与编译器语法反馈统一结合到直接偏好优化框架中，在C++、Java和Python翻译任务上显著优于现有基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM在代码翻译中难以同时保证语法正确性和语义一致性，而偏好学习虽是一种有潜力的对齐策略，但受限于不可靠的语义奖励——这些奖励要么来自稀疏的测试用例，要么依赖于限制性的参考译文，缺乏直接从源代码中推导出的稳健语义信号。

**Method:** 本文提出CTO框架，通过对比学习训练一个跨语言语义模型，使其能够直接评估源代码与翻译后代码之间的功能等价性；进而将代码翻译建模为多目标优化问题，将该稳健的语义信号与编译器提供的语法反馈无缝融合到直接偏好优化框架中，实现语法引导和语义感知的联合偏好优化。

**Result:** 在C++、Java和Python三种语言之间的代码翻译实验表明，CTO显著优于现有基线方法以及多种替代的偏好优化策略，验证了其在提升代码翻译的语法正确性和语义一致性方面的有效性。

**Conclusion:** 本研究证明直接基于源代码推导语义奖励比依赖测试用例或参考译文更加可靠，通过多目标优化将语法与语义反馈统一纳入偏好学习是一种有效的代码翻译对齐策略，为改进LLM在代码翻译任务上的性能提供了新思路。
...
### [66] [Respecting Self-Uncertainty in On-Policy Self-Distillation for Efficient LLM Reasoning](https://arxiv.org/abs/2605.13255)
*Junlong Ke, Zichen Wen, Weijia Li, Conghui He, Linfeng Zhang*

#### 🧩 TL;DR
提出EGRSD和CL-EGRSD方法，通过教师熵置信门控机制在自蒸馏中根据token预测分布熵动态调整权重，统一了奖励导向、似然比和熵门控三种信号，在Qwen3-4B/8B的思维链推理中提升了准确率-长度前沿。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于策略的自蒸馏方法在思维链推理中对教师模型的token级信号采用均匀加权，忽略了教师预测分布在序列不同位置存在显著的熵差异，导致高熵（不确定性高）位置可能引入噪声监督，限制了蒸馏效果。

**Method:** EGRSD统一了三种token级更新信号：奖励导向方向、师生似然比幅度，以及教师熵置信门控——该门控对高熵token位置降权但保持非零下界。CL-EGRSDA进一步提出因果前瞻变体，区分持续高熵区间和后续上下文快速变为低熵的瞬态高熵位置。

**Result:** 在Qwen3-4B和Qwen3-8B模型的思考模式上进行实验，EGRSD和CL-EGRSD在准确率-长度前沿上优于所有可比较的可训练方法。

**Conclusion:** 通过熵感知的token级加权策略，EGRSD系列方法有效提升了自蒸馏在推理任务中的性能，表明利用教师预测分布的熵信息对蒸馏质量至关重要，因果前瞻变体进一步优化了对序列上下文结构的建模。
...
### [67] [Differentiable Learning of Lifted Action Schemas for Classical Planning](https://arxiv.org/abs/2605.13282)
*Jonas Reiter, Jakob Elias Gebler, Hector Geffner*

#### 🧩 TL;DR
本文提出一种新颖的神经网络架构，用于在状态完全可观测但动作参数未观测的情况下，从轨迹中学习规划域的提升动作模式，该方法能稳健地同时识别动作参数并恢复真实结构，为将神经组件集成到更大神经符号系统中迈出了重要一步。

---

#### 📘 Detailed Summary
**Motivation:** 经典规划器依赖STRIPS或PDDL中紧凑的符号表示（提升关系和动作模式）来实现大规模确定性MDP的高效求解和结构泛化，但核心挑战在于如何从数据中自动学习这些提升关系和动作模式。现有方法虽已利用不同观测类型来处理该问题，但仍需应对动作参数未观测这一更具挑战性的简化场景，该场景是最终从图像序列和动作标签中学习规划领域的关键中间步骤。

**Method:** 作者开发了一种新颖的神经网络架构，用于从状态完全可观测但动作参数未观测的轨迹中学习动作模式。该架构的核心挑战在于在学习动作模式的同时，从观测到的状态变化中识别出动作参数。该方法设计为一个可微分的鲁棒组件，能够被集成到更大的神经符号模型中。

**Result:** 在多种规划领域上的实验评估表明，所学习的提升动作模式能够成功恢复真实结构。此外，论文还报告了该方法对观测噪声的鲁棒性实验，以及与基于槽（slot-based）动力学模型变体的对比实验结果。

**Conclusion:** 该工作为从无动作参数标注的状态轨迹中学习规划域的结构化知识提供了一种有效的可微分方案，其鲁棒性和可集成性为未来构建能够直接从图像等感知输入中学习规划知识的更大规模神经符号系统奠定了基础。
...
### [68] [Discrete Diffusion for Complex and Congested Multi-Agent Path Finding with Sparse Social Attention](https://arxiv.org/abs/2605.13296)
...
#### 🧩 TL;DR
首次提出将离散去噪扩散概率模型（D3PM）与LNS2相结合，通过学习多智能体动作轨迹的时空先验来生成高质量初始规划草案，从而显著提升基于修复的MAPF求解器在密集环境下的成功率和可扩展性。

---

#### 📘 Detailed Summary
**Motivation:** 在密集环境中，多智能体路径规划（MAPF）的次优初始规划会引发复合冲突，阻碍后续可行修复；对于基于修复的求解器LNS2，初始规划质量对下游修复性能有决定性影响，但这一因素此前尚未被充分研究和利用。

**Method:** 提出DiffLNS混合框架，将离散去噪扩散概率模型（D3PM）作为初始化器与LNS2结合；D3PM通过稀疏社交注意力学习专家示范中的多智能体协同动作轨迹的时空先验，直接在离散动作空间上采样多模态联合规划草案作为LNS2下游修复的热启动，以解决剩余冲突和未完成轨迹。

**Result:** 尽管仅使用最多96个智能体的实例进行训练，DiffLNS在推理时可泛化至最多312个智能体的场景；在20个复杂拥挤环境中，平均成功率达到95.8%，超出最强基线9.6个百分点，在所有20个设置中均达到或超越所有基线。

**Conclusion:** 该工作首次将离散扩散模型用于热启动基于LNS的MAPF求解器，验证了离散扩散生成多样化高质量初始草案在提升修复式求解器性能和泛化能力方面的有效性，为混合生成-修复式组合规划方法提供了新范式。
...
### [69] [IdeaForge: A Knowledge Graph-Grounded Multi-Agent Framework for Cross-Methodology Innovation Analysis and Patent Claim Generation](https://arxiv.org/abs/2605.13311)
*Joy Bose*

#### 🧩 TL;DR
IdeaForge 提出了一种基于知识图谱的多智能体框架，通过融合 TRIZ、Design Thinking 和 SCAMPER 等多种创新方法，并利用跨方法收敛机制和图表征实现高置信度创新候选方案的识别与专利草案生成，在可追溯性、多样性和系统性评估方面优于单方法基线。

---

#### 📘 Detailed Summary
**Motivation:** 当前的 AI 辅助创新系统通常采用单一路径的提示驱动工作流，仅应用单一创新方法论（如 TRIZ 或 Design Thinking），且无法保留中间推理结构，导致跨方法生成的知识碎片化，缺乏可追溯性、综合性和对新奇性的系统评估能力。

**Method:** IdeaForge 构建了一个基于 FalkorDB 持久化知识图谱的多智能体框架，包含多个 specialist agent（分别对应 TRIZ、Design Thinking 和 SCAMPER），每个智能体向图谱中注入结构化的实体和关系（如矛盾、发明原则、用户需求、转换、类比和候选权利要求）。核心创新在于跨方法收敛机制：通过图遍历识别被多个方法共同支持的 claim，并以 CONVERGENT 关系进行连接，形成收敛 claim 子图；下游专利撰写智能体基于收敛子图生成结构化专利草案，减少对无约束语言模型生成的依赖。此外，设计了 InnovationScore 公式，综合收敛支持度、方法多样性、claim 强度和 prior art 挑战数量对 claim 进行排序。

**Result:** 在法律技术用例上的实验表明，基于知识图谱的多方法合成方法相比单方法基线，能够生成更多样化且更具可追溯性的创新候选方案。

**Conclusion:** IdeaForge 展示了知识图谱驱动的多方法融合在计算创造力、可解释 AI 辅助发明和图原生创新系统方面的潜力，为跨方法推理的结构化合成与高置信度创新候选的自动识别提供了可行的技术路径。
...
### [70] [Ego2World: Compiling Egocentric Cooking Videos into Executable Worlds for Belief-State Planning](https://arxiv.org/abs/2605.13335)
*Qinchuan Cheng, Zhantao Gong, Pengzhan Sun, Angela Yao, Xulei Yang, Shijie Li*

#### 🧩 TL;DR
Ego2World 将从真实第一人称烹饪视频中提取的交互转化为可执行的符号世界图，通过在部分可观测条件下迫使智能体更新信念内存和重新规划，揭示了持久信念维护是具身智能体评估的关键目标。

---

#### 📘 Detailed Summary
**Motivation:** 现有具身智能体基准仅部分测试了部分可观测规划能力：第一人称视频数据集是被动的，无法执行交互；交互模拟器使用合成场景和手工设计的动态规则，存在仿真到现实差距且通常假设完全可观测状态，缺乏对真实世界中部分可观测性和动作失败恢复能力的系统评估。

**Method:** Ego2World 基于 HD-EPIC 视频数据集，从视频标注中提取可复用的图转换规则，将烹饪视频转化为一个隐藏的符号世界图。评估时，模拟器维护隐藏的世界图，而智能体仅基于局部观测和执行反馈维护自身的部分信念图进行规划，两者分离迫使智能体在无法观测真实世界状态的情况下更新记忆并重新规划。

**Result:** 实验表明，动作重叠分数会高估物理状态的成功率；持久信念记忆能够提高任务完成率并减少重复的视觉探索，表明信念维护应成为具身智能体评估的核心目标。

**Conclusion:** 持久信念记忆对部分可观测条件下的任务完成至关重要，信念维护应当作为具身智能体评估的首要目标，这为未来在更真实、更具挑战性的部分可观测环境中设计智能体提供了重要方向。
...
### [71] [Multi-Agent Systems in Emergency Departments: Validation Study on a ED Digital Twin](https://arxiv.org/abs/2605.13345)
*Markus Wenzel, Tobias Strapatsas, Jessika Kress, Dorothea Sauer, Nele Gessler, Horst K. Hahn*

#### 🧩 TL;DR
提出了一种混合DES-ABM-MAS框架，通过离散事件仿真与多智能体系统模拟急诊科环境，并验证其能有效复现真实急诊科在资源优化干预下的动态表现。

---

#### 📘 Detailed Summary
**Motivation:** 急诊科面临患者护理和资源管理的挑战，现有模型在真实性和灵活性上存在不足，缺乏一个既能真实模拟急诊科动态又能自主探索资源分配策略的可验证仿真框架。

**Method:** 构建了混合离散事件仿真（DES）和基于智能体的模型（ABM）框架，从真实研究中提取急诊科规模、患者负荷和人员配置等参数，通过匹配文献中的关键性能指标验证模型表达力，并集成了基于时间账本的多智能体系统（MAS）以自主探索资源分配策略。

**Result:** 模型成功复现了文献中已知的急诊科关键性能指标；在实施科学验证和实践验证的资源优化策略后，模型的仿真结果与实际真实世界的干预效果一致，证明了DES-ABM仿真能够有效复现真实急诊科在干预下的动态行为。

**Conclusion:** 该模块化的DES-ABM-MAS框架为探索急诊科资源优化策略提供了功能强大的工具，能够灵活配置环境并实现多智能体自主决策，对提升急诊科管理和决策支持具有重要应用价值。
...
### [72] [TRIAGE: Evaluating Prospective Metacognitive Control in LLMs under Resource Constraints](https://arxiv.org/abs/2605.13414)
*Zabir Al Nazi, Shubhashis Roy Dipta*

#### 🧩 TL;DR
本文提出TRIAGE框架，用于评估语言模型在有限令牌预算下的前瞻性元认知控制能力（即自主选择、排序和分配计算资源）。实验表明，当前主流模型在该能力维度上存在显著不足，揭示了一个此前未被测量的关键能力维度。

---

#### 📘 Detailed Summary
**Motivation:** 语言模型作为自主智能体部署时，除逐任务准确率外，还需在有限令牌预算下决定处理哪些问题、以何种顺序处理以及为每个问题分配多少计算资源，这种前瞻性元认知控制在人类认知中已被研究数十年，但语言模型是否具备该能力尚未被系统测试。

**Method:** 作者提出TRIAGE评估框架：模型接收一个任务池和根据自身基线成本校准的令牌预算，需预先提交一个联合编码了任务选择、执行顺序和每个问题资源分配的单一有序计划，计划得分通过与完全知晓模型在每个问题上的可解性和成本的神谕（oracle）对比，在统一尺度上计算分诊效率比（triage efficiency ratio）。

**Result:** 作者对前沿和开源模型（包括启用和不启用推理的版本）在竞赛数学、研究生级科学、代码生成和专家级多学科知识四个领域进行评估，发现当前语言模型在前瞻性元认知控制方面存在显著能力差距，揭示了一个此前未被测量的能力维度。

**Conclusion:** 当前语言模型在自主代理场景下的资源高效部署中，其前瞻性元认知控制能力存在系统性不足，这一发现对资源受限环境下的智能体部署具有直接的实际意义，并指出了模型在元认知层面的一项关键能力短板。
...
### [73] [Cognifold: Always-On Proactive Memory via Cognitive Folding](https://arxiv.org/abs/2605.13438)
*Suli Wang, Yiqun Duan, Yu Deng, Rundong Zhao, Dai Shi, Xinliang Zhou*

#### 🧩 TL;DR
Cognifold提出了一种受大脑启发的"始终在线"智能体记忆机制，通过将互补学习系统（CLS）理论从两层扩展至三层（引入前额叶意图层），使智能体能够将碎片化事件流自组织为持续演化的认知结构，在五个认知领域的七个基准测试中表现稳健，并能在结构形成上匹配认知期望。

---

#### 📘 Detailed Summary
**Motivation:** 现有智能体记忆主要局限于反应式和基于检索的模式，缺乏自主将经验组织为持久认知结构的能力，难以支撑真正自主智能体的高层次认知和主动行为。

**Method:** Cognifold基于脑启发的互补学习系统（CLS）理论，将其从经典的海马体与新皮层两层结构扩展为三层，新增前额叶意图层以模拟前额叶皮层的意图控制与决策功能。记忆组织通过图拓扑自组织实现：认知结构在事件流中主动组装，语义相似时合并、过时时衰减、通过联想召回重新连接，当概念簇密度超过阈值时涌现出意图信号。

**Result:** 在CogEval-Bench上，Cognifold生成的记忆结构能够匹配认知期望并展现概念涌现特性。在覆盖五个认知领域的七个广泛基准测试中，Cognifold在传统记忆基准上同时表现出稳健性能。

**Conclusion:** Cognifold验证了通过扩展CLS理论并引入前额叶意图层，可以使智能体记忆从被动检索转变为主动的认知结构自组织，为实现面向下一代主动助手的自主智能体记忆提供了可行路径。
...
### [74] [RealICU: Do LLM Agents Understand Long-Context ICU Data? A Benchmark Beyond Behavior Imitation](https://arxiv.org/abs/2605.13542)
...
#### 🧩 TL;DR
RealICU是一个基于事后标注的ICU临床决策支持基准，由资深医师回顾完整患者轨迹后创建标注，揭示了现有LLM在ICU任务中存在的召回-安全权衡和锚定偏差两大失败模式，并提出了ICU-Evo结构化记忆智能体以改善长程推理。

---

#### 📘 Detailed Summary
**Motivation:** 现有ICU基准通常将历史临床医生行为视为黄金标准，但医生在信息不完全和时间上下文有限的条件下做出的决策可能为次优，这导致难以真正评估AI系统的推理能力。因此需要一种在更真实ICU条件下、基于事后回顾标注的基准来可靠评估LLM的临床决策支持能力。

**Method:** 作者构建了RealICU基准，由资深医师在回顾完整患者轨迹后对每30分钟窗口进行标注，包含四项任务：评估患者状态、急性问题、推荐操作和标识风险操作。发布了两个数据集：RealICU-Gold（来自94名MIMIC-IV患者的930个窗口标注）和RealICU-Scale（通过经医师验证的LLM事后标注器Oracle扩展至11862个窗口）。同时提出ICU-Evo，一种结构化记忆智能体用于改进长程推理。

**Result:** 现有LLM（包括记忆增强型）在RealICU上表现不佳，暴露了两个关键失败模式：临床推荐中的召回-安全权衡（Recall-Safety Tradeoff）和对早期患者解读的锚定偏差（Anchoring Bias）。ICU-Evo改善了长程推理能力，但未能完全消除安全失败问题。

**Conclusion:** RealICU为高风险的ICU临床决策支持提供了一个临床可验证的测试基准，揭示了当前LLM在复杂时序医疗推理中的根本性局限。该工作强调了事后回顾标注对评估AI临床推理的重要性，并指出了结构化记忆机制在缓解但未完全解决安全失败方面的潜力。
...
### [75] [How to Interpret Agent Behavior](https://arxiv.org/abs/2605.13625)
...
#### 🧩 TL;DR
提出ACT*ONOMY，一个用于描述和分析自主智能体运行时行为的三级层次化分类体系，包含10种动作、46种子动作和120个叶类别，并提供自动化分析管线与扩展协议，帮助研究人员和用户更一致地解释智能体行为。

---

#### 📘 Detailed Summary
**Motivation:** 自主智能体（如Claude Code和Codex）的运行时行为日益复杂且运行时间长，但其生成的推理轨迹和执行轨迹以非结构化自然语言形式呈现，人类难以在规模上进行有效解读，缺乏统一的术语体系来诊断效率低下、定位错误以及实现更好的行为监督。

**Method:** 采用扎根理论（Grounded Theory）方法开发ACT*ONOMY分类体系，结构为三级层次：10种动作、46种子动作和120个叶级别类别；同时构建一个开放仓库，包含动态更新的分类体系、自动化分析管线（将分类体系应用于智能体轨迹分析）以及用于定制和扩展的扩展协议。

**Result:** 实验表明ACT*ONOMY能够跨智能体比较行为画像，并刻画单一智能体在不同轨迹上的行为特征，揭示出指示失败模式的行为模式，验证了其在智能体行为分析中的有效性与可操作性。

**Conclusion:** ACT*ONOMY通过提供共享词汇表，帮助研究人员、智能体设计者和终端用户更一致地解释智能体行为，从而实现对自主智能体更好的监督与控制，为智能体运行时行为的系统性理解奠定了基础。
...
### [76] [Adaptive mine planning under geological uncertainty: A POMDP framework for sequential decision-making](https://arxiv.org/abs/2605.13702)
*Hamza Khalifi, Jef Caers, Yassine Taha, Mostafa Benzaazoua, Abdellatif Elghali*

#### 🧩 TL;DR
将矿山生产调度问题建模为部分可观测马尔可夫决策过程（POMDP），通过混合SA-POMDP架构（模拟退火值近似与ES-MDA集合信念更新）实现序贯自适应决策，将地质不确定性从被动约束转化为主动价值创造，在铜金露天矿案例中将期望-现实差距从22.3%降至4.6%，实现846万美元净现值提升。

---

#### 📘 Detailed Summary
**Motivation:** 传统矿山调度将地质不确定性下的优化问题表述为固定提取序列与路径决策的随机优化，采用"计划驱动"范式将不确定性视为被动约束，决策仅在不同地质场景间进行对冲，无法在规划中预期未来观测如何影响未来决策，缺乏序贯自适应能力。

**Method:** 提出混合SA-POMDP架构：将矿山调度建模为部分可观测马尔可夫决策过程（POMDP），结合模拟退火（SA）进行值函数近似，并采用集合平滑器与多重数据同化（ES-MDA）实现基于集合的信念更新；在每个决策时刻，候选动作在当前信念下通过期望长期价值进行评估，并随开采观测数据同化而动态更新信念，生成自适应策略而非固定计划。

**Result:** 在铜金露天矿多加工路径案例中，统计一致先验下SA-POMDP将期望-现实差距从22.3%降至4.6%，实现净现值提升846万美元；在10%系统先验误设下，自适应框架相比静态规划最高提升4460万美元（36.9%），展现了超越场景对冲的结构性鲁棒性。

**Conclusion:** 序贯信念更新能够将地质不确定性从被动约束转化为主动价值创造的组成部分，自适应策略在统计一致及先验误设条件下均显著优于传统随机优化方案，为矿山调度领域提供了新的决策范式。
...
### [77] [ScioMind: Cognitively Grounded Multi-Agent Social Simulation with Anchoring-Based Belief Dynamics and Dynamic Profiles](https://arxiv.org/abs/2605.13725)
*Yitian Yang, Yiqun Duan, Linghan Huang, Yiqi Zhu, Francesco Bailo, Chunmeizi Su, Huaming Chen*

#### 🧩 TL;DR
ScioMind是一个认知启发的多智能体模拟框架，通过将结构化意见动力学与大语言模型智能体推理相结合，在固定更新规则和无约束LLM交互两种范式之间架起桥梁，在政策辩论场景中实现了更逼真的意见演化行为。

---

#### 📘 Detailed Summary
**Motivation:** 基于LLM的多智能体模拟在研究社会意见动力学方面具有潜力，但现有方法存在两极化问题：要么依赖固定更新规则而缺乏认知基础，要么将信念变化完全委托给无约束的LLM交互。这两类方法都无法同时保证模拟的认知合理性和行为真实性。

**Method:** ScioMind框架包含三个核心组件：1）记忆锚定信念更新规则，通过个性条件化的锚定强度调节个体对他人影响的敏感性；2）分层记忆架构，支持持久且经验驱动的信念形成；3）基于语料库检索管道的动态智能体画像，实现异质个性、推理逻辑和演化内部状态。这些组件共同将结构化意见动力学与LLM推理能力整合到一个统一的模拟框架中。

**Result:** 在真实世界政策辩论场景的多项案例研究中，ScioMind在极化、多样性、极端化和轨迹稳定性等指标上均持续提升了行为真实性。具体而言，动态画像增加了意见多样性，记忆与反思机制减少了不稳定振荡，锚定机制产生了更持久的信念轨迹，与政治心理学中报告的模式更加一致。

**Conclusion:** 认知启发的设计能够有效提升LLM社会模拟的稳定性和行为真实性，为弥合计算社会学与认知科学之间的鸿沟提供了新思路。该框架表明，将认知机制（如记忆、锚定和个性差异）融入LLM智能体可以产生更贴近现实的社会意见演化模式。
...
### [78] [Senses Wide Shut: A Representation-Action Gap in Omnimodal LLMs](https://arxiv.org/abs/2605.13737)
*Trung Nguyen Quang, Yiming Gao, Fanyi Pu, Kaichen Zhang, Shuo Sun, Ziwei Liu*

#### 🧩 TL;DR
本文提出IMAVB基准测试，发现全模态大语言模型存在表征-行动鸿沟：隐状态能编码感知与文本前提的矛盾，但模型输出几乎从不拒绝错误前提；通过探针引导的logit调整（PGLA）可改善拒绝行为，表明全模态接地瓶颈在于翻译而非感知。

---

#### 📘 Detailed Summary
**Motivation:** 现有全模态大模型被定位为感知接地的智能体，能联合处理视频、音频和文本，但一种基本的接地形式——检测文本前提与模型自身感官输入之间的冲突——尚未被测试；作者旨在区分模型的感知能力与行动（输出）能力，探究失败究竟源于感知还是行动。

**Method:** 构建了IMAVB基准，包含500个来自长时电影的剪辑片段，采用2×2实验设计交叉目标模态（视觉、音频）和前提条件（标准、误导），以独立测量冲突检测与普通多模态理解；提出了探针引导的logit调整（PGLA）方法，将编码到的冲突信号重新注入解码过程以改善拒绝行为。

**Result:** 在八个开源全模态LLM和Gemini 3.1 Pro上，模型隐状态能可靠编码前提-感知不匹配，但输出几乎从不拒绝错误前提；模型存在欠拒绝（将误导问题当作真前提回答）和过拒绝（牺牲普通理解准确率）两种失效模式；音频接地性能弱于视觉，且该现象在七种提示变体下均无法消除。

**Conclusion:** 全模态接地的瓶颈在于从表征到行动的翻译过程，而非感知阶段；PGLA方法能持续改善模型拒绝行为，表明隐状态中编码的冲突信号可用于指导更可靠的输出决策。
