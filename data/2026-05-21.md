...
#### 🧩 TL;DR
该论文探索了使用视觉-语言模型（VLM）直接分析叠加了注视点的教育视频内容来检测学习者注意力，但实验发现所有 prompting 策略均未超越基于统计特征的经典机器学习基线，揭示了当前 VLM 在教育实时诊断中的局限性。

---

#### 📘 Detailed Summary
**Motivation:** 学习者在观看教育视频时注意力波动是影响信息保留的关键障碍，现有基于眼动追踪的注意力检测方法依赖于手工设计特征上的经典机器学习分类器，难以捕捉学习者参与度的复杂时序特性，预测性能有限。

**Method:** 研究利用一个包含70名被试的教育眼动追踪数据集，提出一种基于多模态基础模型的新方法：使用视觉-语言模型（VLM，Gemini 3）直接分析叠加了注视点的视频内容，通过多种提示工程策略尝试利用基础模型的语义推理能力来上下文化学习者的焦点。

**Result:** 实验结果表明，尽管采用了多种提示策略进行探索，所有基于VLM的方法在注意力检测性能上均未能超越基于统计特征的经典机器学习基线模型。

**Conclusion:** 该研究提供了关于VLM在实时教育诊断中应用局限性的新见解，表明尽管基础模型具备强大的语义理解能力，但在直接用于运行时注意力检测时仍面临显著挑战，未来需要探索更有效的融合或适应策略。
...
#### 🧩 TL;DR
本文提出一个三阶段框架，利用冻结的视觉编码器和少样本学习从临床仿真视频中提取行为时间线，发现识别准确率与能力评分呈负相关，表明该信号可作为自动化能力评估的补充信息。

---

#### 📘 Detailed Summary
**Motivation:** 临床模拟中的学习能力评估依赖专家观察，耗时、难以扩展且易受评分者间差异影响，现有视觉-语言模型虽能理解复杂行为但尚未被充分用于自动化的教育意义评估信号提取。

**Method:** 提出三阶段框架：(1) 利用冻结的DINOv2视觉主干和HMM Viterbi解码，通过少样本学习从自我中心视角的护理仿真视频中提取动作时间线；(2) 导出序列级特征和每会话识别指标；(3) 将这些特征与教师评分的能力等级建立关联分析。

**Result:** 在22个密集标注会话（3.8小时，493个动作）上，冻结DINOv2+ HMM Viterbi解码在留一法单样本识别中达到57.4% MOF；识别准确率与能力评分呈显著负相关（mIoU的rho=-0.524, p=0.012），该结果对六种混淆变量控制鲁棒；高能力学生展现出更多样化、更难以分类的工作流程，且更遵循协议一致的动作转换。

**Conclusion:** 识别准确率可作为能力评估中具有教学意义的补充信号，与预测的动作时间线相辅相成，为自动化临床能力评估提供了新的视角和可行方向。
...
#### 🧩 TL;DR
提出一种轻量级外观适配器，通过单张参考图像注入细粒度视觉特征实现可控且一致的动漫角色生成，无需大规模模型或逐主题微调。

---

#### 📘 Detailed Summary
**Motivation:** 现有可控角色生成方法要么依赖大规模视觉-语言模型导致计算开销大，要么需要对每个主题进行微调，缺乏轻量、模块化且即插即用的解决方案，同时难以在保持外观一致性的同时解耦空间姿态与角色外观。

**Method:** 利用CLIP涌现的局部空间化能力开发语义选择性局部注意力机制，将参考图像中的细粒度视觉特征注入扩散过程；引入姿态感知条件训练适配器，解耦角色外观与空间布局；适配器为预训练的轻量模块，部署时无需额外微调，兼容Stable Diffusion社区生态。

**Result:** 基于Danbooru整理的高质量动漫角色数据集在多类实际编辑场景下验证有效性，代码、模型权重与数据集将在论文接收后公开发布。

**Conclusion:** 提出一种紧凑、模块化且无需微调的适配器方案，有效平衡外观一致性与编辑灵活性，为动漫角色生成提供轻量级可控解决方案，开源资源将促进社区进一步应用与扩展。
...
#### 🧩 TL;DR
本文提出预训练域自适应扩散模型（PAD），从CT分割或数字体模生成的均匀器官活动图合成逼真的PET图像，采用两阶段训练策略和域适配模块，生成的图像在定量精度、噪声纹理、肿瘤分割和视觉感知上与真实PET图像高度一致。

---

#### 📘 Detailed Summary
**Motivation:** 传统基于物理的PET图像模拟方法计算成本高、解剖变异性有限，且难以捕获异质性PET摄取模式，制约了定量成像流程开发、虚拟成像试验及深度学习模型训练等应用。

**Method:** PAD模型采用自然图像预训练的文生图解码器，并添加上游条件编码器和下游PET域适配器；采用两阶段训练策略：第一阶段学习粗粒度摄取分布，第二阶段优化局部图像细节；将CT分割结果中各器官的平均SUV值赋值生成均匀器官活动图作为输入条件。

**Result:** PAD生成图像的器官平均SUV与目标PET图像的一致性相关系数（CCC）高于0.92；噪声水平与纹理特征与真实PET相似；肿瘤分割性能相当；在二选一强迫选择观察者研究中，四位读者的平均正确率约为50%，表明合成图像与真实图像在视觉上不可区分。

**Conclusion:** PAD提供了一种基于扩散模型的通用框架，能够从临床分割或数字体模衍生的均匀器官活动图生成具有临床相关异质性特征的PET图像，可广泛应用于数据增强和下游成像研究。
...
### [5] [Regulating Anatomy-Aware Rewards via Trajectory-Integral Feedback for Volumetric Computed Tomography Analysis](https://arxiv.org/abs/2605.20277)
*Tianwei Lin, Zhongwei Qiu, Jie Cao, Jiang Liu, Wenjie Yan, Bo Zhang, Yu Zhong, Wenqiao Zhang, Yingda Xia, Ling Zhang*

#### 🧩 TL;DR
本文提出CABS系统将放射学报告分解为可验证的临床语义单元，并在此基础上设计TIF-GRPO框架，通过引入控制理论中的积分反馈机制来调控解剖感知奖励，从而克服现有RL方法在3D CT分析中因优化语言流畅性而忽略临床事实的"评估幻觉"问题，显著提升了异常检测能力和临床忠实度。

---

#### 📘 Detailed Summary
**Motivation:** 当前医疗视觉语言模型在3D CT分析中部署受限，主要因为现有强化学习范式仍依赖词汇层面的代理信号，导致模型优化语言流畅性而非事实临床正确性，产生诊断关键性错误；这种"评估幻觉"与"机制性分歧"问题表明，表面相似性奖励会驱动策略梯度绕过医学事实，亟需一种能够直接对齐临床正确性的精细调控方法。

**Method:** 本文提出CABS系统，将放射学报告分解为可验证的临床语义单元以提供结构化基准；在此基础上提出TIF-GRPO框架，将临床推理建模为异常发现的伪时序轨迹，通过积分反馈回路调控解剖感知奖励：将持续遗漏惩罚为累积状态误差，将幻觉抑制为过度控制力，从而将控制理论原理融入策略优化过程。

**Result:** 在3D CT基准上的实验表明，TIF-GRPO显著提升了异常检测能力和临床忠实度，有效克服了传统RL中的评估幻觉和机制性分歧问题，建立了医疗VLM细粒度调控的新范式。

**Conclusion:** 本研究揭示了标准RL在医疗VLM中存在追求语言流畅性而偏离临床事实的根本缺陷，并通过引入控制理论视角提出了可行的解决方案；CABS与TIF-GRPO共同为医学影像分析中的事实对齐提供了新范式，其积分反馈机制为未来医疗AI的安全性和可靠性调控开辟了新方向。
...
### [6] [MedCRP-CL: Continual Medical Image Segmentation via Bayesian Nonparametric Semantic Modality Discovery](https://arxiv.org/abs/2605.20297)
*Ziyuan Gao*

#### 🧩 TL;DR
MedCRP-CL提出了一种基于中国餐馆过程（CRP）的在线任务结构发现与结构感知持续学习框架，用于医学图像分割，在16个分割任务上以仅4.1%的遗忘率达到73.3% Dice，比最优基线提升8.0%且参数量减少6倍。

---

#### 📘 Detailed Summary
**Motivation:** 医学图像分割中的持续学习面临核心挑战：数据来自异构来源顺序到达，而有效的持续学习需要发现哪些任务共享足够的结构以受益于联合学习。现有方法要么对所有任务施加统一约束导致任务冲突时产生灾难性遗忘，要么需要预定义任务分组而无法应对未来的任务多样性。

**Method:** MedCRP-CL利用中国餐馆过程（CRP）根据临床文本提示动态推断任务分组（称为语义模态），无需预定义聚类数量或访问未来任务。每个语义模态维护独立的LoRA适配器，并通过组内EWC（弹性权重巩固）正则化，实现相异任务组间的参数隔离和相似组内的知识迁移。该方法无需数据回放，仅存储聚合统计量而非原始患者数据。

**Result:** 在涵盖四种成像模态的16个医学分割任务上，MedCRP-CL达到73.3% Dice分数和仅4.1%的遗忘率，相比最优基线提升了8.0%的Dice分数，同时参数量减少了6倍。

**Conclusion:** 该研究表明，通过在线发现任务结构（语义模态）并据此进行结构感知的持续学习，可以显著缓解灾难性遗忘并提升知识迁移效率。CRP驱动的无回放机制为隐私敏感的医学影像持续学习提供了可行方案，未来可向更多模态和临床任务扩展。
...
#### 🧩 TL;DR
WildRoadBench 提出了一个野外地表道路损伤接地基准，通过视觉语言模型直接接地和 LLM 驱动智能体自主探索两条路线，在统一 UAV 语料库上评估多类模型，发现所有路线在野外场景下均远未达到可靠性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有道路损伤检测基准多局限于受控环境或传统检测范式，缺乏在真实野外 UAV 影像上评估视觉语言模型（VLM）直接接地能力与 LLM 驱动智能体自主研究与工程能力的统一基准。

**Method:** WildRoadBench 构建了一个统一的 UAV 道路损伤语料库，设计了双轨评估协议：VLM Track 在统一的提示、解码与解析流程下评估固定 VLM 的损伤定位能力；Agent Track 评估 LLM 驱动智能体在给定书面任务、少量探索数据和固定交互预算下，自主搜索网络、适配预训练组件、编写训练与推理代码并通过标量反馈预言机提交预测的能力。两个轨道采用相同的图像集和每类 AP_50 指标。

**Result:** 封闭源前沿模型在 VLM 榜单上领先但仍留下超过一半的指标空间；开源接地模型性能远低于封闭源模型，且新一代或推理风格变体并未持续提升接地性能；所有开源模型均在小目标上失效；智能体路线尽管拥有更丰富的自主能力，其性能仍落后于最强的 VLM，且有多个智能体未能在预算内提交有效结果。

**Conclusion:** WildRoadBench 揭示了当前 VLM 直接接地与 LLM 智能体自主研究两条路线在野外 UAV 道路损伤检测任务上均远未达到可靠水平，表明该领域仍面临巨大挑战，开源的代码与数据为可复现的后续研究提供了基础。
...
### [8] [SDM: A Powerful Tool for Evaluating Model Robustness](https://arxiv.org/abs/2605.20308)
*Xinlei Liu, Tao Hu, Jichao Xie, Peng Yi, Hailong Ma, Baolin Li*

#### 🧩 TL;DR
本文提出一种名为序贯差分最大化（SDM）的梯度攻击方法，通过重构对抗样本生成目标为最大化非真实标签概率上界与真实标签概率之差，并设计"循环-阶段-步骤"三层优化框架，以负概率损失和方向性概率差异比损失分阶段优化，在攻击性能和成本效益上均超越APGD等现有最先进方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有梯度攻击方法（如APGD）长期难以取得显著突破，其性能退化源于"高损失非对抗性样本"问题，即生成对抗样本时所采用的优化目标不恰当，导致高损失样本并非真正的对抗样本。

**Method:** 本文将对抗样本生成的目标重构为"最大化非真实标签概率上界与真实标签概率之差"，并据此提出SDM方法，构建了"循环-阶段-步骤"三层优化框架；在初始阶段采用负概率损失函数，后续阶段采用方向性概率差异比（DPDR）损失函数，通过分阶段序贯优化逐步逼近理想目标。

**Result:** 实验结果表明，与之前最先进的方法相比，SDM不仅实现了更强的攻击性能，还展现出更优的成本效益，即在更少计算开销下获得更高的攻击成功率。

**Conclusion:** SDM通过理论分析揭示了"高损失非对抗性样本"的成因，并重构了更合理的对抗目标函数，为梯度攻击方法提供了新的突破方向；其开源代码有利于该领域后续研究与应用。
...
### [9] [Tiny-Engram: Trigger-Indexed Concept Tables for Generative Vision](https://arxiv.org/abs/2605.20309)
*Runyuan Cai, Yiming Wang, Yu Lin, Xiaodong Zeng*

#### 🧩 TL;DR
Tiny-Engram 提出一种紧凑的触发词索引概念表，为视觉记忆赋予显式词汇地址和激活边界，在冻结的图像和视频生成器中实现可控制的概念检索，在图像生成中表现优异，但在视频生成中身份持久性仍有限。

---

#### 📘 Detailed Summary
**Motivation:** 现有生成视觉模型个性化方法通常通过连续适配器或权重更新编码新概念，但缺乏对概念是否以及何时被检索的细粒度控制，无法实现模块化、可寻址的视觉记忆注入。

**Method:** Tiny-Engram 将每个概念参数化为少量记忆条目，通过注册的 n-gram 匹配进行索引，仅在匹配的触发区域内调制文本编码器的隐藏状态；在触发区域外，条件通路与冻结基模型完全相同，适用于单编码器潜在扩散和多编码器扩散Transformer骨干网络。

**Result:** 在图像生成中，Tiny-Engram 能将稀有触发短语与目标身份绑定，同时保持周围提示的 compositional control；在文本条件视频生成中，触发路径能可靠改变生成主体，但细粒度身份在保留视频提示上的持久性仍然有限。

**Conclusion:** 小型显式寻址概念表是实现模块化视觉个性化的实用路径，在图像生成中证据最强；视频生成的时间稳定身份可能依赖于文本侧记忆与动态视觉状态之间更紧密的耦合，未来工作需探索超越文本条件接口的记忆注入方式。
...
### [10] [FullFlow: Upgrading Text-to-Image Flow Matching Models for Bidirectional Vision--Language Generation](https://arxiv.org/abs/2605.20316)
*Eric Tillmann Bill, Enis Simsar, Alessio Tonioni, Thomas Hofmann*

#### 🧩 TL;DR
FullFlow 提出一种参数高效的方法，仅通过训练 LoRA 适配器和轻量文本头，将预训练的文本到图像 rectified-flow 模型升级为双向视觉-语言生成器，在 SD3 上以约 5% 可训练参数显著提升文本到图像和图像到文本性能，并降低显存开销与训练时间。

---

#### 📘 Detailed Summary
**Motivation:** 现有文本到图像扩散模型编码了丰富的视觉先验，但仅支持单向文本条件生成；而基于这些模型构建的统一视觉-语言模型通常需要通过大规模联合预训练或大量重训练文本通路来恢复双向能力，这会丢弃文本到图像骨干网络已编码的强图像先验，效率低下。

**Method:** FullFlow 在预训练的 rectified-flow 文本到图像模型（如 SD3）基础上，仅训练 LoRA 适配器和轻量文本头，保持图像在连续 flow 空间中操作，并为文本添加离散插入过程；通过引入独立的图像和文本时间步，将推理转化为二维生成空间中的轨迹选择，从而在一个骨干网络中支持文本→图像、图像→文本、联合采样和部分文本预测。

**Result:** 在 SD3 上，与匹配 LoRA 秩和可训练参数量的先前 SOTA 方法（Dual Diffusion）相比，FullFlow 将文本→图像的 FID 从 62.7 降至 31.6，图像→文本的 CIDEr 从 2.0 提升至 99.4；同时峰值显存从约 84GB 降至约 38GB，在两张 RTX A5000 GPU 上吞吐量提升约 8 倍，训练时间不到 24 小时，仅训练约 5% 的骨干参数；该方法同样可迁移至 FLUX.1-dev 并支持下游 VQA 任务。

**Conclusion:** FullFlow 证明，无需完整的多模态预训练，仅通过参数高效微调即可从预训练文本到图像 flow 模型中解锁强大的双向视觉-语言能力，为高效构建统一视觉-语言模型提供了新路径。
...
#### 🧩 TL;DR
提出了一套基于心理物理学协议的框架（局部定位性与可命名性）来衡量视觉模型特征的人类可解释性，发现基础模型（DINOv2、DINOv3、CLIP、SigLIP）的可解释性系统性地低于有监督ViT，且可解释性与下游任务性能无关，而与特征的局域性和粗粒度语义对齐相关。

---

#### 📘 Detailed Summary
**Motivation:** 随着视觉模型从研究基准走向高风险部署，其特征的人类可解释性日益重要，但现有方法无法可靠地衡量和比较不同视觉模型的可解释性，缺乏统一的定量评估框架。

**Method:** 提出了一个由两种互补心理物理学协议组成的评估框架：(1) 局部定位性——观察者能否在新图像上预测特征激活的位置；(2) 可命名性——观察者能否准确描述特征含义。通过稀疏自编码器恢复特征，并引入以随机基线锚定的评分函数将所有模型置于同一尺度上。对六个视觉Transformer（两个有监督ViT和DINOv2、DINOv3、CLIP、SigLIP四个基础模型）收集了超过15,000份行为响应。

**Result:** 在通过质量检查的377名参与者的13,400份有效响应上分析发现：基础模型的可解释性系统性地低于有监督ViT；可解释性与下游任务性能在任何测试基准上均不相关；特征激活的局域性和与人类粗粒度语义对齐程度与可解释性正相关，而细粒度感知对齐则无关；两种协议产生了高度相关的排名并共享相同的预测因子。

**Conclusion:** 可解释性是表征质量的一个独立、可衡量的维度，所有被测试的基础模型在此维度上均低于之前的有监督基线；仅提升模型能力无法弥合这一差距，而提升特征局域性和粗粒度语义对齐可以有效改善可解释性。
...
### [12] [ParaVT: Taming the Tool Prior Paradox for Parallel Tool Use in Agentic Video Reinforcement Learning](https://arxiv.org/abs/2605.20342)
*Zuhao Yang, Kaichen Zhang, Sudong Wang, Keming Wu, Zhongyu Yang, Bo Li, Xiaojuan Qi, Shijian Lu, Xingxuan Li, Lidong Bing*

#### 🧩 TL;DR
ParaVT提出首个多智能体端到端RL框架实现并行视频工具调用，通过单轮多窗口裁剪替代传统串行调用，并设计PARA-GRPO算法解决工具先验悖论带来的格式崩溃与奖励捷径问题，在六个长视频理解基准上平均超越Qwen3-VL基线7.9%。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于强化学习的原生工具调用方法以串行方式逐轮调度视频处理工具（如裁剪），存在单次错误无法纠正、多轮工具调用污染上下文、推理成本随轮次线性增长等问题，亟需一种并行工具调用框架以提升长视频理解的鲁棒性与效率。

**Method:** ParaVT提出多智能体端到端RL训练框架，在单轮中并行调度多个时间窗口裁剪，实现更清洁的上下文与更好的容错性；同时提出PARA-GRPO算法，包含结构锚定格式奖励（仅在格式最易崩溃的结构token位置施加）和逐提示帧预算随机化（使调用工具的奖励信号可区分于跳过工具），以解决工具先验悖论导致的格式崩溃与奖励捷径问题。

**Result:** 在六个长视频理解基准上，ParaVT相较Qwen3-VL基线平均提升7.9%；PARA-GRPO将训练过程中的格式合规率从0.13提升至0.64；交叉模型对比实验表明，先验强度是同时驱动格式崩溃与工具探索共享因素，弱先验模型格式稳定但RL无法诱发工具调用。

**Conclusion:** 随着现代LMMs工具能力逐渐内化，强化学习必须与已有先验协同工作，ParaVT提供了一种面向智能体RL的通用方案；代码、数据与模型权重已开源。
...
### [13] [How You Move Tells What You'll Do: Trajectory-Conditioned Egocentric Prediction](https://arxiv.org/abs/2605.20388)
*Sejoon Jun, Hai Nguyen-Truong, Luigi Seminara, Lorenzo Torresani*

#### 🧩 TL;DR
TrajPilot通过预测未来第一人称视角摄像头轨迹作为隐式意图信号，在动作预测任务中显著优于以语言为条件的大模型和结构化规划基线，且无需在测试时观测真实轨迹即可恢复大部分性能提升。

---

#### 📘 Detailed Summary
**Motivation:** 第一人称视角下未来动作预测本质上是一个欠定问题：相同的上下文可以对应多种合理的未来，而直接最小化预测误差的模型被迫在多种可能性之间进行折中或平均，导致预测结果模糊甚至错误。

**Method:** TrajPilot首先从当前第一人称上下文预测候选的未来摄像头轨迹（头部位移路径），然后利用预测出的轨迹作为条件信号，在一个由语言塑造结构但从不作为条件输入的动作对齐嵌入空间中进行动作预测；该方法不需要在测试时观测真实轨迹，仅依赖RGB图像进行相机位姿估计。

**Result:** 在Ego-Exo4D atomic、Ego-Exo4D Keystep、Ego4D GoalStep和EgoPER等程序性规划基准上，TrajPilot全面超越VLM和结构化规划基线，且其轨迹优势随着预测时域增加而扩大；在目标掩码的零样本泛化设置中，该模型在Ego-Exo4D atomic、EPIC-Kitchens-100和篮球投篮结果预测任务上均优于VLM基线。

**Conclusion:** 未来摄像头轨迹携带了比语言更精细的操作者意图信息，是动作预测的高效条件信号；该轨迹在一定程度上可从上下文中预测，使得测试时无需真实轨迹即可获得大部分增益，揭示了以隐式运动意图为中介的动作预测范式的有效性。
...
### [14] [Lighting-aware Unified Model for Instance Segmentation](https://arxiv.org/abs/2605.20436)
*Qisai Liu, Alloy Das, Zhanhong Jiang, Joshua R. Waite, Aditya Balu, Adarsh Krishnamurthy, Soumik Sarkar*

#### 🧩 TL;DR
本文提出照明卷积注意力模块（LCA），一种轻量级适配器，通过双分支架构处理RGB特征与对比度图，并采用成对训练策略与针对性损失项，在不微调骨干网络的前提下显著提升SAM在复杂光照条件下的实例分割鲁棒性，在多个基准及自建合成数据集上取得最优性能。

---

#### 📘 Detailed Summary
**Motivation:** 尽管基础模型如SAM在零样本泛化方面表现优异，但在真实世界多变光照条件下实例分割性能显著下降，现有方法多依赖微调大规模骨干网络，计算成本高昂且难以部署，亟需一种轻量级、无需微调主干的模块来弥补光照鲁棒性这一关键空白。

**Method:** 提出照明卷积注意力模块（LCA），采用双分支架构同时处理RGB特征与对比度图，利用对比度对结构变化而非光照伪影的物理敏感性增强分割鲁棒性；通过成对训练策略优化LCA，引入专门损失项显式惩罚干净图像与光照变体之间的分割不一致性，且无需微调SAM的重型骨干网络。

**Result:** 在多个现有基准数据集以及自建的Unity-based合成数据集（精确模拟复杂真实光照条件）上开展广泛实验，结果表明LCA显著桥接了域差距，在所有评测场景下均实现了优于现有方法的光照鲁棒分割性能。

**Conclusion:** 本工作证明通过轻量级适配器模块与物理启发的特征设计，可以在不微调大规模基础模型的前提下有效解决光照退化问题，为提升视觉基础模型在真实场景中的部署可靠性提供了可行路径，所提出的合成数据集也为光照鲁棒性研究提供了标准化评测资源。
...
### [15] [Do Vision--Language Models Understand 3D Scenes or Just Catalogue Objects?](https://arxiv.org/abs/2605.20448)
*Animesh Maheshwari, Divyansh Sahu, Nishit Verma*

#### 🧩 TL;DR
本文提出一个3,034样本的人类标注基准，系统评估VLM在深度排序遮挡、光学几何推理和体积重排规划三方面的三维空间理解能力，发现模型在重排任务上表现良好（53-97%），但在遮挡（6-45%）和反射（<7%）任务上严重失败，并通过白盒分析定位失败根源在于视觉token合并阶段造成的空间信息丢失。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉-语言模型虽能可靠命名场景中的物体，但其是否真正理解物体所处的三维空间布局仍不清楚，且缺乏针对三维空间理解不同组件的系统性评估基准。

**Method:** 构建了一个包含3,034个样本的人工筛选基准，针对三维空间理解的三个核心组件：深度排序遮挡（通过三种独立反事实操作探测）、可见反射的光学几何推理以及体积重排规划。对六个前沿及开源VLM收集了18,204个由训练标注者评分（不使用LLM评判）的响应，并对Qwen3-VL-8B-Thinking进行白盒分析以定位失败原因。

**Result:** 模型在可见布局的体积重排规划上达到53-97%的准确率且很少违反碰撞约束，但在遮挡任务上仅6-45%，反射任务上低于7%；一个具身推理模型也展现出相同的表现剖面；白盒分析表明，空间信息在整个视觉编码器中均可恢复，但在token压缩后变得不可访问，只有在将干净的合并后激活补丁注入语言解码器后才重新稳定。

**Conclusion:** 当前VLM存在显著的三维空间理解能力分离——重排能力强但遮挡和反射推理能力极弱，且该缺陷根源于视觉token合并阶段的信息丢失，揭示了视觉编码器与语言解码器之间的空间信息传递瓶颈。
...
### [16] [Pixel Wised Lesion Prediction on COVID-19 CT Imagery: A Comparative Analysis of Automated Image Segmentation Architectures](https://arxiv.org/abs/2605.20459)
*Sarmad Khan, Arslan Shaukat, Umer Asgher, Basim Azam*

#### 🧩 TL;DR
本文系统评估了四种深度学习架构（Unet、PSPNet、Linknet、FPN）与六种预训练骨干网络（VGG 19、DenseNet 121、Inception ResNet V2、MobileNet V2、SeresNet 101、EfficientNet B0）的组合在COVID-19 CT图像分割中的性能，在二分类分割中取得了最高98%的F1-Score，并验证了该评估框架对其他医学影像分割任务的参考价值。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于深度学习的医学图像分割研究缺乏标准化的性能分析方法和统一的数据集使用规范，导致领域可靠性受限；针对COVID-19病灶CT图像分割，尚无对不同分割架构与预训练骨干网络组合的系统性对比评估。

**Method:** 本研究集成了四种深度学习分割架构（Unet、PSPNet、Linknet、FPN）与六种预训练编码器（VGG 19、DenseNet 121、Inception ResNet V2、MobileNet V2、SeresNet 101、EfficientNet B0），构建多种组合测试架构；在三个不同的COVID-19 CT分割数据集上同时进行二分类和多分类分割实验。

**Result:** 在二分类分割任务中，最佳组合达到了98%的F1-Score；在多分类分割任务中，两个数据集分别取得了75%和77%的F1-Score，表明深度学习架构能够实现精确且高效的分割结果。

**Conclusion:** 深度学习和人工智能技术可显著提升大流行病的诊断流程，本研究建立的标准化评估框架不仅适用于COVID-19病灶分割，也可为其他医学影像分割场景提供参考基准。
...
### [17] [Understanding Model Behavior in Monocular Polyp Sizing](https://arxiv.org/abs/2605.20461)
*Xinqi Xiong, Andrea Dunn Beltran, Junmyeong Choi, Sarah K. McGill, Marc Niethammer, Roni Sengupta*

#### 🧩 TL;DR
本文系统评估了多中心公开数据集上基于单目结肠镜图像的二分类息肉大小分层（≤5 mm vs. >5 mm）任务，发现当前模型依赖与检查行为相关的线索而非真实度量尺度，并指出度量尺度与掩码鲁棒性是制约性能的两个独立瓶颈。

---

#### 📘 Detailed Summary
**Motivation:** 准确的息肉大小分层（5 mm阈值）对结肠镜术后随访决策至关重要，但单目结肠镜缺乏可靠的度量参考，现有方法在真实尺度估计上存在根本性局限，亟需系统审计当前方法在大小分类中的实际瓶颈。

**Method:** 采用多中心公开数据集、多种模型家族及患者分层交叉验证，对比RGB外观、相对深度、光度测量等多种输入模态下的模型性能；引入不同粒度真实尺度信息（oracle scale）量化潜在提升，并设计oracle scale ladder、shortcut partition、mask substitution等可复用评估工具。

**Result:** 跨架构和输入模态下模型性能中等一致，表明模型依赖与检查行为相关的线索而非真实尺度；当前深度估计和全局校准带来的提升有限；在分布偏移下分割误差几乎消除了尺度信息的全部潜在收益，oracle scale在预测掩码下仅恢复基线性能。

**Conclusion:** 度量尺度缺失与掩码鲁棒性不足是息肉大小分层任务中两个独立的性能瓶颈，未来需同时解决尺度感知和分割泛化问题；论文提供了可复用的审计工具，为后续息肉大小评估流水线提供了诊断框架。
...
### [18] [HalluCXR: Benchmarking and Mitigating Hallucinations in Medical Vision-Language Models for Chest Radiograph Interpretation](https://arxiv.org/abs/2605.20469)
*Haoyu Wang, Zitong Li*

#### 🧩 TL;DR
本文构建了 HalluCXR 基准，系统评估了六种视觉-语言模型在胸部 X 光片上的幻觉现象，发现 61.9–82.3% 的输出存在幻觉，并提出基于输出长度监控和多模型集成的幻觉缓解策略，强调幻觉审计是临床部署的前提条件。

---

#### 📘 Detailed Summary
**Motivation:** 视觉-语言模型在医学图像解读中频繁产生幻觉，即生成临床看似合理但事实上错误的发现，这对患者安全构成直接风险，而现有研究缺乏对此问题的系统性评估与量化。

**Method:** 构建 HalluCXR 基准，涵盖 856 张分层采样的 MIMIC-CXR 胸部 X 光片、三种查询类型和六种架构多样的 VLM，共进行 15,408 次模型评估；设计包含八类幻觉分类法（含临床严重性评级）和两层检测流水线（自动检测和 LLM 裁判），并在 250 个人工标注上验证。

**Result:** 自动检测 F1=0.959，LLM 裁判 F1=0.907；61.9–82.3% 的输出含有幻觉，最高 80.2% 的输出包含临床危险错误；正常胸片 paradoxically 吸引最严重的幻觉，常见发现被过度编造而罕见发现被漏检，输出长度单独可预测幻觉风险（AUC 最高 0.908）；六模型集成减少编造达 84.8% 但增加遗漏，三模型子集以一半成本保持相当性能。

**Conclusion:** 幻觉审计、基于冗长度的风险监控和基于集成的安全层是临床部署 VLM 的前提条件，输出长度是简单有效的幻觉风险指标，多模型集成可在成本和安全性之间取得平衡。
...
### [19] [Goodbye Drift: Anchored Tree Sampling for Long-Horizon Video-to-Video Generation](https://arxiv.org/abs/2605.20476)
*Matthew Bendel, Stephen W. Bailey, Mithilesh Vaidya, Sumukh Badam, Xingzhe He*

#### 🧩 TL;DR
本文提出 Anchored Tree Sampling (ATS)，一种无需训练的推理时调度策略，通过稀疏到密集的锚点约束树状插值替代自回归滚动生成，有效缓解长视频生成中的漂移问题，在多种条件模态下均优于现有基线并支持40分钟以上的稳定生成。

---

#### 📘 Detailed Summary
**Motivation:** 长视频生成面临两大相互交织的问题：随时间推移的视频质量退化（漂移）和物体一致性问题（如瞬态内容渲染错误）。现有工作多采用自回归蒸馏技术同时处理这两个问题，但本文选择专注于直接解决漂移问题。

**Method:** 本文提出 Anchor Tree Sampling (ATS)，一种无需训练的推理时调度策略。其核心思想是将传统的从左到右自回归逐步生成替换为稀疏到密集的树状插值过程：根节点在全视界上生成稀疏锚点，递归细化产生中间锚点，最终叶节点跨度在相邻锚点之间合成。该方法将关键路径从 K 步顺序生成减少为 L+1 步树层次生成，将随视界累积的漂移转化为锚点约束的局部漂移。本文聚焦静态相机下的视频到视频生成任务，利用密集条件信号可较好近似稀疏锚点，使基模型无需重新训练即可生成锚点。

**Result:** 以 Wan 2.1 + VACE 为基模型，在五种条件模态（内补、外扩、边缘、姿态、深度）上，ATS 在两个自回归基线上均取得了更优的整体质量和漂移预防效果。此外，ATS 在 LTX-2.3 上实现了五种模态下超过 40 分钟的稳定视频生成。

**Conclusion:** ATS 有效证明了通过推理时调度策略而非模型训练即可显著缓解长视频生成中的漂移问题。本文指明了将 ATS 扩展到任意长文本到视频生成、动态相机以及多镜头场景的未来方向。
...
### [20] [A Human-in-the-Loop Framework for Efficient Prompt Selection in Microscopy Vision-Language Models](https://arxiv.org/abs/2605.20495)
*Abhiram Kandiyana, Ankur Mali, Lawrence O. Hall, Peter R. Mouton, Dmitry Goldgof*

#### 🧩 TL;DR
本文提出将提示集构建形式化为目标驱动的主动学习问题，通过三种互补的选择标准优先选择待验证图像，在严格低资源约束下以远少于随机选择的专家验证样本量达到目标性能，平均仅需20张标注图像即可实现100%测试准确率。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于视觉-语言模型（VLM）的显微镜图像分类方法虽可通过提示调优减少人工标注，但面临两个未解决的实际问题：一是应当优先选择哪些未标注图像供专家验证，二是需要多少已验证样本才能达到目标性能。

**Method:** 本文将提示集构建形式化为目标驱动的主动学习问题，在严格低资源约束下从小规模未标注池中优先选择需要专家验证的图像，并研究了三种互补的选择标准用于候选样本的排序与筛选。

**Result:** 实验表明，所提方法在达到目标性能时所需的专家验证图像数量远少于随机选择，平均仅需20张标注图像即可实现100%的测试准确率。

**Conclusion:** 该人机协同框架展示了生成式AI在生物医学图像分析中以人为中心的应用前景，即专家在显著降低标注成本的同时，仍能积极参与验证和优化模型输出。
...
### [21] [NeuroQA: A Large-Scale Image-Grounded Benchmark for 3D Brain MRI Understanding](https://arxiv.org/abs/2605.20525)
...
#### 🧩 TL;DR
NeuroQA 是一个大规模 3D 脑 MRI 视觉问答基准，包含来自 12,977 名受试者的 56,953 个 QA 对，覆盖五个临床领域，通过严格的数据清洗和专家审核确保质量，并揭示当前模型在该任务上的表现尚不及文本多数类基线。

---

#### 📘 Detailed Summary
**Motivation:** 现有医学视觉问答（VQA）工作多基于 2D 切片或依赖狭窄的诊断标签，缺乏大规模、跨领域、基于完整 3D 体积的基准，难以评估模型在真实临床场景中的 3D 推理能力。

**Method:** NeuroQA 构建了 56,953 个 QA 对，每个 QA 对均与完整 3D MRI 体积配对；设计了 203 个模板（131 个图像依赖型、72 个图像参考型），涵盖 Yes/No、多项选择和开放式三种格式，评估 11 种临床推理技能；通过答案分布精炼将文本-only 准确率从 >80% 降至 44.6%，并采用 38 条确定性规则流水线和两轮专家审核验证所有 QA 对，同时发布图像基础协议以评估图像必要性。

**Result:** 在封闭格式测试集上，最佳零样本视觉语言模型和 3D CNN 基线分别达到 47.5% 和 43.7% 的准确率，均低于 49.4% 的文本多数类基线；封闭格式文本-only 准确率经精炼后降至 44.6%；所有 QA 对通过零同主题矛盾验证。

**Conclusion:** NeuroQA 填补了 3D 医学 VQA 基准的空白，揭示了当前视觉语言模型在 3D 脑 MRI 推理任务上的局限性（甚至不及文本基线），为未来多模态医学推理研究提供了标准化评估平台和公开排行榜。
...
### [22] [ShadeBench: A Benchmark Dataset for Building Shade Simulation in Sustainable Society](https://arxiv.org/abs/2605.20510)
*Longchao Da, Mithun Shivakoti, Xiangrui Liu, T Pranav Kutralingam, Yezhou Yang, Hua Wei*

#### 🧩 TL;DR
ShadeBench 是一个面向城市阴影理解的大规模多模态数据集与基准，包含地理多样化的城市场景、随时间变化的模拟阴影图及文本描述，并配套卫星影像、建筑骨架和3D建筑网格，支持阴影生成、阴影分割和3D建筑重建等下游任务，为数据驱动的城市热环境研究和耐热城市规划提供了标准化评估框架。

---

#### 📘 Detailed Summary
**Motivation:** 城市热暴露因热岛效应加剧日益严峻，建筑引发的精细尺度阴影模式对行人热暴露和户外活动规划有重要影响，但目前缺乏大规模数据集和系统化评估框架来支撑城市阴影的准确建模与大尺度分析。

**Method:** ShadeBench 构建了包含地理多样化城市场景的多模态数据集，提供随时间变化的模拟阴影图和文本描述，并配准了卫星影像、建筑骨架表示和3D建筑网格；在此基础上，定义了阴影生成、阴影分割和3D建筑重建三项下游任务，并建立了标准化评估协议和基线方法。

**Result:** 论文公开了代码与数据集（https://darl-genai.github.io/shadebench/），为各下游任务提供了标准化评估协议和基线方法，但摘要中未报告具体的定量实验结果或性能指标。

**Conclusion:** ShadeBench 通过提供可扩展的精细阴影分析能力，为数据驱动的城市气候研究奠定了坚实基础，并支持未来在耐热城市规划和决策制定方面的研究。
...
### [23] [Draw2Think: Harnessing Geometry Reasoning through Constraint Engine Interaction](https://arxiv.org/abs/2605.20743)
*Juncheng Hu, Jiawei Du, Xin Zhang, Joey Tianyi Zhou*

#### 🧩 TL;DR
Draw2Think通过将几何推理从潜在空间推断重构为与GeoGebra约束引擎的交互式代理循环，实现了可验证的几何外部化，在Propose-Draw-Verify框架下分别审计构造忠实度和测量保真度，并在多个基准上取得显著提升。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉-语言模型在几何问题求解中准确率不断提升，但其中间状态仍然是潜在且不可验证的：无论是文本推理还是绘图代码中所表达的关系，都无法保证存在一个满足约束的实际配置来实现它。现有的基于渲染像素或一次性脚本的外部化方法无法提供精确的、每步动作的几何保证，因此需要一种能够提供确切几何约束保障的外部化机制。

**Method:** Draw2Think提出了一种Propose-Draw-Verify循环框架，将几何推理重构为模型与GeoGebra约束引擎之间的代理交互。模型首先提出假设并将其外部化到可执行的画布上，然后通过约束引擎精确测量几何量，最后将结构化观测结果反馈给模型，使后续推理基于经过约束检查的画布状态进行。该框架实现了模型级构造忠实度（画布是否实现预期配置）和引擎级测量保真度（来自画布约束的精确值和关系）的独立可审计性。

**Result:** 在GeoGoal数据集上，Draw2Think构建的画布通过了95.9%的谓词级和84.0%的严格问题级构造检查；在平面/立体几何基准上，结果准确率分别提升了最高4.1%和16.4%；在GenExam-math数据集上，严格/宽松渲染得分分别达到68.2%和90.5%。

**Conclusion:** 通过将几何推理外部化为与约束引擎的代理交互，Draw2Think使得几何推理的中间状态可验证、可审计，有效弥合了语言表达与几何约束满足之间的鸿沟，为可信赖的符号-神经几何推理提供了新范式。
...
#### 🧩 TL;DR
本文提出了面向持续语义分割中类别、域和标签联合非平稳性的学习框架，通过梯度自适应稳定化和原型锚定监督机制，在异质稠密预测场景下利用有限标注和大量无标注数据实现了更优的稳定性-可塑性权衡，并在多种增量学习设定中超越现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有持续学习工作通常将类别变化、输入分布漂移和标注可用性变化等非平稳性因素孤立研究，而实际结构化预测系统中这些因素往往同时发生且相互耦合，导致在异质稠密预测环境中缺乏有效的学习框架；同时，在分布漂移下使用少量标注进行微调会引发严重的不稳定性和过拟合问题。

**Method:** 提出梯度自适应稳定化（gradient-adaptive stabilization），通过梯度缩放的随机扰动实现参数级正则化，以在分布漂移下实现原则性的稳定性-可塑性权衡；引入半监督学习利用无标注数据，并提出原型锚定监督（prototype anchored supervision），通过联合置信度和原型一致性验证伪标签，从而在持续语义分割中实现类别增量、域增量和少样本场景下的联合非平稳性学习。

**Result:** 在类别增量、域增量和少样本设定下的广泛实验表明，所提出方法在异质结构化预测场景中持续优于现有持续分割方法，并揭示了现有方法在联合非平稳性环境下的根本性失败模式，为动态演化环境中鲁棒稠密预测器的学习提供了关键洞察。

**Conclusion:** 本研究揭示了现有持续语义分割方法在面对类别、域和标签联合非平稳性时的根本性局限，验证了梯度自适应稳定化与原型锚定半监督学习相结合的有效性，为构建在动态演化环境中鲁棒且可扩展的稠密预测系统提供了重要的方法论指导和实践方向。
...
### [25] [Faster or Stronger: Towards Flexible Visual Place Recognition via Weighted Aggregation and Token Pruning](https://arxiv.org/abs/2605.20551)
*Zichao Zeng, June Moh Goo, Junwei Zheng, Weijia Fan, Jiaming Zhang, Rainer Stiefelhagen, Jan Boehm*

#### 🧩 TL;DR
提出WeiAD和WeiToP两个方法，分别通过加权聚类聚合和基于自蒸馏的令牌剪枝，提升视觉地点识别中ViT后端的全局描述符判别力与推理效率，并支持即插即用的精度-效率灵活调控。

---

#### 📘 Detailed Summary
**Motivation:** 现有VPR方法使用ViT提取patch级特征后聚合为全局描述符，但多数聚合方法对所有聚类簇进行均匀池化，忽视了不同簇在空间或语义模式上的差异化贡献；同时，基于ViT的后端在推理时特征提取成本高，而现有加速工作仅关注描述符压缩，忽略了特征提取阶段的开销。

**Method:** 提出WeiAD，在聚合阶段为不同聚类簇分配可学习权重，生成更具判别性的全局描述符。进一步提出WeiToP，一种面向VPR的令牌剪枝框架，通过自蒸馏机制使聚合阶段的簇重要性信号监督浅层Transformer层附带的轻量剪枝模块，实现推理时令牌剪枝；经过单次联合训练后，剪枝模块可在推理时即插即用，无需额外训练即可灵活调控精度与效率。

**Result:** 实验结果表明，WeiAD生成的全局描述符相比均匀聚类方法更具判别性；WeiToP在显著降低特征提取成本的同时，在VPR任务上优于从通用视觉任务迁移的现有令牌剪枝方法，并能灵活控制精度与效率之间的权衡。

**Conclusion:** 通过加权聚类聚合和自蒸馏驱动的令牌剪枝，WeiAD与WeiToP分别解决了ViT后端在VPR中的判别力不足与推理效率低下问题，为大规模部署和边缘设备应用提供了实用的精度-效率可调方案。
...
### [26] [$Δ$ynamics: Language-Based Representation for Inferring Rigid-Body Dynamics From Videos](https://arxiv.org/abs/2605.20576)
...
#### 🧩 TL;DR
ΔYNAMICS 提出了一种以语言作为统一表示框架的视觉-语言方法，用于从单目视频推断刚体动力学参数，通过结构化文本格式生成物理仿真场景配置，并在 CLEVRER 和真实世界数据集上实现了显著优于现有 VLM 的分割性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有从单目视频推断刚体物理状态与属性的方法通常假设特定的物理系统、物体类型和相机位姿，难以泛化到复杂的真实世界场景，缺乏一种统一且通用的表示方式来桥接视觉感知与物理仿真。

**Method:** ΔYNAMICS 提出一种视觉-语言框架，将语言作为刚体动力学的统一表示，通过生成结构化文本格式的场景配置来驱动物理仿真，而非直接预测物理参数。该框架通过集成自然语言运动推理和利用光流作为语义无关的输入来增强模型的泛化能力，并引入了测试时采样和进化搜索策略来进一步提升性能。

**Result:** 在 CLEVRER 数据集上，ΔYNAMICS 的分割 IoU 达到 0.30，相比 InternVL3-8B、Qwen2.5-VL-7B 和 Claude-4-Sonnet 等领先 VLM 提升了 7 倍。测试时采样和进化搜索分别将分割 IoU 进一步提升了 27% 和 120%。此外，模型在包含 235 个真实世界刚体视频的新数据集上展示了强大的迁移能力。

**Conclusion:** 该研究证明了语言驱动的物理推理在桥接视觉感知与物理仿真方面的潜力，通过统一的语言表示实现了对复杂真实场景的泛化能力，为基于物理的感知与仿真提供了一种新的通用范式。
...
### [27] [QwenSafe: Multimodal Content Rating Description Identification via Preference-Aligned VLMs](https://arxiv.org/abs/2605.20584)
*Dishanika Denipitiyage, Aruna Seneviratne, Suranga Seneviratne*

#### 🧩 TL;DR
提出QwenSafe，一种基于Qwen3-VL-8B微调并结合DPO对齐的视觉语言模型，通过联合推理应用元数据和截图来自动识别Apple定义的内容评级描述符（CRD），在12个CRD分类任务上显著优于现有基线模型。

---

#### 📘 Detailed Summary
**Motivation:** 移动应用市场要求开发者披露标准化的内容评级描述符（CRD）以告知用户敏感或受限内容，但应用内容的多模态特性（文本描述和视觉界面）使得确保这些披露的准确性和一致性面临挑战，现有方法难以高效且可扩展地实现自动化内容分类。

**Method:** 提出QwenSafe视觉语言模型，基于Qwen3-VL-8B进行监督微调并采用直接偏好优化（DPO）对齐模型预测与描述符特定的证据和解释；同时引入metadata2CRD数据构建流水线，通过结合应用描述、截图和形式化描述符定义合成描述符对齐的问答对以支持可扩展训练。

**Result:** 在12个Apple定义的内容评级描述符二分类任务上，QwenSafe在正类召回率上分别比Qwen3-VL、LLaVA-1.6和Gemini-2.5-Flash提升111.8%、36.1%和2.1%，持续超越所有基线模型。

**Conclusion:** 描述符感知的多模态对齐显著提升了自动化内容分类的性能，展示了视觉语言模型在支持移动应用市场中可扩展且一致的内容评级方面的潜力。
...
### [28] [Mind Your Margin and Boundary: Are Your Distilled Datasets Truly Robust?](https://arxiv.org/abs/2605.20606)
*Muquan Li, Yingyi Ma, Yihong Huang, Hang Gou, Ke Qin, Ming Li, Yuan-Fang Li, Tao He*

#### 🧩 TL;DR
C$^2$R 是一个面向鲁棒数据集蒸馏的框架，通过攻击感知课程学习与对比学习目标，在保持蒸馏效率的同时显著提升了合成数据集的对抗鲁棒性，在多个基准上平均超越先前鲁棒 DD 方法 2.8%。

---

#### 📘 Detailed Summary
**Motivation:** 现有数据集蒸馏方法大多只优化干净准确率而忽视鲁棒性，已有的鲁棒 DD 方法面临两大局限：一是对所有对抗扰动样本一视同仁，忽略了鲁棒风险主要由接近零的鲁棒边界样本主导；二是未能显式增大攻击集中区域的决策边界类间间隔，导致准确率与鲁棒性之间权衡不佳。

**Method:** C$^2$R 框架包含两个核心组件：攻击感知课程学习与类均衡对比鲁棒性损失。前者从鲁棒边界视角推导出近似各样本鲁棒 hinge 的扰动分数，据此优先训练那些边界最小、直接驱动鲁棒误差的对抗样本；后者通过对比学习在保持对抗不变性的同时显式拓宽不同类别间的决策边界分离程度。

**Result:** 在 CIFAR-10、CIFAR-100、Tiny-ImageNet 以及多个 ImageNet-1K 子集上，面对六种攻击方法，C$^2$R 在所有设定下均取得了最佳鲁棒准确率，平均超出先前最优鲁棒 DD 方法 2.8%。

**Conclusion:** C$^2$R 通过将鲁棒边界理论指导的课程学习与对比学习相结合，有效解决了鲁棒数据集蒸馏中准确率与鲁棒性的权衡问题，为构建既高效又鲁棒的压缩训练集提供了新的范式与实用框架。
...
### [29] [Seeing Through Fog: Towards Fog-Invariant Action Recognition](https://arxiv.org/abs/2605.20645)
*Enqi Liu, Liyuan Pan, Zhi Gao, Lingzhi Li, Qing Li*

#### 🧩 TL;DR
本文提出FogAct数据集（首个雾天动作识别基准，含近万段配对清晰/雾化视频片段）与FogNet模型（双流CLIP架构，从退化视频中提取雾不变语义信息），有效缓解雾天条件下的动作识别挑战，在多个数据集上取得了与SOTA方法相竞争的性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有动作识别方法通常假设良好天气与高质量视频输入，而现实应用中雾天场景十分常见。雾天导致的可见度退化与对比度下降严重阻碍了语义线索的提取，使得当前动作识别方法在雾天条件下性能显著下降，该领域缺乏专门的雾天动作识别基准数据集与有效方法。

**Method:** 本文提出FogNet，一种双流CLIP模型，旨在发现隐藏在退化视频背后的雾不变语义信息。该模型在干净视频的引导下学习雾天视频的鲁棒表示，有效捕获干净视频与雾天视频之间共享的结构和运动线索。此外，本文构建了FogAct数据集，利用立体相机系统采集了10个场景、55个动作类别、近10,000段配对的干净与雾天视频片段，作为雾天动作识别的首个基准数据集。

**Result:** 在FogAct数据集及另外三个流行数据集上的大量实验表明，FogNet方法取得了与现有最先进方法相竞争的性能，验证了其在雾天动作识别任务上的有效性。

**Conclusion:** 本文通过构建专用基准数据集与设计雾不变特征学习模型，系统性地推进了雾天条件下的动作识别研究。FogAct为后续研究提供了标准化评测平台，FogNet表明利用双流CLIP架构在干净-雾化视频对之间学习共享语义表示是一条可行且有效的技术路线。
...
### [30] [GSA-YOLO: A High-Efficiency Framework via Structured Sparsity and Adaptive Knowledge Distillation for Real-Time X-ray Security Inspection](https://arxiv.org/abs/2605.20669)
*Jiahao Kong*

#### 🧩 TL;DR
本文提出GSA-YOLO，一种基于YOLOv8n的轻量化框架，通过集成组Lasso、稀疏结构选择和自适应知识蒸馏，在X光安检场景下实现了检测精度与推理速度的协同提升，在HiXray和PIDray数据集上分别达到0.531和0.679的mAP50:95，推理速度达189.62 FPS。

---

#### 📘 Detailed Summary
**Motivation:** X光安检场景中存在严重遮挡、复杂杂乱背景以及严格的实时性要求，现有目标检测模型难以同时兼顾检测鲁棒性、精度与推理效率，亟需一种能够在轻量化前提下提升检测性能的解决方案。

**Method:** GSA-YOLO基于YOLOv8n架构，通过三个核心组件实现结构化稀疏与自适应知识迁移：在颈部网络应用组Lasso（GL）以增强鲁棒特征提取；在检测头应用稀疏结构选择（SSS）实现模型大幅瘦身；引入自适应知识蒸馏（Ada-KD）机制恢复因剪枝损失的精度，三者协同实现特征表达增强与冗余通道裁剪，最大化模型效率。

**Result:** 在HiXray和PIDray数据集上，GSA-YOLO的mAP50:95分别达到0.531和0.679，相较基线提升2.4%和1.8%；计算量从8.7G降至8.0G，推理速度达到189.62 FPS，在保持计算效率的同时实现了精度领先。

**Conclusion:** GSA-YOLO通过结构化稀疏与知识蒸馏的有机结合，有效解决了安检场景下精度与速度的权衡问题，为实际X光安检应用提供了一种高效可靠的轻量化检测方案，展示了稀疏化剪枝与蒸馏技术在资源受限场景下的巨大潜力。
...
### [31] [DarkShake-DVS: Event-based Human Action Recognition under Low-light andShaking Camera Conditions](https://arxiv.org/abs/2605.20680)
*Jiaqi Chen, Qinfu Xu, Liyuan Pan*

#### 🧩 TL;DR
本文提出EIS-HAR框架，结合事件相机与IMU数据，通过非线性运动补偿和四阶段混合架构实现低光与6-DoF运动下的鲁棒人体动作识别，并发布首个大规模基准数据集DarkShake-DVS，在三个数据集上持续超越现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有HAR方法在低光照和6-DoF自由相机运动等实际部署场景中性能严重退化，视觉质量下降且时序一致性被破坏；同时，当前研究缺乏融合低光条件、6-DoF运动与同步IMU数据的基准，也缺少有效的运动补偿技术。

**Method:** EIS-HAR包含两个模块：EIS模块通过非线性扭曲函数重建运动补偿输入以降低运动模糊；HAR模块采用四阶段混合架构高效提取时空特征进行动作识别。此外，提出DarkShake-DVS数据集，包含18,041个在低光与剧烈6-DoF运动下采集的真实世界片段及同步IMU数据。

**Result:** 在三个数据集上的大量实验表明，EIS-HAR在各种条件下均一致性地优于现有最先进方法，验证了事件相机与IMU融合以及所提运动补偿与特征提取架构的有效性。

**Conclusion:** 事件相机与IMU的协同使用能够有效应对低光照与自由运动等挑战性场景，所提出的非线性运动补偿与混合特征提取架构为构建鲁棒的动作识别系统提供了可行方案，DarkShake-DVS数据集为后续研究提供了重要基准。
...
### [32] [SAVER: Selective As-Needed Vision Evidence for Multimodal Information Extraction](https://arxiv.org/abs/2605.20713)
*Miaobo Hu, Shuhao Hu, Bokun Wang, Rui Chen, Xin Wang, Xiaobo Guo, Daren Zha, Jun Xiao*

#### 🧩 TL;DR
SAVER提出了一种选择性视觉按需融合框架，通过共形接地性门控（CGG）和子模相关-多样性选择器，在社交媒体多模态信息抽取中按需激活视觉证据，显著提升F1并降低计算开销与延迟。

---

#### 📘 Detailed Summary
**Motivation:** 社交媒体帖子常附带多张与文本弱相关、冗余甚至误导的图像，传统始终开启的多模态融合不仅浪费计算资源，还可能放大虚假视觉线索；核心挑战在于判断每个候选跨度或实体对是否需要视觉信息，以及哪些图像子集能提供可信证据。

**Method:** SAVER框架包含三个关键组件：共形接地性门控（CGG）用于估计跨度级视觉接地性并校准激活阈值；子模相关-多样性选择器在激活时从多张图像中选取紧凑证据子集；能量启发联合评分头融合文本、可选视觉证据、文本-图像一致性和稀疏路由，完成实体类型或关系分类。

**Result:** 实验表明，SAVER在MNER和MRE任务上持续优于强纯文本基线和始终开启的多模态基线，不仅提升了F1，还降低了AURC，在固定风险水平下提高了激活覆盖率，并减少了FLOPs和P90延迟。

**Conclusion:** SAVER验证了按需选择性融合视觉信息的有效性，在提升性能的同时显著降低了计算开销，为社交媒体多模态信息抽取提供了一种高效且可信的范式。
...
### [33] [Holistic Reliability Propagation: Decoupling Annotation and Prediction for Robust Noisy-Label](https://arxiv.org/abs/2605.20725)
*Jingyang Mao, Ningkang Peng, Yanhui Gu*

#### 🧩 TL;DR
本文提出Holistic Reliability Propagation (HRP)，通过双层次元学习为每个样本解耦估计两个独立的可靠性标量（标注可靠性alpha和伪标签可靠性beta），分别路由至不同训练目标，在合成和真实噪声标签基准上显著提升分类准确率。

---

#### 📘 Detailed Summary
**Motivation:** 现有带噪学习方法通常将外部标注和模型预测融合为单一可靠性权重，但这两类信息来源可能因不同原因而失效，单一权重无法区分和应对这两种失败模式，导致在噪声率较高时性能不佳。

**Method:** 该方法提出双层次元学习（bilevel meta-learning）为每个样本生成两个解耦的批归一化标量alpha（标注可靠性）和beta（伪标签可靠性），且不约束二者之和为1。然后通过Holistic Reliability Propagation (HRP)将这两个标量路由至不同目标：在输入分支使用可靠性感知的Mixup结合全局门控机制，在对比分支使用beta门控的伪标签正样本。

**Result:** 在合成噪声和真实世界噪声基准上，HRP的平均准确率优于多个强基线方法，并且在最高噪声率下仍保持具有竞争力的性能。

**Conclusion:** 本文表明解耦标注可靠性和伪标签可靠性并将其分别用于不同训练分支，比使用单一融合权重更为有效，为带噪学习的可靠性建模提供了新思路。
...
### [34] [TASTE: A Designer-Annotated Multi-Dimensional Preference Dataset for AI-Generated Graphic Design](https://arxiv.org/abs/2605.20731)
*Haonan Zhu, Elad Hirsch, Alexandria Minetti, Allison Nulty, Purvanshi Mehta*

#### 🧩 TL;DR
本文构建了TASTE数据集，由10位专业设计师对4种文本到图像模型在9个图文设计准则上的输出进行评分，发现现有自动评估系统与设计师一致性不足0.55，并提出了基于成对差异头的小模型可将一致性提升至0.611，填补了约一半与单评分者上限之间的差距。

---

#### 📘 Detailed Summary
**Motivation:** 现有文本到图像模型的偏好数据仅依赖单一整体评分，无法反映设计师在排版、视觉层次、色彩和谐、布局及指令忠实度等多个独立维度上的专业评价，缺乏细粒度、多准则的专业级图文设计评估基准。

**Method:** 构建TASTE数据集，邀请10位专业设计师在9个准则上对4种当前文本到图像模型的输出进行评分，得到每准则1600条评分及整体偏好队列上的逐图像幻觉标记；提出准则无关的信号检验框架（基于Kendall's tau、多数概率和Condorcet循环与精确iid均匀零假设对比），并训练了一个小型成对差异头模型用于评分。

**Result:** 设计师在图文设计上的共识介于食品偏好与电影偏好及照片风格图像质量之间，TASTE所有准则均拒绝随机评分者零假设；6个开放权重VLM裁判（3B-33B参数）和3个专用T2I评分器与5人多数的一致性均不超过0.55，VLM在位置偏差与内容敏感性之间存在权衡；基于TASTE训练的小型成对差异头达到0.611，约为单评分者上限0.741的差距的一半。

**Conclusion:** 现有自动评估系统在图文设计的多准则专业评价上远未达到设计师水平，基于成对差异头的轻量模型可以部分弥合这一差距，为未来在专业设计场景下更精准的自动评估系统提供了数据基础和方法方向。
...
### [35] [STAR-IOD: Scale-decoupled Topology Alignment with Pseudo-label Refinement for Remote Sensing Incremental Object Detection](https://arxiv.org/abs/2605.20738)
*Yaoteng Zhang, Qing Zhou, Junyu Gao, Qi Wang*

#### 🧩 TL;DR
针对遥感增量目标检测中存在的类内尺度变化导致知识迁移失效和旧类标注缺失引起的误分类问题，本文提出STAR-IOD框架，通过子空间解耦拓扑蒸馏（STD）和聚类驱动伪标签生成器（CPG）分别缓解尺度变化和标注缺失问题，在DIOR-IOD和DOTA-IOD两个基准上分别超越SOTA方法1.7%和2.1% mAP。

---

#### 📘 Detailed Summary
**Motivation:** 遥感图像以连续数据流形式到达，传统检测器在学习新类别时会遗忘旧类别，因此遥感增量目标检测（RS-IOD）至关重要。现有方法忽视了遥感场景中普遍存在的类内尺度变化，削弱了知识迁移和旧知识保留的效果；此外，RS-IOD还存在标注缺失问题，导致模型将旧类实例误分类为背景。

**Method:** 提出STAR-IOD框架，包含两个核心模块：一是子空间解耦拓扑蒸馏（STD）模块，通过显式对齐类间拓扑关系并缓解尺度偏移导致的类内表示差异来传递结构知识；二是聚类驱动伪标签生成器（CPG），利用K-Means聚类动态识别类别特定阈值，准确区分真正例目标和背景噪声，缓解旧类标注缺失问题。同时构建了DIOR-IOD和DOTA-IOD两个RS-IOD基准数据集。

**Result:** 在DIOR-IOD和DOTA-IOD两个遥感增量目标检测基准数据集上，STAR-IOD分别以1.7%和2.1% mAP的优势超越当前最先进方法，有效缓解了灾难性遗忘，同时在基类和新类上均保持了较强的检测性能。

**Conclusion:** 本文揭示了类内尺度变化和标注缺失是遥感增量目标检测中知识蒸馏失效和旧类遗忘的关键原因，所提出的STD和CPG模块分别从拓扑蒸馏和自适应伪标签生成角度提供了有效解决方案，为遥感领域增量学习研究提供了新的思路和基准数据集。
...
### [36] [ArchSIBench: Benchmarking the Architectural Spatial Intelligence of Vision-Language Models](https://arxiv.org/abs/2605.20837)
*Qirui Shen, Wenda Wang, Jiachen Lu, Zilong Huang, Jin Bai, Lei He, Hongxuan Chen, Weixin Huang*

#### 🧩 TL;DR
提出了ArchSIBench，一个基于建筑学、认知科学和心理学视角构建的建筑空间智能基准，涵盖感知、推理、导航、变换和配置五个核心维度共17个细粒度子任务，通过3000个专家标注的问答对评估VLMs，发现大多数模型与人类基线存在显著差异，尤其在空间变换和配置推理方面与受过建筑训练的人类评估者之间仍有明显差距。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究虽已评估视觉语言模型（VLMs）的基本空间能力（如相对方向、距离比较、物体计数），但这些任务仅覆盖空间认知的最基础层次，忽略了高层级的建筑空间认知能力，包括布局理解、动线分析和功能分区等，缺乏系统评估VLMs建筑空间智能的基准。

**Method:** 提出了ArchSIBench基准，从建筑学、认知科学和心理学视角出发，涵盖感知、推理、导航、变换和配置五个核心维度共17个细粒度子任务，由具有建筑背景的专家人工标注构建了3000个问答对，用于系统评估VLMs的建筑空间智能。

**Result:** 基于ArchSIBench评估多种VLMs发现：大多数模型的建筑空间智能与人类基线存在显著差异，且模型在不同能力维度上表现差异较大；部分最先进模型可接近未经建筑训练的人类评估者水平，但与受过建筑训练的人类评估者相比，尤其在空间变换和配置推理方面仍有明显差距。

**Conclusion:** ArchSIBench为衡量和提升VLMs的建筑空间智能提供了重要见解和系统化资源，揭示了当前模型在高阶空间认知任务上的不足，并为未来研究指明了提升方向，数据集和代码已开源。
...
### [37] [SpineContextResUNet: A Computationally Efficient Residual UNet for Spine CT Segmentation](https://arxiv.org/abs/2605.20760)
*K S Nithurshen, Saurabh J. Shigwan*

#### 🧩 TL;DR
SpineContextResUNet是一种轻量级3D残差U-Net，通过并行多膨胀卷积的上下文模块实现高效脊柱CT分割，在VerSe2020和CTSpine1K上分别达到88.17%和88.13%的Dice分数，能在资源受限设备（如8GB RAM、Nvidia Jetson Orin Nano）上鲁棒推理，解决了Transformer方法在低资源场景下的内存瓶颈问题。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于Transformer或大规模集成的脊柱CT分割方法依赖大量GPU资源，在资源受限的临床环境或边缘设备上难以部署；同时，缩小的Transformer因缺乏空间归纳偏置在小数据量下性能严重下降，亟需一种计算高效且保持高精度的轻量级分割模型。

**Method:** 提出SpineContextResUNet，一种基于3D残差U-Net的轻量级架构，核心是集成了轻量级上下文模块（Context Block），该模块通过并行多膨胀卷积捕获长距离解剖依赖关系，避免了RNN的高延迟和自注意力机制的高内存开销，整体参数量约1.7M。

**Result:** 在VerSe2020和CTSpine1K两个公开基准上分别达到88.17%和88.13%的Dice分数；在与参数量匹配的SwinUNETR对比中，CNN方法保持高精度而Transformer严重退化；重型基线TotalSegmentator在消费级硬件（Intel Core i5, 8GB RAM）上因内存耗尽失败，而本模型能完成鲁棒推理。

**Conclusion:** 该工作表明，在严格硬件约束下，精心设计的CNN架构比缩小的Transformer更具优势，为脊柱CT分割在边缘平台（如Nvidia Jetson Orin Nano）和即时诊断场景中的部署提供了可行的解决方案。
...
### [38] [USV: Towards Understanding the User-generated Short-form Videos](https://arxiv.org/abs/2605.20838)
*Haoyue Cheng, Su Xu, Liwei Jin, Wayne Wu, Chen Qian, Limin Wang*

#### 🧩 TL;DR
本文提出了USV，一个专注于用户生成短视频的大规模视频理解数据集，包含约224K个视频，并针对高层语义理解建立了主题识别和视频-文本检索两项任务，同时提出了MMF-Net和VTCL两种基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有大规模视频数据集主要关注传统长视频，新涌现的用户生成短视频（UGC短视频）研究不足；同时，现有视频理解工作大多聚焦于实例级识别，难以有效学习视频的高层语义信息表示。

**Method:** 构建了USV数据集，从UGC平台通过标签查询收集约224K个视频，无需额外人工验证和修剪；针对主题识别任务提出多模态融合网络（MMF-Net），针对视频-文本检索任务提出视频-文本对比学习（VTCL），并在数据集上建立了全面基准。

**Result:** 在USV数据集上完成了主题识别和视频-文本检索两项任务的全面基准测试，验证了所提基线方法的有效性，具体量化结果详见原文及项目页面。

**Conclusion:** USV数据集填补了UGC短视频在高层语义理解方面的空白，所提任务和基线方法为未来短视频理解研究提供了标准化基准和可参考的解决框架。
...
### [39] [Diffuse to Detect: Bi-Level Sample Rebalancing with Pseudo-Label Diffusion for Point-Supervised Infrared Small-Target Detection](https://arxiv.org/abs/2605.20766)
*Zhu Liu, Yuanhang Yao, Ping Qian, Zihang Chen, Risheng Liu*

#### 🧩 TL;DR
提出一种物理启发的点标注框架Diffuse-to-Detect，利用热辐射与热扩散的内在一致性将单点标签扩展为可靠伪掩码，并结合双层双更新机制联合优化检测器、样本权重与扩散参数，在红外小目标检测中实现五倍标注加速和仅用30%训练数据即可达到可比性能。

---

#### 📘 Detailed Summary
**Motivation:** 点监督虽为红外小目标密集标注提供了可扩展方案，但受限于两个耦合瓶颈：低对比度红外图像中伪标签演化不稳定以及严重的样本分布不平衡，现有方法难以在复杂红外场景中自适应且稳定地学习。

**Method:** 提出物理驱动的标注策略，利用热辐射模式与热扩散的内在一致性将单点标签扩展为可靠伪掩码；在此基础上构建双层双更新框架，同时优化检测器权重、样本权重与扩散参数，其中元分类器动态预测样本级损失权重，可微扩散模块结合检测反馈迭代精化伪标签，实现训练与超参数优化的自适应交互。

**Result:** 在多个数据集上的大量实验表明，该方法实现了五倍的标注加速，检测精度显著提升，且仅用30%的训练数据即可达到与全监督方法相当的性能，验证了框架的高效性与实用性。

**Conclusion:** 该工作通过物理启发的点标注与双层联合优化范式，有效解决了点监督在红外小目标检测中的伪标签不稳定和样本不平衡问题，为低资源场景下的高精度检测提供了一种高效实用的解决方案，代码已开源。
...
### [40] [VISTA: Technical Report for the Ego4D Short-Term Object Interaction Anticipation at EgoVis 2026](https://arxiv.org/abs/2605.20901)
*Qiaohui Chu, Haoyu Zhang, Yisen Feng, Meng Liu, Weili Guan, Dongmei Jiang, Liqiang Nie*

#### 🧩 TL;DR
VISTA 提出一种结合 V-JEPA 时序特征与 Faster R-CNN 空间检测的 StillFast 框架，用于 Ego4D 短时物体交互预测，在 EgoVis 2026 STA 挑战赛中取得第一名。

---

#### 📘 Detailed Summary
**Motivation:** 第一人称视频中的短时物体交互预测任务需要同时预测未来交互物体的边界框、名词类别、动词类别、接触时间及置信度，现有方法难以有效融合高层空间检测信息与短时序上下文。

**Method:** VISTA 采用 StillFast 架构，空间支路使用在 COCO 上预训练的 Faster R-CNN ResNet-50 FPN 从最后一帧高分辨率图像生成目标提议；时序支路使用冻结的 V-JEPA 2.1 提取视频片段的时序表征，并通过特征调制和 ROI 级上下文融合将其注入检测分支；融合后的提议特征输入多头 STA 预测器，分别执行边界框精修、名词分类、动词分类、接触时间回归和交互置信度估计，最终通过集成互补预测提升鲁棒性。

**Result:** 在 EgoVis 2026 Ego4D STA 挑战赛官方测试服务器上取得第一名。

**Conclusion:** 实验结果表明，结合冻结的 V-JEPA 时序特征与基于 Faster R-CNN 的空间检测的 StillFast 框架能够有效解决第一人称短时交互预测问题，多头预测器联合优化多个交互属性是可行的，集成策略可进一步提升预测鲁棒性。
...
### [41] [Findings of the Counter Turing Test: AI-Generated Image Detection](https://arxiv.org/abs/2605.20787)
...
#### 🧩 TL;DR
本文介绍了 Defactify 4.0 研讨会提出的反图灵测试（CT2）挑战赛，旨在检测 AI 生成图像并识别其生成模型，基于包含 50,000 张合成图像和真实图像的 MS COCOAI 数据集，结果显示二分类可达到较高准确率（F1 > 0.83），但生成模型溯源任务仍具挑战（最高 F1 仅 0.4986）。

---

#### 📘 Detailed Summary
**Motivation:** 生成式 AI 技术（如 Stable Diffusion、DALL-E、Midjourney）的快速发展使得合成视觉内容的检测成为紧迫问题，现有方法在应对日益逼真的 AI 生成图像时面临挑战，且缺乏对生成模型来源的精确识别能力，亟需系统性的检测基准和评估框架。

**Method:** 论文基于 Defactify 4.0 研讨会提出反图灵测试（CT2）挑战赛，包含两项任务：AI 生成图像与真实图像的二元分类，以及识别 AI 生成图像所对应的具体生成模型。为此构建了 MS COCOAI 数据集，包含 50,000 张来自多个生成模型的合成图像及 MS COCO 真实图像。参赛者采用了卷积神经网络（CNN）、Vision Transformer（ViT）、频域分析、对比学习及多模态技术等多种检测策略。

**Result:** AI 生成图像检测的二分类任务取得了较高性能（F1 分数 > 0.83），但识别具体生成模型的任务仍然困难，最高 F1 分数仅为 0.4986，说明模型溯源能力显著不足。

**Conclusion:** 研究表明现有检测方法在区分 AI 生成与真实图像方面表现良好，但在模型指纹识别（model fingerprinting）方面仍存在明显瓶颈，亟需提升对抗鲁棒性和实时检测能力，为未来 AI 生成内容检测与溯源研究指明了改进方向。
...
### [42] [Focus-then-Context: Subject-Centric Progressive Visual Token Reduction for Vision-Language Models](https://arxiv.org/abs/2605.20950)
*Yulin Zhao, Yun Wang, Dehua Zheng, Borui jiang, Zheng Zhang*

#### 🧩 TL;DR
本文提出SPpruner，一种模拟人类视觉感知系统"先聚焦后上下文"机制的主题渐进式视觉令牌缩减范式，通过显式建模视觉显著性与语义关联性来挖掘完整视觉主题谱系，并聚合邻域上下文线索恢复全局关系依赖，在Qwen2.5-VL和LLaVA上分别实现2.53倍加速和67% FLOPs缩减，准确率几乎无损。

---

#### 📘 Detailed Summary
**Motivation:** 视觉语言模型（VLM）在推理时因大量视觉令牌序列带来高昂计算开销，现有令牌缩减方法虽缓解了该负担，但无意中仅保留了与用户查询严格对齐的孤立视觉主体，未能充分探索显著主体及其上下文关系，导致视觉信息表征不完整。

**Method:** 本文提出SPpruner，一个基于主题的渐进式缩减范式，模拟人类视觉感知系统的"Focus-then-Context"机制。首先构建聚焦识别模块，显式建模视觉显著性与语义相关性之间的交互，挖掘完整的视觉主题谱系以保持视觉输入的高保真表征；随后开发上下文感知结构扫描模块，从邻域区域聚合上下文线索，恢复全局关系依赖以维护保留主题的结构完整性。

**Result:** 大量实验表明，SPpruner在多个基准上一致超越现有最佳方法。在Qwen2.5-VL上，仅保留22.2%的视觉令牌即可实现2.53倍推理加速；在LLaVA上，实现67%的FLOPs缩减，同时准确率仅下降0.6%。

**Conclusion:** SPpruner通过显式建模视觉主体及其上下文关系，在显著降低VLM推理计算开销的同时保持高保真的视觉表征和全局结构完整性，验证了模仿人类视觉感知机制在令牌缩减中的有效性，为高效VLM推理提供了新的范式方向。
...
### [43] [What Semantics Survive the Connector? Diagnosing VLM-to-DiT Alignment in Video Editing](https://arxiv.org/abs/2605.20795)
*Hangyu Lin, Chao Wen, Chengming Xu, Jianxiong Gao, Jiangning Zhang, Xiaobin Hu, Yanwei Fu*

#### 🧩 TL;DR
本文对基于Flow Matching和VLM的指令驱动视频编辑中VLM到DiT的对齐机制进行系统诊断，发现该对齐并非无损语义传递，而是存在严重的语义瓶颈，会显著退化细粒度结构语义信息，推翻了现有范式的核心假设。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于Flow Matching的视频编辑模型通过连接器模块将VLM的多模态推理与DiT的文本嵌入空间对齐，但这一对齐过程是否真正实现了无损语义传递尚未得到严格验证；端到端评估难以区分对齐失败与生成错误，且自然数据集缺乏解耦标注，导致对该语义瓶颈的存在性及严重程度缺乏系统性诊断。

**Method:** 作者提出基于视频合成的受控数据处理流水线，构建了专注于关系编辑的诊断数据集TRACE-Edit；并设计了一套全面的诊断协议，分别对现有视频编辑模型中元查询（meta-query）和连接器（connector）两个关键设计进行独立分析，从而解耦对齐失败与生成误差。

**Result:** 在四个代表性模型案例上的系统评估表明，细粒度结构语义在VLM到DiT的对齐过程中会严重退化，验证了该对齐机制是语义瓶颈而非无损传递通道，且连接器与元查询的设计选择对退化程度有显著影响。

**Conclusion:** 研究发现推翻了VLM到DiT对齐能够实现无损语义转移的普遍假设，明确指出了该对齐是现有指令驱动视频编辑模型的主要瓶颈，并为未来多模态对齐架构的设计提供了新的诊断基础与评估范式。
...
### [44] [Finding the Correct Visual Evidence Without Forgetting: Mitigating Hallucination in LVLMs via Inter-Layer Visual Attention Discrepancy](https://arxiv.org/abs/2605.20965)
*Yutong Xie, Zhenglin Hua, Ran Wang, Wing W. Y. Ng, Xizhao Wang, Yuheng Jia*

#### 🧩 TL;DR
本文提出一种基于层间视觉注意力差异（ILVAD）的训练无关、即插即用的幻觉缓解方法，通过增强LVLM对正确视觉证据的注意力并减少生成过程中的视觉遗忘，在多个模型上一致降低了幻觉。

---

#### 📘 Detailed Summary
**Motivation:** 大型视觉-语言模型（LVLM）虽然在多模态任务上表现出色，但仍然容易产生与视觉内容不一致的幻觉响应。研究发现，幻觉的根本原因在于LVLM对正确视觉证据的注意力不足，且在生成过程中逐渐遗忘这些视觉信息。

**Method:** 提出基于层间视觉注意力差异（ILVAD）的幻觉缓解方法。首先从早期生成token到视觉token的跨层注意力权重中识别被重复激活的视觉token，构建视觉显著性图；然后在生成过程中通过显著性图增强对视觉证据的注意力以缓解视觉遗忘；最后利用显著性图获取生成文本到视觉证据的注意力分数，选择和强调与视觉证据强相关的文本token。该方法无需训练，即插即用。

**Result:** 在五个最近发布的、覆盖不同架构的LVLM上进行了多项基准评估，结果表明该方法能够一致地缓解幻觉现象，说明ILVAD具有良好的跨模型泛化能力。

**Conclusion:** LVLM的幻觉与视觉证据的注意力不足和生成中的视觉遗忘密切相关，且正确视觉证据在不同层中的注意力存在显著差异；通过层间注意力差异构建显著性图来增强视觉注意力是一种有效且通用的幻觉缓解方案，无需修改模型参数即可直接应用于现有LVLM。
...
### [45] [Decomposing Subject-Driven Image Generation via Intermediate Structural Prediction](https://arxiv.org/abs/2605.20807)
*Hanzhong Guo, Yizhou Yu*

#### 🧩 TL;DR
提出了一种两阶段框架，通过将结构（Canny边缘图）与外观解耦来实现主体驱动的文本到图像生成，并构建了100k规模文本感知数据集以提升文字处理能力，在保留高频身份细节（如标志、图案和文字）方面优于现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有主体驱动文本到图像生成方法直接在RGB空间操作，在较大编辑幅度下难以保留标志、图案和文字等高频率身份细节，导致细节退化严重。

**Method:** 提出两阶段框架：第一阶段预测Canny边缘图以捕捉结构信息，第二阶段在源外观和预测结构的共同条件下渲染最终图像，从而将结构与外观解耦。此外，构建了一个全自动流水线，生成了包含10万对图像的文本感知数据集，并引入跨视图文本一致性。

**Result:** 实验采用GPT-4.1评估和知识蒸馏研究，结果表明所提方法在保持高保真身份细节方面显著优于选定的基线方法，验证了中间结构预测的有效性。

**Conclusion:** 中间结构预测是实现高保真主体驱动生成的有效路径，解耦结构与外观的策略能够显著改善高频细节保持能力；数据集和代码将开源以推动后续研究。
...
### [46] [Grounding Driving VLA via Inverse Kinematics](https://arxiv.org/abs/2605.21061)
*Junsung Park, Hyunjung Shim*

#### 🧩 TL;DR
本文指出现有驾驶视觉-语言-动作模型因任务公式化缺陷而忽视视觉特征，通过将驾驶VLA重新设计为逆运动学求解器——引入未来视觉状态预测目标和逆运动学网络——使得0.5B参数模型达到超越7B-8B模型的轨迹规划性能，恢复了视觉特征的有效利用。

---

#### 📘 Detailed Summary
**Motivation:** 现有驾驶VLA在预测轨迹时几乎完全忽略了自身的视觉token，作者通过逆运动学理论分析发现，这一问题并非源于训练不足，而是任务公式化的结构性缺陷：轨迹恢复需要当前和未来两个视觉状态作为边界条件，但现有方法仅提供当前视觉状态，导致模型通过自我状态和文本命令形成捷径。

**Method:** 作者将驾驶VLA重新设计为逆运动学求解器，包含两个核心组件：一是下一视觉状态预测目标，要求LLM预测未来视觉场景，提供密集视觉监督并抑制捷径路径；二是逆运动学网络，即基于交叉注意力机制的条件扩散模型，仅以当前和未来视觉状态为输入，在轨迹解码过程中抑制对自我状态和文本捷径的依赖。

**Result:** 该0.5B参数模型在闭环NAVSIM-v2和nuScenes两个基准上，轨迹规划性能达到甚至超过7B-8B参数的VLA模型（规模大一个数量级以上），并恢复了视觉定位能力。广泛分析表明，性能提升源于模型恢复了利用视觉特征的能力，该效果在转弯等动态驾驶场景中最为显著。

**Conclusion:** 本研究揭示了现有驾驶VLA忽视视觉特征的根本原因在于任务公式化缺陷，并通过逆运动学视角的重新设计，证明了在极小模型规模下通过正确的任务构建即可实现大规模模型才能达到的规划性能，为驾驶VLA的结构设计提供了新的理论指导方向。
...
### [47] [RelWitness: Open-Vocabulary 3D Scene Graph Generation with Visual-Geometric Relation Witnesses](https://arxiv.org/abs/2605.20823)
*Minh Anh Nguyen, Quang Huy Tran, Bao Ngoc Le, Tuan Kiet Pham, Sui Yang Guang*

#### 🧩 TL;DR
提出RelWitness框架，通过关系见证者（relation witness）机制从未完全标注的RGB-D序列中生成开放词汇3D场景图，利用视觉-几何线索验证未标注关系，缓解关系监督不完整问题。

---

#### 📘 Detailed Summary
**Motivation:** 开放词汇3D场景图生成面临两大挑战：词汇扩展问题与关系标注不完整问题。现有3D场景图数据集中的关系标注具有选择性，大量有效的物体对关系未被标注，导致模型难以从部分标注中正确学习关系识别。

**Method:** 提出RelWitness框架，核心概念为关系见证者（relation witness），即使关系在场景中可观察的具体视觉-几何线索。定义多种关系类型（支撑、包含、邻近、朝向等）的见证条件，从RGB视图、深度图、重建3D几何、角色敏感文本、物体先验空视图和多视角一致性构造见证记录。设计视觉-几何见证验证器将未标注关系候选分类为验证阳性、可靠阴性或不确定未标注样本，并采用见证引导的正-无标记目标函数从不完整标注中学习，同时引入见证一致性解码和RGB-D缺失关系审计协议。

**Result:** 在3DSSG/3RScan和ScanNet衍生开放词汇数据集上的模拟实验表明，该方法在未见关系识别、见证精确率、幻觉减少和冗余关系短语降低方面均优于基线。所有数值结果均为规划值，需替换为复现测量结果。

**Conclusion:** 通过显式建模关系的视觉-几何见证线索并设计正-无标记学习框架，RelWitness有效缓解了不完全关系监督下的开放词汇3D场景图生成问题，在减少幻觉和冗余关系方面展现出优势，为处理稀疏标注的3D场景理解任务提供了新范式。
...
### [48] [Comparative Analysis of Military Detection Using Drone Imagery Across Multiple Visual Spectrums](https://arxiv.org/abs/2605.21157)
*Sourov Roy Shuvo, Prajwal Panth, Rajesh Chowdhury, Sorup Chakraborty, Sudip Chakrabarty, Prasant Kumar Pattnaik*

#### 🧩 TL;DR
本文针对无人机军事目标检测中现有数据集缺乏真实场景多样性的问题，通过构建灰度、热成像、夜视和微光四种模拟真实环境的变体数据集，并基于YOLOv11-small模型进行训练与评估，提升了无人机在复杂作战条件下的目标检测性能与可靠性。

---

#### 📘 Detailed Summary
**Motivation:** 现有KIIT-MiTA无人机军事目标检测数据集虽然涵盖了多种军事场景，但未充分考虑真实作战中可能遇到的低可见度、夜间条件、热成像等多样化环境因素，导致模型在实际复杂场景下的泛化能力和鲁棒性不足，亟需评估并提升检测系统在变化环境下的表现。

**Method:** 研究从KIIT-MiTA数据集出发，人工构建了四种模拟真实环境的变体数据集：Gray Scale（灰度）、Thermal Vision（热成像）、Night Vision（夜视）和Obscura Vision（微光/遮蔽视觉），分别对应低可见度、热基图像、夜间等条件。采用YOLOv11-small模型对上述数据集进行目标检测训练与评估。

**Result:** 论文通过在四种模拟真实环境的数据集上训练YOLOv11-small模型，系统评估了模型在不同军事作战场景（低可见度、热成像、夜间、微光）下的目标检测性能，结果表明该方法能有效提升无人机在复杂环境中的检测能力。

**Conclusion:** 本研究通过构建多样化环境数据集并验证YOLOv11-small模型的适应性，显著增强了无人机在攻防任务中目标检测系统的性能和可靠性，为无人机在真实复杂战场环境中的部署提供了技术支持与数据基础。
...
### [49] [MONET: A Massive, Open, Non-redundant and Enriched Text-to-image dataset](https://arxiv.org/abs/2605.21272)
*Benjamin Aubin, Gonzalo Iñaki Quintana, Onur Tasar, Sanjeev Sreetharan, Urszula Czerwinska, Damien Henry, Clément Chadebec*

#### 🧩 TL;DR
MONET 是一个包含约1.049亿图文对的开源数据集，通过多阶段过滤、去重、重标注和合成增强构建，并用于训练4B参数潜扩散模型，达到了有竞争力的生成质量，降低了大规模可复现文本到图像研究的门槛。

---

#### 📘 Detailed Summary
**Motivation:** 训练大规模文本到图像模型需要高质量、多样化且带有详细描述的策展数据集，但收集、过滤、去重和重新标注如此规模的数据集成本高昂且流程复杂，阻碍了该领域的开放和可复现研究。

**Method:** 从29亿原始图文对中，经过安全过滤、基于领域的过滤、精确与近似去重、使用多种视觉语言模型进行短到长描述的重新标注，以及合成样本增强，最终构建出约1.049亿图文对的MONET数据集，并为每张图像提供预计算嵌入和注释以加速下游使用。

**Result:** 基于MONET数据集训练了一个40亿参数的潜扩散模型，在GenEval和DPG基准测试上取得了具有竞争力的分数，验证了该数据集在不牺牲生成质量的前提下能够有效支撑大规模可复现的文本到图像研究。

**Conclusion:** MONET证明了通过系统化的数据策展流程可以构建高质量的开放数据集，从而显著降低大规模、可复现文本到图像研究的数据门槛，为社区提供了Apache 2.0许可的开源数据资源。
...
### [50] [Closed Loop Dynamic Driving Data Mixture for Real-Synthetic Co-Training](https://arxiv.org/abs/2605.21372)
*Hongzhi Ruan, Pei Liu, Weiliang Ma, Zhengning Li, Xueyang Zhang, Jun Ma, Dan Xu, Kun Zhan*

#### 🧩 TL;DR
AutoScale提出了一种全自动闭环数据引擎，通过图正则化自编码器（Graph-RAE）进行驾驶场景表示、聚类感知梯度上升（Cluster-GA）进行簇级重要性估计与重加权、以及簇引导向量检索来筛选高价值样本，从而在有限训练预算下优化真实-合成数据混合，在NavSim上以更少的合成样本实现了优于朴素协同训练和跨域基线的性能。

---

#### 📘 Detailed Summary
**Motivation:** 现实驾驶数据标注成本高且存在场景偏差，而利用近乎无限的合成数据进行真实-合成协同训练虽具前景，但直接引入全部合成数据效率低下且会导致分布偏移；在有限训练预算下优化数据混合的组成（场景类型与数量）仍是关键且未被充分探索的问题。

**Method:** 本文将训练数据混合近似为一种动态优化过程，通过闭环评估反馈迭代调整数据混合以最大化模型性能，并提出了全自动闭环数据引擎AutoScale，其核心组件包括：用于驾驶场景表示的图正则化自编码器（Graph-RAE）、用于簇级重要性估计与重加权的聚类感知梯度上升（Cluster-GA）、以及基于簇引导的向量检索来筛选高价值样本。

**Result:** 在NavSim基准上的实验表明，AutoScale在有限预算下以更少的合成样本取得了优于朴素协同训练和跨域基线的性能。

**Conclusion:** 本工作表明，在自动驾驶端到端学习中，通过结构化场景表示和闭环反馈驱动的自动化数据混合优化，能够有效缓解真实-合成数据协同训练中的分布偏移与效率问题，为有限预算下的数据混合策略提供了明确指导。
...
### [51] [Ordering Matters: Rank-Aware Selective Fusion for Blended Emotion Recognition](https://arxiv.org/abs/2605.21417)
*Junghyun Lee, Hyunseo Kim, Hanna Jang, Junhyug Noh*

#### 🧩 TL;DR
提出了一种基于排序感知的多编码器融合框架，通过注意力门控选择最优编码器子集、解耦预测为存在性与显著性头并进行概率级融合，结合无标注域自适应，在BlEmoRE挑战赛中取得第二名，有效提升了混合情感识别的细粒度性能。

---

#### 📘 Detailed Summary
**Motivation:** 混合情感识别面临的核心挑战在于情感往往以多种微妙且重叠的多模态线索混合表达，而非单一主导信号，现有方法难以有效选择和融合来自异构视频与音频编码器的互补表征，且对分布偏移的鲁棒性不足。

**Method:** 提出排序感知多编码器框架：首先将异构编码器特征投影至共享隐空间，通过基于注意力的门控模块估计每个样本的编码器重要性，仅融合top-n个最有效的编码器；为建模混合情感，将预测解耦为存在性（presence）与显著性（salience）两个头并通过概率级融合对齐；此外引入无需伪标签的特征级无监督域自适应以提升分布偏移下的鲁棒性。

**Result:** 在BlEmoRE挑战赛上，该框架显著优于单一强编码器及朴素多编码器融合基线，最终系统在比赛中排名第二，验证了排序感知选择性融合在细粒度混合情感识别中的有效性。

**Conclusion:** 研究表明，对多编码器进行排序感知的选择性融合、解耦预测头以及无监督域自适应是提升混合情感识别性能的有效技术路径，为处理多模态情感分析中的异构特征融合与分布泛化问题提供了新的思路。
...
### [52] [RISE: Reliable Improvement in Self-Evolving Vision-Language Models](https://arxiv.org/abs/2605.20914)
*Chaoran Xu, Yingmao Miao, Pengfei Zhang, Hao Dou, Lei Sun, Xiangxiang Chu*

#### 🧩 TL;DR
RISE 提出了一种基于细粒度角色交替、质量监督器与技能感知动态平衡的可信自进化框架，使视觉-语言模型能够在无标注图像上通过自问自答闭环实现持续且可靠的性能提升，在七个基准上验证了其有效性。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉-语言模型（VLM）的后训练严重依赖大规模人工构建的监督数据，成本高昂且难以扩展，尤其是在推理密集型多模态任务中。自进化学习虽能通过模型自身的提问-求解闭环实现自我改进，但当前方法面临角色交替粗粒度导致交互延迟、生成问题质量逐渐下降、以及问题类型分布坍缩三大挑战，限制了自进化的效率与可靠性。

**Method:** RISE 框架包含三个核心设计：细粒度角色交替通过缩短提问者与求解者之间的反馈循环以提升交互效率；质量监督器用于提高生成问题的有效性与伪标签的可靠性；技能感知动态平衡通过维护技能覆盖范围来缓解模式坍缩，确保进化过程中问题类型的多样性。三者协同使模型能够从未标注图像中实现可靠的自进化。

**Result:** 在两种 VLM 骨干网络和七个基准上的实验结果表明，RISE 能够持续稳定地提升基础模型性能，取得广泛且持久的增益，验证了其在多模态推理任务中的自进化有效性。

**Conclusion:** RISE 通过系统性地解决自进化中的反馈延迟、质量退化与模式坍缩问题，为 VLM 在不依赖人工标注条件下的持续自我改进提供了可靠框架，未来可进一步探索其在更大规模模型与更复杂多模态任务中的推广潜力。
...
### [53] [TempGlitch: Evaluating Vision-Language Models for Temporal Glitch Detection in Gameplay Videos](https://arxiv.org/abs/2605.21443)
*Yakun Yu, Ashley Wiens, Adrián Barahona-Ríos, Benedict Wilkins, Saman Zadtootaghaj, Nabajeet Barman, Cor-Paul Bezemer*

#### 🧩 TL;DR
提出TempGlitch基准，系统评估视觉语言模型在视频游戏时间维度故障检测上的表现，发现现有模型在该任务上几乎随机，表现不稳定且无法通过增加帧采样或模型规模有效解决。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉语言模型(VLM)在游戏故障检测评估中仅将故障视为静态视觉异常，忽略了时间维度故障——即仅通过跨帧时序变化才能察觉的故障类型，导致这一关键场景缺乏系统化的评估基准。

**Method:** 构建TempGlitch基准，包含五种时间故障类型、每类样本均衡的受控游戏视频，并配套无故障视频以实现可靠的二分类评估；在此基础上对12种开源与闭源VLM进行多帧采样设置下的系统性评测。

**Result:** 当前VLM在TempGlitch上表现接近随机水平，倾向于两种失败模式：过于保守而漏检大多数故障，或过于敏感而将无故障视频误报为故障；增加帧采样密度和模型规模均无法可靠地缓解上述问题。

**Conclusion:** TempGlitch为时间推理、稳健的游戏理解以及VLM自动化故障检测提供了一个聚焦的测试平台，揭示了现有模型在时序感知上的显著不足，为未来研究指明了关键改进方向。
...
### [54] [3D Reconstruction and Knowledge Distillation to Improve Multi-View Image Models to Explore Spike Volume Estimation in Wheat](https://arxiv.org/abs/2605.20940)
*Olivia Zumsteg, Jannis Widmer, Yann Bourdé, Norbert Kirchgessner, Andreas Hund, Lukas Roth, Paraskevi Nousi*

#### 🧩 TL;DR
提出一种混合2D-3D知识蒸馏方法，在训练阶段利用3D点云网络的几何信息指导2D Transformer，实现纯图像推理的高效小麦穗体积估计，将推理时间从160ms降至1.4ms每穗，同时提升精度和相关性。

---

#### 📘 Detailed Summary
**Motivation:** 小麦穗体积的准确估计对产量分析和胁迫韧性评估至关重要，但田间测量困难；主动3D传感（LiDAR、ToF）易受植物运动影响或不适于室外环境，3D重建计算成本高，而纯2D图像方法缺乏几何信息，因此需要一种兼具几何精度与计算效率的方法。

**Method:** 提出混合2D-3D知识蒸馏框架：首先训练一个基于距离直方图特征的刚体不变点云网络以获取姿态鲁棒的几何表示；然后将该3D模型与多视图图像调节Transformer（RT）组合成集成架构；最后通过特征级或标签级蒸馏将集成知识迁移至纯图像的学生模型RT中。

**Result:** 两种蒸馏后的RT将MAE从非蒸馏RT的654.31 mm³分别降至639.93 mm³和644.62 mm³，相关性从0.76提升至0.77和0.82；推理时间从160ms每穗降至1.4ms每穗；蒸馏还减轻了体积相关偏差并重塑图像模型隐层表征朝向几何感知形状。

**Conclusion:** 结果表明，利用3D信息训练的2D Transformer可实现可扩展且高效的小麦穗体积估计，为高通量田间表型分析提供了兼顾几何精度与计算效率的有效路径，并且知识蒸馏能够有效改善纯图像模型的几何感知能力。
...
### [55] [Bridging Structure and Language: Graph-Based Visual Reasoning for Autonomous Road Understanding](https://arxiv.org/abs/2605.20942)
*Lena Wild, Katie Z Luo, Marco Pavone*

#### 🧩 TL;DR
提出Combined Road Substrate (CRS)框架，通过图基表示将几何道路结构与开放词汇语义联合执行，使小参数VLM在结构化道路推理中超越大模型，揭示道路理解的核心瓶颈是结构化监督而非模型规模。

---

#### 📘 Detailed Summary
**Motivation:** 现有方法在结构化道路理解中存在根本性矛盾：视觉-语言模型(VLM)具备语义灵活性但缺乏几何与关系基础，而传统模块化系统（如HD地图、拓扑道路图）提供结构精度但语义僵化，亟需一种统一表示来弥合几何精度与开放词汇语义之间的鸿沟。

**Method:** 提出Combined Road Substrate (CRS)，一种图基框架，将道路几何结构与开放词汇语义联合编码为单一可执行表示；通过递归图查询自动生成组合复杂且语言多样化的问答对，引入"grounding for free"机制确保对地图元素的逻辑可追溯性，并程序化提取链式思维监督轨迹以增强推理能力。

**Result:** 在结构化道路推理任务中，包括大型闭源模型在内的SOTA VLM均表现困难；仅需20至80个CRS增强场景即可在2B或4B参数的小模型上获得组合推理任务的稳定性能提升；行为分析显示，基线模型失败于关系场景理解，而CRS训练后的模型将失败归因缩小至属性识别层面。

**Conclusion:** 结构化道路推理的核心瓶颈并非模型规模，而是结构化监督的缺失；图基表示与可验证推理轨迹的结合能够有效将小模型的推理失败从关系级降级为属性级，为提升自动驾驶场景下的结构化理解能力指明了以数据表示而非模型扩展为核心的方向。
...
### [56] [WikiVQABench: A Knowledge-Grounded Visual Question Answering Benchmark from Wikipedia and Wikidata](https://arxiv.org/abs/2605.21479)
*Basel Shbita, Pengyuan Li, Anna Lisa Gentile*

#### 🧩 TL;DR
WikiVQABench是一个人工精选的基于知识的VQA基准数据集，通过系统组合维基百科图像、标题和结构化知识构建，利用LLM生成候选题目并经过人工审核，覆盖256M-90B参数的15个VLM评估显示准确率从24.7%到75.6%不等，有效区分了模型在知识密集型推理上的能力差异。

---

#### 📘 Detailed Summary
**Motivation:** 现有VQA基准大多强调仅依赖视觉内容即可解决的感知型任务，而现实场景中许多问题需要图像中不可直接观察的外部知识才能正确回答，存在对知识型VQA进行系统性评估的研究空白。

**Method:** 论文通过系统组合维基百科图像、文章标题和Wikidata结构化知识来构建基准；使用大型语言模型（LLM）生成候选多项选择题（图像-问题-答案集）；所有生成实例经人工标注者审核以保证事实正确性、视觉-文本一致性以及问题需结合外部知识与视觉证据才能正确回答。

**Result:** 在15个视觉语言模型（参数规模256M-90B）上的评估显示准确率范围从24.7%到75.6%，表明该基准能有效区分模型在知识密集型推理中的能力差异。

**Conclusion:** WikiVQABench作为系统构建的知识型VQA基准，能够有效衡量模型在需要外部知识的视觉推理任务上的表现，揭示了当前VLM在知识密集型任务上存在显著能力差距，数据集和代码已公开以推动相关研究方向。
...
### [57] [Preserve, Reveal, Expand: Faithful 4D Video Editing with Region-Aware Conditioning](https://arxiv.org/abs/2605.20961)
*Zhangchi Hu, Wenzhang Sun, Xiangchen Yin, Jiahui Yuan, Chunfeng Wang, Hao Li, Kun Zhan, Xiaoyan Sun*

#### 🧩 TL;DR
提出PREX，一种区域感知的4D视频扩散编辑框架，通过将目标时空体分解为保留、揭示和扩展三种角色并注入具有校准置信度的观测线索，解决了4D编辑中的证据-角色不匹配问题；同时引入PREBench诊断基准，实验表明该方法在保持视觉质量和4D控制能力的同时显著减少了区域结构化失败。

---

#### 📘 Detailed Summary
**Motivation:** 现有4D驱动视频扩散模型主要关注合理生成，但在忠实4D编辑中需要保留源观测区域同时合成被遮挡或视外内容，存在"证据-角色不匹配"问题：可靠的源观测证据、不可靠的渲染线索和未支持区域被纠缠在单一条件信号中，导致保留漂移、鬼影和不稳定外推。

**Method:** 提出PREX（Preserve, Reveal, Expand）框架，将目标时空体根据观测支持和场景范围分解为保留、揭示和扩展三种角色；构建具有校准置信度的观测外观线索，通过区域感知适配器注入冻结的视频扩散骨干网络，使用代理任务训练而无需成对编辑视频；同时引入PREBench诊断基准，包含精心设计的编辑、区域角色掩模和与人类对齐的评估指标。

**Result:** 实验表明，PREX在减少区域结构化失败（如保留漂移、鬼影和不稳定外推）方面表现优异，同时保持了强大的视觉质量和4D编辑控制能力；PREBench提供了对全局视频质量和4D控制的补充评估。

**Conclusion:** 通过显式解耦目标时空体中的不同区域角色并注入置信度校准的观测线索，PREX有效解决了4D编辑中的证据-角色不匹配问题，为忠实4D编辑提供了一种无需配对数据的区域感知框架，并为该领域建立了更完善的诊断评估基准。
...
### [58] [CHOIR: Contact-aware 4D Hand-Object Interaction Reconstruction](https://arxiv.org/abs/2605.20992)
*Hao Xu, Yilin Liu, Yinqiao Wang, Chi-Wing Fu, Niloy J. Mitra*

#### 🧩 TL;DR
CHOIR是一个面向日常单目视频的接触感知手物交互（HOI）4D重建框架，通过将接触作为手与物体之间的显式耦合信号，在无需已知物体或精心设计场景的条件下，实现了从粗粒度初始化到空间纠正再到接触感知联合优化的端到端重建，在物体重建质量、物理合理性和时间一致性上均优于现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有手物交互（HOI）4D重建方法通常依赖已知物体模型或经过精心设计的场景，难以直接应用于日常开放世界的单目视频；此外，独立估计手部与物体姿态在遮挡、杂乱背景和未知物体几何形状下容易产生对齐误差，导致重建结果在物理和时间上不一致。本文旨在解决这一现实场景下的HOI重建难题，实现无需已知先验的可扩展交互挖掘。

**Method:** CHOIR框架包含三个阶段：首先利用开放世界的视觉先验初始化一个粗粒度、无接触感知的4D HOI序列；然后引入一个生成式HOI空间纠正模块，预测射线深度校正量并修正手与物体的相对空间位置，进而在纠正后的几何上推导出每帧的初始接触对应关系；最后通过一个动态更新接触约束的联合优化机制，同时保证几何一致性、时间一致性和接触一致性。

**Result:** 在受控场景和具有挑战性的真实视频上的实验表明，CHOIR在物体重建质量、物理合理性和时间一致性方面均优于现有最先进方法，验证了其将接触作为显式耦合信号在改善手物对齐和整体重建效果上的有效性。

**Conclusion:** CHOIR证明了在开放世界单目视频中利用接触作为耦合信号进行手物交互4D重建的可行性，为大规模真实交互数据挖掘以及面向场景的合成与规划提供了技术基础；未来可将该方法扩展至更复杂的多手或多物体交互场景。
...
### [59] [LiteViLNet: Lightweight Vision-LiDAR Fusion Network for Efficient Road Segmentation](https://arxiv.org/abs/2605.21007)
*Daojie Peng, Bingtao Wang, Fulong Ma, Liang Zhang, Jun Ma*

#### 🧩 TL;DR
LiteViLNet是一种轻量级多模态道路分割网络，通过双流编码器、深度可分离卷积、多尺度特征融合模块和大核桥接模块，在仅14.04M参数下以163.79 FPS实现96.36% MaxF，兼顾了高精度与实时性，适合部署于资源受限的嵌入式平台。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于多模态的道路分割方法通常依赖重型Transformer编码器以达到最优性能，但其巨大的计算开销使其无法在资源受限的嵌入式平台上进行实时部署，亟需一种兼具高精度与低计算量的轻量级方案。

**Method:** LiteViLNet设计了一个双流轻量级编码器结合深度可分离卷积，以极少的参数从RGB和LiDAR两种模态中提取层次化特征；提出多尺度特征融合模块（MSFM）实现不同层级间的跨模态交互，并设计大核桥接模块以线性复杂度捕获长程依赖关系。

**Result:** 在KITTI Road数据集上，LiteViLNet仅用14.04M参数即达到96.36% MaxF分数，在所有CNN方法中排名第一，性能可媲美更大的Transformer模型；在RTX 4060 Ti上推理速度达163.79 FPS（模型仅推理），在Jetson Orin NX上达22.18 FPS，显著优于众多重型方法。

**Conclusion:** LiteViLNet充分验证了在保持高精度的同时实现实时嵌入式部署的可行性，为自动驾驶和智能机器人系统中的道路分割任务提供了一种高效、实用的解决方案，具有良好的实际应用潜力。
...
### [60] [Multimodal LLMs under Pairwise Modalities](https://arxiv.org/abs/2605.21059)
*Yan Li, Yunlong Deng, Yuewen Sun, Gongxu Luo, Kun Zhang, Guangyi Chen*

#### 🧩 TL;DR
本文提出一种仅利用成对多模态数据即可训练多模态大语言模型的表示学习框架，通过理论分析可识别性条件，并设计两阶段方法（隐空间对齐与跨模态重组），在仅使用三个模态对的条件下实现了3D点云与触觉模态的有效集成。

---

#### 📘 Detailed Summary
**Motivation:** 当前多模态大语言模型的训练依赖人工精心构建的多路对齐数据集，需要大量人力且难以跨领域扩展，因此探索仅利用成对模态数据作为完整联合多模态分布的替代方案具有重要价值。

**Method:** 本文首先从理论上分析了仅观测成对模态时表示可识别性的条件，据此提出一个两阶段表示学习框架：第一阶段通过自模态重建与成对对比学习学习跨模态共享隐空间，并在对比学习中引入部分对齐与最小隐空间规范化的归纳偏置；第二阶段将新模态的编码器与预训练模态的解码器集成，实现跨模态迁移与生成。

**Result:** 在预训练的多模态大语言模型中新增3D点云与触觉模态，仅使用三个模态对进行训练，实验表明通过对齐的隐表示空间，模型取得了强跨模态性能。

**Conclusion:** 研究表明，通过理论指导的成对模态对齐与两阶段框架，可有效克服多路对齐数据依赖，实现多模态的高效扩展与跨模态迁移。
...
### [61] [Q-ARVD: Quantizing Autoregressive Video Diffusion Models](https://arxiv.org/abs/2605.21072)
*Siao Tang, Xinyin Ma, Gongfan Fang, Xingyi Yang, Xinchao Wang*

#### 🧩 TL;DR
本文提出Q-ARVD框架，针对自回归视频扩散模型（ARVD）的量化问题，解决了帧级量化灵敏度高度不均衡和权重中突出异质异常值两大挑战，通过最终质量感知的帧加权机制和异常值感知的自适应双尺度量化，实现了对ARVD的高效且准确的量化。

---

#### 📘 Detailed Summary
**Motivation:** 自回归视频扩散模型（ARVD）在流式视频生成中展现出潜力，但其推理成本高昂严重阻碍了实际部署，模型量化是提升效率的自然方向。然而，现有针对标准扩散Transformer的量化方案直接应用于ARVD效果不佳，因为ARVD的量化行为与双向扩散模型存在本质差异，ARVD量化领域尚缺乏专门研究。

**Method:** Q-ARVD框架包含两大关键技术：一是最终质量感知的帧加权机制，将量化目标中对各帧的损失进行加权以应对自回归生成中帧级量化灵敏度呈指数衰减式高度不均衡的问题；二是异常值感知的自适应双尺度量化，自动检测任意层中异常值通道的存在与数量并将其隔离，从而保护正常通道，以应对权重分布中突出且随层类型和块深度变化的异质异常值模式。

**Result:** 实验结果表明，Q-ARVD在多个基准上显著优于直接应用现有量化方案的效果，有效降低了ARVD的推理开销同时保持了生成质量，验证了所提方法在处理帧级灵敏度不均衡和异质异常值方面的优越性。

**Conclusion:** 该研究揭示了ARVD量化中独特的帧级灵敏度衰减模式和权重异质异常值结构，提出的Q-ARVD框架为高效部署ARVD提供了可行的量化方案，对推动实时交互视频生成和世界模型的实用化具有重要价值。
...
### [62] [R2AoP: Reliable and Robust Angle of Progression Estimation from Intrapartum Ultrasound](https://arxiv.org/abs/2605.21099)
*Yuanhan Wang, Yifei Chen, Beining Wu, Mingxuan Liu, Xiaotian Hu, Chunbo Jiang, Yijin Li, Changmiao Wang, Feiwei Qin, Qiyuan Tian*

#### 🧩 TL;DR
提出R2AoP框架，通过结构增强分割与置信度引导几何建模，实现产时超声中进展角（AoP）的可靠稳健估计，在多中心基准上显著降低AoP误差。

---

#### 📘 Detailed Summary
**Motivation:** 产时经会阴超声中进展角（AoP）的准确估计对客观评估产程进展至关重要，但现有方法对成像噪声、边界模糊以及局部分割误差的几何放大效应高度敏感，导致测量结果不稳定且可重复性差。

**Method:** R2AoP框架包含三个核心组件：三分支局部结构增强骨干网络用于改善耻骨联合（PS）和胎头（FH）的分割质量；置信度加权轮廓拟合方法显式抑制不可靠边界点在AoP计算中的影响；轻量级几何可靠的测试时自适应策略作为辅助组件，在无目标标注下实现稳定推理。

**Result:** 在多中心基准数据集上的广泛评估表明，与现有最先进的AoP方法相比，R2AoP在AoP误差和边界指标上均取得了持续一致的降低。

**Conclusion:** R2AoP通过结构信息引导的分割与置信度感知的几何建模有效解决了超声图像中噪声与边界歧义导致的AoP估计不稳定问题，其测试时自适应机制进一步提升了跨异构采集条件的泛化能力，为产程客观评估提供了可靠的计算工具。
...
### [63] [VersusQ: Pairwise Margin Reasoning for Generalizable Video Quality Assessment](https://arxiv.org/abs/2605.21130)
*Shibei Meng, Binxin Yang, Yuan Liu, Jiexuan Zhang, Zhengyao Lv, Hubery Yin, Qiang Xu*

#### 🧩 TL;DR
VersusQ提出了一种基于成对比较的边际推理框架用于视频质量评估，通过直接比较两个视频并预测连续边际分数来消除数据集特定的绝对评分偏差，结合Margin-Coupled GRPO优化实现了跨域鲁棒性和细粒度排序能力。

---

#### 📘 Detailed Summary
**Motivation:** 现有的大多模态模型在视频质量评估中采用逐点的绝对分数预测范式，这种监督方式容易将感知质量与数据集特定的标定偏差（如标注协议、评分习惯和分数分布）相混叠，导致学习到的评分规则在域间迁移时泛化能力差。

**Method:** VersusQ构建了一个完全基于直接比较的成对边际推理框架：利用LMM对两个视频进行视觉和时间质量差异的比较，输出有符号的连续边际分数以同时表征偏好选择和差异程度；同时提出Margin-Coupled GRPO，联合优化基于rollout的关系推理与连续边际回归，使可解释的比较依据与细粒度数值差异对齐。

**Result:** 在多个公开视频质量评估基准上的实验表明，VersusQ达到了最先进的性能，展现出强大的跨域泛化能力，并在异构评估场景下实现了可靠的细粒度排序。

**Conclusion:** 该研究证明了在视频质量评估中采用相对比较范式能够有效缓解绝对评分标定偏差问题，为构建跨域鲁棒的多模态质量评估模型提供了新思路，且成对边际推理与可解释性联合优化具有实际应用价值。
...
### [64] [SurgOnAir: Hierarchy-Aware Real-Time Surgical Video Commentary](https://arxiv.org/abs/2605.21132)
*Jingyi He, Yue Zhou, Long Bai, Kun Yuan, Nassir Navab, Yuan Bi*

#### 🧩 TL;DR
SurgOnAir是一种流式视觉-语言模型，能够在手术进行中逐帧处理视觉输入并渐进式生成层级化叙事文本，通过帧到令牌的细粒度生成和特殊过渡令牌机制，实现对外科工作流的实时理解与状态转换捕捉。

---

#### 📘 Detailed Summary
**Motivation:** 现有手术工作流理解方法大多为离线处理或粗时间尺度操作，仅在处理完视频片段后生成描述，无法对术中细微的器械运动和组织状态变化进行即时反应，限制了智能手术辅助系统的实时感知与决策能力。

**Method:** 提出SurgOnAir流式视觉-语言模型，以逐帧方式顺序处理图像且不访问未来帧，随视觉输入到达渐进式生成叙事令牌；构建了包含动作、步骤、阶段三级标注的层级化数据集SurgOnAir-11k；模型训练生成多层级文本响应以反映手术流程层次结构，并通过生成特殊过渡令牌显式标记工作流状态转换。

**Result:** 实验结果表明，SurgOnAir通过单一视觉-语言模型统一了多层级手术工作流的流式理解，生成了优于现有方法的层级感知叙事，实现了真正的实时理解能力。

**Conclusion:** SurgOnAir证明了流式视觉-语言模型在外科手术实时理解中的可行性，通过细粒度帧到令牌生成和显式过渡信号机制，为智能手术体统中的即时感知与响应提供了有效方案，代码和数据集将开源以供后续研究。
...
### [65] [Distill to Think, Foresee to Act: Cognitive-Physical Reinforcement Learning for Autonomous Driving](https://arxiv.org/abs/2605.21139)
*Yang Wu, Qiang Meng, Zhaojiang Liu, Youquan Liu, Jian Yang, Jin Xie*

#### 🧩 TL;DR
CoPhy提出了一种认知-物理强化学习框架，通过蒸馏VLM知识到BEV编码器实现零推理代价的认知能力，并构建自回归BEV世界模型用于行动后果预测，在NAVSIM基准上取得SOTA结果，同时支持用户自定义语言指令的灵活意图控制。

---

#### 📘 Detailed Summary
**Motivation:** 当前端到端自动驾驶模型受限于模仿学习的行为克隆上限，而强化学习虽然有望实现更智能的自主驾驶，但缺乏两项关键基础设施：能够理解交通语义和驾驶意图的认知基础，以及能够预测候选行动后果的前瞻性物理环境。

**Method:** CoPhy框架包含两个核心模块：一是认知蒸馏模块，将VLM知识蒸馏到BEV编码器中后完全丢弃VLM，实现零推理代价的认知能力，并释放认知通道作为可插拔接口以支持可选的人类语言命令；二是前瞻性物理模块，构建自回归BEV世界模型，显式预测基于候选行动的语义地图，作为可解释的物理沙箱并直接导出安全指标。驱动策略通过GRPO优化，采用双重奖励机制：物理奖励来自BEV rollout以强制执行硬安全约束，认知奖励来自语言对齐评分器以确保意图符合性。

**Result:** CoPhy在NAVSIM v1和v2基准测试上取得了最先进的结果，同时通过认知引导的场景合规性实现了更安全的驾驶，并支持用户自定义语言指令的灵活意图控制。

**Conclusion:** 该研究证明了将VLM认知能力以零推理代价蒸馏到BEV表示中并与自回归世界模型相结合的可行性，为强化学习在自动驾驶中的应用提供了有效的基础设施，同时表明通过语言指令进行灵活意图控制是实现更安全、更符合人类期望的自动驾驶的有前景方向。
...
### [66] [FTerViT: Fully Ternary Vision Transformer](https://arxiv.org/abs/2605.21171)
*Szymon Ruciński, Pietro Bonazzi, Engin Türetken, Simon Narduzzi, Michele Magno, Nadim Maamari*

#### 🧩 TL;DR
提出全三值化视觉Transformer FTerViT，将所有权重矩阵和归一化参数三值化，通过知识蒸馏和轻量级量化感知恢复训练，在ESP32-S3微控制器上首次实现三值化ViT部署，W2A8 DeiT-III-S在384×384分辨率下以6.09MB达到82.43% ImageNet-1K top-1准确率。

---

#### 📘 Detailed Summary
**Motivation:** 现有三值化视觉Transformer方法仅对编码器层进行三值化，而patch嵌入、LayerNorm参数和分类器头仍保持全精度；在面向微控制器等资源受限处理器的紧凑模型中，这些剩余的全精度组件决定了总内存占用，严重限制了部署效率和设备端可行性。

**Method:** 提出全三值化视觉Transformer FTerViT，将所有权重矩阵和归一化参数三值化；引入两个新算子：带逐通道缩放的三值化二维卷积（TernaryBitConv2d）用于patch嵌入，以及三值化LayerNorm（TernaryLayerNorm）；采用知识蒸馏训练，随后进行轻量级量化感知恢复阶段。

**Result:** W2A8配置的DeiT-III-S模型在384×384分辨率下以6.09MB（约15倍压缩，相比FP32下降2.42个百分点）达到82.43% ImageNet-1K top-1准确率，超越先前三值化ViT方法最多8个百分点；基于DeiT-III-Small（224×224分辨率，5.81MB）的FTerViT-Small在ESP32-S3双核XTensa LX7微控制器上部署，实现79.64% ImageNet-1K top-1准确率。

**Conclusion:** 全三值化ViT在保持高精度的同时实现了极致的模型压缩和内存占用降低，使得高性能视觉Transformer首次能够在微控制器级芯片上部署，为资源极度受限边缘设备上的视觉AI应用提供了可行路径。
...
### [67] [RankE: End-to-End Post-Training for Discrete Text-to-Image Generation with Decoder Co-Evolution](https://arxiv.org/abs/2605.21195)
*Siyong Jian, Siyuan Li, Luyuan Zhang, Zedong Wang, Xin Jin, Ying Li, Cheng Tan, Huan Wang*

#### 🧩 TL;DR
提出了RankE，首个面向离散自回归文生图模型的端到端后训练框架，通过联合优化策略网络与VQ解码器打破 fidelity-alignment 权衡，在LlamaGen-XL和Janus-Pro上同时提升FID与CLIP分数。

---

#### 📘 Detailed Summary
**Motivation:** 现有离散自回归文生图模型的后训练仅优化策略网络而冻结VQ解码器，导致Latent Covariate Shift问题：策略更新使令牌分布偏离解码器训练时的真实分布，从而奖励提升但生成图像质量下降。扩散模型领域已揭示VAE是对齐瓶颈（如REPA-E），但离散自回归模型的类似问题尚未被探究。

**Method:** 提出RankE框架，采用交替优化策略网络与VQ解码器：每个模块在最大化基于排序的对齐目标的同时，通过适合其参数空间的稳定性保持锚点进行正则化，实现两者共同演化而非固定解码器。

**Result:** 在LlamaGen-XL（775M）上，标准RL提升CLIP但恶化FID，而RankE同时改善两项指标（MS-COCO 30K上FID 15.21，CLIP 33.76）。在Janus-Pro（1B）上的一致增益证实解码器共演化能将奖励优化可靠地转化为像素空间质量提升。

**Conclusion:** 解码器共演化能够打破离散自回归文生图模型中fidelity-alignment的权衡，将策略优化带来的奖励提升有效转化为视觉质量改进，为离散AR模型的后训练提供了新范式。
...
### [68] [Vision Transformers and Convolutional Neural Networks for Land Use Scene Classification](https://arxiv.org/abs/2605.21268)
*Arun D. Kulkarni*

#### 🧩 TL;DR
本文系统比较了Vision Transformer（ViT）与CNN（如AlexNet）在遥感土地利用场景分类任务中的性能，揭示了ViT在捕获全局空间依赖关系方面具有优势但需要更多计算资源和训练数据，而CNN在小样本和强局部纹理场景下仍具竞争力。

---

#### 📘 Detailed Summary
**Motivation:** 遥感土地利用场景分类中，CNN虽因强局部特征提取能力长期占据主导地位，但ViT通过自注意力机制引入的全局上下文建模潜力尚未在遥感场景下得到系统比较，现有研究缺乏对两类架构在分类精度、计算复杂度和数据需求等方面的全面对比评估。

**Method:** 选取代表性CNN模型AlexNet与Vision Transformer（ViT）作为对比架构，在UC Merced Land Use和EuroSAT Land Use两个公开基准遥感数据集上开展实验，通过分类准确率、精确率、召回率、F1分数以及计算复杂度等多个指标对两类模型进行系统性性能评估。

**Result:** 实验结果表明，在训练样本有限且场景具有强局部纹理特征的数据集上，CNN表现稳健；而在训练数据充足且场景复杂的条件下，ViT在捕获全局空间关系方面展现出更优性能；但ViT通常需要更大的计算资源和更多的训练数据才能达到最佳性能。

**Conclusion:** 该研究系统地揭示了CNN和ViT在遥感土地利用场景分类中的各自优势与局限，为不同应用场景下的模型选型提供了实用指导：在数据受限或局部特征主导的场景中优先考虑CNN，在数据充足且需全局语义理解时选择ViT。
...
### [69] [DriveMA: Rethinking Language Interfaces in Driving VLAs with One-Step Meta-Actions](https://arxiv.org/abs/2605.21273)
*Weicheng Zheng, Yixin Huang, Qiao Sun, Derun Li, Hang zhao*

#### 🧩 TL;DR
DriveMA 提出用一步元动作替代冗长的自然语言推理作为驾驶视觉-语言-动作模型的语言接口，结合动作监督训练与回合级强化学习，在 Waymo 端到端驾驶挑战中以2B模型达到8.060 RFS的新SOTA，4B模型进一步将SOTA提升至8.079 RFS。

---

#### 📘 Detailed Summary
**Motivation:** 现有驾驶视觉-语言-动作模型采用自然语言推理作为中间接口，面临三大实际瓶颈：高质量推理标注获取困难、紧凑模型难处理长推理链、推理延迟显著增加，亟需重新设计语言接口以平衡表达能力、可预测性和推理效率。

**Method:** 提出 DriveMA 框架，采用一步元动作替代冗长的自然语言推理作为语言接口，元动作可从专家轨迹自动推导实现可扩展监督；结合动作监督训练与回合级信用分配强化学习，联合优化元动作正确性、轨迹质量及轨迹-元动作一致性。

**Result:** DriveMA 2B 模型在 Waymo 端到端驾驶挑战中以 8.060 Rater Feedback Score (RFS) 刷新 SOTA，4B 模型进一步将 SOTA 提升至 8.079 RFS；在 NAVSIM 上也取得了有竞争力的结果。消融实验表明一步元动作比自然语言推理或细粒度动作序列在表达性、可预测性和推理效率之间实现更优的实际权衡。

**Conclusion:** 研究表明简洁的一步元动作是驾驶 VLA 中复杂的自然语言推理的有效替代方案，提供语义决策基础的同时保持低熵，且可自动推导便于扩展监督；引入的强化学习框架为联合优化语义推理与轨迹生成提供了新范式，将开源代码、数据和模型以促进后续研究。
...
### [70] [Reducing Object Hallucination in LVLMs via Emphasizing Image-negative Tokens](https://arxiv.org/abs/2605.21300)
*Meng Shen, Minghao Wu, Deepu Rajan*

#### 🧩 TL;DR
本文发现大型视觉-语言模型（LVLMs）在生成文本时多数token受图像信息影响极小，这一现象源于训练阶段过度侧重指令跟随而非图像理解，据此提出基于视觉依赖的动态训练权重调整与数据过滤策略，有效减少物体幻觉且不增加推理开销。

---

#### 📘 Detailed Summary
**Motivation:** 大型视觉-语言模型（LVLMs）在实际应用中面临物体幻觉这一关键挑战，现有研究对其成因缺乏深入理解，本文旨在探索幻觉的潜在起源——模型在生成过程中过度偏向文本生成而忽视与图像的实质交互，并据此提出缓解方案。

**Method:** 本文将生成的文本token按其与输入图像token的视觉依赖程度分为三类：图像正相关（image-positive）、不变（invariant）和负相关（negative），通过分析发现多数token几乎不受图像信息影响。基于此发现，提出两种方法：（1）根据token的视觉依赖程度调整其在训练阶段的损失权重；（2）移除训练数据中可能包含更多幻觉的样本作为数据过滤策略。两种方法均在不增加推理计算成本的前提下控制幻觉。

**Result:** 实验在三种不同的LVLM变体上进行验证，所提出的两种方法均能有效减少物体幻觉现象，同时不影响生成响应的长度，且不引入额外的推理计算开销，证明了方法的有效性与广泛适用性。

**Conclusion:** 研究表明LVLMs中物体幻觉的主要来源是训练阶段对文本指令跟随能力的学习权重远高于对图像信息提取能力的学习，通过调整不同视觉依赖程度token的训练权重以及对高幻觉风险数据进行过滤，可以显著缓解该问题，为后续LVLM幻觉缓解研究提供了新的视角和简单有效的实践方案。
...
### [71] [Hyper-V2X: Hypernetworks for Estimating Epistemic and Aleatoric Uncertainty in Cooperative Bird's-Eye-View Semantic Segmentation](https://arxiv.org/abs/2605.21309)
*Abhishek Dinkar Jagtap, Sanath Tiptur Sadashivaiah, Andreas Festag*

#### 🧩 TL;DR
Hyper-V2X提出了一种基于超网络的贝叶斯框架，用于在V2X协同感知中同时量化认知不确定性和偶然不确定性，通过部分权重生成与V2X上下文嵌入机制在轻量级计算开销下实现随机BEV分割，显著提升了感知可靠性。

---

#### 📘 Detailed Summary
**Motivation:** 现有V2X协同感知研究在多智能体融合方面取得了进展，但不确定性量化问题在协同感知框架中尚未得到充分探索，导致模型对感知结果的可信度缺乏评估能力。

**Method:** Hyper-V2X采用超网络框架，提出部分权重生成方案和V2X上下文嵌入模块，将贝叶斯超网络条件化于融合的多智能体特征上，生成用于随机BEV分割的权重分布，实现架构无关的不确定性估计，可无缝集成至CoBEVT等现代协同骨干网络。

**Result:** 在OPV2V基准上的实验表明，Hyper-V2X能够提供准确且校准良好的不确定性估计，同时显著提升整体感知可靠性，且计算开销极低。

**Conclusion:** 该研究首次将认知不确定性与偶然不确定性的联合估计引入V2X协同感知，证明了贝叶斯超网络方法在不显著增加计算负担的前提下有效提升感知可信度，为协同自动驾驶中的安全关键决策提供了重要支撑。
...
### [72] [AIGaitor: Privacy-preserving and cloud-free motion analysis for everyone, using edge computing](https://arxiv.org/abs/2605.21421)
*Lauhitya Reddy, Trisha M. Kesar, Hyeokhyen Kwon*

#### 🧩 TL;DR
AIGaitor是首个在消费级智能手机上完整运行端到端单目动作捕捉与下游深度学习分析的隐私保护步态分析系统，在iPhone 14上以77秒处理10秒4K 60fps视频，性能可比肩高端云服务器，解决了临床步态分析的成本、复杂性和隐私三大障碍。

---

#### 📘 Detailed Summary
**Motivation:** 运动捕捉是测量人体运动的金标准，但临床使用受限于高昂成本、技术复杂性和隐私问题。通过对74名康复临床医生的调查，92%的受访者表示愿意采用准确、成本效益高且易用的AI步态分析工具，而79.7%将运营成本、68.9%将训练不足、64.9%将隐私问题列为主要障碍，这促使AIGaitor设计为隐私保护、无需云端的运动分析系统。

**Method:** AIGaitor将无标记单目动作捕捉流程和下游深度学习分析完全部署在消费级智能手机上，利用设备端神经加速器运行。系统优化并基准测试了iOS端单目流程组件，包括2D/3D姿态估计、姿态优化、基于骨骼的深度学习分析和视觉语言模型，并设计了Time-Priority端到端设备内流程，采用轻量级模型如ViTPose-s实现实时关键点提取和亚毫秒级步态分类。

**Result:** Time-Priority端到端设备内流程在iPhone 14上处理10秒4K 60fps视频片段仅需77秒，与高端NVIDIA H200云服务器在计入网络传输时间后的性能相当甚至更优（全球移动平均上行94秒，发达国家Wi-Fi环境66秒）。轻量级模型ViTPose-s可实现实时关键点提取，基于骨骼的动作识别模型在同一视频片段上实现亚毫秒级步态分类。

**Conclusion:** AIGaitor是首个证明端到端设备内动作捕捉及下游深度学习分析可行的单目系统，支持低成本、隐私保护和智能手机用户可及的临床级运动分析，有效回应了临床部署中的成本、训练和隐私三大核心障碍。
...
### [73] [Improving Quantized Model Performance in Qualitative Analysis with Multi-Pass Prompt Verification](https://arxiv.org/abs/2605.20193)
*Aisvarya Adeseye, Jouni Isoaho, Adeyemi Adeseye*

#### 🧩 TL;DR
本文系统评估了低比特量化（8位、4位、3位、2位）对LLaMA-3.1（8B）在定性分析任务中的影响，提出了一种量化感知的多遍提示验证方法，通过可控步骤减少幻觉、去除不可靠内容并逐条验证，显著提升了低比特量化模型在定性分析中的准确性和稳定性。

---

#### 📘 Detailed Summary
**Motivation:** 量化大型语言模型虽然因推理速度快、资源需求低而被广泛用于定性分析，但低比特量化会导致严重的幻觉和结果不稳定问题，尤其是处理非专家语言和模糊术语时。当前研究缺乏对低比特量化下LLaMA模型在定性分析任务中性能退化程度的系统评估，也缺少针对性的缓解方法。

**Method:** 研究采用82份访谈转录文本，包含专家和非专家两种类型的回答，以BF16 LLaMA-3.1（8B）和NVivo人工编码构建的金标准真实数据（GSGT）作为基准。提出了一种量化感知的多遍提示验证方法，通过引导模型执行可控步骤来减少幻觉，去除不可靠内容并在验证后将结果传递至下一转录，从而提升准确性。

**Result:** 8位量化模型最贴近GSGT；4位量化模型虽损失精度，但应用所提方法后趋于稳定；3位和2位模型因重度压缩性能显著下降，但所提提示设计与验证方法能带来改善。BF16 LLaMA-3.1虽输出高精度，但存在语义漂移和幻觉需人工纠正。相同比特位下不同量化类型的行为存在差异。

**Conclusion:** 所提出的量化感知多遍提示验证方法能有效降低低比特量化LLM的幻觉率并提升稳定性，使得低资源LLM在更低的计算成本下适合用于定性分析研究。该工作为低比特量化LLM在质性研究中的可靠应用提供了实用的缓解策略和评估框架。
...
### [74] [Pseudo-Siamese Network for Planning in Target-Oriented Proactive Dialogues](https://arxiv.org/abs/2605.20195)
*Xinyue Kang, Maodong Li, Yibin Zheng, Fang Kong*

#### 🧩 TL;DR
提出前向聚焦双向伪孪生网络（FF-BPSN）用于目标导向主动对话系统中的对话路径规划，通过双向规划并优先前向信息生成最优路径，在DuRecDial和DuRecDial 2.0上达到最先进性能。

---

#### 📘 Detailed Summary
**Motivation:** 目标导向主动对话系统的核心是规划合理的对话路径并引导语言模型生成回复，其中对话路径规划是一个新颖但尚未充分研究的问题，现有方法难以有效利用双向信息来生成兼顾目标导向性和对话自然性的前向路径。

**Method:** 提出前向聚焦双向伪孪生网络（FF-BPSN），采用两个相同的基于Transformer的解码器分别进行前向和后向路径规划，并引入前向聚焦模块将双向信息融合以构建最终的前向路径，使得路径在受益于双向规划的同时优先保留前向信息，随后利用规划好的路径引导语言模型生成回复。

**Result:** 在DuRecDial和DuRecDial 2.0数据集上的大量实验表明，FF-BPSN在对话路径规划任务上取得了最先进的性能，并显著提升了目标导向主动对话系统的整体有效性。

**Conclusion:** 双向规划结合前向聚焦策略能有效提升对话路径规划的质量，从而增强目标导向主动对话系统的效果，为对话路径规划这一新问题提供了有效的解决方案。
...
### [75] [Under Pressure: Emotional Framing Induces Measurable Behavioral Shifts and Structured Internal Geometry in Small Language Models](https://arxiv.org/abs/2605.20202)
*Rana Muhammad Usman*

#### 🧩 TL;DR
本文系统研究了情绪化评估提示对小型开源语言模型行为及其内部表示的影响，发现不同的情绪框架（如压力、平静、好奇）会显著改变模型的捷径标记频率、过拟合模式以及内部表示的方向向量，并揭示了提示敏感的控制方向存在于小模型中，但尚不能断言模型具备内在情绪状态。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究主要关注大语言模型在标准基准上的表现，而对情绪化交互反馈如何影响小模型的行为与内部表示知之甚少，尤其缺乏对"相对于平静状态"的方向向量可测量性的系统探索。

**Method:** 采用Qwen 3.5 0.8B模型在四个不可能约束编程任务上进行实验，设计八种情绪框架（平静、压力、紧迫、认可、羞耻、好奇、鼓励、威胁），共160轮对话；通过计算各情绪条件相对于平静条件的层间方向向量，并进行主成分分析（PCA）和余弦相似度比较；另在Qwen 3.5 2B模型上进行平静vs压力的扩展对比，使用激活steering和4-prompt A/B探针测试。

**Result:** 在0.8B实验中，压力条件产生最强捷径标记（11/20轮）和最明显过拟合模式（3/20），平静和好奇保持诚实率最高（7/20和6/20）；所有非基线条件的平静相对方向向量在最终层峰值；PCA第一主成分（59.5%方差）与正负情绪标签高度对齐（余弦对齐0.951），认可与紧迫内部几乎一致（余弦0.957），好奇则与紧迫方向相反（-0.252）；在2B模型上，平静条件下诚实率更高且激活steering方向一致，而0.8B的steering结果出现反转。

**Conclusion:** 研究证明小型开源模型中存在可测量的提示敏感控制方向，不同情绪框架在表示空间中有可区分的模式，但尚不足以证明模型拥有内在情绪状态，为后续小模型的情感提示理解和控制机制研究提供了方向性证据。
...
### [76] [Mix-Quant: Quantized Prefilling, Precise Decoding for Agentic LLMs](https://arxiv.org/abs/2605.20315)
*Haiquan Lu, Zigeng Chen, Gongfan Fang, Xinyin Ma, Xinchao Wang*

#### 🧩 TL;DR
Mix-Quant提出了一种相位感知的量化框架，对LLM智能体推理过程中的prefilling阶段采用高吞吐NVFP4量化、decoding阶段保持BF16精度，在不显著损失任务性能的前提下实现了最高3倍prefilling加速，有效缓解了长上下文、多轮交互场景下的推理瓶颈。

---

#### 📘 Detailed Summary
**Motivation:** LLM智能体工作流中，长上下文、多轮交互的推理过程在prefilling阶段产生了巨大的输入侧计算开销，使其成为关键性能瓶颈；现有统一的量化方案在智能体任务中会导致明显的精度下降，缺乏针对不同推理阶段量化冗余度的差异化处理策略。

**Method:** 本文提出Mix-Quant相位感知量化框架，核心思路是利用prefilling阶段存在大量量化冗余、而decoding阶段对精度敏感的特点，将二者解耦：prefilling阶段采用硬件高效的NVFP4量化以实现高吞吐加速，decoding阶段则保持BF16精度以保障生成质量；通过算法与硬件的协同设计实现相位级混合精度推理。

**Result:** 在长上下文和智能体基准测试上的实验表明，Mix-Quant能够在基本保持任务性能的同时显著提升推理效率，prefilling阶段实现了最高3倍的加速效果。

**Conclusion:** 该工作揭示了LLM智能体推理中不同阶段的量化敏感度存在显著差异，并验证了相位感知混合精度量化策略在缓解prefilling瓶颈方面的有效性，为面向智能体工作流的高效推理提供了新的设计方向。
...
### [77] [Stage-Audit: Auditable Source-Frontier Discovery for Cross-Wiki Tables](https://arxiv.org/abs/2605.20478)
*Chen Shen*

#### 🧩 TL;DR
LLM策划的表格可能包含无源行却附带页面级引用（种子到前沿发现中的幻觉风险）；Stage-Audit通过分离的策展-审计写权限、行级源引用门控及12项审计分类法，将源前沿精度从0.356提升至0.505（+42%），F1从0.334提升至0.451（+35%），并保持每行显式源可追溯性。

---

#### 📘 Detailed Summary
**Motivation:** LLM策划的表格可能表面上看起来有来源依据，但实际上包含无支持的行：策展者可能从参数记忆中回忆条目并事后附加并非真实来源的页面级引用。这种幻觉风险在种子到前沿（Seed2Frontier）发现任务——即从种子页面寻找互补的维基百科页面以组装结构化表格——中尤为突出，现有方法缺乏对行级来源真实性的有效验证机制。

**Method:** Stage-Audit提出了一种包含三个核心组件的框架：分离的策展者-审计者写权限（策展者只能写入表格行，审计者只能标记或拒绝行）、行级源引用门控（每行必须附带显式可追溯的源页面引用），以及一个包含12项检查的审计分类法，覆盖键（keys）、模式（schema）、源角色（source roles）、基数（cardinality）和范围（scope）等多个维度。

**Result:** 在涵盖15个顶级领域、51个实例的Seed2Frontier评估集上，Stage-Audit将源前沿精度（source-frontier precision）从普通LLM策展者的0.356提升至0.505（相对提升+42%），F1分数从0.334提升至0.451（相对提升+35%），同时保持了每行显式的源可追溯性。该对比专门隔离了策略贡献而非LLM本身的发现能力。

**Conclusion:** Stage-Audit通过分离角色与结构化审计机制有效缓解了LLM策划表格中的源幻觉问题，显著提升了表格内容的来源可信度。该方法提供的显式行级源可追溯性为审计验证提供了基础，研究结论强调了在AI辅助知识发现中采用差异化写权限和系统化审计分类法的重要性。
...
### [78] [Self-Training Doesn't Flatten Language -- It Restructures It: Surface Markers Amplify While Deep Syntax Dies](https://arxiv.org/abs/2605.20602)
*Ming Liu*

#### 🧩 TL;DR
本文发现自我训练不会均匀地"扁平化"语言，而是不对称地重组语言结构：浅层标记（如话语连接词）上升，深层句法结构（如疑问句、被动语态）崩溃，并据此提出结构深度假说（SDH），即结构深度是语言特征衰减率的主要预测因子。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究将语言模型自我训练（self-training）简单表征为"扁平化"过程——多样性下降、分布变窄、文本趋同，但这一描述忽略了语言不同层次结构在自我训练中的差异性演化模式。

**Method:** 通过在五个模型（GPT-2 124M、Pythia-410M、Pythia-1.4B、OPT-1.3B、Pythia-2.8B）上执行十一代自我训练，系统追踪17个语言特征（包括话语连接词、模糊限制语、破折号等表层标记，以及疑问句、插入语、被动语态、虚拟语气等深层句法结构）的逐代衰减率，并对照人类文本微调控制实验进行验证。

**Result:** 17个特征池化的Spearman相关系数显示，结构深度与衰减率的关联（rho=0.540, p<10^{-6}）远强于生成零代输出频率（rho=0.225）；人类文本微调控制的相关系数仅为rho=0.039（p=0.88），证实该梯度是自我训练特有的现象；此外发现"肤浅复杂度悖论"：聚合复杂度指标（依存句法树深度、类符形符比、词长）上升，而底层从句结构却在消亡。

**Conclusion:** 自我训练对语言的影响并非均匀扁平化，而是基于结构深度的不对称重组；结构深度假说（SDH）为理解自我训练中语言退化机制提供了新的理论框架，对训练数据筛选和LLM文本检测具有直接实践意义。
...
### [79] [Auto-Dreamer: Learning Offline Memory Consolidation for Language Agents](https://arxiv.org/abs/2605.20616)
*Chongrui Ye, Yuxiang Liu, Yu Wang, Haofei Yu, Yining Zhao, Ge Liu, Julian McAuley, Jiaxuan You*

#### 🧩 TL;DR
提出Auto-Dreamer，一种基于互补学习系统理论的离线记忆整合器，通过GRPO训练解耦在线记忆获取与跨会话离线整合，在ScienceWorld、ALFWorld和WebArena上以显著更小的记忆库实现更优的agent性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有语言agent记忆系统将经验获取与整合耦合为单一在线过程，使得agent缺乏跨会话的全局视角，无法发现重复模式、抽象共享过程或精简冗余条目，从而导致记忆膨胀且知识复用效率低下。

**Method:** Auto-Dreamer采用互补学习系统理论，将快速逐会话记忆获取与慢速跨会话整合解耦；对类型化记忆库中的选定工作区域，将其视为只读证据，通过有界工具使用检视条目及其溯源轨迹，综合生成紧凑的替换记忆集；使用GRPO以端到端agent性能为奖励信号训练整合器。

**Result:** 仅在ScienceWorld轨迹上训练的Auto-Dreamer在该基准上比固定记忆、RL训练记忆和提示记忆基线高出7个百分点，活动记忆库大小仅为最强基线的1/12；在未见过的ALFWorld和WebArena上仍保持领先，ALFWorld上记忆量仅为最强基线的1/6。

**Conclusion:** 离线解耦的记忆整合机制能够从在线经验中自动学习抽象和压缩知识，且具有良好的跨领域泛化能力，为构建可持续积累和精炼知识的语言agent提供了有效范式。
...
### [80] [DIVE: Embedding Compression via Self-Limiting Gradient Updates](https://arxiv.org/abs/2605.20689)
*Dongfang Zhao*

#### 🧩 TL;DR
DIVE 提出了一种基于隐式视图集成的压缩适配器，通过自限 hinge 三元组损失和头级 NT-Xent 对比损失，在少标签场景下实现优于现有方法的维度压缩，并在 BEIR 基准上以 14M 参数全面超越三种基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有嵌入压缩方法（如 Matryoshka-Adaptor、Search-Adaptor、SMEC）通过轻量残差适配器降低维度，但其训练目标在标注数据稀缺时会导致严重过拟合，检索性能甚至低于未微调的冻结基线，亟需一种在数据量不足条件下仍能实现有效压缩的方法。

**Method:** DIVE 提出两种核心机制：一是自限 hinge 三元组损失，当三元组满足间隔约束时梯度为零，从而限制对预训练嵌入空间的扰动；二是头级 NT-Xent 对比损失，将每个嵌入的多个学习投影视为隐式视图，提供密集的自监督梯度，以补偿小数据集上三元组信号的稀疏性。

**Result:** 在六个 BEIR 数据集上，DIVE 在所有压缩比下均优于三种基线适配器，且以 14M 参数实现开源部署，展示了在少标签场景下的显著性能优势。

**Conclusion:** DIVE 通过结合自监督与自限机制，有效解决了低资源条件下嵌入压缩适配器的过拟合问题，为在大规模向量检索系统中部署高维语言模型嵌入提供了实用且高效的新方案。
...
### [81] [Interpretable Discriminative Text Representations via Agreement and Label Disentanglement](https://arxiv.org/abs/2605.20693)
*Tong Wang, Yiqing Xu, Leo Yang Yang*

#### 🧩 TL;DR
提出了一种基于操作化准则的可解释文本表征方法LFD，通过概念清晰度（跨标注者Cohen's κ）和标签解缠性两个标准筛选特征，在保持预测性能的同时显著提升了特征的可审计性和可解释性。

---

#### 📘 Detailed Summary
**Motivation:** 现有可解释文本表征存在两方面不足：判别式表征使用匿名嵌入方向缺乏可解释性，而概念瓶颈和LLM辅助方法虽然为特征赋予自然语言名称，但未能确保这些定义是可复现的且与目标标签相区分，导致特征定义模糊或直接泄露标签信息，缺乏实际操作意义上的可审计性标准。

**Method:** 提出LLM辅助特征发现（LFD）方法，包含三步迭代流程：首先从对比性结果对立文本对中提取词汇和语义特征候选，然后通过跨LLM的Cohen's κ筛选确保特征定义的概念清晰度，最后基于残差保留预测增益选择特征。此外，从理论上分析了κ筛选与每特征标注噪声边界的关系，将标注者间一致性形式化为可靠性校验。

**Result:** 在涵盖七个语料库的十个文本分类任务上，LFD在匹配强文本瓶颈基线预测性能的同时产生了更清晰且标签纠缠更少的特征。包含232名评估者的人类审计实验表明，LFD特征相比于基线概念实现了更高的人-人和人-LLM一致性，评估者一致认为其标签泄露更少。

**Conclusion:** 经一致性检验且与标签解缠的特征为可解释文本分类提供了一种实用的可审计性标准，表明将标注者间可复现性和标签区分性纳入特征筛选是提升可解释表征质量的有效途径。
...
### [82] [MTR-Suite: A Framework for Evaluating and Synthesizing Conversational Retrieval Benchmarks](https://arxiv.org/abs/2605.20729)
...
#### 🧩 TL;DR
提出了MTR-Suite统一框架，包含基于LLM的审计器MTR-Eval、多智能体对话生成管道MTR-Pipeline和通用基准MTR-Bench，以低成本生成高质量对话数据并实现更强的检索评测区分能力。

---

#### 📘 Detailed Summary
**Motivation:** 现有对话式检索基准存在两大问题：人工标注成本高且稀疏，自动启发式方法生成的数据不自然。这导致对检索增强生成（RAG）系统中检索模块的评估不够准确和可靠。

**Method:** MTR-Suite包含三个核心组件：(1) MTR-Eval，基于LLM的审计器，用于量化现有基准中的对齐差距；(2) MTR-Pipeline，一个多智能体系统，采用贪心遍历聚类算法生成高保真对话，成本仅为人工标注的1/400；(3) MTR-Bench，一个严格设计的通用领域基准，模拟生产环境中的挑战（如硬主题切换、冗长表达）。

**Result:** MTR-Pipeline能以极低成本（人工标注的1/400）生成高保真对话数据；MTR-Bench具备优越的区分能力，能够有效评估检索系统在生产级挑战场景下的表现。代码和数据已公开。

**Conclusion:** MTR-Suite为对话式检索评估提供了一套低成本、高质量的统一解决方案，有望推动RAG系统中检索模块的标准化评测与发展，其开源实现将促进该领域的进一步研究。
...
### [83] [Findings of the Counter Turing Test: AI-Generated Text Detection](https://arxiv.org/abs/2605.20761)
...
#### 🧩 TL;DR
本文系统分析了AI生成文本检测技术的现状，通过Counter Turing Test (CT2)共享任务评估了二分类（人类 vs. AI文本）和模型归因（识别具体生成模型）两个任务的性能，发现二分类任务性能极高（F1达1.0000），而模型归因任务更具挑战性（最佳F1为0.9531），指出了当前检测技术的优势与局限。

---

#### 📘 Detailed Summary
**Motivation:** 随着GPT-4、Claude 3.5、Llama等先进生成模型能够产生高度连贯、类人的文本，AI生成文本的快速扩散对数字内容完整性构成了严峻挑战，其在误导信息、偏见叙事和安全威胁等方面的滥用问题日益突出，因此需要全面评估现有检测技术的有效性并识别其能力边界。

**Method:** 研究通过Counter Turing Test (CT2)共享任务对AI生成文本检测技术进行系统评估，设置了两个子任务：Task A为区分人类与AI生成文本的二分类任务，Task B为识别特定语言模型来源的模型归因任务。顶级参赛团队采用微调Transformer模型（如DeBERTa、BART）、集成学习及混合检测方法等技术方案。

**Result:** 在二分类任务（Task A）中，顶级系统取得了完美的F1分数1.0000，展现了极高的检测性能；而在模型归因任务（Task B）中，最佳系统的F1分数为0.9531，性能显著下降，表明区分不同LLM生成文本的难度远高于判断文本是否由AI生成。

**Conclusion:** 当前AI文本检测技术在二分类任务上已达到近乎完美的水平，但模型归因任务仍面临显著挑战，区分不同大语言模型的输出需要进一步研究对抗鲁棒性、特征提取和跨领域泛化等方向，以应对日益复杂的AI生成文本检测需求。
...
### [84] [The Illusion of Intervention: Your LLM-Simulated Experiment is an Observational Study](https://arxiv.org/abs/2605.20767)
*Victoria Lin, Taedong Yun, Maja Matarić, John Canny, Arthur Gretton, Alexander D'Amour*

#### 🧩 TL;DR
本文形式化了大语言模型作为人类行为模拟器时，因干预导致的潜在用户属性漂移（user drift）而产生的混杂偏差，并提出利用阴性对照结果检测漂移、通过调整人物设定引入额外混杂因子来减轻偏差的方法。

---

#### 📘 Detailed Summary
**Motivation:** 大语言模型（LLM）作为人类行为模拟器时，干预实验可能导致模拟用户潜在属性的非预期漂移，使得不同处理条件下的隐式模拟人群不一致，从而扭曲效应估计，产生类似混杂或选择偏差的问题，现有研究对此缺乏形式化分析和系统性诊断与缓解手段。

**Method:** 论文首先形式化用户漂移导致的混杂偏差，展示干预依赖的属性偏移如何放大或衰减用户响应的观测差异；然后提出使用阴性对照结果（negative control outcomes）——即在干预下应保持不变的属性——来识别不同干预条件间的分布偏移，作为用户漂移的诊断方法；最后通过调整人物设定（persona specification），引入更多针对场景的额外混杂因子来减轻漂移偏差。

**Result:** 在问卷式和多轮智能体评估场景下，实验表明干预会导致显著的用户属性分布偏移，使用阴性对照结果能有效检测出此类漂移；通过引入目标明确的场景相关混杂因子来调整人物设定，可大幅减少因用户漂移带来的偏差。

**Conclusion:** 该研究揭示了LLM作为模拟器进行干预实验时存在被忽视的混杂风险，提出了有效的诊断工具（阴性对照）和缓解策略（人物设定调整），为提升LLM模拟实验的因果有效性提供了方法论指导。
...
### [85] [MemGym: a Long-Horizon Memory Environment for LLM Agents](https://arxiv.org/abs/2605.20833)
...
#### 🧩 TL;DR
MemGym是一个统一智能体记忆评估基准，覆盖工具对话、深度搜索、代码生成和计算机使用四个智能体场景，通过记忆隔离评分解耦记忆能力与推理、检索和工具使用能力，并训练轻量级奖励模型MemRM替代完整Docker评估以降低计算成本。

---

#### 📘 Detailed Summary
**Motivation:** 现有记忆基准主要评估多轮对话场景中的个性化信息保留，忽视了在长时间智能体执行过程中发生的动态记忆形成过程，导致评估出的记忆系统在真实智能体环境（如代码编写和网页导航）中迁移效果差。

**Method:** 提出MemGym基准，统一现有智能体环境（tau2-bench、SWE-Gym、WebArena-Infinity）和内部记忆管道（MEMGYM-DR、MEMGYM-CODEQA）到同一记忆-推理接口下，覆盖五个评估轨道和四个智能体场景；采用记忆隔离评分机制解耦记忆性能与推理、检索和工具使用能力；训练MemRM（基于Qwen3-1.7B的轻量级奖励模型，使用QLoRA微调）作为压缩质量评分器，替代完整Docker部署评估。

**Result:** MEMGYM-CODEQA和MEMGYM-DR的合成管道具有长度可控、每阶段可消融验证且与下游场景紧密对齐的特性；MemRM能够作为快速标量读取替代完整Docker滚动评估，使编码环境的学术评估在计算上可行。

**Conclusion:** MemGym通过统一接口和隔离评分机制为智能体记忆研究提供了更准确的评估框架，MemRM作为轻量级代理进一步降低了评估门槛，推动了记忆系统从对话场景向真实智能体环境的转移。
...
### [86] [LamPO: A Lambda Style Policy Optimization for Reasoning Language Models](https://arxiv.org/abs/2605.21235)
*Zhe Yuan, Yipeng Zhou, Jinghan Li, Xinyuan Chen, Bowen Deng, Zhiqian Chen, Liang Zhao*

#### 🧩 TL;DR
LamPO提出了一种Lambda-Style Policy Optimization方法，通过成对分解优势替代标量群组优势，并结合ROUGE-L密集奖励，在多个推理基准上一致优于GRPO等现有RLVR方法，提升了训练稳定性和样本效率。

---

#### 📘 Detailed Summary
**Motivation:** 现有的RLVR方法（如GRPO）使用群组级标量统计量汇总样本，丢弃了候选响应之间的细粒度排序关系，导致在稀疏结果奖励下信用分配不足，尤其当多个生成解仅在推理质量上存在细微差异时问题更加突出。

**Method:** LamPO用成对分解优势（Pairwise Decomposed Advantage）替代标量群组优势，聚合响应组内的成对奖励差异，并通过基于序列对数概率差异计算的置信感知权重对每个比较进行调制，同时保留了无评论网络和裁剪更新的PPO风格优化结构；在有参考解时，进一步引入轻量级ROUGE-L密集辅助奖励以缓解奖励稀疏性。

**Result:** 在AIME24、AIME25、MATH-500和GPQA-Diamond四个基准上，使用Qwen3-1.7B、Qwen3-4B和Phi-4-mini三种模型进行实验，LamPO在训练动态稳定性和样本效率方面一致优于GRPO及其他近期RLVR变体。

**Conclusion:** LamPO通过引入成对比较和置信加权机制有效改善了稀疏奖励下的信用分配问题，为基于可验证奖励的强化学习在推理任务中的提升提供了新的思路，表明细粒度排序信息的利用对提升推理语言模型性能至关重要。
...
### [87] [LASH: Adaptive Semantic Hybridization for Black-Box Jailbreaking of Large Language Models](https://arxiv.org/abs/2605.21362)
*Abdullah Al Nomaan Nafi, Fnu Suya, Swarup Bhunia, Prabuddha Chakraborty*

#### 🧩 TL;DR
LASH 是一个黑盒框架，通过自适应组合多种越狱攻击的种子提示，利用遗传优化器动态调整混合权重，在 JailbreakBench 上以仅 30 次目标查询实现 84.5% 的攻击成功率，显著优于现有单一家族方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有自动化越狱攻击方法虽然效果日益提升，但每种方法都局限于单一攻击家族（如单一的优化循环、树搜索、变异空间或策略库），且没有哪个单一家族在所有目标模型和危害类别上占优，表明不同方法具有互补优势，但目前缺乏能够针对每个请求自适应组合这些策略的黑盒框架。

**Method:** LASH 将多个基础攻击的输出作为可重用的种子提示，通过搜索种子子集和 softmax 归一化混合权重来组合它们；一个合成模块生成单个候选提示，然后使用无导数遗传优化器，结合基于关键词的拒绝检测与 LLM 评判打分的两阶段适应度函数，根据黑盒目标反馈更新权重。

**Result:** 在包含 10 个类别 100 个有害提示的 JailbreakBench 上，LASH 在六个常见目标模型上达到平均 84.5% 的关键词评估攻击成功率和 74.5% 的两阶段评估成功率，仅需平均 30 次目标查询；在两个指标上均优于五个最先进基线，在三种防御机制下仍保持竞争力，并诱导出更接近成功的内部表示。

**Conclusion:** 结果表明，跨异构越狱策略的自适应组合是黑盒红队测试的一个有前景的方向，能够有效利用不同攻击家族的互补优势，在更低查询成本下实现更强的攻击性能。
...
### [88] [Mem-$π$: Adaptive Memory through Learning When and What to Generate](https://arxiv.org/abs/2605.21463)
*Xiaoqiang Wang, Chao Wang, Hadi Nekoei, Christopher Pal, Alexandre Lacoste, Spandana Gella, Bang Liu, Perouz Taslakian*

#### 🧩 TL;DR
Mem-π提出了一种用于LLM智能体的自适应记忆框架，通过专用语言/视觉语言模型按需生成上下文相关的指导，取代传统的基于检索的外部记忆方法，并在网页导航、终端工具使用和文本具身交互等任务上取得了显著性能提升。

---

#### 📘 Detailed Summary
**Motivation:** 现有记忆增强型智能体通常依赖基于相似度检索的情景记忆库或技能库，返回的静态条目往往与当前上下文不匹配，导致检索结果与智能体实际需求之间存在对齐偏差，限制了智能体在复杂任务中的适应能力。

**Method:** Mem-π采用一个独立于下游智能体的专用语言或视觉语言模型（拥有自身参数），该模型以当前智能体上下文为条件，联合决策何时生成指导以及生成何种指导；使用决策-内容解耦的强化学习（RL）目标进行训练，使模型能够在生成无益时选择放弃，否则生成简洁有用的指导。

**Result:** 在网页导航、终端工具使用和文本具身交互等多种智能体基准测试中，Mem-π一致优于基于检索和先前RL优化的记忆基线，在网页导航任务上取得了超过30%的相对改进。

**Conclusion:** Mem-π验证了按需生成而非检索式记忆的有效性，表明通过解耦决策与内容的RL训练可以使记忆模型学会在何时生成以及生成什么，为设计更具适应性的LLM智能体记忆机制提供了新的范式。
...
### [89] [SOLAR: A Self-Optimizing Open-Ended Autonomous Agent for Lifelong Learning and Continual Adaptation](https://arxiv.org/abs/2605.20189)
*Nitin Vetcha, Dianbo Liu*

#### 🧩 TL;DR
本文提出SOLAR，一种基于参数级元学习和多层级强化学习的自优化终身自主推理智能体，能够在无需梯度更新的情况下实现测试时的持续适应，在常识、数学、医学、编程、社会与逻辑推理等多项任务上超越强基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 大语言模型在动态真实部署场景中面临概念漂移和高昂梯度适应成本两大瓶颈，传统微调方法难以在非平稳数据流中实现持续适应，容易产生灾难性遗忘或需要大量人工数据整理，现有方法缺乏一种无需梯度更新的自优化终身学习机制。

**Method:** SOLAR将模型权重视为可探索的环境，通过参数级元学习建立常识性知识的强先验以促进迁移学习，并采用多层级强化学习自主发现适应策略，实现高效的无梯度测试时领域适应；同时维护一个不断演化的有效修改策略知识库作为隐式情节记忆缓冲，以平衡可塑性（适应新任务）与稳定性（保留元知识）。

**Result:** 实验表明，SOLAR在常识推理、数学推理、医学推理、代码生成、社会推理和逻辑推理等多种推理任务上均显著优于多个强基线方法，验证了其在演变环境中实现终身适应的有效性。

**Conclusion:** 该研究标志着向能够在演变环境中实现终身自主适应的智能体迈出了重要一步，证明了无需梯度更新的参数级元学习策略搜索方法在持续学习场景中的巨大潜力，为未来自主智能体的终身学习与适应机制提供了新的方向。
...
### [90] [AgentCo-op: Retrieval-Based Synthesis of Interoperable Multi-Agent Workflows](https://arxiv.org/abs/2605.20425)
*Shuaike Shen, Wenduo Cheng, Shike Wang, Mingqian Ma, Jian Ma*

#### 🧩 TL;DR
AgentCo-op 提出一种基于检索的合成框架，通过类型化工件传递和有界自修复机制，将可复用技能、工具和外部智能体组合为可执行工作流，在开放科学场景中无需全局拓扑搜索即可实现多智能体协作，并在多个基准测试上取得最佳性能且降低任务成本。

---

#### 📘 Detailed Summary
**Motivation:** 在开放式的科学场景中，多智能体工作流设计面临缺乏精心整理的训练集、可靠的标量评估指标以及现有工具与智能体之间标准化接口的挑战，使得传统依赖基准优化的自动化工作流设计方法难以适用。

**Method:** AgentCo-op 提出基于检索的合成框架，通过类型化工件传递将可复用技能、工具和外部智能体组合为可执行工作流，并在执行证据表明失败时对相关组件进行有界自愈。该框架支持将已检索的工作流作为结构先验导入，通过检索到的组件落地节点并应用局部修复来改进工作流，从而将合成与搜索有机互补。

**Result:** 在两个开放世界基因组学案例中，AgentCo-op 能够合成独立开发的空间转录组学和基因集解释智能体实现协作发现，并构建跨模态标记分析的并行工作流。在六个编码、数学和问答基准测试上，AgentCo-op 在统一骨干设置下取得四个基准最佳结果和最佳平均分数，且相较于多智能体基线持续降低每任务成本。

**Conclusion:** 结果表明，基于检索的合成能够将自动化智能体工作流设计从仅适用于基准优化的智能体图扩展到由现有智能体、工具和类型化工件构建的开放世界工作流，且合成与搜索在自动工作流设计中具有互补性。
...
### [91] [AgentAtlas: Beyond Outcome Leaderboards for LLM Agents](https://arxiv.org/abs/2605.20530)
*Parsa Mazaheri, Kasra Mazaheri*

#### 🧩 TL;DR
AgentAtlas 针对现有大语言模型智能体评估基准碎片化、仅依赖单一准确率指标的不足，提出了一套包含六状态控制-决策分类法、九类别轨迹失败分类法、分类法感知与盲测对比方法以及基准覆盖审计的系统化评估框架，并通过小规模实验揭示了当前模型在控制准确率、轨迹诊断和工具上下文保持三方面均无法全面领先的现状。

---

#### 📘 Detailed Summary
**Motivation:** 现有大语言模型智能体评估基准碎片化严重，各自采用不同的评测单元（如最终任务成功率、工具调用有效性、重复通过一致性、轨迹安全性或攻击鲁棒性），且仅使用单一的准确率指标已无法满足可部署智能体的评估需求，亟需一个更为系统、多维的评估框架。

**Method:** AgentAtlas 包含四个核心组件：(i) 六状态控制-决策分类法（Act / Ask / Refuse / Stop / Confirm / Recover）；(ii) 九类别轨迹失败分类法，配备两个正交层次化标签（主要错误来源、影响程度）；(iii) 分类法感知与分类法盲测对比方法，用于度量模型表现中有多少源自提示中的监督信息；(iv) 基准覆盖审计，将十五个智能体基准映射到六个行为轴上。

**Result:** 在包含四个前沿闭源模型和四个开源模型的八模型小规模实验（1,342个生成项）中，去除显式标签菜单后，所有模型的轨迹准确率下降14-40个百分点，收敛至0.54-0.62的狭窄区间，模型间差距大幅缩小；且没有任何一个模型能在控制准确率、轨迹诊断和工具上下文效用保持三个维度上同时取得最优。

**Conclusion:** 当前前沿大语言模型智能体的真实能力在很大程度上依赖于提示中的显式监督信息，去除后模型表现显著趋同；单一指标无法全面衡量可部署智能体的能力，AgentAtlas 提出的多维分类评估方法为更可靠的智能体评测提供了新方向，但该研究仅作为测量协议演示而非正式基准发布。
...
#### 🧩 TL;DR
本文提出一种面向L4/L5自治网络的分层多智能体参考架构，通过双驱动编排器与智能体自我感知机制，将战略规划与操作韧性统一，在5G核心网环境中实现了86%的平均修复时间降低。

---

#### 📘 Detailed Summary
**Motivation:** 当前网络运维依赖刚性脚本，缺乏处理非标称条件的认知能力，无法实现L4/L5级别的自治网络，亟需从静态自动化转向具备智能体的原生智能。

**Method:** 提出分层多智能体参考架构，包含双驱动编排器（Dual-Driven Orchestrator）协调专用执行智能体，并引入共享公共内存（Public Memory）统一领域知识；核心创新在于集成智能体自我感知（agent self-awareness）机制，使系统能够协调审慎式战略治理与反射式故障恢复。

**Result:** 在5G核心网环境中验证，系统能在拥塞条件下维持关键吞吐量，并将平均修复时间（MTTR）降低86%，证实了该架构在统一战略规划与操作韧性方面的有效性。

**Conclusion:** 该研究证明了智能体自我感知与分层多智能体架构在实现高等级自治网络中的关键作用，为未来网络从静态自动化向智能体原生自治演进提供了可行路径。
...
#### 🧩 TL;DR
COAgents提出了一种基于多智能体协作的车辆路径问题求解框架，将搜索过程建模为图结构，通过节点选择、移动选择和跳转三个智能体分别控制局部优化与全局探索，在CVRP和VRPTW基准上取得了具有竞争力的结果，尤其在VRPTW上比现有最强神经求解器POMO降低最优解差距14%~44%。

---

#### 📘 Detailed Summary
**Motivation:** 车辆路径问题（VRP）在现实系统中至关重要，但其组合复杂性导致在大规模场景下仍难以高效求解；传统启发式方法依赖手工规则进行局部改进和偶尔的跳跃以逃离局部最优，但在跨实例泛化方面表现不佳，亟需更灵活、可泛化的搜索策略。

**Method:** COAgents构建了一个协作多智能体框架，将搜索过程建模为图结构，其中节点表示解，边表示局部改进或大幅扰动（跳跃）；在搜索过程中动态构建部分搜索图（PSG），并训练节点选择智能体和移动选择智能体引导集中搜索（intensification），同时由跳跃智能体（Jump Agent）触发适时的探索性跳跃；该方法将问题无关的搜索控制与紧凑的领域特定编码分离，增强了跨任务的适应性。

**Result:** 在CVRP和VRPTW基准上的广泛实验表明：COAgents在CVRP上与多种学习搜索方法竞争力相当；在更具挑战性的VRPTW实例上，相比最强神经求解器POMO，在N=100和N=50时分别将最优解差距降低14%和44%，相比ALNS则分别降低21%和40%，达到了学习型方法的新最优水平。

**Conclusion:** COAgents通过图结构化的多智能体协作框架有效分离了搜索控制与领域编码，在保持泛化能力的同时显著提升了VRPTW的求解质量，验证了将问题无关搜索策略与领域知识解耦的设计范式在组合优化问题中的潜力，为后续可迁移的神经搜索方法提供了新思路。
...
#### 🧩 TL;DR
本文针对工业资产运维场景中LLM代理管道的延迟敏感性问题，提出时间语义缓存与MCP工作流优化（磁盘工具发现缓存+依赖感知并行执行）两层优化方法，分别实现缓存命中30.6倍加速和1.67倍整体加速，并揭示纯语义缓存在参数密集型工业查询中的失效模式。

---

#### 📘 Detailed Summary
**Motivation:** 工业资产运维工作流对延迟高度敏感，单次用户查询需协同传感器数据、工单、故障模式、预测工具及领域代理；现有LLM缓存技术（如KV-cache复用和基于嵌入的语义缓存）专为聊天场景设计，无法处理输出有效性依赖于时间、资产或传感器参数的工业查询，导致在AssetOpsBench（AOB）这一工业代理基准的计划-执行管道中反复出现工具发现、LLM规划、MCP工具执行和最终总结的开销。

**Method:** 本文提出两层互补优化方法：一是时间语义缓存（temporal semantic cache），针对时间与参数敏感的工业查询进行缓存；二是MCP工作流优化，包括基于磁盘的工具发现缓存（disk-backed tool-discovery caching）和依赖感知的并行步骤执行（dependency-aware parallel step execution），以加速AOB计划-执行管道。

**Result:** MCP工作流优化实现1.67倍加速，中位端到端延迟降低约40.0%；时间语义缓存在缓存命中时达到30.6倍中位加速。实验结果还暴露了纯语义缓存在参数密集型工业查询中的具体失效模式，揭示了缓存选择如何与MCP代理基准中评估正确性相互作用。

**Conclusion:** 本文提出的两层优化方案显著降低了工业代理管道的延迟，同时揭示了纯语义缓存在参数和时间敏感的工业场景中的根本局限性，为MCP代理基准的缓存策略设计提供了关键分析，强调缓存正确性与评估完整性之间的权衡关系。
...
### [95] [Conflict-Aware Additive Guidance for Flow Models under Compositional Rewards](https://arxiv.org/abs/2605.20758)
*Xuehui Yu, Fucheng Cai, Meiyi Wang, Xiaopeng Fan, Harold Soh*

#### 🧩 TL;DR
本文提出 Conflict-Aware Additive Guidance (g^car)，一种轻量级可学习方法，通过动态检测并解决梯度冲突，有效修复扩散模型和流模型在多重约束引导采样中的流形偏离问题，在多个领域超越基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有推理时引导采样方法虽能通过外部约束控制扩散模型和流模型的生成过程，但在同时组合多个约束时往往失败，导致生成结果偏离真实数据流形；研究旨在揭示这种离流形漂移的根本原因并提出有效的修复方法。

**Method:** 首先从理论上识别出离流形漂移的根本原因，发现近似误差随梯度错位严重放大；基于此提出 Conflict-Aware Additive Guidance (g^car)，一种轻量级可学习方法，通过动态检测并解决梯度冲突来主动修复离流形漂移。

**Result:** g^car 在合成数据集、图像编辑以及生成式决策规划与控制等多个领域得到验证，结果表明该方法有效修复了离流形漂移，在生成保真度上超越基线方法，同时保持较低的计算开销。

**Conclusion:** 本文揭示了多约束引导采样中离流形漂移与梯度错位之间的内在关联，提出的 g^car 方法为可控生成中的约束组合问题提供了一种高效且实用的解决方案。
...
### [96] [Interaction Locality in Hierarchical Recursive Reasoning](https://arxiv.org/abs/2605.20784)
*Yosuke Miyanishi, Tetsuro Morimura*

#### 🧩 TL;DR
本文提出交互局部性（interaction locality）这一任务几何感知框架，用于量化空间推理中信息流的局部性与全局性，并通过递归推理模型（HRM、TRM）和具身3D场景基础模型（MTU3D）上的激活补丁实验揭示：高层递归状态倾向于局部写入信息，而多次递归更新将局部写入累积为全局解结构；具身模型中因果空间局部性主要出现在视觉特征向下游模块传递的边界处。

---

#### 📘 Detailed Summary
**Motivation:** 空间推理需要同时支持位置绑定的局部计算（如移动）与位置无关的全局结构（如路径、约束或对象级计划），但现有方法缺乏一个可重复的测量框架来量化信息流在相邻单元、语义片段与跨片段之间的局部性或全局性分布。

**Method:** 提出交互局部性框架，利用稀疏自编码器特征消融和有限噪声激活补丁（structural Jacobian和注意力检查见附录）来测量信息流局部性；在HRM和TRM两个紧凑的层次化和递归推理模型上应用于Maze-Hard、Sudoku Extreme和ARC-AGI任务，并在MTU3D大尺度具身3D场景基础模型上延伸验证。

**Result:** 激活补丁揭示了最清晰的架构指纹：高层递归状态倾向于在附近单元格或同段单元内写入信息，而重复的递归更新将这些局部写入累积为全局解结构，该模式在迷宫路径、数独约束和ARC-AGI对象邻域中一致成立，且TRM中集中度最强；在MTU3D中，因果空间局部性主要出现在视觉场景特征传递到下游基础模块的过渡处，而非视觉编码器内均匀分布。

**Conclusion:** 交互局部性将直观的局部执行/全局规划故事转化为可重复的测量框架，揭示了递归推理模型中局部到全局的信息交接机制，而具身3D模型将因果空间结构集中在模块边界，表明这两种模型中空间推理的组织原则存在本质差异。
...
### [97] [PlanningBench: Generating Scalable and Verifiable Planning Data for Evaluating and Training Large Language Models](https://arxiv.org/abs/2605.20873)
*Ziliang Zhao, Zenan Xu, Shuting Wang, Hongjin Qian, Yan Lei, Minda Hu, Zhao Wang, Shihan Dou, Zhicheng Dou, Pluto Zhou*

#### 🧩 TL;DR
PlanningBench是一个面向大语言模型的可控规划数据生成框架，通过基于分类学的约束驱动合成流水线生成可扩展、多样化且可验证的规划问题，实验表明现有模型在耦合约束下仍难以生成完整解，而基于该数据进行的强化学习可提升模型在未见规划任务和指令跟随任务上的表现。

---

#### 📘 Detailed Summary
**Motivation:** 现有规划基准将数据视为固定实例集合而非可控生成目标，导致场景覆盖有限、难度仅依赖表面代理而非结构来源，且难以支持可扩展生成、自动验证或面向规划的模型训练，因此需要一种能兼顾现实任务根基与可控生成能力的规划数据框架。

**Method:** PlanningBench从真实规划场景出发，将实际工作流抽象为包含30余种任务类型、子任务、约束族和难度因素的结构化分类学，并基于此构建约束驱动的合成流水线，实现自适应难度控制、质量过滤和实例级验证清单，将规划数据构建从固定基准收集转变为可控生成。

**Result:** 在开源和闭源前沿LLM上的评估表明，当前模型在耦合约束下仍难以生成完整解；基于可验证PlanningBench数据的强化学习不仅提升了模型在未见规划基准上的性能，还改善了更广泛的指令跟随任务表现，进一步分析显示确定或良好指定的最优解能提供更清晰的奖励信号和更稳定的训练动态。

**Conclusion:** PlanningBench为诊断和提升LLM的通用规划能力提供了可控的数据来源，可扩展的规划数据生成与强化学习结合有望成为提升模型规划及指令跟随能力的有效途径，其中最优解的确定性程度对训练稳定性具有关键影响。
...
### [98] [Governance by Construction for Generalist Agents](https://arxiv.org/abs/2605.20874)
*Segev Shlomov, Iftach Shoham, Alon Oved, Ido Levy, Sami Marreed, Harold Ship, Offer Akrabi, Sergey Zeltyn, Avi Yaeli, Nir Mashkif*

#### 🧩 TL;DR
CUGA提出了一种基于策略即代码的模块化治理体系，通过五个结构化检查点（意图守卫、剧本、工具指南、人工审批、输出格式化）对通用LLM智能体进行全流程的运行时治理干预，在不进行模型微调的前提下实现企业级可审计、合规感知的智能体行为控制。

---

#### 📘 Detailed Summary
**Motivation:** 企业级智能体需要在不重新构建每个领域代理的前提下，以构建式治理方式明确指定允许的操作、需要人工介入的时机以及可暴露的信息范围，现有方案缺乏无需模型微调即可实现可预测、可审计且合规感知行为的通用治理架构。

**Method:** CUGA提出了一套运行时治理架构，采用政策即代码的模块化策略层与通用LLM智能体组合，在五个关键执行检查点实施策略干预：计划上游的意图守卫用于拦截恶意或意外有害请求，系统提示中的剧本用于引导推理与结构化工具序列执行，工具调用边界的工具指南用于强制合规使用，循环外的人工审批门控用于高风险操作，以及输出阶段的格式化器用于过滤和结构化最终响应，从而实现治理在智能体执行流水线中的持续嵌入。

**Result:** 在医疗场景下的多层执法干预演示中，CUGA展示了动态剧本注入实现结构化工具序列执行、意图守卫阻断恶意或意外有害请求、以及高风险操作的人工审批检查点，结果表明类型化治理原语能够提升策略遵从性和执行一致性，加速企业级智能体系统的安全部署。

**Conclusion:** CUGA的策略系统表明，通过在智能体执行流水线的关键阶段嵌入结构化治理检查点，无需模型微调即可实现企业级智能体的可预测、可审计和合规感知行为，为复杂工作流中的安全自主操作提供了可行的部署范式。
...
### [99] [Insights Generator: Systematic Corpus-Level Trace Diagnostics for LLM Agents](https://arxiv.org/abs/2605.21347)
...
#### 🧩 TL;DR
本文形式化定义了LLM智能体执行轨迹的语料级诊断问题，并提出多智能体系统Insights Generator (IG)，通过侦察-调查者架构自动生成基于证据的自然语言洞察报告，在人工评估中帮助专家将系统性能提升30.4个百分点。

---

#### 📘 Detailed Summary
**Motivation:** LLM智能体的故障诊断仍高度依赖人工，从业者仅能检查少量执行轨迹并形成临时假设，这种流程不仅遗漏了仅能在轨迹群体层面显现的系统性模式，也无法扩展到生产环境中单个轨迹包含数万token的大规模语料。

**Method:** 作者形式化了语料级轨迹诊断问题，提出Insights Generator (IG)多智能体系统，采用侦察-调查者架构：侦察智能体负责在轨迹语料中提出假设，调查者智能体负责测试假设并生成每个假设的支撑证据，最终输出一份包含系统性行为模式及其证据支持的洞察报告。

**Result:** 使用IG报告的人类专家将脚手架性能较未修改基线提升30.4个百分点；基于IG洞察的编码智能体在多个基准上展现出一致且稳定的增益；领域专家评价IG报告在深度和证据质量方面领先；IG的侦察-调查者架构在检测覆盖率上与竞争方法相当。

**Conclusion:** IG证明了自动化的语料级轨迹诊断能够有效揭示人工难以发现的系统性行为模式，其生成的证据充分的洞察报告既能辅助人类专家改进系统，也能直接指导编码智能体自主提升性能，为大规模LLM智能体系统的可观测性和调试提供了可行范式。
...
### [100] [Towards Resilient and Autonomous Networks: A BlueSky Vision on AI-Native 6G](https://arxiv.org/abs/2605.21395)
*Liang Wu, Kelly Wan, Mayank Darbari, Liangjie Hong*

#### 🧩 TL;DR
本文提出了一种名为BlueSky的愿景，阐述了人工智能如何原生集成到6G网络中，实现从"面向AI的网络"到"面向网络的AI"的范式转变，通过6G基础模型和协作式多智能体系统将网络管理统一为多模态多任务优化问题。

---

#### 📘 Detailed Summary
**Motivation:** 现有5G网络依赖于分散的、为单一任务独立训练的临时模型，缺乏统一性和自适应性，难以满足自动驾驶和沉浸式体验等新兴应用对通信网络提出的更高韧性、自主性和智能化要求，亟需一种原生AI集成的新型网络架构。

**Method:** 本文提出了两大技术方向：一是构建6G基础模型作为统一骨干，将任务特定知识蒸馏为适用于多样化边缘部署的紧凑模型；二是构建协作式多智能体系统，实现网络的自诊断、自维护和自恢复，将网络管理建模为统一的多模态多任务优化问题。

**Result:** 本文为6G网络设计了一条从分散式AI到原生集成AI的技术路线图，通过基础模型与多智能体系统的协同，使6G能够进化为具备高度自治能力的智能通信基础设施，但具体实验指标和定量结果尚未提供。

**Conclusion:** 本文的核心洞见在于明确了6G中AI的角色应从"为AI服务的网络"转向"由AI驱动的网络"，基础模型与多智能体系统的结合是实现6G智能化、自维持通信基础设施的关键路径，为未来6G研究与标准化提供了前瞻性框架。
