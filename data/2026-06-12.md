<div id=toc></div>

# Table of Contents

- [cs.CV](#cs.CV) [Total: 27]
- [cs.CL](#cs.CL) [Total: 16]
- [cs.AI](#cs.AI) [Total: 33]


<div id='cs.CV'></div>

# cs.CV [[Back]](#toc)

### [1] [High-Fidelity Two-Step Image Generation via Teacher-Aligned End-to-End Distillation](https://arxiv.org/abs/2606.12575)
*Dongyang Liu, Ruoyi Du, David Liu, Dengyang Jiang, Liangchen Li, Qilong Wu, Zhen Li, Steven C. H. Hoi, Hongsheng Li, Peng Gao*

#### 🧩 TL;DR
Z-Image Turbo++ 通过分布对齐对抗学习、步解耦参数化和端到端迭代正则化训练三项设计，将图像生成扩散蒸馏从8步压缩至2步，显著缩小了2步与8步生成之间的质量差距。

---

#### 📘 Detailed Summary
**Motivation:** 现有少步扩散蒸馏在4-8步生成上已较为成熟，但进一步压缩至2步时面临任务难度显著增大和模型容量有限两大瓶颈，导致生成质量与效率之间的权衡仍不理想。

**Method:** 提出三种针对性设计：1) 分布对齐对抗学习，使用教师模型生成图像而非真实图像作为GAN训练的"真实"样本，提供更可达且信息丰富的对抗目标；2) 步解耦参数化，为两个去噪步骤分配独立的模型参数，以匹配各自不同的容量需求；3) 端到端迭代正则化训练，允许第一步从最终图像质量接收梯度，同时通过显式的第一步损失保留有意义的中间生成。

**Result:** 从8步Z-Image Turbo教师模型蒸馏得到的2步Z-Image Turbo++在定性和定量评估中均显著缩小了与8步生成的质量差距，验证了针对2步生成量身定制的蒸馏策略的有效性。

**Conclusion:** 精心定制的蒸馏策略能够有效改善少步生成中的质量-效率权衡，为将扩散模型推向极少数步数（如2步）的高质量生成提供了可行方向。

---

#### 📄 Abstract
Few-step diffusion distillation has become increasingly mature for 4-8-step generation, yet pushing further to 2 steps remains challenging. In this work, we introduce Z-Image Turbo++, a high-quality 2-step image generation model distilled from the 8-step Z-Image Turbo teacher. Our method addresses the central bottlenecks of increased task difficulty and limited model capacity in 2-step generation through three simple but effective design choices tailored to this regime. First, we propose Distribution-Aligned Adversarial Learning, which uses teacher-generated images rather than external real images as real samples for GAN training, providing a more attainable and informative adversarial target. Second, we adopt Step-Decoupled Parameterization, assigning independent model parameters to the two denoising steps to better match their distinct capacity demands. Third, we perform End-to-End Training with Iterative Regularization, allowing the first step to receive gradients from final image quality while preserving a meaningful intermediate generation through an explicit step-1 loss. Together, these designs substantially narrow the quality gap between 2-step and 8-step generation in both qualitative and quantitative evaluations, highlighting the potential of carefully tailored distillation strategies for improving the quality-efficiency trade-off in few-step generation.


### [2] [Analyzing and Improving Fine-grained Preference Optimization in Medical LVLMs](https://arxiv.org/abs/2606.12590)
*Shayan Mohammadizadehsamakosh, Pritam Sarkar, Leonid Sigal, Ali Etemad, Elham Dolatabadi*

#### 🧩 TL;DR
本文提出一种面向医学视觉-语言模型的细粒度、在策略对齐框架，通过双向token级KL正则化和视觉对比基础目标，仅修正临床错误片段同时保留原始语言风格，有效缓解了事实不一致、视觉基础薄弱和与临床反馈对齐不足的问题。

---

#### 📘 Detailed Summary
**Motivation:** 大型视觉-语言模型在医学影像任务中存在事实不一致、视觉基础薄弱和与临床意义反馈对齐不足的问题。现有后训练对齐方法（如DPO及其变体）在医学领域面临三个关键局限：序列级奖励信号无法区分临床关键token与填充文本、依赖静态监督微调参考导致离策略分布偏移使优化偏向风格伪影而非临床正确性、对齐目标缺乏显式视觉基础约束使模型对细微但诊断关键的病理特征不敏感。

**Method:** 该方法提出一个细粒度、在策略对齐框架，核心包含两个组件：双向token级KL正则化（用于在token层面精细化约束对齐过程）和视觉对比基础目标（通过配对干净图像与病变破坏图像，惩罚缺乏充分视觉证据生成的响应）。框架通过在策略方式构造偏好对，对模型生成输出进行最小编辑，仅纠正临床错误片段同时保留原始语言风格。

**Result:** 在医学影像任务和临床文本生成基准上的大量实验验证了该方法的有效性，表明其在提升事实准确性、视觉基础能力和临床对齐性方面优于现有对齐方法。

**Conclusion:** 该研究揭示了现有序列级对齐方法在医学领域的根本性缺陷，证明了token级细粒度约束和显式视觉基础建模对于医学视觉-语言模型对齐的必要性，为构建临床可信的医疗AI系统提供了重要方法论基础。

---

#### 📄 Abstract
Large Vision-Language Models (LVLMs) have achieved strong performance across medical imaging tasks, yet they remain prone to factual inconsistencies, poor visual grounding, and misalignment with clinically meaningful feedback. Existing post-training alignment approaches, including Direct Preference Optimization (DPO) and its variants, face three critical limitations in the medical domain: (1) sequence-level reward signals treat clinically critical tokens identically to generic filler text; (2) reliance on static supervised fine-tuning references as preferred responses introduces an off-policy distribution shift, steering optimization toward stylistic artifacts over clinical correctness; and (3) alignment objectives lack explicit visual grounding constraints, leaving models insensitive to subtle yet diagnostically decisive pathological features. Our method leverages a bidirectional token-wise KL regularizer alongside a visual-contrastive grounding objective that pairs clean and lesion-corrupted images to penalize responses generated without adequate visual evidence. Together, these components form a fine-grained, on-policy alignment framework that constructs preference pairs by minimally editing model-generated outputs, correcting only clinically erroneous spans while preserving the original linguistic style. Extensive experiments across medical imaging tasks and clinical text generation benchmarks validate the effectiveness of our approach.


### [3] [ECA: Efficient Continual Alignment for Open-Ended Image-to-Text Generation](https://arxiv.org/abs/2606.12633)
*Jiangtao Kong, Peijun Zhao, Chun-Fu Chen, Youngwook Do, Shaohan Hu, Tianyi Zhou, Huajie Shao*

#### 🧩 TL;DR
本文提出高效持续对齐（ECA），一种无样本的增量学习方法，用于解决开放域图像到文本生成中视觉数据类别漂移时的灾难性遗忘问题，通过混合查询令牌、Fisher动态扩展和字典重播三种机制实现持续对齐，实验表明ECA显著优于基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有增量学习研究主要关注分类任务或静态数据分布，缺乏对开放域图像到文本生成（OpenITG）中视觉数据主导类别随时间漂移这一实际场景的探索；核心挑战在于模型需持续获取新任务特定特征，同时在不访问旧数据的情况下最小化对已建立跨模态对齐的干扰。

**Method:** ECA提出三种核心机制：混合查询（MoQ）模块用于自适应任务特定查询令牌；基于Fisher信息矩阵的Fisher动态扩展（FeDEx）机制实现模型结构动态扩展；以及嵌入字典与字典重播（DR）策略保留历史知识。整体上，该方法在预训练视觉语言模型中对齐模块进行增量式持续对齐，且无需依赖旧任务原始数据。

**Result:** 论文构建了四个更贴近真实场景的增量学习OpenITG基准，实验结果显示ECA在缓解灾难性遗忘方面效果显著，整体增量学习性能全面优于对比基线方法。

**Conclusion:** 本研究验证了通过持续对齐机制在无样本条件下有效实现开放域图像到文本增量学习的可行性，为视觉语言模型在动态环境下的持续学习提供了新思路，代码和基准已开源。

---

#### 📄 Abstract
Incremental Learning (IL) for Open-ended Image-to-Text Generation (OpenITG) enables models to continuously generate accurate, contextually relevant text for new images while preserving previously acquired knowledge. Unlike prior studies, this paper addresses a more practical scenario in which the predominant category of visual data shifts over time as environments evolve. In this context, we introduce a new notion of continual alignment, which incrementally adapts the alignment module within pre-trained VLMs to preserve high-quality cross-modal representations. Based on this idea, we propose Efficient Continual Alignment (ECA), a novel exemplar-free IL approach for OpenITG. The key challenge is enabling the model to acquire new, task-specific features while minimizing interference with the established alignment without accessing raw data from previous tasks. To address this, ECA employs three core mechanisms: a Mixture of Query (MoQ) module that adapts task-specific query tokens, a Fisher Dynamic Expansion (FeDEx) that dynamically expands model structure based on a Fisher Information Matrix (FIM)-based metric, and an embedding dictionary with Dictionary Replay (DR) to retain past knowledge. To evaluate ECA's performance, we construct four new IL OpenITG benchmarks that better reflect real-world scenarios. Experimental results demonstrate that ECA significantly mitigates catastrophic forgetting and improves IL performance compared to baseline methods. Code and benchmarks are available at https://github.com/Snowball0823/ECA.


### [4] [SalArt-VQA: Diagnosing Whether VLMs Understand Salient Artifacts in Generated Images](https://arxiv.org/abs/2606.12671)
*Xiaoxiao Sun, Ruotian Zhang, Junzhe Huang, James Burgess, Serena Yeung-Levy*

#### 🧩 TL;DR
提出SalArt-VQA基准，用于细粒度评估视觉语言模型对AI生成图像中显著人工痕迹的理解能力，揭示图像级检测准确率无法反映的深层失败模式，如错误依赖视觉线索、错误定位或提出图像不支持的错误缺陷。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究主要关注视觉语言模型（VLM）能否正确检测AI生成图像中的人工痕迹，但忽略了模型可能仅凭偶然正确的线索做出正确判断，实际存在依赖错误视觉线索、选择错误区域或描述图像不支持的缺陷等问题，这些隐藏的失败模式无法通过图像级检测准确率反映。

**Method:** 构建SalArt-VQA诊断基准，包含950张图像和3,681道人工编写的多选题，涵盖人工痕迹图像、匹配的真实参考图像和配对生成参考图像；设计四种对齐问题类型（存在检测、语义定位、空间 grounding 和证据支持缺陷识别），并通过参考样本分裂测试模型在缺陷缺失时的校准和弃权能力。

**Result:** 在20个VLM上评估发现，最强模型在人工痕迹图像上的检测召回率达99.37%，但仅在53.26%的图像上正确回答所有四类问题；人工痕迹图像与无痕迹参考图像的对比揭示模型的敏感性-校准权衡：敏感模型易提出无证据支持的人工痕迹主张，而保守模型主要通过遗漏真实痕迹避免误报。

**Conclusion:** 高检测准确率并不意味着模型对人工痕迹有 grounded 的理解；SalArt-VQA暴露了传统评估无法发现的隐藏失败模式，提供了基于局部视觉证据的细粒度评估方法，表明需要超越图像级指标来评估VLM的工件理解能力。

---

#### 📄 Abstract
Vision-language models (VLMs) are increasingly used to detect whether AI-generated images contain visible artifacts, yet their ability to analyze such artifacts remains poorly understood. A correct image-level decision can still hide important failures: a model may correctly flag an artifact while relying on the wrong visual cue, selecting the wrong region, or describing a defect that the image does not support. To evaluate these behaviors directly, we introduce SalArt-VQA, a diagnostic benchmark for fine-grained SALient ARTifact understanding in AI-generated images. SalArt-VQA contains 950 images and 3,681 human-authored multiple-choice questions spanning artifact images, matched real reference images, and paired generated reference images. Four aligned question types evaluate presence detection, semantic localization, spatial grounding, and evidence-grounded defect identification, while the reference splits test calibration and abstention when the annotated defect is absent. Across 20 VLMs, SalArt-VQA reveals failures that image-level detection accuracy hides: the strongest model reaches 99.37% detection recall on artifact images but answers all four artifact-side questions correctly on only 53.26% of images. Comparing artifact images with artifact-free references reveals a sensitivity-calibration tradeoff: sensitive models often make unsupported artifact claims, while conservative models avoid false alarms largely by missing real artifacts. These results show that high artifact detection accuracy alone does not imply grounded artifact understanding. SalArt-VQA exposes these hidden failure modes and provides a fine-grained evaluation of whether VLM artifact claims are supported by local visual evidence.


### [5] [Perceive, Interact, Reason: Building Tool-Augmented Visual Agents for Spatial Reasoning](https://arxiv.org/abs/2606.12830)
*Changye Li, Meng Lu, Yi Wu, Ligeng Zhu*

#### 🧩 TL;DR
提出PERIA，一个通过工具增强和多智能体策略优化来提升视觉语言模型空间推理能力的视觉代理框架，在多个空间推理任务上显著超越同规模模型并达到与超大模型相当的性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有的视觉语言模型尽管具有强大的多模态理解能力，但在需要主动证据获取和多步视觉交互的空间推理任务中表现有限，仅依赖视觉编码器的隐式表示不足以恢复细粒度的空间证据。

**Method:** 提出PERIA框架，包含两类轻量工具族：视觉感知工具用于暴露文本、符号和空间证据，视觉交互工具用于操作视觉上下文、追踪路径和验证空间关系；采用监督式工具使用轨迹合成、复合奖励以及观察松弛组内组策略优化（OR-GIGPO）进行联合训练。

**Result:** 在8个数据集的13个基准上，PERIA-8B相较于Qwen3-8B骨干网络在分布内基准上提升10.0%，在分布外基准上提升4.4%，超越此前同规模最优基线7.0%-14.8%，性能可与Qwen3-VL-235B-A22B-Thinking和GPT-5等超大模型相媲美。

**Conclusion:** PERIA证明了通过工具增强和有效的策略优化可以显著提升小型VLM的空间推理能力，使其达到甚至接近超大模型的水平，为构建高效的视觉代理系统提供了可行路径。

---

#### 📄 Abstract
While recent vision-language models (VLMs) demonstrate strong multimodal understanding, they remain limited in spatial reasoning tasks that require active evidence acquisition and multi-step visual interaction. This limitation suggests that relying solely on implicit visual representations from vision encoders is insufficient for recovering fine-grained spatial evidence. We introduce PERception-Interaction-reason Agent (PERIA), a tool-augmented visual agent for spatial reasoning tasks across map reasoning, visual probing, and vision reconstruction. PERIA uses two lightweight tool families: vision perception tools for exposing textual, symbolic, and spatial evidence, and vision interaction tools for manipulating visual context, tracing paths, and verifying spatial relations. To train PERIA, we develop a unified recipe that combines supervised tool-use trajectory synthesis, composite rewards, and Observation-Relaxed Group-in-Group Policy Optimization (OR-GIGPO) for effective multi-tool behavior. Experiments on 13 benchmarks from 8 datasets show that PERIA-8B improves over the Qwen3-8B backbone by 10.0% on in-distribution benchmarks and 4.4% on out-of-distribution benchmarks, while outperforming previous state-of-the-art baselines of similar size by 7.0%-14.8%. It also achieves performance comparable to much larger models such as Qwen3-VL-235B-A22B-Thinking and GPT-5, demonstrating the effectiveness of PERIA in enhancing spatial reasoning capabilities.


### [6] [Language-Guided Abstraction for Visual Reasoning](https://arxiv.org/abs/2606.12847)
*Xu-Jing Ye, Yuan-Gen Wang, Ruping Wang*

#### 🧩 TL;DR
提出L-VARC框架，通过语言引导的学习利用特权信息（LUPI）分支增强视觉推理，仅用18M参数即在ARC任务上取得超越现有方法的效果，推理时不引入额外语言分支以保持轻量。

---

#### 📘 Detailed Summary
**Motivation:** 现有ARC研究方法存在两极化问题：纯语言方法依赖LLM需数十亿参数，视觉方法则难以捕捉高层语义、容易过拟合像素级模式，两者均难以有效平衡语义理解与计算效率。

**Method:** 设计语义压缩模块，将统一任务无关提示输入DeepSeek-V3对原始LARC语言描述精炼结构化，使其适配文本编码器（如CLIP）的上下文长度约束；设计交叉注意力投影器对齐视觉特征与语义嵌入，以LUPI分支在训练阶段引导视觉模型，推理时丢弃该分支，最终模型仅18M参数。

**Result:** L-VARC在ARC基准上超越现有最先进方法，消融实验验证了语义压缩模块和交叉注意力投影器的各自贡献，模型仅18M参数即实现高效视觉推理。

**Conclusion:** 语言引导的LUPI机制可在不增加推理负担的前提下有效提升视觉推理能力，为ARC任务提供了一条兼顾性能与轻量化的可行路径。

---

#### 📄 Abstract
The Abstraction and Reasoning Corpus (ARC) is viewed as a critical avenue to Artificial General Intelligence (AGI), as it enables models to learn abstract transformation rules from few-shot examples and then generalize to new tasks. However, prevalent ARC methodology is either pure language or vision-only (i.e., VARC). The former depends heavily on LLMs, consuming billions of parameters. The latter often struggles to capture high-level semantics, leading to overfitting on pixel-level patterns. To bridge this gap, we propose L-VARC, a novel framework that enhances visual reasoning via a language-guided Learning Using Privileged Information (LUPI) branch. Specifically, we design a Semantic Compression Module by feeding a unified, task-agnostic prompt into DeepSeek-V3. In this way, the raw LARC (a crowd-sourced language description dataset) can be substantially refined and structured, fitting with the context length constraint of standard text encoders (e.g., CLIP). Moreover, we design a Cross-Attention Projector to align visual features with semantic embeddings, aiming to guide the training of the ARC model. Notably, the LUPI branch is taken in the training process and will be discarded during inference, thereby yielding a lightweight model with a mere 18 million parameters. Extensive experiments demonstrate that our L-VARC effectively leverages linguistic priors to boost visual reasoning and outperforms state-of-the-art. Ablation studies further confirm the contribution of the two new designs towards the L-VARC framework. The code is available at https://github.com/GZHU-DVL/L-VARC.


### [7] [Magnifying What Matters: Attention-Guided Adaptive Rendering for Visual Text Comprehension](https://arxiv.org/abs/2606.12898)
*Shenglai Zeng, Qirui Wang, Kai Guo, Xinnan Dai, Xianxuan Long, Hui Liu*

#### 🧩 TL;DR
本文揭示了视觉语言模型在视觉文本理解任务中存在"定位而不利用"的注意力特性，并提出AGAR——一种无需训练、模型无关的自适应渲染方法，通过利用模型自身中后层注意力机制定位关键视觉区域并重渲染来提升模型性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉文本理解流水线将文本渲染和布局视为固定的、与内容无关的预处理步骤，并且缺乏对VLM在内部如何处理可视化文本的机制性理解，这限制了模型在长页OCR、多页记忆QA等任务中的性能提升。

**Method:** AGAR是一种无需训练、模型无关的方法，它利用VLM自身中后层的注意力机制识别top-K重要的视觉补丁，将这些补丁映射回文本跨度，然后重新渲染页面以放大这些关键跨度，最后重新推理答案。

**Result:** 在九个VTC基准测试（包括短文本、长上下文和多页记忆QA）以及四个VLM主干模型上的大量实验表明，AGAR作为即插即用的增强模块能一致性地提升现有VLM的性能，可与VLM后训练组合以获取进一步增益，并且在视觉和文本侧的输入退化下保持鲁棒。

**Conclusion:** VLM在VTC任务中表现出"定位而不利用"的注意力机制——证据定位注意力在中后层显著出现但与答案正确性基本解耦，而仅需放大渲染页面上的定位跨度即可恢复大部分失败案例，这为理解VLM内部机理和设计高效的推理时增强策略提供了重要启示。

---

#### 📄 Abstract
Visual Text Comprehension (VTC) renders text into images for a vision-language model (VLM) to read, sidestepping LLM context-window limits and powering applications from long-page OCR to multi-page memory QA. Yet existing VTC pipelines treat rendering and layout as a fixed, content-agnostic preprocessing step and offer little mechanistic understanding of how VLMs internally process visualized text. Through a focused empirical study on VTC QA tasks, we reveal that VLMs exhibit a localization-without-utilization regime: evidence-localizing attention emerges sharply in the middle-to-late layers and is largely decoupled from answer correctness, yet simply enlarging the localized spans on the rendered page recovers a large fraction of the failures. Building on these observations, we propose AGAR (Attention-Guided Adaptive Rendering), a training-free, model-agnostic method that leverages a VLM's own middle-to-late layer attention to identify the top-K important visual patches, maps them back to word spans, and re-renders the page with those spans enlarged before re-inferring the answer. Extensive experiments across nine VTC benchmarks (short-form, long-context, and multi-page memory QA) and four VLM backbones show that AGAR (i)consistently improves off-the-shelf VLMs as a plug-and-play enhancement, (ii)composes with VLM post-training to yield further gains, and (iii)remains robust under both visual- and text-side input degradation.


### [8] [Multi-Label Test-Time Adaptation with Bayesian Conditional Priors](https://arxiv.org/abs/2606.12925)
*Qiru Li, Ao Zhou, Zhiwei Jiang, Zifeng Cheng, Cong Wang, Yafeng Yin, Qing Gu*

#### 🧩 TL;DR
提出贝叶斯条件先验估计（BCP），一种无需梯度的测试时自适应方法，通过估计图像的条件标签先验来注入标签依赖关系，在不微调视觉-语言模型骨干网络的情况下显著提升多标签识别在分布偏移下的鲁棒性。

---

#### 📘 Detailed Summary
**Motivation:** 现有的基于冻结视觉-语言模型（VLM）的多标签零样本识别方法在分布偏移下表现脆弱，因为标准零样本推理独立地对每个标签进行打分，忽略了标签之间的共现结构，导致主导概念抑制了兼容的弱标签，产生不连贯的预测标签集。

**Method:** BCP将零样本logits视为在固定图像-文本似然下的边际后验代理，将分布偏移引起的误差主要归因于标签先验的不匹配；对每个测试图像，先选择高置信度的锚标签，然后进行以锚为条件的贝叶斯修正，该更新在logit空间中有闭式解并可解释为点互信息（PMI），从而显式促进兼容标签并抑制不兼容标签；无需目标域标注，通过从无标签测试流中在线估计锚条件先验，仅需轻量级的二阶共现统计量，计算开销几乎等同于单次前向传播。

**Result:** 在标准多标签基准和多种CLIP骨干网络上，BCP一致优于强测试时自适应（TTA）基线，例如在RN50上将平均mAP从57.31提升至69.22，在ViT-B/16上从62.61提升至71.79。

**Conclusion:** 该方法表明，通过轻量级贝叶斯先验修正，无需微调或额外标注即可有效恢复VLM在多标签识别中的分布外泛化能力，标签依赖关系的显式建模是提升鲁棒性的关键。

---

#### 📄 Abstract
Multi-label recognition with frozen Vision-Language Models (VLMs) is brittle under distribution shift: standard zero-shot inference scores labels independently, ignoring co-occurrence structure and producing incoherent label sets where dominant concepts suppress weaker but compatible labels. We introduce Bayesian Conditional Priors (BCP) Estimation, a gradient-free test-time adaptation method that injects label dependency without tuning the backbone. BCP views zero-shot logits as a proxy for marginal posteriors under a fixed image-text likelihood and attributes shift-induced errors mainly to a mismatched label prior. For each test image, it selects a high-confidence anchor label and applies an anchor-conditioned Bayesian refinement. This update is closed-form in logit space and admits a pointwise mutual information (PMI) interpretation, explicitly promoting compatible labels and suppressing incompatible ones. BCP operates without target annotations by estimating anchor-conditioned priors online from the unlabeled test stream via lightweight second-order co-occurrence statistics, adding negligible overhead beyond a single forward pass. Across standard multi-label benchmarks and multiple CLIP backbones, BCP consistently outperforms strong TTA baselines, e.g., improving RN50 average mAP from 57.31 to 69.22 and ViT-B/16 from 62.61 to 71.79.


### [9] [MAMVI: 3D Test-Time Adaptation via Masked Multi-View Point Clouds](https://arxiv.org/abs/2606.12939)
*Inseok Kong, Geunyoung Jung, Jiyoung Jung*

#### 🧩 TL;DR
提出MAMVI方法，通过混合掩码策略和单步多视图聚合优化，替代传统顺序多视图优化，在三维点云测试时自适应中实现4.9-8.9倍加速，并在ShapeNet-C和ScanObjectNN-C上达到最优精度。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于多视图增强的三维点云测试时自适应方法采用顺序优化，每个视图独立处理，导致推理延迟高、难以满足实时应用需求。

**Method:** 提出MAMVI框架，核心设计包括：混合掩码策略（固定比例掩码保证稳定性+基于Beta分布的采样掩码增强多样性）；通过多视图损失聚合进行单步反向传播实现统一自适应；以及基于置信度的自适应学习率动态调整每个样本的适应强度。

**Result:** 在ModelNet-40C, ShapeNet-C和ScanObjectNN-C三个基准上评估，MAMVI在ShapeNet-C和ScanObjectNN-C上取得最优精度，在ModelNet-40C上保持竞争力，同时实现了4.9-8.9倍的推理加速。

**Conclusion:** MAMVI通过单步多视图聚合替代顺序优化，在保持甚至提升自适应精度的同时大幅降低推理延迟，证明了其在实时三维点云测试时自适应中的实用性和有效性。

---

#### 📄 Abstract
3D point cloud models suffer significant performance degradation under distribution shifts caused by sensor noise, occlusions, and environmental changes. Test-time adaptation (TTA) has emerged as a practical paradigm for mitigating this issue during inference. Recently, leveraging multi-view augmentation has shown promise in improving 3D TTA performance. However, existing multi-view approaches are often constrained by sequential optimization that treats each view independently. This sequential optimization leads to substantial inference latency due to repetitive optimization steps, making real-time adaptation impractical. To address this, we propose Masked Multi-View Test-Time Adaptation (MAMVI), which replaces sequential optimization with a unified single-step adaptation. Specifically, MAMVI utilizes a hybrid masking strategy that combines fixed ratios for stability with Beta-distributed sampling for diversity. By aggregating losses across multiple views, MAMVI performs adaptation through a single backward pass based on multi-view consensus. Additionally, a confidence-based adaptive learning rate is used to dynamically adjust the adaptation intensity for each sample. Extensive experiments on ModelNet-40C, ShapeNet-C, and ScanObjectNN-C demonstrate that MAMVI achieves state-of-the-art accuracy on ShapeNet-C and ScanObjectNN-C. Moreover, it remains competitive on ModelNet-40C while delivering 4.9-8.9 times faster inference, making it highly suitable for real-time applications. Our code is available at https://github.com/Inseok-kong/MAMVI


### [10] [Efficient, Robust, and Anti-Collusion Fingerprinting of Image Diffusion Models](https://arxiv.org/abs/2606.12977)
*Jianwei Fei, Yunshu Dai, Zhihua Xia, Xiaochun Cao, Jiantao Zhou, Alessandro Piva, Benedetta Tondi*

#### 🧩 TL;DR
本文首次揭示现有生成式模型指纹方法在面对共谋攻击时缺乏鲁棒性，并提出一种面向文本到图像模型的具有抗共谋能力的鲁棒指纹方法，通过个性化归一化模块编码指纹并引入基于无损函数不变参数变换的抗共谋机制，实现了超过99.5%的指纹提取准确率并显著增加共谋模型的FID。

---

#### 📘 Detailed Summary
**Motivation:** 现有生成式文本到图像模型的指纹方法用于保护知识产权和防止未授权再分发，但被发现存在系统性的安全漏洞：它们缺乏对共谋攻击的鲁棒性，即多个攻击者联合其模型可以移除或混淆指纹，这一问题此前未被系统性地探索过。

**Method:** 提出将指纹位串编码到个性化归一化模块（PNM）的系数中，并将其嵌入T2I模型，使得指纹可从任何生成图像中可靠恢复；引入基于无损函数不变参数变换的抗共谋机制，该机制会显著降低共谋模型的图像生成质量，使其不可用；此外，通过重参数化PNM实现高效创建多个带指纹的模型副本而无需重新训练，并采用最坏情况优化策略提升对模型级攻击的鲁棒性。

**Result:** 在多个T2I图像生成和编辑任务上，该方法实现了高保真度和鲁棒性，指纹提取准确率超过99.5%；与现有方法相比，该方法首次在对共谋攻击的主动鲁棒性上取得显著效果，能够大幅提升共谋模型的FID（弗雷歇初始距离），使其生成质量严重退化。

**Conclusion:** 本文揭示了现有指纹方法在共谋攻击下的脆弱性，并提出了一种具备抗共谋能力的鲁棒指纹框架，为T2I模型的知识产权保护提供了新的技术路径；所提出的机制不仅保持了高精度指纹提取，还能主动惩罚共谋行为，具有重要的实际应用价值和未来研究方向。

---

#### 📄 Abstract
Model fingerprinting, embedding user-specific identifiers (fingerprints) into generated outputs, has recently emerged as a popular solution to protect the intellectual property rights (IPR) of generative text-to-image (T2I) models and prevent unauthorized redistribution. In this work, we reveal a previously unexplored systematic vulnerability in existing generative model fingerprinting methods: they lack robustness against collusion attacks, where multiple attackers combine their models to remove or obscure the fingerprints. To address this issue, we take the first step towards a robust fingerprinting method for T2I models with anti-collusion capabilities. The proposed method encodes strings of bits, namely fingerprints, into the coefficients of a personalized normalization module (PNM) incorporated into T2I models, so that fingerprints can be reliably recovered from any generated image. To defend against collusion attacks and prevent unauthorized model redistribution, we introduce an anti-collusion mechanism based on lossless function-invariant parameter transformations. This mechanism significantly degrades the image generation quality of colluded models, making them effectively unusable. Moreover, our method allows developers to efficiently create multiple copies of fingerprinted T2I models by reparameterizing the PNM without the need for retraining. We also introduce a worst-case optimization strategy to improve robustness against model-level attacks. Our experiments demonstrate that the proposed method achieves high fidelity and robustness across multiple T2I image generation and editing tasks, with fingerprint extraction accuracy exceeding 99.5%. Compared with existing methods, our method demonstrates, for the first time, a notable proactive robustness to collusion attacks by significantly increasing the FID of colluded models.


### [11] [Objects Before Words: Object-First Inductive Biases for Grounding Language in Child-View Video](https://arxiv.org/abs/2606.12985)
*Sathira Silva, Abrham Kahsay Gebreselasie, Muhammad Umer Sheikh, Kartik Kuckreja, Daniel Harari, Muhammad Haris Khan*

#### 🧩 TL;DR
BabyMind提出了一种面向婴儿视角对比学习的物体优先偏置方法，通过离线掩码区域接口提取候选物体嵌入、基于追踪将窗口内物体关联为轻量级物体文件、以及使用原型空间多实例对比目标将话语与物体文件袋对齐，在SAYCam-S上提升了标签选择的准确率。

---

#### 📘 Detailed Summary
**Motivation:** 婴儿视角记录中，看护者言语稀疏且与自我中心视频弱同步，单帧对比配对会产生噪声正样本（目标物体缺失或被干扰物干扰），使得从自然经验中学习接地词汇含义面临"何时出现"和"何地存在"的双重歧义问题。

**Method:** BabyMind采用物体优先偏置，首先利用离线掩码区域接口提取候选物体嵌入，通过追踪将短话语窗口内的候选物体关联为轻量级物体文件，然后使用原型空间多实例对比目标将话语与物体文件袋对齐，并引入轨道一致性和全局物体一致性正则化项来稳定学习并将物体文件结构迁移到全局帧嵌入中。

**Result:** 在SAYCam-S数据集上，BabyMind在Labeled-S 15项二选一准确率上比CVCL提升了2.6个百分点，并在词内分布外基准测试上取得了一致的性能提升。

**Conclusion:** 该研究证明了在稀疏、噪声监督条件下，引入物体优先偏置和多实例对比学习能够有效提升婴儿视角下词汇接地学习的效果，且物体文件结构可成功迁移到全局嵌入表征中。

---

#### 📄 Abstract
Learning grounded word meaning from natural experience requires resolving two ambiguities in infant-view recordings: when the named referent appears and where it is in a cluttered frame. In SAYCam-style data, caregiver speech is sparse and weakly synchronized with egocentric video, so single-frame contrastive pairing yields noisy positives in which the intended object is absent or entangled with distractors. We propose BabyMind, an object-first bias for child-view contrastive learning under sparse, noisy supervision. BabyMind extracts candidate object embeddings using an offline mask-based region interface, links candidates across a short utterance-centered window into lightweight object files via tracking, and aligns utterances to bags of object files with a prototype-space multiple-instance contrastive objective. Track-coherence and global-object agreement regularizers stabilize learning and transfer object-file structure into the global frame embedding used at evaluation. On SAYCam-S, BabyMind improves Labeled-S 15 forced-choice accuracy by +2.6 points over CVCL and yields consistent gains on in-vocabulary out-of-distribution benchmarks. Code is available at https://github.com/sathiiii/BabyMind.


### [12] [Diffusion Transformer World-Action Model for AV Scene Prediction](https://arxiv.org/abs/2606.12987)
*Ruslan Sharifullin, Benjamin Jiang, Kai Xi Chew*

#### 🧩 TL;DR
本文提出一个紧凑的潜在世界模型，用于自动驾驶中基于动作条件的前置摄像头场景预测，并揭示传统失真指标（如SSIM、余弦相似度）会误导地奖励模糊的回归均值，而扩散模型在感知指标（FID/KID）上显著优于回归模型（KID 0.078 vs 0.375，提升4.8倍），同时实现真实动作可控性（转向位移Spearman ρ=0.81）。

---

#### 📘 Detailed Summary
**Motivation:** 现有动作条件世界模型在紧凑可训练规模下预测未来场景存在歧义性，且传统失真评估指标（如SSIM、余弦相似度）会误导性地奖励模糊的回归均值而非真实感预测，导致质量评估与实际视觉效果严重偏差，亟需一种能产出真实感、可动作控制的未来帧预测方法及与之匹配的评估体系。

**Method:** 构建了一个紧凑的潜在世界模型流水线：利用冻结编码器将当前前置摄像头帧编码为潜在表示，基于未来自车动作序列预测未来场景潜在表示，再通过冻结解码器渲染为256×256像素、最长8秒的未来帧；系统性地比较了六种冻结编码器（涵盖四种表征族），发现V-JEPA2结合时序上下文使转向RMSE降低40%；训练潜在扩散Transformer（DiT）并诊断出四个关键组件：空间token、x_0目标、残差锚定、匹配目标不确定性的采样；在Stable Diffusion VAE的编码-预测-解码流水线中揭示了感知-失真前沿的张力。

**Result:** 扩散模型在KID上达到0.078，回归模型为0.375（提升4.8倍）；扩散模型在转向位移上与真实值呈现强相关性（Spearman ρ=0.81），回归模型仅为-0.18；设计了一个1.7M参数的紧凑跳跃（jump）模型，可恢复完整的地面真实运动幅度（1.02× GT），而单次传播模型捕获量不到一半。

**Conclusion:** 该工作确立了感知指标（FID/KID）应作为评估生成式世界模型的核心标准，揭示了失真指标掩盖扩散模型真实优势的关键问题；提出了面向部署的可训练校准方法，无需测试时真实值即可实用化；指出当前单次传播模型因共享当前帧锚定而限制了运动幅度，并提供了有效的轻量级解决方案。

---

#### 📄 Abstract
Action-conditioned world models let an autonomous vehicle predict future camera scenes from its own planned controls, enabling planning and simulation without real-world rollouts, but at compact, trainable scale the futures are ambiguous and the field's standard distortion metrics actively mislead: they reward a blurry regression mean over a realistic prediction. We confront this with a compact latent world model that, given the present front-camera latent and a sequence of ego-actions, predicts future scene latents a frozen decoder renders to $256 \times 256$ frames up to 8 seconds ahead, evaluated on 150 held-out nuScenes scenes. We first benchmark where to predict: across six frozen encoders spanning four representation families, V-JEPA2 with temporal context reduces steering RMSE by 40% over the best single-frame encoder. We then train a latent Diffusion Transformer (DiT) and, through a controlled diagnosis, identify the four ingredients it needs: spatial tokens, the $x_0$ objective, residual anchoring, and sampling matched to target uncertainty. In a Stable-Diffusion-VAE encode-predict-decode pipeline we expose the central tension: distortion metrics (cosine similarity, SSIM) favor the blurry mean, masking that the diffusion model is far closer to the real frame distribution. Inception-based FID and KID reveal a clean perception-distortion frontier: diffusion attains KID 0.078 versus 0.375 for regression ($4.8\times$ better), and a deployable train-derived calibration makes this practical without test-time ground truth. The model is genuinely action-controllable (steering drives scene displacement, Spearman $ρ= 0.81$, vs $-0.18$ for regression). We trace limited single-pass motion to a shared-present anchor and engineer a compact 1.7M-parameter "jump" model that recovers full ground-truth motion magnitude ($1.02\times$ GT), where single-pass models capture less than half.


### [13] [Quality-Preserving Imperceptible Adversarial Attack on Skeleton-based Human Action Recognition](https://arxiv.org/abs/2606.13022)
*Ziyi Chang, Kanglei Zhou, Xiaohui Liang, Hubert P. H. Shum*

#### 🧩 TL;DR
本文提出一种基于分布的对抗攻击方法，在不引入噪声类扰动的前提下生成高质量对抗样本，并设计了一种与人类感知对齐的新评估指标，揭示了现有骨架动作识别系统在对抗鲁棒性方面的严重缺陷。

---

#### 📘 Detailed Summary
**Motivation:** 现有针对骨架人体动作识别的对抗攻击方法通常引入噪声类扰动，导致攻击后动作质量显著下降，容易被先进识别系统察觉。作者发现这一问题源于攻击优化过程中经验风险与真实风险之间的差距，因此亟需一种既能保持动作质量又能实现高攻击成功率的对抗攻击方法。

**Method:** 本文提出一种基于分布的对抗攻击方法，通过最小化经验风险与真实风险之间的差距来保持动作质量，避免引入噪声类扰动。同时，作者提出了一种与人类感知对齐的新评估指标，用于真实地衡量攻击后动作的自然性。实验在两个数据集上对多个最先进的骨架动作识别模型进行了评估。

**Result:** 实验结果表明，所提方法在攻击成功率和攻击后动作质量两方面均优于现有方法，定性和定量分析均验证了其有效性。该方法成功实现了质量保持的对抗攻击，凸显了当前动作识别系统鲁棒性的不足。

**Conclusion:** 本文的工作揭示了骨架动作识别系统在对抗环境下的脆弱性，表明即使在不破坏动作质量的前提下，攻击者仍能有效欺骗识别模型。这为该领域的鲁棒性增强研究提供了重要的警示和方向。

---

#### 📄 Abstract
Adversarial attacks on skeletal human action recognition have received significant attention. However, existing methods typically introduce noise-like perturbations that degrade motion quality post-attack, and thereby are inherently perceptible with recent advancements in S-HAR systems. We discover that this degradation stems from the gap between empirical and true risks during the optimization process of previous adversarial attacks. To address this issue, we propose an attack where adversarial motions are obtained without compromising their motion quality. To minimize the risk gap and preserve motion quality, we propose a distribution-based adversarial attack method without introducing noise-like perturbations. To faithfully evaluate the motion quality, we propose a new metric that aligns with human perception on real-world naturalness. Experiments have been conducted on the state-of-the-art S-HAR methods across two datasets, demonstrating the superiority of our method in both the attack success rate and the post-attack motion quality through qualitative and quantitative analyses. The success of our quality-preserving attack application and distribution-based method raises serious concerns about the robustness of action recognizers, highlighting the need for further enhancements in this domain.


### [14] [A Multi-Modal Framework with Cross-Subject Pseudo-Labeling and Semantic Alignment for Micro-Gesture Recognition](https://arxiv.org/abs/2606.13030)
*Haoran Zhang, Haokun Zhang, Pengyu Liu, Yujia Zhang, Weibao Xue, Yanbin Hao*

#### 🧩 TL;DR
本文针对微手势识别中信号微弱、长尾分布和跨主体域偏移等挑战，提出了一种结合显著性引导多模态提取、温和平方根平滑加权、正交语义嵌入损失和跨模态伪标签无监督域自适应的多模态框架，在第四届MiGA-IJCAI挑战赛Track 1中以68.13%的F1-score获得第四名。

---

#### 📘 Detailed Summary
**Motivation:** 微手势作为自发的细微身体动作常携带隐藏情绪信息，但未经裁剪视频中的微手势识别面临三大核心挑战：极低的信噪比导致信号难以捕捉、严重的长尾类别分布使得尾部类识别困难、跨主体评估场景中固有的域偏移问题导致模型泛化能力不足。

**Method:** 提出一个多模态框架，包含显著性引导的多模态提取流水线，整合68关键点骨架关节坐标、3D热图体积和高分辨率RGB视觉特征以捕获细粒度表征；引入温和平方根平滑加权机制结合正交语义嵌入损失以保护尾部类别且不损害整体识别能力；提出跨模态伪标签策略用于无监督域自适应以桥接跨主体泛化差距，最后采用温度缩放软投票机制缓解晚期融合中的过度自信问题。

**Result:** 在第四届MiGA-IJCAI挑战赛Track 1上，所提框架取得了具有竞争力的68.13% F1-score，最终排名第四，验证了各模块在微手势识别中的有效性。

**Conclusion:** 该研究证明结合多模态细粒度特征提取、类别平衡加权机制与跨模态无监督域自适应策略能够有效应对微手势识别的低信噪比、长尾分布和跨主体泛化三大核心难题，为复杂场景下的细微动作识别提供了可行的技术路径。

---

#### 📄 Abstract
Micro-gestures (MGs) are spontaneous and subtle body movements that frequently convey hidden human emotions. Recognizing MGs in untrimmed videos remains highly challenging due to their extremely low signal-to-noise ratio, severe long-tailed class distribution, and the inherent domain shift encountered in cross-subject evaluation scenarios. In this paper, we propose a comprehensive multi-modal framework for Track 1 of the 4th MiGA-IJCAI Challenge. To capture fine-grained representations, we design a saliency-guided multi-modal extraction pipeline integrating 68-keypoint skeleton joint coordinates, 3D heatmap volumes, and high-resolution RGB visual features. We introduce a gentle square-root smoothed weighting mechanism paired with an Orthogonal Semantic Embedding Loss to protect tail classes without compromising overall recognition capabilities. More importantly, to bridge the cross-subject generalization gap, we propose a Cross-Modal Pseudo-Labeling (CMPL) strategy for unsupervised domain adaptation, which significantly boosts single-modal robustness. A temperature-scaled soft-voting mechanism is finally utilized to alleviate overconfidence during late fusion. Extensive experiments demonstrate that our framework achieves a competitive F1-score of 68.13\%, securing the 4th place.


### [15] [TetherCache: Stabilizing Autoregressive Long-Form Video Generation with Gated Recall and Trusted Alignment](https://arxiv.org/abs/2606.13035)
*Yu Meng, Xiangyang Luo, Letian Li, Wenyuan Jiang, Chen Gao, Xinlei Chen, Yong Li, Xiao-Ping Zhang*

#### 🧩 TL;DR
TetherCache 是一种无需训练、即插即用的缓存管理策略，通过门控召回与记忆对齐机制，有效缓解自回归视频扩散模型在超长视频生成中的上下文分布漂移和视觉退化问题，在240秒生成任务中将质量漂移从7.84降至1.33。

---

#### 📘 Detailed Summary
**Motivation:** 自回归视频扩散模型通过条件化已生成帧来实现流式与可变长度视频生成，但在扩展到分钟级生成时面临两大挑战：有限的KV缓存预算使模型无法保留完整历史信息，而反复基于自生成帧进行条件化会累积上下文分布漂移，导致视觉伪影、质量退化及时间漂移。

**Method:** TetherCache 将缓存组织为sink、memory和recent三个区域，并提出两种互补机制：GRAB（门控召回与注意力多样性平衡）通过结合注意力相关性和时间多样性的门控分数，在固定缓存预算下选择信息丰富且多样化的远程记忆帧；TAME（基于记忆编辑的可信对齐）通过将新召回的记忆token的统计量对齐到可信上下文分布，减少漂移历史特征造成的污染。

**Result:** 基于Self-Forcing框架，TetherCache在VBench-Long基准上持续提升30秒、60秒和240秒设置下的长视频生成质量。在240秒生成任务中，TetherCache显著提高了整体评分和语义评分，同时将质量漂移从7.84降至1.33，证明了其在稳定长程自回归视频扩散中的有效性。

**Conclusion:** TetherCache通过训练无关的缓存管理与记忆对齐策略，有效解决了自回归视频扩散模型在长视频生成中的上下文分布漂移问题，为稳定、高质量的超长视频生成提供了一种即插即用的实用解决方案。

---

#### 📄 Abstract
Autoregressive video diffusion models provide a natural formulation for streaming and variable-length video generation by conditioning newly generated frames on previously generated content. However, extending these models to minute-level generation remains challenging: the limited KV-cache budget prevents the model from retaining the full history, while repeatedly conditioning on self-generated frames induces a context distribution shift that accumulates over time, leading to visual artifacts, quality degradation, and temporal drift. In this paper, we propose TetherCache, a training-free and plug-and-play cache management strategy for drift-resistant long video generation. TetherCache organizes the cache into sink, memory, and recent regions, and introduces two complementary mechanisms. First, GRAB (Gated Recall with Attention-Diversity Balancing) selects long-range memory frames using a gated score that combines attention-based relevance with temporal diversity, preserving informative yet diverse historical context under a fixed cache budget. Second, TAME (Trusted Alignment via Memory Editing) lightly edits newly recalled memory tokens by aligning their statistics to a trusted context distribution, reducing the pollution caused by drifted historical features. Built on Self-Forcing, TetherCache consistently improves long-video generation quality on VBench-Long across 30s, 60s, and 240s settings. In particular, for 240s generation, it substantially improves overall and semantic scores while reducing quality drift from 7.84 to 1.33, demonstrating its effectiveness for stable long-horizon autoregressive video diffusion.


### [16] [SeamEdit: A Black-Box VLM-Agnostic Pipeline for Large-Image Semantic Editing](https://arxiv.org/abs/2606.13041)
*Xiangyu Lyu, Dan Lei*

#### 🧩 TL;DR
SeamEdit 是一个无需训练、模型无关的五阶段后处理流水线，通过将任意具有修补能力的视觉语言模型作为黑盒预言机，解决了大图像语义区域编辑中的语义形变、画布对齐偏移和可见接缝伪影问题。

---

#### 📘 Detailed Summary
**Motivation:** 大图像的语义区域编辑需要同时满足高质量生成与自然融入周围内容，但现有方法多依赖白盒模型，未充分利用闭源模型的强生成能力；而直接将闭源模型应用于分块编辑会导致语义形变、画布对齐偏移和可见接缝伪影等故障模式。

**Method:** SeamEdit 提出一个无需训练、模型无关的五阶段后处理流水线：基于覆盖的分块分解、黑盒视觉语言模型修补、几何与色彩一致性校正、基于接缝风险的多候选排序、以及动态规划曲线接缝融合，旨在减少接缝可见性并支持任意分块区域的语义修改。

**Result:** 该流水线有效降低了接缝可见性，支持对任意分块区域进行语义修改，同时保持了高生成质量与自然的内容融合。

**Conclusion:** SeamEdit 提供了一种无需训练且模型无关的方案，成功将闭源视觉语言模型的强生成能力引入大图像语义编辑任务，为黑盒模型在分块编辑中的应用提供了有效路径。

---

#### 📄 Abstract
Semantic region editing for large images must satisfy two requirements at the same time: high generative quality and natural integration with surrounding content. Some related methods rely on white-box models and leave the strong generation capability of closed-source models underexplored. Directly applying closed-source models to tiled editing, however, introduces several failure modes: semantic deformation, canvas-level alignment drift, and visible seam artifacts. This paper presents SeamEdit, a training-free and model-agnostic pipeline that treats any VLM with inpainting capability as a black-box oracle. SeamEdit mitigates these issues through a five-stage post-hoc pipeline: overlay-based tile decomposition, black-box VLM inpainting, geometric and color-consistency correction, seam-risk-based multi-candidate ranking, and dynamic-programming curved seam fusion. The pipeline reduces seam visibility and supports semantic modification of arbitrary tile regions.


### [17] [Unified MRI Brain Image Translation via Hierarchical Tumor Structure Comparison](https://arxiv.org/abs/2606.13096)
*Yupeng Cai, Jia Wei, Jianlong Zhou*

#### 🧩 TL;DR
本文提出HTSCGAN，一种融合肿瘤区域层次结构信息的统一多模态脑图像翻译生成对抗模型，通过三种尺度的Patch对比模块、预训练补丁分类器和结构感知编码器来保持肿瘤区域的结构保真度，在BraTS2020和BraTS2021上展现了优秀的翻译及下游分割性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有脑图像翻译方法在生成过程中忽略了不同肿瘤区域的结构信息，导致翻译后图像中肿瘤区域的保真度不足，难以满足临床对早期诊断、治疗方案规划和结果评估中对肿瘤结构完整性的严格要求。

**Method:** 提出HTSCGAN模型，生成器集成三种不同尺度的Patch对比模块（PCM）以捕获肿瘤区域的层次结构信息；同时引入预训练的Patch Classifier（PC）通过补丁分类损失、以及预训练的Structure-Aware Encoder（SAE）通过肿瘤感知损失，共同约束生成图像与真实图像在肿瘤区域结构上保持一致。

**Result:** 在BraTS2020和BraTS2021数据集上的实验表明，所提模型在翻译任务和下游分割任务中均表现出强劲性能，验证了其有效提升翻译后脑图像质量及临床相关性的能力。

**Conclusion:** 研究揭示了显式建模肿瘤区域层次结构信息对于提升多模态脑图像翻译质量及临床应用价值的关键作用，为后续将结构感知先验融入医学图像生成任务提供了可行范式。

---

#### 📄 Abstract
Multi-modal MRI brain image translation via available modalities holds significant practical importance in modern medicine, providing robust support for early diagnosis, treatment planning, and outcome assessment of diseases. For this purpose, it is important to ensure the fidelity of the tumor regions after translation. However, existing brain image translation methods ignore the structure information of different tumor regions, which could assist translation models in enhancing the quality and clinical applicability of the translated images.
  In this work, we propose a novel translation model called HTSCGAN, which is a unified multi-modal brain image translation generative adversarial model integrating the structural information within tumor regions with the aim of improving the quality of brain image translation. Specifically, the generator employs three Patch Contrast Module (PCM) with different patch sizes to capture the hierarchical structural information of the tumor regions. In addition, a pretrained Patch Classifier (PC) and a pretrained Structure-Aware Encoder (SAE) are employed to derive the generated image containing the same tumor region structure as the ground truth image via patch classification loss and tumor perceptual loss, respectively. The experiments on BraTS2020 and BraTS2021 demonstrate strong performance of our model in both translation tasks and down stream segmentation tasks, highlighting its effectiveness in enhancing the quality and clinical relevance of the translated brain images. Our code is available at https://anonymous.4open.science/r/HTSCGAN.


### [18] [PP-OCRv6: From 1.5M to 34.5M Parameters, Surpassing Billion-Scale VLMs on OCR Tasks](https://arxiv.org/abs/2606.13108)
*Yubo Zhang, Xueqing Wang, Manhui Lin, Yue Zhang, Penglongyi Deng, Ting Sun, Tingquan Gao, Zelun Zhang, Jiaxuan Liu, Changda Zhou, Hongen Liu, Suyin Liang, Cheng Cui, Yi Liu, Dianhai Yu, Yanjun Ma*

#### 🧩 TL;DR
PP-OCRv6 提出基于统一 MetaFormer 模块的轻量级 OCR 系统，通过结构重参数化与数据驱动优化，在中、小、微三档模型规模下，以较之前代服务器模型更少的参数量在识别精度和检测 Hmean 上分别提升 5.1% 和 4.6%，并超越多个超大 VLM 模型，同时在 CPU 上实现 3.9 倍加速。

---

#### 📘 Detailed Summary
**Motivation:** 现有 VLM 在通用视觉语言任务上表现出色，但在面向 OCR 的专用场景中面临幻觉、定位不精确和计算开销过大等突出问题，亟需一种轻量且高效的专业 OCR 系统以兼顾精度与部署效率。

**Method:** PP-OCRv6 围绕统一的 MetaFormer 风格构建块重新设计了骨干网络、检测颈部和识别颈部，采用结构重参数化将空间 token 混合与通道混合解耦，并通过任务特定的步长配置同时支持检测和识别任务，提供 medium、small、tiny 三种模型规模以覆盖从服务器到边缘的部署场景。

**Result:** 在内部基准上，PP-OCRv6_medium 达到 83.2% 的识别准确率和 86.2% 的检测 Hmean，相比 PP-OCRv5_server 分别提升 5.1% 和 4.6%，并以数个数量级更少的参数量超越 Qwen3-VL-235B、GPT-5.5 和 Gemini-3.1-Pro；tiny 模型在 Intel Xeon CPU 上的推理速度比 PP-OCRv5_mobile 快 3.9 倍，同时保持相近精度。

**Conclusion:** PP-OCRv6 表明，通过统一的 MetaFormer 架构设计、结构重参数化和任务感知的步长配置，可以在极轻量化的模型上实现显著优于超大视觉语言模型的 OCR 性能，为工业级 OCR 系统的轻量化和高效部署提供了有力方案。

---

#### 📄 Abstract
Vision-Language Models (VLMs) have achieved impressive results on general vision-language tasks, yet they suffer from hallucination, imprecise localization, and prohibitive computational cost when applied to dedicated OCR scenarios. This paper presents PP-OCRv6, a lightweight OCR system that combines architectural innovation with data-centric optimization. PP-OCRv6 redesigns the backbone, detection neck, and recognition neck around a unified MetaFormer-style building block with structural reparameterization, decoupling spatial token mixing from channel mixing and supporting both tasks through task-specific stride configurations. Three model tiers (medium, small, tiny) share the same block primitives, covering deployment scenarios from server to edge. On our in-house benchmarks, PP-OCRv6_medium achieves 83.2% recognition accuracy and 86.2% detection Hmean, outperforming PP-OCRv5_server by +5.1% and +4.6% respectively while surpassing Qwen3-VL-235B, GPT-5.5, and Gemini-3.1-Pro with orders of magnitude fewer parameters. The tiny tier achieves 3.9$\times$ faster inference than PP-OCRv5_mobile on Intel Xeon CPU while maintaining comparable accuracy.


### [19] [Cascade Classification of Dermoscopic Images of Skin Neoplasms with Controllable Sensitivity and External Clinical Validation](https://arxiv.org/abs/2606.13135)
*Elena S. Kozachok, Sergey S. Seregin, Aleksandr V. Kozachok, Ilya P. Latyshev, Oleg I. Samovarov*

#### 🧩 TL;DR
本文系统比较了四种深度学习架构（ViT-B/16、Swin-S、ConvNeXt-S、EfficientNetV2-S）在三种分类方案（二分类、单阶段四分类、两阶段级联）下对皮肤镜图像进行皮肤肿瘤分类的性能，并量化了从开放国际数据集迁移到俄罗斯临床独立数据集时存在的泛化差距，发现校准误差显著增大且恶性病变被低估，同时证明了两阶段级联方案在恢复恶性病变召回方面的优势。

---

#### 📘 Detailed Summary
**Motivation:** 现有深度学习模型在开放数据集上表现优异，但其在临床实践中对独立数据集的泛化能力，特别是在排名准确性、校准性能以及不同分类方案（如二分类、多分类、级联分类）之间的系统性比较尚不充分，且缺乏从国际数据集到特定临床场景（如俄罗斯临床实践）的迁移评估。

**Method:** 研究比较了四种架构（ViT-B/16、Swin-S、ConvNeXt-S、EfficientNetV2-S）在三种分类方案下的表现：二分类（恶性/良性）、单阶段四分类（良性、MEL、SCC、BCC）以及两阶段级联（先二分类分流，再进行MEL/SCC/BCC三分类）。所有模型均使用ImageNet预训练权重和统一的增强方案，在聚合的ISIC Archive开放数据上训练，并在内部留存样本及两个独立临床数据集（Melanoscope AI移动系统、Sechenov大学数据集）上评估。

**Result:** 内部测试中二分类ROC-AUC达0.952-0.966；在Sechenov大学数据集上降至0.797-0.893，灵敏度降至0.53-0.67，期望校准误差（ECE）从0.02升至0.27-0.39，且模型低估恶性病变概率。配对检验确认了临床数据上ViT-B/16在二分类阶段的性能显著落后（p<0.05），而在鉴别阶段无架构具备显著优势。级联方案相比单阶段四分类提升了多数架构的宏F1，但对ViT-B/16的提升具有统计显著性，主要通过恢复被主导良性类别掩盖的恶性病变实现。

**Conclusion:** 两阶段级联方案通过可调的分流阈值实现了标准单阶段argmax分类无法达到的灵敏度控制，并更好地复现了临床鉴别诊断逻辑。研究揭示了从开放数据集到临床实践存在持久的泛化差距，强调在部署前必须进行外部临床验证和重校准。

---

#### 📄 Abstract
Purpose. To compare deep learning architectures and classification schemes for dermoscopic images of skin neoplasms and assess their generalization on transfer from open international datasets to independent clinical datasets of Russian practice.
  Methods. Four architectures (ViT-B/16, Swin-S, ConvNeXt-S, EfficientNetV2-S) were compared in three schemes: binary (malignant/benign), single-stage four-class (benign, MEL, SCC, BCC), and a two-stage cascade (binary triage, then three-class differentiation MEL/SCC/BCC). All models used ImageNet-pretrained weights and a single augmentation protocol on aggregated open ISIC Archive data, and were evaluated on an internal held-out sample and two clinical datasets (Melanoscope AI mobile system; Sechenov University).
  Results. Internally the binary stage attains ROC-AUC 0.952-0.966; on Sechenov University it drops to 0.797-0.893, sensitivity to 0.53-0.67, and ECE rises from 0.02 to 0.27-0.39 with underestimation of malignancy, quantifying a generalization gap in ranking and calibration. Paired tests confirm one inter-architecture result on clinical data: the deficit of ViT-B/16 at the binary stage (p<0.05); at the differentiation stage no architecture has a proven advantage. The cascade raises macro F1 over single-stage four-class classification for most architectures, but significantly only for ViT-B/16, by recovering malignant lesions assigned to the dominant benign class. On ISIC MILK10k, direct 11-class classification yields mean-class sensitivity 0.525.
  Conclusion. A tunable triage threshold gives sensitivity control not attainable in standard single-stage (argmax) classification and better reproduces clinical differential-diagnosis logic. The persistent generalization gap mandates external clinical validation and recalibration before deployment.


### [20] [Iterative Visual Thinking: Teaching Vision-Language Models Spatial Self-Correction through Visual Feedback](https://arxiv.org/abs/2606.13156)
*Animesh Tripathy, Aswanth Krishnan*

#### 🧩 TL;DR
提出迭代视觉推理（IVT）框架，通过预测-观察-修正的闭环机制和两阶段训练（SFT数据蒸馏+GRPO强化学习），以极小成本（2400样本、单GPU）弥合了视觉语言模型空间定位中的自修正能力鸿沟，使Acc@0.5从79.6%提升至82.0%。

---

#### 📘 Detailed Summary
**Motivation:** 视觉语言模型（VLM）在单次空间定位中表现强劲，但缺乏任何观测和修正自身预测的机制；直接对预测结果的可视化渲染进行迭代提示会导致灾难性失败（Acc@0.5从79.6%暴跌至48.7%），暴露出定位能力与自修正能力之间根本性的鸿沟。

**Method:** 提出迭代视觉推理（IVT）闭环框架：模型预测边界框、观测渲染在图像上的预测结果、通过视觉反馈迭代优化。两阶段训练策略：第一阶段利用基座模型自身预测作为真实错误，提示教师VLM生成修正推理轨迹以构建无需人工标注的监督数据；第二阶段应用分组相对策略优化（GRPO），以IoU为奖励信号稳定多步修正过程。

**Result:** 在包含RefCOCOg、Ref-Adv和Ref-L4的混合基准（505个测试样本）上，SFT预热后的IVT在所有指标上超越单次基座模型：Acc@0.5提升至82.0%（+2.4pp），Acc@0.7提升至74.1%（+3.2pp），Acc@0.9提升至48.3%（+2.8pp）。GRPO进一步将每一步的IoU退化降低了5倍，稳定了修正轨迹。全部训练仅使用2400个样本和单块GPU。

**Conclusion:** 空间定位的自修正能力是一种可学习的技能，能够以极小的数据规模和计算资源被注入VLM中；两阶段训练方案有效解决了直接迭代导致的性能坍塌问题，GRPO在维持多步修正稳定性方面发挥了关键作用。

---

#### 📄 Abstract
Vision-language models (VLMs) achieve strong singleshot spatial grounding, yet lack any mechanism to observe and correct their own predictions. We find that naively prompting a VLM to iterate over rendered visualizations of its predictions causes catastrophic failure: Acc@0.5 on referring expression comprehension collapses from 79.6% to 48.7% (a 31 percentage point drop), revealing a fundamental gap between grounding capability and self-correction ability. We propose Iterative Visual Thinking (IVT), a closed-loop framework in which the model predicts a bounding box, observes the prediction rendered on the image, and iteratively refines through visual feedback. A two-phase training recipe closes the self-correction gap: first, we exploit the base model's own predictions as realistic errors and prompt a teacher VLM to generate corrective reasoning traces, yielding supervised data without human annotation; second, we apply Group Relative Policy Optimization (GRPO) with a simple IoU reward to stabilize multi-step refinement. On a mixed benchmark spanning RefCOCOg, Ref-Adv, and Ref-L4 (505 test samples), SFT warm-up with IVT surpasses the single-shot base model on every metric: Acc@0.5 rises to 82.0% (+2.4pp), Acc@0.7 to 74.1% (+3.2pp), and Acc@0.9 to 48.3% (+2.8pp). GRPO further reduces per-step IoU degradation by 5x, stabilizing the refinement trajectory. All training uses only 2,400 samples on a single GPU, demonstrating that spatial self-correction is a learnable capability that can be instilled at modest scale.


### [21] [Zero-Shot Captioning for Cultural Heritage: Automated Image Analysis of Traditional Indonesian Clothing](https://arxiv.org/abs/2606.13275)
*Anugrah Aidin Yotolembah, Novanto Yudistira, Gembong Edhi Setyawan*

#### 🧩 TL;DR
Custom ZeroCLIP 提出了一种基于检索增强的视觉-语言框架，结合冻结的 CLIP 图像编码器、双文本编码器和 LSTM 解码器，在零样本条件下为 8 个未见过的印尼省份生成传统服饰的说明文本，在 CLIPScore、BLEU-4 和 METEOR 上分别达到 0.8536、0.3342 和 0.4859，并实现 19.3% 的 METEOR 文化词汇恢复增益。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉-语言模型在低资源、文化特定场景（如印尼传统服饰）中缺乏领域适应性，且零样本条件下无法获取未见省份的标签与说明文本，导致模型难以生成文化上准确且流畅的说明文本。

**Method:** Custom ZeroCLIP 采用冻结的 CLIP ViT-B/32 图像编码器提取视觉特征，结合 CLIP 文本编码器与 BERT 文本编码器进行语义编码，并通过 LSTM 解码器生成说明文本。训练时使用 24 个可见省份的 3,800 张专家标注图像，验证使用 6 个可见省份，评估使用 8 个未见省份。推理时检索库仅包含训练省份的说明文本，不使用任何未见省份的图像、标签或说明文本。

**Result:** Custom ZeroCLIP 在未见省份上取得 CLIPScore 0.8536、BLEU-4 0.3342、METEOR 0.4859，优于所有基线模型。消融实验表明检索增强带来 19.3% 的 METEOR 增益，显著提升文化词汇恢复能力；人工评估进一步验证了模型在文化准确性和流畅性上的优势。

**Conclusion:** 该研究证明检索增强的域自适应方法能有效应对低资源文化遗产场景下的零样本说明生成任务，为文化特定视觉-语言任务提供了一种可行的范式，并公开了数据集以推动后续研究。

---

#### 📄 Abstract
This paper presents Custom ZeroCLIP, a retrieval-augmented vision-language framework for zero-shot captioning of Indonesian traditional garments. The dataset contains 3,800 expert-annotated images from all 38 Indonesian provinces. Using a province-level inductive zero-shot protocol, the model is trained on 24 seen provinces, validated on 6 seen provinces, and evaluated on 8 unseen provinces. The framework combines a frozen CLIP ViT-B/32 image encoder, a CLIP text encoder, a BERT text encoder, and an LSTM caption decoder. During inference, unseen-province labels and captions are unavailable, and retrieval uses only captions from training provinces. No unseen-province image, label, or caption is used during training, validation, or retrieval-bank construction. Custom ZeroCLIP achieves a CLIPScore of 0.8536, BLEU-4 of 0.3342, and METEOR of 0.4859, outperforming existing baselines. Ablation results show that retrieval improves cultural vocabulary recovery with a 19.3\% METEOR gain, while human evaluation confirms stronger cultural accuracy and fluency. The results demonstrate the effectiveness of retrieval-augmented domain adaptation for culturally grounded caption generation in low-resource heritage settings. The dataset is publicly available at https://github.com/AnugrahAidinYotolembah/Traditional-Indonesian-Clothing-Captioning-Dataset.


### [22] [Cross-Modal Masked Compositional Concept Modeling for Enhancing Visio-Linguistic Compositionality](https://arxiv.org/abs/2606.13288)
*Wei Li, Zhen Huang, Xinmei Tian*

#### 🧩 TL;DR
提出MACCO框架，通过跨模态掩码重建和辅助对齐正则化目标，显著提升视觉-语言模型在组合性理解、句法结构捕获以及下游生成任务上的表现。

---

#### 📘 Detailed Summary
**Motivation:** 对比训练的视觉-语言模型（如CLIP）在组合性理解上存在显著不足，表现为"词袋"行为——难以捕捉对象关系、属性-对象绑定以及词序依赖，原因既包括全局单向量表示的限制，也包括对图文数据中丰富组合信息的利用不充分。

**Method:** 提出MACCO框架，在一种模态中掩码组合概念，并利用另一模态的完整上下文信息进行重建，从而有效捕获和对齐跨模态组合结构；同时引入两个辅助目标，在模态间和模态内联合对齐与正则化掩码特征。

**Result:** 在五个组合性基准数据集上的广泛实验表明，该方法显著增强了视觉-语言模型的组合性理解能力，并提升了模型对句法结构和语言信息的捕获能力；改进的组合性还能有效迁移到文本到图像生成和多模态大语言模型任务中。

**Conclusion:** 通过跨模态掩码重建与多层级对齐正则化，MACCO有效解决了视觉-语言模型组合性理解不足的问题，并验证了组合性增强对下游多模态任务的正向迁移效果，为提升VLM的结构化理解能力提供了可行路径。

---

#### 📄 Abstract
Contrastively trained vision-language models like CLIP, have made remarkable progress in learning joint image-text representations, but still face challenges in compositional understanding. They often exhibit a "bag-of-words" behavior--struggling to capture the object relations, attribute-object bindings, and word order dependencies. This limitation arises not only from the reliance on global, single-vector representations for optimization, but also from the insufficient exploitation and modeling of the rich compositional information inherently present in paired image text data. In this work, we propose MACCO (MAsked Compositional Concept MOdeling), a framework that masks compositional concepts in one modality and reconstructs them conditioned on the full contextual information from the other, enabling the model to capture and align cross-modal compositional structures more effectively. To facilitate this process, we introduce two auxiliary objectives that jointly align and regularize masked features both inter-modally and intra-modally. Extensive experiments on five compositional benchmarks, along with in-depth analyses, demonstrate that our approach not only significantly enhances compositionality in VLMs but also improves their ability to capture syntactic structure and linguistic information. Additionally, the improved compositionality also benefits text-to-image generation and multimodal large language model. Code is available at https://github.com/hiker-lw/MACCO.


### [23] [Edit the Bits, Diff the Codes: Bitwise Residual Editing for Visual Autoregressive Models](https://arxiv.org/abs/2606.13558)
*Shengqiang Zhang, Ruotong Liao, Volker Tresp, Barbara Plank, Hinrich Schütze*

#### 🧩 TL;DR
提出BitResEdit，一种面向比特级残差VAR生成器的免训练图像编辑框架，通过结合比特级源-负引导(BitEdit)与多尺度残差代码注入(ResEdit)，在保持背景完整性的同时实现了更强的文本对齐效果。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于视觉自回归(VAR)生成器的文本引导图像编辑方法主要操作于token流、特征或扁平化next-token logits，未能充分利用比特级残差VAR模型的两个原生结构：逐比特Bernoulli预测头和加性多尺度残差代码场，导致编辑的定位精度与背景保持能力受限。

**Method:** BitResEdit包含两个核心模块：BitEdit通过源-负引导，在共享编辑前缀上计算源-目标对比度后倾斜后CFG的逐比特log-odds，并将每次更新投影到清洁CFG采样器周围的闭式Bernoulli-KL信任域内；ResEdit将采样比特转换为每尺度连续代码残差，用定位掩码进行门控，并通过生成器原生的尺度求和机制重新注入，实现掩码外潜在特征的精确保持与掩码内局部尺度感知编辑。

**Result:** 在PIE-Bench数据集上与Infinity-2B骨干网络结合，BitResEdit在同骨干的VAR编辑器中取得了最强的文本对齐效果，编辑区域CLIP分数相比此前最强编辑方法提升+1.07，同时背景保持性能与之相当。消融实验表明BitEdit和ResEdit在目标对齐和背景保持方面发挥互补作用。

**Conclusion:** BitResEdit通过解耦决策时比特引导与组合时代码组合，利用代码算术精确保持掩码外特征，同时实现局部化的尺度感知编辑，为VAR模型的图像编辑提供了利用其原生结构的新范式，揭示了比特级引导与残差代码组合在文本驱动图像编辑中的互补价值。

---

#### 📄 Abstract
Text-guided image editing with visual autoregressive (VAR) generators requires controlling both what the model samples and where the sampled change is written back into the image code. Existing VAR editors mainly operate on token streams, features, or flat next-token logits, leaving two native structures of bitwise-residual VAR models underused: the per-bit Bernoulli prediction head and the additive multi-scale residual code field from which the image is assembled. We propose BitResEdit, a training-free editor for bitwise-residual VAR generators such as Infinity. BitEdit performs source-negative guidance by tilting the post-CFG per-bit log-odds along a source--target contrast computed on a shared edited prefix, then projects each update into a closed-form Bernoulli-KL trust region around the clean CFG sampler. ResEdit converts the sampled bits into per-scale continuous-code residuals, gates them with a localization mask, and re-injects them through the generator's native sum-of-scales. Together they couple decision-time bit guidance with combination-time code composition, so masked-out latent features are preserved exactly by code arithmetic while localized, scale-aware edits are applied inside the target region. On PIE-Bench with Infinity-2B, BitResEdit attains the strongest text alignment among same-backbone VAR editors, improving CLIP on the edited region by +1.07 over the strongest prior editor while keeping background preservation competitive with it. Ablations show BitEdit and ResEdit play complementary roles in target alignment and background preservation.


### [24] [VISA: VLM-Guided Instance Semantic Auditing for 3D Occupancy World Models](https://arxiv.org/abs/2606.13460)
*Ruiqi Xian, Yuehan Xian, Jing Liang, Xuewei Qi, Dinesh Manocha*

#### 🧩 TL;DR
VISA提出了一种训练时语义审计方法，利用离线VLM对占据世界模型中的物体实例进行结构化审计，并通过可靠性加权蒸馏提升闭集语义占据预测的mIoU，表明VLM更适合作为语义审计器而非通用嵌入目标。

---

#### 📘 Detailed Summary
**Motivation:** 现有语义3D占据预测中，直接采用VLM策略（对齐3D体素或物体特征与裁剪-字幕嵌入）虽能提升文本空间相似度，但并未可靠改善闭集占据预测的mIoU，尤其物体类和稀有类错误会影响自由空间解释、碰撞检测和时间状态传播，亟需一种能有效利用VLM先验知识提升闭集语义占据精度的方法。

**Method:** VISA在训练时对每个物理物体实例的裁剪区域查询离线VLM，获取包含类别假设、可能混淆项、可靠性、属性和证据的结构化审计结果，并沿物体轨迹传播；将审计结果与匹配的3D物体体素对齐，通过可靠性加权分类损失、属性因子损失和场景级审计图损失蒸馏为语义logits，推理时不依赖VLM且保持原有流程不变。

**Result:** 在nuScenes数据集上，VISA将OccWorld的mIoU从19.06提升至20.05，将GaussianWorld的mIoU从21.36提升至21.91；在GaussianWorld上，物体类mIoU从18.18提升至19.16，稀有类mIoU从15.60提升至16.79。

**Conclusion:** VLM在闭集语义占据任务中更适合作为可靠性感知的语义审计器而非通用字幕-嵌入对齐目标，其结构化审计和可靠性加权蒸馏机制能有效提升占据世界模型的语义预测精度，尤其对物体类和稀有类改善显著。

---

#### 📄 Abstract
Semantic 3D occupancy provides a voxelized world state for autonomous driving and robot decision making, but object and rare-class errors can affect free-space interpretation, collision checking, and temporal state propagation. We show that a common VLM strategy, aligning 3D voxel or object features with crop-caption embeddings, improves text-space similarity without reliably improving closed-set occupancy mIoU. Motivated by this mismatch, we propose VISA, a training-time semantic auditing approach for existing occupancy world models. VISA queries an offline VLM on a representative crop of each physical object instance, obtains a structured audit with class hypotheses, plausible confusions, reliability, attributes, and evidence, and propagates it along the object track. The audit is grounded to matched 3D object voxels and distilled into semantic logits through reliability-weighted taxonomy, attribute-factor, and scene-level audit graph losses, while inference remains unchanged and requires no VLM. On nuScenes, averaged across three runs, VISA improves OccWorld from 19.06 to 20.05 mIoU and GaussianWorld from 21.36 to 21.91 mIoU; on GaussianWorld, object mIoU improves from 18.18 to 19.16 and rare-class mIoU from 15.60 to 16.79. These results suggest that VLMs are better suited to closed-set occupancy as reliability-aware semantic auditors than as generic caption-embedding targets.


### [25] [What's Old is New Again: Classical Dimensionality Reduction for Efficient Saliency-Guided Biometric Attack Detection](https://arxiv.org/abs/2606.13528)
*Samuel Webster, Walter Scheirer*

#### 🧩 TL;DR
本文提出了一种基于经典降维技术（PCA和LDA）的低成本、高可扩展性的显著性图生成方法，无需人工标注或领域知识即可直接从训练数据中获取显著性信息，并在多种生物特征呈现攻击检测（PAD）任务中显著提升模型性能，克服了显著性引导训练在实际部署中的关键障碍。

---

#### 📘 Detailed Summary
**Motivation:** 显著性引导训练在生物特征呈现攻击检测（PAD）中已被证明能有效提升模型鲁棒性和泛化能力，但其实际应用受限于现有显著性获取方法的高成本、领域特异性及有限可扩展性——例如人工标注不仅昂贵且仅适用于有限数据集，难以在大规模和多领域场景下推广。

**Method:** 本文提出利用经典降维技术PCA（主成分分析）和LDA（线性判别分析）直接从原始训练数据中生成显著性图，该方法完全无需人工标注或任何领域知识；通过PCA/LDA对数据分布进行建模，投影方向反映了最具判别性或方差最大的特征，进而构建出与任务相关的显著性热图，用于引导模型训练。

**Result:** 在三个已有显著性探索的领域（虹膜PAD、合成人脸检测、指纹PAD）中，所提方法的性能超越了基线方法，有时甚至优于现有最先进的显著性方法；同时在两个显著性全新的领域（指纹静脉PAD和身份证PAD）中验证了其强大可扩展性；所有实验均表明，基于降维显著性图的模型无需任何资源投入或领域特定工具即可取得显著提升。

**Conclusion:** 本研究表明，基于PCA和LDA的自动化显著性图生成方法能够以零额外成本有效替代传统高成本的显著性获取方式，解除了显著性引导训练在生物特征攻击检测及其他视觉识别任务中广泛部署的关键瓶颈，具有重要的实践意义和推广价值。

---

#### 📄 Abstract
Saliency-guided training is a paradigm in visual recognition that encourages models to focus on the most relevant image regions during learning. While its application in biometric presentation attack detection (PAD) has shown strong benefits in robustness and generalization, adoption is often limited by the high cost, domain specificity, and limited scalability of existing saliency acquisition methods, such as human annotations over a limited dataset. We present a novel, cost-efficient, and highly-scalable approach to saliency acquisition using maps inspired by classical dimensionality reduction techniques: PCA and LDA. Our proposed methods generate saliency maps directly from raw training data, requiring no human annotation nor domain knowledge. We contextualize the effectiveness of these saliency sources in three saliency-explored domains (iris PAD, synthetic face detection, fingerprint PAD) and demonstrate its scalability in two saliency-novel domains (fingerprint vein PAD and ID card PAD). Across all domains tested, models trained using dimensionality reduction-sourced saliency maps exceed baseline and sometimes SOTA saliency methods without any resource investment or domain-specific tooling. Our findings overcome an important yet unaddressed barrier to saliency-guided training for biometric attack detection and beyond.


### [26] [SpatialClaw: Rethinking Action Interface for Agentic Spatial Reasoning](https://arxiv.org/abs/2606.13673)
*Seokju Cho, Ryo Hachiuma, Abhishek Badki, Hang Su, Byung-Kwan Lee, Chan Hee Song, Sifei Liu, Subhashree Radhakrishnan, Seungryong Kim, Yu-Chiang Frank Wang, Min-Hung Chen*

#### 🧩 TL;DR
SpatialClaw 提出一种以代码为行动接口、维持有状态 Python 内核的训练无关框架，使 VLM 代理能逐步组合感知原语进行开放式的 3D/4D 空间推理，在 20 个基准上平均准确率 59.9%，超越此前最优空间代理 11.2 个百分点。

---

#### 📘 Detailed Summary
**Motivation:** 现有空间推理代理要么采用单次代码执行策略（提交完整分析后才看到中间结果），要么依赖结构化工具调用接口（灵活性不足，难以自由组合操作或针对任务定制分析），两种设计在应对开放性的复杂 3D/4D 空间推理时均受限于行动接口的灵活性。因此需要一种能逐步观测中间结果并灵活组合感知模块的行动接口设计。

**Method:** SpatialClaw 是一个训练无关框架，核心思路是以代码作为行动接口：它维护一个状态化的 Python 内核，预加载输入帧以及一系列感知和几何原语，让 VLM 驱动的智能体每一步编写一个可执行的代码单元，该单元的执行依赖于之前所有输出，从而使得智能体能够基于中间文本和视觉观测灵活组合及操作感知结果，并针对不同问题自适应调整分析策略。

**Result:** 在涵盖静态和动态 3D/4D 空间推理的 20 个基准任务上，SpatialClaw 以 59.9% 的平均准确率超越了此前最优的空间代理 11.2 个百分点，并且在来自两个模型家族的六种不同 VLM 骨干网络上均取得一致增益，无需任何基准或模型特定的适配。

**Conclusion:** 代码作为行动接口比单次执行或结构化工具调用具有更强的灵活性和表达能力，能够有效支持开放式的复杂空间推理；该方法不依赖训练且跨模型泛化良好，为工具增强型 VLM 代理的行动接口设计提供了新的方向。

---

#### 📄 Abstract
Spatial reasoning, the ability to determine where objects are, how they relate, and how they move in 3D, remains a fundamental challenge for vision-language models (VLMs). Tool-augmented agents attempt to address this by augmenting VLMs with specialist perception modules, yet their effectiveness is bounded by the action interface through which those tools are invoked. In this work, we study how the design of this interface shapes the agent's capacity for open-ended spatial reasoning. Existing spatial agents either employ single-pass code execution, which commits to a full analysis strategy before any intermediate result is observed, or rely on a structured tool-call interface that often offers less flexibility for freely composing operations or tailoring the analysis to each task. Both designs offer limited flexibility for open-ended, complex 3D/4D spatial reasoning. We therefore propose SpatialClaw, a training-free framework for spatial reasoning that adopts code as the action interface. SpatialClaw maintains a stateful Python kernel pre-loaded with input frames and a suite of perception and geometry primitives, letting a VLM-backed agent write one executable cell per step conditioned on all prior outputs, enabling the agent to flexibly compose and manipulate perception results and adapt its analysis to both intermediate text and visual observations and the demands of each problem. Evaluated across 20 spatial reasoning benchmarks spanning a broad range of static and dynamic 3D/4D spatial reasoning tasks, SpatialClaw achieves 59.9% average accuracy, outperforming the recent spatial agent by +11.2 points, with consistent gains across six VLM backbones from two model families without any benchmark- or model-specific adaptation.


### [27] [InterleaveThinker: Reinforcing Agentic Interleaved Generation](https://arxiv.org/abs/2606.13679)
*Dian Zheng, Harry Lee, Manyuan Zhang, Kaituo Feng, Zoey Guo, Ray Zhang, Hongsheng Li*

#### 🧩 TL;DR
InterleaveThinker 是一个多智能体管道，通过规划器和评论家智能体使现有图像生成器具备交错生成（文本-图像序列）能力，采用监督微调和基于 GRPO 的单步强化学习训练，在交错生成基准上取得与 GPT-5 相当的性能，同时还能提升基础模型在推理基准上的表现。

---

#### 📘 Detailed Summary
**Motivation:** 现有图像生成器虽具有逼真的照片级渲染效果和指令跟随能力，但受限于架构无法实现交错生成（文本-图像序列），这在视觉叙事、引导和具身操控等应用中至关重要，而现有统一多模态模型在此方面的表现也有限。

**Method:** 提出 InterleaveThinker，首个用于赋予已有图像生成器交错生成能力的多智能体管道，包含规划器智能体（组织图像-文本输入序列并指示生成器每步执行任务）和评论家智能体（评估输出、识别偏离指令的样本并优化指令以重新生成）。构建 Interleave-Planner-SFT-80k 和 Interleave-Critic-SFT-112k 进行格式冷启动，再构建 Interleave-Critic-RL-13k 并采用 GRPO 方法，通过提出准确率奖励和单步奖励，实现单步强化学习有效引导整个生成轨迹。

**Result:** InterleaveThinker 在多种图像生成器上均提升了交错生成性能，在交错生成基准上达到与 Nano Banana 和 GPT-5 相当的性能；此外，在推理基准上显著增强了基础模型，例如在 4-step FLUX.2-klein 上，WISE 和 RISE 基准均有大幅提升。

**Conclusion:** InterleaveThinker 证明了通过多智能体管道和单步强化学习策略，可以低成本地赋予现有图像生成器交错生成能力，并且这种能力还能泛化到推理任务中，为统一多模态生成提供了新的思路。

---

#### 📄 Abstract
Recent image generators have demonstrated impressive photorealism and instruction-following capabilities in single-image generation and editing. However, constrained by their architectures, they cannot achieve interleaved generation (text-image sequence), which has crucial applications in visual narratives, guidance, and embodied manipulation. Even the latest open-source Unified Multimodal Models (UMMs) exhibit limited performance in this regard. In this paper, we introduce InterleaveThinker, the first multi-agent pipeline designed to endow any existing image generator with interleaved generation capabilities. Specifically, we employ a planner agent to organize the image-text input sequence, instructing the image generator on the required execution at each step. Subsequently, we introduce a critic agent to evaluate the generator's outputs, identify samples that deviate from the planned instructions, and refine the instructions for regeneration. To implement this pipeline, we construct the Interleave-Planner-SFT-80k and Interleave-Critic-SFT-112k to perform a format cold-start. Then we develop Interleave-Critic-RL-13k to reinforce the step-wise instruction correction capability within a generation trajectory using GRPO. Since a single interleaved generation trajectory may involve over 25 generator calls, optimizing the entire trajectory is computationally impractical. Therefore, we propose accuracy reward and step-wise reward, allowing single-step RL to effectively guide the entire generation trajectory. The results show that InterleaveThinker improves performance across various image generators. On interleaved generation benchmarks, it achieves performance comparable to Nano Banana and GPT-5. Surprisingly, it also significantly enhances the base model on reasoning-based benchmarks; for example, on 4-step FLUX.2-klein, we observe substantial gains on WISE and RISE.


<div id='cs.CL'></div>

# cs.CL [[Back]](#toc)

### [28] [Does AI Reviewer See the Full Picture? Attacking and Defending Multimodal Peer Review](https://arxiv.org/abs/2606.12716)
*Xinyu Zhao, Rana Muhammad Shahroz Khan, Zhen Xu, Zhen Tan, Tianlong Chen*

#### 🧩 TL;DR
PaperGuard 是首个系统性评估和防御针对 AI 同行评审的跨模态对抗攻击的基准框架，揭示了多模态大模型在科学评审中普遍存在的脆弱性，并提出了基于分块嵌入搜索的实用防御方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究对 AI 同行评审的鲁棒性评估几乎完全局限于纯文本场景，忽视了科学论文中图表等多模态信息带来的对抗攻击风险；同时，针对同行评审的领域特定目标攻击（如操控评分）与通用越狱攻击本质不同，且缺乏有效的防御机制。

**Method:** PaperGuard 框架包含三个核心部分：(1) 构建涵盖多个科学领域的多模态同行评审数据集；(2) 设计统一的攻击套件，包括面向文本的黑盒提示注入（GCG）和面向图像的白盒扰动（PGD）；(3) 提出一种基于分块嵌入搜索的实用防御方法，利用学术论文的长上下文特性，通过分块检索高效定位并消除有害指令。

**Result:** 在多个最先进模型上的大量实验表明，AI 评审者普遍存在脆弱性，面对跨模态攻击时缺乏鲁棒性，验证了 PaperGuard 作为评估基准和防御框架的有效性。

**Conclusion:** PaperGuard 为可信、抗攻击的 AI 辅助学术评审奠定了基准、协议和可实用的防御方法，强调了多模态环境下 AI 评审安全性的重要性和紧迫性。

---

#### 📄 Abstract
The integration of Large Language Models (LLMs) and Multimodal LLMs (MLLMs) into scientific peer-review workflows introduces novel and significant risks for adversarial manipulation, especially given the multimodal nature of scientific papers where figures, not just text, convey core evidence. This creates a significant gap: current robustness studies on AI peer-review are overwhelmingly text-only. Moreover, the problem is distinct from standard jailbreaking, as a peer-review attack seeks to induce a domain-specific, targeted failure (e.g., "inflate this score") rather than a general safety policy violation, for which no practical defenses exist. To address this, we introduce PaperGuard, the first comprehensive benchmark designed to systematically evaluate and defend AI-generated peer-review against these domain-specific, cross-modal attacks. Our framework is built on three pillars: (1) a new multimodal peer-review dataset spanning multiple scientific domains; (2) a unified suite of attacks, including black-box prompt injections and white-box perturbations, specifically designed to target both text (GCG) and figures (PGD); and (3) a practical defense, motivated by the long-context challenge of academic papers, that uses chunk-based embedding search to efficiently localize and mitigate harmful instructions. Our extensive experiments, conducted across state-of-the-art models, confirm that AI reviewers are pervasively vulnerable. PaperGuard establishes the foundational benchmark, protocols, and actionable defense necessary to pioneer trustworthy, attack-resilient AI-assisted scholarly reviewing.


### [29] [Agent-based models for the evolution of morphological alternation patterns](https://arxiv.org/abs/2606.12748)
*Aravinth Kulanthaivelu, Richard Sproat*

#### 🧩 TL;DR
提出了一种多智能体仿真框架，模拟语言中词干和屈折交替现象（如go/went）的涌现，并引入基于大语言模型的AI历史语言学家来评估仿真生成形态的合理性，发现无标度社交网络和伯努利采用策略有助于生成更可信的形态交替模式。

---

#### 📘 Detailed Summary
**Motivation:** 语言中广泛存在看似无功能性的形态交替现象（如英语go的过去式went），这类现象既不利于沟通也不利于学习，却能在语言中持续数百年甚至数千年，其涌现和持久的机制尚不清晰，现有计算仿真在词汇规模、音系规则真实性和评估方法上存在不足。

**Method:** 构建了多智能体仿真系统，支持自然主义词汇形式、真实音系规则、数百至数千词条的词典以及数十至数百规模的智能体种群，并允许多种网络拓扑、扩散模式和采用策略；同时提出AI历史语言学家——一种基于大语言模型的系统，通过模拟两位历史语言学家的辩论来评估仿真生成形态与真实语言形态的相似性。

**Result:** 实验表明，在影响形态合理性的因素中，无标度社交网络和随机伯努利采用策略倾向于生成更可信的形态交替模式；系统还成功建模了三个真实历史语言变化案例，并测试了历史可能的不同演化路径。

**Conclusion:** 多智能体仿真可以有效再现语言中形态交替现象的涌现过程，大语言模型可以用于评估仿真形态的真实性，网络结构和采用策略是影响形态演变结果的关键因素；所有代码和数据已开源，为历史语言学和计算语言学提供了新的研究工具。

---

#### 📄 Abstract
Why is the past of English "go" the apparently unrelated "went"? Such alternations are frequent in languages. They neither aid communication nor learnability, yet they can be persistent, surviving over centuries or millennia.
  We present a multi-agent simulation of the emergence of morphological stem and inflection alternations. Alternate forms arise by phonological changes or, as with "go/went", from lexical alternatives associated with a subset of the population. When an agent 'hears' another agent use a novel form for a slot in the paradigm of a word (say, the past tense of go), they will with some probability adopt that form, possibly spreading its use to other slots in the paradigm that shared the same original form. Thus alternative forms can spread through the population and become entrenched as stem or inflectional marker alternants. Unlike many previous computational studies, our system allows for naturalistic lexical forms, realistic phonological rules, lexicons with hundreds or thousands of entries, and agent populations in the tens or hundreds. It supports several network topologies, diffusion patterns and agent adoption policies.
  One issue with such simulations is evaluation: how realistic is the resulting morphology compared to those of real languages? We introduce the AI Historical Linguist, a novel Large Language Model-driven system that models a debate between two historical linguists. We use this to compare a set of real language morphologies, disguised morphologies, and experimentally evolved morphologies. The results suggest that among the factors that favor more plausible morphologies are scale-free social networks and random Bernoulli adoption of forms.
  We also present three case studies modeling attested historical changes, allowing us to test what might have happened if history had been different.
  All code and data are released.


### [30] [Small LLMs for Biomedical Claim Verification: Cost-Effective Fine-Tuning, Structural Dataset Shortcuts, and Cross-Domain Generalization](https://arxiv.org/abs/2606.12854)
*Gaurav Kumar*

#### 🧩 TL;DR
该研究首次将QLoRA微调的小型LLM（Mistral-7B、Phi-3-mini、Qwen2.5-3B）应用于生物医学声明验证，发现Mistral-7B QLoRA在仅使用1,008个训练样本的情况下，以极低计算成本超越GPT-4o和GPT-5最多12%的F1分数，并揭示了SciFact数据集中的结构性伪影问题。

---

#### 📘 Detailed Summary
**Motivation:** 大型语言模型（如GPT-4o、GPT-5）在生物医学声明验证中虽具备强零样本能力，但其高昂成本和模型不透明性限制了可扩展的实际应用，亟需探索低成本、高效且可解释的替代方案。

**Method:** 采用QLoRA方法对三个小型LLM——Phi-3-mini（3.8B）、Qwen2.5-3B和Mistral-7B——进行参数高效微调，训练数据来自SciFact和HealthVer数据集（共1,008个样本），并与GPT-4o、GPT-5及微调的BioLinkBERT编码器进行全面对比。

**Result:** Mistral-7B QLoRA在生物医学声明验证任务上以极低的推理成本超越GPT-4o和GPT-5，最高F1分数提升达12%。通过同域和跨域双向评估发现，SciFact数据集中存在此前未被报告的结构性伪影，该伪影会虚高同域评估得分，而基于结构无偏数据训练的模型具备更强的跨域泛化能力。

**Conclusion:** 小型LLM通过QLoRA微调可以在生物医学声明验证中超越大规模闭源模型，实现成本与性能的显著平衡；研究中发现的数据集结构性伪影问题对现有评测基准的可信度提出警示，未来应关注数据质量和跨域泛化能力。

---

#### 📄 Abstract
Large Language Models such as GPT-4o and GPT-5 achieve strong zero-shot performance on biomedical claim verification, but cost and opacity limit scalable use. We fine-tune three small LLMs: Phi-3-mini (3.8B), Qwen2.5-3B, and Mistral-7B, via QLoRA on SciFact and HealthVer, providing the first study of QLoRA models against GPT-4o and fine-tuned BioLinkBERT encoders. Mistral-7B QLoRA surpasses both GPT-4o and GPT-5 (up to 12% F1 gain) at a fractional cost using just 1,008 training examples. We conduct extensive in-domain and cross-domain evaluation: models trained on SciFact tested on HealthVer and vice versa, at matched sizes to isolate dataset structure from data quantity. We identify a previously unreported structural artifact in SciFact that inflates in-domain scores, and show through bidirectional out-of-domain evaluation that training on structurally sound data enables robust cross-domain transfer. We plan to release all code and adapter checkpoints.


### [31] [PRISM: Prosody-Integrated Multi-Agent Reasoning Framework for Empathetic Spoken Dialogue](https://arxiv.org/abs/2606.12902)
*Wen Zhang, Xiaocui Yang, Zhuoyue Gao, Shi Feng, Daling Wang, Yifei Zhang*

#### 🧩 TL;DR
PRISM是一个多智能体框架，通过解耦语音感知、响应生成和语音合成，并引入韵律到语言翻译机制，解决了共情口语对话系统中级联流水线丢失声学线索和端到端模型缺乏可解释控制的问题，在共情性、韵律适当性和文本响应质量上取得了一致提升。

---

#### 📘 Detailed Summary
**Motivation:** 现有共情口语对话系统面临双重挑战：级联流水线在语音转文本过程中会丢弃声学线索（如韵律信息），导致共情表达缺失；而端到端语音模型虽然保留了声学信息，但缺乏对情感和知识融合的可解释性控制，难以在推理过程中灵活调用外部知识。

**Method:** PRISM提出多智能体框架将共情口语对话解耦为三个协调组件：语音感知、响应生成和语音合成。核心创新包括一个韵律到语言翻译机制，用于将声学韵律信息转化为大语言模型可理解的表示以稳定推理；同时支持按需调用外部知识工具，增强共情对话生成中的知识感知能力。

**Result:** 实验结果表明，PRISM在客观和主观评价指标上均取得一致改进，具体体现在共情性（empathy）、韵律适当性（prosodic appropriateness）和文本响应生成质量三个维度上均优于基线方法。

**Conclusion:** PRISM通过解耦式多智能体框架有效桥接了级联流水线与端到端模型之间的鸿沟，证明了显式韵律信息注入和大语言模型推理结合能够显著提升口语对话系统的共情表达能力，为共情口语对话系统的设计提供了新的范式。

---

#### 📄 Abstract
Empathetic spoken dialogue systems require not only semantically appropriate responses but also emotionally aligned prosodic expression. However, cascade pipelines often discard acoustic cues during speech-to-text conversion, while end-to-end speech models lack interpretable control over emotion and knowledge integration. To address these challenges, we propose PRISM, a multi-agent framework for empathetic spoken dialogue that decouples speech perception, response generation, and speech synthesis into coordinated components. PRISM introduces a prosody-to-language translation mechanism to stabilize large language model reasoning and enables on-demand invocation of external knowledge tools for empathetic dialogue generation. Experimental results demonstrate that PRISM achieves consistent improvements in empathy, prosodic appropriateness, and text response generation quality across objective and subjective metrics. Our code is available at: https://github.com/Bxzfrm/PRISM.


### [32] [SENTINEL: Failure-Driven Reinforcement Learning for Training Tool-Using Language Model Agents](https://arxiv.org/abs/2606.12908)
*Ziyi Wang, Yuxuan Lu, Yimeng Zhang, Qun Liu, Chen Luo, Jiri Gesi, Hanqing Lu, Yisi Sang, Manling Li, Jing Huang, Dakuo Wang*

#### 🧩 TL;DR
本文提出SENTINEL，一种失败驱动的强化学习框架，通过将智能体的回合失败转化为针对性训练任务，有效缓解了工具使用语言模型智能体在训练中因任务分布与策略能力不匹配导致的低效问题，在Tau2-Bench Retail上将Pass@1从66.4提升至74.9。

---

#### 📘 Detailed Summary
**Motivation:** 训练可靠的工具使用语言模型智能体仍具挑战性。强化学习的有效性高度依赖训练任务分布，当任务在训练前固定时，任务分布会与策略不断演进的能力产生失配，导致大量交互回合浪费在低信息量的任务上，影响训练效率。

**Method:** SENTINEL采用Controller–Proposer–Solver循环架构：Controller分析失败轨迹并总结重复出现的错误模式，Proposer基于这些模式生成可执行的压力测试任务，Solver在针对性生成的任务上进行强化学习训练，从而将回合失败转化为有目标的训练信号。

**Result:** 在Tau2-Bench Retail基准上，基于Qwen3-4B-Thinking-2507模型，SENTINEL将Pass@1从66.4提升至74.9，且在多个Pass@k指标上全面优于在通用合成任务上训练的强化学习方法。

**Conclusion:** 模型自身的失败可以为改进工具使用语言模型智能体提供有效且可扩展的针对性训练信号，SENTINEL框架成功验证了利用失败驱动范式来提升智能体在复杂工具调用任务中的表现。

---

#### 📄 Abstract
Language model agents are increasingly effective in solving realistic tasks through multi-turn tool use. However, training reliable tool-using agents remains challenging in practice. While reinforcement learning provides an on-policy paradigm for improving agents from their own environment interactions, its effectiveness depends heavily on the training task distribution. When tasks are fixed before training, the task distribution can become increasingly mismatched with the policy's evolving capabilities, causing many rollouts to be spent on uninformative tasks. We propose SENTINEL, a failure-driven reinforcement learning framework that turns the Solver's rollout failures into targeted training tasks. SENTINEL follows a Controller--Proposer--Solver loop: the Controller analyzes failed trajectories and summarizes recurring error patterns, the Proposer generates executable tasks that stress these weaknesses, and the Solver is trained on the targeted tasks. On Tau2-Bench Retail with Qwen3-4B-Thinking-2507, SENTINEL improves Pass\^{}1 from 66.4 to 74.9 and outperforms RL on general synthetic tasks across Pass\^{}k metrics. These results demonstrate that model failures provide an effective and scalable source of targeted training signal for improving tool-using language model agents.


### [33] [No Hidden Prompts Needed! You Can Game AI Peer Review with Presentation-Only Revisions](https://arxiv.org/abs/2606.13044)
*Xu Yang, Zhizhou Sha, Junbo Li, Jian Yu, Yifan Sun, Matthew Zhao, Jinrui Fang, Xinyue Guo, Yining Wu, Xu Hu, Yifu Luo, Qiang Liu, Zhangyang Wang*

#### 🧩 TL;DR
本文揭示了AI审稿系统在仅修改论文呈现层面内容（如摘要、贡献表述、讨论框架）而保持所有科学证据不变的情况下，仍可被一种名为"对抗性重包装"的闭环攻击策略显著操纵，攻击成功率达75.1%，表明论文呈现本身已成为可优化的对抗面，部署风险不仅来自显式恶意指令。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究对AI审稿系统的鲁棒性关注主要集中在显式攻击（如隐藏指令和提示注入），但更隐蔽且更具政策相关性的失效模式——即不修改任何科学证据、仅调整呈现层面内容即可误导审稿系统——尚未被系统研究。

**Method:** 提出对抗性重包装（adversarial repackaging）：一种闭环攻击框架，利用AI审稿系统的反馈信号搜索仅改变呈现层面（如摘要、贡献表述、相关工作定位、叙事结构）的修订方案，同时保持方法、实验、图表、方程、证明和数值结果等科学证据完全不变。

**Result:** 在三个主流AI审稿系统上，对抗性重包装实现了75.1%的攻击成功率和平均+1.21/10的分数提升；效果远超普通散文润色。分析揭示两种结构性失效模式：AI审稿者更易被"打动"而非"说服"（强调优势可靠地提升感知价值，而试图消除弱点常适得其反），以及AI审稿者会将"看起来解决了局限性"与"实际解决了局限性"混淆。

**Conclusion:** AI审稿系统的部署风险不仅来自恶意隐藏指令，还来自论文呈现本身作为优化表面的涌现性漏洞；研究发布了无污染滚动基准测试和攻击框架，用于检验AI审稿者在仅呈现编辑下是否仍锚定于科学内容。

---

#### 📄 Abstract
As AI-generated reviews move from experimental tools into peer-review infrastructure, most robustness concerns have focused on explicit attacks such as hidden instructions and prompt injection. We study a harder and more policy-relevant failure mode: no hidden text, no prompt injection, and no changes to methods, experiments, figures, equations, proofs, or numerical results. The attacker modifies only presentation-level content, such as the abstract, contribution framing, related work, discussion, and narrative structure. We introduce adversarial repackaging: a closed-loop attack that uses AI-reviewer feedback to search for presentation-level revisions while keeping the scientific evidence fixed. Across three mainstream AI reviewers, adversarial repackaging achieves a 75.1% attack success rate and a mean score gain of +1.21/10. The effect is not explained by ordinary prose polishing. We also reveal that strategies that change how the reviewer interprets the paper, such as related-work repositioning and analytical discussion expansion, substantially outperform surface edits such as local polishing, table formatting, and algorithm boxes.
  Our analysis reveals two deeper structural failure modes. First, AI reviewers are easier to impress than to convince: highlighting strengths reliably increases perceived merit, while attempts to dissolve weaknesses frequently backfire. Second, AI reviewers can confuse the appearance of addressing a limitation with actually resolving it, allowing unchanged evidence to be reinterpreted as stronger scientific contribution. These results show that the deployment risk is not only malicious hidden instructions, but the emergence of paper presentation itself as an optimization surface. We release a contamination-free rolling benchmark and attack framework for testing whether AI reviewers remain anchored to scientific content under presentation-only edits.


### [34] [G-Long: Graph-Enhanced Memory Management for Efficient Long-Term Dialogue Agents](https://arxiv.org/abs/2606.13115)
*Minjun Choi, Yoonjin Jang, Sangwon Youn, Youngjoong Ko*

#### 🧩 TL;DR
G-Long 提出一种基于图增强的小语言模型框架，通过微调小模型进行结构化三元组提取与关联检索，并结合注意力感知重要性评分机制，在显著降低计算成本的同时，在响应生成和记忆检索任务上达到了最先进性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于大语言模型（LLM）的开放域对话系统在维护长期一致性方面存在两大难题：一是长上下文推理存在固有局限，二是处理大量原始文本效率低下且计算成本高昂。已有方法中，非结构化记忆存储容易导致信息丢失，而依赖LLM的方案则面临高延迟和计算开销大的问题。

**Method:** G-Long 提出一种图增强框架，核心包含两个组件：一是利用微调后的小语言模型（sLM）进行结构化三元组提取与关联检索，大幅降低运行成本；二是提出注意力感知重要性评分机制，通过利用T5摘要模型内在的交叉注意力信号来识别关键记忆。该框架通过图结构组织记忆，并实现高效的检索与生成。

**Result:** 在多个公开基准上的广泛实验表明，G-Long 在响应生成和记忆检索任务上均达到最先进水平：在MSC数据集上响应质量提升达9.8%，在LME数据集上检索召回率提升达40.8%，同时显著降低了计算开销。

**Conclusion:** G-Long 通过结合小语言模型与图结构记忆以及注意力感知评分机制，有效解决了开放域对话系统中长期一致性与计算效率之间的权衡问题，为构建低成本、高性能的长期对话记忆系统提供了可行的新路径。

---

#### 📄 Abstract
While Large Language Models (LLMs) have advanced open-domain dialogue systems, maintaining long-term consistency remains a challenge due to inherent limitations in long-context reasoning and the inefficiency of processing extensive raw text. Existing approaches typically rely on either unstructured memory storage, which is prone to information loss, or computationally expensive LLMs that incur high latency. To address these limitations, we propose G-Long, a graph-enhanced framework that utilizes a fine-tuned small Language Model (sLM) for structured triplet extraction and associative retrieval, significantly reducing operational costs. Furthermore, we introduce the novel attention-aware importance scoring mechanism that leverages the intrinsic cross-attention signals of a T5 summarizer to identify salient memories. Extensive experiments across diverse benchmarks demonstrate that G-Long achieves state-of-the-art performance in both response generation and memory retrieval, yielding performance gains of up to 9.8% in response quality on MSC and 40.8% in retrieval recall on LME, while significantly minimizing computational overhead.


### [35] [SkillCAT: Contrastive Assessment and Topology-Aware Skill Self-Evolution for LLM Agents](https://arxiv.org/abs/2606.13317)
*Kunfeng Chen, Qihuang Zhong, Juhua Liu, Bo Du*

#### 🧩 TL;DR
SkillCAT 是一个无需训练的技能自进化框架，通过对比因果提取、评估增强进化和拓扑感知任务执行三个模块，显著提升了 LLM 智能体在多个基准上的任务执行性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有 LLM 智能体的技能自进化方法存在三方面不足：每个任务仅从单条轨迹学习、候选技能补丁在未经验证前直接合并、推理时需加载完整技能语料库，导致技能进化效率低且可靠性不足。

**Method:** SkillCAT 提出三阶段无训练框架：对比因果提取（CCE）为每个任务采样多条轨迹并对比同任务成功/失败配对以识别因果证据；评估增强进化（AAE）在源任务克隆上回放每个候选补丁，仅保留能改善或维持结果的补丁后进行层次化合并；拓扑感知任务执行（TTE）将进化后的技能编译为可路由的子技能拓扑，推理时仅加载与任务相关的能力节点。

**Result:** 在 SpreadsheetBench、WikiTableQuestions 和 DocVQA 等智能体基准上，SkillCAT 相较于基线方法平均性能提升最高达 40.40%，并在跨模型和分布外泛化测试中均展现出可靠的技能进化能力。

**Conclusion:** SkillCAT 证明了一种无需模型训练的轻量级技能自进化范式可行，其核心在于通过多轨迹对比、评估验证和拓扑路由来提升技能补丁的质量与推理效率，为 LLM 智能体的持续能力提升提供了新方向。

---

#### 📄 Abstract
Skill self-evolution methods for LLM agents aim to turn execution trajectories into reusable skill documents, but current pipelines typically learn from one trajectory per task, merge candidate skill patches before checking them, and load the full skill corpus before inference. We propose SkillCAT, a training-free framework that separates this process into three stages. Contrastive Causal Extraction (CCE) samples multiple trajectories for each task and compares same-task success/failure pairs to identify evidence that explains outcome differences. Assessment-Augmented Evolution (AAE) replays each candidate patch on source-task clones and keeps only patches that improve or preserve task outcomes before hierarchical skill patch merging. Topology-Aware Task Execution (TTE) compiles the evolved skills into a routable sub-skill topology, so inference loads only the capability nodes relevant to the task. We evaluate SkillCAT on common agent benchmarks, including SpreadsheetBench, WikiTableQuestions, and DocVQA, and further test cross-model and out-of-distribution generalization. Across these settings, SkillCAT raises the average score over baselines by up to 40.40%, demonstrating reliable skill evolution without model training.


### [36] [From Passive Generation to Investigation: A Proactive Scientific Peer Review Agent](https://arxiv.org/abs/2606.13349)
*Haishuo Fang, Yue Feng, Iryna Gurevych*

#### 🧩 TL;DR
提出ProReviewer，一种基于MDP框架的LLM科学同行评审智能体，通过维护结构化评审日志实现主动调查式审查，在五个质量维度上平均得分最高，以8B模型超越包括更大前沿LLM在内的多种基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于LLM的自动化科学同行评审方法难以生成有具体证据支持的深度评审，其关键局限性在于缺乏像人类评审者那样根据积累的证据主动调查论文可疑部分的能力。

**Method:** 将LLM评审智能体的主动调查过程形式化为马尔可夫决策过程（MDP），提出ProReviewer框架，该智能体通过维护一个结构化评审日志作为工作空间来追踪评审过程中收集的证据和中间发现，采用8B基座模型，经过监督微调（SFT）和强化学习（RL）优化训练。

**Result:** ProReviewer在五个质量维度上平均得分最高，相比基于提示的更大前沿LLM方法提升高达39%，相比最强的微调基线方法相对提升16%，并在人工评估中取得了最高的胜率。

**Conclusion:** 将主动调查式评审形式化为MDP并结合结构化评审日志是提升LLM评审智能体性能的有效途径，较小的模型经过适当的训练策略也能超越更大的模型，表明方法设计和训练范式比模型规模更为关键。

---

#### 📄 Abstract
Large language models (LLMs) have shown promise in automating scientific peer review. However, existing approaches often struggle to generate in-depth reviews supported by concrete evidence. We argue that a key limitation is the lack of flexibility to proactively investigate suspicious parts of a paper based on accumulated evidence, as human reviewers do. In this paper, we explore how to enable an LLM-based review agent to perform such proactive investigation. We find that this can be naturally formulated as a Markov Decision Process (MDP), and propose ProReviewer, a scientific peer review agent that proactively reviews a paper guided by a maintained, structured review log. The structured review log serves as a workspace for the agent to track evidence and intermediate findings collected during review. Experiments show that ProReviewer with an 8B backbone, trained by supervised fine-tuning and optimized by reinforcement learning, achieves the highest average score across five quality dimensions, outperforming prompt-based methods with much larger frontier LLMs by up to 39% and the strongest fine-tuned baseline by 16% relatively. It also attains the highest win rates against baselines in human evaluation.


### [37] [S-GBT: Smooth Growth Bound Tensor for Certified Robustness Against Word Substitution Attacks in NLP](https://arxiv.org/abs/2606.13439)
*Mohammed Bouri, Mohammed Erradi, Adnane Saoud*

#### 🧩 TL;DR
本文提出Smooth Growth Bound Tensor (S-GBT)，一种基于二阶信息（Hessian矩阵元素级上界）的鲁棒性正则化方法，用于防御自然语言处理中的词替换攻击。实验表明，结合一阶与二阶正则化可将认证鲁棒准确率提升高达23.4%，同时保持干净准确率竞争力。

---

#### 📘 Detailed Summary
**Motivation:** 现有针对词替换攻击的防御方法主要关注一阶敏感性（梯度），忽略了敏感性随输入变化的趋势（即曲率信息）。当梯度变化剧烈时，即使梯度本身被控制，模型仍可能失效。本文旨在填补这一空白，引入二阶方法以更全面地刻画并提升模型鲁棒性。

**Method:** 本文提出Smooth Growth Bound Tensor (S-GBT)，一种对Hessian矩阵进行元素级上界估计的二阶方法，并提供了形式化的鲁棒性界理论证明。S-GBT分别针对LSTM和CNN两种架构进行推导，并在训练目标中加入正则化项以最小化这些上界，从而同时约束输出变化的线性项（一阶）和二次项（二阶）。

**Result:** 在多个基准数据集上的实验结果表明，结合一阶与二阶正则化的方法相比先前方法，认证鲁棒准确率提升了高达23.4%，同时干净样本准确率保持竞争力。

**Conclusion:** 本文研究表明，同时控制梯度的幅度（一阶）及其变化程度（二阶）是构建更鲁棒模型的有效方向。S-GBT将二阶信息直接融入训练目标，为对抗防御提供了新的理论视角与实践工具。

---

#### 📄 Abstract
Despite recent progress in Natural Language Processing (NLP), models remain vulnerable to word substitution attacks. Most existing defenses focus on first order sensitivity and measure how much the output changes when the input is slightly perturbed. However, they ignore how this sensitivity evolves, which is described by curvature. When gradients vary sharply, models can still fail. This paper introduces the Smooth Growth Bound Tensor (S-GBT), a second order method that bounds the Hessian element-wise, for which we provide formal theoretical proofs on the resulting robustness bounds. A regularization term is added during training to minimize these bounds. This yields tighter certified robustness against word substitution attacks. The change in the output under word substitution is bounded by both a linear term and a quadratic term. S-GBT is derived for two architectures: Long Short-Term Memory (LSTM) and Convolutional Neural Networks (CNN). The method is integrated directly into the training objective. Its effectiveness is evaluated on multiple benchmark datasets. The results show that combining first and second order regularization improves certified robust accuracy by up to 23.4% compared to prior methods, while clean accuracy remains competitive. These findings indicate that controlling both the gradient and its variation is a promising direction for building more robust models.


### [38] [Leveraging Audio-LLMs to Filter Speech-to-Speech Training Data](https://arxiv.org/abs/2606.13507)
*Qixu Chen, Satoshi Nakamura*

#### 🧩 TL;DR
本文提出Rank-to-Distill策略，通过两阶段方式训练音频大语言模型对语音数据进行质量筛选，无需人工标注即可提升端到端语音翻译性能，在CVSS-C和SpeechMatrix上取得最高+1.4 ASR-BLEU的提升。

---

#### 📘 Detailed Summary
**Motivation:** 大规模挖掘的语料库为端到端语音到语音翻译提供了丰富训练数据，但其中普遍存在噪声、错位和语义错误，而如何有效过滤这些低质量数据是提升翻译鲁棒性的关键挑战。

**Method:** 采用可扩展的两阶段Rank-to-Distill策略：首先训练一个轻量级排序器从噪声语音对生成keep/drop伪标签，然后利用这些伪标签训练一个音频大语言模型直接从原始配对语音中预测keep/drop决策。该模型能够联合捕捉声学保真度和跨语言语义一致性，实现基于语音条件的数据选择。

**Result:** 在CVSS-C和SpeechMatrix数据集上的实验表明，该方法相较于未过滤训练数据取得了一致的性能提升，端到端语音翻译的ASR-BLEU指标最高提升+1.4。

**Conclusion:** 该研究证明通过自监督方式训练音频大语言模型直接对语音数据进行质量筛选是可行且有效的，无需人工标注即可自动过滤噪声数据，为大规模构建高质量语音翻译训练集提供了实用方案。

---

#### 📄 Abstract
Large-scale mined corpora provide abundant training data for end-to-end speech-to-speech translation (S2ST) but may contain noise, misalignment, and semantic errors. Filtering noisy data is crucial to maintain robust speech translation performance. We study how to train an audio-language model to make keep/drop decisions on paired speech directly from audio. To obtain reliable supervision without manual labels, we adopt a scalable two-stage Rank-to-Distill strategy. A lightweight ranker generates keep/drop pseudo-labels from noisy speech pairs, then trains an audio large language model to predict keep/drop directly from raw paired speech. The resulting model jointly captures acoustic fidelity and cross-lingual semantic consistency for the selection of speech-conditioned data. Experiments on CVSS-C and SpeechMatrix show consistent improvements over unfiltered training, yielding up to +1.4 ASR-BLEU for end-to-end S2ST.


### [39] [ArogyaSutra: A Multi-Agent Framework for Multimodal Medical Reasoning in Indic Languages](https://arxiv.org/abs/2606.13572)
*Tanmoy Kanti Halder, Akash Ghosh, Subhadip Baidya, Arijit Roy, Sriparna Saha*

#### 🧩 TL;DR
本文提出了ArogyaBodha数据集和ArogyaSutra多智能体框架，用于提升多语言多模态医学推理能力，实验证明其在多种印度语言上显著优于现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有的多模态大语言模型在通用领域表现良好，但在医疗等专业场景中性能有限，尤其是在多语言和低资源环境下。这一问题在印度农村等地区尤为突出，患者通常用本土语言表达复杂医疗问题并依赖医学图像等模态输入，而现有以英语为中心的MLLMs无法有效支持这些场景，限制了AI辅助医疗的公平可及性。

**Method:** 本文构建了ArogyaBodha大规模多语言多模态医学问答数据集，涵盖8个异构来源、31个身体系统、6种成像模态和21个临床领域，支持英语及7种主要印度语言。同时提出ArogyaSutra，一个基于演员-评论家范式的多智能体框架，集成工具调用与双记忆机制以实现逐步推理感知决策，并利用存储的演员-评论家仿真轨迹进行蒸馏。

**Result:** 实验表明，所提出的数据集和框架在所有印度语言上均提升了多语言医学推理准确率，消融实验验证了各组件（数据集、多智能体框架、双记忆机制、蒸馏轨迹）的贡献。

**Conclusion:** 本文验证了面向多语言低资源医疗场景构建专用数据集和推理框架的有效性，表明通过多智能体协作与记忆增强机制可显著提升MLLMs在多语言医疗推理中的表现，为AI驱动的公平医疗可及性提供了可行路径。

---

#### 📄 Abstract
Multimodal Large Language Models (MLLMs) have shown promising reasoning capabilities in general domains, yet their performance remains limited in specialized settings such as healthcare, especially in multilingual and low-resource scenarios. This gap is critical in regions like rural India, where patients often express complex medical queries in native Indic languages and rely on multimodal inputs such as medical images. Existing English-centric MLLMs struggle to support such use cases, limiting equitable access to AI-driven healthcare assistance. To address this challenge, we introduce ArogyaBodha, a large-scale multilingual multimodal medical question-answer dataset constructed from eight heterogeneous sources, covering 31 body systems, six imaging modalities, and 21 clinical domains across English and seven major Indian languages. We further propose ArogyaSutra, an actor-critic-based multi-agent framework that integrates tool grounding with dual-memory mechanisms for step-wise, reasoning-aware decision making, and uses stored actor-critic simulation trajectories for distillation. Experiments show that our dataset and framework improve multilingual medical reasoning accuracy across all Indic languages, with ablations validating the contribution of each component. The source code and dataset are available at: https://iitp-cse.github.io/ ArogyaSutra/


### [40] [LabVLA: Grounding Vision-Language-Action Models in Scientific Laboratories](https://arxiv.org/abs/2606.13578)
*Baochang Ren, Xinjie Liu, Xi Chen, Yanshuo Liu, Chenxi Li, Daqi Gao, Zeqin Su, Jintao Xing, Zirui Xue, Rui Li, Xiangyu Zhao, Shuofei Qiao, Minting Pan, Wangmeng Zuo, Lei Bai, Dongzhan Zhou, Ningyu Zhang, Huajun Chen*

#### 🧩 TL;DR
本文提出LabVLA模型和RoboGenesis仿真数据引擎，通过FAST动作令牌预训练与流匹配后训练两阶段策略，解决科学实验室场景下视觉-语言-动作模型的领域适应问题，在LabUtopia基准上取得最优性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉-语言-动作（VLA）模型主要基于家居和桌面场景数据训练，难以应对科学实验室中特有的仪器、透明液体和固定协议流程等复杂场景，且缺乏统一框架来适配科学实验中的多样化机器人形态，数据和具身形态成为核心瓶颈。

**Method:** 本文提出RoboGenesis仿真数据引擎，通过从原子技能组合配置化的实验室工作流、验证过滤并导出跨多种机器人形态的结构化演示数据；同时提出LabVLA模型，采用两阶段训练策略：第一阶段通过FAST动作令牌预训练使Qwen3-VL-4B-Instruct骨干网络具备动作感知能力，第二阶段通过流匹配后训练在知识隔离条件下附加DiT动作专家模块。

**Result:** 在LabUtopia基准测试中，LabVLA在分布内和分布外两种设置下均取得了所有评估基线中最高的平均成功率。

**Conclusion:** 数据和具身形态是科学实验室VLA模型面临的核心瓶颈，通过仿真数据引擎和两阶段训练策略可有效弥合实验室特定监督与通用VLA模型之间的差距，为机器人自动执行科学实验协议提供了可行方案。

---

#### 📄 Abstract
Scientific laboratories increasingly rely on AI systems to reason about experiments, but the physical act of doing science remains largely outside their reach. AI can help read literature, generate hypotheses, and plan protocols, yet the execution of those protocols at the bench still requires a human operator. Vision-Language-Action (VLA) models provide one possible interface between written protocols and robot execution, but existing policies are trained mostly on household and tabletop demonstrations and rarely encounter the instruments, transparent liquids, or fixed protocol workflows found in scientific laboratories. Closing this gap requires both laboratory-specific supervision and a unified learning framework that can accommodate the diverse robot embodiments used to execute experimental protocols. We therefore identify data and embodiment as central bottlenecks alongside model design. To address the data side, we build RoboGenesis, a simulation-based workflow and data engine that composes configured laboratory workflows from atomic skills, validates and filters rollouts, and exports structured demonstrations across supported robot profiles. On the policy side, we present LabVLA, trained with a two-stage recipe: FAST action token pretraining first makes the Qwen3-VL-4B-Instruct backbone action aware before any continuous control is learned, and flow matching posttraining then attaches a DiT action expert under knowledge insulation. On the LabUtopia benchmark, LabVLA achieves the highest average success rate among all evaluated baselines under both in-distribution and out-of-distribution settings.


### [41] [Recursive Agent Harnesses](https://arxiv.org/abs/2606.13643)
*Elias Lumer, Sahil Sen, Kevin Paul, Vamse Kumar Subbiah*

#### 🧩 TL;DR
本文提出了递归代理框架（Recursive Agent Harness, RAH），将递归语言模型（RLM）中基于模型调用的递归思想扩展到代码优先的代理级递归，其中父代理生成并运行可执行脚本以并行生成子代理进行细粒度任务执行，在长上下文推理任务上显著超越既有基线。

---

#### 📘 Detailed Summary
**Motivation:** 现有递归语言模型（RLM）证明了递归式模型调用对长上下文推理的有效性，而生产级编码代理已开始大规模生成生成子代理的代码，但两者之间缺乏对代理级递归模式的系统性研究与命名。本文旨在识别、定义并系统评估这种以完整代理环境（含文件系统工具、代码执行和规划能力）而非仅模型调用为递归单元的模式。

**Method:** 提出的递归代理框架（RAH）采用代码优先的递归策略：父代理生成并运行可执行脚本，该脚本并行生成子代理以处理细粒度工作负载，并利用结构化函数调用处理小型子任务。这本质上是将RLM的模型递归扩展为代理级递归（harness recursion），其中递归单元是包含文件系统工具、代码执行和规划能力的完整代理环境。

**Result:** 在固定GPT-5为骨干模型、以已发表的Codex和RLM基线为对照的受控评估中，RAH在Oolong-Synthetic基准（199个样本、13个上下文长度区间、最长4M token）上将Codex编码代理基线从71.75%提升至81.36%，该增益源于代理框架而非模型本身。使用更强骨干Claude Sonnet 4.5时，同一设计达到89.77%。

**Conclusion:** 代理解释级递归（harness recursion）相比模型级递归能带来显著的长上下文推理性能提升，且增益可归因于代理框架设计本身而非底层模型能力的差异。该工作为递归代理架构的未来探索提供了明确的基准和形式化框架，有助于推进生产级动态工作流的设计。

---

#### 📄 Abstract
Recursive language models (RLMs) showed that recursion over model calls is an effective strategy for long-context reasoning, and production coding agents have begun to write code that spawns subagents at scale, most recently in Anthropic's dynamic workflows. We name and study the pattern between these two lines of work, where the recursive unit is a full agent harness with filesystem tools, code execution, and planning rather than a model call with no tools. We call this the Recursive Agent Harness (RAH) and frame it as harness recursion, the code-first extension to the model recursion of RLMs. A parent agent generates and runs an executable script that spawns subagent harnesses in parallel for fine-grained workloads and uses structured function calls for small subtasks. We provide a controlled evaluation on long-context reasoning. With the backbone held fixed at GPT-5 to match the published Codex and RLM baselines, RAH improves the Codex coding-agent baseline from 71.75% to 81.36% on Oolong-Synthetic (199 samples, 13 context-length buckets up to 4M tokens), a gain attributable to the harness rather than the model. With a stronger backbone, Claude Sonnet 4.5, the same design reaches 89.77%.


### [42] [HyperTool: Beyond Step-Wise Tool Calls for Tool-Augmented Agents](https://arxiv.org/abs/2606.13663)
*Yaxin Du, Yifan Zhou, Yujie Ge, Jiajun Wang, Xianghe Pang, Shuo Tang, Tuney Zheng, Bryan Dai, Jian Yang, Siheng Chen*

#### 🧩 TL;DR
HyperTool 提出一种可执行的 MCP 风格工具接口，将原子化工具调用折叠为单一代码块调用，解决了工具执行粒度不匹配问题，在 MCP-Universe 上将 Qwen3-32B 的准确率从 15.69% 提升至 35.29%。

---

#### 📘 Detailed Summary
**Motivation:** 现有工具增强型 LLM 智能体采用逐步骤原子化工具调用方式，每次调用、观测和值传递都暴露在主推理轨迹中，导致执行粒度不匹配：局部确定性的工具工作流被拆解为多次模型可见决策，浪费上下文并迫使模型在轨迹中管理底层数据流。

**Method:** HyperTool 提出一种统一的、可执行的 MCP 风格工具接口，模型通过一个代码块调用 HyperTool，该代码块可调用已有工具、操作返回值并在本地传递中间结果，将确定性工具子例程折叠为单一外部调用。为训练模型使用此接口，作者从跨工具组合任务中合成 HyperTool 格式的轨迹，并在真实 MCP 环境中进行验证。

**Result:** 在 MCP-Universe 基准上，HyperTool 将 Qwen3-32B 的平均准确率从 15.69% 提升至 35.29%，将 Qwen3-8B 从 9.93% 提升至 33.33%，并在平均准确率上超过 GPT-OSS 和 Kimi-k2.5，显著改进了多步工具使用性能。

**Conclusion:** HyperTool 证明通过改变工具执行的模型可见单元，将确定性子过程折叠为单次调用，可大幅提升多步工具组合任务的效率与准确性，为工具增强型 LLM 智能体的接口设计提供了新方向。

---

#### 📄 Abstract
Tool-augmented LLM agents commonly rely on step-wise atomic tool calls, where each invocation, observation, and value transfer is exposed in the main reasoning trace. This creates an \emph{execution-granularity mismatch}: locally deterministic tool workflows are unfolded into repeated model-visible decisions, consuming context and forcing the model to manage low-level dataflow in the trace. We introduce \textbf{HyperTool}, a unified executable MCP-style tool interface that changes the model-visible unit of tool execution. A model invokes HyperTool with a code block that can call existing tools through their original schemas, manipulate returned values, and pass intermediate results locally, folding deterministic tool subroutines into a single outer call. To train models to use this interface, we synthesize HyperTool-format trajectories from cross-tool compositional tasks and verify them in real MCP environments. On MCP-Universe, HyperTool improves average accuracy from 15.69\% to 35.29\% on Qwen3-32B and from 9.93\% to 33.33\% on Qwen3-8B, and surpass GPT-OSS and Kimi-k2.5 on average accuracy, showing that our HyperTool can substantially improve multi-step tool use.


### [43] [EvoArena: Tracking Memory Evolution for Robust LLM Agents in Dynamic Environments](https://arxiv.org/abs/2606.13681)
*Jundong Xu, Qingchuan Li, Jiaying Wu, Yihuai Lan, Shuyue Stella Li, Huichi Zhou, Bowen Jiang, Lei Wang, Jun Wang, Anh Tuan Luu, Caiming Xiong, Hae Won Park, Bryan Hooi, Zhiyuan Hu*

#### 🧩 TL;DR
提出EvoArena基准套件和EvoMem补丁式记忆范式，用于评估和提升LLM智能体在动态环境中的适应能力。实验表明现有智能体在演化环境下表现不佳（平均39.6%），而EvoMem能持续提升性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM智能体评估大多基于静态环境，而真实部署场景本质上是动态的，智能体需要持续适应环境变化和任务条件更新，但缺乏针对演化环境的评估基准和相应的记忆机制。

**Method:** 构建EvoArena基准套件，将环境变化建模为终端、软件和社交三个领域的渐进式更新序列；提出EvoMem补丁式记忆范式，将记忆演化记录为结构化的更新历史，使智能体能够通过记忆变化推理环境演化。

**Result:** 现有智能体在EvoArena上平均准确率仅39.6%；EvoMem在EvoArena上平均提升1.5%，在GAIA和LoCoMo上分别提升6.1%和4.8%，在演化子任务链上提升3.7%的链级准确率。机制分析表明EvoMem改善了记忆中的证据捕获能力。

**Conclusion:** 在评估和记忆两个层面显式建模环境演化对于实现可靠的智能体部署至关重要，补丁式记忆范式能够有效帮助智能体推理环境变化并提升任务表现。

---

#### 📄 Abstract
Large language model (LLM) agents have achieved strong performance on a wide range of benchmarks, yet most evaluations assume static environments. In contrast, real-world deployment is inherently dynamic, requiring agents to continually align their knowledge, skills, and behavior with changing environments and updated task conditions. To address this gap, we introduce EvoArena, a benchmark suite that models environment changes as sequences of progressive updates across terminal, software, and social domains. We further propose EvoMem, a patch-based memory paradigm that records memory evolution as structured update histories, enabling agents to reason about environmental evolution through changes in their memory. Experiments show that current agents struggle on EvoArena, achieving an average accuracy of 39.6% across evolving terminal, software, and social-preference domains. EvoMem consistently improves performance, yielding an average gain of 1.5% on EvoArena and also improving standard benchmarks such as GAIA and LoCoMo by 6.1% and 4.8%. Beyond individual tasks, EvoMem further improves chain-level accuracy by 3.7% on EvoArena, where success requires completing a consecutive sequence of related evolutionary subtasks. Mechanistic analysis shows that EvoMem improves evidence capture in the memory, indicating better preservation of complete evolving environment states. Our results highlight the importance of modeling evolution in both evaluation and memory for reliable agent deployment.


<div id='cs.AI'></div>

# cs.AI [[Back]](#toc)

### [44] [Arbor: Tree Search as a Cognition Layer for Autonomous Agents](https://arxiv.org/abs/2606.12563)
*Neha Prakriya, Chaojun Hou, Zheng Gong, Huasha Zhao, Xi Zhao, Mou Li, Zhenyu Gu, Emad Barsoum*

#### 🧩 TL;DR
Arbor是一个多智能体框架，通过结构化树搜索作为认知层，结合Orchestrator和Critic双智能体制衡架构，在全栈LLM推理优化中实现高达193%的吞吐量-延迟Pareto改进，且硬件无关、可复现。

---

#### 📘 Detailed Summary
**Motivation:** 现有自主优化系统仅针对孤立目标且基于无状态评估，缺乏在大型、有状态动作空间中进行协同探索的能力；全栈LLM推理优化历来依赖跨应用、框架、编译器、内核和硬件栈的工程团队协调，亟需一种能够有效协调多领域专家智能体并利用失败信号引导探索的自动化框架。

**Method:** Arbor构建一个显式的带评分假设搜索树作为所有智能体的共享工作记忆，随每次测量演化；采用Orchestrator与Critic双智能体制衡架构，Orchestrator驱动优化并委派领域专家，Critic通过根因分析、内省和测量验证维护稳定性；智能体能力分解为硬技能（领域专长）和软技能（协调协议），实现完全自主的多日优化。

**Result:** Arbor在LLM推理优化中实现高达193%的吞吐量-延迟Pareto改进，远超供应商优化基线；无框架支撑的单智能体仅实现33%吞吐量提升并在数小时内崩溃且无法恢复；方法跨多代硬件平台泛化，运行间方差在2个百分点以内，表明其硬件无关且可复现。

**Conclusion:** 结构化树搜索作为多智能体共享认知层能够有效处理大规模有状态动作空间的自主优化问题；制衡架构（Checks-and-Balances）可防止单智能体驱动导致的系统崩溃；将智能体能力分解为硬技能与软技能使得复杂跨栈优化任务可完全自主执行，且该方法具有硬件通用性和可复现性。

---

#### 📄 Abstract
Arbor is a multi-agent framework that introduces structured tree search as a cognition layer for autonomous agents operating in large, stateful action spaces. Prior autonomous optimization systems operate on isolated targets with stateless evaluation. Arbor instead maintains an explicit search tree of scored hypotheses that serves as the shared working memory across agents, evolving with every measurement, treating failures as diagnostic signal that reshapes subsequent exploration, and expanding as prior successes shift the bottleneck distribution.
  We validate Arbor on full-stack LLM inference optimization, a domain where achieving peak performance has historically required coordinated effort from engineering teams across the application, framework, compiler, kernel, and hardware stack. Arbor pairs an Orchestrator agent, which drives optimization by delegating to Domain Specialists across the inference stack, with a Critic agent that safeguards stability through root-cause analysis, introspection, and measurement validation -- a checks-and-balances architecture where neither agent can unilaterally drive the system. Agent capabilities are decomposed into hard skills (domain expertise) and soft skills (coordination protocols that determine how contributions compose), enabling fully autonomous multi-day campaigns. Arbor achieves up to 193% inference throughput-latency Pareto improvement over vendor-optimized baselines, while a single agent without the harness plateaus at +33% throughput improvement and crashes irrecoverably within hours. Arbor generalizes to multiple generations of hardware platform, and run-to-run variance is within 2 percentage points demonstrating that the method is hardware-agnostic and reproducible.


### [45] [Strategic Decision Support for AI Agents](https://arxiv.org/abs/2606.12587)
*Shayan Kiyani, Sima Noorani, George Pappas, Hamed Hassani*

#### 🧩 TL;DR
本文提出了一种面向AI智能体的策略性决策支持框架，通过优化问题最小化支持使用量，同时控制反事实遗漏支持误差，并开发了在线自适应阈值算法和即时校准方法，在信息收集、人机协作和工具使用等场景中可靠地控制目标误差并大幅减少不必要的支持调用。

---

#### 📘 Detailed Summary
**Motivation:** 传统决策支持研究聚焦于人类如何借助机器学习模型做出更好决策，但在现代智能体系统中角色发生逆转——AI智能体代表用户行动，人类和工具成为辅助机制。这种角色反转带来了可靠性问题，智能体的错误可能产生严重后果，且其行为必须与人类目标和约束保持一致。因此，需要重新审视决策支持的基本原理（支持的成本-价值权衡以及不确定性量化的作用），以适应当AI智能体作为核心行动者的新范式。

**Method:** 本文提出一个面向AI智能体的策略性决策支持框架，通过一个优化问题来最小化支持使用量，同时控制反事实遗漏支持误差（即智能体在缺乏支持时单独行动、而支持本可实质改善其输出的概率）。在总体层面证明最优策略是支持价值上的阈值规则。基于此结构，开发了一种在线自适应阈值算法，通过随机探索在无需分布假设的情况下控制遗漏支持误差；进一步引入即时校准方法，在线减少不必要的支持调用。

**Result:** 该框架被实例化于信息收集、人机协作和工具使用等多种场景中，实验表明该方法能够可靠地控制目标误差，同时在实际中大幅减少支持使用量。

**Conclusion:** 本文从根本上逆转了决策支持的传统视角，将AI智能体而非人类作为决策支持的核心对象，为构建更可靠、资源高效的自主智能体系统提供了新的理论框架和实用算法。

---

#### 📄 Abstract
Traditionally, decision support studies how humans use machine learning models to make better decisions. In modern agentic systems, this division of roles is increasingly reversed: AI agents act on behalf of users, while humans and tools becomes support mechanisms around them. This role reversal brings reliability concerns to the forefront, since agentic errors can be consequential and agent behavior must remain aligned with human goals and constraints. Departing from the classical view of decision support, we revisit its two basic principles, the cost--value tradeoff of seeking support and the role of uncertainty quantification, in a setting where AI agents are the central actors. We propose a framework for strategic decision support for AI agents through an optimization problem that minimizes support usage subject to controlling a counterfactual missed-support error: the probability that the agent acts alone on instances where support would have materially improved its output. At the population level, we show that the optimal policy is a threshold rule on the value of support. Building on this structure, we develop an online algorithm that adaptively thresholds such a score and uses randomized exploration to control missed-support error without distributional assumptions. We further introduce a calibration-on-the-fly method that reduces unnecessary support calls online. We instantiate this framework across diverse scenarios, including information gathering, human--AI collaboration, and tool use, showing how each can be modeled through the same strategic decision-support lens. Experiments across these settings show that our method reliably controls the target error while substantially reducing support usage in practice.


### [46] [PersonaDrive: Human-Style Retrieval-Augmented VLA Agents for Closed-Loop Driving Simulation](https://arxiv.org/abs/2606.12616)
*Mahmoud Srewa, Praneetsai Iddamsetty, Mohammad Abdullah Al Faruque, Salma Elmalaki*

#### 🧩 TL;DR
PersonaDrive 提出一种基于风格指令的人类驾驶数据集和检索增强的视觉-语言-动作框架，通过少样本上下文演示实现驾驶风格的条件化控制，无需针对每种风格重新训练，在 Bench2Drive 上达到最优驾驶分数。

---

#### 📘 Detailed Summary
**Motivation:** 现有闭环驾驶模拟器中的非自车交通智能体行为模式单一，要么基于规则、要么基于单模式学习的模型；已有的风格变异方法依赖事后标签或 LLM 推断的奖励权重作为风格代理信号，而非来自被明确要求以特定风格驾驶的人类演示，缺乏真实的人类风格驾驶数据支撑。

**Method:** PersonaDrive 是一个三阶段流水线：(i) 使用图像-文本联合相似度对风格指令下的人类驾驶数据进行离线三元组挖掘；(ii) 训练一个轻量级检索头，融合冻结的视觉特征与小型控制编码器，构建各风格的检索数据库；(iii) 微调单一 VLA 骨干网络，将检索到的上下文点作为少样本行为演示进行路径点预测；推理时通过切换检索数据库实现风格条件化，无需每风格重新训练。

**Result:** 在 Bench2Drive 上，无风格条件下 PersonaDrive 驾驶分数比 SimLingo 提升 4.6%、比 HiP-AD 提升 2.5%；风格条件下每种风格均取得最高驾驶分数，且最弱风格仍超过最强基线 DMW 达 5.4%；从保守到激进指令，平均速度和加速度分别上升 18% 和 25%。

**Conclusion:** PersonaDrive 证实了基于真实人类风格驾驶演示的检索增强方法能够有效实现驾驶风格的条件化控制，无需每风格重新训练即可生成风格多样的非自车智能体，为闭环模拟中更真实、多样的交通行为生成提供了可行路径。

---

#### 📄 Abstract
Closed-loop driving simulators typically populate their environments with non-ego traffic agents that behave largely the same way, produced either by rule-based traffic managers or by learned models trained toward a single behavioral mode. Recent work introduces style variation through post-hoc labels on observational data or LLM-inferred reward weights, but these signals act as proxies for what a style should reward rather than demonstrations of humans explicitly asked to drive in that style. We introduce PersonaDrive, a pipeline that conditions a vision-language-action (VLA) driving agent on retrieved demonstrations from a style-instructed human driving dataset, in which participants drive CARLA leaderboard routes under aggressive, neutral, and conservative instructions on a driver-in-the-loop rig. The pipeline has three stages: (i) offline triplet mining over per-style human driving data using a combined image-text similarity score; (ii) training a lightweight retrieval head that fuses frozen visual features with a small control encoder over per-style databases; and (iii) fine-tuning a single VLA backbone to treat retrieved context points as in-context behavioral demonstrations during waypoint prediction. At inference, the same backbone is conditioned on any style by swapping which per-style database the retrieval head queries, so selecting a style requires no per-style retraining while enabling human-style, style-diverse non-ego agents for closed-loop simulation. On Bench2Drive, PersonaDrive (no style) improves the driving score by 4.6% over SimLingo and 2.5% over HiP-AD, and under style conditioning attains the highest driving score in every style within a roughly 2% band (its weakest style surpassing the strongest baseline, DMW, by 5.4%), while average speed and acceleration rise by 18% and 25% from the conservative to the aggressive instruction.


### [47] [TrajGenAgent: A Hierarchical LLM Agent for Human Mobility Trajectory Generation](https://arxiv.org/abs/2606.12657)
*Siyu Li, Toan Tran, Lingyi Zhao, Khurram Shafique, Li Xiong*

#### 🧩 TL;DR
TrajGenAgent 提出了一种语义感知的分层 LLM 智能体框架，通过无需微调的两阶段编排器-工作者设计，结合上下文学习与确定性工作流，生成具有高时空保真度、语义一致性和个体行为真实感的人类移动轨迹。

---

#### 📘 Detailed Summary
**Motivation:** 现有人类移动轨迹生成方法存在两难：基于提示工程的 LLM 生成器虽保留零样本推理能力但缺乏细粒度时空 grounding，而轨迹级微调虽提升统计精度却计算成本高昂且可能削弱通用推理能力，因此亟需一种无需模型微调、兼顾语义推理与时空精确性的轨迹生成框架。

**Method:** TrajGenAgent 采用两阶段编排器-工作者设计：首先由 LLM 通过上下文学习从历史证据中合成个体和星期几条件化的活动链，随后通过确定性工作流将每个活动落地为完整访问，具体包括个性化 POI 检索、距离感知位置选择、运动学感知旅行时间传播和 LLM 驱动的时长估计；此外还引入基于异常检测的评估框架，使用两个互补检测器评估行为和语义合理性。

**Result:** 在基准数据集和大规模仿真数据集上的实验表明，TrajGenAgent 在时空保真度、语义一致性以及个体特定行为真实感方面均优于代表性神经方法和基于 LLM 的基线模型，且无需任何参数更新。

**Conclusion:** TrajGenAgent 通过分层智能体设计有效解耦了高层语义推理与底层时空 grounding，为无需微调的 LLM 轨迹生成提供了可行范式，其异常检测评估框架也为超越传统聚合统计指标的行为真实性评估提供了新视角。

---

#### 📄 Abstract
Human mobility data is important for transportation, urban planning, and epidemic control, but large-scale trajectory collection is often costly and privacy-constrained, motivating realistic synthetic trajectory generation. Existing LLM-based generators typically rely on either prompt engineering, which preserves zero-shot reasoning but lacks fine-grained spatiotemporal grounding, or trajectory-level fine-tuning, which improves statistical precision but incurs substantial computational cost and may weaken general reasoning. We propose TrajGenAgent, a semantic-aware hierarchical LLM-agent framework for human mobility trajectory generation without model fine-tuning. TrajGenAgent uses a two-stage orchestrator-worker design: an LLM first synthesizes an individual- and weekday-conditioned activity chain from historical evidence via in-context learning, and a deterministic workflow then grounds each activity into a complete visit using personalized POI retrieval, distance-aware location selection, kinematics-aware travel-time propagation, and LLM-based duration estimation. To evaluate realism beyond aggregate spatiotemporal statistics, we introduce an anomaly-detection-based evaluation framework using two complementary detectors to assess behavioral and semantic plausibility. Experiments on benchmark and large-scale simulation datasets show that TrajGenAgent improves spatiotemporal fidelity, semantic coherence, and individual-specific behavioral realism over representative neural and LLM-based baselines, while avoiding parameter updates.


### [48] [Evoflux: Inference-Time Evolution of Executable Tool Workflows for Compact Agents](https://arxiv.org/abs/2606.12674)
*Kushal Raj Bhandari, Ling Yue, Ching-Yun Ko, Dhaval Patel, Shaowu Pan, Pin-Yu Chen, Jianxi Gao*

#### 🧩 TL;DR
Evoflux提出了一种推理时演化搜索方法，将轻量级语言模型的工具使用问题转化为可执行工作流图的修复问题，通过结构化编辑、执行反馈和自适应搜索，在MCP-Bench任务上将执行可行性从约3%提升至17-24%，证明了在教师轨迹预算稀缺时基于执行反馈的搜索比蒸馏训练更可靠。

---

#### 📘 Detailed Summary
**Motivation:** 紧凑型语言模型在MCP风格的工具调用中面临一个关键问题：它们生成的工作流图在工具解析、参数验证、依赖跟踪和执行环节频繁失败，而小规模教师轨迹蒸馏虽然能教会工作流格式，却难以覆盖修复失败计划所需的恢复行为，导致现有方法在高延迟、高成本场景下表现不佳。

**Method:** Evoflux是一种推理时演化搜索方法，将紧凑型工具使用问题形式化为可执行工具工作流图的修复过程。它通过结构化编辑、执行反馈、自适应搜索强度、元引导重设计和多样性剪枝来演化带类型的工作流图，利用执行结果作为反馈信号驱动搜索过程。

**Result:** 在涵盖250个工具和实时MCP服务器的MCP-Bench任务上，Evoflux将各种小型规划器的执行可行性从约3%提升至17-24%；相比之下，基于相同搜索数据的SFT和SFT+DPO方法表现持平、不如甚至低于零样本基线，ReAct虽然能达到更高峰值但方差更大且token成本更高。

**Conclusion:** 基于执行反馈的搜索在教师轨迹预算稀缺时比小规模蒸馏训练更可靠，Evoflux表明通过推理时搜索而非单纯依赖于蒸馏，可以显著提升紧凑型模型在工具使用场景中的鲁棒性和执行成功率。

---

#### 📄 Abstract
Compact language models (LMs) reduce cost, latency, and deployment risk for tool agents. Yet MCP-style tool use requires more than isolated function calling: an agent must discover tools from live catalogs, satisfy schemas, preserve dependencies across intermediate outputs, and ground final responses in executed evidence. Small planners often generate plausible workflow graphs that fail under tool resolution, parameter validation, dependency tracking, or execution. We argue that this failure mode is poorly handled by small-corpus distillation. A few hundred teacher traces can teach workflow format, but rarely cover the recovery behavior needed to repair failed plans over changing tool catalogs. We introduce Evoflux, an inference-time evolutionary search method that treats compact tool use as the repair of executable tool workflows. It evolves typed workflow graphs through structured edits, execution feedback, adaptive intensity, meta-guided redesign, and diversity pruning. On held-out MCP-Bench tasks spanning live MCP servers and 250 tools, Evoflux raises execution feasibility from roughly 3% to 17-24% across small planners. In contrast, SFT and SFT+DPO on the same search-mined data match, underperform, or collapse below zero-shot performance; ReAct reaches higher peaks, but with higher variance and token cost. These results show that execution-grounded search is more reliable under scarce teacher-trace budgets.


### [49] [From AGI to ASI](https://arxiv.org/abs/2606.12683)
*Tim Genewein, Matija Franklin, Alexander Lerchner, Laurent Orseau, Samuel Albanie, Adam Bales, Cole Wyeth, Stephanie Chan, Iason Gabriel, Joel Z. Leibo, Allan Dafoe, Marcus Hutter, Thore Graepel, Shane Legg*

#### 🧩 TL;DR
本报告探讨了从人类级通用人工智能（AGI）向通用超级智能（ASI）转变的路径、摩擦与瓶颈，指出该转变可能并非单一变革性步骤，而是一系列由AI驱动的科学技术突破所引发的连锁社会变革。

---

#### 📘 Detailed Summary
**Motivation:** 尽管构建人类级AGI已成为许多顶级AI组织的近十年目标，但AGI之后AI如何沿着智能连续体继续发展、特别是从AGI过渡到超越大型组织认知能力的通用超级智能（ASI）的路径与挑战尚缺乏系统研究，本报告旨在填补这一空白。

**Method:** 报告在理论上以通用AI（Universal AI）的形式化框架为起点，明确了从AGI到ASI的智能连续体概念；随后提出了四条从AGI通往ASI的潜在路径：AGI规模扩展、AI范式转变、递归自我改进以及大规模多智能体集体涌现出ASI，并分析了各路径中可能存在的摩擦与瓶颈。

**Result:** 报告指出，由于预测ASI进展存在巨大不确定性，不能排除AI进展在未来数年继续加速的可能性；这意味着人类级AGI的引入可能并非单一变革性事件，而是一系列由AI驱动、横跨多个科学与技术领域的突破所引发的持续社会变革。

**Conclusion:** 应对这种持续变革的前景需要全球范围内的大规模跨学科合作；该研究提出了多项具体的开放研究问题，用以评估从AGI到ASI路径中摩擦因素的显著性，为未来的安全与治理研究提供了方向。

---

#### 📄 Abstract
Over the last decade, building human-level artificial general intelligence has moved from far-fetched speculation to being a concrete next-decade target for many of the largest AI organisations. Achieving this goal would have profound and far-reaching impacts on human society, which raises many complex questions for the decade ahead. This report investigates how AI itself might continue to develop in a post-AGI world along the continuum of machine intelligence. The endpoint of this continuum, Universal AI, is theoretically well understood, which provides some formal grounding for the main focus of this report: the transition from human-level AGI to artificial general superintelligence, which, intuitively, can be understood as a system that is more intelligent and cognitively capable than large organisations of humans. After characterizing ASI, the report discusses four potential pathways from AGI to ASI: scaling AGI, AI paradigm shifts, recursive improvement, and ASI emerging from large-scale multi-agent collectives. The report then discusses possible frictions and bottlenecks along these pathways. Determining whether the impact of these frictions will be negligible or substantial raises a number of concrete open research questions. Due to large uncertainties for predicting ASI progress, it cannot be ruled out that AI progress might continue to accelerate over the next years. This could imply that the image of a single transformative step change, caused by the introduction of human-level AGI into our society, could be inaccurate. More apt might be the prospect of a series of transformative societal changes caused by AI-enabled progress and breakthroughs across many areas of science and technology. Preparing for this prospect requires a massively interdisciplinary endeavour of global scope and interest.


### [50] [Reducing the Complexity of Deep Learning Models for EEG Analysis on Wearable Devices](https://arxiv.org/abs/2606.12742)
*Farough Shayeste Roodi, Parham Zilouchian Moghaddam, Mahdi Mohammadi-nasab, Mehdi Modarressi, Mostafa Ersali Salehi Nasab, Masoud Daneshtalab*

#### 🧩 TL;DR
本文研究了在资源受限的可穿戴设备上部署深度神经网络用于脑电信号（EEG）分析的可行性，重点探索了参数量化和电极缩减方法在降低计算复杂性与维持模型精度之间的权衡关系。

---

#### 📘 Detailed Summary
**Motivation:** 可穿戴医疗设备是物联网中增长最快的领域，其中ECG和EEG信号处理对自动化医疗服务至关重要。然而，深度神经网络的高计算、能耗和内存需求与可穿戴设备的极严格资源约束之间存在严重矛盾，阻碍了深度学习在实用可穿戴服务中的部署。

**Method:** 本文研究了多种最先进的DNN模型（专用于癫痫发作检测的EEG信号分析模型），采用参数量化和电极缩减两种方法来降低模型复杂度，并系统评估了这些方法在精度与计算复杂度之间的权衡关系。

**Result:** 实验结果表明，在合理应用的情况下，参数量化和电极缩减技术能够显著降低DNN模型的计算复杂度，同时对模型精度的影响极小。研究揭示了在适配基于DNN的在线EEG分析至可穿戴设备时，精度与复杂度缩减之间存在明确的权衡关系。

**Conclusion:** 通过参数量化和电极缩减技术，可以在不影响可穿戴设备实际可用精度的前提下大幅降低DNN模型复杂度，为深度学习方法在资源受限的可穿戴医疗设备上的部署提供了可行路径和明确的权衡指导。

---

#### 📄 Abstract
Wearable healthcare devices are the fastest-growing Internet of Things (IoT) sector. Many automated healthcare services rely on two crucial biological signals, namely ECG and EEG, which reflect the activity of the heart and brain, respectively. Although deep neural networks are considered the primary way to process and analyze these signals, the very tight energy and computational power constraints in wearable devices are far below the computational, energy, and memory bandwidth demands of DNN models, thereby impeding the deployment of deep learning in many practical wearable services. This paper investigates the feasibility of deploying state-of-the-art DNN models in resource-constrained wearable devices. Notably, we explore the trade-off between accuracy and computational complexity of DNNs when parameter quantization and electrode reduction methods are used. Our investigation centers on several state-of-the-art DNN models designed for EEG signal analysis, specifically for detecting epileptic seizures. Our findings demonstrate that, when applied judiciously, these techniques can significantly reduce the complexity of the DNNs under consideration with minimal adverse effects on accuracy. These results reveal the explicit trade-offs between accuracy and complexity reduction encountered when adapting DNN-based online EEG analysis for wearable devices.


### [51] [A Tutorial on World Models and Physical AI](https://arxiv.org/abs/2606.12783)
*Il-Seok Oh*

#### 🧩 TL;DR
本文提出了一个统一框架，将显式世界模型（通过结构化动力学进行推理和规划）与隐式世界模型（在可扩展表示中编码预测结构）整合起来，并基于预测结构这一共享原则对多样化方法进行分类和差异化分析。

---

#### 📘 Detailed Summary
**Motivation:** 世界建模正成为构建具备预测、推理和决策能力的智能系统的核心原则，但现有方法在显式与隐式世界模型之间存在显著范式差异，缺乏统一的视角来理解其内在联系与互补关系；同时，在层次推理、长时域规划和自主目标形成等关键问题上仍面临重大挑战，亟需一个连贯的理论框架来整合多样化的世界建模方法。

**Method:** 本文以综述教程的形式，提出了一个统一框架，将显式世界模型（学习结构化动力学以支持基于推演的推理和规划）与隐式世界模型（在可扩展的表示学习中编码预测结构）作为互补范式进行系统分类；该框架基于“共享预测结构”这一核心原则统一各类方法，并通过该结构如何被表示和利用来区分不同方法，同时探讨了近期基础模型在统一感知、预测和行动方面的潜在路径。

**Result:** 本文系统梳理了显式与隐式世界模型两大范式在机器人、自动驾驶等物理AI领域的应用基础，指出其在超越反应式控制方面展现出显著能力；同时明确了在层次推理、长时域规划和自主目标形成等维度仍存在的关键难题，并认为基础模型为构建统一系统提供了可行方向。

**Conclusion:** 世界建模方法虽形式多样，但可通过共享预测结构这一原则统一理解，不同方法的本质差异在于该结构如何被表示和利用；迈向通用人工智能的关键突破将依赖于解决层次推理、长时域规划和自主目标形成等核心挑战，而基础模型是向统一系统演进的重要路径。

---

#### 📄 Abstract
World modeling is emerging as a central principle for building intelligent systems capable of prediction, reasoning, and decision making. A central distinction can be drawn between explicit world models, which learn structured dynamics for rollout-based reasoning and planning, and implicit world models, which encode predictive structure within scalable learned representations. These complementary paradigms provide a foundation for physical AI in domains such as robotics and autonomous driving, enabling intelligence beyond reactive control under real-world constraints. Recent foundation models further suggest a pathway toward unified systems integrating perception, prediction, and action. Despite rapid progress, major challenges remain in hierarchical reasoning, long-horizon planning, and autonomous goal formation, which are critical for advancing toward artificial general intelligence. This tutorial presents a coherent framework in which diverse world modeling approaches are unified through shared predictive structure and differentiated by how such structure is represented and exploited.


### [52] [The Containment Gap: How Deployed Agentic AI Frameworks Fail Public-Facing Safety Requirements](https://arxiv.org/abs/2606.12797)
*Md Jafrin Hossain, Mohammad Arif Hossain, Weiqi Liu, Nirwan Ansari*

#### 🧩 TL;DR
本文系统性审计了三个主流智能体框架（LangChain、AutoGPT、OpenAI Agents SDK），发现所有框架均未原生满足安全防护原则，特别是缺乏内存完整性保护；通过模拟政府福利智能体的实证攻击，单次内存投毒将目标申请人的错误拒绝率提升至88.9%，并提出了两种轻量级防护机制，以低于0.2ms的开销完全消除攻击向量。

---

#### 📘 Detailed Summary
**Motivation:** 大型语言模型智能体系统已部署于政府服务、医疗分诊、金融咨询等面向公众的高风险领域，但当前用于构建这些系统的框架是否在架构层面提供结构性的安全保障尚不明确，因此需要系统性地评估其安全缺陷。

**Method:** 基于组合式智能体架构模型推导出六项隔离防护原则，据此审计LangChain、AutoGPT和OpenAI Agents SDK三个主流框架；随后在LangChain上构建模拟政府福利智能体，实施内存投毒攻击以实证评估；最后提出内存完整性验证器和策略门控两种轻量级防护机制。

**Result:** 三个框架均未原生遵守任何一项防护原则，尤其均缺乏内存完整性保护；单次内存投毒攻击可在所有测试种子和后端下对目标申请人产生持续性的定向错误，将错误拒绝率提升至88.9%；在复杂五因素政策下，攻击使目标定向错误拒绝率提升3.5倍，同时保持整体准确率不变，难以通过标准监控检测；所提出的两种防护机制以低于0.2ms的调用开销完全消除两种攻击向量。

**Conclusion:** 当前智能体框架生态系统尚未达到面向公开展部署时应具备的默认安全标准，亟需优先进行架构层面的安全干预，以实现高风险社会影响应用中的可信部署。

---

#### 📄 Abstract
Agentic large language model systems that autonomously invoke tools, maintain persistent memory, and execute multi-step plans are increasingly deployed in public-facing domains, including government services, healthcare triage, and financial advising. We ask whether the frameworks used to build these systems provide architectural-level structural safety guarantees. Applying six containment principles derived from a compositional model of agentic architectures, we audit three dominant frameworks (LangChain, AutoGPT, and OpenAI Agents SDK) and find no native compliance in any of them. Memory integrity, a defense against one of the most prevalent vulnerability classes, is not observed in any of the three evaluated frameworks. We validate these findings empirically: in a simulated government benefits agent built on LangChain, a single memory-poisoning write induces persistent targeted corruption across all tested seeds and backends, increasing the wrongful denial rate for targeted applicants to 88.9%. Under a complex five-factor policy, the same attack preserves aggregate accuracy while increasing targeted wrongful denials by 3.5x, rendering the corruption difficult to detect through standard monitoring. We then introduce two lightweight containment mechanisms: a memory integrity validator and a policy gate, which eliminate both attack vectors with sub-millisecond overhead (<0.2ms per call). We conclude that the current agentic framework ecosystem may not yet meet secure-by-default expectations for public-facing deployments and outline priority architectural interventions to enable trustworthy deployment in high-stakes, socially impactful applications.


### [53] [Teach-and-Repeat: Accurately Extracting Operational Knowledge from Mobile Screen Demonstrations to Empower GUI Agents](https://arxiv.org/abs/2606.12817)
*Yudong Zhang, Lei Hu, Daoyang Liu, Jiawei Liu, Yangfan Luo, Xingyu Liu, Zuojian Wang, Zhilin Gao*

#### 🧩 TL;DR
本文提出 Teach VLM 模型与 Teach-and-Repeat 范式，将移动设备屏幕轨迹自动转化为可解释的操作知识（步骤式自然语言描述），并利用该知识指导下游执行代理，显著提升了操作语义预测与任务自动化的性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉-语言模型在将移动屏幕动态视觉状态转化为操作知识（如动作类型、目标UI元素、文本参数及执行顺序）时，因UI设计的高度异构性与多样性而难以准确推理；同时，缺乏大规模对齐训练数据，制约了模型对操作语义的理解能力。

**Method:** Teach VLM 通过从移动屏幕演示视频中提取和分析操作相关关键帧，将轨迹翻译为步骤式操作知识；为缓解训练数据稀缺问题，构建了系统化的数据飞轮用于规模化数据采集；并提出中文移动屏幕教学基准（Chinese Mobile Screen Teach Benchmark）用于细粒度评估；进一步，基于该模型提出 Teach-and-Repeat 范式，将生成的操作知识作为可解释的过程性参考，指导下游执行代理完成自动化任务。

**Result:** 在操作语义预测任务上，Teach VLM 显著优于强视觉-语言基线模型，达到最先进水平；在 Android World 实验环境中，Teach-and-Repeat 范式在下游代理的任务成功率上取得了一致性提升。

**Conclusion:** Teach VLM 与 Teach-and-Repeat 范式共同提供了一条从原始演示到可复用任务自动化的实用路径，验证了可解释操作知识对提升移动端屏幕执行代理性能的有效性，并为UI理解与操作推理研究提供了新范式。

---

#### 📄 Abstract
Understanding the digital world on mobile devices is shifting from static UI perception to dynamic action comprehension. This capability enables models to convert visual state transitions into operational knowledge, defined as short natural-language sentences that describe action types, target UI elements, textual arguments, and execution orders. However, due to the highly diverse and heterogeneous UI designs across applications, existing vision-language models (VLMs) struggle to accurately infer these underlying operations. To bridge this gap, we introduce Teach VLM, a core model designed to translate mobile screen trajectories into step-wise operational knowledge by extracting and analyzing operation-related keyframes from demonstration videos. To address the scarcity of aligned training data, we develop a systematic data flywheel for scalable data acquisition. We further introduce a novel Chinese Mobile Screen Teach Benchmark for fine-grained evaluation. Building upon Teach VLM, we propose the Teach-and-Repeat paradigm, where the generated operational knowledge serves as an interpretable procedural reference to guide downstream screen-based execution agents. Extensive evaluations demonstrate that Teach VLM significantly outperforms strong VLM baselines, achieving state-of-the-art performance in operation semantics prediction. Furthermore, experiments in Android World show that our paradigm yields consistent Task Success Rate improvements for downstream agents. Together, Teach VLM and the Teach-and-Repeat paradigm offer a practical pathway from raw demonstrations to reusable task automation.


### [54] [GeoNatureAgent Benchmark: Benchmarking LLM Agents for Environmental Geospatial Analysis Across Frontier and Open-Weight Foundation Models](https://arxiv.org/abs/2606.12821)
*Gabriel Diaz-Ireland, Diego Prieto-Herráez, Mario García Peces, Javier Velázquez, Devika Jain*

#### 🧩 TL;DR
GeoNatureAgent Benchmark 是首个针对通过结构化工具调用真实地理空间 API 的环境分析智能体设计的基准测试，包含93个任务覆盖18个类别，评估了7个 LLM 的表现，发现 Claude Sonnet 4 以60.8%准确率领先，但开源模型 DeepSeek V3.2 以11倍更低成本达到其93%的能力，同时揭示了比较推理任务的系统性瓶颈。

---

#### 📘 Detailed Summary
**Motivation:** 环境科学家在数据整理上耗费过多精力，而能够自动化地理空间工作流的 AI 智能体尚未得到有效验证：现有基准测试缺乏对智能体通过结构化工具调用操作真实 API 的能力评估，这一空白阻碍了该类系统在实际环境分析中的应用落地。

**Method:** 构建了 GeoNatureAgent Benchmark，包含93个任务（18个类别），涵盖行政区分析、多轮对话、空间推理、跨指标综合、错误处理与恢复、排序、比较、多语言理解、栖息地分析和任务拒绝等维度；任务基于一个开源自部署 API（16个工具，覆盖西班牙和葡萄牙的三个环境指标）进行评估，并集成 BigEarthNet V2 土地覆盖数据扩展了葡萄牙的评估能力。

**Result:** 评估了7个 LLM（Claude Sonnet 4, DeepSeek V3.2, GLM-5, Gemini 2.5 Pro, Qwen3-235B, GPT-OSS-120B, Llama 4 Scout），在 temperature=1.0 的三个种子下报告准确率和单案例成本：Claude Sonnet 4 以60.8%±0.8%领先，DeepSeek V3.2 以56.3%±3.1%紧随其后，其余模型均低于51%；成本-准确率帕累托前沿主要由开源模型占据，DeepSeek V3.2 以11倍更低成本（$0.011/案例）达到 Claude 93% 的能力；比较任务在数值相近时完全无法解决（0%），结构化工具调用准确率比通用 GIS 基准低25-35个百分点。

**Conclusion:** 该基准表明结构化工具调用真实 API 比通用 GIS 基准更具区分度（准确率差距达25-35个百分点），揭示了当前 LLM 在环境分析任务中的系统性局限（尤其是比较推理）；开源模型在成本-性能权衡上占据优势，为未来研究提供了标准化评估框架和可扩展的公开资源。

---

#### 📄 Abstract
Environmental scientists spend disproportionate effort on data wrangling rather than analysis, and AI agents that automate geospatial workflows remain unvalidated: no benchmark evaluates agents operating through structured tool calling against real APIs. We introduce the GeoNatureAgent Benchmark, the first benchmark for environmental analysis agents that operate via structured tool calls to a production-style geospatial API. It comprises 93 tasks across 18 categories, covering municipality analysis, multi-turn conversation, spatial reasoning, cross-indicator synthesis, error handling and recovery, ranking, comparison, multilingual understanding, habitat analysis, and task rejection. Tasks are evaluated against an open, self-hostable API serving three environmental indicators across Spain and Portugal via sixteen tools. We evaluate seven LLMs (Claude Sonnet 4, DeepSeek V3.2, GLM-5, Gemini 2.5 Pro, Qwen3-235B, GPT-OSS-120B, Llama 4 Scout) under three temperature-1.0 seeds, reporting capability and per-case cost as orthogonal axes. We find: (1) Claude Sonnet 4 leads at 60.8% +/- 0.8%, followed by DeepSeek V3.2 at 56.3% +/- 3.1%, with no other model above 51%; (2) the cost-accuracy Pareto frontier is occupied mostly by open-weight models, with DeepSeek V3.2 offering 93% of Claude's capability at 11x lower cost ($0.011/case); (3) comparison tasks remain universally unsolved (0% on close-value comparisons), exposing systematic reasoning limits; and (4) structured tool calling against a real API is more discriminative than general-purpose GIS benchmarks, with accuracies 25-35 points lower. We further show extensibility by integrating BigEarthNet V2 land cover for Portugal alongside Spanish CO2 and erosion indicators. The benchmark, harness, and self-hostable API are publicly available.


### [55] [Topical Phase Transitions in Artificial Intelligence Research: Large-Scale Evidence and an Early-Warning Signature for Emerging Topics](https://arxiv.org/abs/2606.12828)
*Rasul Khanbayov, Hasan Kurban*

#### 🧩 TL;DR
通过对2017-2025年间五大顶级AI会议80,814篇论文的分析，研究发现AI研究主题通过“主题相变”——长期边缘化后在1-3年内爆发式涌现——而非渐进式增长来推进；并提出了一个基于出版动力学准则的早期预警信号，能够以27%的精度和63%的召回率提前识别即将到来的相变。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究对AI领域的发展轨迹多基于定性观察或单一会议分析，缺乏跨会议、大规模、定量化的研究来揭示不同AI研究主题的演变模式，特别是缺乏对主题从边缘到主流这一“相变”过程的系统刻画及其可预测性评估。

**Method:** 收集五大AI会议（ACL、CVPR、ICLR、ICML、NeurIPS）2017-2025年的80,814篇录用主会论文；定义主题相变的概念并基于论文主题分布的时间演化进行刻画；提出一套基于出版动力学（publication dynamics）的四个早期预警准则，使用2017-2021年数据拟合，在2023-2025年的相变上进行样本外评估。

**Result:** 大型语言模型在2025年成为跨会议的支配性主题，扩散模型同样以突增方式崛起，语言模型方法通过视觉-语言模型进入计算机视觉领域，而强化学习则呈现平滑增长；早期预警签名在13.5%基线下达到27%精度和63%召回率；该签名将推理与测试时计算、自主智能体、多模态LLM、检索增强生成和世界模型标记为2026-2028年需监测的主题。

**Conclusion:** AI研究主题的发展并非渐进式，而是经历可检测的相变过程，这一发现为理解AI学科演化提供了新视角；基于出版动力学的早期预警签名虽精度有限但能有效提升对相变的识别能力，为科研战略制定和研究方向预测提供了定量工具。

---

#### 📄 Abstract
Do research topics in artificial intelligence grow gradually, or do they advance through abrupt, detectable jumps? Analyzing 80,814 accepted main-track papers from five premier AI conferences (ACL, CVPR, ICLR, ICML, NeurIPS) spanning 2017 to 2025, we show major AI topics advance through topical phase transitions: remaining marginal for years, then surging across venues within one to three years. Large language models became the dominant cross-venue topic by 2025, diffusion models rose with comparable abruptness, and language-model methods crossed into computer vision via vision-language models, whereas reinforcement learning compounded smoothly, distinguishing genuine phase transitions from ordinary growth. This structure is our primary contribution: a large-scale, cross-venue characterization of how AI research reorganizes. We then ask whether a transition leaves a detectable footprint before it peaks. We define an early-warning signature, four publication-dynamics criteria frozen on 2017-2021 data, and evaluate it out of sample on 2023-2025 transitions, obtaining a precision of 27% and recall of 63% against a 13.5% base rate. Applied to 2025 data, the signature flags reasoning and test-time compute, agentic AI, multimodal LLMs, retrieval-augmented generation, and world models as topics to monitor over 2026-2028. The source code is also publicly available on GitHub at https://github.com/KurbanIntelligenceLab/ai-phase-transitions.


### [56] [(Human) Attention Is (Still) All You Need: Human oversight makes AI-assisted social science reliable](https://arxiv.org/abs/2606.12848)
*Chen Zhu, Xiaolu Wang, Weilong Zhang*

#### 🧩 TL;DR
本文提出了人类参与的经济研究（HLER）框架，通过预承诺、决策序列化、问责制和注意力分配三种架构性约束，将多智能体LLM系统的关键失败率从72%降至16%，证明AI辅助研究的可靠性不仅取决于模型能力，更取决于人机认知劳动的结构化设计。

---

#### 📘 Detailed Summary
**Motivation:** LLM正越来越多地被用于假设生成、规范选择和结论草拟等传统上由训练有素的研究人员完成的任务，但AI辅助研究的可靠性不仅取决于模型能力，还取决于人机之间的认知劳动如何结构化，现有方法缺乏对这种分工架构的系统性研究。

**Method:** 研究通过人类参与的经济研究（HLER）框架实现，该框架基于预承诺、决策序列化、问责制和注意力分配四项原则，具体施加三项架构约束：LLM负责推理但不执行数据工作，数据和估计以确定性方式处理，三个关键人类决策门控点绑定整个工作流。实验采用预注册的2×4因子设计，在四个数据集上进行280次完整研究运行，并进行了80次消融实验。

**Result:** 无约束的多智能体基线在280次运行中产生72%的关键失败率，而使用相同底层模型、相同智能体分解和相同提示词的HLER将失败率降低至16%，Fisher精确检验拒绝失败率相等假设（p<0.001）。可靠性提升幅度最大的数据集是清代人口登记册这一公开代表性最低的数据集，消融实验表明确定性计算和人类门控独立贡献且存在互补性证据。

**Conclusion:** HLER应被理解为一个研究约束框架（research harness）而非自主AI科学家，它显著降低失败率、使残余弱点更可见，并防止不可靠的结论被作为可发表的输出提出，凸显了从系统层面设计人机分工架构对AI辅助研究可靠性的关键意义。

---

#### 📄 Abstract
Large language models (LLMs) are increasingly used for tasks once reserved for trained researchers, including hypothesis generation, specification choice, and drafting conclusions. We argue that the reliability of AI-assisted research depends not only on model capability, but also on how cognitive labour is structured between humans and machines. We study this problem through Human-in-the-Loop Economic Research (HLER), a decision architecture based on pre-commitment, decision sequencing, accountability, and attention allocation. In a pre-specified 2*4 factorial experiment with 280 complete research runs across four datasets, an unconstrained multi-agent baseline produced critical failures in 72% of runs. Using the same underlying model, the same agent decomposition, and identical prompts for the shared reasoning agents, HLER reduced the failure rate to 16% by imposing three architectural commitments: LLMs reason but do not execute data work, data and estimation are handled deterministically, and three human decision gates bind the workflow. Fisher's exact test rejects equality of failure rates at p<0.001. Reliability gains were largest on the least publicly represented dataset, a Qing-dynasty population register, consistent with a task-based production model with Frechet-distributed output quality. An 80-run ablation suggests that deterministic computation and human gates contribute independently, with exploratory evidence of complementarity. We interpret HLER as a research harness rather than an autonomous AI scientist: it sharply reduces failures, makes residual weaknesses more visible, and prevents unreliable claims from being advanced as publication-ready outputs.


### [57] [HarnessBridge: Learnable Bidirectional Controller for LLM Agent Harness](https://arxiv.org/abs/2606.12882)
*Xiaoxuan Wang, Haixin Wang, Alexander Taylor, Jason Cong, Yizhou Sun, Wei Wang*

#### 🧩 TL;DR
HarnessBridge是一种轻量级可学习的智能体-环境接口控制器，通过双向投影参数化观测和动作空间，在终端任务和软件工程基准上匹配或超越专用人工设计框架，同时显著降低token开销和轨迹长度。

---

#### 📘 Detailed Summary
**Motivation:** 现有大语言模型智能体的性能不仅取决于模型能力和环境设计，还依赖于连接智能体与环境的"harness"（框架）。当前这些框架主要依赖人工设计，难以随着轨迹长度增长和交互复杂性增加而有效扩展，亟需一种可学习、可端到端训练的替代方案。

**Method:** HarnessBridge是一个轻量级可学习的框架控制器，将智能体-环境接口参数化为双向投影。它学习两种投影：观测投影将原始轨迹蒸馏为紧凑的决策相关状态，动作投影将提议的动作转换为可执行的动作或基于轨迹的拒绝。该模型通过统一指令微调在框架监督数据集上进行训练。

**Result:** 在Terminal-Bench 2.0和SWE-bench Verified基准上，HarnessBridge匹配甚至超越了强专用人工设计框架，同时大幅减少了token使用量和轨迹长度，并且能够从较小的生成器泛化到更大的商业模型。

**Conclusion:** HarnessBridge证明了可学习框架控制器的有效性，表明智能体-环境接口可以通过端到端训练的方式自动学习，从而替代人工设计的框架。该方法在提升性能的同时显著降低计算开销，并展现出良好的跨模型泛化能力，为大规模智能体部署提供了更可扩展的接口解决方案。

---

#### 📄 Abstract
Large language models are increasingly deployed as agents for long-horizon tasks, yet their performance is shaped not only by model capability and environment design, but also by the harness that mediates agent--environment interaction. Existing harnesses are largely manually engineered, making them difficult to scale as trajectories grow longer and interactions become more complex. In this work, we ask whether harness can be generated by a learnable plug-in module that can be trained in an end-to-end fashion. We introduce HarnessBridge, a lightweight learnable harness controller that parameterizes the agent--environment interface as a bidirectional projection. HarnessBridge learns two bidirectional projections: observation projection, which distills raw trajectories into compact, decision-relevant states, and action projection, which converts proposed actions into executable transitions or trajectory-grounded rejections. We train HarnessBridge on a harness supervision dataset via unified instruction tuning. On Terminal-Bench~2.0 and SWE-bench Verified, HarnessBridge matches or surpasses strong specialized harnesses while substantially reducing token usage and trajectory length, and generalizes from smaller generators to larger commercial models.


### [58] [Zero-source LLM Hallucination Detection with Human-like Criteria Probing](https://arxiv.org/abs/2606.12900)
*Jiahao Yang, Shuhai Zhang, Hailong Kang, Feng Liu, Qi Chen, Mingkui Tan*

#### 🧩 TL;DR
提出HCPD框架，通过模拟人类多维度推理的可解释准则探测机制，结合奖励对齐与多采样聚合策略，在仅依赖查询-答案对的零源约束下实现高效且可解释的幻觉检测。

---

#### 📘 Detailed Summary
**Motivation:** 大语言模型（LLM）生成的幻觉内容（事实错误或不忠实内容）严重威胁其安全使用。在零源约束下（无模型内部状态或外部参考可用），仅凭文本查询-答案对进行幻觉检测极具挑战性，现有方法缺乏可解释的多维推理能力。

**Method:** 提出Human-like Criteria Probing for Hallucination Detection (HCPD)框架，其核心是Human-like Criteria Probing (HCP)机制：LLM智能体自适应地将判决分解为一组带权重的可解释准则，并聚合各准则分数得到最终真实性度量。通过基于语义一致性的弱监督奖励对齐实现自适应能力，推理时采用多采样聚合策略确保鲁棒性并保持完全可解释性。还提供了理论分析支持方法可靠性。

**Result:** 大量实验表明，HCPD在零源幻觉检测任务上持续优于现有最先进基线方法，在性能与可解释性上均取得显著优势。代码已开源在GitHub上。

**Conclusion:** HCPD通过模拟人类评估者的多面推理、引入可解释准则探测与弱监督对齐机制，为零源约束下的幻觉检测提供了一种高效、可解释且理论基础扎实的解决方案，为LLM安全使用提供了重要保障。

---

#### 📄 Abstract
Large language models (LLMs) often hallucinate by generating factually incorrect or unfaithful content, posing significant risks to their safe use. Detecting such hallucinations is particularly challenging under the zero-source constraint, where no model internals or external references are available, and detection must rely solely on the textual query-answer pair. In this paper, we propose Human-like Criteria Probing for Hallucination Detection (HCPD), a paradigm that emulates the multi-faceted reasoning of human evaluators. Its core is a Human-like Criteria Probing (HCP) mechanism, in which a LLM agent adaptively decomposes its judgment into a weighted set of interpretable criteria and aggregates criterion-specific scores into a final truthfulness measure. To achieve this adaptive capability, we introduce a reward-based alignment scheme using only weak supervision from semantic consistency. At inference, we employ a multi-sampling aggregation strategy to ensure robust decisions while preserving full interpretability. We further provide theoretical analysis supporting the reliability of our approach. Extensive experiments show that HCPD consistently outperforms state-of-the-art baselines, offering an effective and explainable solution for zero-source hallucination detection. Code is available at https://github.com/TRISKEL10N/HCPD.


### [59] [MDForge: Agentic Molecular Dynamics Pipeline Design under Sparse Simulator Feedback](https://arxiv.org/abs/2606.12916)
*Zehong Wang, Yijun Ma, Connor R. Schmidt, Tianyi Ma, Weixiang Sun, Ziming Li, Xiaoguang Guo, Chuxu Zhang, Matthew J. Webber, Yanfang Ye*

#### 🧩 TL;DR
提出MDForge，一个基于LLM agent的框架，通过开放式代码生成和多智能体辩论将稀疏的专家口头奖励转化为稠密信号，自动为分子动力学流水线设计生成竞争性方案，并成功发现了一种新型高亲和力picomolar级别CB[7]结合物。

---

#### 📘 Detailed Summary
**Motivation:** 分子动力学（MD）作为原子级分子科学的关键计算方法，其流水线设计依赖大量专家知识，且针对单个分子运行成本高昂，无法通过试错方式优化，因此亟需一种自动化方法来替代人工专家完成MD流水线的设计。

**Method:** MDForge将MD流水线设计建模为开放式代码生成问题，LLM agent通过在线口头奖励（verbal reward）重塑其行为；具体采用多智能体辩论（multi-agent debate）机制将稀疏奖励稠密化，物理学专家角色通过辩论生成密集反馈信号以指导agent的在线更新。

**Result:** 在三个SAMPL主-客体结合自由能基准上，MDForge自动设计的MD流水线性能与人类专家相当；将其CB[7]流水线部署到未见过的候选客体库中，发现了一个新型结合物，竞争性湿实验NMR确认其为高亲和力、picomolar级别的CB[7]结合物。

**Conclusion:** 该工作表明LLM agent能够通过开放式代码生成和奖励稠密化技术自动完成复杂科学流水线的端到端设计，其性能可达专家水平，且能够发现新型高亲和力分子结合物，为自动化计算化学研究提供了可行范式。

---

#### 📄 Abstract
Molecular dynamics (MD) is the canonical in-silico method for atomistic molecular science, simulating molecular behavior from first-principle physics. Designing an MD pipeline for a new system requires substantial expert knowledge: running it on even one molecule is expensive, ruling out trial-and-error. We automate this expert pipeline-design process with an LLM agent. Unlike existing MD agents that orchestrate a predefined tool set, we treat pipeline design as open-ended code generation in which the agent's behavior is reshaped online by verbal reward. Specifically, we build MDForge, an LLM agent whose in-context update rule densifies the sparse reward via a multi-agent debate among physics experts. On three SAMPL host-guest binding free-energy benchmarks, MDForge automatically designs MD pipelines competitive with human experts. Deployed on a library of unseen candidate guests, its CB[7] pipeline discovers a novel binder that wet-lab competition NMR confirms is a high-affinity, picomolar CB[7] binder. Our data and code are available at https://github.com/Zehong-Wang/MDForge.


### [60] [PRISMR: Overcoming Parse Collapse in Multimodal Listwise Ranking via Parameterized Representation Internalization](https://arxiv.org/abs/2606.12942)
*Hao Jiang, Xin Li, Annan Wang, Zhi Yang, Haoxiang Zhang, Yichi Zhang, Weisi Lin*

#### 🧩 TL;DR
提出PRISMR框架，通过参数化结构条件替代上下文列表处理，利用轻量级超网络生成实例级LoRA权重以缓解多模态长序列排序中的解析崩塌问题，显著提升列表排序性能并支持跨领域迁移。

---

#### 📘 Detailed Summary
**Motivation:** 基于大型多模态模型（LMM）的生成式列表排序在长上下文场景下性能严重退化，其根源在于自回归解码器会出现"解析崩塌"——在生成流畅文本的同时静默跳过候选并提前终止，导致排序不完整；该问题源于有限的上下文利用率而非格式错误，使得提示工程和约束解码等传统手段难以奏效。

**Method:** PRISMR框架采用参数化结构条件替代瞬态上下文列表处理，具体包括：使用轻量级超网络并行编码多模态候选并生成每个项目特定的LoRA权重，再将这些权重合成为实例适配器注入LMM中，从而实现列表结构的鲁棒内化并保持基座模型不变。

**Result:** 实验表明，PRISMR大幅减少了解析崩塌现象，显著提升了列表排序性能，并且能够有效跨领域迁移以及在经过指令微调的骨干网络上泛化；此外，论文还构建了一个大规模多模态评论排序基准用于评估。

**Conclusion:** 解析崩塌是长上下文多模态排序中一个关键且被忽视的失败模式，PRISMR通过参数化结构内化机制从根本上缓解了该问题，为多模态列表排序提供了一种不依赖提示工程的鲁棒范式，并展示了良好的跨领域与跨骨干网络迁移能力。

---

#### 📄 Abstract
Generative listwise ranking with Large Multimodal Models (LMMs) aims to capture global list context in a single forward pass, but
  its effectiveness degrades in long-context multimodal scenarios. We identify a recurring failure mode, parse collapse, where the
  autoregressive decoder produces fluent yet incomplete rankings by silently omitting candidates and terminating early. This
  failure stems from limited context utilization rather than simple formatting mistakes, making prompt engineering and constrained
  decoding insufficient. We propose PRISMR (Parameterized Representation Internalization for Semantic Multimodal Ranking), a
  framework that replaces transient in-context list processing with parametric structural conditioning. PRISMR uses a lightweight
  hypernetwork to encode multimodal candidates in parallel and generate item-specific LoRA weights, which are synthesized into an
  instance-specific adapter for a LMM. This paradigm enables more robust internalization of list structure while preserving the
  base model. We further introduce a large-scale multimodal review-ranking benchmark for evaluation. Experiments demonstrate that
  PRISMR substantially reduces parse collapse, improves listwise ranking performance, and transfers effectively across domains and
  instruction-tuned backbones.


### [61] [Learning What to Remember: A Cognitively Grounded Multi-Factor Value Model for Agentic Memory](https://arxiv.org/abs/2606.12945)
*Zhibao Chen, Qian Cheng*

#### 🧩 TL;DR
提出一种多因子记忆价值函数V(m)=∑w_i f_i(m)，通过七个可解释因子（情绪强度、目标相关性、价值对齐、自我/用户相关性、任务效用、可靠性和使用历史）的加权组合来统一控制LLM智能体的编码深度、遗忘风险和检索排序，权重通过无梯度优化器从下游目标中学习，在盲遗忘设定下显著优于基于语义相似度或新近度的基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 长期运行的LLM智能体产生的交互历史远超任何上下文窗口，需要决定哪些信息深度编码、哪些遗忘、哪些在固定记忆预算下检索。现有生产系统依赖语义相似度或新近度来做出遗忘决策，但遗忘决策在合并时（consolidation time）就已做出，而未来查询未知，因此这两种策略都是错误指定的。

**Method:** 提出了一个多因子记忆价值函数V(m)=∑w_i f_i(m)，包含七个来自认知心理学的可解释因子（情绪强度、目标相关性、价值对齐、自我/用户相关性、任务效用、可靠性和使用历史），权重通过无梯度优化器（gradient-free optimiser）从下游目标中学习，该函数输出的单一标量统一控制编码深度、遗忘风险和检索排名。在LongMemEval上进行了盲遗忘（blind regime）评估，并与均匀权重、单因子最优、新近度等基线对比。

**Result:** 在LongMemEval的盲遗忘设定下（479个可用案例），学习的多因子价值函数保留0.770±0.011的金标准证据，而均匀权重为0.657，最优单因子为0.518，新近度为0.368；所有配对差距的95%自助法置信区间均大于零，且使用相同因子的神经网络与线性模型性能相当。学习到的权重具有可解释性：可靠性、情绪强度和自我/用户相关性占主导，而查询时的目标相似度被正确降权。在含人为混淆的受控合成任务中，学习器恢复了区分性权重（1.00保留率），而均匀权重仅0.62。

**Conclusion:** 论文做出了一个方法论上的关键观点：在LongMemEval上使用与留出评估问题直接匹配的目标相关性评分来评估，会将金标准证据保留率饱和在约0.98——这实际上测量的是检索能力而非遗忘能力。实验结果表明，多因子学习的线性模型在遗忘决策上显著优于现有方法，且权重可解释，整个框架仅依赖单CPU运行，无需API调用，开源可复现。

---

#### 📄 Abstract
Long-running LLM agents accumulate interaction histories far larger than any context window, forcing a standing decision: what to encode deeply, what to forget, and what to retrieve under a fixed memory budget. Production systems answer with semantic similarity or recency -- both mis-specified for the forgetting decision, which is made at consolidation time before the future query is known. We propose a multi-factor memory value function V(m)=\sum_i w_i f_i(m) over seven interpretable factors (emotional intensity, goal relevance, value alignment, self/user relevance, task utility, reliability, and usage history) drawn from cognitive psychology, whose weights are learned from a downstream objective by a gradient-free optimiser, and whose single scalar uniformly controls encoding depth, forget risk, and retrieval rank. We make a methodological point: on LongMemEval, scoring goal relevance against the held-out evaluation question saturates gold-evidence retention at \approx 0.98 -- this measures retrieval, not forgetting. In the realistic blind regime, a learned multi-factor value retains 0.770 \pm 0.011 of gold evidence across 479 usable cases, versus 0.657 for uniform weights, 0.518 for the best single factor, and 0.368 for recency; every paired gap's 95% bootstrap CI is above zero, and a neural network over the same factors ties the linear model. The learned weights are interpretable -- reliability, emotional intensity, and self/user relevance dominate, while query-time goal similarity is correctly down-weighted for the forgetting decision. A controlled synthetic task with planted confounds confirms the learner recovers a separating weighting (1.00 retention) where uniform weighting fails (0.62). The substrate is open-source; all experiments run on a single CPU with no API calls.


### [62] [OpenMedQ: Broad Open Pretraining for Medical Vision-Language Models](https://arxiv.org/abs/2606.12953)
*Ibrahim Gulluk, Max Van Puyvelde, Olivier Gevaert*

#### 🧩 TL;DR
OpenMedQ 是一个基于 14 个公开医学数据集（约 335 万样本）预训练的医学视觉-语言模型，在 PathVQA 上以仅 562B 参数规模 1/80 的模型量达到 SOTA BLEU-1（75.9），并且在 8 个未见医学分类任务上以最高平均 macro-F1（0.757）超越现有开源医学 CLIP 模型，为社区提供了完全开放的可复现基线。

---

#### 📘 Detailed Summary
**Motivation:** 现有医学视觉-语言模型（如 Med-PaLM M）虽性能强大但模型规模巨大（高达 562B 参数），且部分模型并非完全开放，限制了社区的可复现性和广泛适用性；此外，缺乏在统一、广泛且公开的医学多模态数据上进行预训练的强基线模型。

**Method:** OpenMedQ 使用 14 个公开数据集的约 335 万预训练样本（涵盖病理学、放射学、显微图像和纯文本临床问答）进行视觉-语言预训练；其视觉编码器随后被迁移到 8 个未见过的医学分类基准上，在完全相同的下游微调方案下评估性能。

**Result:** 在 PathVQA 上达到 SOTA BLEU-1 75.9，超越参数规模大 80 倍的 Med-PaLM M 变体；在 VQA-MED 上取得与最佳报告持平的 BLEU-1 64.5；在 8 个未见医学分类任务上，视觉编码器的平均 macro-F1 为 0.757，优于 BiomedCLIP（0.745）、PMC-CLIP（0.745）、PubMedCLIP（0.746）和从头训练基线（0.616）。

**Conclusion:** OpenMedQ 证明了在广泛且完全公开的医学多模态数据上进行预训练可以显著降低模型规模的同时达到或超越超大模型的性能，为医学视觉-语言模型研究提供了一个可复现的强基线，并推动社区向开放科学方向发展。

---

#### 📄 Abstract
We present OpenMedQ, a medical vision-language model pretrained on the broadest fully-open medical mix to date: 14 datasets totaling ~3.35M pretraining samples spanning pathology, radiology, microscopy, and text-only clinical QA. OpenMedQ reaches state-of-the-art BLEU-1 on PathVQA (75.9), beating Med-PaLM M variants up to 562B parameters (~80x larger), and matches the best reported VQA-MED BLEU-1 (64.5). Its vision encoder, transferred to 8 unseen medical classification benchmarks under an identical downstream recipe, obtains the highest average macro-F1 (0.757) among BiomedCLIP (0.745), PMC-CLIP (0.745), PubMedCLIP (0.746), and a from-scratch baseline (0.616). We release our code and an interactive demo is publicly available as a reproducible baseline for the community.


### [63] [Multi-Modal Agents for Power Distribution Defect Detection: An Evaluation of Foundation Models](https://arxiv.org/abs/2606.12969)
*Quan Quan*

#### 🧩 TL;DR
提出了一种面向配电网缺陷检测的多模态智能体框架，系统评估了多模态基础模型在感知、推理和工具使用三个关键能力上的综合表现，并构建了专用评估数据集和基准，揭示了现有模型在工业高风险场景中的优势与局限。

---

#### 📘 Detailed Summary
**Motivation:** 传统配电网巡检方法在语义理解、泛化能力和闭环自动化方面存在明显不足，难以满足高可靠电力输送对智能缺陷检测的严苛需求，亟需一种能够融合多模态信息并实现自主决策与操作的智能框架。

**Method:** 提出了一种多模态智能体框架，将多模态基础模型作为统一认知引擎，系统评估其在三个关键维度的集成性能：感知（识别设备并生成专家级缺陷描述）、推理（基于领域知识诊断原因、评估严重性并规划维护策略）、工具使用（自主执行知识库查询、生成工单等操作以实现闭环维护），同时构建了领域专用评估数据集和综合基准。

**Result:** 实验结果表明，当前多模态基础模型在感知、推理和工具使用三个维度上展现出各自的优势与不足，基于专用评估数据集和基准的量化评测为自主智能体在高风险工业环境中的部署提供了实证依据。

**Conclusion:** 该研究通过系统性的多维度评估揭示了现有基础模型在工业缺陷检测场景下的能力边界，为未来在电力等高可靠性需求领域中部署自主智能体提供了经验证据和评估范式。

---

#### 📄 Abstract
The power distribution network is critical to reliable electricity delivery, yet traditional inspection methods face limitations in semantic understanding, generalization, and closed-loop automation. To address these challenges, this paper proposes a Multi-Modal Agent framework specifically for power distribution defect detection. Central to this study is the systematic evaluation of multimodal foundation models as unified cognitive engines. We rigorously assess their integrated performance across three critical capabilities: (1) Perception, where the model must accurately identify equipment and generate expert-level descriptions of defects; (2) Reasoning, where the model interprets visual findings to diagnose causes, assess severity, and plan maintenance strategies based on domain knowledge; and (3) Tool Usage, where the model acts as an autonomous operator to execute actions -- such as querying knowledge bases or generating work orders -- to achieve closed-loop maintenance. To support this evaluation, a domain-specific evaluation dataset and a comprehensive benchmark are developed. Experimental results demonstrate the strengths and limitations of current foundation models in these three dimensions, providing empirical evidence for deploying autonomous agents in high-stakes industrial environments.


### [64] [The Illusion of Multi-Agent Advantage](https://arxiv.org/abs/2606.13003)
*Prathyusha Jwalapuram, Hehai Lin, Chuyuan Li, Fangkai Jiao, Sudong Wang, Yifei Ming, Zixuan Ke, Chengwei Qin, Giuseppe Carenini, Shafiq Joty*

#### 🧩 TL;DR
本文通过系统对比自动生成的多智能体系统（MAS）与单智能体系统（SAS）中的Chain-of-Thought with Self-Consistency（CoT-SC），发现自动MAS在传统推理和交互式多步任务中均持续落后于CoT-SC，即便成本高出10倍；进一步引入诊断性合成数据集揭示了现有评估框架掩盖了MAS的架构缺陷与低效问题，当前自动设计范式产生的架构臃肿与功能效用严重脱节。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究普遍认为多智能体系统（MAS）优于单智能体系统（SAS），但这一论断主要基于在偏向孤立推理任务的基准上与SAS基线进行的比较，未能充分评估MAS在上下文保护、并行处理和分布式决策等方面的理论优势；同时，自动生成的MAS虽声称具有更好的泛化性，但其真实性能缺乏系统的严格评估。

**Method:** 作者对自动生成的多智能体系统（MAS）与单智能体系统（SAS）中的Chain-of-Thought with Self-Consistency（CoT-SC）进行了严格的系统评估，涵盖传统推理数据集和交互式多步工作流任务（如BrowseComp-Plus）；为隔离任务结构带来的局限性，作者还构建了一个专门针对MAS的合成诊断数据集，该数据集具有显式任务分解、上下文分离和并行化潜力，并在其上对比了专家设计的MAS与自动生成的MAS。

**Result:** 在传统推理和交互式多步任务中，自动生成的MAS持续不如CoT-SC，尽管成本高出10倍；在专为MAS设计的诊断数据集上，专家设计的MAS在原始性能和成本效率两方面均一致优于自动生成的MAS；进一步系统解构发现，当前自动设计范式产生的架构臃肿是表面复杂性的堆积，未能转化为实际功能效用。

**Conclusion:** 现有评估框架未能考虑计算成本增加的边际效用，从而掩盖了复杂MAS的关键架构缺陷和低效问题；当前自动MAS设计范式存在与多智能体基本原则的根本性错位，产生的架构复杂性与功能效用严重脱节，亟需重新审视MAS的自动生成与评估方法。

---

#### 📄 Abstract
Prevailing wisdom posits that Multi-Agent Systems (MAS) are superior to Single-Agent Systems (SAS), citing advantages like context protection, parallel processing and distributed decision-making. However, empirical support for this claim relies primarily on comparisons with SAS baselines using benchmarks that prioritize isolated reasoning tasks, which do not adequately assess these advantages. Focusing on automatically generated MAS that are designed for enhanced generalizability over manually-designed counterparts, we perform a rigorous, systematic evaluation against SAS, specifically Chain-of-Thought with Self-Consistency (CoT-SC). Across traditional reasoning datasets and tasks with interactive multi-step workflows (e.g., BrowseComp-Plus), we demonstrate that automatic MAS consistently underperform CoT-SC despite being up to 10x more expensive. To isolate these failures from limitations inherent to task structure, we introduce a diagnostic synthetic dataset tailored for MAS featuring explicit task decomposition, context separation and parallelization potential. We show that expert-architected MAS consistently outperforms automatically generated architectures in both raw performance and cost-efficiency on this dataset, demonstrating that existing evaluation frameworks mask critical architectural gaps and inefficiencies of complex MAS by failing to account for the marginal utility of increased computational cost. Critically, systematic deconstruction of the generated MAS architectures reveals that current automated design paradigms produce architectural bloat that prioritizes superficial complexity which does not translate into functional utility, exposing a fundamental misalignment with multi-agent principles.


### [65] [Nous: An Attempt to Extract and Inject the Cognition Behind Prediction-Market Behavior](https://arxiv.org/abs/2606.13038)
*Haowei Qian*

#### 🧩 TL;DR
Nous通过从Polymarket交易行为中提取八维行为画像并注入LLM智能体提示词，发现行为提取部分可行（部分参数时间稳定、钱包可识别），但提示层注入无法有效传递认知多样性，无法降低集成误差相关性或提升Brier分数，表明提示级干预不足以解决LLM智能体的认知单一文化问题。

---

#### 📘 Detailed Summary
**Motivation:** 基于共享基础模型的LLM智能体在预测市场和集体决策中产生相关预测（前沿模型误差相关性r~0.77），导致认知单一文化问题，本文探究是否能从人类行为中恢复认知多样性并将其转移给LLM智能体。

**Method:** 提出Nous框架，从Polymarket真实交易活动中提取八维行为画像（含14个参数），通过提示词注入LLM智能体；采用裂半组内相关系数（ICC）评估参数时间稳定性、Top-1检索评估钱包可识别性、语义嵌入度量评估注入效果，并通过集成误差相关性和Brier分数评估多样性传输效果。

**Result:** 8/14个参数时间稳定（裂半ICC≥0.5，bootstrap下限>0.3，反向评分ICC~0.9）；钱包可识别性远高于随机（Top-1检索17-22% vs 1%）；结构化注入在语义嵌入度量上无显著优于长度匹配对照，且未降低集成误差相关性或改善Brier分数；结构到叙述的翻译器发出近均匀的提示词，其散布不跟踪画像散布。

**Conclusion:** 行为提取部分可行但提示层注入无法有效传输认知多样性，认知单一文化问题需要更深层的干预（微调、激活引导），提示级干预存在根本性局限。

---

#### 📄 Abstract
As LLM agents proliferate in prediction markets and collective decision-making, they risk a cognitive monoculture: agents built on shared foundation models produce correlated forecasts, and recent measurement finds frontier-model errors correlated at r ~ 0.77. We ask whether human cognitive diversity can be recovered from behavior and transferred to LLM agents. Nous extracts a structured eight-dimension behavioral profile from real Polymarket trading activity and injects it into agents through prompts. Our central finding is a dissociation between the two halves of that pipeline. Extraction works, partially: across 100 wallets, 8 of 14 parameters are temporally stable (split-half ICC >= 0.5, bootstrap CI lower bound > 0.3; contrarian score reaches ICC ~ 0.9); wallets are identifiable from their profiles well above chance (top-1 retrieval 17-22% vs. 1% chance); and two of four pre-specified dimensions rank-correlate with future realized profit out-of-sample, though the correlations do not survive behavioral-confound controls. Prompt-level injection does not measurably transmit it: on a semantic embedding metric, structured injection shows no significant advantage over a length-matched control on any model, and the diversity it induces neither reduces ensemble error correlation nor improves Brier score -- a null that persists across exploratory checks on sampling temperature, profile diversity, and question difficulty. Measuring the prompts themselves locates the compression before the model: the structure-to-narrative translator emits near-uniform prompts whose spread does not track profile spread. We position Nous as measuring the cognitive-monoculture problem and the limits of a prompt-level remedy, motivating deeper, below-the-prompt injection (fine-tuning, activation steering). Code, frozen profiles, prompts, and model outputs: https://github.com/WillChienT/nous-paper


### [66] [Rethinking RAG in Long Videos: What to Retrieve and How to Use It?](https://arxiv.org/abs/2606.13141)
*Yuho Lee, Jisu Shin, Nicole Hee-Yeon Kim, Jihwan Bang, Juntae Lee, Kyuwoong Hwang, Fatih Porikli, Hwanjun Song*

#### 🧩 TL;DR
提出V-RAGBench基准和CARVE方法，解决视频检索增强生成中现有基准可绕过视频回答问题及查询级单配置忽视块级可变性的两大缺陷，CARVE通过并行检索器和块自适应重排序实现块级多配置交织，在多项基准上超越八种最新方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有VideoRAG基准存在两个关键缺陷：一是允许仅凭文本即可回答问题，无需依赖视频内容，从而掩盖了检索环节的错误；二是先前方法对每个查询仅采用单一模态-粒度配置，忽视了不同视频块在模态和粒度需求上的可变性，导致检索与生成阶段的配置无法灵活适应块级差异。

**Method:** 提出V-RAGBench基准，由⟨查询, 证据块, 答案⟩三元组构成，支持检索与生成的忠实解耦评估；提出CARVE方法，通过并行运行多个不同模态-粒度配置的检索器，并采用块自适应重排序机制为每个视频块选择最优配置，随后各块以其获胜配置进入生成器，形成块级决策在检索与生成两阶段间传播的交错证据形式。

**Result:** CARVE在V-RAGBench基准上超越八种近期VideoRAG基线方法，其关键发现是生成器接收的块来自多个不同配置的交错组合，而非单一共享配置，这种块级多配置交织行为是查询级方法无法实现的，从而显著提升了检索与生成的整体性能。

**Conclusion:** 该研究表明视频RAG中块级配置自适应相较于查询级固定配置具有显著优势，V-RAGBench为忠实评估视频检索与生成提供了可靠基准，而CARVE证明了并行多配置检索与块自适应重排序的有效性，为未来视频RAG研究提供了新的评估框架与方法范式。

---

#### 📄 Abstract
Retrieval-augmented generation is moving beyond text into long, egocentric video, where systems must select query-relevant chunks across multiple modalities and temporal granularities. Yet progress in VideoRAG is limited by two gaps: existing benchmarks allow queries to be answered without the video, obscuring retrieval errors, and prior methods apply a single modality-granularity configuration per query, ignoring chunk-level variability. We address both by introducing V-RAGBench, a benchmark of $\langle$query, evidence chunk, answer$\rangle$ triplets that enables faithful, decoupled evaluation of retrieval and generation, and CARVE, a simple method that runs parallel retrievers across configurations and employs chunk-adaptive reranking to identify the winning configuration for each chunk. Each chunk then enters the generator under its winning configuration selected during retrieval, yielding an interleaved evidence form where the chunk-level decision propagates across both stages. CARVE outperforms eight recent VideoRAG baselines, with the chunks supplied to the generator interleaving multiple configurations rather than sharing a single one, a behavior unattainable by query-level methods.


### [67] [TerraBench: Can Agents Reason Over Heterogeneous Earth-System Data?](https://arxiv.org/abs/2606.13148)
*Dat Tien Nguyen, Thao Nguyen, Fadillah Adamsyah Maani, Huy M. Le, Muhammad Umer Sheikh, Numan Saeed, Muhammad Haris Khan, Salman Khan*

#### 🧩 TL;DR
TerraBench 是一个面向地球科学推理的基准测试，它通过 TerraAgent 框架将大语言模型的规划能力与科学工具耦合，实现了对地球观测影像、网格化数据、GIS 推理和模拟的统一可执行接口，包含 403 个智能体任务和 24,500 个已验证的执行步骤，并首次在此领域引入过程级工具使用度量和容差感知数值评分。

---

#### 📘 Detailed Summary
**Motivation:** 现有的气候与环境决策需要跨异构输入（网格化物理数据、卫星图像、地理空间上下文和模拟器输出）进行推理，但气象基础模型虽能预测却无法以语言形式交互推理，大语言模型虽能进行语言推理却无法直接处理高维地球系统数据，导致真实的地球科学工作流缺乏有效的统一解决方案。

**Method:** 论文提出了 TerraBench 基准测试，其核心是 TerraAgent——一个基于 ReAct 风格的可执行框架，通过交织推理、工具调用和观察结果，将大语言模型的规划能力与环境检索、地理空间处理、模拟以及基于人工制品的计算等科学工具相耦合，从而在单一可执行接口中统一了地球观测影像分析、网格化数据处理、GIS 推理和模拟。

**Result:** 该基准包含 403 个广泛的智能体任务，横跨三个轨道（基础、模拟接地和文档接地验证）和八个应用领域，总计 24,500 个已验证的执行步骤，并且是首个在此领域中引入过程级工具使用度量和容差感知数值评分的基准。

**Conclusion:** 实验结果表明，可靠的地球科学智能体不仅需要工具访问能力，还必须能够协调异构工作流、精确参数化工具并维护人工制品来源，这为未来地球科学自主推理系统的设计提供了重要指导。

---

#### 📄 Abstract
Climate and environmental decision-making increasingly requires reasoning across heterogeneous inputs, including gridded physical data, satellite imagery, geospatial context, and simulator outputs. Weather and climate foundation models can forecast well, but do not reason interactively in language, while large language models (LLMs) reason in language but cannot operate directly on high-dimensional Earth-system data. As a result, real scientific workflows in Earth-science remain underserved. We introduce TerraBench, a benchmark for grounded Earth-science reasoning, built on TerraAgent, a ReAct-style executable framework that interleaves reasoning, tool calls, and observations to couple LLM planning with scientific tools for environmental retrieval, geospatial processing, simulation, and artifact-backed computation. TerraBench unifies analysis of Earth observation imagery, gridded data, GIS reasoning and simulation in a single executable interface, whereas prior benchmarks isolate these capabilities into narrow individual tasks. It is also the first in this space to pair process-level tool-use metrics with tolerance-aware numeric scoring. The benchmark comprises 403 extensive agentic tasks across three tracks (Fundamentals, Simulator-Grounded, and Document-Grounded Verification) and eight application domains with 24,500 verified execution steps. These results indicate that reliable Earth-science agents must go beyond tool access to coordinate heterogeneous workflows, parameterize tools precisely, and preserve artifact provenance.


### [68] [Reasoning for Mobile User Experience with Multimodal LLMs: Task, Benchmark, and Approach](https://arxiv.org/abs/2606.13192)
*Ruichao Mao, Zhou Fang, Teng Guo, Hao Yang, Yaping Li, Shaohua Peng, Maji Huang, Xiaoyu Lin, Shuoyang Liu, Xuepeng Li, Yuyu Zhang, Hai Rao*

#### 🧩 TL;DR
提出UXBench基准（2000个VQA样本，8项UI推理任务）评估MLLM的用户体验推理能力，并构建UI-UX模型（基于Qwen3-VL-4B-Thinking，结合奖励路由机制与非对称过渡奖励的强化学习），在UXBench上达到0.7963准确率，超越Claude-4.5-Sonnet（0.6550）。

---

#### 📘 Detailed Summary
**Motivation:** 现有MLLM在GUI视觉元素定位、设计生成等方面进展迅速，但基于UI截图评估用户体验（可用性、感知一致性、功能清晰度）的研究尚不成熟，缺乏针对UI细粒度推理能力的系统化评测基准。

**Method:** 构建UXBench基准（2000个VQA样本，涵盖布局关系、视觉层次、内容一致性8项任务）；提出UI-UX模型，以Qwen3-VL-4B-Thinking为基础，通过强化学习增强，核心创新包括奖励路由机制（动态平衡感知理解与逻辑推理）和非对称过渡奖励（抑制冗余或不足推理步骤）。

**Result:** 主流MLLM在UXBench上表现有限，UI-UX达到0.7963准确率，显著超越Claude-4.5-Sonnet的0.6550，在多样化UI任务上展现强泛化能力且保持低推理延迟。

**Conclusion:** 当前MLLM在UI推理能力上存在根本性局限，UI-UX通过动态推理平衡与步骤优化机制显著提升了性能，表明该方向仍需进一步发展，为未来UI用户体验自动化评估提供了新基准与方法框架。

---

#### 📄 Abstract
User experience (UX) centered on usability, perceived consistency, and functional clarity is fundamental to real-world user interfaces (UI). The application of
  multimodal large language models (MLLMs) in the field of user interfaces is evolving rapidly, such as visual element grounding, graphical user interface (GUI)
  agents, and design-to-code generation. However, research efforts on evaluating UX based on UI screenshots are still immature. To address this, we propose UXBench,
  a novel multimodal benchmark consisting of 2,000 VQA data samples designed to assess MLLMs' ability to perform UI-based reasoning. UXBench includes 8 tasks based
  on real-world UI screenshots that require fine-grained diagnosis of UX issues across layout relationships, visual hierarchy, and content consistency. Our
  extensive evaluation of mainstream MLLMs shows that they remain fundamentally limited in their capacity for UI-based reasoning. The results underscore the need
  for further advancements in this area. To bridge this gap, we propose UI-UX, an MLLM based on Qwen3-VL-4B-Thinking foundation model and enhanced via reinforcement
  learning with two key innovations: a reward routing mechanism that dynamically balances perceptual understanding and logical reasoning during inference, and an
  asymmetric transition reward that suppresses redundant or insufficient reasoning steps. Experiments demonstrate that UI-UX achieves state-of-the-art (SOTA)
  performance on UXBench, attaining an accuracy of 0.7963 -- surpassing Claude-4.5-Sonnet's 0.6550 -- while exhibiting strong generalization across diverse UI tasks
  and maintaining low inference latency.


### [69] [ARMOR-MAD: Adaptive Routing for Heterogeneous Multi-Agent Debate in Large Language Model Reasoning](https://arxiv.org/abs/2606.13197)
*Fuqiang Niu, Bowen Zhang*

#### 🧩 TL;DR
ARMOR-MAD 提出了一种无需额外训练、基于条件计算的异构多智能体辩论框架，通过辩论前路由、早期停止评估和语义异常检测三个组件，在多个推理基准上显著提升了准确率与计算效率。

---

#### 📘 Detailed Summary
**Motivation:** 现有的多智能体辩论方法采用固定管道，不仅浪费计算资源，还可能在相似智能体之间放大相关错误，缺乏根据辩论进程动态调整计算量的机制。

**Method:** ARMOR-MAD 是一个无需训练的异构多智能体辩论框架，将辩论视为条件计算问题，包含三个核心组件：Pre-debate Agreement Routing (PAR) 决定是否需要辩论；Early Agreement Stopping Evaluator (EASE) 在达成一致后提前终止辩论；Semantic Outlier Detection (SOD) 在聚合时降低异常最终答案的权重。

**Result:** 在 MATH Level 5、GSM8K、MMLU 和 MMLU-Pro 四个基准上，ARMOR-MAD 相较于固定轮次异构辩论一致取得提升，分别达到 65.5%、96.5%、90.0% 和 81.5% 的准确率。

**Conclusion:** 结果表明，真正的模型异构性和基于一致性的控制机制对于提高多智能体辩论的准确性和效率都至关重要。

---

#### 📄 Abstract
Multi-agent debate (MAD) can improve large language model reasoning, but fixed debate pipelines often waste computation and can amplify correlated errors among similar agents. We propose ARMOR-MAD, a training-free heterogeneous MAD framework that treats debate as conditional computation. ARMOR-MAD combines three components: Pre-debate Agreement Routing (PAR) decides whether independently generated Round-0 answers require debate; Early Agreement Stopping Evaluator (EASE) stops debate after convergence; and Semantic Outlier Detection (SOD) down-weights abnormal final answers during aggregation. Across MATH Level 5, GSM8K, MMLU, and MMLU-Pro, ARMOR-MAD consistently improves over fixed-round heterogeneous debate with the same model pool, reaching 65.5\%, 96.5\%, 90.0\%, and 81.5\% accuracy, respectively. The results suggest that genuine model heterogeneity and agreement-based control are both important for making MAD more accurate and efficient.


### [70] [Hallucination in Medical Imaging AI: A Cross-Modality Analytical Framework for Taxonomy, Detection, and Mitigation under Regulatory Constraints](https://arxiv.org/abs/2606.13211)
*Omar Alshahrani, Muzammil Behzad*

#### 🧩 TL;DR
本文通过跨模态分析，系统性地研究了医学影像AI系统中的幻觉现象（包括解剖结构虚构、漏诊、侧别错误和测量编造），提出了统一的幻觉分类学，发现通用基础模型在幻觉专用基准上优于医学专用模型，并将幻觉管理映射为FDA全生命周期监管义务而非部署前检查清单。

---

#### 📘 Detailed Summary
**Motivation:** AI系统在医学影像中的部署速度超过了对其失效模式的理解速度，其中临床最关注的失效模式是幻觉——即生成临床上看似合理但事实上错误的输出，包括虚构解剖结构、遗漏发现、错误侧别和编造的测量值，这些错误会直接影响活检决策、分期和治疗规划。现有研究缺乏跨模态的统一分析框架，无法系统性地对幻觉的分类学、病因、检测和缓解策略进行整合。

**Method:** 本文采用结构化叙述综述方法，整合了同行评审研究、基准数据集和FDA监管指南，覆盖五种成像模态（未具体列出），构建了跨模态的幻觉分析。研究通过统一现有分类学框架、对比通用基础模型与医学专用基础模型在幻觉专用基准上的表现，以及评估物理约束架构、思维链提示和人机协同等缓解策略，并将所有发现映射到FDA的Total Product Lifecycle和Predetermined Change Control Plan框架中。

**Result:** 研究发现三种分类学框架单独均无法完整覆盖成像管线，需联合使用；通用基础模型在幻觉专用基准上优于医学专用模型，表明窄领域的微调可能引入由过拟合导致的虚构（confabulation）；极高比例的AI生成标记需要放射科专家修正后才能用于临床；物理约束架构、思维链提示和人机协同各自针对不同的失效模式，且组合使用时效果最佳。

**Conclusion:** 本文的核心启示在于：幻觉管理不应被视为部署前的一次性检查清单，而应作为贯穿FDA全生命周期的持续性义务；通用模型在幻觉鲁棒性上可能优于医学专用模型，提示领域微调可能带来过拟合风险；放射科医生的监督不可替代；多种缓解策略需组合使用以覆盖不同失效模式。

---

#### 📄 Abstract
AI systems are being deployed across medical imaging faster than their failure modes are understood. At this point in time, the failure of greatest clinical concern is hallucination: clinically plausible but factually incorrect outputs, including fabricated anatomical structures, missed findings, incorrect laterality, and invented measurements in generated reports, with direct consequences, for example, for biopsy decisions, staging, and treatment planning. This structured narrative synthesizes peer-reviewed studies, benchmark datasets, and FDA regulatory guidance across five imaging modalities to produce a cross-modality analysis of hallucination taxonomy, etiology, detection, and mitigation. Specifically, we address three questions in this study: (1) how can existing taxonomies be unified across modalities?, (2) how do medical-specialized foundation models hallucinate less than general-purpose ones?, and (3) which mitigation strategies are effective and compatible with FDA lifecycle oversight? We note that three taxonomic frameworks together cover the imaging pipeline in a way no single framework does alone. We also highlight that general-purpose foundation models outperform medical-specialized models on hallucination-specific benchmarks, indicating that narrow domain fine-tuning can introduce overfitting-induced confabulation. At the same time, the oversight of radiologists remains essential; for instance, a very high percentage of of AI-generated flags required expert correction before clinical use. Physics-informed architectural constraints, Chain-of-Thought prompting, and human-in-the-loop safeguards each address different failure modes and is effective when combined. All findings are mapped to the FDA's Total Product Lifecycle and Predetermined Change Control Plan frameworks, which treat hallucination management as a lifecycle obligation rather than a pre-deployment checklist.


### [71] [Reward Modeling for Multi-Agent Orchestration](https://arxiv.org/abs/2606.13598)
*King Yeung Tsang, Zihao Zhao, Vishal Venkataramani, Haizhou Shi, Zixuan Ke, Semih Yavuz, Shafiq Joty, Hao Wang*

#### 🧩 TL;DR
提出OrchRM框架，通过构建多智能体执行过程中的中间产物作为胜负对来训练Bradley-Terry奖励模型，实现无需人工标注的编排质量评估，从而高效指导多智能体系统的编排器训练和测试时缩放。

---

#### 📘 Detailed Summary
**Motivation:** 基于大语言模型的多智能体系统需要有效的编排机制来协调专业智能体，但编排器的训练面临监督信号稀缺和计算成本高昂的双重挑战，现有方法依赖昂贵的子智能体 rollout，缺乏高效的编排质量评估手段。

**Method:** 提出OrchRM自监督框架，利用多智能体执行过程中产生的中间产物构建胜负对，基于Bradley-Terry模型训练奖励模型；该奖励模型直接在编排层面运作，无需子智能体 rollout，同时支持奖励引导的编排器训练和测试时缩放。

**Result:** 在数学推理、网页问答和多跳推理等多个领域，OrchRM在训练效率上提升高达10倍（token使用量），MAS测试时缩放准确率提升高达8%，且增益在不同领域间一致迁移。

**Conclusion:** 编排级别的奖励建模是一种可扩展的多智能体编排方向，能够在不依赖人工标注和昂贵rollout的情况下实现高效、高性能的编排器训练与测试时缩放。

---

#### 📄 Abstract
Multi-Agent Systems (MAS) built on Large Language Models (LLMs) require effective orchestration to coordinate specialized agents, yet training such orchestrators is hindered by limited supervision and high computational cost. We propose Orchestration Reward Modeling (OrchRM), a self-supervised framework for evaluating orchestration quality without human annotations. OrchRM leverages intermediate artifacts from multi-agent executions to construct win-lose pairs for Bradley-Terry reward model training. Unlike existing MAS test-time scaling and orchestrator training frameworks that rely on costly sub-agent rollouts, OrchRM operates directly at the orchestration level, enabling efficient and high-performing reward-guided orchestrator training and MAS test-time scaling. OrchRM improves training efficiency by up to 10x in token usage while improving MAS test-time scaling performance by up to 8% in accuracy. These gains consistently transfer across multiple domains, including mathematical reasoning, web-based question answering, and multi-hop reasoning, demonstrating orchestration-level reward modeling as a scalable direction for robust multi-agent orchestration. Code will be available at https://github.com/Wang-ML-Lab/OrchRM.


### [72] [EPIG: Emotion-Based Prompting for Personalised Image Generation](https://arxiv.org/abs/2606.13247)
*Emna Othmen, Mohamed Yassine Landolsi, Lotfi Ben Romdhane*

#### 🧩 TL;DR
EPIG是一种轻量级、无需训练的提示增强方法，基于心理学中的效价-唤醒度情感表征和角色感知的结构化提示丰富策略，在文本到图像扩散模型生成前提升提示的情感表现力，在保持语义一致性的前提下显著降低生成图像的平均唤醒度误差。

---

#### 📘 Detailed Summary
**Motivation:** 现有文本到图像扩散模型虽能生成高质量图像，但常用的提示策略较为通用，难以准确表达情感意图和细微的情感属性，导致生成图像在情感表达方面存在不足，缺乏对情感维度（如唤醒度）的有效控制手段。

**Method:** EPIG方法在图像生成之前对提示进行情感增强，基于心理学中的效价-唤醒度（valence-arousal）情感表征理论，结合结构化、角色感知的提示丰富策略，有针对性地增强提示中的情感相关成分，不修改或重新训练图像生成主干模型。

**Result:** 在包含10个多样化提示的基准测试上，EPIG将平均唤醒度误差相比朴素插入方法和基于LLM的提示扩展方法分别降低了14%和12%，差异具有统计学显著性；在包含人类、儿童或动物等显式主体的提示上效果更为显著，误差降低达17%；同时通过CLIPScore和消融实验验证了效价对齐和语义一致性得到保持。

**Conclusion:** EPIG提供了一种轻量级、无需训练且适用于资源受限和个性化图像生成场景的情感控制方案，在控制唤醒度方面尤为有效，并且该方法对提示中显式主体具有敏感性，为情感可控的文本到图像生成提供了实用且有效的提示级干预途径。

---

#### 📄 Abstract
Text-to-image diffusion models have achieved impressive results in synthesizing high-quality images from natural language prompts. However, commonly used prompting strategies remain relatively generic, limiting the model's ability to accurately express emotional intent and nuanced affective attributes.
  This work proposes EPIG, a method that enhances emotional expressiveness at the prompt level prior to image generation. Grounded in psychologically informed emotion representations (valence-arousal) and leveraging structured, role-aware prompt enrichment, EPIG enriches emotion-related components of prompts without modifying or retraining the image generation backbone.
  The resulting emotion-aware prompts guide the generative process toward more emotionally coherent visual outputs, with particular effectiveness in controlling arousal. EPIG is lightweight, training-free, and well suited for resource-constrained and personalized image generation scenarios.
  Experimental results on a benchmark of 10 diverse prompts show that EPIG reduces mean arousal error compared to strong baselines, including naive insertion and LLM-based prompt expansion, with reductions of 14% and 12%, respectively. These improvements are statistically significant. EPIG also preserves valence alignment and semantic consistency, as measured by CLIPScore and supported by ablation studies.
  The effect is more pronounced on prompts containing explicit subjects such as humans, children, or animals, where the reduction reaches 17%, highlighting the subject-sensitive behavior of the proposed method.


### [73] [ERTS: Adversarial Robustness Testing of Ethical AI via Semantic Perturbation in a Bounded Consequence Space](https://arxiv.org/abs/2606.13282)
*Pratyush Chaudhari*

#### 🧩 TL;DR
本文提出伦理鲁棒性测试系统（ERTS），一个将伦理困境编码为22维伦理后果空间、应用17种语义扰动函数并采用伦理不稳定性指数（EII）评估的对抗测试框架，实验发现仅33%的模型通过评估，Llama-3.2在公平性腐败和信息退化攻击下表现尤为脆弱。

---

#### 📘 Detailed Summary
**Motivation:** 当前AI系统在高风险伦理场景（如医疗分诊、自动驾驶、就业筛选）中的部署日益广泛，但针对伦理推理进行对抗性操控的鲁棒性评估方法仍缺乏系统性框架，现有工作未能将受限伦理后果空间、语义一致性约束与领域自适应评估整合为统一的对抗测试流水线。

**Method:** ERTS框架包含四个核心组件：(1)将伦理困境编码为基于经典伦理理论的22维伦理后果空间（ECS）；(2)应用17种语义扰动函数，受6类有效性约束（含新型语义一致性约束）限制；(3)通过4分量伦理不稳定性指数（EII）量化决策偏离；(4)生成领域自适应的预部署鲁棒性评估结论。实验涉及4个结构化基线模型和2个生产级LLM（Gemini 2.0 Flash和Llama 3.2），覆盖50个伦理场景、8个部署领域，生成1500个对抗测试用例。

**Result:** 仅33%的模型通过评估审查，本地部署的Llama-3.2模型在公平性腐败和信息退化攻击下尤为脆弱，其伦理鲁棒性得分（ERS）仅为0.737。

**Conclusion:** ERTS提供了一个系统化、可扩展的伦理鲁棒性评估框架，揭示了当前主流LLM在对抗性伦理测试中的显著脆弱性，对AI系统在高风险伦理场景中的安全部署具有重要启示意义。

---

#### 📄 Abstract
As AI systems are deployed in high-stakes ethical contexts such as healthcare triage, autonomous vehicle control, and employment screening, formal methods for evaluating their robustness against adversarial manipulation of ethical reasoning remain underdeveloped. This paper introduces the Ethical Robustness Testing System (ERTS), a closed-pipeline framework that: (1) encodes ethical dilemmas into a 22-dimensional Ethical Consequence Space (ECS) grounded in established ethical theory; (2) applies 17 semantic perturbation functions subject to 6 validity constraint classes including a novel semantic coherence constraint; (3) measures decision deviation via a 4-component Ethical Instability Index (EII); and (4) produces domain-adaptive pre-deployment robustness assessment verdicts. We evaluate 4 structured baseline models and 2 production LLMs (Gemini 2.0 Flash and Llama 3.2) across 50 ethical scenarios spanning 8 deployment domains, generating 1,500 adversarial test cases. Results demonstrate that only 33% of models achieve assessment clearance, with the local Llama-3.2 model proving particularly vulnerable to fairness corruption and information degradation attacks (ERS = 0.737). To the best of our knowledge, no existing framework combines a bounded ethical consequence space, semantic coherence constraints, and domain-adaptive assessment in a single adversarial testing pipeline.


### [74] [Multi-Agent Reinforcement Learning from Delayed Marketplace Feedback for Objective-Weight Adaptation in Three-Sided Dispatch](https://arxiv.org/abs/2606.13604)
*Haochen Wu, Yi Hou, Shiguang Xie*

#### 🧩 TL;DR
DoorDash 提出了一种基于强化学习（RL）的离线策略学习系统，通过学习存储级策略（store-level policy）调整外卖配送市场中调度优化器的目标权重（dispatch objective weights），在保持生产约束和安全机制的前提下，利用延迟、带噪且耦合的世界反馈信号，提升了配送批次效率并降低了骑手时间成本，同时未损害面向客户的交付质量。

---

#### 📘 Detailed Summary
**Motivation:** 在三方市场中，调度决策受交付速度、骑手利用率及商家拥堵等延迟运营结果（world feedback）评估，但如何利用此类延迟、带噪且耦合的反馈信号安全地在线调整决策策略，同时避免破坏现有组合优化调度器和生产约束，是一个尚未解决的研究与工程挑战。

**Method:** 系统基于离线日志数据训练一个存储级策略（store-level policy），该策略输出离散乘数（discrete multiplier），用于调整调度优化器在交付质量与批次效率之间的权衡。通过 Double Q-learning 目标与保守正则化项（conservative regularizer）训练共享价值函数，实现中心化离线训练与去中心化存储级执行，以缓解分布外价值高估问题。

**Result:** 在生产环境的切换实验（production switchback experiment）中，离线训练的策略显著提升了批次效率（batching）并降低了骑手侧时间成本，同时未对面向客户的交付质量造成负面影响。

**Conclusion:** 该工作展示了如何通过离线强化学习安全地利用实时经济和物流系统中的世界反馈来在线调整决策策略，为在大规模、复杂约束的生产环境中应用 RL 提供了一种可行的接口设计与实践案例。

---

#### 📄 Abstract
Dispatch in three-sided marketplaces provides a natural setting for reinforcement learning from world feedback: decisions are evaluated by delayed operational outcomes such as delivery speed, courier utilization, and merchant congestion. We present a deployed reinforcement learning system at DoorDash that adapts dispatch objective weights in a large-scale food-delivery marketplace using delayed signals. Rather than replacing the combinatorial assignment optimizer, a store-level policy learned from logged marketplace data selects a discrete multiplier that shifts the dispatch optimizer's tradeoff between delivery quality and batching efficiency. This interface enables offline policy learning under noisy, delayed, and coupled feedback while preserving production feasibility constraints and operational safeguards. We train a shared value function using centralized offline data and decentralized store-level execution, with Double Q-learning targets and a conservative regularizer to reduce out-of-distribution value overestimation. In a production switchback experiment, the offline-trained policy increases batching and reduces courier-side time costs without degrading customer-facing delivery quality. Results illustrate how world feedback from a live economic and logistics system can be used to safely adapt decision policies online.


### [75] [AgentBeats: Agentifying Agent Assessment for Openness, Standardization, and Reproducibility](https://arxiv.org/abs/2606.13608)
*Xiaoyuan Liu, Jianhong Tu, Yuqi Chen, Siyuan Xie, Sihan Ren, Tianneng Shi, Gal Gantar, Evan Sandoval, Donghyun Lee, Daniel Miao, Peter J. Gilbert, Nick Hynes, Mauro Staver, Warren He, David Marn, Andrew Low, Xi Zhang, Elron Bandel, Michal Shmueli-Scheuer, Siva Reddy, Alexandre Drouin, Alexandre Lacoste, Ramayya Krishnan, Elham Tabassi, Yu Su, Victor Barres, Chenguang Wang, Wenbo Guo, Dawn Song*

#### 🧩 TL;DR
本文提出Agentified Agent Assessment (AAA)框架，通过标准化协议(A2A/MCP)和评判智能体实现通用、可复现、智能体无关的评估，并基于AgentBeats系统在五个月开放竞赛和编码智能体案例研究中验证了其覆盖率、实用性和保真度。

---

#### 📘 Detailed Summary
**Motivation:** 现有智能体系统评估高度碎片化，多数基准依赖固定的大语言模型评估框架，存在集成成本高、测试与生产环境不匹配、难以在不同智能体设计间公平比较等问题，根本原因是缺乏开放且智能体无关的标准化评估接口。

**Method:** 提出AAA评估范式，使用评判智能体进行评测，所有参与者通过标准化协议交互：A2A (Agent-to-Agent) 用于任务管理，MCP (Model Context Protocol) 用于工具访问；该框架将评估逻辑与智能体实现解耦，仅需单一接口即可完成评估。进一步提出AgentBeats具体实现，定义了五种实际运行模式以适应开放性、隐私性和可复现性约束。

**Result:** 通过两个研究验证：五个月开放竞赛吸引298个评判智能体（12个类别）和467个来自独立参与者的被评估智能体，证明AAA适用于异构基准；编码智能体案例研究确认智能化评估保留了与公开记录一致的保真度，并揭示了此前缺失的智能体间直接对比结果，产生了关于智能体设计的研究洞见。

**Conclusion:** AAA与AgentBeats结合了社区规模田野研究和受控编码案例研究，验证了该框架在异构场景下具备覆盖率、实用性和保真度，为开放、标准化、可复现的智能体评估提供了清晰路径。

---

#### 📄 Abstract
Agent systems are advancing quickly across domains, but their evaluation remains fragmented. Most benchmarks rely on fixed, LLM-centric harnesses that require heavy integration, create test-production mismatch, and limit fair comparison across diverse agent designs. The root problem is the lack of an open, agent-agnostic assessment interface. We advocate Agentified Agent Assessment (AAA), where evaluation is performed by judge agents and all participants interact through standardized protocols: A2A for task management and MCP for tool access. Conventional benchmarking defines two separate interfaces, one for the benchmark and one for the agent, while AAA only needs one; this yields a generic, unified framework that separates assessment logic from agent implementation and enables reproducible, interoperable, and multi-agent evaluation. We further introduce AgentBeats as a concrete realization of AAA: we identify five practical operation modes that make standardized assessment compatible with real-world constraints on openness, privacy, and reproducibility.
  To evaluate our design at scale, we conduct two studies: a five-month open competition that drew 298 judge agents across 12 categories together with 467 subject agents from independent participants, showing that AAA applies across a heterogeneous range of benchmarks; and a case study on coding agents that confirms agentified evaluation preserves fidelity with the public record while surfacing previously missing head-to-head results, yielding research insights about agent design. Combining a community-scale field study and a controlled coding case study, we verify that AAA delivers coverage, practicality, and fidelity across heterogeneous scenarios at scale. Together, AAA and AgentBeats offer a clear path toward open, standardized, and reproducible agent assessment.


### [76] [Beyond Runtime Enforcement: Shield Synthesis as Defensibility Analysis for Adversarial Networks](https://arxiv.org/abs/2606.13621)
*Achraf Hsain, Sultan Almuhammadi*

#### 🧩 TL;DR
本文批判了将屏蔽强化学习仅视为运行时安全机制的传统观点，提出将自动机理论与双人安全博弈重新解读为设计时的结构分析工具，通过网络防御场景中的可防御性判定与指纹提取，证明屏蔽合成的主要价值在于回答系统架构安全问詢，而非产出安全策略。

---

#### 📘 Detailed Summary
**Motivation:** 现有屏蔽强化学习通常将时序逻辑规范编译为自动机，作为运行时限制智能体动作的安全机制。本文认为这种产品定位是错误的，同一套自动机理论工具（规范编译、乘积博弈构建、吸引子计算、胜区提取）更适合作为设计时的分析仪器，输出系统结构层面的洞察而非部署时的运行时约束。

**Method:** 本文将网络防御建模为约束双人安全博弈：防御者规范定义不安全区域，攻击者规范限制对手在吸引子计算中的合法动作。求解博弈得到可防御性判定（拓扑-规范对是否可防御的形式化证书）及对应的胜区和屏蔽。进一步从吸引子结构中提取拓扑级度量，并与屏蔽约束下的对抗多智能体强化学习后收敛行为结合，形成包含形式安全属性与自适应博弈下操作行为的"可防御性指纹"。

**Result:** 假设分析表明，形式可防御性与操作有效性捕捉了安全的不同维度：微小的架构变更可导致操作结果的大幅变化，同时几乎不影响形式安全裕度。这验证了屏蔽合成最有价值的产出是系统架构层面的可防御性判定而非安全策略本身。

**Conclusion:** 屏蔽强化学习的真正价值不在于作为安全智能体的部署机制，而在于作为回答系统"能否被防御、在哪里被防御、如何被防御"等架构问题的框架。可防御性判定才是核心输出，而非安全策略。

---

#### 📄 Abstract
Shielded reinforcement learning is typically presented as a runtime safety mechanism that compiles temporal-logic specifications into automata restricting an agent's actions. We argue this is the wrong product. The same automata-theoretic machinery -- specification compilation, product game construction, attractor computation, and winning-region extraction -- is better read as a design-time analytical instrument whose outputs are structural insights about a system rather than runtime constraints on a deployed agent.
  We instantiate this through a constrained two-player safety game for network defense. The two specifications are enforced asymmetrically: the defender specification defines the unsafe region of the game, whereas the attacker specification restricts the adversary's legal actions during attractor computation. Solving the game yields a defensibility verdict -- a formal certificate that a topology-specification pair is or is not defensible -- with the associated winning region and shield.
  Beyond the binary verdict, we derive topology-level metrics from the attractor structure and combine them with post-convergence behavior from shield-constrained adversarial multi-agent reinforcement learning. Together these form a defensibility fingerprint capturing both a network's formal safety properties and its operational behavior under adaptive play.
  A what-if analysis shows that formal defensibility and operational effectiveness capture distinct aspects of security: small architectural changes can produce large shifts in operational outcomes while leaving formal safety margins nearly unchanged. Shield synthesis is thus most valuable not as a deployment mechanism for safe agents, but as a framework for answering architectural questions about whether, where, and how a system can be defended. The defensibility verdict is the output, not the safe policy.
