...
### [1] [VideoRouter: Query-Adaptive Dual Routing for Efficient Long-Video Understanding](https://arxiv.org/abs/2605.05848)
*Kuanwei Lin, Wenhao Zhang, Ge Li*

#### 🧩 TL;DR
VideoRouter提出了一种查询感知的双路由框架（Semantic Router和Image Router），通过动态分配视觉token预算实现了对长视频的高效压缩，在InternVL基线上最高实现67.9%的token缩减并保持性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有视频大模型在处理长视频时面临视觉token序列过长导致的显存和延迟瓶颈，而现有压缩方法要么查询感知能力弱，要么对帧采用固定的压缩策略，无法适应视觉证据在时间维度上不均匀分布的情况，导致压缩效果次优。

**Method:** VideoRouter基于InternVL构建了一个查询感知的双路由框架（Semantic Router和Image Router），用于长视频视觉token的预算分配。Semantic Router预测主导分配策略，在广泛时间覆盖与自适应高分辨率保留之间进行选择；Image Router利用早期LLM层对每一帧进行相关性评分，从而对不相关的帧进行激进压缩，对关键证据帧保留细节。为训练这两个路由，构建了用于分配策略监督的Video-QTR-10K数据集和用于帧相关性监督的Video-FLR-200K数据集。

**Result:** 在VideoMME、MLVU和LongVideoBench等长视频基准上，VideoRouter在相同或更低预算下始终优于InternVL基线，最高可实现67.9%的token压缩率，同时保持或提升任务性能。

**Conclusion:** VideoRouter验证了面向长视频推理的查询感知型自适应压缩策略的有效性，表明通过轻量级路由网络根据查询语义动态分配视觉token预算，可以在大幅降低计算开销的同时保持甚至提升多模态理解性能，为未来长视频大模型的高效推理提供了新思路。
...
### [2] [Training-Free Dense Hand Contact Estimation with Multi-Modal Large Language Models](https://arxiv.org/abs/2605.05886)
*Daniel Sungho Jung, Kyoung Mu Lee*

#### 🧩 TL;DR
本文提出ContactPrompt，一种无需训练、零样本的密集手部接触估计方法，利用多模态大语言模型（MLLM）的推理能力，通过详细的手部部件分割和分部件顶点网格表示编码3D手部几何，并采用多阶段结构化接触推理实现从全局语义到细粒度几何的渐进式推理，在无需训练的情况下超越此前需要大规模密集接触数据集监督训练的方法。

---

#### 📘 Detailed Summary
**Motivation:** 多模态大语言模型（MLLM）在视觉语义理解方面展现出强大能力，但将其应用于密集手部接触估计仍未被探索。现有应用MLLM面临两大挑战：一是MLLM主要处理视觉和语言模态，难以编码显式的3D手部几何信息；二是MLLM倾向于关注高层语义而非细粒度几何推理，难以捕捉顶点级别的密集接触信息。因此，如何有效利用MLLM的推理能力实现精确的密集手部接触估计是一个亟待解决的研究空白。

**Method:** ContactPrompt是一种无需训练、零样本的密集手部接触估计方法。为有效编码3D手部几何，提出了详细的手部部件分割和分部件顶点网格表示，提供结构化的局部几何信息。为实现精确高效的密集接触预测，设计了多阶段结构化接触推理策略，通过部件条件化渐进地桥接全局语义信息与细粒度几何信息，从而充分利用MLLM的推理能力完成密集手部接触估计任务。

**Result:** 实验结果表明，ContactPrompt在无需任何训练数据的情况下，其密集手部接触估计性能优于此前需要在大规模密集接触数据集上进行监督训练的现有方法。

**Conclusion:** 本研究验证了多模态大语言模型在密集手部接触估计任务上的可行性，提出了一种无需训练即可利用MLLM推理能力实现精确几何推理的有效范式。该工作表明，通过适当的结构化几何表示和渐进式推理策略，MLLM能够在不需要训练的情况下完成传统上需要监督学习的密集几何推理任务，为相关领域提供了新的研究方向。
...
#### 🧩 TL;DR
DeScore 提出了一种解耦的"先思考后评分"范式，通过两阶段优化框架将生成式 CoT 推理与判别式评分模块分离，在保持推理泛化能力的同时克服了生成式奖励模型中推理与评分耦合带来的训练不稳定问题。

---

#### 📘 Detailed Summary
**Motivation:** 现有视频奖励模型面临根本性困境：判别式奖励模型直接在 MLLM 特征上回归奖励，缺乏显式推理，容易陷入捷径学习且严重依赖数据规模；生成式奖励模型虽具备 CoT 推理的泛化潜力，但推理与评分在自回归推理链中紧密耦合，导致固有的优化瓶颈。

**Method:** DeScore 采用解耦的"先思考后评分"范式，包含一个用于生成显式 CoT 的 MLLM 和一个由可学习查询令牌与回归头组成的判别式评分模块；优化采用两阶段框架：第一阶段通过随机掩码机制的判别式冷启动确保稳健的评分能力，第二阶段通过双目标强化学习独立优化 CoT 推理质量并校准最终奖励。

**Result:** 实验结果表明，DeScore 在训练效率和泛化能力上均优于现有基线方法，其解耦设计使得更高质量的推理直接转化为更优的模型性能，验证了"先思考后评分"范式在视频奖励建模中的有效性。

**Conclusion:** DeScore 证明了解耦推理与评分是兼顾生成式奖励模型泛化优势与训练稳定性的有效路径，为视频生成模型的后训练和测试时扩展提供了高质量的奖励信号支持。
...
### [4] [Pest-Thinker: Learning to Think and Reason like Entomologists via Reinforcement Learning](https://arxiv.org/abs/2605.06121)
*Xueheng Li, Yu Wang, Tao Hu, Ji Huang, Ke Cao, Qize Yang, Rui Li, Jie Zhang, Chengjun Xie*

#### 🧩 TL;DR
本文提出Pest-Thinker，一个知识驱动的强化学习框架，通过结合CoT推理轨迹的SFT和基于GRPO的形态特征奖励机制，使多模态大语言模型在细粒度害虫形态识别上实现领域内外显著提升，迈向农业害虫分析的专家级视觉推理。

---

#### 📘 Detailed Summary
**Motivation:** 尽管多模态大语言模型在视觉理解和智慧农业中展现出潜力，但由于害虫识别领域存在物种间高复杂度、物种内高变异性以及专家标注数据稀缺等独特挑战，MLLMs在该领域的直接应用仍受限于细粒度视觉推理能力不足。

**Method:** Pest-Thinker框架首先构建了两个高清害虫基准数据集QFSD和AgriInsect，包含多样物种和专家标注的形态特征；随后基于这些数据合成Chain-of-Thought推理轨迹，通过监督微调使模型学习害虫特定视觉线索的结构化推理；最后采用Group Relative Policy Optimization算法，并引入由LLM-as-a-Judge策略评估的新型特征奖励机制，引导模型聚焦于可观察的形态学证据。

**Result:** 大量实验表明，Pest-Thinker在领域内和领域外的形态学理解任务上均取得显著提升，标志着向专家级视觉推理迈出了关键一步，数据集和源代码将在论文接收后公开。

**Conclusion:** 本文验证了知识驱动的强化学习范式能够有效增强MLLM在细粒度农业害虫识别中的形态推理能力，为智能农业中的专家级视觉分析提供了可行路径，同时公开的基准数据和代码有望推动该领域的后续研究。
...
### [5] [MedHorizon: Towards Long-context Medical Video Understanding in the Wild](https://arxiv.org/abs/2605.06537)
...
#### 🧩 TL;DR
MedHorizon是一个包含759小时完整临床手术视频和1,253个证据驱动选择题的长上下文医学视频理解基准，揭示了现有MLLMs在稀疏证据检索与多跳临床推理方面的严重不足，最佳模型仅达到41.1%准确率。

---

#### 📘 Detailed Summary
**Motivation:** 现有的医学多模态大模型（MLLMs）虽然在图像理解和短视频分析上取得进展，但真实的临床审查往往需要理解完整流程视频。与通用长视频不同，医学手术视频包含高度冗余的解剖视图，而决定性证据在时间上稀疏、空间上细微且依赖上下文。现有基准通常假设证据已通过图像、短片或预分割视频定位，导致“先检索后推理”的问题未得到充分测试。

**Method:** 作者构建了MedHorizon基准，包含759小时完整临床流程视频和1,253个证据支持的多选题，每个问题要求模型先从高度冗余的视频流中检索稀疏证据（平均仅0.166%的证据帧），再进行多跳临床推理。评估覆盖了通用域、医学域和长视频MLLMs的多种代表性模型。

**Result:** 最佳模型在MedHorizon上仅达到41.1%的准确率，表明现有系统远未实现鲁棒的完整流程理解。进一步分析得出四个关键发现：模型性能并未随帧数增加而可靠提升；证据检索和临床解释仍是主要瓶颈；这些瓶颈源于弱过程推理能力和冗余下的注意力漂移；通用采样方法仅能部分平衡局部细节与全局覆盖。

**Conclusion:** MedHorizon为评估MLLMs在完整临床工作流中检索稀疏证据并进行推理的能力提供了严格的测试平台，揭示了当前模型在处理长上下文医学视频时的根本性局限，指出了过程推理和注意力机制改进的关键方向。
...
### [6] [The Cost of Context: Mitigating Textual Bias in Multimodal Retrieval-Augmented Generation](https://arxiv.org/abs/2605.05594)
*Hoin Jung, Xiaoqian Wang*

#### 🧩 TL;DR
本文识别并形式化了多模态大语言模型在检索增强生成中的"再破坏"现象，揭示了由注意力坍塌（视觉盲区和位置偏差）导致的失败模式，并提出了无需训练的时间级瓶颈注意力干预方法BAIR来恢复视觉显著性并修复多模态 grounding。

---

#### 📘 Detailed Summary
**Motivation:** 尽管检索增强生成被广泛用于缓解多模态大语言模型的幻觉问题，但引入外部文档反而可能在实例层面隐藏严重的失败模式，现有研究缺乏对这一现象的正式定义和机制性理解。

**Method:** 本文首先通过分析内部注意力矩阵对再破坏现象进行机制诊断，发现其根源是双重注意力坍塌：视觉盲区（视觉注意力质量和尖锐度系统性抑制）和结构性的位置偏见（模型强制优先关注边界标记而非语义相关性）。随后提出BAIR，一个参数无关的推理时框架，通过恢复视觉显著性并对文本干扰项施加位置感知惩罚来修复多模态 grounding。

**Result:** 在医学事实性、社会公平性和地理空间基准测试中，BAIR成功恢复了多模态 grounding，提升了诊断可靠性，且无需模型重训练或微调。

**Conclusion:** 本文揭示了RAG中一个"成功幻觉"——许多看似正确的RAG输出仅仅是位置巧合（模型的文本复制偏差恰好与真实标注位置对齐），而非真正的多模态理解；BAIR为诊断和修复此类注意力驱动的失败提供了有效的轻量级方案。
...
### [7] [Large Vision-Language Models Get Lost in Attention](https://arxiv.org/abs/2605.05668)
*Gongli Xi, Ye Tian, Mengyu Yang, Huahui Yi, Liang Lin, Xiaoshuai Hao, Kun Wang, Wendong Wang*

#### 🧩 TL;DR
本文提出一个基于信息论和几何学的统一框架，用于量化视觉-语言模型（LVLM）中残差更新的几何与熵特性，发现注意力模块是保持子空间的重构算子，而FFN是扩展子空间的语义创新算子；进一步实验表明，用预定义噪声替换注意力权重在多数数据集上表现相当甚至更优，揭示当前LVLM存在严重的注意力冗余和资源错配问题。

---

#### 📘 Detailed Summary
**Motivation:** 尽管LVLM的解码器主干始终基于残差连接的Transformer架构，但内部模块（注意力与前馈网络）的功能角色尚未被统一的理论框架所阐明；现有统计归因方法虽提供了一定见解，但缺乏统一的理论基础，难以系统理解模块间的功能解耦与资源分配效率。

**Method:** 提出一个基于信息论和几何学的统一分析框架，从几何和熵两个维度量化残差更新的性质，以此揭示注意力模块和FFN模块在功能上的本质差异；并进一步设计实验，将学习得到的注意力权重替换为预定义值（如高斯噪声），以检验注意力机制的真实贡献与冗余程度。

**Result:** 框架分析揭示注意力模块是子空间保持算子，主要执行特征重构，而FFN是子空间扩展算子，负责语义创新；替换注意力权重为高斯噪声后，模型在多数数据集上的表现与原始模型相当甚至更优，表明当前注意力机制存在严重的资源错配和信息冗余。

**Conclusion:** 当前SOTA LVLM中的注意力机制存在严重的功能冗余与资源错配，模型实际上“迷失在注意力中”而非有效利用视觉上下文，这为未来LVLM的架构优化（如简化或替代冗余注意力模块）提供了理论依据和改进方向。
...
#### 🧩 TL;DR
本文揭示了多模态大语言模型意图混淆越狱攻击中的"重构-隐藏权衡"，并提出基于低有害关键词对齐和多样性的字符删除变体构造方法，结合关键词相关干扰图像，在闭源和开源MLLM上实现了更强的越狱攻击效果。

---

#### 📘 Detailed Summary
**Motivation:** 现有多模态大语言模型（MLLM）的意图混淆越狱攻击方法（将有害查询转换为隐蔽的多模态输入以绕过安全机制）面临一个关键权衡：转换后的输入必须既能隐藏有害意图以绕过安全过滤器，又要保持足够的可恢复性以使受害模型能够重构原始请求。现有方法难以平衡这一权衡，限制了攻击效果。

**Method:** 本文通过重构分析三种代表性黑盒方法，发现现有变换难以平衡重构-隐藏权衡，而字符删除变体能实现更好的平衡。基于此，提出隐蔽感知变体构造方法：贪心选择低有害关键词对齐且多样化的字符删除变体，并通过五种模态感知提示策略实例化。此外，引入关键词相关干扰图像，在多样化上下文中展示有害关键词，提供比通用干扰图像更有效的辅助视觉上下文。

**Result:** 在闭源和开源MLLM上的实验表明，所提出的策略显著优于强基线方法，揭示了模型自身的重构能力可以被利用来恢复隐藏的有害意图并产生不安全响应这一未被充分探索的漏洞。

**Conclusion:** 本文证明了MLLM意图混淆越狱攻击中存在重构-隐藏权衡，字符删除变体结合隐蔽感知构造和关键词相关干扰图像能有效突破该权衡，这揭示了模型自身重构能力可被反向利用的安全漏洞，为MLLM安全机制设计提供了新的警示方向。
...
### [9] [Null Space Constrained Contrastive Visual Forgetting for MLLM Unlearning](https://arxiv.org/abs/2605.05909)
*Yuhang Wang, Zhenxing Niu, Haoxuan Ji, Guangyu He, Linlin Zhang, Haichang Gao*

#### 🧩 TL;DR
提出了一种针对多模态大语言模型(MLLM)的机器遗忘方法，通过冻结LLM主干、微调视觉模块，结合对比视觉遗忘(CVF)机制和零空间约束，在有效遗忘目标视觉知识的同时最大程度保留非目标视觉知识和全部文本知识，并扩展至连续遗忘场景。

---

#### 📘 Detailed Summary
**Motivation:** 机器遗忘的核心挑战在于平衡目标知识移除与非目标知识保留，而在多模态大语言模型(MLLM)中，知识进一步划分为视觉和文本两种模态且紧密交织，使得该挑战更加突出。现有方法难以在遗忘特定视觉知识的同时有效保留非目标视觉知识和全部文本知识。

**Method:** 该方法冻结LLM主干，仅通过微调视觉模块实现遗忘。首先提出对比视觉遗忘(CVF)机制，将目标视觉知识与保留视觉知识在特征空间中分离，引导目标视觉概念表征至适当区域；其次，识别与保留知识相关的零空间，将遗忘过程约束在该空间内以减轻知识保留性能下降；最后，将方法扩展至连续遗忘场景以处理顺序到达的遗忘请求。

**Result:** 在多种基准测试上的大量实验表明，该方法在有效遗忘与稳健知识保留之间取得了良好的平衡，能够实现目标视觉知识的有效移除同时保持非目标视觉知识和文本知识的完整性。

**Conclusion:** 该研究为MLLM中的视觉知识遗忘提供了一种有效的解决方案，通过视觉模块微调与零空间约束实现了细粒度的模态特异性遗忘，并为连续遗忘场景提供了可扩展的框架，对多模态模型的安全性和合规性具有重要启示。
...
### [10] [ICU-Bench:Benchmarking Continual Unlearning in Multimodal Large Language Models](https://arxiv.org/abs/2605.05938)
*Yuhang Wang, Wenjie Mei, Junkai Zhang, Guangyu He, Zhenxing Niu, Haichang Gao*

#### 🧩 TL;DR
ICU-Bench是一个面向多模态大语言模型的持续机器遗忘基准，包含来自医疗报告和劳动合同的1,000份隐私敏感文档、9,500张图像、16,000个问答对和100个遗忘任务，并引入新的持续遗忘评估指标。实验表明，现有遗忘方法在持续场景下表现不足，难以平衡遗忘效果、历史遗忘保持、模型效用和长序列稳定性。

---

#### 📘 Detailed Summary
**Motivation:** 现有多模态大语言模型的机器遗忘基准主要关注静态或短序列场景，无法有效评估模型在面对现实中持续到来的隐私删除请求时的表现，因而缺乏针对持续多模态遗忘任务的系统性评测框架和公开数据集。

**Method:** 构建了包含1,000个隐私敏感档案（来自医疗报告和劳动合同两个文档领域）、9,500张图像、16,000个问答对和100个遗忘任务的持续多模态遗忘基准ICU-Bench，并设计了遗忘效果、历史遗忘保持、保留效用和过程稳定性四个维度的持续遗忘评估指标。基于该基准对代表性遗忘方法进行了系统实验分析。

**Result:** 实验结果表明，现有代表性遗忘方法在持续遗忘场景下普遍表现不佳，在遗忘质量、模型效用保持及长任务序列可扩展性之间存在明显的权衡困境，反映出当前方法在应对持续遗忘需求时的根本性不足。

**Conclusion:** 该研究揭示了现有多模态遗忘方法在持续场景下的局限性，强调了设计明确面向持续隐私删除需求的多模态遗忘方法的必要性，为未来该领域的研究提供了基准平台和评估标准。
...
### [11] [CrossCult-KIBench: A Benchmark for Cross-Cultural Knowledge Insertion in MLLMs](https://arxiv.org/abs/2605.06115)
*Zhen Zeng, Leijiang Gu, Feng Li, Jing Yu, Zenglin Shi*

#### 🧩 TL;DR
本文提出跨文化知识插入任务及CrossCult-KIBench基准（9800个视觉场景，覆盖英/中/阿拉伯三语文化），并设计MCKI基线方法，实验揭示现有MLLMs在文化适应与行为保留之间难以平衡，为开发文化自适应多模态大模型指明了方向。

---

#### 📘 Detailed Summary
**Motivation:** 现有的多模态大语言模型（MLLMs）主要基于英语中心数据进行训练，因此在跨文化场景中经常生成文化不恰当或不对齐的响应，缺乏针对特定文化上下文的适应能力，同时需要避免对非目标文化行为的负面影响。

**Method:** 提出跨文化知识插入任务，并构建CrossCult-KIBench基准，包含9800个图像驱动案例、49个文化相关视觉场景，覆盖英语、中文和阿拉伯语三种语言-文化群体，支持单次插入与顺序插入两种评估设置；同时提出Memory-Conditioned Knowledge Insertion（MCKI）基线方法，从外部记忆库中检索相关文化知识，利用冻结MLLM表征匹配后，将匹配条目作为条件提示前置到输入中。

**Result:** 在CrossCult-KIBench上的大量实验表明，现有方法难以在有效的文化适应与对非目标文化行为保留之间取得平衡，暴露出当前MLLMs在处理跨文化知识插入任务时的核心挑战。

**Conclusion:** 该工作强调了开发更具文化适应性和负责任的MLLMs的重要研究方向，指出跨文化知识插入中适应效果与行为保留之间的权衡是亟待解决的关键问题。
...
### [12] [Towards Annotation-Free Validation of MLLMs: A Vision-Language Logical Consistency Metric](https://arxiv.org/abs/2605.06201)
*Ying Gu, Mei Chee Leong, Hui Li Tan, Shangbo Mao, Liyuan Li, Nancy Chen*

#### 🧩 TL;DR
本文提出VL-LCM框架，在无需真实标注的条件下评估多模态大模型的视觉-语言逻辑一致性，实验表明当前模型在准确性提升的同时逻辑一致性严重滞后，VL-LCM可作为无标注场景下的有效评估与模型选择工具。

---

#### 📘 Detailed Summary
**Motivation:** 传统准确率评估会奖励大型语言模型无根据的猜测，且对于缺乏真实标注的新任务无法进行模型验证，现有评估方法忽略了模型在因果逻辑关系上的推理一致性。

**Method:** 基于基本逻辑原则，提出视觉-语言逻辑一致性度量（VL-LCM）框架，在传统MC-VQA测试和无需真实标注的NaturalBench测试上，从充分因果关系和必要因果关系两个维度评估多模态大模型的逻辑一致性。

**Result:** 在MMMU和NaturalBench等基准上对4个前沿家族共11个开源多模态大模型进行系统评估，发现模型准确率虽有显著提升但逻辑一致性严重滞后；通过VL-LCM与真实标注指标的关联性、LCM可靠性及与响应分布的关系等实验，验证了VL-LCM在无真实标注场景下的有效性与适用性。

**Conclusion:** 逻辑一致性可作为准确率之外衡量模型准确性与可靠性的补充指标，VL-LCM可用于无真实标注场景下的多模态大模型选择、验证及可靠答案合理解释。
