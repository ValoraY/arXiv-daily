...
#### 🧩 TL;DR
本文将视觉文本压缩（VTC）建模为测度传输问题，提出标签无关的路由准则和传输引导的注视机制，在24个NLP数据集上以70.8%的准确率匹配oracle选择，平均任务分数提升3.3%的同时节省10.3%的token。

---

#### 📘 Detailed Summary
**Motivation:** 视觉文本压缩（VTC）通过将文本渲染为图像并用视觉语言模型重新编码，能够大幅减少解码器token数量（3–20倍），但token节省与下游任务效用之间缺乏可预测的对应关系：在某些任务上视觉路径表现优异，在另一些任务上则完全失效，且压缩比本身无法预测哪种情况会发生。现有研究缺少一个原则性的、与任务相关的信息损失度量来解释这种不一致性。

**Method:** 本文将VTC建模为测度传输问题，将文本和视觉token视为经验概率测度，证明ViT补丁编码器诱导了一个推送映射（push-forward map），其传输代价分解为补丁内聚合的精度代价（precision cost）和跨补丁碎片化的覆盖代价（coverage cost），两项均可通过不依赖下游标签的探针估计。基于此提出两个操作化方案：一个无需下游标签的路由准则，用于为每个输入或基准实例选择是否使用视觉路径；一个传输引导的注视机制（transport-informed foveation），对高代价区域以更高分辨率重新编码。

**Result:** 在Qwen3-4B上跨越24个NLP数据集的实验中，所提无标签路由规则在17/24个数据集（70.8%）上匹配了针对每个数据集的oracle选择，平均任务分数提升3.3%，同时相对于纯LLM路径平均节省10.3%的token。

**Conclusion:** 本文揭示了VTC中token效率与下游效用之间不一致的根本原因在于视觉编码引入了可分解的精度代价和覆盖代价，并证明了通过测度传输理论可以构建有效的标签无关路由机制来在实际部署中做出最优路径选择，为视觉语言模型的高效推理提供了理论基础和实用工具。
...
#### 🧩 TL;DR
HumanNet是一个包含100万小时人类中心视频的大规模语料库，通过系统化的数据策展范式将非结构化互联网视频转化为可用于具身学习的可扩展数据基础设施，实验表明基于第一人称人类视频的继续训练效果优于同等规模的真实机器人数据。

---

#### 📘 Detailed Summary
**Motivation:** 具身智能的进步受限于缺乏大规模、多样化且富含标注的人类活动数据，与视觉和语言领域已利用互联网语料库实现规模化不同，物理交互学习的数据基础设施严重不足，现有机器人数据规模有限且成本高昂。

**Method:** HumanNet包含100万小时、涵盖第一人称和第三人称视角的人类中心视频，覆盖细粒度活动、人-物交互、工具使用及长时序行为，并提供包括描述、运动描述及手部和身体相关信号在内的交互中心标注。该数据集引入了一种系统化的具身学习数据策展范式，将人类中心过滤、时间结构化、视角多样性和标注丰富性作为一等设计原则，将非结构化互联网视频转化为可扩展的表示学习、活动理解、运动生成和人到机器人迁移的基础数据。

**Result:** 在固定验证集下，使用HumanNet中1000小时第一人称视频对Qwen VLM模型进行继续训练，其性能超过了使用Magic Cobot中100小时真实机器人数据继续训练的结果，表明第一人称人类视频可成为机器人数据的可扩展且成本低廉的替代品。

**Conclusion:** 该研究表明，以人类中心视频替代机器人专用数据来规模化具身基础模型是可行且高效的路径，通过系统化的数据策展设计可以释放互联网视频在具身学习中的潜力，为具身智能的数据基础设施建设提供了新的方向。
...
### [3] [TRAJGANR: Trajectory-Centric Urban Multimodal Learning via Geospatially Aligned Neural Representations](https://arxiv.org/abs/2605.06990)
*Maria Despoina Siampou, Gengchen Mai, Ni Lao, Jinmeng Rao, Neha Arora, Cyrus Shahabi, Shushman Choudhury*

#### 🧩 TL;DR
TrajGANR 提出了一种以轨迹为中心的多模态自监督学习框架，通过连续轨迹神经表示实现与街景图像及地理位置的细粒度对齐，在多个城市理解任务上超越了现有地理空间基础模型。

---

#### 📘 Detailed Summary
**Motivation:** 现有地理空间多模态自监督学习方法主要针对静态模态对（如卫星图像、街景图像和文本）设计，依赖同一或邻近位置的离散观测对齐，无法处理人类移动轨迹这类连续路径数据。轨迹虽能通过捕捉道路、社区和场所上的人类活动对城市理解至关重要，但在地理空间自监督学习框架中仍未得到充分探索。

**Method:** TrajGANR 学习轨迹在路径上任意点的连续神经表示，实现与附近街景图像的细粒度对齐，即使图像与轨迹路点不共置。基于此能力，提出一个联合对齐轨迹、街景图像及其地理位置三种模态的多模态自监督学习目标。

**Result:** 在四个城市移动和道路理解任务上，TrajGANR 持续优于现有地理空间自监督学习框架及一个轨迹特定基础模型。消融实验表明，所提出的多模态自监督学习目标和多模态学习框架是性能提升的主要驱动因素，验证了细粒度地理空间对齐优于粗粒度聚合，以及地理空间多模态学习的价值。

**Conclusion:** 该研究证明了对连续轨迹进行细粒度神经表示并与静态模态对齐是提升地理空间基础模型性能的有效途径，强调了在轨迹模态中细粒度地理空间对齐和多模态联合学习的重要性，为城市理解和移动性分析提供了新的范式。
...
#### 🧩 TL;DR
LensVLM 提出一种推理框架与训练策略，使视觉语言模型先扫描压缩图像，再通过学习的工具选择性地将相关图像展开为未压缩形式，在 4.3 倍有效压缩下保持与全文本上界相当的精度，并在最高 10.1 倍压缩下超越检索式、文本压缩和视觉压缩基线。

---

#### 📘 Detailed Summary
**Motivation:** 视觉语言模型将文本渲染为图像处理可绕过分词带来的长序列问题，但图像压缩会导致字符低于编码器有效分辨率而难以辨认，精度迅速下降，现有压缩方法在强压缩下无法维持可靠性能。

**Method:** LensVLM 构建在 Qwen3.5-9B-Base 之上，采用两阶段推理：先对压缩图像进行低分辨率扫描，再通过学习到的工具调用选择性地将相关图像展开为原始未压缩形式进行精细阅读，配合后训练微调策略实现压缩鲁棒性。

**Result:** 在 4.3 倍有效压缩下 LensVLM 精度与全文本上界相当，在最高 10.1 倍压缩下在七个文本问答基准上优于检索式、文本压缩和视觉压缩基线；该方法还可泛化至多模态文档与代码理解任务，且压缩越大相对基线优势越明显。

**Conclusion:** 训练使视觉压缩对渲染选择具有鲁棒性，模型在高压缩下更依赖展开内容而非不可靠的视觉阅读；工具选择方面，文本展开更适合渲染文本，而高分辨率图像展开适合保留布局线索的原生文档理解任务。
...
### [5] [Pan-FM: A Pan-Organ Foundation Model with Saliency-Guided Masking for Missing Robustness](https://arxiv.org/abs/2605.07055)
*Qiangqiang Wu, Grace McIlvain, Zhou Yu, Junhao Wen*

#### 🧩 TL;DR
提出Pan-FM，一个跨七个器官（脑、心脏、脂肪、肝脏、肾脏、脾脏、胰腺）的全身基础模型，通过显著性引导掩码（SGM）解决多器官预训练中的主导器官捷径学习偏差，在UK Biobank上实现13种疾病类别和14种单疾病实体的预测性能提升，且对器官缺失场景具有鲁棒性。

---

#### 📘 Detailed Summary
**Motivation:** 现有医学影像基础模型多局限于单模态、单器官数据（如仅脑部MRI），无法捕捉人体衰老与疾病中跨器官的协调生物学过程；同时，真实世界多模态生物医学数据存在非随机的器官缺失问题，导致模型效能下降、泛化能力受限并引入偏差。

**Method:** Pan-FM采用统一骨干网络，设计上支持训练和推理阶段的器官缺失；预训练采用基于掩码的自蒸馏范式；提出显著性引导掩码（Saliency-Guided Masking, SGM），利用模型注意力分布自适应地掩盖主导器官（如脂肪、心脏），从而抑制主导器官捷径学习偏差，促进跨器官的均衡全身表示学习，SGM计算开销极低且可无缝集成到现有自监督学习框架中。

**Result:** 在UK Biobank数据集上，Pan-FM在13种疾病类别和14种单疾病实体的预测任务中均优于单器官和多器官基线模型；在器官缺失场景下表现出更强的鲁棒性。

**Conclusion:** Pan-FM为真实世界中多模态学习面临的结构化模态缺失问题提供了可扩展的解决方案，是迈向更可泛化的全身基础模型的重要一步，对系统神经科学和跨器官疾病建模具有重要启示。
...
### [6] [Learning to Track Instance from Single Nature Language Description](https://arxiv.org/abs/2605.07064)
*Yaozong Zheng, Bineng Zhong, Qihua Liang, Shuimu Zeng, Haiying Xia, Shuxiang Song*

#### 🧩 TL;DR
本文提出了一种自监督的视觉-语言跟踪框架\tracker，通过动态令牌聚合模块实现无需边界框标注的、基于自然语言描述的目标跟踪，在多个VL跟踪基准上超越了现有自监督方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有的视觉-语言跟踪方法严重依赖边界框标注作为监督信号，限制了在大规模无标签视频数据上的可扩展性。本文旨在探索一种不依赖任何边界框真值的自监督VL跟踪范式，解决语言引导跟踪中缺乏标注数据的问题。

**Method:** 提出\tracker自监督VL跟踪器，核心是动态令牌聚合模块。该模块首先基于锚点令牌从模板帧中选出多个重要目标令牌；然后根据注意力分数合并这些目标令牌并聚合到语言令牌中，以消除视觉冗余噪声并增强语义对齐；最后将融合后的语言令牌作为引导信号，从搜索帧中提取潜在目标令牌并传播至后续帧，实现从无标签视频中自主学习的实例跟踪。

**Result:** 在多个VL跟踪基准上的大量实验表明，\tracker在性能上超越了现有的最先进自监督方法，验证了所提动态令牌聚合机制在无边界框标注条件下学习语言引导跟踪表示的有效性。

**Conclusion:** 本文证明自监督范式可以成功应用于视觉-语言跟踪任务，无需大规模边界框标注即可学习有效的语言引导跟踪表示。该工作为减少VL跟踪对人工标注的依赖提供了新思路，并展示了动态非均匀令牌聚合在跨模态对齐中的重要作用。
...
#### 🧩 TL;DR
本文发现VLMs内部存在一个潜在的3D拓扑空间图，但被非几何视觉语义所掩盖；通过跨场景线性特征提取和基于Dirichlet能量的正则化微调，该方法在空间拓扑理解任务上相比标准SFT提升高达12.1%。

---

#### 📘 Detailed Summary
**Motivation:** 尽管现代视觉-语言模型（VLM）展现出从2D自我中心输入中涌现的空间推理能力，但现有研究尚未明确回答VLM是否构建了类似认知地图的3D内部表示，以及这种表示如何被非几何视觉特征所干扰。

**Method:** 通过跨场景线性特征提取，作者从VLM的隐藏状态中分离出一个与3D场景拓扑结构对应的干净空间子空间，并数学上证明该子空间等价于场景3D高斯核图的拉普拉斯特征映射；进一步提出基于Dirichlet能量的潜在空间正则化方法，仅需在简单合成数据上进行500步监督微调即可生效。

**Result:** 实验表明，所提出的潜在正则化方法在真实世界空间基准任务上显著提升了VLM的空间拓扑理解能力，相比标准SFT和竞争基线最高提升12.1%。

**Conclusion:** 该研究揭示了VLM内部确实存在一个可提取、可操控的3D拓扑空间表示，并提供了一个数学上严谨且数据高效的微调范式，为提升VLM在几何空间推理任务上的表现开辟了新路径。
...
### [8] [PRIMED: Adaptive Modality Suppression for Referring Audio-Visual Segmentation via Biased Competition](https://arxiv.org/abs/2605.07154)
*Yuchen He, Jing Zhang*

#### 🧩 TL;DR
PRIMED提出基于认知神经科学中偏竞争理论的自适应模态抑制方法，通过显式建模视觉感知与语言驱动的先验调制，结合令牌蒸馏器与空间感知语义对齐损失，在Ref-AVS基准上达到最优性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有Ref-AVS方法将多模态线索视为同质输入进行融合或推理，缺乏对跨模态相关性的选择性利用，在面对与指代表达或场景不相关、具误导性的模态时表现脆弱，亟需一种能自适应抑制无关模态、突出相关模态的机制。

**Method:** PRIMED受偏竞争理论启发，包含三个核心组件：模态先验解码器（Modality Prior Decoder）估计指代表达主要依赖音频、视觉或两者联合交互，生成模态先验以自适应引导高层注意力；令牌蒸馏器（Token Distiller）从高层特征中提取紧凑全局视觉令牌，跨竞争感知交叉模态融合模块（Competition-aware Cross-modal Fusion）共享以提供层次化全局上下文；空间感知语义对齐损失（Spatial-Aware Semantic Alignment Loss）通过对比学习增强前景-背景判别。

**Result:** 在Ref-AVS基准上的大量实验表明，PRIMED取得了整体最优性能，验证了自适应模态抑制策略在复杂多模态场景下的有效性。

**Conclusion:** 将认知神经科学中的偏竞争理论引入多模态分割任务，通过显式建模模态先验与竞争机制，为处理模态相关性动态变化的场景提供了新思路，未来可在更广泛的多模态理解任务中推广该自适应调制框架。
...
### [9] [See Tomorrow, Act Today: Foresight-Driven Autonomous Driving](https://arxiv.org/abs/2605.07195)
*Bozhou Zhang, Nan Song, Yuang Wang, Jiankang Deng, Xiatian Zhu, Li Zhang*

#### 🧩 TL;DR
ForeSight 提出一种以基础世界模型为中心的端到端自主驾驶规划框架，将未来场景想象作为动作预测的主要驱动力，实现从被动反应式规划到前瞻性决策的范式转变，在 NAVSIM 和 nuScenes 上显著超越现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有端到端自主驾驶规划器本质上是反应式的，仅依靠历史与当前观测来预测未来动作，缺乏类似人类驾驶员在行动前模拟"接下来会发生什么"的主动想象能力，导致在动态交互场景中决策能力受限。

**Method:** ForeSight 采用两阶段框架：第一阶段利用预训练的世界模型生成可信的未来视觉场景；第二阶段在这些想象出的未来场景基础上进行动作规划。该框架将世界模型从辅助组件提升为核心决策引擎，使规划过程从"我现在该做什么"转变为"未来会发生什么，我该如何应对"。

**Result:** 在 NAVSIM 和 nuScenes 基准上的大量实验表明，显式的未来想象策略显著优于之前的最先进方法，验证了前瞻驱动型规划在动态交互场景中的有效性。

**Conclusion:** 该研究揭示了将未来场景预测作为规划核心环节的可行性与优越性，为自主驾驶从反应式走向真正前瞻式决策提供了新的范式，并指出了将基础世界模型嵌入自主系统的新路径。
...
### [10] [Towards multi-modal forgery representation learning for AI-generated video detection and localization](https://arxiv.org/abs/2605.07232)
*Dat Le, Khoa Nguyen, Xin Wang, Shu Hu*

#### 🧩 TL;DR
提出一种多模态架构，联合大语言模型语义分支、时空视觉分支和多尺度部分伪造音频分支，实现对AI生成视频（包括局部篡改）的检测与细粒度时间定位，性能超越现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有AI生成视频检测器大多局限于单模态或部分模态数据建模，且缺乏对细粒度时间伪造定位的能力，难以应对包含视觉和音频通道局部篡改的AI生成视频所带来的语义失真与滥用风险。

**Method:** 核心架构创新性地联合三个分支：大语言模型（LMM）语义分支用于高层语义理解、时空（ST）视觉分支用于空-时特征提取、以及多尺度部分伪造（PS）音频分支用于音频级篡改分析，三者协同实现多模态联合建模与细粒度时间伪造定位。

**Result:** 广泛的实验结果表明，所提出的多模态方法在AI生成视频的检测与局部分类任务上全面优于现有最先进方法，验证了联合语义、视觉和音频多模态信息对提升检测与定位性能的有效性。

**Conclusion:** 该研究揭示了多模态联合建模对于检测AI生成视频（尤其是局部篡改视频）的重要性，提出的架构为细粒度时间伪造定位提供了新范式，有助于推动视频取证领域向更全面、更鲁棒的检测方向发展。
...
### [11] [Sat3R: Satellite DSM Reconstruction via RPC-Aware Depth Fine-tuning](https://arxiv.org/abs/2605.07264)
...
#### 🧩 TL;DR
Sat3R通过RPC感知的度量深度微调（基于Depth Anything V2和SiLog损失），将单目深度基础模型适配到卫星影像域，无需逐场景优化即可实现与优化方法相当的DSM重建精度，同时获得超过300倍的加速。

---

#### 📘 Detailed Summary
**Motivation:** 现有卫星影像DSM重建方法面临根本性权衡：优化方法精度高但每场景需数小时计算，而泛化几何基础模型推理快但无法泛化到卫星影像，原因在于RPC模型引入的域差距和深度尺度分布不匹配。

**Method:** 提出Sat3R前馈框架，通过对Depth Anything V2进行RPC感知的度量深度微调（使用尺度不变对数SiLog损失），利用从RPC几何构造的物理一致的伪深度监督，使单目深度基础模型适应卫星域，无需任何逐场景优化。

**Result:** 在DFC2019基准上，Sat3R相比零样本前馈基线降低MAE 38%，达到与优化方法竞争的精度，同时实现超过300倍的推理加速。

**Conclusion:** 证明前馈模型在适当适配卫星域后，能够以极低计算成本匹配优化方法的DSM重建精度，为大规模实用化卫星DSM重建铺平道路。
...
### [12] [From Clouds to Hallucinations: Atmospheric Retrieval Hijacking in Remote Sensing Vision-Language RAG](https://arxiv.org/abs/2605.07273)
...
#### 🧩 TL;DR
CloudWeb提出了一种针对遥感多模态RAG系统的检索劫持攻击，通过在输入图像上叠加参数化的云/雾模式并优化检索导向目标函数，实现了在保持检索器、生成器和知识库不变的情况下将对抗性图像嵌入拉向目标气象证据，实验表明该方法在多个CLIP风格检索器上显著提升了气象相关证据的TOP-K命中率并导致下游生成出现气象幻觉。

---

#### 📘 Detailed Summary
**Motivation:** 现有对抗性研究主要针对RAG系统的检索语料库或记忆进行操纵，或直接攻击视觉语言模型和遥感模型的最终预测任务，而针对遥感多模态RAG系统在证据检索阶段的输入空间攻击尚未被探索，存在研究空白。

**Method:** CloudWeb在输入遥感图像上叠加参数化的云状和雾状图案，并通过一个检索导向的目标函数进行优化，该函数同时包含多个约束项：将对抗性图像嵌入拉向目标大气证据、抑制源场景证据、强制排序分离以及正则化自然性和覆盖度，攻击过程中检索器、生成器和知识库均保持固定。

**Result:** 在包含七个数据集的遥感RAG基准测试中，针对GeoRSCLIP、RemoteCLIP、OpenAI CLIP和OpenCLIP等五种CLIP风格检索器，CloudWeb在注入气象相关证据到排名结果方面一致优于干净检索、手工大气基线、随机云扰动和固定变体等对比方法，其中在GeoRSCLIP ViT-B/32上Weather@5从0.71%提升至43.29%，且下游生成出现可观测的气象幻觉和语义漂移。

**Conclusion:** 该研究揭示了遥感多模态RAG系统存在一种实用的失败模式：自然外观的大气变化可以在下游生成开始之前就破坏证据检索的可靠性，且检索阶段的劫持会传播至最终RAG响应，这对遥感RAG系统的安全性评估和防御提出了新的挑战。
...
### [13] [Amortized-Precision Quantization for Early-Exit Vision Transformers](https://arxiv.org/abs/2605.07317)
*Rui Fang, Hsi-Wen Chen, Ming-Syan Chen*

#### 🧩 TL;DR
提出了一种名为MAQEE的双层量化框架，通过联合优化退出阈值和位宽，在显式风险控制下提高视觉Transformer动态推理的稳定性，在准确率-效率权衡中建立了更优的Pareto前沿，BOPs最高降低95%，准确率提升最高达20%。

---

#### 📘 Detailed Summary
**Motivation:** 视觉Transformer在低精度早期退出场景下部署时性能脆弱，现有量化方法假设全深度静态执行，未考虑量化噪声扰动退出决策时沿动态推理路径放大误差的问题，缺乏对深度-精度权衡的系统性建模。

**Method:** 首先提出摊销精度量化（APQ），一种利用率感知的建模方法，从理论上刻画逐层对量化噪声的随机暴露程度并揭示深度-精度权衡关系；在此基础上提出MAQEE双层框架，联合优化退出阈值和位宽，在显式风险控制下同时提升推理稳定性与效率。

**Result:** MAQEE在准确率-效率权衡上建立了更优的Pareto前沿，相比强基线方法，在分类、检测和分割任务中BOPs最多降低95%的同时保持准确率，性能提升最高达20%。

**Conclusion:** 通过将量化噪声与早期退出动态决策耦合建模，揭示了深度-精度之间的内在权衡关系，MAQEE框架证明了对量化扰动具有鲁棒性的动态推理路径可控性是实现高效视觉Transformer部署的关键。
...
### [14] [ShellfishNet: A Domain-Specific Benchmark for Visual Recognition of Marine Molluscs](https://arxiv.org/abs/2605.07338)
*Ziheng Zhou, Yang Wang, Nan Wang, Chengliang Wu, Jun Yan*

#### 🧩 TL;DR
提出ShellfishNet基准数据集（8,691张图像，32个分类单元），系统评估了80种视觉模型（CNN、ViT、SSM、SSL）在真实水下环境下的细粒度分类与鲁棒性表现，并引入图像损坏测试模拟水下退化场景，为底栖生物智能监测提供数据基础与评估标准。

---

#### 📘 Detailed Summary
**Motivation:** 全球贝类生物多样性下降对沿海生态系统构成严重威胁，尽管AI技术在自动化生态监测中展现出潜力，但现有海洋底栖数据集缺乏对真实水下环境复杂性（如光照变化、物种姿态多样）的适配，导致视觉模型在实际生态监测中的鲁棒泛化能力不足。

**Method:** 构建了ShellfishNet基准数据集，包含8,691张涵盖32个分类单元的图像，通过实地拍摄与网络爬取采集，并包含带描述性标注的精选子集；在此基础上系统评估了80种代表性神经网络模型，涵盖CNN、ViT、SSM及自监督学习方法，同时评估了细粒度视觉分类（FGVC）模型和主流多模态大语言模型（MLLM）的图像描述能力，并引入图像损坏基准测试以模拟水下退化场景（如浑浊、恶劣天气）。

**Result:** 基于ShellfishNet基准，系统评估了80种视觉模型在真实水下环境中的表现，涵盖多种架构（CNN、ViT、SSM、SSL），并测试了FGVC模型和MLLM的图像描述能力；图像损坏基准测试揭示了不同模型在模拟水下退化场景（浑浊、恶劣天气）下的鲁棒性差异，为可信生态保护决策提供了依据。

**Conclusion:** ShellfishNet为底栖生物的智能监测提供了专门适配真实水下环境的数据基础与模型评估基准，其引入的图像损坏测试有助于评估视觉模型在野外环境下的鲁棒性，推动AI在生态保护中的可信应用。
...
### [15] [TTF: Temporal Token Fusion for Efficient Video-Language Model](https://arxiv.org/abs/2605.07355)
*Simin Huo, Ning LI*

#### 🧩 TL;DR
提出时序令牌融合（TTF），一种训练无关的即插即用令牌压缩框架，通过局部窗口相似性搜索融合冗余视觉令牌，在去掉约67%视觉令牌的同时保留99.5%的基线精度，显著加速视频-语言模型推理。

---

#### 📘 Detailed Summary
**Motivation:** 视频-语言模型（VLM）面临推理成本随视频长度快速增长的瓶颈问题，例如Qwen3-VL在32帧448×448分辨率下已产生超过8000个视觉令牌，导致LLM预填充阶段成为吞吐量主要瓶颈，而现有压缩方法依赖全局相似性或注意力引导，引入额外开销抵消了压缩收益。

**Method:** TTF是一种训练无关、即插即用的令牌压缩框架，运行于LLM预填充之前。它自动选择一个锚帧，然后对后续每一帧通过局部窗口（如3×3）相似性搜索，将超过阈值的令牌融合为单一令牌，并通过坐标对齐保持预填充和解码阶段的位置一致性，无缝集成现有VLM流水线。

**Result:** 在Qwen3-VL-8B模型上，设置阈值t=0.70时，TTF移除了约67%的视觉令牌，保留了99.5%的基线准确率，仅引入约0.16 GFLOPs的匹配开销，实现了精度与效率的有效平衡。

**Conclusion:** TTF通过利用视频中结构性时序冗余，提供了一种实用高效的视频理解加速方案，其训练无关、即插即用的特性使其易于集成到现有VLM流水线中，为长视频场景下的高效推理提供了可行方向。
...
### [16] [UniD-Shift: Towards Unified Semantic Segmentation via Interpretable Share-Private Multimodal Decomposition](https://arxiv.org/abs/2605.07356)
*Shuai Zhang, Zhecheng Shi, Zhuxiao Li, Jing Ou, Tengxi Wang, Yuan Liu, Wufan Zhao*

#### 🧩 TL;DR
提出UniD-Shift统一多模态框架，通过显式分解2D-3D共享与私有特征子空间实现跨模态对齐，在SemanticKITTI和nuScenes基准上取得一致改进，并展现出跨域泛化能力。

---

#### 📘 Detailed Summary
**Motivation:** 大规模3D点云语义分割中，LiDAR的稀疏采样模式与图像观测中的视角相关几何畸变导致跨模态对齐困难，阻碍了2D图像与3D点云特征的稳定融合，现有方法难以充分挖掘两种模态间的共享语义与各自独有信息。

**Method:** 提出UniD-Shift统一多模态框架，结合SAM视觉编码器与SPTNet几何编码器分别提取语义与几何特征；将双模态特征显式分解为共享子空间和私有子空间，共享成分总结跨域公共语义因子，私有成分保留模态特有属性；采用轻量级注意力融合模块聚合共享特征为一致跨模态表示，并设计正则化训练目标同时保证语义对齐与子空间独立性。

**Result:** 在SemanticKITTI和nuScenes基准上，该方法相较于代表性多模态基线在分割精度上取得一致提升，且计算效率具有竞争力；在nuScenes USA-Singapore跨域评估中，该方法在分布偏移下性能稳定，展现出强泛化能力。

**Conclusion:** 该研究揭示了2D与3D语义分割中共享与私有特征分解的有效性，验证了显式跨模态对齐与子空间正则化可显著提升多模态分割性能与跨域鲁棒性，为自动驾驶等场景下的点云-图像融合提供了新思路。
...
### [17] [UniISP: A Unified ISP Framework for Both Human and Machine Vision](https://arxiv.org/abs/2605.07359)
*Hanxi Li, Yao Cheng, Bo Zhang, Li Zeng*

#### 🧩 TL;DR
UniISP提出了一种统一的ISP框架，通过混合注意力模块和特征适配器，同时满足人类视觉感知和计算机视觉任务的需求，在多个数据集上取得了最先进的性能。

---

#### 📘 Detailed Summary
**Motivation:** 传统ISP流水线为人类视觉生成美观的RGB图像，但其中的压缩和损失操作会破坏信息完整性，不利于下游视觉任务；而现有直接利用raw数据的方法要么整合了最小化ISP处理导致图像难以可视化，要么生成的图像不符合人类审美偏好。因此，需要一种能够同时满足人类视觉感知和计算机视觉应用需求的统一ISP框架。

**Method:** UniISP框架包含两个关键模块：一是精心设计的混合注意力模块（Hybrid Attention Module, HAM），通过监督学习确保生成图像在视觉上美观；二是特征适配器（Feature Adapter）模块，用于将ISP阶段的丰富特征有效地传播到后续下游网络中，从而兼顾视觉质量与任务性能。

**Result:** 在多种场景和多个数据集上的大量实验表明，UniISP达到了最先进的性能水平，验证了其泛化能力和有效性。

**Conclusion:** 该研究证明，通过合理设计注意力机制和特征适配策略，ISP框架可以同时服务于人类视觉感知和计算机视觉任务，为raw数据在视觉识别中的高效利用提供了新的思路和框架。
...
### [18] [Sparse Autoencoders as Plug-and-Play Firewalls for Adversarial Attack Detection in VLMs](https://arxiv.org/abs/2605.07447)
*Hao Wang, Yiqun Sun, Pengfei Wei, Lawrence B. Hsieh, Daisuke Kawahara*

#### 🧩 TL;DR
提出基于稀疏自编码器（SAE）的轻量级对抗攻击检测框架SAEgis，通过在预训练VLM中插入SAE模块，利用学习到的稀疏潜在特征区分对抗扰动图像与正常图像，在无需对抗训练的条件下实现跨领域与跨攻击场景下的鲁棒检测。

---

#### 📘 Detailed Summary
**Motivation:** 视觉-语言模型（VLM）在现实应用尤其是智能体系统中部署日益广泛，但其安全性问题受到相对较少的关注，现有的闭源与开源VLM仍高度易受对抗攻击威胁，下游应用面临显著风险。

**Method:** 提出SAEgis框架，在预训练VLM中插入稀疏自编码器（SAE）模块，通过标准重构目标训练SAE，利用其学习到的稀疏潜在特征中自然捕获的攻击相关信号，对输入图像是否受到对抗扰动进行可靠分类，支持多层信号融合以增强鲁棒性。

**Result:** 实验结果表明，SAEgis在域内、跨域和跨攻击场景下均表现出强检测性能，尤其在跨域泛化方面相比现有基线有显著提升，多层信号融合进一步改善了检测的鲁棒性和稳定性。

**Conclusion:** 该工作是首个将SAE作为即插即用机制用于VLM对抗攻击检测的研究，方法无需额外对抗训练、引入极小开销，为提升现实VLM系统安全性提供了一种实用方案。
...
### [19] [RELO: Reinforcement Learning to Localize for Visual Object Tracking](https://arxiv.org/abs/2605.07379)
*Xin Chen, Chuanyu Sun, Jiao Xu, Houwen Peng, Dong Wang, Huchuan Lu, Kede Ma*

#### 🧩 TL;DR
RELO提出了一种基于强化学习的视觉目标跟踪定位方法，将目标定位建模为马尔可夫决策过程，用策略网络替代手工设计的空间先验，在多个基准上取得领先性能，证实了奖励驱动定位可作为先验驱动定位的有效替代方案。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉目标跟踪器依赖手工设计的空间先验（如热力图）进行目标定位，这些先验仅提供代理监督信号，与跟踪优化目标和评价指标（如IoU和AUC）之间存在严重不对齐，限制了跟踪性能的提升。

**Method:** RELO将目标定位形式化为马尔可夫决策过程，通过强化学习在空间位置上学习定位策略，取代传统的手工空间先验；奖励函数结合了帧级IoU和序列级AUC；同时引入层对齐的时间令牌传播机制以提升帧间语义一致性，且计算开销可忽略不计。

**Result:** 在多个跟踪基准上，RELO取得了优越的结果，在LaSOText基准上无需模板更新即可达到57.5%的AUC，显著优于依赖手工空间先验的传统方法。

**Conclusion:** 该研究证实，奖励驱动的定位方法能够有效替代传统先验驱动的定位方法，为视觉目标跟踪提供了一条新的范式路径，表明强化学习与跟踪评价指标的直接对齐是提升跟踪性能的关键方向。
...
### [20] [InterLV-Search: Benchmarking Interleaved Multimodal Agentic Search](https://arxiv.org/abs/2605.07510)
*Bohan Hou, Jiuning Gu, Jiayan Guo, Ronghao Dang, Sicong Leng, Xin Li, Xuemeng Song, Jianfei Yang*

#### 🧩 TL;DR
InterLV-Search is a benchmark for interleaved language-vision agentic search where textual and visual evidence recursively conditions subsequent search, containing 2,061 samples across three levels. Experiments show current multimodal agents perform poorly, with the best model achieving under 50% accuracy.

---

#### 📘 Detailed Summary
**Motivation:** Existing multimodal agentic search benchmarks treat visual evidence either as static input or as final answer endpoints, lacking the capability to evaluate interleaved search trajectories where textual and visual evidence recursively informs later search steps. This gap prevents systematic assessment of agents' abilities in visual evidence seeking, search control, and multimodal evidence integration.

**Method:** The benchmark spans three difficulty levels: Level 1 (active visual evidence seeking), Level 2 (controlled offline interleaved multimodal search), and Level 3 (open-web interleaved multimodal search), additionally incorporating multimodal multi-branch samples involving cross-entity comparison. Levels 1 and 2 are constructed via automated pipelines, while Level 3 uses a machine-led, human-supervised open-web pipeline, complemented by InterLV-Agent for standardized tool use, trajectory logging, and evaluation.

**Result:** Evaluation of proprietary and open-source multimodal agents on InterLV-Search reveals that current systems remain far from solving interleaved multimodal search, with the best-performing model achieving below 50% overall accuracy, underscoring significant deficiencies in visual evidence seeking, search control, and multimodal evidence integration.

**Conclusion:** InterLV-Search exposes critical limitations of existing multimodal agents in handling interleaved search trajectories, highlighting the need for improved architectures and training paradigms that can recursively integrate textual and visual evidence throughout the search process, and provides a standardized evaluation framework to drive future progress.
...
### [21] [Exposing and Mitigating Temporal Attack in Deepfake Video Detection](https://arxiv.org/abs/2605.07398)
*Zheyuan Gu, Minghao Shao, Zhen Wang, Yusong Wang, Mingkun Xu, Shijie Zhang, Hao Jiang*

#### 🧩 TL;DR
本文提出SpInShield，一种时间频谱不变的深度伪造检测防御框架，通过可学习频谱对抗器与捷径抑制优化策略，解耦语义运动与可操纵频谱伪影，在频谱攻击下AUC比最强基线提升21.30个百分点。

---

#### 📘 Detailed Summary
**Motivation:** 现有时空深度伪造检测器虽然AUC表现优异，但实验表明其极易受到规避攻击，根本原因在于模型过度依赖脆弱的时序频谱线索而非学习鲁棒的语义因果关系，因此亟需一种能抵御频谱层面对抗攻击的鲁棒检测方法。

**Method:** SpInShield提出了一种可学习的频谱对抗器，能够动态合成严重的频谱形变以模拟极端攻击场景；同时采用捷径抑制优化策略，迫使编码器提取可靠的取证线索并从隐空间中剔除不稳定的频谱统计量，从而将语义运动与可操纵的频谱伪影进行解耦。

**Result:** 实验表明，SpInShield在多个广泛使用的数据集上取得了竞争力强的检测性能，并且在模拟振幅频谱攻击下，其AUC比最强基线方法高出21.30个百分点，展现了显著的鲁棒性优势。

**Conclusion:** 该研究揭示了深度伪造检测器对时序频谱线索的过拟合脆弱性，并证明了通过解耦语义运动与频谱伪影、结合对抗训练与表征净化策略可有效提升模型在对抗环境下的鲁棒性，为可信赖的伪造检测提供了新思路。
...
### [22] [GazeVLM: Active Vision via Internal Attention Control for Multimodal Reasoning](https://arxiv.org/abs/2605.07817)
*Brown Ebouky, Gabriele Carrino, Niccolo Avogaro, Christoph Studer, Andrea Bartezzaghi, Mattia Rigotti*

#### 🧩 TL;DR
GazeVLM提出了一种内化元认知注意力控制的视觉语言模型架构，通过自主生成凝视令牌（\texttt{<LOOK>}）建立自顶向下的因果注意力掩码控制机制，在无需外部裁剪工具或扩展上下文窗口的前提下，实现全局场景感知与局部聚焦推理的流畅切换，在HRBench-4k和HRBench-8k基准上超越同参数级别的最新VLM近4%。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉语言模型（VLM）被动处理视觉信息，依赖静态积累的大量令牌上下文，这稀释了空间推理能力并引发语言幻觉，缺乏人类主动视觉中元认知控制驱动的自上而下目标导向注意力机制。

**Method:** GazeVLM是一种多模态架构，使VLM能够自主生成凝视令牌（\texttt{<LOOK>}），建立对自身因果注意力掩码的自顶向下控制机制，通过持续抑制偏置动态地抑制无关视觉特征以实现空间选择性注意力并模拟中央凹注视，局部推理结束后偏置解除以恢复全局视图，并采用定制化的组相对策略优化（GRPO）程序进行训练以奖励有效定位。

**Result:** 4B参数的GazeVLM在HRBench-4k和HRBench-8k基准上，超越了同参数级别的最新VLM近4%，并比基于思维链图像构建的智能体多模态管线高出超过5%。

**Conclusion:** GazeVLM通过将元认知注意力控制直接内化到推理循环中，为VLM实现主动视觉提供了一种无需外部工具或膨胀上下文窗口的新范式，显著提升了高分辨率多模态推理性能。
...
### [23] [GPO-V: Jailbreak Diffusion Vision Language Model by Global Probability Optimization](https://arxiv.org/abs/2605.07399)
*Yu Pan, Andi Zhang, Yi Wang, Sibei Yang, Wenjie Wang*

#### 🧩 TL;DR
本文首次揭示了基于非自回归扩散机制的视觉-语言模型（dVLMs）在面对传统越狱攻击时表现出的虚假鲁棒性，并提出了一种针对扩散模型去噪轨迹的新型越狱范式——全局概率优化（GPO）及其视觉模态扩展GPO-V，实验表明该方法能够生成具有高跨模型迁移性的隐蔽扰动，揭示了扩散生成架构中存在关键安全漏洞。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究表明扩散视觉-语言模型（dVLMs）似乎对传统越狱攻击（如固定前缀优化FPO）具有天然鲁棒性，但这种鲁棒性是虚假的。作者发现dVLMs存在独特的拒绝模式（即时拒绝与渐进式拒绝），且渐进式精炼过程本身暴露出一种潜在的对抗攻击面，亟需系统性地揭示并利用这一安全漏洞。

**Method:** 作者首先将传统越狱攻击归纳为固定前缀优化（FPO）范式，并分析了dVLMs的拒绝模式。在此基础上，提出了全局概率优化（GPO）——一种针对掩码扩散模型去噪轨迹的通用越狱范式，通过操纵全局生成动态而非前缀来绕过安全护栏。进一步地，作者将GPO扩展为GPO-V，这是首个专门针对dVLMs的视觉模态越狱框架。

**Result:** 实验结果表明，GPO-V能够生成具有高度隐蔽性的视觉扰动，并且展现出卓越的跨模型迁移能力，在非序列生成架构中成功揭示了关键的安全缺口。

**Conclusion:** 该研究强调了在dVLMs中解决安全对齐问题的紧迫性，指出当前基于自回归范式的防御机制无法有效防御针对扩散模型的越狱攻击，需要从根本上重新评估和设计面向扩散生成架构的安全防御范式。
...
### [24] [EditRefiner: A Human-Aligned Agentic Framework for Image Editing Refinement](https://arxiv.org/abs/2605.07457)
...
#### 🧩 TL;DR
本文构建了细粒度人工反馈数据集 EditFHF-15K，并提出 EditRefiner 层次化智能体框架，将图像编辑后修正重塑为类人的感知-推理-行动-评估循环，在失真定位、诊断准确性和人类感知对齐方面显著超越现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有文本引导图像编辑模型虽进展显著，但编辑图像仍常出现物体不自然、光照不匹配和意外变化等细粒度问题；已有的精炼方法要么依赖高代价的迭代重生成，要么使用空间定位能力弱的视觉-语言模型，导致语义漂移和不可靠的局部修正。

**Method:** 首先构建 EditFHF-15K 数据集，包含来自 12 个 TIE 模型在 43 个编辑任务上的 15K 图像、60K 标注伪影区域、80K 编辑失败区域及对应的文本推理、45K 平均意见评分；在此基础上提出 EditRefiner，一个层次化、可解释、与人类对齐的智能体框架，包含四个智能体：感知智能体检测伪影和编辑失败的上下文显著图，推理智能体进行类人诊断推断，行动智能体规划并执行局部重编辑，评估智能体评估结果并决定是否需要进一步精炼。

**Result:** 大量实验表明，EditRefiner 在失真定位、诊断准确性和人类感知对齐方面持续优于现有最先进方法，建立了自纠正且感知可靠的图像编辑新范式。

**Conclusion:** EditRefiner 通过构建细粒度人工反馈数据集和类人智能体循环，有效解决了编辑后图像的细粒度质量问题，为可解释、可自纠正的图像编辑后修正提供了新的技术范式，代码已开源。
...
### [25] [ForgeVLA: Federated Vision-Language-Action Learning without Language Annotations](https://arxiv.org/abs/2605.07474)
*Yuhao Zhou, Yunpeng Zhu, Yang Zhou, Jindi Lyu, Jian Lan, Zhangyuan Wang, Dan Si, Thomas Seidl, Qing Ye, Jiancheng Lyu*

#### 🧩 TL;DR
ForgeVLA提出了一种联邦学习框架，用于从分布式的、无标注的视觉-动作数据中训练VLA模型，通过嵌入指令分类器恢复缺失的语言模态，并设计对比规划损失和自适应聚合策略缓解视觉-语言特征坍塌问题，显著优于现有基线。

---

#### 📘 Detailed Summary
**Motivation:** VLA模型的规模化训练严重受限于人工标注数据的高昂成本，而实际部署的机器人虽能产生大量视觉-动作数据，但这些数据因隐私、监管等约束无法集中聚合，且存在严重的异构性（如不同的任务、环境、机器人形态），同时缺失语言模态的标注，导致难以直接用于VLA训练。

**Method:** ForgeVLA采用联邦学习范式，使各客户端在本地训练VLA模型而不共享原始数据；每个客户端配备一个具身指令分类器，将视觉-动作对映射到预定义的指令集以自动恢复语言模态，从而构建完整的视觉-语言-动作三元组；针对视觉-语言特征坍塌这一关键问题，提出客户端侧的对比规划损失（contrastive planning loss）与服务端侧的自适应聚合策略，以学习任务判别性表示并实现高效聚合。

**Result:** 在多个基准测试上的实验表明，ForgeVLA显著优于其他基线方法；消融实验进一步验证了具身指令分类器、对比规划损失和自适应聚合策略各自对整体性能的贡献。

**Conclusion:** ForgeVLA证明了在无需集中原始数据和人工标注的条件下，通过联邦学习从分布式异构视觉-动作数据中高效训练VLA模型的可行性，为解决机器人学习中数据规模化瓶颈提供了新思路，并揭示了视觉-语言特征坍塌在联邦VLA场景中的关键影响。
...
### [26] [How Far Is Document Parsing from Solved? PureDocBench: A Source-TraceableBenchmark across Clean, Degraded, and Real-World Settings](https://arxiv.org/abs/2605.07492)
...
#### 🧩 TL;DR
PureDocBench 是一个程序化生成、来源可追溯的文档解析基准，通过 HTML/CSS 渲染文档图像并生成可验证标注，覆盖 66 个子类别共 4,425 张图像；在 40 个模型上的评估表明，文档解析远未解决（最佳模型约 74 分），小参数专用解析器可匹敌大模型，但公式识别是共同瓶颈，且真实退化下通用 VLM 与专用解析器的排名反转使仅基于干净数据的评估具有误导性。

---

#### 📘 Detailed Summary
**Motivation:** 现有的文档解析基准 OmniDocBench 主要依赖 1,355 页的人工标注数据集，顶部得分已饱和超过 90%；经三阶段审计发现其 21,353 个评估块中有 2,580 个错误（12.08%），且经过一年多的公开，标注质量与污染风险使排名可信度受损，亟需更可靠、可追溯的基准。

**Method:** PureDocBench 通过 HTML/CSS 程序化生成文档图像，并从同一来源产生可验证的标注，覆盖 10 个领域、66 个子类别、1,475 页，每页包含干净、数字退化、真实退化三个版本共 4,425 张图像；在此基础上评估了 40 个模型，涵盖管道专用解析器、端到端专用解析器和通用视觉语言模型三类。

**Result:** 最佳模型得分仅约 74/100，最强与最弱模型间差距达 44.6 分；参数量不超过 4B 的专用解析器可匹敌或超越规模大 5-100 倍的通用 VLM，但公式识别是共同瓶颈（所有模型在三类退化上平均公式识别得分均未超过 67%）；通用 VLM 在数字/真实退化下仅损失 0.99/8.52 总体分，而管道专用解析器损失达 4.90/14.21，导致排名反转。

**Conclusion:** 文档解析任务远未解决，小参数量专用模型在干净场景下具有竞争力，但公式识别是关键短板；仅基于干净数据的评估会误导实际部署选择，因为不同模型对退化的鲁棒性差异巨大，考虑退化情况的评估对真实场景应用至关重要。
...
### [27] [DIMoE-Adapters: Dynamic Expert Evolution for Continual Learning in Vision-Language Models](https://arxiv.org/abs/2605.07494)
*Mengxin Qin, Xiang Zhang, Xi Wang, Kun Wei, Xu Yang, Cheng Deng*

#### 🧩 TL;DR
提出DIMoE-Adapters框架，通过动态增量混合专家适配器（包含自校准专家演化和原型引导专家选择）来平衡视觉-语言模型在多领域任务增量学习中的稳定性与可塑性，在多种设置下超越现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有持续学习方法在多领域任务增量学习中面临大领域偏移加剧的稳定性-可塑性困境，大多数方法依赖固定架构和静态参数分配，限制了模型对新领域的适应能力并加剧了灾难性遗忘。

**Method:** 提出DIMoE-Adapters框架，核心是一种动态专家演化范式，包含两个协同组件：自校准专家演化（SCEE）通过专家优化动力学构建并演化稀疏专家池，提升可塑性的同时减少冗余容量；原型引导专家选择（PGES）基于SCEE塑造的专家池控制专家利用率，增强在面对已见和未见任务时的稳定性。

**Result:** 大量实验表明，DIMoE-Adapters在多种设置下均优于此前的最先进方法，验证了其在多领域任务增量学习中的有效性。

**Conclusion:** 动态专家演化机制能够有效平衡持续学习中的稳定性与可塑性，通过稀疏专家池的构建与自适应选择，在不增加冗余容量的前提下提升了模型对新旧任务的泛化能力。
...
### [28] [Hierarchical Dual-Subspace Decoupling for Continual Learning in Vision-Language Models](https://arxiv.org/abs/2605.07512)
*Mengxin Qin, Xiang Zhang, Kun Wei, Xu Yang, Cheng Deng*

#### 🧩 TL;DR
提出HDSD框架，通过分层双子空间解耦机制将参数空间分解为通用与任务特定子空间，结合通用融合模块和层次学习模块减少子空间干扰，在视觉语言模型的类增量学习中达到最先进性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有类增量学习方法主要通过限制参数更新来缓解灾难性遗忘，但忽视了参数在高维空间中的结构特性；不同任务产生的更新往往位于多个重叠的低秩子空间中，导致跨任务子空间干扰和严重遗忘。

**Method:** 提出分层双子空间解耦（HDSD）框架，包含三个关键组件：轻量级特征调制模块（FMM）将参数空间显式分解为通用与任务特定子空间；通用融合模块（GFM）评估跨任务参数相对变化并采用自适应阈值捕获稳定可迁移知识；层次学习模块（HLM）通过奇异值分解（SVD）进行结构化参数分解并利用缩放机制约束不同子空间尺度内的更新。

**Result:** 在多个常规基准数据集上的广泛实验表明，HDSD方法在类增量学习任务上达到了最先进（state-of-the-art）的性能表现。

**Conclusion:** 通过显式解耦通用与任务特定子空间并约束子空间尺度内的参数更新，HDSD有效减少了子空间干扰和参数漂移，为视觉语言模型中的持续学习提供了新的结构化参数管理范式。
...
### [29] [Probabilistic Object Detection with Conformal Prediction](https://arxiv.org/abs/2605.07549)
*Christopher Ries, Moussa Kassem Sbeyti, Nicolas Bianco, Nadja Klein*

#### 🧩 TL;DR
本文系统比较了无缩放和缩放共形预测在多类目标检测中的应用，通过结合基于损失衰减的概率检测器提供的偶然不确定性估计来缩放共形区间，并扩展至两阶段流水线（RAPS分类集+共形化边界框），在三个自动驾驶数据集上实现最高19%的IoU提升和39%的区间分数下降，且不牺牲覆盖率。

---

#### 📘 Detailed Summary
**Motivation:** 尽管共形预测（CP）为安全关键的目标检测提供了分布无关的有限样本覆盖保证，但目标检测涉及结构化多输出预测，经典CP理论难以直接应用；同时，标准无缩放CP产生固定宽度的预测区间，对低不确定性预测过于保守，而缩放CP虽能自适应调整区间宽度，但此前缺乏对两类CP在多类目标检测中的系统比较，也未将CP与互补的不确定性量化方法进行集成。

**Method:** 本文提出：(i) 对边界框角点进行坐标级共形预测，并采用Bonferroni校正以获得框级覆盖保证；(ii) 利用基于损失衰减的概率目标检测器提供的逐预测偶然不确定性估计，对共形区间进行缩放，并评估未校准及两种校准变体；(iii) 扩展为两阶段流水线：先用RAPS构建分类预测集，再在此基础上对共形化边界框进行条件化处理。

**Result:** 在KITTI、BDD、CODA三个自动驾驶数据集（含跨域分布偏移场景）上，缩放CP相比无缩放CP持续提升了区间锐利度：IoU提升最高达19%，区间分数降低最高达39%，且未牺牲覆盖保证。类别级校准进一步改善了两种CP变体的覆盖性能，同时对锐利度影响极小。

**Conclusion:** 缩放共形预测结合偶然不确定性估计能显著提升目标检测中预测区间的适应性与实用性，类别级校准可作为补充手段优化覆盖性能，这些改进共同为实时、真实场景中的目标检测提供了更具可操作性的不确定性估计。
...
### [30] [Beyond GSD-as-Token: Continuous Scale Conditioning for Remote Sensing VLMs](https://arxiv.org/abs/2605.07562)
*Song Zhang, Yanlong Chen, Yilin Li, Yining Chen, Zili Yi, Xiaowei Zhang, Yawei Li*

#### 🧩 TL;DR
ScaleEarth基于Qwen3-VL提出CS-HLoRA参数高效微调框架，将GSD作为连续条件变量动态调控LoRA子空间，配合SSE-U不确定性估计头和GeoScale-VQA规模分层数据集，在遥感基准上取得最优结果。

---

#### 📘 Detailed Summary
**Motivation:** 遥感视觉-语言模型面临一个根本性失配：同一地理目标在不同地面采样距离下呈现截然不同的视觉证据，而现有RS-VLM大多丢弃GSD信息或仅将其作为离散文本token注入，迫使单一静态参数集吸收整个尺度谱系，缺乏对物理尺度的显式、连续建模能力。

**Method:** ScaleEarth以Qwen3-VL为骨干，提出CS-HLoRA（Continuous Scale-Conditioned Hyper-LoRA），通过GSD驱动的门控机制调制LoRA低秩子空间，实现按物理尺度动态路由计算；同时提出SSE-U轻量异方差子头，从视觉特征中预测GSD及其不确定性以消除部署时对传感器元数据的依赖；构建GeoScale-VQA（150万样本）规模分层遥感VQA语料库，其问答生成由与CS-HLoRA相同的物理标量驱动，形成方法与数据的闭环。

**Result:** 在8B骨干上采用QLoRA训练，ScaleEarth在覆盖多样地球系统任务的遥感基准（包括XLRS-Bench和OmniEarth-Bench）上取得了最优结果（state-of-the-art）。

**Conclusion:** 该研究表明将物理尺度作为连续条件变量融入模型计算路径能有效提升RS-VLM在跨尺度遥感任务上的泛化能力，所提出的方法-数据闭环框架为遥感基础模型的尺度感知微调提供了新范式，且SSE-U设计对部署场景下元数据缺失问题具有实用价值。
...
### [31] [PolarVLM: Bridging the Semantic-Physical Gap in Vision-Language Models](https://arxiv.org/abs/2605.07574)
*Yuliang Li, Chu Zhou, Heng Guo, Boxin Shi, Imari Sato, Zhanyu Ma*

#### 🧩 TL;DR
PolarVLM首次将偏振物理参数集成到视觉-语言模型中，通过双流架构和两阶段训练策略解决标准RGB在光学模糊场景下的语义理解局限，并在偏振感知VQA基准PolarVQA上展现出显著性能提升。

---

#### 📘 Detailed Summary
**Motivation:** 主流视觉-语言模型在处理反射、透明物体等严重光学模糊场景时存在固有局限，原因是标准RGB输入无法捕捉解析这些模糊所需的偏振物理信息；现有偏振成像方法受限于固定格式输出，无法与开放式的语言推理结合，存在语义-物理鸿沟。

**Method:** PolarVLM提出了首个将偏振物理参数集成到VLM中的多模态框架，采用双流架构分别处理RGB和偏振信息，并设计渐进式两阶段训练策略以避免物理参数被错误语义化解释，同时保持模型的通用视觉能力；此外还构建了PolarVQA基准，包含75K针对反射和透明场景的物理感知指令微调数据对。

**Result:** 在五个评估任务上，PolarVLM整体性能超越RGB基线25.4%，其中反射识别任务提升26.6%，玻璃计数任务提升34.0%，验证了物理感知语义理解的有效性。

**Conclusion:** 该工作证明将偏振物理参数引入视觉-语言模型能够有效解锁物理感知的语义理解能力，为解决光学模糊场景下的开放世界推理问题提供了新范式，并为后续物理信息增强的多模态研究提供了基准与方向。
...
### [32] [Response-G1: Explicit Scene Graph Modeling for Proactive Streaming Video Understanding](https://arxiv.org/abs/2605.07575)
*Ke Ma, Jiaqi Tang, Bin Guo, Xueting Han, Ruonan Xu, Qingfeng He, Ziheng Wang, Xu Wang, Qifeng Chen, Zhiwen Yu, Yunhao Liu*

#### 🧩 TL;DR
Response-G1 提出了一种基于场景图的显式结构化对齐框架，通过在线查询引导的场景图生成、历史场景图检索和检索增强的触发提示，实现了流式视频理解中高效且可解释的响应时机决策，在主动和被动任务上均取得了优越性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有 Video-LLMs 在流式视频理解中缺乏显式的、查询感知的视觉证据建模能力，导致无法准确判断何时应该对视频内容做出响应，响应时机决策缺乏可解释性和结构化对齐。

**Method:** Response-G1 是一个无需微调的三阶段框架：首先在在线流式剪辑上进行查询引导的场景图生成；其次通过基于记忆的检索机制获取最语义相关的历史场景图；最后利用检索增强的触发提示进行逐帧的“静默/响应”决策，将累积视频证据与查询预期响应条件通过共享的场景图表示进行显式结构化对齐。

**Result:** 在已建立的基准测试上，Response-G1 在主动和被动流式视频理解任务中均优于现有方法，验证了显式场景图建模与检索机制在流式视频理解中的有效性。

**Conclusion:** 将视频证据和查询条件统一在场景图表示下进行显式对齐，能够显著提升流式视频理解中响应时机的准确性和可解释性，为主动式视频理解提供了新的技术路径。
...
### [33] [Beyond Defenses: Manifold-Aligned Regularization for Intrinsic 3D Point Cloud Robustness](https://arxiv.org/abs/2605.07590)
*Pedro Alonso, Chongshou Li, Tianrui Li*

#### 🧩 TL;DR
本文提出 MAPR 框架，通过正则化潜在几何与内在几何的对齐，在不依赖对抗训练或额外数据的情况下，显著提升点云神经网络对多种对抗攻击的鲁棒性，在 ModelNet40 和 ScanObjectNN 上分别取得 +20.02% 和 +8.58% 的平均鲁棒性增益。

---

#### 📘 Detailed Summary
**Motivation:** 现有点云鲁棒性方法主要通过数据增强或防御机制提升性能，但忽略了对抗脆弱性的几何根源；本文指出 3D 网络的对抗脆弱性源于模型学习的潜在几何与底层表面的内在几何之间存在流形错配，该错配导致沿输入流形的微小保几何扰动在特征空间中引发过度扭曲。

**Method:** 本文提出 Manifold-Aligned Point Recognition (MAPR) 框架，通过正则化潜在几何来对齐内在扰动下的预测：首先为每个点云提取捕捉局部曲率和扩散结构的内在特征，然后施加一致性损失以保持对内在保几何扰动的不变性，从而在不依赖对抗训练或额外数据的情况下实现鲁棒性提升。

**Result:** 在 ModelNet40 和 ScanObjectNN 数据集上，MAPR 在多种对抗攻击下一致提升了鲁棒性，平均鲁棒性增益分别为 +20.02% 和 +8.58%，且无需对抗训练或额外数据。

**Conclusion:** MAPR 通过将经典对抗理论与点云内在几何结构相联系，揭示了 3D 鲁棒性的几何本质，为无需对抗训练的点云鲁棒性提升提供了新的理论视角和实用框架。
...
### [34] [TraceAV-Bench: Benchmarking Multi-Hop Trajectory Reasoning over Long Audio-Visual Videos](https://arxiv.org/abs/2605.07593)
*Hengyi Feng, Hao Liang, Mingrui Chen, Bohan Zeng, Meiyi Qiang, Zhengyang Zhao, Zimo Meng, Zeang Sheng, Wentao Zhang*

#### 🧩 TL;DR
TraceAV-Bench是首个联合评估长视频多模态轨迹多跳推理与幻觉鲁棒性的基准，包含2200道题、578个长视频（共339.5小时），揭示了现有最强模型（Gemini 3.1 Pro仅68.29%）在该任务上面临的显著挑战，且多模态幻觉鲁棒性与通用推理能力基本解耦。

---

#### 📘 Detailed Summary
**Motivation:** 真实场景的视听理解需要将稀疏、时域分散且横跨视觉与听觉流的证据进行链式推理，而现有基准局限于短片段、隔离模态或将问题简化为单跳感知，无法评估这一关键能力，因此亟需一个能同时评测长视频多跳推理与幻觉鲁棒性的基准。

**Method:** TraceAV-Bench采用三步半自动化流程构建：首先从长视频中提取稀疏的视听轨迹，然后设计跨模态的多跳推理链（平均3.68跳，跨越15.1分钟），最后生成多项选择题并经过严格质量审查。基准涵盖4个评估维度和15个子任务，共计2200道经过严格验证的多选题，基于578个总时长339.5小时的长视频。

**Result:** 对多个代表性OmniLLM的评估显示，最强的闭源模型Gemini 3.1 Pro在通用任务上仅达68.29%，最佳开源模型Ming-Flash-Omni-2.0仅达51.70%，表明该基准对现有模型构成持续挑战且存在显著提升空间。实验进一步发现，多模态幻觉鲁棒性与通用多模态推理能力基本解耦。

**Conclusion:** TraceAV-Bench揭示了现有OmniLLM在长视频跨模态多跳推理方面的不足，以及幻觉鲁棒性作为独立维度的评估必要性，有望激励未来研究发展能够对长视频视听内容进行连贯且忠实推理的模型。
...
### [35] [LithoBench: Benchmarking Large Multimodal Models for Remote-Sensing Lithology Interpretation](https://arxiv.org/abs/2605.07640)
*Jun Wang, Fengpeng Li, Hang Dong, Tianjin Huang, Wei Han*

#### 🧩 TL;DR
LithoBench是一个面向遥感岩性解译中地质语义理解的多层次基准数据集，包含10,000个专家标注样本和五级认知任务，实验表明现有大型视觉语言模型在地质语义理解尤其是高阶推理任务上存在显著局限。

---

#### 📘 Detailed Summary
**Motivation:** 遥感岩性解译高度依赖领域知识，需从视觉、光谱、纹理、地貌和上下文等多维线索推断岩石类型，而现有大模型评估缺乏同时涵盖岩性标注、多层次地质语义和专家评估的基准，限制了地质知识引导的大多模态模型的发展与评估。

**Method:** 提出LithoBench基准，包含10,000个专家标注样本，覆盖12类代表性岩性类别，包含4,000道选择题和6,000道开放题，按五个认知层次组织：识别与描述、对比分析、机制解释、实际应用和综合推理；并设计了专家参与、知识引导的半自动化构建管线，整合结构化地质图像描述等多子流程以提升地质有效性和评估可靠性。

**Result:** 在多个大型视觉语言模型上的实验表明，现有模型在地质语义理解上存在显著局限，尤其是在高阶解释、应用和推理任务上的表现严重不足。

**Conclusion:** LithoBench揭示了当前大模型在岩性解译这一知识密集型任务中的深层语义理解短板，为地质知识引导的多模态模型评估提供了标准化基准，并为后续研究指明了提升高阶地质推理能力的方向。
...
#### 🧩 TL;DR
本文系统评估了视觉语言模型（VLM）作为零样本ODD感知器的可行性，通过自定义数据集和Mapillary Vistas上的实验，发现基于定义锚定的思维链提示结合角色分解策略效果最优，为安全关键应用中透明有效的ODD感知奠定了基础。

---

#### 📘 Detailed Summary
**Motivation:** 自动驾驶系统的大规模实际部署需要严格遵守安全法规，而运营设计域（ODD）是定义自主智能体运行条件的关键要素。实现对ODD元素的可靠感知对于安全实现和审计至关重要，但传统方法难以适应不断演变的ODD定义。本文旨在探索视觉语言模型（VLM）是否能够作为零样本"ODD传感器"，在不依赖任务特定训练数据的情况下适应ODD定义的动态变化。

**Method:** 本文在自定义ODD数据集和Mapillary Vistas数据集上，使用四种不同的VLM进行了零样本ODD分类与检测的实证研究；对多种零样本优化策略进行了消融实验，并提供了成本-性能对比分析；此外，还构建了一套可复用的提示模板及相应的适配指南。

**Result:** 实验结果表明，基于定义锚定的思维链提示结合角色分解策略在所有方法中表现最佳，而其他优化方法可能导致召回率下降。研究同时给出了不同零样本优化策略的成本-性能权衡分析，为实际部署中的方法选择提供了量化依据。

**Conclusion:** VLM具备作为零样本ODD感知器的潜力，其中定义锚定的思维链与角色分解的组合策略是实现透明、有效ODD感知的关键方法。该研究为安全关键自动驾驶场景中基于ODD的感知应用提供了可行的技术路径和实践指导。
...
### [37] [Towards Billion-scale Multi-modal Biometric Search](https://arxiv.org/abs/2605.07655)
*Arka Koner, Chetan S. Naik, Lokesh Kurre, Vivek Raghavan, Barada P. Sabut, Tanusree Deb Barma, Anoop M. Namboodiri, Anil K. Jain*

#### 🧩 TL;DR
本文首次公开介绍了基于开源架构的超大规模多模态生物特征检索系统Bharat ABIS，该系统为印度Aadhaar数据库中的15.5亿条记录提供指纹、人脸和虹膜融合的去重搜索服务，在2.2亿身份规模的画廊上实现了FNIR 0.3% @ FPIR 0.5%的性能以及单服务器每秒100次搜索的吞吐量。

---

#### 📘 Detailed Summary
**Motivation:** 国家级身份系统需要对包含十亿级别记录的多模态生物特征数据库进行高效准确的1:N检索（去重），这要求在采集、预处理、特征提取、匹配速度、活体检测以及特殊案例处理等所有环节突破现有极限，而此前尚无工作公开发表过此类超大规模多模态生物特征搜索系统的端到端设计与实现细节。

**Method:** Bharat ABIS基于开源架构构建，针对指纹、人脸和虹膜三种模态分别设计了预处理（分割）、质量评估、呈现攻击检测和嵌入学习（特征提取）的独立处理流水线，最终将各模态特征拼接为每人13.5KB的模板，形成统一的1:N搜索（去重）解决方案。

**Result:** 在从印度Aadhaar数据库15.5亿条记录中随机抽样、按人口统计学分层的2.2亿身份画廊上，针对成人探针（18岁以上）的评估取得FNIR 0.3% @ FPIR 0.5%的性能；在2000万身份画廊上与三款商用货架系统（COTS）的对比中表现更优；在单台服务器（8×Nvidia H100 GPU、2TB RAM）上对4000万身份画廊实现了每秒100次搜索的吞吐量。

**Conclusion:** 该研究证明了基于开源架构构建的超大规模多模态生物特征去重系统在实际国家级身份识别场景中能够达到甚至超越商用系统的性能水平，为其他国家级或大规模身份系统提供了可复现的架构参考和性能基准，并揭示了多模态融合在十亿级规模下的工程可行性与关键设计权衡。
...
### [38] [APEX: Assumption-free Projection-based Embedding eXamination Metric for Image Quality Assessment](https://arxiv.org/abs/2605.07786)
*Caterina Gallegati, Monica Bianchini, Franco Scarselli, Vittorio Murino, Barbara Toniella Corradini*

#### 🧩 TL;DR
APEXT提出了一种基于切片 Wasserstein 距离的无假设图像评估框架，结合 CLIP 和 DINOv2 作为开放词汇特征提取器，克服了传统指标（如 FID）在闭集特征瓶颈和刚性参数假设方面的局限，在鲁棒性和跨数据集稳定性上均优于现有基线。

---

#### 📘 Detailed Summary
**Motivation:** 传统图像质量评估指标（如 FID）受限于闭集特征瓶颈（使用过时特征）和刚性参数化公式的假设性偏差，而现有基于现代骨干网络的替代方法虽缓解了特征瓶颈问题，但仍受限于参数化假设，亟需一种无需假设的相似性度量方法。

**Method:** APEXT 框架采用切片 Wasserstein 距离作为数学上严谨且无假设的相似性度量，具备对高维空间的有效可扩展性（理论和实验均证明）；同时，APEXT 是嵌入无关的，使用 CLIP 和 DINOv2 两种开放词汇基础模型作为特征提取器，避免闭集瓶颈。

**Result:** 在多个基准测试中，APEXT 相较于传统基线指标展现出对视觉退化更强的鲁棒性；同时，APEXT 指标在数据集内部和跨数据集上均表现出稳定性，确保在分布外数据集上的评估高度稳定。

**Conclusion:** APEXT 通过引入无假设的分布距离和开放词汇特征提取，有效解决了传统指标的两大核心瓶颈（闭集特征和参数假设），为图像生成质量评估提供了一种更鲁棒、更稳定的新范式。
...
### [39] [SARA: Semantically Adaptive Relational Alignment for Video Diffusion Models](https://arxiv.org/abs/2605.07800)
*Jiesong Lian, Zixiang Zhou, Ruizhe Zhong, Yuan Zhou, Qinglin Lu, Rui Wang, Long Hu, Yixue Hao, Baoru Huang*

#### 🧩 TL;DR
SARA引入文本条件显著性机制，通过语义自适应的成对路由操作将视觉基础模型的token关系蒸馏导向与提示相关的实体对，在Wan2.2持续训练设定下同时提升文本对齐和运动质量，相较于SFT、VideoREPA和MoAlign在多项评测中取得一致优势。

---

#### 📘 Detailed Summary
**Motivation:** 现有视频扩散模型虽然能生成视觉上逼真的片段，但仍存在实体丢失、属性绑定错误和交互描述弱化等问题。VideoREPA、MoAlign等表示对齐方法通过从冻结视觉基础模型蒸馏时空token关系来改善细粒度文本跟随，但其成对监督预算的分配依据的是视觉或运动线索，而非每对token与提示的相关性，导致监督信号偏离语义关键区域。

**Method:** SARA在保留冻结VFM目标上的token关系蒸馏（TRD）的基础上，引入文本条件显著性机制：首先训练一个轻量级Stage 1对齐器，利用逐实体SAM 3.1掩码监督和InfoNCE正则化，输出连续显著性分数；然后通过成对路由操作将其融合到TRD中，该操作为每个token对分配权重——当两个端点中至少有一个显著时赋予高权重，从而将监督路由导向主体-主体和主体-背景对，同时抑制背景-背景对的贡献。

**Result:** 在Wan2.2持续训练设定下，SARA在13维VLM评测指标、公开VBench基准和盲测用户研究中，均一致优于SFT、VideoREPA和MoAlign，同时提升了文本对齐度和运动质量。

**Conclusion:** SARA表明，在token关系蒸馏中引入语义自适应的路由机制，能够有效解决现有表示对齐方法中监督分配与提示语义脱节的问题，为提升视频扩散模型的细粒度文本跟随能力提供了新的技术路径，同时验证了实体级显著性引导在多模态对齐中的有效性。
...
### [40] [Consensus Entropy: Harnessing Multi-VLM Agreement for Self-Verifying and Self-Improving OCR](https://arxiv.org/abs/2504.11101)
*Yulong Zhang, Tianyi Liang, Xinyue Huang, Erfei Cui, Guoqing Wang, Xu Guo, Chenhui Li, Gongshen Liu*

#### 🧩 TL;DR
提出共识熵（Consensus Entropy, CE），一种无需训练、与模型无关的度量标准，通过测量模型间输出一致性的熵值来估计OCR输出的可靠性；基于CE构建CE-OCR框架，在无需监督或训练的情况下显著提升了OCR质量验证与性能。

---

#### 📘 Detailed Summary
**Motivation:** 尽管OCR平均精度持续提升，现有最先进的视觉语言模型（VLMs）在检测样本级错误方面仍存在不足，缺乏有效的无监督质量控制手段，难以可靠地识别低质量OCR输出。

**Method:** 提出共识熵（CE），通过度量多个模型输出之间的共识熵来估计输出可靠性，核心思想是正确预测在输出空间中趋于一致而错误预测则发散；基于CE构建CE-OCR框架，利用集成共识验证输出、选择最优结果，并通过自适应路由提升效率，整个方法无需训练且与模型无关。

**Result:** 实验表明，CE在质量验证任务上相比VLM-as-Judge方法将F1分数提升42.1%；CE-OCR在相同计算成本下持续优于自一致性方法和单模型基线，取得了一致的OCR性能增益。

**Conclusion:** 共识熵作为一种无需训练、即插即用的度量标准，为OCR质量验证和VLM输出可靠性估计提供了有效且实用的解决方案，具有广泛的应用潜力。
...
### [41] [MedVIGIL: Evaluating Trustworthy Medical VLMs Under Broken Visual Evidence](https://arxiv.org/abs/2605.07919)
...
#### 🧩 TL;DR
提出medvigil基准测试套件，用于评估医学视觉语言模型在证据扰动下的静默失败问题，包含300例由放射科医生监督构建的病例和2556个MCQ探针，发现最强模型(Claude Opus 4.7)的MCS得分为69.2，而人类放射科医生为83.3，存在显著性能差距。

---

#### 📘 Detailed Summary
**Motivation:** 现有医学视觉语言模型(VLM)通常在完整图像-问题对上进行评估，但临床可信赖应用要求模型能在答案的"证据基础"失效时识别问题。论文针对这一关键安全缺口，研究模型在扰动证据条件下的静默失败现象——即当视觉问题被配以错误前提、措辞扰动、纯知识改写或ROI损坏图像时，模型仍返回流畅的非拒绝回答而未能察觉证据失效。

**Method:** 构建medvigil评估套件，包含300例由四位委员会认证放射科医生全程监督的病例，来源于四个公开医学VQA数据集。每个病例的金标准答案、拒绝选项、候选答案集、释义、错误前提陷阱、ROI框和临床风险层级均由临床医生编写。两位主治放射科医生并行标注每个病例，一位高级放射科医生整合发布清单，第四位独立放射科医生回答所有探针以提供人类基线。套件包含2556个MCQ探针、240个反事实三元组、风险层级和可回答性标志、ROI框及开放变体版本。

**Result:** 提出七个正确性条件审计指标并汇总为medvigil Composite Score (MCS)。审计了16个视觉能力模型和2个纯文本基线。独立放射科医生的MCS为83.3，静默失败率为5.8%；最强模型Claude Opus 4.7的MCS为69.2，存在14.1分的复合分数差距，表明当前最强模型与临床医生之间存在显著性能鸿沟。

**Conclusion:** 研究表明当前最先进的医学视觉语言模型在检测证据失效方面仍远不及人类放射科医生，存在较大的性能提升空间。medvigil基准和评估工具已公开发布，为未来研究提供了评估医学VLM临床可信赖性和安全性的标准化测试平台。
...
### [42] [Delta-Adapter: Scalable Exemplar-Based Image Editing with Single-Pair Supervision](https://arxiv.org/abs/2605.07940)
*Jiacheng Chen, Songze Li, Han Fu, Baoquan Zhao, Wei Liu, Yanyan Liang, Li Qing, Xudong Mao*

#### 🧩 TL;DR
Delta-Adapter 提出了一种基于单样本对监督的范例图像编辑方法，通过从预训练视觉编码器中提取语义增量（semantic delta）并将其注入到图像编辑模型中，无需文本指导即可学习可迁移的编辑语义，显著提升了编辑精度与内容一致性，并有效泛化到未见编辑任务。

---

#### 📘 Detailed Summary
**Motivation:** 现有的基于范例的图像编辑方法依赖“成对-成对”监督范式，即需要两个共享相同编辑语义的图像对来学习目标变换，这使得训练数据的构建成本高昂且难以规模化，同时限制了模型对不同编辑类型的泛化能力。

**Method:** Delta-Adapter 采用单样本对监督策略，无需文本指导。它利用预训练视觉编码器从源-目标图像对中提取“语义增量”（semantic delta），该增量编码了两幅图像之间的视觉变换；随后通过一个基于 Perceiver 的适配器将该语义增量注入到预训练的编辑模型中。由于目标图像对模型不可见，其本身可作为预测目标，从而支持单样本对训练。此外，引入语义增量一致性损失，确保生成输出的语义变化与真实语义增量对齐。

**Result:** 在四个强基线模型上的大量实验表明，Delta-Adapter 在已见编辑任务上一致提升了编辑精度和内容一致性，并且在未见编辑任务上展现出更强的泛化能力。

**Conclusion:** Delta-Adapter 通过单样本对监督和语义增量注入机制，有效降低了范例图像编辑对数据标注的依赖，同时提升了编辑质量与泛化性能，为可迁移的视觉编辑学习提供了一种新范式。
...
### [43] [STARFlow2: Bridging Language Models and Normalizing Flows for Unified Multimodal Generation](https://arxiv.org/abs/2605.08029)
...
#### 🧩 TL;DR
STARFlow2提出基于自回归归一化流的统一多模态生成框架，利用其与LLM同构的因果掩码和KV缓存机制，通过Pretzel架构将VLM流与TarFlow流垂直交错，结合深浅流设计与统一FAE潜空间，实现了文本与图像的可缓存交错生成，在图像生成与多模态理解基准上表现优异。

---

#### 📘 Detailed Summary
**Motivation:** 现有统一多模态系统通常组合自回归语言模型与扩散图像生成器，导致因果文本生成与迭代视觉去噪之间存在结构性不匹配，缺乏真正统一的多模态生成范式。

**Method:** 提出STARFlow2，基于Pretzel架构将预训练的VLM流与TarFlow流通过残差跳跃连接垂直交错，两者共享相同因果掩码；采用深浅流设计并结合统一FAE潜空间，使文本与视觉输出直接进入KV缓存而无需重新编码，实现缓存友好的交错生成。

**Result:** 在图像生成与多模态理解基准上取得强性能表现，验证了自回归归一化流作为统一多模态建模可行基础的有效性。

**Conclusion:** 自回归归一化流（Autoregressive Normalizing Flows）因其与LLM共享因果掩码、KV缓存和从左到右结构，成为实现真正统一多模态生成的最自然范式，为未来多模态模型设计提供了新方向。
...
### [44] [PET-Adapter: Test-Time Domain Adaptation for Full and Limited-Angle PET Image Reconstruction](https://arxiv.org/abs/2605.08030)
*Rüveyda Yilmaz, Yuli Wu, Johannes Stegmaier, Volkmar Schulz*

#### 🧩 TL;DR
PET-Adapter 提出了一种测试时域适应框架，使仅在体模数据上预训练的生成式PET重建模型能够适应临床数据集，支持不同解剖结构、示踪剂和扫描仪配置，无需配对真实数据，并将扩散步数从50减少至2而不牺牲质量。

---

#### 📘 Detailed Summary
**Motivation:** PET图像重建受到泊松噪声和物理退化因素的严重影响，在有限角度采集场景中问题更为突出。现有深度学习方法在未见过的临床数据分布上泛化能力有限，通常需要大量重新训练，且多数预训练模型仅基于体模数据，难以直接迁移至临床环境。

**Method:** PET-Adapter 提出了一种测试时域适应框架，核心包括两个关键技术：层间低秩解剖条件注入（layer-wise low-rank anatomical conditioning），在适应过程中将解剖先验信息引入生成模型各层；以及基于有序子集期望最大化（OSEM）的热启动策略，从物理信息重建结果初始化生成过程，从而将扩散步数从50大幅减少至2。

**Result:** 在多个临床数据集上的实验表明，PET-Adapter 在全角度和有限角度设置下均实现了优越的3D重建性能，证明了该方法在不同解剖结构、示踪剂和扫描仪配置下的临床可行性和计算效率。

**Conclusion:** PET-Adapter 通过测试时域适应有效弥合了体模预训练模型与临床数据之间的分布差异，无需配对真实数据即可实现高质量重建。该工作表明，结合物理先验（OSEM热启动）与轻量级条件注入策略，能够显著提升生成模型在临床PET重建中的实用性和泛化能力，同时大幅降低推理计算成本。
...
### [45] [Object Hallucination-Free Reinforcement Unlearning for Vision-Language Models](https://arxiv.org/abs/2605.08031)
*Kaidi Jia, Yujie Lin, Chengyi Yang, Jiayao Ma, Jinsong Su*

#### 🧩 TL;DR
提出HFRU，一种基于强化学习的视觉编码器反学习框架，通过对齐破坏与GRPO优化实现深度语义移除，在保留性能和抗幻觉方面显著优于现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有的视觉语言模型反学习方法主要针对语言解码器进行微调，导致遗忘表面化，无法有效擦除底层视觉表征，同时容易引发物体幻觉问题。

**Method:** 提出HFRU，一种两步式强化反学习框架：首先通过对齐破坏操作视觉编码器，然后结合GRPO优化与复合奖励函数（包括抽象奖励，用于鼓励语义有效替换并缓解幻觉）进行深度语义移除。

**Result:** 在物体识别和人脸身份任务上，HFRU实现了超过98%的遗忘与保留性能，同时引入的物体幻觉几乎可忽略，显著优于先前方法。

**Conclusion:** 将反学习从语言解码器扩展到视觉编码器，并通过强化学习与复合奖励机制实现深层语义移除，为VLM的安全反学习提供了新的有效范式。
...
### [46] [Proxy3D: Efficient 3D Representations for Vision-Language Models via Semantic Clustering and Alignment](https://arxiv.org/abs/2605.08064)
*Jerry Jiang, Haowen Sun, Denis Gudovskiy, Yohei Nakata, Tomoyuki Okuno, Kurt Keutzer, Wenzhao Zheng*

#### 🧩 TL;DR
提出Proxy3D方法，利用紧凑的三维代理表示替代传统的二维像素对齐视觉表征，在更短的视觉序列长度下实现三维视觉问答、视觉定位和空间智能基准上的竞争性或最先进性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉语言模型在三维空间智能推理中面临两难困境：基于对应关系的方法隐式理解三维场景但空间一致性差，基于表征的方法引入三维几何先验但视觉序列序列化效率低下。亟需一种既能保持三维空间一致性又具备序列效率的视觉表征方法。

**Method:** 提出Proxy3D方法，仅以视频帧为输入，通过语义编码器和几何编码器提取场景特征，进行语义感知聚类获得三维空间中的一组代理表征；同时构建SpaceSpan数据集并采用多阶段训练策略，使大语言模型适配这种紧凑的三维代理表示。

**Result:** 在更短的视觉序列长度下，Proxy3D在三维视觉问答、视觉定位和通用空间智能基准测试中取得竞争性或最先进的性能。

**Conclusion:** 三维代理表征能够以紧凑的形式有效编码三维空间信息，兼顾空间一致性与序列化效率，为视觉语言模型在三维世界中的空间智能推理提供了一种高效且可扩展的范式。
...
### [47] [MELD: Multi-Task Equilibrated Learning Detector for AI-Generated Text](https://arxiv.org/abs/2605.06903)
*Chenjun Li, Cheng Wan, Johannes C. Paetzold*

#### 🧩 TL;DR
MELD 是一个多任务均衡学习的 AI 文本检测器，通过引入生成器家族、攻击类型和源域等辅助监督头，结合 EMA 教师-学生蒸馏和难负样本排序损失，在保持推理成本不变的情况下，在 RAID 榜单和跨域基准上达到领先性能，尤其在攻击场景和低误报率下表现优异。

---

#### 📘 Detailed Summary
**Motivation:** 现有 AI 文本检测器通常仅优化单一的 AI/人类二元分类目标，导致表示学习在二元任务饱和后缺乏对生成器、攻击类型或源域结构的建模能力，难以同时满足对攻击和对抗改写鲁棒、跨未见生成器和域泛化以及低假阳性率运行的实用需求。

**Method:** MELD 在共享编码器上附加了生成器家族、攻击类型和源域三个辅助分类头，并通过可学习的同方差不确定性权重均衡四个损失；采用 EMA 教师网络对干净输入预测，攻击增强的学生网络向其蒸馏以提升鲁棒性；引入难负样本对排序损失扩大 AI 文本与最易混淆人类文本之间的得分间隔；推理时丢弃所有辅助头，保持与标准检测器相同的接口和计算开销。

**Result:** 在公开 RAID 榜单上，MELD 是最强的开源检测器，与领先的商业模型竞争，尤其在攻击场景和低 FPR 下表现突出；在标准跨域基准上匹配或超越监督基线；在基于四大 LLM 提供商最新聊天模型构建的 MELD-eval 评估集上，无需额外微调即可达到 99.9% TPR at 1% FPR，而多数基线性能显著下降。

**Conclusion:** MELD 证明通过多任务辅助监督和均衡训练策略，可以在不增加推理成本的前提下显著提升 AI 文本检测器的泛化能力和鲁棒性，为实际部署中应对多样化攻击和域漂移提供了有效方案。
...
### [48] [Cognitive Agent Compilation for Explicit Problem Solver Modeling](https://arxiv.org/abs/2605.07040)
*Hyeongdon Moon, Carolyn Rosé, John Stamper*

#### 🧩 TL;DR
提出认知代理编译（CAC）框架，利用强教师LLM将问题求解知识编译为显式目标代理，分离知识表示、策略与验证规则，以提高教育场景中受限问题求解的可检查性与可编辑性，并用小型语言模型验证了关键设计权衡。

---

#### 📘 Detailed Summary
**Motivation:** 大语言模型虽广泛用于教育场景，但其广泛预训练使其难以约束，且无法替代可控的学习者；教育系统需要可检查、可编辑的知识状态，使教育者能了解系统假设的学习者知识，学习者能理解系统基于显式技能、误解和策略的推理过程。

**Method:** 提出认知代理编译（CAC）框架，利用强大的教师LLM将问题求解知识编译为显式目标代理；该框架将知识表示、问题求解策略、验证与更新规则三者分离，以实现受限且可检查的推理，并使用小型语言模型构建了初步原型以探索关键设计权衡。

**Result:** 通过小型语言模型实现的早期概念验证揭示了显式控制与可扩展泛化之间的关键设计权衡，验证了CAC框架在有限范围内实现可检查性与可编辑性的可行性，但未给出具体的量化性能指标。

**Conclusion:** CAC作为迈向教育应用中受限知识AI的初步步骤，证明了将认知架构思想与LLM结合以构建显式、可检查知识代理的潜力，为未来在可控性与泛化之间寻求更好平衡的研究指明了方向。
...
### [49] [MedExAgent: Training LLM Agents to Ask, Examine, and Diagnose in Noisy Clinical Environments](https://arxiv.org/abs/2605.07058)
*Yicheng Gao, Xiaolin Zhou, Yahan Li, Yue Zhao, Ruishan Liu*

#### 🧩 TL;DR
本文提出将临床诊断建模为部分可观测马尔可夫决策过程（POMDP），并构建了包含七种患者噪声和三种检查噪声的系统性噪声模型，在此基础上通过两阶段训练（监督微调+DAPO强化学习）得到诊断智能体MedExAgent，在保持成本效益的同时实现了与更大模型相当的诊断性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有的医疗大语言模型基准和自动诊断方法严重简化了真实临床诊断过程，将其降级为单轮问答、无噪声对话或顺序检查等简化场景，忽视了临床诊断的交互性和不确定性本质，无法反映医生在真实诊疗中需要面对不同患者人格、噪声信息和不完整信息等复杂情况。

**Method:** 本文将临床诊断形式化为部分可观测马尔可夫决策过程（POMDP），包含三类动作：询问患者、调用医疗检查工具和给出诊断；引入包含七种患者噪声和三种检查噪声的系统性噪声模型；提出了MedExAgent智能体，通过两阶段训练：先基于Calgary-Cambridge临床访谈模型构建的合成对话进行监督微调，再使用DAPO算法优化复合奖励函数（综合诊断准确性、工具调用质量和检查成本，包括经济成本和患者不适度）。

**Result:** 通过大量实验和消融研究，MedExAgent在诊断性能上达到了与更大规模模型相当的水平，同时保持了成本效益更高的检查策略，验证了所提出的POMDP形式化框架和两阶段训练方法在处理噪声、不完整信息的交互式诊断场景中的有效性。

**Conclusion:** 该研究揭示了将临床诊断建模为部分可观测决策过程并结合结构化临床访谈模板与强化学习的可行性，为构建更贴近真实临床实践的交互式自动诊断系统提供了新的范式，并指出了考虑噪声和不确定性对提升诊断智能体鲁棒性的重要价值。
...
### [50] [Securing Computer-Use Agents: A Unified Architecture-Lifecycle Framework for Deployment-Grounded Reliability](https://arxiv.org/abs/2605.07110)
*Zejian Chen, Zhanyuan Liu, Chaozhuo Li, Mengxiang Han, Songyang Liu, Litian Zhang, Feng Gao, Yiming Hei, Xi Zhang*

#### 🧩 TL;DR
提出了一种架构-生命周期统一框架，用于分析计算机使用智能体在真实软件环境中的部署可靠性，从感知、决策、执行三层架构与创建、部署、运维、维护四阶段生命周期出发，系统性地连接能力形成、权限暴露、故障显现与控制机制之间的关联。

---

#### 📘 Detailed Summary
**Motivation:** 现有计算机使用智能体（CUA）研究从受控基准测试向真实软件环境迁移，可靠性不仅取决于任务成功率，还受感知错误、规划漂移、内存使用、工具中介、权限范围及运行时监控等多因素共同影响。既有综述按方法、平台、基准或安全威胁组织研究，但缺乏对能力形成、权限暴露、故障显现与控制放置之间关系的系统性连接。

**Method:** 本文提出一个面向部署可靠性的架构-生命周期分析框架：架构层面将CUA建模为感知（Perception）、决策（Decision）、执行（Execution）三层耦合系统，解释软件观测如何转化为带有权限的动作；生命周期层面划分为创建（Creation）、部署（Deployment）、运维（Operation）、维护（Maintenance）四个阶段，分析先验知识学习、工具与权限绑定、运行时轨迹压力测试以及漂移下的保障保持。利用该框架综合分析了代表性系统、基准测试和安全/隐私研究，区分故障可见位置与其使能条件的引入点，并映射了控制、监控与保障的介入面。

**Result:** 通过所提出的框架系统性地梳理了现有CUA系统、基准测试及安全/隐私研究，明确了故障在何处变得可见与其使能条件在何处引入之间的区别，并定位了可进行干预的控制、监控与保障接口。OpenClaw仅作为开放部署模式的公开示例被引用，未作为经过验证的内部案例。

**Conclusion:** 指出开放挑战包括可控的基础对齐、长程约束保持、安全的权限绑定、混合信任运行时防御、隐私保护记忆以及持续保障。
...
### [51] [Region4Web: Rethinking Observation Space Granularity for Web Agents](https://arxiv.org/abs/2605.07134)
*Donguk Kwon, Dongha Lee*

#### 🧩 TL;DR
本文提出Region4Web框架，通过将网页AXTree层次化分解为功能区域，并设计PageDigest推理管线为智能体提供跨步骤持久的区域级观测，在WebArena基准上显著缩短观测长度并提升多种LLM骨干和智能体方法的任务成功率。

---

#### 📘 Detailed Summary
**Motivation:** 现有Web智能体将观测空间与动作空间均设为元素级粒度，导致网页的功能组织结构被隐式化，智能体需在每一步从海量元素级信号中自行推断页面功能，缺乏更紧凑、语义更丰富的观测表示。

**Method:** 提出Region4Web框架，通过层次化分解与语义抽象将网页AXTree重组为功能区域，暴露页面的功能组织结构作为页面状态理解的基础；并设计PageDigest，一种面向Web的专用推理管线，将区域级观测以紧凑的逐页摘要形式传递给执行智能体，该摘要跨步骤持续有效。

**Result:** 在WebArena基准上，PageDigest显著缩短了观测长度，并在多种骨干LLM（不同容量）以及已有智能体方法上一致提升了任务成功率，验证了功能区域粒度观测比纯元素级处理更紧凑且信息更丰富。

**Conclusion:** 功能区域粒度的观测为Web智能体提供了比元素级处理更紧凑且更富含功能信息的表示基础，表明将观测粒度与页面功能组织对齐是提升Web智能体性能的有效设计方向。
...
### [52] [CLIPer: Tailoring Diverse User Preference via Classifier-Guided Inference-Time Personalization](https://arxiv.org/abs/2605.07162)
*Jinyan Su, Jinpeng Zhou, Claire Cardie, Wen Sun*

#### 🧩 TL;DR
本文提出CLIPer，一种基于分类器引导的推理时个性化方法，通过在推理阶段动态调整LLM生成行为以适应不同用户偏好，避免了昂贵的全模型微调，实现了轻量级、可控且可扩展的个性化语言生成。

---

#### 📘 Detailed Summary
**Motivation:** 现有的个性化LLM方法通常依赖针对不同用户偏好的全模型微调，这在面对多种偏好组合时计算成本极高且不切实际；因此需要一种轻量级、无需重新训练即可在推理时动态适配多样用户偏好（如帮助性、简洁性、幽默感）的个性化方案。

**Method:** CLIPer（Classifier-guided Inference-time Personalization）采用一个额外的分类器模型在推理阶段动态引导LLM的生成过程，通过分类器对当前生成结果与目标偏好之间的匹配度进行评分，从而调整后续生成方向，实现无需微调即可适配单一或多维用户偏好，且仅引入极小的额外计算开销。

**Result:** 综合实验表明，CLIPer在单维度和多维度偏好场景下均能有效实现可控且细腻的个性化生成，该方法具有良好的可扩展性，能够在保持极低额外计算成本的前提下显著提升LLM对用户偏好的适配能力。

**Conclusion:** CLIPer证明了推理时引导机制可以作为传统微调方法的高效替代方案，为低成本、细粒度的LLM个性化提供了新范式；其轻量特性使其在实际部署中具有显著优势，未来可进一步拓展至更复杂的多维偏好控制和实时自适应场景。
...
### [53] [Learning Agent Routing From Early Experience](https://arxiv.org/abs/2605.07180)
*Yimin Wang, Jiahao Qiu, Xuan Qi, Xinzhe Juan, Jingzhe Shi, Zelin Zhao, Hongru Wang, Shilong Liu, Mengdi Wang*

#### 🧩 TL;DR
BoundaryRouter是一种无需训练的路由框架，通过早期行为经验和基于评分准则的推理，在轻量级LLM推理与完整智能体执行之间进行查询路由，在RouteBench上实现推理时间降低60.6%的同时性能比直接LLM推理提升28.6%。

---

#### 📘 Detailed Summary
**Motivation:** LLM智能体在复杂推理任务上表现优异但延迟和计算成本高昂，而实际应用中大量查询处于前沿LLM能力边界内无需完整智能体执行，因此如何在冷启动场景下有效将查询路由至轻量级LLM推理或完整智能体执行成为关键挑战。

**Method:** BoundaryRouter是一种无需训练（training-free）的路由框架，通过收集两个系统在共享种子集上的早期行为经验构建紧凑的经验记忆库，并在推理时检索相似案例，结合基于评分准则（rubric-guided）的推理来决定查询是直接由LLM回答还是升级至智能体执行。

**Result:** 在提出的RouteBench基准（涵盖领域内、释义和领域外路由设置）上，BoundaryRouter相比完整智能体执行减少60.6%的推理时间，同时比直接LLM推理提升28.6%的性能，且分别优于基于提示的路由和纯检索路由平均37.9%和8.2%。

**Conclusion:** 该研究验证了在冷启动场景下，通过早期经验记忆与评分准则推理相结合的无训练路由策略能够有效平衡LLM推理效率与智能体性能，为实际部署中查询的动态分配提供了可行方案。
...
### [54] [Teaching Language Models to Think in Code](https://arxiv.org/abs/2605.07237)
*Hyeon Hwang, Jiwoo Lee, Jaewoo Kang*

#### 🧩 TL;DR
本文提出 ThinC 框架，将代码本身作为推理主体而非辅助工具，通过纯代码驱动推理结合监督微调与强化学习训练小型模型，在竞赛级数学基准上超越包括 Qwen3-235B-A22B-Thinking 在内的多种工具集成推理基线模型。

---

#### 📘 Detailed Summary
**Motivation:** 现有工具集成推理（TIR）范式在数学问题求解中交织自然语言推理与代码执行，存在三个关键缺陷：代码常仅作为事后验证工具、中间自然语言计算易出错、自然语言与代码角色重叠导致分工不明。作者旨在解决这些问题，探索一种让代码承担主要推理职责的新型推理框架。

**Method:** ThinC 框架要求模型在简短的自然语言规划步骤后，所有后续推理完全通过代码块进行，代码块之间仅通过执行输出连接。作者从教师模型中蒸馏出 12.2k 条纯代码驱动推理轨迹，基于此对 ThinC-1.7B 和 ThinC-4B 进行监督微调，随后使用强化学习进一步训练。

**Result:** ThinC-4B 在五个竞赛级数学基准上一致优于所有 TIR 基线模型，甚至超越了参数规模远大的 Qwen3-235B-A22B-Thinking 模型。分析表明，ThinC 的 99.2% 最终答案源于解释器输出，且模型能在代码执行失败时可靠恢复，无需中间自然语言推理。

**Conclusion:** ThinC 证明纯代码驱动的推理范式能够在数学推理任务中以更小的模型规模取得超越大规模 TIR 模型的性能，其核心优势在于推理过程紧密依赖解释器输出而非自然语言中间步骤，从而显著降低计算错误并提升鲁棒性，为语言模型的数学推理提供了新的方向。
...
### [55] [PaT: Planning-after-Trial for Efficient Test-Time Code Generation](https://arxiv.org/abs/2605.07248)
*Youngsik Yoon, Sungjae Lee, Seockbean Song, Siwei Wang, Wei Chen, Jungseul Ok*

#### 🧩 TL;DR
提出Planning-after-Trial (PaT)自适应策略，在代码生成任务中仅在验证失败后调用规划器，结合异构模型配置（轻量模型生成+强模型规划），在多个基准上显著提升成本-性能帕累托前沿，同等性能下推理成本降低约69%。

---

#### 📘 Detailed Summary
**Motivation:** 现有测试时计算扩展方法多采用刚性的"先规划后尝试"(PbT)策略，即使在可直接求解的问题上也会产生不必要的规划开销，导致测试时计算资源分配效率低下。

**Method:** 提出Planning-after-Trial (PaT)自适应策略：先由模型直接生成代码，仅在验证失败时才触发规划器进行干预；该策略天然支持异构模型配置，即低成本模型负责生成尝试，高性能模型仅在必要时用于规划修正。

**Result:** 在多个代码生成基准和模型系列上的实验表明，PaT方法显著推进了成本-性能帕累托前沿；异构配置（轻量生成+强模型规划）在达到与大规模同构模型相近性能的同时，推理成本降低约69%。

**Conclusion:** PaT表明自适应、按需规划的测试时计算策略能够有效替代传统的刚性规划范式，在代码生成任务中实现更优的计算效率与性能权衡，为LLM推理阶段的资源优化提供了新思路。
...
### [56] [MIPIAD: Multilingual Indirect Prompt Injection Attack Defense with Qwen -- TF-IDF Hybrid and Meta-Ensemble Learning](https://arxiv.org/abs/2605.07269)
*Al Muhit Muhtadi, Mostafa Rifat Tazwar*

#### 🧩 TL;DR
MIPIAD是一个针对多语言环境下间接提示注入攻击的防御框架，通过结合LoRA微调的序列分类器（XLPID）、TF-IDF特征和集成学习（晚期融合、堆叠、梯度提升），在英语和孟加拉语的合成基准上达到0.9205的最佳F1和0.9378的最佳AUROC，并通过集成方法有效缩小了跨语言性能差距。

---

#### 📘 Detailed Summary
**Motivation:** 间接提示注入攻击在检索增强和工具使用的LLM系统中持续构成安全威胁，而在多语言场景下这一问题更加难以刻画和防御，现有研究缺乏针对非英语语言的系统化防御方案。

**Method:** 提出MIPIAD防御框架，包含三个核心组件：通过LoRA从Qwen2.5-1.5B微调的序列分类器XLPID、基于TF-IDF的词汇特征提取、以及通过晚期融合、堆叠和梯度提升三种集成策略验证调优的混合方法；框架使用NLLB-200支持200多种语言，XLPID的多语言骨干网络可在无需架构修改的情况下重新适配到其他语言。评估基于从BIPIA模板扩展的合成基准，覆盖邮件、表格、问答、摘要和代码五类任务，共超过143万样本，训练集与测试集使用互斥的攻击类别。

**Result:** 在英语和孟加拉语的合成基准上，纯词汇方法TF-IDF+SVM的F1达到0.77，展现出强信号；XLPID与TF-IDF的混合集成取得整体最佳F1（0.9205），梯度提升集成取得最佳AUROC（0.9378）；集成方法一致性地缩小了英语和孟加拉语之间的跨语言性能差距，优于独立的神经模型。

**Conclusion:** MIPIAD框架验证了词汇特征与微调神经模型的多语言集成可以显著提升对间接提示注入的检测性能，并能有效降低跨语言性能差异；框架具有良好的可扩展性，可通过NLLB-200和XLPID的多语言骨干支持更多语言，但当前实证验证仅限于英语和孟加拉语。
...
### [57] [Activation Differences Reveal Backdoors: A Comparison of SAE Architectures](https://arxiv.org/abs/2605.07324)
*Sachin Kumar*

#### 🧩 TL;DR
本文研究了两种稀疏自编码器架构（Crosscoders和差分SAE）在检测微调语言模型中后门特征方面的效果，发现差分SAE在隔离后门相关特征上大幅优于Crosscoders，表明后门表现为方向性激活偏移而非稀疏特征激活。

---

#### 📘 Detailed Summary
**Motivation:** 后门攻击对语言模型安全构成严重威胁，模型在大多数输入上正常但在特定触发模式下表现出有害行为，而通过机制可解释性检测此类后门仍然是一个开放挑战。

**Method:** 研究比较了两种稀疏自编码器架构——Crosscoders和差分SAE（Diff-SAE），在基于年份上下文（"2024"触发脆弱代码，"2023"触发安全代码）的SQL注入后门场景下，评估了它们在SmolLM2-360M模型上通过LoRA和全秩微调两种范式隔离后门相关特征的能力。

**Result:** Diff-SAE在大多数实验条件下实现了0.40的后门隔离分数（BIS），完美精度（1.0）和零假阳性率，而Crosscoders在大多数情况下的BIS低于0.02；该性能差距在多个Transformer层（14、18、22、26）和两种微调范式下均保持一致，其中全秩微调产生的后门信号尤为干净。

**Conclusion:** 后门表现为方向性激活偏移而非稀疏特征激活，这使得基于差分的表示在检测上更为有效；该发现对AI安全监控和检测模型操纵的可解释性工具开发具有重要意义。
...
### [58] [Mean-Pooled Cosine Similarity is Not Length-Invariant: Theory and Cross-Domain Evidence for a Length-Invariant Alternative](https://arxiv.org/abs/2605.07345)
*Sibayan Mitra, Dhruv Kumar*

#### 🧩 TL;DR
本文证明均值池化余弦相似度作为跨语言、跨模态表征比较的默认度量不具有长度不变性：由于Transformer表征的各向异性，该度量随序列长度单调增长。实验表明CKA等长度不变度量应作为替代，且基于均值池化余弦的跨语言表征趋同结论需重新审视。

---

#### 📘 Detailed Summary
**Motivation:** 现有研究广泛使用均值池化余弦相似度来比较不同语言、模态或任务下的神经表征，但该度量是否具有长度不变性尚未被严格检验，且当表征存在各向异性时，序列长度可能成为混淆因素，导致跨语言表征趋同等结论不可靠。

**Method:** 论文首先理论分析均值池化余弦相似度因各向异性而随序列长度单调增长的机制；随后在HumanEvalPack数据集上对四种代码LLM进行实验，对比均值池化余弦与Centered Kernel Alignment (CKA)的表现；并在Mistral-7B的WMT平行语料对和CLIP ViT-B/32上进一步验证理论预测。

**Result:** 在HumanEvalPack上，长度比单独解释了"Python邻近性"中R²=0.52–0.75的方差，而AST深度和共享token分数仅增加不到3%；使用CKA后解释方差降低83%，长度系数符号反转（β_len从+0.86变为-0.37）。Mistral-7B上余弦的R²为0.23（EN-FR）和0.33（EN-DE），CKA的R²<0.01。CLIP ViT-B/32中均值池化相比EOS池化将长度效应R²从0.21降至<0.01。

**Conclusion:** 均值池化余弦相似度因各向异性而缺乏长度不变性，不适合作为跨表征比较的默认度量；CKA等长度不变度量应成为首选。基于均值池化余弦得出的跨语言表征趋同结论可能受到长度伪影的干扰，需重新检验。
...
### [59] [MAVEN: Multi-Agent Verification-Elaboration Network with In-Step Epistemic Auditing](https://arxiv.org/abs/2605.07646)
*Yinsheng Yao, Jiehao Tang, Zhaozhen Yang, Dawei Cheng*

#### 🧩 TL;DR
MAVEN提出了一种黑板式多智能体框架（怀疑者-研究者-法官循环），通过显式角色解耦和逐步认知审计将LLM转变为审慎推理者，在多个基准测试上超越潜在推理模型和基于共识的基线方法，且完全模型无关。

---

#### 📘 Detailed Summary
**Motivation:** 现有显式推理范式依赖单一链条且缺乏中间验证机制，导致早期错误级联传播，缺乏模块化能力使得精细审计困难，在高风险应用场景中损害了认知可信度。

**Method:** MAVEN（多智能体验证-精炼网络带逐步认知审计）是一种黑板启发式框架，核心通过对抗性的怀疑者-研究者-法官循环实现角色解耦：怀疑者负责逻辑辩护，研究者负责事实依据，法官进行综合裁决，从而将LLM转变为具备显式结构化、模块化且可验证的审慎推理者。

**Result:** 在OpenBookQA、TruthfulQA、HALUEVAL和StrategyQA四个基准测试上，MAVEN在四个细粒度指标上均展现出更优的推理质量，持续超越GEMINI-3.1-Pro等潜在推理模型和ReConcile等基于共识的基线方法；同时MAVEN完全模型无关，作为通用推理增强器可显著提升不同骨干模型的性能。

**Conclusion:** MAVEN通过显式解耦角色和逐步审计机制有效解决了推理过程中错误级联和缺乏可审计性的问题，其模型无关性和强迁移能力表明这种结构化多智能体协作范式是提升LLM推理可靠性的有效途径，对高风险认知任务具有重要应用价值。
...
#### 🧩 TL;DR
提出SOD（步级蒸馏）框架解决小语言模型在工具集成推理中因错误级联导致教师监督信号失效的问题，通过自适应重加权蒸馏强度，在0.6B模型上实现AIME 2025 26.13%的准确率，并比次优基线提升最高20.86%。

---

#### 📘 Detailed Summary
**Motivation:** 现有强化学习方法（如GRPO）仅提供稀疏的结局级奖励，而近期流行的在线策略蒸馏（OPD）虽能提供密集的令牌级监督，但在工具集成推理场景中出现严重失效：错误的工具调用会在后续推理步中级联放大，导致学生与教师模型之间的分歧逐步扩大，使教师提供的令牌级监督信号变得不可靠。

**Method:** 提出SOD（Step-wise On-policy Distillation）框架，在每一步推理中基于步级分歧（step-level divergence）自适应地重新加权蒸馏强度，在高分歧区域衰减可能误导的教师信号，同时在良好对齐的状态下保留密集的教师引导，从而在不牺牲密集监督优势的前提下抑制错误级联。

**Result:** 在数学、科学和代码等多个挑战性基准测试上，SOD相比次优基线取得最高20.86%的提升；0.6B规模的学生模型在AIME 2025上达到26.13%的准确率，证明了将智能体推理能力有效迁移至轻量级模型的可行性。

**Conclusion:** 步级分歧自适应重加权策略有效解决了工具集成推理中在线蒸馏的级联错误问题，为小语言模型在复杂工具交互场景下的能力迁移提供了一条可行路径，代码已开源。
...
#### 🧩 TL;DR
提出链式蒸馏（CBD）范式，通过构建稀疏的中间模型蒸馏链并利用参数插值初始化变尺寸语言模型，在无需反复访问大型教师模型的情况下高效迁移知识，显著提升小模型训练效率与下游性能。

---

#### 📘 Detailed Summary
**Motivation:** 大语言模型在资源受限场景下部署成本高昂，小语言模型从头训练计算开销大，传统知识蒸馏针对不同目标尺寸需反复访问大型教师模型，可扩展性差，亟需一种高效、可扩展的变尺寸语言模型初始化方法。

**Method:** 提出链式蒸馏（CBD）范式，通过逐步蒸馏构建一个稀疏且有限的中间模型序列（称为锚点），形成渐进式知识蒸馏链；引入桥接蒸馏机制实现跨架构和跨词表的异构知识迁移；变尺寸模型则通过相邻锚点之间的参数插值进行初始化，从而消除对大型教师模型的重复推理需求。

**Result:** 在100亿词元的语料上，一个138M参数的小语言模型无需恢复预训练，仅通过CBD初始化即在下游任务上超越从头训练的模型；CBD还展示了在异构设置下初始化不同架构和词表模型的通用能力。

**Conclusion:** 链式蒸馏范式通过渐进式蒸馏链与参数插值机制，在显著提升效率的同时保持了良好的下游性能，为资源受限场景下的变尺寸语言模型部署提供了可扩展的解决方案，且支持跨架构和跨词表的知识迁移。
...
### [62] [MatryoshkaLoRA: Learning Accurate Hierarchical Low-Rank Representations for LLM Fine-Tuning](https://arxiv.org/abs/2605.07850)
*Ionut-Vlad Modoranu, Mher Safaryan, Dan Alistarh*

#### 🧩 TL;DR
MatryoshkaLoRA提出一种基于俄罗斯套娃思想的新型训练框架，通过在LoRA适配器之间插入固定对角矩阵P来学习层次化低秩表示，支持动态秩选择且精度损失极小，并提出了新的评估指标AURAC。

---

#### 📘 Detailed Summary
**Motivation:** 现有LoRA方法需要预设静态秩r并通过穷举网格搜索来平衡效率与性能，而DyLoRA等秩自适应方法虽然在训练过程中对秩进行采样，但由于无法在整个秩层次上获得一致的梯度信号，导致较高秩时性能次优且数据效率低下。

**Method:** MatryoshkaLoRA在LoRA的A和B适配器之间插入一个精心构建的固定对角矩阵P，通过对子秩进行缩放来学习准确的层次化低秩表示；该框架通过仅改变P即可恢复LoRA和DyLoRA，确保所有子秩都能高效嵌入可用梯度信息；同时提出了AURAC指标用于统一评估层次化低秩适配器的性能。

**Result:** 实验结果表明，MatryoshkaLoRA相比先前的秩自适应方法学习了更准确的层次化低秩表示，在多种评估数据集上实现了各秩下更优的精度-性能权衡。

**Conclusion:** MatryoshkaLoRA通过简单的矩阵插入操作实现了对不同子秩的高效梯度利用，为参数高效微调提供了一个通用且有效的动态秩选择框架，未来可进一步推广至更广泛的低秩适配场景。
...
### [63] [CA-SQL: Complexity-Aware Inference Time Reasoning for Text-to-SQL via Exploration and Compute Budget Allocation](https://arxiv.org/abs/2605.08057)
*James Petullo, Nianwen Xue*

#### 🧩 TL;DR
CA-SQL提出了一种基于任务难度动态扩展候选查询搜索空间的Text-to-SQL流水线，结合进化搜索启发的自定义提示播种和新型投票机制，在BIRD基准的困难子集上以GPT-4o-mini达到了51.72%的准确率，超越了包括更大模型在内的其他上下文学习方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有推理时学习方法在Text-to-SQL任务上虽取得进展，但在BIRD基准中最具挑战性的任务上表现不佳，根本原因在于解空间探索不充分，难以发现可进一步精炼产生正确输出的有前途候选查询。

**Method:** CA-SQL提出了一种新型Text-to-SQL流水线，利用任务预估难度动态扩展候选生成时的探索广度；同时采用基于进化搜索原理的自定义提示播种方法增强基础LLM的探索行为，并设计了一种新颖的投票机制在搜索结束时选择最佳候选解。

**Result:** 在BIRD开发集的"困难"子集上，CA-SQL使用GPT-4o-mini取得了51.72%的最新最优准确率，超越了其他上下文学习方法（包括使用更大模型的方法）；在完整BIRD开发数据集上达到61.06%的执行准确率和68.77%的Soft F1分数。

**Conclusion:** 研究表明，通过动态调整探索广度并结合进化搜索启发式策略，即使使用相对较小的基础模型也能在极具挑战性的Text-to-SQL任务上取得领先性能，验证了结构化解空间探索对于复杂推理任务的重要性。
...
### [64] [The Memory Curse: How Expanded Recall Erodes Cooperative Intent in LLM Agents](https://arxiv.org/abs/2605.08060)
...
#### 🧩 TL;DR
本文发现大语言模型在多代理社会困境中扩展上下文窗口会系统性地破坏合作行为，提出"记忆诅咒"现象，并通过词法分析、记忆消毒和思维链消融实验揭示其机制在于记忆内容引发的远见意图侵蚀而非记忆长度本身。

---

#### 📘 Detailed Summary
**Motivation:** 上下文窗口扩展通常被视为大语言模型能力的直接提升，但其在多代理社会困境中的影响尚未被系统研究——该文旨在揭示扩展可访问历史记忆对多智能体合作行为的实际作用机制。

**Method:** 该文在7个LLM和4种博弈场景上进行500轮实验；对378,000条推理轨迹进行词法分析；使用仅基于前瞻性推理轨迹训练的LoRA适配器作为认知探针进行微调；通过记忆消毒（用合成合作记录替换可见历史）分离记忆内容与长度的影响；对显式思维链推理进行消融实验。

**Result:** 在28个模型-博弈设定中，扩展历史记忆导致18个出现合作退化（记忆诅咒）；词法分析表明退化源于前瞻性意图的侵蚀而非偏执加剧；记忆消毒实验证明触发因素是记忆内容而非长度；LoRA微调可缓解退化并零样本迁移到不同博弈；消除思维链推理通常能减轻记忆诅咒。

**Conclusion:** 记忆是多代理行为的一个主动决定因素：更长的上下文并非必然有益，其效果取决于所引发的推理模式；远见性推理意图的维持是合作稳定性的关键，而显式推理放大记忆诅咒的现象质疑了"更多推理总是更好"的假设。
...
### [65] [GraphDC: A Divide-and-Conquer Multi-Agent System for Scalable Graph Algorithm Reasoning](https://arxiv.org/abs/2605.06671)
*Wenjin Li, Jiaming Cui*

#### 🧩 TL;DR
GraphDC 是一个基于分治策略的多智能体框架，通过将大图分解为子图并分配专门智能体进行局部推理，再由主智能体整合全局信息，显著提升了大型语言模型在图算法推理任务上的可扩展性和鲁棒性，尤其在大规模图上表现优于现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有大型语言模型在图算法推理任务上表现不佳，因为图具有天然的拓扑复杂性，且在大规模图上需要进行系统性的多步推理，而当前端到端推理方法难以应对这种可扩展性挑战。

**Method:** GraphDC 采用分治策略的多智能体框架，首先将输入图分解为较小的子图，然后为每个子图分配一个专门智能体进行局部推理，最后通过一个主智能体整合各局部输出和子图间交互信息，生成最终解决方案，从而降低单个智能体的推理负担并缓解计算瓶颈。

**Result:** 大量实验表明，GraphDC 在多种图算法推理任务和不同规模上均持续优于现有方法，尤其是在大规模图实例上，端到端推理不可靠时优势更为显著。

**Conclusion:** 分治策略的多智能体协同推理框架能够有效提升 LLM 在图算法推理任务上的可扩展性和鲁棒性，为大图上的复杂推理问题提供了一种可行且高效的解决方案。
...
### [66] [Fast and Effective Redistricting Optimization via Composite-Move Tabu Search](https://arxiv.org/abs/2605.06682)
*Hai Jin, Diansheng Guo*

#### 🧩 TL;DR
提出了一种复合移动禁忌搜索（CM-Tabu）方法，通过识别并利用图论中的关节点和双连通分量来构造保连通性的复合移动，系统性地扩展现有禁忌搜索在空间重划问题中的可行邻域空间，从而显著提升解质量、鲁棒性和计算效率。

---

#### 📘 Detailed Summary
**Motivation:** 空间重划是一类需要高质量解、快速迭代以及支持多准则目标与交互式优化的实际组合优化问题，其核心挑战在于连通性约束：在整数规划或启发式搜索中强制保持连通性会严重压缩可行邻域空间，削弱搜索的探索能力，使算法容易陷入较差的局部最优解。

**Method:** 本文提出复合移动禁忌搜索（CM-Tabu），其核心思想是当边界单元无法在不破坏所在选区连通性的前提下被单独重新分配时，通过分析每个选区的连通图，利用关节点和双连通分量在O(n)时间内生成两类保连通性的复合移动：一是一组可一起移动的单元集合（复合移动），二是可交换的一对单元或单元集合（交换移动），从而系统性地扩展禁忌搜索的可行邻域空间。

**Result:** 大量实验表明，与传统禁忌搜索及其他基线方法相比，CM-Tabu在解质量、运行间鲁棒性和计算效率上均有显著提升。以费城案例为例，该方法能够在人口均衡性方面持续达到理论全局最优，并支持多准则目标的权衡优化。

**Conclusion:** CM-Tabu通过图论驱动的复合移动机制有效破解了空间重划中连通性约束带来的搜索困境，提供了适用于实际决策工作流的高性能优化方案，表明在启发式搜索中系统性地扩展可行邻域是提升组合优化效果的有效途径。
...
#### 🧩 TL;DR
提出了一种通过分析多智能体系统内部神经表征的互信息图并进行谱分解来检测联盟结构的方法，并在多智能体强化学习和大型语言模型两个领域中验证了其有效性，为监控分布式AI系统中的涌现组织提供了可扩展的诊断工具。

---

#### 📘 Detailed Summary
**Motivation:** 多智能体AI系统中，智能体可以形成联盟并产生涌现的群体级组织，这对AI安全与对齐至关重要。然而，仅观察智能体的外在行为往往不足以区分真正的信息耦合与虚假的相似性，因为具有实质影响的联盟可能在行为变化之前就已在内部表征层面形成，现有方法难以从内部表征层面检测这种结构。

**Method:** 提出了一种从多智能体系统内部神经表征中检测联盟结构的实用方法。该方法首先基于智能体隐藏状态构建成对互信息图，然后应用谱分解（谱划分）来识别最显著的联盟边界。该方法无需依赖外部行为观测即可捕捉信息层面的耦合关系。

**Result:** 在多智能体强化学习环境中，该方法成功恢复了预设的分层和动态联盟结构，并正确排除了由行为协调而无信息耦合导致的假阳性。在大型语言模型中，该方法识别出了由描述性提示暗示的联盟结构，追踪了动态团队重组，并揭示了显式标签主导冲突交互模式这一表征层级性。在这两种场景下，谱分解恢复的划分揭示了标量交叉智能体互信息度量无法区分的子群组织。

**Conclusion:** 通过谱分解分析隐藏状态互信息提供了一种可扩展的诊断方法，用于识别表征层面的联盟结构，为监测分布式AI系统中的涌现组织提供了有价值的工具。该方法能捕捉到行为层面无法直接观察到的内部表征耦合，对AI安全与对齐研究具有重要启示。
...
### [68] [CASCADE: Case-Based Continual Adaptation for Large Language Models During Deployment](https://arxiv.org/abs/2605.06702)
*Siyuan Guo, Yali Du, Hechang Chen, Yi Chang, Jun Wang*

#### 🧩 TL;DR
本文提出部署时学习（DTL）作为LLM生命周期的第三阶段，并设计了CASCADE框架，通过显式的演化情景记忆和上下文赌博机机制，使LLM agent在不修改参数的情况下从部署经验中持续改进，在16个任务上平均成功率提升20.9%。

---

#### 📘 Detailed Summary
**Motivation:** 当前LLM的生命周期严格区分为训练和部署两个阶段，部署后学习能力基本停滞，这与自然智能中通过环境交互持续适应的特性存在显著差距。该研究旨在填补这一空白，使LLM agent能够在部署后继续从交互经验中学习提升。

**Method:** 论文形式化定义了部署时学习（DTL）作为LLM生命周期的第三阶段，并提出CASCADE框架。该框架为LLM agent配备显式且可演化的情景记忆，将经验复用建模为上下文赌博机问题，从而在理论上实现无遗憾保证的探索-利用权衡，使agent能够积累、选择和精炼任务相关案例，将过去经验转化为可操作知识。

**Result:** 在覆盖医学诊断、法律分析、代码生成、网页搜索、工具使用和具身交互等16个多样化任务中，CASCADE相比零样本提示将宏平均成功率提升20.9%，并持续优于基于梯度和基于记忆的基线方法。

**Conclusion:** 该工作将部署重新定义为一种自适应学习过程，为构建持续改进的AI系统奠定了方法论基础。CASCADE的核心价值在于无需修改模型参数即可实现部署时的持续学习，为LLM agent的长周期自主演化提供了可行的技术路径。
...
### [69] [From Storage to Experience: A Survey on the Evolution of LLM Agent Memory Mechanisms](https://arxiv.org/abs/2605.06716)
*Jinghao Luo, Yuchen Tian, Chuxue Cao, Ziyang Luo, Hongzhan Lin, Kaixin Li, Chuyi Kong, Ruichao Yang, Jing Ma*

#### 🧩 TL;DR
该综述提出了一个面向大语言模型智能体记忆机制的三阶段演化框架（存储、反思、经验），系统整合了当前碎片化的研究现状，并明确了从轨迹保存到跨轨迹抽象的技术演进路径。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于LLM的智能体记忆机制研究在操作系统工程与认知科学两个范式之间碎片化分布，缺乏统一的技术综合视角和连贯的演化脉络，阻碍了下一代智能体系统的系统化设计。

**Method:** 该工作提出了一个三阶段演化框架来形式化智能体记忆机制的发展：存储阶段（轨迹保存）、反思阶段（轨迹精炼）和经验阶段（轨迹抽象），并分析了驱动这一演化的三个核心动力（长程一致性需求、动态环境挑战和持续学习目标），特别探讨了经验阶段的前瞻性探索与跨轨迹抽象两种变革性机制。

**Result:** 本文为综述性工作，通过整合碎片化的记忆机制研究，识别出记忆机制从存储到反思再到抽象的关键演化路径，为理解和设计LLM智能体的记忆架构提供了系统性的分类与分析框架。

**Conclusion:** 该工作通过提出统一的演化框架弥合了现有研究范式的理论鸿沟，为下一代LLM智能体的记忆系统设计提供了稳健的设计原则和清晰的发展路线图，指明了从被动存储向主动探索与抽象演进的未来方向。
...
### [70] [Weblica: Scalable and Reproducible Training Environments for Visual Web Agents](https://arxiv.org/abs/2605.06761)
*Oğuzhan Fatih Kar, Roman Bachmann, Yuanzheng Gong, Anders Boesen Lindbo Larsen, Afshin Dehghan*

#### 🧩 TL;DR
Weblica是一个通过HTTP级缓存和LLM环境合成来构建可复现、可扩展网页环境的框架，使强化学习训练能够扩展到数千个多样化环境。其训练的Weblica-8B模型在多个网页导航基准上优于同尺寸开源模型，且与API模型竞争力相当。

---

#### 📘 Detailed Summary
**Motivation:** 现有视觉网页代理的训练数据收集方法存在局限：离线轨迹仅适用于监督微调，而少数模拟环境无法支撑强化学习训练，两者均难以捕捉真实网络的多样性和开放性，导致训练数据在规模和多样性上严重不足。

**Method:** Weblica框架包含两个核心组件：1) HTTP级缓存机制，用于捕获和重放稳定的视觉状态同时保留可交互行为；2) 基于LLM的环境合成方法，以真实网站和核心网页导航技能为基础生成多样化环境。该框架将强化学习训练扩展到数千个不同的环境和任务上。

**Result:** Weblica-8B模型在多个网页导航基准上超越同尺寸开源基线模型，且使用更少的推理步骤；在测试时计算量增加时性能呈正向扩展；与API模型的竞争力相当。

**Conclusion:** Weblica证明了通过缓存与合成相结合的方式构建可复现、可扩展的网页环境是可行的，能够有效弥合模拟环境与真实网络之间的差距，为视觉网页代理的强化学习训练提供了规模化路径。
...
### [71] [Towards Security-Auditable LLM Agents: A Unified Graph Representation](https://arxiv.org/abs/2605.06812)
...
#### 🧩 TL;DR
本文提出Agent-BOM，一种面向LLM智能体系统的统一结构化审计表示方法，通过分层属性有向图分离静态能力基座与动态运行时语义状态，将碎片化执行轨迹转化为可查询的审计路径，在多种真实攻击场景下有效重构了隐蔽攻击链。

---

#### 📘 Detailed Summary
**Motivation:** LLM智能体系统在执行复杂自主任务时，其语义驱动的执行范式在底层物理事件与高层执行意图之间存在严重的语义鸿沟，导致事后安全审计极为困难。现有的表示机制如静态SBOM和运行时日志只能提供碎片化的证据，无法捕捉认知状态演化、能力绑定、持久化内存污染以及跨智能体的级联风险传播，亟需一种统一的结构化表示来支撑智能体系统的安全审计。

**Method:** Agent-BOM将智能体系统建模为一个分层属性有向图，将静态能力基座（模型、工具、长期记忆）与动态运行时语义状态（目标、推理轨迹、动作）相分离，并通过语义边和安全属性连接两层，将碎片化执行轨迹转化为可查询的审计路径。在此基础上，提出了基于图查询的路径级风险评估范式，并基于OWASP Agentic Top 10进行了实例化，同时在OpenClaw环境中实现了审计插件以从实时执行中构建Agent-BOM。

**Result:** 在多种代表性真实世界智能体攻击场景下的评估表明，Agent-BOM能够有效重构隐蔽攻击链，包括跨会话记忆投毒与工具滥用、能力供应链劫持与意外代码执行、多智能体生态系统劫持以及特权与信任滥用，验证了其在复杂智能体生态系统中为根因分析和安全裁决提供统一可审计基础的能力。

**Conclusion:** Agent-BOM提供了一种统一且可审计的基础，能够有效支撑复杂智能体生态系统中的根因分析与安全裁决，填补了现有表示机制在语义鸿沟上的空白，为智能体系统的安全审计提供了全新范式。
...
### [72] [Randomness is sometimes necessary for coordination](https://arxiv.org/abs/2605.06825)
*Rohan Patil, Jai Malegaonkar, Henrik I. Christensen*

#### 🧩 TL;DR
提出Diamond Attention，一种基于交叉注意力与随机标量排序掩码的架构，通过单轮广播的随机比特协调协议实现同质智能体在对称观测下的角色分化，并在完全对称任务和零样本迁移场景中显著超越确定性基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 在同质智能体的合作多智能体强化学习中，标准做法是采用全参数共享，但在置换对称观测下，共享确定性策略会为所有智能体输出相同的动作分布，导致无法实现角色分化。尽管理论上可以通过匿名相同处理器间的对称性破缺（需要随机性）来解决，但现有方法缺乏有效的随机性利用机制。

**Method:** 提出Diamond Attention，一种交叉注意力架构，每个智能体在每个时间步采样一个标量随机数，通过该随机数诱导瞬态排序，在智能体间的注意力机制中屏蔽排名较低的同伴，同时保留任务注意力的完全可见性。该机制在单轮广播中实现随机比特协调协议，且基于集合的注意力支持零样本部署到不同规模的智能体团队。

**Result:** 在完全对称的XOR游戏中，该方法取得1.0的成功率，而所有确定性基线方法停滞在约0.5。在控制协调任务中，基于N=4训练的策略可零样本泛化到N∈[2,8]。在SMACLite跨场景迁移中实现了标准基线因结构限制无法完成的零样本迁移。用标准dropout随机性替代结构化掩码后胜率为0%，证实协议空间结构而非随机噪声是关键因素。

**Conclusion:** 研究表明，在对称观测下的同质多智能体系统中，结构化随机性（而非随机噪声）是实现角色分化和零样本迁移的关键因素。Diamond Attention通过简单的随机排序掩码机制，在单轮广播通信中实现了有效的协调协议，为多智能体系统中的对称性破缺提供了一种实用且高效的解决方案。
...
### [73] [Extracting Search Trees from LLM Reasoning Traces Reveals Myopic Planning](https://arxiv.org/abs/2605.06840)
*Sixing Chen, Ji-An Li, Saner Cakir, Sinan Akcali, Kayla Lee, Marcelo G. Mattar*

#### 🧩 TL;DR
本文通过从四子棋游戏中提取和量化搜索树，发现LLM的规划过程较浅，其决策主要由浅层节点而非深层节点驱动，与人类依赖深层搜索的规划模式形成显著对比。

---

#### 📘 Detailed Summary
**Motivation:** 尽管大语言模型（尤其是推理模型）在思维链推理中会生成对未来结果的显式推演，但这种推演是否构成真正的规划、其结构如何以及哪些方面驱动了模型性能，目前尚不清楚，缺乏系统性的分析方法。

**Method:** 本文提出了一种新的方法来刻画LLM的规划过程，通过在四子棋游戏中从推理轨迹中提取并量化搜索树，并拟合计算模型来分析搜索树的结构及其对落子决策的影响。此外，还进行了因果干预实验，选择性剪枝思维链段落以验证深层和浅层节点的作用。

**Result:** 实验发现，LLM的搜索深度浅于人类，其性能主要由搜索广度而非深度决定。尽管LLM的推理轨迹中包含了深层节点，但其落子选择最好由一个忽略深层节点的短视模型来解释。因果干预实验进一步表明，落子选择主要由浅层节点驱动。这些结果与人类规划模式形成对比，人类的表现主要由深层搜索驱动。

**Conclusion:** 本研究发现LLM与人类规划之间存在关键差异：人类专家依赖于深度搜索，而LLM并不基于深层前向搜索做出决策。这种分离为对齐LLM与人类规划提供了有针对性的指导，所提出的框架也为跨战略领域解释LLM规划结构提供了通用方法。
...
### [74] [Agentick: A Unified Benchmark for General Sequential Decision-Making Agents](https://arxiv.org/abs/2605.06869)
*Roger Creus Castanyer, Pablo Samuel Castro, Glen Berseth*

#### 🧩 TL;DR
Agentick是一个用于公平评估RL、LLM、VLM、混合和人类智能体的统一基准，包含37个程序化生成任务、六种能力类别及多模态观测，实验表明无单一范式占优，GPT-5 mini综合得分最高但PPO在规划和多智能体任务中领先，推理增强框架可将LLM性能提升3-10倍。

---

#### 📘 Detailed Summary
**Motivation:** 智能体研究涵盖从零开始学习的RL智能体到利用预训练知识的基础模型智能体，但缺乏统一基准来公平比较这些不同范式，阻碍了对序列决策智能体根本挑战的系统性研究。

**Method:** Agentick提供了37个程序化生成的任务，覆盖六种能力类别、四种难度级别和五种观测模态，所有任务通过统一的Gymnasium兼容接口暴露；配套提供Coding API、所有任务的oracle参考策略、预构建SFT数据集、可组合智能体框架和实时排行榜。

**Result:** 在27种配置、超过90,000个episode的评估中，GPT-5 mini以0.309的oracle归一化得分综合领先，PPO在规划和多智能体任务中占优；推理增强框架将LLM性能提升3-10倍；ASCII观测始终优于自然语言观测。

**Conclusion:** 无单一智能体范式在所有任务中占据主导地位，所有范式仍有显著改进空间；Agentick的能力分解、多模态设计为通用自主智能体的进步提供了评估框架和训练基础设施，尤其支持基础模型在真实序列环境中的RL后训练研究。
...
### [75] [Beyond the Black Box: Interpretability of Agentic AI Tool Use](https://arxiv.org/abs/2605.06890)
*Hariom Tatsat, Ariye Shater*

#### 🧩 TL;DR
本文提出一种基于稀疏自编码器和线性探针的机制可解释性工具包，通过对模型内部状态进行特征分解与探测，实现对AI智能体工具调用行为的内部可观测性，帮助在长程任务中诊断和预防因工具使用错误导致的执行失败和安全隐患。

---

#### 📘 Detailed Summary
**Motivation:** 现有AI智能体部署中，工具调用失败（如遗漏、误用或后果难以预判）难以诊断和控制，现有的可观测性方法（如提示分析、输出评分、外部日志）均为事后或外部手段，无法在模型动作前捕获其内部决策信号；尤其在长程任务中，早期工具调用错误会扰动后续轨迹、增加token消耗并引发安全风险，亟需对智能体内部工具决策机制的可视化能力。

**Method:** 该方法构建了一个基于稀疏自编码器（SAE）和线性探针的机制可解释性工具包，通过在每个动作前读取模型状态，从中推断是否需要工具以及下一步工具调用的后果严重程度；利用SAE将激活分解为稀疏特征，定位与工具决策最相关的内部层和特征，并通过特征消融测试其功能重要性；探针在NVIDIA Nemotron函数调用数据集的多步轨迹上训练，并应用于GPT-OSS 20B和Gemma 3 27B模型。

**Result:** 该工作展示了机制可解释性可在工具决策前提供模型内部信号的可视化能力，弥补了外部评估的盲区；在长程任务中，该方法能够暴露早期错误如何重塑智能体交互轨迹，从而辅助识别深层失败原因；探针训练基于Nemotron数据集的完整多步轨迹，并在两种不同规模的开源模型上验证了工作流的通用性。

**Conclusion:** 本研究的核心贡献在于为AI智能体系统增加了一个长期缺失的内部可观测性层，使开发者能够在动作执行前了解模型内部关于工具使用的信号；这有助于在长程运行中揭示智能体失败的深层机制原因，并表明机制可解释性可有效支持对工具调用和风险的实用内部监控，而不是取代外部评估方法。
...
### [76] [Learning and Reusing Policy Decompositions for Hierarchical Generalized Planning with LLM Agents](https://arxiv.org/abs/2605.06957)
*Shirin Sohrabi, Haritha Ananthakrishnan, Harsha Kokel, Kavitha Srinivas, Michael Katz*

#### 🧩 TL;DR
提出了一种结合经典规划概念与LLM智能体的动态策略学习方法HCL-GP，通过层次化任务分解和可复用组件库实现策略泛化，在AppWorld基准上达到98.2%的准确率，并将开源模型的成功率从接近零提升至62.5%。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM智能体在面对未见过的任务实例时缺乏泛化能力，静态合成方法在挑战性场景下性能严重不足，且缺乏将成功执行经验抽象为可复用组件的机制，导致每次推理都需要从头规划。

**Method:** 提出HCL-GP框架，通过自动化层次化任务分解从成功执行中学习参数化策略并提取可复用组件，构建组件库；利用语义搜索实现高效组件检索，支持组合式策略生成，从而在任务实例间实现策略泛化。

**Result:** 在AppWorld基准上，HCL-GP在常规任务上达到98.2%准确率，在包含未见应用的挑战任务上达到97.8%，相比静态合成方法提升15.8个百分点；对于开源模型，动态组件复用将成功率从近乎为零提升至62.5%。

**Conclusion:** 该研究证实了经典规划中的泛化规划和层次化分解概念能够有效集成到LLM智能体中，显著提升其在复杂、未见场景下的准确性和效率，为将符号规划思想与神经方法结合提供了有效路径。
...
### [77] [The Context Gathering Decision Process: A POMDP Framework for Agentic Search](https://arxiv.org/abs/2605.07042)
*Chinmaya Kausik, Adith Swaminathan, Nathan Kallus*

#### 🧩 TL;DR
本文形式化了大模型智能体在复杂环境中的上下文收集过程为上下文收集决策过程（CGDP），并提出基于谓词的状态表示与程序化穷举检测两项即插即用干预，在保持多跳推理能力的同时减少冗余搜索和令牌消耗。

---

#### 📘 Detailed Summary
**Motivation:** 大模型智能体在代码库、企业数据库等复杂环境中工作时，相关状态远超其上下文窗口，必须通过迭代探索来寻找信息；然而缺乏显式基础设施时，智能体的工作记忆会退化为有损的搜索状态表示，导致重复循环和过早停止等冗余行为，因此需要形式化该挑战并提出有效干预手段。

**Method:** 本文将大模型智能体的迭代搜索过程形式化为上下文收集决策过程（CGDP），一种特殊的部分可观测马尔可夫决策过程，并将LLM的行为建模为CGDP下的近似汤普森采样。在此基础上提出基于谓词的方法，将隐式搜索分解为显式模块化操作，进而设计两种即插即用干预：持久化的基于谓词的信念状态（在有限上下文内保持多跳推理能力），以及程序化穷举检测门（在无性能损失的前提下终止无效搜索）。

**Result:** 在四种方法和三个问答领域的实验中，用CGDP引导的信念状态替代LLM的隐式状态使多跳推理准确率提升最高11.4%；程序化穷举检测在不降低智能体性能的前提下节省最多39%的令牌消耗。

**Conclusion:** 将大模型智能体循环框架化为CGDP可以指导设计模块化、无干扰的搜索改进；基于谓词的显式状态表示和程序化终止机制能有效提升搜索效率与推理能力，为构建更可靠的LLM智能体系统提供了形式化基础。
...
### [78] [Switchcraft: AI Model Router for Agentic Tool Calling](https://arxiv.org/abs/2605.07112)
*Sharad Agarwal, Pooria Namyar, Alec Wolman, Rahul Ambavat, Ankur Gupta, Qizheng Zhang*

#### 🧩 TL;DR
Switchcraft 是首个面向智能体工具调用的模型路由器，基于 DistilBERT 分类器进行在线路由选择，在五项基准测试上以 82.9% 的准确率匹配或超越最佳单一模型，同时降低 84% 的推理成本，每百万查询节省超 3,600 美元。

---

#### 📘 Detailed Summary
**Motivation:** 现有 Agentic AI 系统在调用外部工具时默认使用大型模型，导致推理预算严重超支；模型路由技术虽可用于优化成本，但已有路由器专为对话补全设计，缺乏针对工具调用场景的优化方案。

**Method:** Switchcraft 提出一种在线模型路由方法，基于 DistilBERT 训练二分类器，在每次工具调用请求时动态选择满足正确性的最低成本模型；构建包含五个函数调用基准的评估框架，并在延迟预算约束下部署路由系统。

**Result:** Switchcraft 在五项函数调用基准上达到 82.9% 的准确率，与最佳单一模型相当或更优；推理成本降低 84%，每百万查询节省超过 3,600 美元。实验还发现，大型模型在工具调用任务中并非始终优于小型模型，且名义上更便宜的模型可能因推理 token 消耗过多而导致总成本更高。

**Conclusion:** 该研究证明，针对工具调用场景设计的轻量级路由模型能够在保持任务正确性的前提下显著降低推理成本，提示开发者应重新审视模型规模与工具调用能力之间的非单调关系，为成本感知的 Agentic AI 部署提供了实用方向。
...
### [79] [Repeated Deceptive Path Planning against Learnable Observer](https://arxiv.org/abs/2605.07174)
*Shiyue Cao, Pei Xu, Likun Yang, Lei Cui, Shizhao Yu, Shiyu Zhang, Yongjian Ren, Xiaotang Chen, Kaiqi Huang*

#### 🧩 TL;DR
本文提出重复欺骗路径规划（RDPP）问题，并设计了一种名为DeMP的双层元学习框架，使智能体能够持续欺骗具备学习能力的外部观察者，显著优于现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有欺骗路径规划（DPP）研究假定外部观察者是静态且非学习的，而现实场景（如关键物资运输或军事行动）中的对手会通过学习历史轨迹来适应。本文旨在填补这一空白，研究智能体如何在与可学习观察者的重复交互中持续隐藏真实目的地。

**Method:** 本文提出重复欺骗路径规划（RDPP）问题形式化，并设计DeMP（Deceptive Meta Planning）框架，采用双层优化结构：内层进行回合级适应（episode-level adaptation），实现对观察者更新的短期策略调整；外层进行元级更新（meta-level updates），利用跨回合反馈捕捉观察者的学习模式，加速未来回合的适应，从而缓解增量更新带来的累积滞后问题。

**Result:** 在多种环境下的实验结果表明，DeMP在RDPP任务中显著优于现有DPP方法，同时保持竞争性的路径代价。现有方法由于无法适应观察者演化预测或在增量更新中产生累积滞后，均无法在RDPP设定下取得持续有效的欺骗效果。

**Conclusion:** 本工作揭示了在可学习对抗者的重复交互情境下欺骗路径规划的关键挑战，强调建模观察者学习过程的重要性，为多智能体系统中的欺骗与隐私问题提供了新的见解和研究方向。
...
### [80] [Three-in-One World Model: Energy-Based Consistency, Prediction, and Counterfactual Inference for Marketing Intervention](https://arxiv.org/abs/2605.07199)
*Junichiro Niimi*

#### 🧩 TL;DR
提出一种三合一世界模型架构，将DBM学习的冻结信念表示与轻量级任务适配器结合，在同一框架内统一支持能量一致性评估、结果预测和反事实推断，在模拟营销环境中比多种元学习器更准确地恢复异质处理效应。

---

#### 📘 Detailed Summary
**Motivation:** 营销决策反映潜在消费者异质性、时变内部状态和显式干预之间的复杂交互，而现有预测导向和语言导向模型无法以统一方式捕获这一结构。

**Method:** 提出三合一世界模型架构：DBM从人口统计特征、时间和滞后行为与结果中学习冻结的信念表示，并附加轻量级任务特定适配器。同一信念支持三项任务——基于DBM自由能的能量一致性评估、通过适配器的结果预测、以及固定信念仅改变动作输入的反事实推断。

**Result:** 在已知潜在价格敏感度、促销响应度和基础偏好的受控模拟中，适配器在访问和购买AUC上匹配强MLP基线，同时在恢复异质处理效应上显著优于S、T、X、DR-learner元学习器和基于相同原始特征的因果森林基线，在混淆的价格-促销干预上差距最大。自由能钳制系统性地惩罚缺乏先前促销暴露的反事实购买轨迹，且惩罚依赖潜在基础偏好。

**Conclusion:** DBM信念能够以在反事实查询中依然保持的形式解缠潜在特征，为营销干预提供统一的世界模型基础。
...
### [81] [HMACE: Heterogeneous Multi-Agent Collaborative Evolution for Combinatorial Optimization](https://arxiv.org/abs/2605.07214)
*Yuping Yan, Jirui Han, Fei Ming, Yuanshuai Li, Yaochu Jin*

#### 🧩 TL;DR
HMACE是一个异构多智能体协作进化框架，将启发式搜索重构为组织设计问题，通过角色分工的四个智能体（Proposer、Generator、Evaluator、Reflector）协同工作，在多个NP-hard组合优化问题上实现了优于现有单智能体和多智能体方法的性能与效率权衡。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于LLM的自动启发式设计方法通常依赖单一工作流和刚性模板，限制了记忆引导的探索能力并容易导致过早收敛到局部最优解，亟需一种自主协作的架构来克服上述局限。

**Method:** HMACE将每一轮进化生成分解为一个自主的、角色专业化的循环，包含四个协调的智能体：Proposer负责策略探索、Generator负责可执行启发式合成、Evaluator负责实证评估、Reflector负责基于归档的记忆更新；通过行为感知检索、轻量级候选过滤和基于适应度的归档更新机制，引导搜索朝向多样且富有前景的启发式行为。

**Result:** 在TSP、Online BPP、MKP和PFSP等多个代表性组合优化问题上的广泛评估表明，HMACE相比最先进的单智能体和多智能体基线实现了更优的质量-效率权衡；在匹配的LLM驱动参考对比中，HMACE在TSP和Online BPP上分别达到最低的平均差距（0.464%和0.223%），同时仅需0.13M和0.42M tokens，远少于对比基线。

**Conclusion:** HMACE通过将启发式搜索建模为多智能体组织设计问题，有效避免了过早收敛并提升了搜索效率，验证了角色专业化与协作记忆机制在自动化启发式设计中的潜力，为NP-hard组合优化问题提供了一种高质量且高效的新范式。
...
### [82] [Can Agents Price a Reaction? Evaluating LLMs on Chemical Cost Reasoning](https://arxiv.org/abs/2605.07251)
...
#### 🧩 TL;DR
本文提出ChemCost，一个基于化学品采购成本估算任务的科学工具使用基准，包含1,427个可评估反应和23万余条供应商报价数据，用于严格评估LLM代理在化学领域工具使用中的表现，实验表明最强代理在干净输入下仅达50.6%的准确率，且面对噪声时性能显著下降。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM代理在通用任务上的工具使用评估已较为成熟，但在科学领域尤其是化学任务中，评估多依赖人工筛选的演示、专家评判或LLM作为裁判的打分，缺少基于精确地面真值的自动化评估方法，难以客观衡量代理在真实科学场景中的工具使用能力。

**Method:** 本文构建了ChemCost基准，包含1,427个可评估的化学反应，冻结定价快照覆盖2,261种化学品和230,775条供应商报价，支持标量评分和分阶段诊断（识别、检索、采购、算术四个阶段）。为评估鲁棒性，进一步构造了受控噪声注入视图，扰动化学别名、数量表达、缺失字段和输入格式。

**Result:** 对前沿、开源和化学专用LLM代理的实验表明，工具访问是必要但不充分的。最强代理在干净输入下仅达50.6%的准确率（25%相对误差内），在引入现实噪声后性能大幅下降。阶段级分析显示失败源于脆弱的解析、无效的证据整合、不合格的包装选择以及不收敛的工具使用。

**Conclusion:** 本研究表明当前LLM代理在科学工具使用任务中仍存在显著短板，尤其在面对现实输入噪声时鲁棒性不足；分阶段诊断揭示了多个关键失效环节，为未来提升科学代理的推理与工具整合能力提供了明确方向。
...
### [83] [Structured Role-Aware Policy Optimization for Multimodal Reasoning](https://arxiv.org/abs/2605.07274)
*Bingqing Jiang, Difan Zou*

#### 🧩 TL;DR
提出SRPO方法，通过角色感知的token级信用分配替代序列级GRPO优势，在不改变奖励函数的情况下，将感知令牌和推理令牌分别加权，从而提升多模态推理中证据支撑的正确答案质量。

---

#### 📘 Detailed Summary
**Motivation:** 现有RLVR方法（如GRPO）在多模态推理中仅分配序列级最终答案奖励，无法区分感知令牌和推理令牌的不同功能角色，导致难以判断正确答案是否真正由任务相关的视觉证据支撑，缺乏细粒度的角色感知信用分配机制。

**Method:** 提出结构化角色感知策略优化（SRPO），在不改变奖励函数的前提下，将序列级GRPO优势分解为角色感知的token级优势：对感知令牌，通过原始与损坏视觉输入下的视觉依赖性对比进行自蒸馏加权；对推理令牌，通过其与生成感知的一致性进行加权；最后通过共享轨迹级基线统一两类信号，生成正向token权重以调整相对更新幅度。

**Result:** 在多种多模态推理基准上的实验表明，SRPO显著提升了基于证据的推理能力，验证了从统一序列级信用分配转向角色感知优化的有效性。

**Conclusion:** SRPO揭示了在多模态强化学习推理中，超越序列级信用分配、引入角色感知优化对于实现可靠的多模态推理至关重要，且该方法无需外部奖励模型或独立教师模型，具有良好的实用性和扩展性。
...
### [84] [SOM: Structured Opponent Modeling for LLM-based Agents via Structural Causal Model](https://arxiv.org/abs/2605.07301)
*Shiyue Cao, Pei Xu, Likun Yang, Lei Cui, Xiaotang Chen, Kaiqi Huang*

#### 🧩 TL;DR
提出结构化对手建模（SOM）框架，通过结构因果模型（SCM）将对手模型构建与预测分离，使LLM在多智能体博弈环境中实现更准确和稳定的对手行为预测与战略决策。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于LLM的对手建模方法通常将对手建模与预测纠缠在一起，依赖隐式的上下文推理，在动态交互中适应性受限，缺乏明确的结构化对手表示来指导预测过程。

**Method:** SOM采用两阶段框架：在对手模型构建阶段，使用结构因果模型（SCM）以图形式化方法捕获对手观测与行动之间的有向依赖关系，生成显式结构化的对手表示；在预测阶段，LLM沿着SCM导出的清晰路径进行结构化推理，提升预测准确性与稳定性。

**Result:** 在多种多智能体基准测试上的大量实验表明，SOM在对手行为预测和适应性战略决策方面一致优于最先进的基于LLM的推理基线方法，在复杂动态多智能体交互中表现更优。

**Conclusion:** SOM通过显式解耦对手模型构建与预测过程，为LLM在多智能体博弈中的对手建模提供了更结构化、可解释且稳健的范式，具有提升复杂动态环境下智能体战略决策能力的潜力。
...
### [85] [Discovering Ordinary Differential Equations with LLM-Based Qualitative and Quantitative Evaluation](https://arxiv.org/abs/2605.07323)
*Sum Kyun Song, Bong Gyun Shin, Jae Yong Lee*

#### 🧩 TL;DR
DoLQ 提出一种基于 LLM 多智能体架构的符号回归方法，通过定性与定量联合评估来发现常微分方程，在多个基准上取得了更高的成功率和更准确的符号项恢复效果。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于符号回归的微分方程发现方法主要依赖定量指标，难以融入领域知识以确保结果的物理合理性，亟需一种能结合定性与定量评估的框架来弥补这一不足。

**Method:** DoLQ 采用多智能体架构：Sampler Agent 负责提出动态系统候选方程，Parameter Optimizer 对候选方程进行参数优化以提高拟合精度，Scientist Agent 利用 LLM 同时进行定性（物理合理性）和定量（数据拟合误差）评估，并综合两种评估结果迭代引导搜索过程。

**Result:** 在多维常微分方程基准测试上，DoLQ 相比现有方法不仅获得了更高的方程发现成功率，还能更准确地恢复出真实方程中的正确符号项。

**Conclusion:** 将 LLM 的语义理解能力引入符号回归，通过定性与定量联合评估机制，能有效提升微分方程发现的物理合理性和准确性，为该领域提供了一种新的可行范式。
...
### [86] [Tools as Continuous Flow for Evolving Agentic Reasoning](https://arxiv.org/abs/2605.07339)
*Tairan Huang, Siyu Shang, Qiang Chen, Xiu Su, Yi Chen*

#### 🧩 TL;DR
提出FlowAgent框架，将工具链推理重定义为语义空间中的连续轨迹生成，利用条件流匹配实现全局规划视角，在长时程推理任务中展现出更优的鲁棒性和泛化能力。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于大语言模型的工具编排方法依赖逐步（step-wise）范式，缺乏全局视角，导致在长时程任务中存在误差累积问题，且难以泛化到未见过的工具。

**Method:** 提出FlowAgent框架，将工具链推理重概念化为语义空间中的连续轨迹生成，利用条件流匹配（conditional flow matching）生成连续的隐空间轨迹，以全局规划视角确保工具执行的连贯性和鲁棒性；同时引入首个面向规划级智能体推理的闭环基准测试。

**Result:** 理论分析建立了效用收敛的正式界，并证明连续形式化方法能够保证鲁棒泛化和误差衰减；实证评估表明FlowAgent在长时程推理任务中具有优异的鲁棒性和适应性。

**Conclusion:** 连续轨迹生成范式可有效克服逐步推理中的误差累积问题，为长时程、动态环境下的智能体推理提供了一种兼具理论保障和实践有效性的新途径。
...
### [87] [LiteGUI: Distilling Compact GUI Agents with Reinforcement Learning](https://arxiv.org/abs/2605.07505)
*Yubin Wu, Zicheng Cai, Liping Ning, Hua Wang, Zhi Chen, Yaohua Tang, Hao Chen*

#### 🧩 TL;DR
提出了一种无需监督微调（SFT-free）的训练范式，通过引导式在策略知识蒸馏和多解双层GRPO框架显著提升轻量级GUI智能体性能，使2B/3B规模模型在所有基准测试中达到SOTA，并与大规模模型竞争。

---

#### 📘 Detailed Summary
**Motivation:** 当前的轻量级设备上视觉语言GUI智能体受限于模型容量，需要进一步提升性能；传统监督微调（SFT）在小规模模型上容易导致过拟合、灾难性遗忘和策略僵化，无法有效解决上述挑战。

**Method:** 提出了一种无SFT的训练范式，首先通过引导式在策略蒸馏（Guided On-policy Distillation）将知识蒸馏系统性地引入GUI智能体领域，结合oracle参考轨迹和动态检索机制减少幻觉并缓解多解GUI任务中的认知错位；在此基础上进一步提出多解双层GRPO框架（Multi-solution Dual-level GRPO），联合对齐宏观层面的子任务规划与微观层面的执行匹配，增强长时序GUI场景中的探索能力；同时构建了自动化数据生成管线，合成带有丰富多解标注的GUI任务轨迹。

**Result:** 该方法在轻量级模型中达到了最优性能（SOTA），并在所有基准测试中与规模更大的模型保持竞争力；消融实验表明，结构化在策略蒸馏和多解双层探索能够充分释放2B/3B规模智能体的能力，超越传统模仿学习的性能极限。

**Conclusion:** 结构化在策略蒸馏与多解双层探索可以完全解锁小规模GUI智能体的潜力，无SFT的训练范式相比传统模仿学习具有显著优势，为轻量级设备上GUI智能体的性能提升提供了有效新路径。
...
### [88] [Model-Driven Policy Optimization in Differentiable Simulators via Stochastic Exploration](https://arxiv.org/abs/2605.07520)
*Yuval Aroosh, Ayal Taitler*

#### 🧩 TL;DR
提出模型驱动策略优化（MDPO）框架，通过在可微规划的动作空间中注入自适应噪声来改善高度非线性和混合离散-连续决策问题中的优化景观，实验表明其显著优于确定性可微规划和无模型基线方法。

---

#### 📘 Detailed Summary
**Motivation:** 可微规划在高度非线性和混合离散-连续决策域中面临优化景观病态问题，存在平坦区域和尖锐过渡，阻碍梯度基优化方法有效求解。

**Method:** MDPO框架在可微规划的动作空间中注入噪声以引入随机探索，并利用模型访问能力根据轨迹目标的梯度敏感性动态调整噪声幅度，生成时变探索剖面，实现跨时间步和迭代的动态探索分配。

**Result:** 在基准测试中，MDPO在挑战性非线性和混合设置下持续优于确定性可微规划（包括无噪声变体和现有最先进实现）以及PPO等无模型基线，显著提升了求解质量。

**Conclusion:** 自适应噪声幅度在时间步和优化迭代中的演化分析揭示了学习过程中的探索分配机制，表明动态分配探索有助于逃离局部较差最优，提升了可微规划在复杂决策问题中的实用性。
...
### [89] [From Feasible to Practical: Pareto-Optimal Synthesis Planning](https://arxiv.org/abs/2605.07521)
*Friedrich Hastedt, Dongda Zhang, Antonio del Rio Chanona*

#### 🧩 TL;DR
MORetro* 将逆合成规划建模为多目标搜索问题，通过加权标量化和贝叶斯优化采样生成帕累托前沿，在多个基准上展现了优于单目标方法的多样性路线，更贴合工业决策需求。

---

#### 📘 Detailed Summary
**Motivation:** 现有计算机辅助逆合成规划（CASP）方法通常将找到一条可行路线视为任务完成，仅关注收敛性或最短路径等单一指标，这与化学工业实践中需要同时权衡成本、可持续性、毒性和总收率等多重目标的真实需求严重脱节。

**Method:** MORetro* 将逆合成规划形式化为多目标搜索问题，基于多目标 A* 搜索框架，引入加权标量化策略将多目标化为单目标以引导搜索方向，并利用贝叶斯优化（BO）驱动的采样方法高效探索组合搜索空间、优先发现具有良好折衷性质的合成路线，最终生成显式表征各目标间权衡关系的帕累托前沿。

**Result:** 在多个逆合成基准数据集上，MORetro* 生成了多样且高质量的帕累托前沿，能够发现被单目标方法遗漏的合成路线，从而在目标空间覆盖度、路线多样性等方面显著超越传统单目标方法。理论分析证明，在固定单步逆合成模型的前提下，MORetro* 可保证恢复真实的帕累托前沿。

**Conclusion:** MORetro* 通过引入多目标优化视角为逆合成规划提供了与工业决策更契合的解决方案，证明了在合成路线搜索中显式处理目标间权衡是可行且有效的，有望推动 CASP 系统从“寻路”范式向“权衡决策支持”范式转变。
...
### [90] [From Pixels to Prompts: Vision-Language Models](https://arxiv.org/abs/2605.07544)
*Khang Hoang Nhat Vo*

#### 🧩 TL;DR
本文旨在为视觉-语言模型（VLM）领域提供一套清晰的思维框架而非详尽的技术目录，帮助读者在快速发展的领域中获得深层理解，从而能够自信地阅读前沿论文并自主设计系统。

---

#### 📘 Detailed Summary
**Motivation:** 视觉-语言模型领域发展极为迅速，新模型层出不穷，导致研究者（尤其是初学者）很容易在该领域迷失方向。作者指出，当前存在一种令人不安的脱节：了解行业术语与真正理解底层工作原理之间存在巨大鸿沟。现有资源往往偏向于罗列数据集、基准和模型变种，缺乏能够帮助读者建立系统性认知的持久性知识结构。

**Method:** 本文不提出新的模型或算法，而是采用知识体系化的方法：通过构建一个清晰的"思维地图"（mental map）来组织视觉-语言模型的核心概念、架构范式和设计原则。该地图旨在提供足够的结构化知识，使读者能够自信地理解新论文，并培养足够的直觉来自主设计VLM系统，而非像盲目拼装乐高积木般操作。

**Result:** 本文作为一本专著而非研究论文，没有提供具体的实验指标或基准测试结果。其核心成果是一种认知框架的构建——弥合了从"知道术语"到"真正理解原理"之间的认知差距，使读者能够在快速变化的领域中建立持久的、可迁移的理解能力。

**Conclusion:** 本文的核心洞见在于：在技术快速迭代的AI领域，相比提供随时可能过时的详尽技术清单，构建可迁移的思维框架和价值定位具有更持久的价值。理解视觉-语言模型的关键不在于记忆每个新模型的名称，而在于掌握其背后的设计逻辑和共性原理，这才能让研究者具备独立设计和批判性分析的能力。
...
### [91] [Parallel Lifted Planning via Semi-Naive Datalog Evaluation](https://arxiv.org/abs/2605.07584)
*Dominik Drexler, Oliver Joergensen, Jendrik Seipp*

#### 🧩 TL;DR
本文提出了一种基于Datalog评估的双层并行执行模型，用于加速提升式经典规划。该方法结合基于团枚举的半朴素Datalog地面化器，在单核上即可超越基线，且随核心数增加优势扩大，在难地面化任务上实现了8核6倍加速和92.4%的平均并行分数。

---

#### 📘 Detailed Summary
**Motivation:** 提升式经典规划虽避免了计算昂贵的显式地面化步骤，但因搜索过程中需要反复实例化地面结构而通常运行较慢。现有研究虽已利用Datalog评估分析了提升式规划的核心组件，但缺乏利用并行计算来加速该过程的系统性方法。

**Method:** 本文构建并分析了一个包含两个并行层次（规则级并行和地面化并行）的执行模型，并针对规划工作负载进行了专门优化。具体而言，提出了一种基于团枚举的地面化器，并将其扩展以支持半朴素Datalog评估，从而高效处理规划领域中的实例化任务。

**Result:** 在贪婪最佳优先搜索与FF启发式下的实验表明，该实现在单核上即可解决比基线更多的任务，且随核心数增加差距进一步扩大。在难地面化任务中（平均97.6%的运行时间用于Datalog执行），所提执行模型实现了92.4%的平均并行分数，并在8核上获得了高达6倍的加速效果。

**Conclusion:** 研究表明，利用双层并行Datalog评估能够有效加速提升式经典规划，尤其是在地面化困难的领域。该工作为规划系统的并行化执行提供了可扩展的框架，并揭示了Datalog作为规划推理底层引擎的高并行潜力。
...
### [92] [Learning to Communicate Locally for Large-Scale Multi-Agent Pathfinding](https://arxiv.org/abs/2605.07637)
*Valeriy Vyaltsev, Alsu Sagirova, Anton Andreychuk, Yuri Kuratov, Konstantin Yakovlev, Aleksandr Panov, Alexey Skrynnik*

#### 🧩 TL;DR
提出一种可泛化的预训练模型LC-MAPF，通过引入局部通信模块实现相邻智能体间的多轮信息共享，在不牺牲可扩展性的前提下显著优于现有基于学习和强化学习的多智能体路径规划方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于学习的分散式MAPF求解器将问题建模为Dec-POMDP，但缺乏高效的智能体间协作机制；而引入通信的MAPF求解器往往面临可扩展性瓶颈，难以在保持低计算开销的同时提升协调效果。

**Method:** 提出LC-MAPF，一种基于预训练的通用模型，核心是一个可学习的局部通信模块，支持相邻智能体之间进行多轮信息交换，从而实现高效的特征共享与协作决策，且设计上避免了通信带来的可扩展性问题。

**Result:** 在多种未见过的测试场景中，LC-MAPF在多个评估指标上全面超越现有基于强化学习和模仿学习的MAPF求解器，且通信机制的引入并未损害模型的可扩展性，克服了通信型MAPF求解器的常见瓶颈。

**Conclusion:** 通过局部通信实现智能体间高效特征共享是提升学习型MAPF求解器性能的有效路径，且合理设计的通信模块可以在不牺牲可扩展性的前提下显著增强协调能力，为实际多机器人路径规划提供了可行方案。
...
### [93] [GASim: A Graph-Accelerated Hybrid Framework for Social Simulation](https://arxiv.org/abs/2605.07692)
*Xuan Zhou, Yanhui Sun, Hantao Yao, Allen He, Yongdong Zhang, Wu Liu*

#### 🧩 TL;DR
提出GASim，一种图加速的混合多智能体框架，通过图优化记忆和图消息传递分别加速LLM核心智能体和数值ABM普通智能体，实现9.94倍端到端加速并保持真实舆论趋势对齐。

---

#### 📘 Detailed Summary
**Motivation:** 现有混合社会模拟方法将LLM智能体与数值ABM结合以扩展规模，但面临高延迟问题，原因在于LLM智能体需要昂贵的内存检索，且ABM的执行是顺序的，难以高效处理大规模仿真场景。

**Method:** 提出GASim框架，包含三项核心技术：Graph-Optimized Memory (GOM)将LLM核心智能体的记忆检索替换为稀疏记忆图上的轻量级传播；Graph Message Passing (GMP)通过细粒度特征聚合与图注意力网络实现普通智能体的并行更新，取代顺序ABM执行；Entropy-Driven Grouping (EDG)利用信息熵动态识别信息多样性邻域中的新兴核心智能体，协调混合分区。

**Result:** 实验表明，GASim相比传统混合框架实现9.94倍端到端加速，消耗基线token的不到20%，显著降低成本，同时保持与真实世界舆论趋势的强对齐。

**Conclusion:** GASim验证了图结构加速在大规模社会模拟中的有效性，通过图优化记忆与图消息传递并行化显著降低延迟与计算开销，为构建高效、低成本且保真的大规模社会仿真系统提供了可行方案。
...
### [94] [Finite-Time Analysis of MCTS in Continuous POMDP Planning](https://arxiv.org/abs/2605.07703)
*Da Kong, Vadim Indelman*

#### 🧩 TL;DR
本文为部分可观测马尔可夫决策过程（POMDP）中的蒙特卡洛树搜索（MCTS）提供了有限时间收敛分析，在离散和连续观测空间下分别给出了概率浓度界；并提出了带Voronoi自适应分区保证的Voro-POMCPOW算法，在保持理论保障的同时取得了有竞争力的实证表现。

---

#### 📘 Detailed Summary
**Motivation:** 尽管基于MCTS的求解器（如POMCP）在诸多领域取得了经验性成功，但由于POMDP中启发式动作选择（如UCB）带来的非平稳性和状态-观测间的相互依赖性，该类算法在有限时间内的严格理论保证仍是一个悬而未决的问题。

**Method:** 在离散观测空间下，论文将多项式探索奖励扩展至POMDP中的UCB策略，从而导出根节点经验值估计的多项式浓度界。在连续观测空间下，引入了一个抽象分区框架，并提出了分区损失（partitioning loss）的有限时间界；进一步提出了Voro-POMCPOW算法，该算法通Voronoi单元自适应地划分连续观测空间，在保持原始观测生成器的同时维持有限的分支因子。

**Result:** 理论证明在温和条件下，所提方法可为POMDP中连续观测空间下的值估计以高概率提供有限时间误差界。实验验证表明，Voro-POMCPOW在提供理论保证的同时，性能与现有方法具有竞争力。

**Conclusion:** 本文首次为POMDP中MCTS类算法提供了严格的有限时间概率保证，所发展的技术同样适用于连续MDP，从而填补了MDP侧的理论空白。
...
### [95] [Hierarchical Task Network Planning with LLM-Generated Heuristics](https://arxiv.org/abs/2605.07707)
*Felipe Meneguzzi, Alexandre Buchweitz, Augusto B. Corrêa, Victor Scherer Putrich, André Grahl Pereira*

#### 🧩 TL;DR
本文研究利用大型语言模型（LLM）为HTN规划生成搜索启发式，在六个基准域上的实验表明，LLM生成的启发式在覆盖率上接近最优HTN规划器，并在83%的共享问题上显著降低了搜索代价。

---

#### 📘 Detailed Summary
**Motivation:** HTN规划的启发式方法虽已有进展，但其信息量仍远不及经典规划中的启发式，这限制了HTN规划的搜索效率。本文旨在探索LLM能否为HTN规划生成有效的搜索启发式，以弥补这一差距。

**Method:** 本文将Corrêa等人（2025）从经典规划扩展到层次化规划的方法论，使用Pytrich规划器在六个标准全序HTN基准域上，通过域特定提示由九种不同LLM生成启发式，并与TDG和LMCount两种域无关基线以及PANDA规划器进行对比。

**Result:** 实验结果表明，LLM生成的启发式在问题覆盖率上几乎达到了现有最优HTN规划器的水平，同时在83%的共享问题上显著减少了搜索代价（即搜索空间探索量）。

**Conclusion:** 本研究证明了LLM能够为HTN规划生成接近最先进水平的高效启发式，尤其在降低搜索代价方面表现突出，为层次化规划中启发式自动生成提供了新的可行路径。
...
### [96] [Online Goal Recognition using Path Signature and Dynamic Time Warping](https://arxiv.org/abs/2605.07736)
*Douglas Tesch, Nathan Gavenski, Leonardo Amado, Odinaldo Rodrigues, Felipe Meneguzzi*

#### 🧩 TL;DR
本文提出一种基于路径签名（path signatures）的在线目标识别方法，利用粗糙路径理论的紧凑表示高效编码轨迹并实现有意义的比较，在预测准确率和在线规划效率上持续超越现有方法。

---

#### 📘 Detailed Summary
**Motivation:** 现有在线目标识别方法在连续域中面临两大挑战：高效编码大规模轨迹和有效比较轨迹。尽管已有工作采用自定义状态空间表示和度量来进行假设比较，但这些方法往往忽略了其他领域已被验证的高效编码技术。

**Method:** 本文引入基于粗糙路径理论（rough path theory）的路径签名（path signatures）作为轨迹的紧凑且富有表现力的表示，该表示能够高效捕获轨迹的关键语义特征，从而实现对轨迹之间更有意义的比较。

**Result:** 实验结果表明，该方法在预测准确率和在线规划效率上持续优于现有最先进方法，同时在离线场景下也能保持具有竞争力的性能。

**Conclusion:** 路径签名作为一种来自粗糙路径理论的轨迹编码技术，能够有效弥补现有在线目标识别方法在轨迹表示和比较方面的不足，为连续域中的在线目标识别提供了一种更高效、更准确的解决方案。
...
### [97] [Alternating Target-Path Planning for Scalable Multi-Agent Coordination](https://arxiv.org/abs/2605.07744)
*Yu Kumagai, Keisuke Okumura*

#### 🧩 TL;DR
本文提出了一个解耦目标分配与路径规划的迭代优化框架，利用现代快速次优MAPF求解器（如LaCAM）反复求解当前分配下的MAPF问题，通过路径规划反馈识别瓶颈智能体并调整目标分配，从而大幅提升TAPF问题的可扩展性，在保持可接受解质量的同时远超现有基于CBS的最优求解器的规模上限。

---

#### 📘 Detailed Summary
**Motivation:** 现有TAPF求解方法完全依赖于Conflict-Based Search (CBS)，将目标分配与路径规划紧密耦合，导致计算开销巨大且难以扩展到大规模问题场景，无法满足实际部署对可扩展性的需求。

**Method:** 提出了一种迭代优化框架，将目标分配与路径规划解耦；在给定时间预算内，框架反复调用现代快速次优MAPF求解器（如LaCAM）求解当前目标分配下的路径规划问题，然后利用路径规划反馈识别瓶颈智能体并据此调整目标分配，形成反馈驱动的分配优化循环。

**Result:** 实验结果表明，该反馈驱动的重分配循环机制在保持良好解质量的同时，可使TAPF求解规模远超当前最先进的CBS求解器，显著提升了可扩展性。

**Conclusion:** 该工作验证了将目标分配与路径规划解耦并结合反馈迭代优化是实现大规模实用化TAPF的有效路径，为实际场景中的大规模多智能体任务分配与路径规划提供了可行方案。
...
### [98] [RuleSafe-VL: Evaluating Rule-Conditioned Decision Reasoning in Vision-Language Content Moderation](https://arxiv.org/abs/2605.07760)
*Zhifeng Lu, Dianyuan Wang, Yuhu Shang, Zhenbo Xu*

#### 🧩 TL;DR
RuleSafe-VL是一个面向视觉语言内容审核中规则条件决策推理的诊断基准，包含93条原子规则、92种规则关系和2166个跨高风险政策族的图文案例，实验表明现有VLM在规则关系恢复和决策状态预测上表现严重不足。

---

#### 📘 Detailed Summary
**Motivation:** 当前多模态安全基准主要将内容审核简化为匹配预定义最终标签，无法检验模型是否真正理解并正确应用平台政策中的规则结构、规则交互及证据充分性判断，导致高基准分数可能仅来自表面线索而非正确的策略推理。

**Method:** RuleSafe-VL从公开的平台审核策略中形式化定义了93条原子规则和92种类型化规则关系，构建了2166个跨三个高风险政策族的图像-文本情境案例，并设计了四项诊断任务来将审核决策分解为规则激活识别、规则交互恢复、决策充分性判断和缺失上下文下的结果推理这一规则条件决策链。

**Result:** 在10个前沿、开源和安全导向的VLM上的实验表明，规则关系恢复是主要瓶颈，最佳模型仅达64.8 Macro-F1，部分安全导向模型低于7 Macro-F1；决策状态预测同样不可靠，最高仅64.5 Macro-F1。

**Conclusion:** RuleSafe-VL将内容审核评估从最终标签评分转向对规则条件决策推理的诊断性评估，揭示了现有VLM在理解规则交互方面存在显著短板，为多模态安全模型的能力评估提供了更细粒度和结构化的新范式。
...
### [99] [AgentEscapeBench: Evaluating Out-of-Domain Tool-Grounded Reasoning in LLM Agents](https://arxiv.org/abs/2605.07926)
*Zhengkang Guo, Yiyang Li, Lin Qiu, Xiaohua Wang, Jingwen Xv, Dongyu Ru, Xiaoyu Li, Xiaoqing Zheng, Xuezhi Cao, Xunliang Cai*

#### 🧩 TL;DR
提出AgentEscapeBench，一个基于密室逃脱范式的基准测试，用于评估LLM智能体在长距离工具依赖推理中的能力，实验发现当前最强模型在深度依赖场景下性能显著下降，主要失败原因在于长期状态跟踪和中间结果传播的断裂。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM智能体评估大多聚焦于短程交互或熟悉工具链的使用，缺乏对智能体在长距离依赖约束下进行工具推理、执行与自适应修正能力的系统测试，因而无法全面衡量智能体在复杂现实场景中的泛化推理与行动能力。

**Method:** 设计AgentEscapeBench基准，每个任务定义了一个工具与物品间的有向无环依赖图，要求智能体调用真实外部函数、追踪逐步揭示的隐藏状态、传播中间结果并提交可确定性验证的最终答案；基准包含270个实例，横跨五个难度层级，并支持全自动评估。

**Result:** 在16个LLM智能体与人类参与者的实验中发现，随依赖深度增加性能急剧下降：人类成功率从难度5的98.3%降至难度25的80.0%，最佳模型从90.0%降至60.0%；轨迹分析表明模型失败主要源于长程状态追踪、线索遵循和中间结果传播的断裂。

**Conclusion:** 当前智能体在处理局部工具使用时表现尚可，但在深层上下文依赖场景中仍存在显著困难；AgentEscapeBench可作为诊断性测试平台，用于衡量当前智能体能力并指导未来针对通用推理、行动和自适应的训练方向。
...
### [100] [TraceFix: Repairing Agent Coordination Protocols with TLA+ Counterexamples](https://arxiv.org/abs/2605.07935)
*Shuren Xia, Qiwei Li, Taqiya Ehsan, Jorge Ortiz*

#### 🧩 TL;DR
TraceFix提出了一种验证优先的多智能体协调流水线，通过TLA+模型检查器生成并修复形式化验证的协议拓扑，将验证通过的过程体编译为智能体提示，运行时监控拒绝越界操作，在48个任务中全部通过验证且运行时任务完成率达89.4%。

---

#### 📘 Detailed Summary
**Motivation:** 现有LLM多智能体协调缺乏形式化保证，智能体间通信可能偏离预期协议，导致死锁、活锁或越界协调操作，影响系统可靠性和任务完成率。

**Method:** TraceFix采用验证优先流水线：智能体从任务描述中合成协议拓扑作为结构化中间表示（IR），生成PlusCal协调逻辑，利用TLA+模型检查器（TLC）产出的反例迭代修复协议直至验证通过；验证通过的过程体被编译为每个智能体的系统提示，并在运行时监控器下执行，拒绝任何越界协调操作。

**Result:** 在覆盖16个场景家族的48个任务上，所有任务均通过TLC完全验证，62.5%首次尝试即通过，最多不超过4次修复迭代；状态空间跨越六个数量级但验证均在60秒内完成；3,456次运行时对比实验表明，拓扑监控执行的最高任务完成率为89.4%（平均）、81.5%（完全），且使用验证协议时的运行时性能下降速度仅为纯提示和纯聊天基线的约一半；固定运行时下的配对消融实验显示，TLC验证的协议将死锁/活锁从31.1%降至14.1%，故障注入下的分离效果最为显著。

**Conclusion:** TraceFix验证了形式化验证与LLM智能体生成相结合的有效性，证明验证优先的流水线能够显著提升多智能体协调的可靠性与鲁棒性，为构建可信赖的LLM协调系统提供了可行路径。
...
### [101] [Reason to Play: Behavioral and Brain Alignment Between Frontier LRMs and Human Game Learners](https://arxiv.org/abs/2605.08019)
...
#### 🧩 TL;DR
本文通过复杂人类游戏行为与fMRI数据，系统比较了大推理模型（LRMs）与深度强化学习及贝叶斯模型，发现LRMs在行为匹配和脑活动预测上显著优于其他方法，且脑对齐效果源于模型对游戏状态的上下文表示而非下游推理。

---

#### 📘 Detailed Summary
**Motivation:** 人类在陌生环境中能快速学习抽象知识并灵活用于规划和决策，而现代AI系统是否具备类似能力尚不明确。现有研究缺乏在复杂、自然化任务中同时评估AI模型的行为表现、学习动态与神经机制对齐程度的系统性工作。

**Method:** 研究基于一个包含并发fMRI记录的人类复杂游戏数据集，参与者在游戏中需进行规则发现、假设修正和多步规划。研究者比较了前沿大推理模型（LRMs）、无模型与基于模型的深度强化学习智能体、以及基于贝叶斯理论的智能体，从游戏完成能力、人类行为模式匹配和脑活动预测三个维度进行联合评估。

**Result:** 前沿LRMs在游戏发现阶段的行为模式与人类最为接近，其对脑活动的预测能力比两种强化学习替代方案高出一个数量级，该效应在皮层和皮层下区域均显著且经排列检验稳健。进一步的分析表明，脑对齐效果主要源于模型对游戏状态的上下文表示，而非其下游规划或推理过程。

**Conclusion:** 研究确立了LRMs作为人类在复杂自然环境中学习与决策的计算模型的优越性，揭示了上下文状态表示而非推理过程是神经对齐的关键因素，为理解AI与人类认知的相似性提供了新视角。
