...
### [1] [Can Multimodal Large Language Models Understand Pathologic Movements? A Pilot Study on Seizure Semiology](https://arxiv.org/abs/2605.03352)
...
#### 🧩 TL;DR
本文首次系统评估了多模态大语言模型（MLLMs）在癫痫发作视频中识别病理性异常运动的零样本能力，发现其在大多数半特征上优于微调的传统视觉模型，且通过目标信号增强策略可进一步提升性能，为无需任务特定训练的临床视频分析提供了可解释的辅助诊断路径。

---

#### 📘 Detailed Summary
**Motivation:** 尽管MLLMs在日常活动识别中表现优异，但其在神经系统疾病临床意义下的不自主运动（如癫痫发作视频中的病理运动）分析能力尚未被探索，现有方法通常需要针对特定任务进行微调训练，缺乏通用的零样本评估与可解释性分析。

**Method:** 研究评估了多种SOTA多模态大语言模型在90例临床癫痫发作视频上的零样本性能，覆盖20个ILAE定义的半学术特征；与微调的CNN和Vision Transformer基线模型对比，并引入特征目标信号增强（面部裁剪、姿态估计、音频去噪）策略提升模型表现；同时通过癫痫专家评估对模型生成解释的忠实度进行打分。

**Result:** MLLMs在18个半学术特征中的13个上优于微调的CNN和ViT基线模型，无需任务特定训练；在识别显著性姿势和上下文特征方面尤为突出，但在高频率细微运动上表现不足；特征目标信号增强在20个特征中的10个上带来性能提升；专家评估显示，在MLLM正确预测的案例中，94.3%的解释获得了至少60%的忠实度评分，与癫痫专家的推理一致性良好。

**Conclusion:** 研究证明了通过目标预处理策略将通用MLLMs适配到专科临床视频分析的可行性，为无需任务特定训练的、可解释且高效的临床诊断辅助工具提供了实现路径，并公开了代码以推动该方向的进一步研究。
...
#### 🧩 TL;DR
提出MASRA框架，利用MLLM在训练时生成事件级描述和片段级字幕，通过事件语义时间对齐（ESTA）和局部关系一致性对齐（LRCA）实现语义与时间特征的精确对齐，并引入解耦对齐交互（DAI）自适应吸收查询无关语义，在视频时间定位任务上取得优于现有方法的效果。

---

#### 📘 Detailed Summary
**Motivation:** 视频时间定位（VTG）面临跨模态语义鸿沟问题，背景特征常被错误地与查询对齐，而直接将查询与时间片段匹配会导致时间语义的判别性和一致性不足，现有方法难以有效缓解这一根本性对齐偏差。

**Method:** MASRA利用MLLM在训练阶段生成两类文本先验：带时间跨度的事件级描述和片段级字幕，并实例化两种对齐机制。事件语义时间对齐（ESTA）将时间上下文与事件语义对齐以增强跨度级可分离性；局部关系一致性对齐（LRCA）构建基于片段级字幕的文本关系矩阵并与模型的时间特征相似性矩阵对齐。此外，引入语义引导增强和二阶梯关系注意力两个支持模块，以及基于上下文感知码本的解耦对齐交互（DAI）自适应吸收查询无关语义。MLLM仅在训练时使用，推理时不引入额外开销。

**Result:** 大量实验表明MASRA在多个视频时间定位基准上优于现有方法，消融研究验证了各组件（ESTA、LRCA、DAI等）的有效性，证明了所提对齐策略和语义解耦机制对提升VTG性能的显著贡献。

**Conclusion:** 通过在训练阶段引入MLLM辅助的语义-关系一致性对齐，有效弥合了跨模态语义鸿沟，提升了时间语义的判别性和一致性，为VTG任务提供了一种无需推理开销的高效优化范式，未来可进一步探索更复杂的语义先验利用方式。
...
### [3] [MHPR: Multidimensional Human Perception and Reasoning Benchmark for Large Vision-Languate Models](https://arxiv.org/abs/2605.03485)
*Kangkang Wang, Qinting Jiang, Wanping Zhang, Bowen Ren, Shengzhao Wen*

#### 🧩 TL;DR
MHPR 是一个面向人类中心场景的联合感知-推理基准，包含多层级数据设计和自动标注管道，通过格式对齐的 SFT 数据和基于坏例分析的 RL 数据，显著提升了 Qwen2.5-VL-7B 在细粒度属性和高层语义理解上的性能，使其达到与更大模型相当的水平。

---

#### 📘 Detailed Summary
**Motivation:** 现有大视觉语言模型（LVLM）基准大多聚焦于单任务设定，缺乏细粒度、面向人类中心场景的综合评估，无法满足电影分析和虚拟数字人等现实应用对多维度人类理解的需求。

**Method:** 提出了 MHPR 基准，包含 Captioned Raw Data (C-RD)、Supervised Fine-Tuning Data (SFT-D)、Reinforcement Learning Data (RL-D) 和 Test Data (T-D) 四层数据设计，并构建了自动标注管道 ACVG，通过类别级属性分解、属性特定重写和多模型投票实现高质量、可扩展的标注。评估涵盖外观、服饰、姿态、部位等细粒度属性以及社会关系、动作语义、空间关系、意图与功能性等高层语义。

**Result:** 实验发现：1）格式对齐的 SFT 数据显著提升指令跟随能力和稳定性；2）基于坏例分析的挑战聚焦 RL 数据进一步增强了在困难样本上的感知与推理能力；3）使用 MHPR 训练 Qwen2.5-VL-7B 取得显著性能提升，实现了与更大模型近乎相当的表现。

**Conclusion:** MHPR 和 ACVG 提供了一个可复现、可扩展的面向人类中心感知与推理的研究框架，证明了格式对齐的 SFT 和基于坏例分析的 RL 在精细化视觉-语言模型训练中的有效性，并为推动人类中心场景的联合感知-推理研究提供了重要资源。
...
#### 🧩 TL;DR
CoVUBench是首个专为评估大型视觉语言模型（LVLMs）中版权内容遗忘（machine unlearning）效果而设计的基准框架，通过程序化生成的合法合成数据与系统化视觉变化，全面衡量遗忘效果与模型通用能力之间的权衡。

---

#### 📘 Detailed Summary
**Motivation:** LVLMs在海量网络数据训练后可能记忆并再生受版权保护的视觉内容（如角色、商标），机器遗忘技术虽可移除此类内容，但现有评估方法在跨模态概念擦除场景下缺乏鲁棒性且无法捕捉其细微差异，亟需一个标准化、鲁棒的评估框架来填补这一空白。

**Method:** CoVUBench采用程序化生成的合法合成数据，并引入涵盖组合变化与多样化领域表现的系统性视觉变体，以模拟真实场景并评估遗忘泛化能力；同时设计了一个多模态评估协议，分别从版权持有者角度衡量遗忘效果（遗忘有效性）和从部署者角度衡量模型通用能力（效用保留）。

**Result:** 通过严格量化遗忘效果与模型通用性之间的关键权衡，CoVUBench为LVLMs中的版权内容遗忘方法提供了标准化评估工具，推动了负责任且有效的遗忘方法的发展。

**Conclusion:** CoVUBench为LVLMs中的版权内容遗忘评估建立了标准化基准，其系统化的评估协议能够有效指导未来遗忘方法的设计与优化，在保障版权合规的同时维持模型的核心能力，对推动负责任的AI部署具有重要意义。
...
### [5] [Before Forgetting, Learn to Remember: Revisiting Foundational Learning Failures in LVLM Unlearning Benchmarks](https://arxiv.org/abs/2605.03759)
*JuneHyoung Kwon, MiHyeon Kim, Eunju Lee, JungMin Yun, Byeonggeuk Lim, YoungBin Kim*

#### 🧩 TL;DR
ReMem是一个面向大规模视觉-语言模型（LVLM）的可靠多跳多图像记忆基准，通过诊断模型记忆不足（under-memorization）和多跳诅咒（multi-hop curse）两类根本问题，并引入Exposure指标量化信息擦除深度，为LVLM的学习与遗忘行为提供了更可信的评估框架。

---

#### 📘 Detailed Summary
**Motivation:** 现有LVLM遗忘基准多采用虚构身份进行隐私风险评估，但忽略了关键的阶段1失败问题：模型在初始阶段未能有效记忆目标信息，导致后续遗忘评估结果不可靠。论文诊断出记忆不足（under-memorization）和多跳诅咒（multi-hop curse）是造成这一问题的根本原因。

**Method:** 提出ReMem基准，通过原则性的数据缩放（principled data scaling）、基于推理的问答对（reasoning-aware QA pairs）以及多样化的视觉上下文来确保模型的基础学习效果。此外，提出新颖的Exposure指标，用于从模型内部概率分布量化信息擦除的深度。

**Result:** 大量实验表明，ReMem能够为诊断LVLM的学习行为和遗忘行为提供严格且可信的评估框架，有效解决了传统基准中因初始记忆不足而导致的评估不可靠问题。

**Conclusion:** ReMem揭示了LVLM遗忘评估中因初始记忆失败导致的可靠性问题，通过强化基础学习和引入新的遗忘深度量化指标，为隐私保护和模型遗忘研究提供了更严谨的评估工具与方法论指导。
...
### [6] [Enhancing Visual Question Answering with Multimodal LLMs via Chain-of-Question Guided Retrieval-Augmented Generation](https://arxiv.org/abs/2605.03790)
*Quanxing Xu, Ling Zhou, Xian Zhong, Xiaohua Huang, Rubing Huang, Chia-Wen Lin*

#### 🧩 TL;DR
本文提出CoVQD逻辑提示策略，融合链式推理与视觉问题分解，并构建CgRAG框架以增强检索增强型VQA中多模态大语言模型的知识获取与推理能力，在E-VQA、InfoSeek和OKVQA基准上验证了有效性。

---

#### 📘 Detailed Summary
**Motivation:** 尽管多模态大语言模型已广泛应用于开放域视觉问答，但现有方法在检索相关知识时仍面临准确性和相关性不足的问题，尤其是在复杂跨领域场景下，缺乏有效的结构化推理与知识获取引导机制。

**Method:** 提出CoVQD（Chain-of-Thought与Visual Question Decomposition融合）逻辑提示策略，通过结构化推理引导检索更准确的相关知识；基于此构建CgRAG（CoVQD-guided Retrieval-Augmented Generation）框架，使MLLM在结构化视觉-文本推理指导下获取更全面连贯的外部知识。

**Result:** 在E-VQA、InfoSeek和OKVQA三个公开基准上进行大量实验，结果表明所提出方法在复杂跨领域VQA场景中显著提升了泛化能力和可靠性，验证了CoVQD与CgRAG框架的有效性。

**Conclusion:** 将链式推理与视觉问题分解相结合的逻辑提示策略能有效增强检索增强型VQA中MLLM的知识获取质量与推理能力，为开放域多模态问答提供了一种结构化引导的新范式，具有良好的跨领域泛化潜力。
