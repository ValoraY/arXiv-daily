...
### [1] [Harnessing Self-Supervised Features for Art Classification](https://arxiv.org/abs/2605.18974)
*Federico Melis, Davide Bilardello, Emanuele Prato, Evelyn Turri, Lorenzo Baraldi*

#### 🧩 TL;DR
本文系统比较了监督与自监督骨干网络（DINO系列及CLIP）作为特征提取器在艺术品分类与检索任务中的有效性，实验表明自监督骨干网络在分类性能上具有一致优势，并探讨了其在VR博物馆导航等实际应用中的潜力。

---

#### 📘 Detailed Summary
**Motivation:** 艺术品分类因兼具细粒度细节与抽象风格特征而极具挑战性，现有研究缺乏对不同表征学习范式（监督 vs. 自监督）作为特征提取器在该任务上效果的系统性比较与评估。

**Method:** 论文以绘画为主要研究对象，系统性地将DINO系列（自监督）和CLIP（多模态对比学习）等模型作为特征提取器，对多种分类策略与特征表征方式开展了大规模的实验评估。

**Result:** 实验结果表明，采用自监督骨干网络相较于监督式骨干网络在艺术品分类任务上取得了一致的性能提升；同时，研究也为分类与检索模块在实际应用（如支持博物馆导航的VR系统）中的适用性提供了实证依据。

**Conclusion:** 自监督表征学习在艺术品分类中优于监督式方法，表明该类任务受益于更通用的特征表示；研究还为将分类与检索技术部署于虚拟现实等真实场景提供了实践指导与参考。
...
### [2] [A Systematic Failure Analysis of Vision Foundation Models for Open Set Iris Presentation Attack Detection](https://arxiv.org/abs/2605.19020)
*Rahul Anand, Siddharth Singh, Dileep A D, Mahadeva Prasanna, Raghavendra Ramachandra*

#### 🧩 TL;DR
本文系统评估了五种视觉基础模型在开集虹膜呈现攻击检测（PAD）中的泛化能力，发现这些模型在跨数据集场景下表现尚可，但在面对未见攻击仪器、跨光谱迁移时性能急剧下降，表明强闭集性能不能等同于开集安全性。

---

#### 📘 Detailed Summary
**Motivation:** 视觉基础模型在多种视觉识别任务中展现出强大的可迁移性，但其在生物特征识别领域尤其是虹膜呈现攻击检测（PAD）中的适用性尚未被充分研究，特别是在现实开集条件下模型如何应对未见攻击仪器、跨数据集和跨光谱分布偏移缺乏系统性失效分析。

**Method:** 本文在统一实验框架下评估了五种代表性视觉基础模型，采用三种开集协议分别隔离未见攻击仪器（PAI）、跨数据集（不同传感器）和跨光谱（近红外到可见光）的分布偏移，同时对比了冻结特征表示与基于低秩适应（LoRA）的参数高效任务适配两种范式，并额外使用虹膜分割输入、全骨干微调、联合跨数据集与跨PAI偏移以及反向可见光到近红外迁移进行验证实验。

**Result:** 实验结果表明，基础模型能够在相似传感特性的数据集间实现跨数据集迁移，但对未见攻击仪器的泛化不可靠，且在跨光谱评估下性能急剧下降；LoRA在部分跨数据集场景下提升了性能，但在攻击级和光谱偏移下反而放大了失效行为；额外的验证实验确认这些失效并非由虹膜周边输入、弱适配或单向光谱评估等人为因素导致。

**Conclusion:** 研究表明强大的闭集或跨数据集性能不能作为开集安全性的证据，虹膜PAD系统需要既能保持对呈现伪影的灵敏性、又能在现实部署变化下保持稳定的表征方法。
...
### [3] [MedFM-Robust: Benchmarking Robustness of Medical Foundation Models](https://arxiv.org/abs/2605.19027)
*Xiangxiang Cui, Tianjin Huang, Yifang Wang, Lijie Hu, Lu Yin*

#### 🧩 TL;DR
该综述系统梳理了医学基础模型（MedFMs）的两大范式——医学视觉语言模型（Med-VLMs）和分割基础模型，并指出临床部署前亟需对模型在真实世界条件下的可靠性进行严格评估。

---

#### 📘 Detailed Summary
**Motivation:** 随着医学基础模型（包括医学视觉语言模型和分割基础模型）在临床应用中日益普及，模型在真实世界条件下的可靠性评估成为关键未解决问题，而现有研究缺乏对此类模型系统性的可靠性分析。

**Method:** 论文将MedFMs划分为两大类别：一是医学视觉语言模型（Med-VLMs），涵盖LLaVA-Med、MedGemma等医学专用模型以及GPT-4o、Gemini等通用模型，用于视觉问答、报告生成和视觉定位等任务；二是以SAM为基础衍生出的医学分割模型，如SAM-Med2D和MedSAM。

**Result:** 论文系统地归纳了现有MedFMs在不同临床应用场景下的能力范围，指出Med-VLMs和分割基础模型在医疗图像理解方面展现出显著潜力，但尚缺乏统一的可靠性评估标准。

**Conclusion:** 临床广泛部署医学基础模型需要对模型在真实世界条件下的可靠性进行严格评估，这为后续建立统一评测基准和可靠性验证框架提供了明确方向。
...
### [4] [Robust Mitigation of Age-Dependent Confounding Effects via Sample-Difficulty Decorrelation](https://arxiv.org/abs/2605.19230)
*Nikhil Cherian Kurian, Victor Caquilpan Parra, Abin Shoby, Luke Whitbread, Lyle J. Palmer*

#### 🧩 TL;DR
本文提出一种鲁棒框架，通过建模并去相关年龄依赖的样本难度趋势而非强制年龄不变性，来缓解医学图像分类中由年龄导致的性能偏差，在降低年龄相关真假阳性差异的同时保持临床有意义的非线性年龄信息。

---

#### 📘 Detailed Summary
**Motivation:** 医学图像分类中常存在年龄相关的性能差异，原因是年龄作为混杂因素同时影响成像形态与疾病流行率，导致高患病率年龄段的过度诊断和低患病率年龄段的不充分诊断，且该问题在训练-测试年龄分布偏移下会加剧。现有方法强制年龄不变性会抑制年龄中包含的诊断性信息，因此需要一种既能缓解年龄混杂效应又不丢失临床有用年龄信息的鲁棒框架。

**Method:** 该方法在预热阶段后，以标签为条件建模样本难度与年龄之间的趋势关系；通过基于Huber加权的亲和权重将年龄与主导年龄难度趋势去相关，从而衰减混杂驱动的捷径同时保留非线性年龄信息；进一步引入年龄覆盖分数，根据小批次年龄方差缩放去相关惩罚项，确保在年龄多样性有限时优化稳定。

**Result:** 在两个放射学数据集上，该方法有效减少了年龄相关的真阳性率和假阳性率差异，同时对AUC的影响极小，并且在训练-测试年龄分布偏移增大时仍保持鲁棒性。

**Conclusion:** 本文表明，相较于强制年龄不变性，针对性地去相关年龄与样本难度中的混杂趋势是一种更有效的缓解年龄偏差的策略，能够在减少不公平性的同时保留诊断所需的年龄相关信息，对实际部署中面临年龄分布偏移的场景尤其重要。
...
### [5] [RE-VLM: Event-Augmented Vision-Language Model for Scene Understanding](https://arxiv.org/abs/2605.19329)
*Hanqing Liu, Mingjie Liu, Luoping Cui, Endian Lin, Donghong Jiang, Chuang Zhu*

#### 🧩 TL;DR
RE-VLM 提出首个双流视觉-语言模型，联合 RGB 图像与事件相机流，在正常与极端光照、高动态范围等挑战性场景下实现鲁棒的场景理解，并通过图驱动数据合成管道缓解多模态标注稀缺问题，在多个基准上显著优于同参数量 RGB 或事件单模态模型。

---

#### 📘 Detailed Summary
**Motivation:** 传统视觉-语言模型（VLM）在低光、高动态范围或快速运动等恶劣条件下性能显著下降，因为标准 RGB 图像在这些环境中信息严重退化。事件相机以异步方式记录逐像素亮度变化，具有高时间分辨率和宽动态范围，能捕捉帧缺失的运动线索，但现有 VLM 缺乏有效融合 RGB 与事件信息的能力，且 RGB-事件-文本三模态标注数据极为稀缺。

**Method:** RE-VLM 采用双流架构，包含并行的 RGB 编码器和事件编码器，并结合渐进式训练策略以对齐异构视觉特征与语言空间。为缓解三模态标注稀缺问题，提出基于图驱动的数据合成管道：将同步的 RGB-事件流转化为可验证的场景图，再从中合成描述性文本与问答对。同时构建了两个新数据集 PEOD-Chat（聚焦光照挑战场景）和 RGBE-Chat（覆盖多样化场景）用于模型训练与评估。

**Result:** 在图像描述和视觉问答基准上，RE-VLM 在参数量相当的情况下持续优于最先进的 RGB 单模态和事件单模态模型，尤其在恶劣条件下性能提升显著。实验结果表明事件增强的 VLM 在广泛真实环境下的视觉-语言理解中具有有效性和鲁棒性。

**Conclusion:** 该研究首次证明了事件相机流作为互补模态可有效增强 VLM 在非理想成像条件下的场景理解能力，双流融合策略与图驱动数据合成方法为多模态鲁棒感知提供了可行框架。未来工作可进一步探索事件流的时序建模优势与更高效的多模态对齐机制。
...
### [6] [Concept-Guided Noisy Negative Suppression for Zero-Shot Classification and Grounding of Chest X-Ray Findings](https://arxiv.org/abs/2605.19374)
*Chenyu Lian, Hong-Yu Zhou, Chun-Ka Wong, Jing Qin*

#### 🧩 TL;DR
CoNNS提出了一种概念引导的噪声负样本抑制框架，通过构建层次化概念本体和跨患者对重标注策略，有效解决了胸部X光与放射学报告对比学习中的语义歧义问题，在零样本分类与定位任务上超越现有最先进模型。

---

#### 📘 Detailed Summary
**Motivation:** 标准对比学习将不同患者的影像与报告简单视为负样本对，但不同患者常存在相似临床发现，导致大量噪声负样本引入语义歧义，严重损害零样本理解的性能。现有方法未能有效解决这一负样本噪声问题。

**Method:** CoNNS提出三阶段机制：首先利用大语言模型构建包含41个关键临床概念的层次化概念本体，显式建模发现的存在性、属性（位置与特征）及文本（证据片段与存在陈述）；然后基于该本体实施跨患者对重标注策略，包括细粒度分解（按发现存在性分类）、噪声负样本过滤（移除假负样本解决语义冲突）和困难负样本挖掘（利用轻量语言模型识别细微属性差异）；最后提出概念感知NCE损失函数，在对齐视觉与文本特征的同时抑制已识别的噪声负样本。

**Result:** 在多粒度零样本定位任务和五个零样本分类数据集上的大量实验验证表明，CoNNS在所有设置中均优于现有最先进模型。

**Conclusion:** 通过显式建模临床概念的层次化结构并有针对性地抑制噪声负样本，CoNNS有效提升了视觉-语言对齐的语义精度，为零样本医学影像理解提供了新的范式。该工作揭示了负样本质量在医学对比学习中的关键作用，并为未来将临床先验知识融入跨模态学习提供了可行路径。
...
### [7] [Vision Harnessing Agent for Open Ad-hoc Segmentation](https://arxiv.org/abs/2605.19410)
*Zilin Wang, Stella X. Yu*

#### 🧩 TL;DR
VASA是一个无需训练的视觉引导代理，通过将VLM代理、分割基础模型和视觉工作流结合，利用持久工作掩码进行推理、构建和验证，在开放即席分割任务上显著超越现有方法，并展示了AI代理从简单工具封装向具备任务知识、视觉例程和工作记忆的编程化路径的演进方向。

---

#### 📘 Detailed Summary
**Motivation:** 当概念已知时，分割可通过检索已学习的视觉定位变得容易，但对于开放即席概念（open ad-hoc concepts），其视觉定位可能不存在于预训练的分割掩码中，而需要从图像证据中通过部件、关系、排除和集合等方式动态构建，现有方法在处理这类开放即席分割任务时存在明显局限。

**Method:** VASA是一个无需训练的视觉引导代理，它将VLM代理、分割基础模型以及视觉引导的工作流耦合在一起，核心创新在于使用持久工作掩码（persistent working mask）来推理、构建和验证分割解决方案，而非仅依赖文本提示修正；它能够规划视觉操作、调用分割工具、检查结果、编辑掩码并从错误中恢复，实现了一个完整的视觉构造循环。

**Result:** 在构建的新基准PARS上（将PartImageNet的部件级标签转换为开放即席概念），VASA超越开放词汇、基于推理和基于代理的基线方法，比SAM3 Agent高出14-25%；在标准多粒度指代分割基准RefCOCOm上，VASA比SAM3 Agent提升5-9%，比其他基于代理的基线方法提升高达20%。

**Conclusion:** VASA验证了代理化视觉构造（agentic visual construction）在开放即席分割中的有效性，并指明了一条超越将基础模型简单封装为工具的AI代理发展路径：即通过任务知识、VLM行为、视觉例程、工作记忆和具备故障感知能力的工作流来对代理进行编程。
...
#### 🧩 TL;DR
Cutverse 是一个面向专业媒体后期制作场景的 GUI Agent 基准测试，覆盖 7 款专业软件和 186 个长时程复杂任务，实验表明现有模型仅有 36.0% 的任务成功率，揭示了当前智能体在长时程可靠性和领域特定规划方面的严重不足。

---

#### 📘 Detailed Summary
**Motivation:** 尽管 GUI 智能体在网页导航和基础操作系统任务上取得了显著进展，但其在专业创意工作流（如媒体后期制作）中的能力尚未被系统性地探索和评估，存在明显的研究空白。

**Method:** 本文构建了 Cutverse 基准，包含来自 7 款专业应用（如 Premiere Pro、Photoshop）的专家演示，涵盖 186 个基于真实编辑工作流的复杂长时程任务；同时开发了一个轻量级解析器，将原始屏幕录制和低层级交互日志转化为结构化的组合式 GUI 动作轨迹，以实现精确的交互定位和可扩展的评估。

**Result:** 在 Cutverse 基准上，现有 GUI 智能体在真实媒体编辑任务中的成功率仅为 36.0%，尽管当前模型在空间定位、多模态对齐和协调动作执行方面表现出一定能力，但在长时程可靠性和领域特定规划方面仍存在明显局限。

**Conclusion:** Cutverse 揭示了专业媒体后期制作工作流对现有 GUI 智能体的巨大挑战，表明长时程可靠性和领域特定规划能力是当前模型的核心瓶颈，为未来 GUI 智能体在复杂创意场景中的研究指明了关键改进方向。
...
### [9] [deadtrees.earth-aerial: A Multi-Resolution Aerial Image Dataset for Tree Cover and Mortality Detection](https://arxiv.org/abs/2605.19605)
...
#### 🧩 TL;DR
本文提出了DTE-aerial-train和DTE-aerial-bench两个大规模、开放、机器学习就绪的数据集，首次在厘米级航空影像上实现全球尺度树木覆盖与树木死亡率的联合分割，并在基准测试中显著提升了各生物群系尤其是北方森林的死亡率分割性能。

---

#### 📘 Detailed Summary
**Motivation:** 全球森林正日益受到气候变化及火灾、病虫害等干扰的威胁，亟需可扩展的树木覆盖与死亡率监测手段。航空影像是实现精细大规模树木冠层和死亡率制图的关键数据来源，但缺乏全球代表性且统一标注的联合分割数据集严重限制了相关研究的进展。

**Method:** 研究构建了两个开放数据集：DTE-aerial-train包含38.5万个1024×1024像素的影像块（分辨率2.5–20 cm），提供多类别专家标注与审核的伪标签用于树木覆盖和死亡率分割；DTE-aerial-bench包含25幅全球分布的5125个影像块的正射影像，作为地理平衡的高质量专家标注基准测试集。两者均涵盖热带、温带、北方及旱地生物群系。

**Result:** 在DTE-aerial-bench基准测试集上的评估表明，所建立的强基线模型在所有生物群系和尺度上均提升了死亡率分割性能，尤其在北方森林等挑战性区域，F1分数从0.40提升至0.58，相对提升约45%。

**Conclusion:** 本研究通过提供首个全球尺度、多生物群系覆盖的航空影像树木覆盖与死亡率联合分割公开数据集及基准，填补了该领域的关键数据空白。所有数据、模型和代码将以开放许可发布，为全球森林监测与气候变化相关研究提供了重要的基础资源。
...
### [10] [White-Balance First, Adjust Later: Cross-Camera Color Constancy via Vision-Language Evaluation](https://arxiv.org/abs/2605.19613)
*Shuwei Li, Lei Tan, Robby T. Tan*

#### 🧩 TL;DR
VLM-CC提出了一种基于视觉语言模型(VLM)反馈引导的迭代校正框架，将颜色恒常性重新定义为迭代感知反馈问题，通过LoRA微调的VLM评估残差色偏并引导光照估计更新，在跨相机颜色恒常性任务上取得了最优的鲁棒性。

---

#### 📘 Detailed Summary
**Motivation:** 基于学习的颜色恒常性模型在处理跨相机图像时性能严重下降，因为它们容易过拟合训练相机的色彩响应特性，导致对新相机的泛化能力不足，亟需一种不依赖直接RGB回归的鲁棒方法。

**Method:** VLM-CC将颜色恒常性建模为迭代精炼过程：在每次迭代中，当前光照估计对原始图像进行白平衡并转换为伪sRGB，一个轻量级LoRA微调的VLM评估校正后图像，识别主导残色偏并提供定性反馈，该反馈被映射为残差光照方向（红、绿、蓝）以更新光照估计，直至收敛。

**Result:** VLM-CC在多个颜色恒常性数据集上的跨相机测试中实现了最先进的鲁棒性，显著优于传统的直接RGB回归方法，证明了VLM引导的感知反馈范式在消除相机特异性过拟合方面的有效性。

**Conclusion:** VLM-CC的关键洞察在于用VLM引导的感知反馈替代直接RGB回归，将颜色恒常性转化为迭代感知反馈问题，这一范式有效解决了跨相机泛化难题，为依赖相机不变语义线索的视觉任务提供了新思路。
...
### [11] [UniRefiner: Teaching Pre-trained ViTs to Self-Dispose Dross via Contrastive Register](https://arxiv.org/abs/2605.19622)
*Congpei Qiu, Zhaoyu Hu, Wei Ke, Zhuotao Tian, Yanhao Wu, Tong Zhang*

#### 🧩 TL;DR
提出UniRefiner框架，通过对比寄存器机制对预训练Vision Transformers中的三种伪影token进行系统分类与自消除，仅需少量微调即可显著提升密集预测任务性能，例如EVA-CLIP-8B在ADE20K上mIoU提升9.4%至51.9%，超越DINOv2。

---

#### 📘 Detailed Summary
**Motivation:** 现有Vision Transformers在大规模预训练后，其空间感知能力受到伪影token（spurious tokens）的严重制约，而此前研究对此类问题的定义过于狭窄（如仅视为高范数异常值），未能全面涵盖那些未能编码位置对齐语义的token，导致密集预测任务中空间表征被破坏。

**Method:** 提出UniRefiner通用精炼框架，系统性地将伪影token分为三类并分别表征，利用对比寄存器（contrastive registers）实现双目标优化：一方面将图像token与经过筛选的正常token对齐以保持语义完整性，另一方面将寄存器token与检测到的伪影token对齐以捕获伪影信号，从而让预训练ViT学会自主消除这些伪影。

**Result:** 在仅约5000张图像上进行少量epoch微调后，UniRefiner在多种规模ViT（包括EVA-CLIP-8B和InternViT-6B）上取得一致且显著的改进：EVA-CLIP-8B在ADE20K上mIoU达到51.9%（+9.4%），超越DINOv2（49.1%），零样本分割精度提升高达22%。

**Conclusion:** UniRefiner有效解锁了现有大规模基础ViT模型在空间感知任务中的潜在能力，通过轻量级精炼即可显著提升密集预测性能，为广泛部署大规模视觉基础模型开辟了新途径。
...
#### 🧩 TL;DR
本文研究了自监督预训练时长对医学图像模型校准置信度与基于置信度的弃权行为的影响，发现预训练时长对选择性预测性能的影响在准确率饱和后仍显著变化，且更长的预训练并不一致地提升可靠性，强调了弃权感知评估的重要性。

---

#### 📘 Detailed Summary
**Motivation:** 现有自监督学习（SSL）预训练在医学图像领域主要以下游准确率评估性能，但对于糖尿病视网膜病变分级等安全关键场景，模型还需在预测不可靠时主动弃权以转交临床审查。当前缺乏对SSL预训练时长如何影响模型校准置信度与置信度基弃权行为的研究。

**Method:** 在固定微调协议下评估多个SSL预训练检查点，从校准置信度、覆盖度、选择性准确率和选择性宏F1等多个维度分析预训练时长对置信度校准与基于校准置信度的弃权决策的影响，并与从头训练基线进行比较。

**Result:** 跨数据集和数据规模下，SSL预训练相比从头训练均能提升选择性预测性能。然而，当准确率饱和后，选择性性能在不同检查点间仍可能发生显著变化，且更长的预训练并不一致地提升可靠性。

**Conclusion:** 研究强调了弃权感知评估的重要性，并建议将预训练时长视为一个与可靠性相关的关键设计选择，而不仅仅是计算细节，为安全关键医学影像任务中SSL使用提供了实践指导。
...
#### 🧩 TL;DR
RotateK提出了一种基于在线PCA旋转的结构化Key通道剪枝框架，通过将token相关的通道重要性对齐到共享低维子空间，实现硬件友好的头级掩码剪枝，在相同KV缓存预算下以更多视觉token保留显著提升VLM推理效率与细粒度感知能力。

---

#### 📘 Detailed Summary
**Motivation:** 视觉-语言模型在推理时面临严重的KV缓存压力，因为单张图像常编码为数千个token。现有方法主要利用token稀疏性进行token剪枝，但永久丢弃视觉内容会严重损害细粒度感知任务。这催生了互补的通道稀疏性方向：在固定KV缓存预算下压缩通道维度能以相同内存成本保留更多视觉token。然而，先前Key通道剪枝方法面临结构性权衡：逐token通道剪枝表达力强但非结构化且速度慢，而逐头方式硬件友好但不够鲁棒。

**Method:** RotateK是一个基于旋转的结构化Key通道剪枝框架，核心创新在于应用在线PCA旋转，将token依赖的通道重要性对齐到共享低维子空间，从而能够通过轻量级头级掩码实现精确剪枝。此外，RotateK设计了一个融合式Triton注意力内核，可直接在稀疏通道的Key上高效解码。

**Result:** 在两个代表性VLM主干上的实验表明，RotateK在精度和解码延迟上均一致优于先前的Key通道剪枝方法。在相同KV缓存预算下，联合token-通道剪枝相比仅token剪枝基线取得了更好的性能表现。

**Conclusion:** RotateK通过结构化旋转方法有效解决了Key通道剪枝中表达力与硬件效率之间的权衡问题，表明通道稀疏性是token稀疏性的重要互补方向，联合优化两者可在不牺牲细粒度感知能力的前提下显著提升VLM推理效率。
...
### [14] [Efficient Long-Context Modeling in Diffusion Language Models via Block Approximate Sparse Attention](https://arxiv.org/abs/2605.19726)
*Wenhu Zhang, Yiming Wu, Huanyu Wang, Yaoyang Liu, Huanzhang Dou, Senqiao Yang, Sitong Wu, Hanbin Zhao, Jiaya Jia*

#### 🧩 TL;DR
提出BA-Att框架，通过块级预下采样操作在紧凑空间识别注意力信息区域，并引入轻量级范数排序模块和协方差补偿校正，在保持近全注意力性能的同时实现高达6.95倍注意力计算加速。

---

#### 📘 Detailed Summary
**Motivation:** 扩散语言模型在全局连贯性和可控生成上优于自回归模型，但扩展至超长序列时计算成本高昂；现有块稀疏注意力方法依赖固定采样模式（如尾部区域或反对角条带）选择块，这种先验驱动策略容易遗漏重要标记且在分布偏移下不稳定。

**Method:** 提出BA-Att（Block Approximate Sparse Attention）框架，核心创新包括：块级预下采样操作在紧凑的下采样空间中识别信息区域以避免位置先验依赖；定义一个后下采样oracle注意力图并形式化预下采样与后下采样方案间的近似误差；引入轻量级范数排序模块和协方差补偿校正，通过使用对角QK方差近似全协方差来降低计算复杂度。

**Result:** 在注意力计算上相比FlashAttention实现最高6.95倍加速；在50%稀疏度下，在语言模型、多模态语言模型和视频生成模型上均保持接近全注意力的性能，展现出强效率和泛化能力。

**Conclusion:** BA-Att通过预下采样策略有效克服了先验驱动稀疏注意力的缺陷，在显著加速的同时保持模型性能，对扩散语言模型及其他长序列生成任务具有重要实用价值，展示了稀疏注意力在多种生成模型中的广泛适用性。
...
### [15] [Targeted Downstream-Agnostic Attack](https://arxiv.org/abs/2605.19446)
*Zhuxin Lei, Ziyuan Yang, Yi Zhang*

#### 🧩 TL;DR
本文提出一种针对预训练编码器的有目标下游无关攻击方法（TDAA），通过引入预选"威胁图像"作为特征级锚点，为每个样本生成例特定的对抗扰动，迫使编码器对对抗样本与威胁图像输出相同特征，从而在更严格的威胁模型下实现对下游任务无关的定向攻击。

---

#### 📘 Detailed Summary
**Motivation:** 现有下游无关攻击（DAA）方法采用宽松威胁模型，仅要求对抗样本改变原始预测而不需指定目标，且通常为所有样本生成单一共享扰动，因图像多样性导致攻击成功率低；同时，由于下游任务未知且编码器不直接输出预测，实现有目标攻击极具挑战。

**Method:** 提出有目标下游无关攻击（TDAA）方法，引入攻击者预选的"威胁图像"作为目标；设计一个生成器为每个输入图像生成例特定的对抗扰动，使受害编码器对对抗样本与威胁图像输出相同的特征表示，从而在特征层面建立任务无关的定向攻击桥接。

**Result:** 在3个基准数据集上对10种自监督学习方法进行广泛实验，结果表明所提方法在攻击成功率和隐蔽性方面均有效，并揭示了预训练编码器存在显著的脆弱性。

**Conclusion:** 通过引入威胁图像作为特征级锚点并结合例特定扰动生成范式，TDAA方法成功实现了更严格威胁模型下对预训练编码器的有目标下游无关攻击，揭示了编码器的安全风险。
...
### [16] [Stitched Value Model for Diffusion Alignment](https://arxiv.org/abs/2605.19804)
...
#### 🧩 TL;DR
StitchVM提出了一种模型拼接框架，将预训练在干净图像上的奖励模型与冻结的扩散骨干网络结合，高效地将奖励信号推广到噪声潜变量空间，从而无需昂贵的逐样本近似即可实现扩散模型的奖励对齐。

---

#### 📘 Detailed Summary
**Motivation:** 扩散或流匹配生成模型需要与任务特定奖励（如提示保真度、美学偏好）对齐，但奖励仅定义在干净输出图像上，而对齐过程需要在含噪声的中间潜变量处进行价值函数估计。现有方法要么采用Tweedie近似（高效但有偏），要么采用蒙特卡洛近似（准确但需要昂贵的 rollout），而训练一个专门针对噪声潜变量的强通用价值模型仍是一个开放问题。

**Method:** StitchVM采用模型拼接（model stitching）框架：从一个已截断的像素空间奖励模型出发，在其头部附加一个冻结的扩散骨干网络（如SD 3.5 Medium），形成混合模型。该混合模型从像素空间模型继承了预训练的稳健奖励能力，从扩散骨干继承了处理噪声潜变量的原生能力。拼接和微调过程极其轻量（例如CLIP ViT-L与SD 3.5 Medium拼接仅需10 GPU小时）。

**Result:** StitchVM将强大的像素空间奖励模型提升到潜变量空间后，在多种下游引导和后训练方法上取得了显著改进：DPS速度提升3.2倍且峰值GPU内存减半，DiffusionNFT速度提升2.3倍。该方法使正确的价值函数一次性构建并可在多个样本和迭代间摊销，避免了粗糙且昂贵的逐样本近似。

**Conclusion:** StitchVM开创了扩散模型对齐的新范式：通过拼接框架将像素空间奖励模型高效迁移到噪声潜变量空间，替代了传统的逐样本价值函数近似策略。该工作表明，轻量级的模型拼接即可实现奖励函数的有效推广，在提升效率的同时保持准确性，具有广泛的应用前景。
...
### [17] [RECIPE: Procedural Planning via Grounding in Instructional Video](https://arxiv.org/abs/2605.19976)
*Luigi Seminara, Antonino Furnari, Lorenzo Torresani*

#### 🧩 TL;DR
RECIPE利用视频-文本对齐质量作为GRPO奖励信号，将大规模含噪教学视频语料转化为验证器而非标签源，在多尺度模型和7个过程性基准上显著超越监督微调方法，实现零样本泛化并保持生成多样性。

---

#### 📘 Detailed Summary
**Motivation:** 视觉规划任务受限于标注瓶颈：现有干净数据集规模小、领域窄且编码单一执行轨迹，而大规模教学视频语料虽数量庞大但其含噪ASR字幕生成的伪标签存在分割和对齐错误，且同样局限于单轨迹监督。核心问题在于从噪声视频中提取干净步骤标签非常困难，而验证生成步骤序列是否在ASR转录中具有时间基础则代价低廉且可规模化。

**Method:** 提出RECIPE框架，利用预计算文本嵌入对百万级视频进行接地质量奖励计算，将其作为GRPO(群体相对策略优化)的奖励信号，将含噪语料转化为验证器而非标签来源。该框架统一适用于Socratic(由冻结VLM提取文本历史)和Video(直接消费视频token)两种规划器输入配置，覆盖有标注和弱监督两种训练范式。

**Result:** 在7个过程性基准上采用基于LLM作为评判员的参考协议评估6项过程性标准，RECIPE-RL在0.5B、3B、7B三个尺度上均优于基础检查点，域内宏观准确率提升7-8个点，零样本场景提升高达16个点；优于有标注和伪标签上的监督微调(后者反而降低基础性能)，且无需人类标注即可保持鲁棒性；作为提案-评估-搜索规划器中的提案阶段，在Visual Planning for Assistance任务上每个规划步长均超越最强零样本基线，在COIN上保持SFT所破坏的生成多样性。

**Conclusion:** 该研究揭示了利用接地质量作为替代奖励信号可以有效避开从噪声数据中提取干净标签的困难，将大规模弱监督视频语料转化为可扩展的验证资源。RECIPE证明了强化学习范式在过程性推理任务中优于监督微调，尤其在多样性和零样本泛化方面具有显著优势，为缺乏高质量标注的视觉规划场景提供了可行路径。
...
### [18] [DECOR: Auditing LLM Deception via Information Manipulation Theory](https://arxiv.org/abs/2605.19270)
*Linyue Cai, Samuel Yeh, Jwala Dhamala, Rahul Gupta, Sharon Li*

#### 🧩 TL;DR
DECOR是一个基于信息操纵理论的多智能体框架，将输入上下文分解为原子信息单元并在四个维度上评分，生成可解释的操纵剖面图并聚合为全局欺骗指数，在单轮和多轮欺骗检测基准上均达到最先进性能，并在15个前沿模型上展现良好泛化能力。

---

#### 📘 Detailed Summary
**Motivation:** 大型语言模型可能通过微妙地操纵真实信息（如省略关键事实、转移焦点或模糊含义）来实施欺骗，现有黑盒检测方法依赖粗粒度判断，可解释性有限，无法精确定位哪些事实被扭曲以及如何被扭曲。

**Method:** DECOR是一个多智能体框架，基于信息操纵理论（Information Manipulation Theory），将输入上下文分解为原子信息单元，并在四个操纵维度上对每个单元与模型响应的匹配程度进行评分，生成可解释的操纵剖面图，最终聚合为一个全局欺骗指数。

**Result:** 在覆盖现实世界领域的单轮和多轮欺骗检测基准上，DECOR均达到最先进性能，超越多个竞争基线；该方法在15个前沿模型上展现出良好的泛化能力，消融实验验证了每个关键设计组件的贡献。

**Conclusion:** 该研究表明，基于理论指导的细粒度信息操纵审计为LLM欺骗检测提供了一条有效且可解释的路径，能够精确定位事实扭曲的具体方式与维度。
...
### [19] [OpenCompass: A Universal Evaluation Platform for Large Language Models](https://arxiv.org/abs/2605.19276)
...
#### 🧩 TL;DR
本文提出了OpenCompass，一个面向通用大语言模型的一站式、可扩展、高并发评估平台，通过模块化设计与组件解耦，支持多种评估器与跨领域基准数据集，为学界和工业界提供统一高效的模型评估工具。

---

#### 📘 Detailed Summary
**Motivation:** 当前大语言模型评估主要依赖静态基准数据集，但面临任务类型多样、评价标准不一致、数据与处理流程碎片化等问题，导致难以高效进行跨领域和大规模模型评估，亟需一个统一、可扩展的评估平台来填补这一空白。

**Method:** OpenCompass采用模块化和组件解耦的设计理念，核心架构包括配置系统、任务划分模块、执行与调度模块、任务执行单元和结果可视化模块五个关键组件；支持基于规则的评估、大语言模型作为裁判（LLM-as-a-Judge）以及级联评估器三种评估方式，以适应不同任务场景需求。

**Result:** 该平台支持涵盖知识、推理、计算、科学、语言、代码等多个领域的主流基准数据集，具备高兼容性、灵活性和高并发三大核心优势，能够为学术界和工业界提供统一的LLM评估工具，实现对大语言模型优缺点的准确识别。

**Conclusion:** OpenCompass通过提供统一、高效的评估平台，有效解决了当前LLM评估中存在的碎片化和标准不一致问题，能够准确识别大语言模型的优势与不足，为其后续优化提供关键支撑，推动了通用大语言模型评估技术的标准化和规模化发展。
...
### [20] [A Multi-Agent Framework for Feature-Constrained Difficulty Control in Reading Comprehension Item Generation](https://arxiv.org/abs/2605.19316)
*Seonjeong Hwang, Jun Seo, Hyounghun Kim, Gary Geunbae Lee*

#### 🧩 TL;DR
提出MAFIG框架，通过多智能体协作与特征特定评估器迭代生成约束一致的阅读理解题目，并结合难度校准约束序列实现了对题目难度的可靠控制。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于大语言模型的难度可控阅读理解题目生成方法通常采用单智能体提示方式，难以一致地满足指定的特征约束，导致生成题目偏离目标难度；因此需要一种能够可靠满足多重特征约束的题目生成框架。

**Method:** 提出MAFIG（Multi-agent Framework for Feature-constrained Item Generation），采用多个LLM智能体与特征特定评估器协作，迭代生成并修订满足目标约束的题目；同时提出一种构造难度单调递增的特征约束序列的方法，以验证框架在难度控制上的有效性。

**Result:** 实验结果表明，MAFIG生成的题目在满足目标约束的比例上显著高于基线方法，并通过难度校准的约束序列实现了稳健的难度控制。

**Conclusion:** 多智能体协作与特征约束迭代修订能够有效提升约束满足率，结合难度校准约束序列可实现可靠的难度控制，为可控题目生成提供了新范式。
...
#### 🧩 TL;DR
本文系统评估了八种大语言模型在不同量化级别（2至8比特）下的性能表现，揭示了量化精度与模型性能之间的权衡关系，表明8比特量化性能最佳但收益递减，2比特量化虽保留一定准确性但部分模型性能显著下降，同时发现7-9B参数的中等规模模型在效率与资源消耗之间达到最优平衡。

---

#### 📘 Detailed Summary
**Motivation:** 大语言模型虽在众多NLP任务中表现优异，但庞大的模型规模给实际部署带来了严峻挑战，亟需高效的模型压缩技术；量化作为其中一种主流方案，其从2比特到6比特不同精度水平对LLM性能和准确性的具体影响尚缺乏系统性的研究。

**Method:** 本文选取了八种不同的大语言模型，在多种量化级别（2至8比特，包括Q2_K和Q8_0等格式）下进行系统评估；评测任务涵盖MMLU-Pro（知识处理与推理）、CRUXEval（代码理解）和MuSR（阅读理解）等多个维度，通过对比不同模型规模和量化精度下的性能变化来揭示量化对LLM的影响规律。

**Result:** 实验结果表明，更高精度（如8比特Q8_0）通常带来更好的性能，但存在收益递减现象；激进的量化（如2比特Q2_K）在大多数情况下仍保留可接受的准确率，但部分模型出现了显著的性能损失。更大规模的模型对激进量化表现出更强的韧性，但在极低精度下性能仍会大幅下降；7-9B参数范围的中等规模模型在效率与资源消耗之间取得了最优平衡。

**Conclusion:** 量化精度对LLM性能的影响存在明显的递减效应，且不同模型和任务对量化的敏感度存在显著差异；模型规模是影响量化鲁棒性的关键因素，中等规模（7-9B参数）模型在效率与性能间实现了最佳折中，为实际部署中的量化策略选择提供了重要参考依据。
...
#### 🧩 TL;DR
本文系统研究了跨模态技能注入（Cross-modal Skill Injection）这一新兴范式，即在无需训练数据和显著计算开销的条件下，将领域专家LLM的能力注入VLM以激发跨模态涌现能力，并对其适用场景、融合方法和超参数进行了系统的定量分析。

---

#### 📘 Detailed Summary
**Motivation:** 尽管VLM在通用多模态理解上表现出色，但在高效获取持续演化的领域特定技能方面存在困难。传统的SFT方法需要大量数据集和计算资源，而模型合并虽然提供了更高效的替代方案，但现有研究缺乏对跨模态技能注入（将领域专家LLM集成到VLM中）的适用性和方法的系统分析。

**Method:** 本文从场景、方法和超参数三个维度系统研究跨模态技能注入。在场景上考察指令遵循、跨语言和数学推理等任务；在方法上对比包括TA和DARE在内的多种经典模型合并方法；在超参数上对关键参数进行系统的定量分析。

**Result:** 跨模态技能注入在指令遵循和跨语言场景中表现良好，但在数学推理任务上存在困难；经典方法如TA和DARE在性能上始终优于其他替代合并方法；经典方法的超参数调优对其性能至关重要，本文提供了系统的定量分析结果。

**Conclusion:** 跨模态技能注入是一种高效、无需额外训练数据的VLM能力增强范式，其有效性依赖具体任务场景和方法选择，指令遵循和跨语言场景最为适用，且TA和DARE方法结合合理的超参数调优是实现最优性能的关键。
...
### [23] [ContextRAG: Extraction-Free Hierarchical Graph Construction for Retrieval-Augmented Generation](https://arxiv.org/abs/2605.19735)
*Roman Prosvirnin, Sergei Kuznetsov, Seungmin Jin*

#### 🧩 TL;DR
ContextRAG 提出一种无需大语言模型进行实体和关系抽取的图式检索增强生成系统，通过残差量化 k-means 与形式概念分析构建模糊概念图，在 130 个任务上仅用 30 次 LLM 调用和 22,073 个 token 完成索引构建，获得 33.6% 的总体 F1 分数。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于图结构的检索增强生成系统在多跳问答中虽能提升答案质量，但其索引构建阶段严重依赖大语言模型来抽取实体、关系和摘要，导致 token 开销和延迟随语料库规模线性增长，成为系统可扩展性的瓶颈。

**Method:** ContextRAG 采用残差量化 k-means 对文本块嵌入进行聚类，结合基于 Lukasiewicz 剩余逻辑的形式概念分析推导出模糊概念图；通过软模糊连接与交运算生成类似桥节点和交节点的上下文节点，替代传统方法中由 LLM 编写的图边，从而避免在索引阶段使用 LLM 进行实体或关系抽取。

**Result:** 在包含 130 个任务的 UltraDomain 子集上，ContextRAG 索引构建仅需 30 次 LLM 调用和 22,073 个 token；相比之下，HiRAG 在 20 个任务的子集上就需要 870 次索引调用和 354 万个 token，线性外推至 130 个任务则超过 2300 万 token。ContextRAG 总体 F1 达 33.6%，多跳任务 F1 为 36.8%；激活分析表明，检索到至少一个格导出的节点位于前五的查询，其 F1 比未检索到的查询高出 3.9 个百分点。

**Conclusion:** ContextRAG 证明在不依赖 LLM 进行图边构建的前提下，可以构建高效的图式 RAG 系统，大幅降低索引阶段的 token 和调用成本；格导出节点对检索质量的贡献具有诊断性而非因果性，为未来混合检索与图结构优化的方向提供了启示。
...
#### 🧩 TL;DR
本文提出Learn-by-Wire Guard (LBW-Guard)，一种位于AdamW优化器之上的有界自主训练控制治理层，通过观测训练遥测并施加有界控制来稳定大语言模型训练，在Qwen2.5-7B上将困惑度从13.21降至10.74（提升18.7%），并在高学习率压力下维持可训练性而AdamW完全退化。

---

#### 📘 Detailed Summary
**Motivation:** 现代语言模型训练在激进学习率、大规模扩展和运行时压力条件下日益暴露于训练不稳定、运行退化及算力浪费的问题，现有方法如梯度裁剪无法有效应对这些压力场景，亟需一种不替代优化器本身但能主动治理训练稳定性的方法。

**Method:** LBW-Guard是一个运行在AdamW优化器之上的有界自主训练控制治理层，它不替换优化器更新规则，而是通过观测训练遥测数据、识别不稳定敏感区域，并在保持固定训练目标的前提下对优化器执行施加有界控制来稳定训练过程。

**Result:** 在Qwen2.5-7B参考设置下，LBW-Guard将最终困惑度从13.21降至10.74（18.7%提升），端到端训练时间从392.54秒降至357.02秒（1.10倍加速）。在高学习率压力下（LR=3e-3和LR=1e-3），AdamW的困惑度分别退化至1885.24和659.76，而LBW-Guard仍保持可训练，困惑度仅为11.57和10.33，梯度裁剪基线无法复现该效果。

**Conclusion:** 实验结果表明，对稳定性敏感的LLM训练可以从优化器之上的治理平面中获益，LBW-Guard证明了有界运行时控制能够在压力下保持有效计算，且该方法区别于优化器替换和局部梯度抑制，为大规模语言模型训练的鲁棒性提供了一种新的系统级方案。
...
### [25] [AgentNLQ: A General-Purpose Agent for Natural Language to SQL](https://arxiv.org/abs/2605.19010)
...
#### 🧩 TL;DR
提出了一种多智能体NL2SQL方法，通过语义增强的schema表示、业务规则注入和基于LLM的编排-反思-自校正机制，在BIRD基准上达到78.1%的语义准确率，显著提升了复杂数据库场景下的自然语言到SQL的转换性能。

---

#### 📘 Detailed Summary
**Motivation:** 尽管大语言模型在自然语言理解方面取得了显著进展，NL2SQL转换在准确率上仍未达到人类专家SQL编写者的水平，现有方法在面对大规模、复杂关系数据库时表现不足，亟需更鲁棒的算法改进来弥合这一性能差距。

**Method:** 本文提出了一种多智能体NL2SQL方法，核心包括：(a) 设计了一个基于LLM的优化编排器，负责规划、编排、反思和自我校正以生成准确的SQL查询；(b) 开发了一种先进的schema语义增强方法，通过创建上下文感知的元数据来丰富用户提供的schema表示；(c) 引入了用户提供的业务规则以进一步提升查询准确性。

**Result:** 该方法在BIRD（BIg Bench for LaRge-scale Database）基准测试上达到了78.1%的语义准确率，并在不同领域和数据集上验证了其准确性和泛化能力。

**Conclusion:** 研究表明，融合多智能体协作、语义增强的schema表示和业务规则注入能有效提升NL2SQL的性能，该方法具有良好的跨领域泛化能力，为构建接近人类水平的NL2SQL系统提供了一条可行路径。
...
### [26] [SimGym: A Framework for A/B Test Simulation in E-Commerce with Traffic-Grounded VLM Agents](https://arxiv.org/abs/2605.19219)
...
#### 🧩 TL;DR
SimGym是一个基于视觉语言模型智能体的A/B测试模拟框架，通过在实时浏览器中模拟购物会话，将电商界面实验周期从数周缩短至一小时以内，并在加购率方向性对齐上达到77%的准确率。

---

#### 📘 Detailed Summary
**Motivation:** A/B测试作为电商店面修改评估的黄金标准，存在分流真实流量、需数周才能达到统计显著性以及可能损害用户体验等问题，亟需一种能够在不影响真实买家的前提下快速评估界面变体效果的替代方案。

**Method:** SimGym框架包含三个核心组件：一是基于生产点击流数据的流量驱动人格生成管线，用于推导每店铺的买家原型与购物意图；二是实时浏览器智能体架构，结合多模态感知（视觉与浏览器结构化观测）、情景记忆与防护机制，在控制和实验店面间执行连贯购物会话；三是评估协议，将模拟结果偏移与真实买家行为偏移进行对比验证。

**Result:** 在来自大型电商平台的多类店面与产品类别上，SimGym对视觉驱动的UI主题变更A/B测试进行验证，智能体行为与真实买家结果偏移的加购率方向性对齐达到77%，并将实验周期从数周缩短至一小时以内。

**Conclusion:** SimGyn证明了利用VLM智能体在浏览器中模拟A/B测试的可行性与高效性，能够在不暴露真实买家于候选变体的前提下实现快速实验，为电商界面评估提供了一种高效替代方案。
...
#### 🧩 TL;DR
AquaUI提出了一种无需额外训练、基于自适应四叉树的推理时令牌缩减方法，通过在保持空间位置信息的同时利用GUI截图的空间冗余性，在GUI智能体模型中实现了显著的效率提升与性能保持。

---

#### 📘 Detailed Summary
**Motivation:** 现有LMM-based GUI智能体模型在每个推理步骤中引入高分辨率截图，但GUI截图存在高度非均匀的空间信息密度——大片区域信息稀疏且视觉同质，而关键文本和图标则需要高保真度；现有方法要么需要额外训练，要么依赖基于注意力的令牌压缩，忽略了GUI截图的结构化布局和空间冗余性。

**Method:** AquaUI是一种无需训练的推理时令牌缩减方法，核心包含自适应四叉树算法和条件四叉树算法：前者将每张输入截图构建为自适应四叉树，每个叶子节点保留一个代表性合并令牌并维持其空间位置以确保位置编码一致性；后者利用连续截图间的连续性，以先前的四叉树为参考优化当前四叉树，在静态或轻微变化的GUI状态下保留细粒度区域。

**Result:** 在多个标准grounding和导航基准上，AquaUI在先进GUI智能体模型上持续展现出优于基线方法的准确率-效率权衡效果；在GUI-Owl-1.5-32B-Instruct模型上，AquaUI最多实现13.22%的加速和29.52%的视觉令牌缩减，同时保留了全令牌性能的99.06%。

**Conclusion:** 该研究证实GUI截图的空间冗余性可以在无需重训练的条件下在推理阶段被有效利用，提示了针对结构化GUI场景设计令牌缩减方法的巨大潜力，为高效LMM-based GUI智能体的部署提供了可行方向。
...
### [28] [Swimming with Whales: Analysis of Power Imbalances in Stake-Weighted Governance](https://arxiv.org/abs/2605.19264)
*Yuzhe Zhang, Manvir Schneider, Qin Wang, Davide Grossi*

#### 🧩 TL;DR
本文从计算社会选择视角出发，系统研究了权益证明区块链中基于持币权重的投票机制所导致的权力扭曲问题，通过理论分析和Project Catalyst的真实数据验证，揭示了权力与相对持币比例之间的非对齐现象及其可近似的条件。

---

#### 📘 Detailed Summary
**Motivation:** 在权益证明（PoS）区块链中，基于持币权重的投票是核心治理范式，但该范式容易导致权力扭曲——少数大额持币者即使未持有全部权益，也可能完全掌控决策。现有研究缺乏对这一权力失衡现象的严格量化分析，本文旨在填补这一空白。

**Method:** 本文采用计算社会选择理论的分析框架，以Penrose-Banzhaf权力指数作为权力量化工具，从理论和实证两个层面展开研究。理论上，通过数学分析考察权力与相对持币比例之间的对齐程度及其可近似条件；实证上，利用真实世界链上治理系统Project Catalyst的数据进行细粒度分析。

**Result:** 理论分析表明，权力与相对持币比例之间的完美对齐在一般情况下无法实现，但在特定条件下可以在期望意义上得到近似。基于Project Catalyst真实数据的实证分析进一步揭示了当前持币权重治理系统中权力失衡的具体程度和特征。

**Conclusion:** 本研究为理解和量化PoS区块链治理中权力扭曲问题提供了严格的理论基础和实证证据，指出了权力与持币比例对齐的局限性及可近似条件，为设计更公平的链上治理机制提供了重要参考。
...
### [29] [Conflict-Resilient Multi-Agent Reasoning via Signed Graph Modeling](https://arxiv.org/abs/2605.19418)
*Longgang He, Longzhu He, Daojing He, Chaozhuo Li*

#### 🧩 TL;DR
SIGMA 提出了一种基于符号图的多智能体推理框架，通过显式建模智能体间的信任、冲突与中立关系，并采用冲突感知的消息传递与加权聚合机制，在多个基准数据集上显著提升了多智能体系统的推理准确性与冲突鲁棒性。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于图的多智能体系统（MAS）在推理过程中存在两个关键缺陷：一是当出现冲突信号时无法加以控制，导致错误传播；二是缺乏对智能体间冲突关系的显式建模与结构感知能力，难以识别可靠的交互模式。这些问题源于现有方法采用假设均匀合作的朴素聚合机制，限制了MAS在复杂决策场景中的性能。

**Method:** SIGMA 框架首先根据查询选择一组相关且多样化的智能体，然后构建一个带有置信度权重的结构化符号交互图，显式编码智能体间的信任、冲突与中立关系。推理过程通过冲突感知的符号消息传递机制进行，该机制增强来自可信智能体的信息并压制冲突信号，最后通过结构感知与冲突感知的加权聚合生成全局一致且抗冲突的预测结果。

**Result:** 在六个基准数据集上的大量实验表明，SIGMA 在多种LLM骨干网络和多样化的多智能体配置下，持续优于当前最先进的基线方法，在准确率和冲突鲁棒性方面均取得了显著的性能提升。

**Conclusion:** 该研究揭示了在多智能体系统中显式建模智能体间符号关系（信任与冲突）对于提升推理质量的关键作用，为构建更可靠、更鲁棒的多智能体协作框架提供了新的思路，未来可进一步探索动态图更新与更细粒度的关系建模方向。
...
### [30] [EngiAI: A Multi-Agent Framework and Benchmark Suite for LLM-Driven Engineering Design](https://arxiv.org/abs/2605.19743)
*Gioele Molinari, Florian Felten, Soheyl Massoudi, Mark Fuge*

#### 🧩 TL;DR
提出了一个针对多智能体工程AI系统的三维度基准套件（EngiBench）及参考实现EngiAI，用于评估LLM智能体在工程设计中工作流执行、检索增强生成和高性能计算编排方面的能力，实验揭示了闭源与开源模型间的性能差距以及条件分支等任务的挑战性。

---

#### 📘 Detailed Summary
**Motivation:** 现有评估框架未能充分覆盖结合了仿真、检索和制造准备的多智能体工程AI系统，缺乏针对工程设计中多步骤工作流、参数检索贡献隔离以及高性能计算编排的标准化评测手段。

**Method:** 提出了EngiBench基准套件，包含三个维度：七种提示风格的工作流基准、带门控评分的RAG基准（隔离检索对参数选择的贡献）、以及基于SLURM集群的端到端ML训练编排的HPC基准；同时提出了EngiAI，一个基于LangGraph的多智能体系统参考实现，通过监督架构协调七个专用智能体，统一拓扑优化、文档检索、HPC作业编排和3D打印机控制。

**Result:** 在Beams2D问题上，闭源模型平均任务完成率达96-97%，开源4B参数模型达55-78%且存在代际改进；条件分支在Photonics2D上表现最差（20-53%）；RAG门控评分验证了检索增强近乎完美（≈1.0）而无法检索时近乎为零；HPC编排中某模型100%完成所有流水线步骤，另一模型仅50%，揭示了多步骤指令遵循在长工作流中的退化问题。

**Conclusion:** 闭源模型在工程多智能体任务中显著优于轻量级开源模型，条件分支和长序列工作流编排仍是关键瓶颈，RAG门控设计有效量化了检索组件的独立贡献，为未来工程AI系统的标准化评估和模型改进提供了方向。
...
### [31] [Memory-Augmented Reinforcement Learning Agent for CAD Generation](https://arxiv.org/abs/2605.19748)
*Yin Xiaolong, Liu Yu, Shen Jiahang, Lu Xingyu, Ni Jingzhe, Fan Fengxiao, Sang Fan*

#### 🧩 TL;DR
提出一种记忆增强的强化学习框架用于CAD生成代理，通过将几何内核封装为可调用工具链、设计双轨记忆模块（案例库和技能库）以及引入动态效用检索算法，解决了现有LLM方法在处理长操作序列、多样操作类型和强几何约束的复杂CAD模型时推理链断裂和缺乏纠错机制的问题，显著提升了复杂CAD模型生成的成功率和几何一致性。

---

#### 📘 Detailed Summary
**Motivation:** 现有基于大语言模型的CAD模型自动生成方法在处理具有长操作序列、多样化操作类型和强几何约束的复杂CAD模型时表现不足，主要原因是推理链容易断裂且缺乏有效的纠错机制，亟需一种能够实现自我纠错和持续进化的生成框架。

**Method:** 提出了一种记忆增强的强化学习框架，将底层几何内核封装为代理可调用的结构化工具链，构建了设计意图理解、全局规划、执行和多维度验证的闭环机制；设计了包含案例库和技能库的双轨记忆模块，并提出了动态效用检索算法；通过将强化学习引入检索和策略优化，代理能够避免语义相似但几何不可行的检索陷阱，实现在线自我纠错和持续进化。

**Result:** 实验结果表明，所提方法在复杂CAD模型生成任务上显著提升了生成成功率和几何一致性，有效克服了长序列操作中的推理断裂和几何约束违反问题。

**Conclusion:** 该研究证明了将强化学习与记忆增强机制相结合能够有效解决复杂CAD生成中的推理链断裂和纠错难题，为无需大规模标注数据即可实现代理持续进化提供了可行路径，对智能制造中的自动化CAD建模具有重要应用价值。
...
### [32] [Distribution-Free Uncertainty Quantification for Continuous AI Agent Evaluation](https://arxiv.org/abs/2605.19779)
*Yuxuan Gao, Megan Wang, Yi Ling Yu*

#### 🧩 TL;DR
本文将分裂保形预测和自适应保形推断(ACI)扩展到连续AI智能体评估中，为质量评分预测提供无分布覆盖保证，并提出了多智能体管线的组合不确定性界限、成对排序的保形弃权规则以及排行榜规模的FDR校正弃权机制，实验表明校准误差低于0.02，跨源情感分歧可预测排序不稳定性。

---

#### 📘 Detailed Summary
**Motivation:** 连续AI智能体评估中缺乏严格的统计保证，现有方法难以在动态部署环境中为质量评分预测提供可靠的覆盖概率保证，且多智能体管线的不确定性量化、成对排序的假阳性控制以及大规模排行榜的多重检验校正等问题尚未得到系统解决。

**Method:** 采用分裂保形预测和自适应保形推断(ACI)为AI智能体质量评分预测提供无分布覆盖保证；开发了用于多智能体管线的组合不确定性界限（跨阶段相关性rho在[-0.5, 0.9]范围内通过仿真验证）；提出用于成对排序的保形弃权规则以控制假排序率；以及用于排行榜规模多重检验的FDR校正弃权机制。

**Result:** 在24小时预测范围内，保形区间的校准误差在所有名义水平上均低于0.02；ACI在智能体发布后正确将区间宽度扩大35%后再收敛；单智能体条件覆盖均值达80.4%，90%的智能体覆盖在[72%, 90%]内；跨源情感分歧与排序不稳定性相关系数r=0.64(p<0.01)；经循环控制验证，该框架捕获超越基准的信号(rho_s=0.52, p<0.01, n=35)。

**Conclusion:** 保形推断为连续AI智能体评估提供了有效的无分布覆盖保证和不确定性量化工具，覆盖质量良好且能自适应环境变化；跨源情感分歧可作为排序不稳定性的预测指标，代码与数据以CC BY 4.0协议开源，为动态评估场景提供了可靠统计框架。
...
### [33] [Prior Knowledge or Search? A Study of LLM Agents in Hardware-Aware Code Optimization](https://arxiv.org/abs/2605.19782)
*Dmitry Redko, Albert Fazlyev, Konstantin Sozykin, Maria Ivanova, Evgeny Burnaev, Egor Shvetsov*

#### 🧩 TL;DR
本文通过三个受控实验系统性地揭示了LLM在代码优化任务中高度依赖预训练先验知识，而非提供的反馈信息或智能体结构。实验发现LLM在纯黑盒优化中表现为贪婪优化器，在零样本内核生成中输入尺寸信息不可见，且反馈回路优化中CUDA表现单调提升而TVM IR主动退化，表明低密度语言会导致优化性能下降。

---

#### 📘 Detailed Summary
**Motivation:** 现代LLM智能体在结构上日益复杂，基于"提议-评估-修正"循环的优化与发现系统被广泛应用于各领域，但难以评估哪些组件贡献最大，以及这种探索在何时、如何可能失败。已有研究缺乏对LLM在优化任务中各组成部分贡献的系统性分解与诊断。

**Method:** 作者设计了三组受控实验：(1)纯黑盒优化实验，考察LLM在无梯度信息环境下的优化行为；(2)零样本内核生成实验，测试显式输入尺寸信息对模型生成内核参数的影响，并探究不同语言和温度设置下的表现；(3)反馈回路内核优化实验，对比CUDA与TVM IR两种语言在迭代反馈条件下的优化轨迹，评估反馈信号对不同密度语言的作用效果。

**Result:** (1)LLM在纯黑盒优化中表现为贪婪优化器；(2)零样本内核生成中提供显式输入尺寸信息无测量效应，模型收敛到相同内核参数，尺寸指令如同不可见；针对非寻常内核尺寸时性能急剧下降，且与使用语言无关；(3)反馈回路优化中CUDA在迭代反馈下单调提升，而TVM IR主动退化，表明低密度语言会导致优化性能下降。

**Conclusion:** LLM在代码优化任务中高度依赖预训练先验知识，而非所提供的反馈或智能体结构。这一发现对当前基于LLM的自动化优化系统设计提出了重要警示，即反馈机制的有效性受语言密度和模型先验的显著影响，未来需关注如何增强模型对反馈信号的真实响应能力及跨语言泛化能力。
...
### [34] [GeoX: Mastering Geospatial Reasoning Through Self-Play and Verifiable Rewards](https://arxiv.org/abs/2605.20006)
*Kyeongjin Ahn, Seungeon Lee, Krishna P. Gummadi, Meeyoung Cha*

#### 🧩 TL;DR
GeoX is a自我博弈框架，通过可执行程序进行地理空间推理，无需大量人工标注数据即可在推理任务上提升基础VLM性能，并发布了自我博弈积累的地理空间理解基准。

---

#### 📘 Detailed Summary
**Motivation:** 地理空间推理需要对场景的复杂空间结构进行基于图像的推理，但开发这一能力受到标注大规模组合性问题空间的高昂成本制约，现有方法依赖海量人工标注数据，效率低下且可扩展性差。

**Method:** GeoX提出自我博弈框架，采用单一多模态策略模型，将空间问题表达为可执行程序，支持溯因、演绎和归纳三种推理模式，结合空间原语和图像理解工具；通过验证器执行程序生成可验证奖励信号，利用强化学习联合优化问题生成与求解两个角色。

**Result:** GeoX在基础VLM上持续提升性能，平均提高最多5.5分，其效果可媲美甚至超越基于数百万精心标注数据训练的传统基线方法，并同时通过自我博弈过程发布了地理空间理解基准数据集。

**Conclusion:** 该研究表明通过可执行程序驱动的自我博弈范式，可以在不依赖大规模人工标注的情况下有效提升地理空间推理能力，为视觉-语言模型在空间智能方面的发展提供了低成本、可扩展的新路径。
...
### [35] [When Skills Don't Help: A Negative Result on Procedural Knowledge for Tool-Grounded Agents in Offensive Cybersecurity](https://arxiv.org/abs/2605.20023)
*Samuel Jacob Chacko, James Hugglestone, Chashi Mahiul Islam, Xiuwen Liu*

#### 🧩 TL;DR
通过重分析一篇180次运行的对照研究，本文发现当环境反馈带宽足够高时，基于MCP的黑客智能体在CTF任务中技能包（Agent Skills）的边际收益急剧下降甚至为负，并据此提出环境反馈带宽是决定技能包是否有用的关键缺失变量。

---

#### 📘 Detailed Summary
**Motivation:** 已有研究广泛报道Agent Skills（在推理时加载到LLM智能体中的结构化程序知识包）能平均提升16.2个百分点的任务通过率，但同一基准测试中不同任务间方差极大，16/84个任务出现负收益。社区尚未阐明Skill何时真正有帮助、何时只是冗余开销的机制。

**Method:** 本文重新分析了关于MCP接地自主CTF智能体的180次运行对照研究，该研究在四种文档化条件下（55、1,478、1,976和4,147行），恰好对应无技能、经验技能、策展技能和综合技能的消融实验。通过统计检验（χ²检验、Cochran-Armitage趋势检验、Cohen's h效应量）评估不同技能条件下的性能差异，并引入环境反馈带宽作为关键解释变量。

**Result:** 在进攻性网络安全领域（现有技能基准未深入覆盖），无技能与全技能条件之间的性能差距仅为8.9个百分点（p=0.71, χ²；p=0.25, Cochran-Armitage趋势检验）；六对比较中有五对的Cohen's h低于0.2的小效应阈值。技能包在某些情况下（如时序侧信道设置）甚至主动降低性能。

**Conclusion:** 本文提出环境反馈带宽是决定Skill边际收益的缺失变量：当智能体的工具层返回严格、模式验证、低延迟的观察时，环境本身提供了技能包通常所需的程序性校正信号，从而导致策展技能的边际收益大幅下降。该假说可证伪，并对复合AI系统的设计有重要启示。
...
### [36] [AutoResearchClaw: Self-Reinforcing Autonomous Research with Human-AI Collaboration](https://arxiv.org/abs/2605.20025)
...
#### 🧩 TL;DR
AutoResearchClaw 提出了一种基于多智能体辩论、自愈执行、可验证报告、人机协作和跨轮进化的五机制自主科研流水线，在 ARC-Bench 上超越 AI Scientist v2 达 54.7%，并发现高杠杆决策点上的精准人机协作优于全自动或全人工监督。

---

#### 📘 Detailed Summary
**Motivation:** 现有自主科研系统将研究建模为线性流水线，依赖单智能体推理、遇到执行失败即停止、且无法跨轮次积累经验，无法模拟真实科研中假设被多角度质疑、实验失败驱动迭代、知识跨循环累积的迭代特性。

**Method:** AutoResearchClaw 提出五个核心机制：结构化多智能体辩论用于假设生成与结果分析；基于 Pivot/Refine 决策循环的自愈执行器将失败转化为信息；可验证结果报告机制防止虚构数字与幻觉引用；七种干预模式（从全自动到逐步监督）的人机协作；以及将过去错误转化为未来保障的跨轮进化机制。

**Result:** 在包含 25 个主题的 ARC-Bench 实验阶段基准上，AutoResearchClaw 较 AI Scientist v2 提升 54.7%。消融实验表明，在高杠杆决策点上进行精准、有针对性的人机协作，其性能稳定优于全自动模式与全面逐步监督模式。

**Conclusion:** AutoResearchClaw 定位为增强而非替代人类科学判断的研究放大器，其核心启示是：自主科研系统不应追求完全自动化，而应在关键决策点实现高效人机协同，同时具备从失败中学习与跨轮次知识积累的能力。
...
#### 🧩 TL;DR
本文提出随机-确定性边界（SDB）作为生产级LLM代理运行时的核心原语，将其定义为提议者、验证者、提交步骤和拒绝信号四部分契约，并围绕此原语组织六个运行时模式、一个模式选择方法论和一个故障诊断程序。

---

#### 📘 Detailed Summary
**Motivation:** 生产级LLM代理将随机模型输出与确定性软件系统相结合，但两者之间的边界从未被当作一等架构对象来对待，导致缺乏系统化的方法来设计和分析此类系统的可靠性与运行时行为。

**Method:** 本文提出随机-确定性边界（SDB）概念，将其形式化为提议者、验证者、提交步骤和拒绝信号四部分契约。围绕SDB，将代理运行时设计组织为协调、状态和控制三类关注点，并编目六种运行时模式：层次化委派、散布-收集加Saga、事件驱动序列、共享状态机、监督者加门控、以及人在环中。此外提出五步模式选择方法论、将生产故障映射到模式弱点的诊断程序，以及一种称为重放发散（replay divergence）的失效模式。

**Result:** 论文将方法论应用于五个工作负载，并提供一个可运行的90天合同续约代理参考实现。通过将每调用模型方差与架构惯性分离的风格化可靠性分解，论证了当模型方差降低时，模式选择与SDB强度将成为长期可靠性的日益重要的杠杆。

**Conclusion:** SDB是生产级代理运行时的负载承载原语，其设计直接决定了系统的可靠性与可演化性；重放发散是LLM代理系统特有的关键失效模式，而系统性的模式选择和SDB契约设计比单纯降低模型方差对长期可靠性更为重要。
