...
### [1] [SIMMER: Cross-Modal Food Image--Recipe Retrieval via MLLM-Based Embedding](https://arxiv.org/abs/2604.15628)
*Keisuke Gomi, Keiji Yanai*

#### 🧩 TL;DR
本文提出SIMMER，一种基于多模态大语言模型的单编码器方法，用于食品图像与食谱文本的跨模态检索，取代传统的双编码器架构，在Recipe1M数据集上实现了最先进的性能。

---

#### 📘 Detailed Summary
**Motivation:** 现有食品图像与食谱文本跨模态检索方法主要依赖双编码器架构，需要复杂的对齐策略和特定任务网络设计来弥合模态间的语义鸿沟，这限制了方法的通用性和效率。

**Method:** SIMMER采用基于多模态大语言模型的单统一编码器架构，具体使用VLM2Vec模型处理食品图像和食谱文本；设计了针对食谱结构化特征（标题、成分、烹饪步骤）的提示模板，并引入了组件感知数据增强策略，通过训练模型处理完整和部分食谱来提高对不完整输入的鲁棒性。

**Result:** 在Recipe1M数据集上的实验表明，SIMMER在1k和10k评估设置下均实现了最先进的性能，最佳模型将1k图像到食谱的R@1从81.8%提升至87.5%，将10k图像到食谱的R@1从56.5%提升至65.5%，显著优于所有先前方法。

**Conclusion:** 该研究证明了基于多模态大语言模型的单编码器方法在跨模态检索任务中的有效性，特别是通过针对性的提示工程和数据增强策略，能够有效处理结构化食谱数据，为食品计算领域的检索系统提供了更简洁高效的解决方案。
...
### [2] [RefereeBench: Are Video MLLMs Ready to be Multi-Sport Referees](https://arxiv.org/abs/2604.15736)
*Yichen Xu, Yuanhang Liu, Chuhan Wang, Zihan Zhao, jinghan luo, Jianzhe Ma, Wenxuan Wang, Qin Jin*

#### 🧩 TL;DR
本文提出了RefereeBench，这是首个用于评估多模态大语言模型作为自动体育裁判能力的大规模基准测试，涵盖11项运动、925个视频和6,475个问答对，揭示了当前模型在规则应用和时间定位方面的显著不足。

---

#### 📘 Detailed Summary
**Motivation:** 尽管多模态大语言模型在通用视频理解方面表现出色，但其支持专业化、基于规则的决策能力尚未得到充分探索。本研究旨在填补这一空白，通过构建首个大规模基准测试来评估MLLMs作为自动体育裁判的能力，以推动可信赖的AI辅助裁判和更广泛的多模态决策系统的发展。

**Method:** 研究团队开发了RefereeBench基准测试，涵盖11项不同运动，包含925个精心策划的视频和6,475个问答对。该基准评估五个核心裁判能力：犯规存在性判断、犯规和处罚分类、犯规和处罚推理、实体感知以及时间定位。所有标注均经过人工处理，确保基于真实裁判逻辑和多模态证据的高质量标注。

**Result:** 对最先进MLLMs的广泛评估显示，即使是性能最强的模型如Doubao-Seed-1.8和Gemini-3-Pro，准确率也仅达到约60%，而最强的开源模型Qwen3-VL仅达到47%。进一步分析表明，模型虽然能够识别事件和涉及实体，但在规则应用和时间定位方面表现不佳，且经常在正常片段上过度判罚犯规。

**Conclusion:** 研究结果表明当前多模态大语言模型距离成为可靠的体育裁判仍有很大差距，特别是在规则应用和时间定位方面存在显著不足。该基准测试强调了未来MLLMs需要更好地整合领域知识和多模态理解能力，这对于推进可信赖的AI辅助裁判系统和更广泛的多模态决策应用具有重要意义。
...
### [3] [UniEditBench: A Unified and Cost-Effective Benchmark for Image and Video Editing via Distilled MLLMs](https://arxiv.org/abs/2604.15871)
*Lifan Jiang, Tianrun Wu, Yuhang Pei, Chenyang Wang, Boxi Wu, Deng Cai*

#### 🧩 TL;DR
本文提出了UniEditBench，一个统一的图像和视频编辑基准，通过蒸馏大语言模型构建轻量级评估器，解决了现有视觉编辑评估方法碎片化、成本高昂的问题。

---

#### 📘 Detailed Summary
**Motivation:** 当前视觉编辑模型的评估存在碎片化问题，不同方法和模态使用不同的基准，难以进行公平的跨范式比较；视频编辑缺乏可靠的评估基准；现有的自动评估指标与人类偏好不一致，而直接使用大语言模型作为评估器又带来过高的计算和财务成本。

**Method:** 提出了UniEditBench统一基准，支持重建型和指令驱动型方法，包含九种图像操作和八种视频操作的结构化分类；通过蒸馏高容量大语言模型评估器（Qwen3-VL-235B-A22B Instruct）构建轻量级4B/8B评估器，提供结构保真度、文本对齐、背景一致性、自然度和时空一致性等多维度评分。

**Result:** 实验表明，蒸馏得到的轻量级评估器与人类判断保持高度一致，同时相对于教师模型大幅降低了部署成本；UniEditBench为现代视觉编辑方法提供了实用且可复现的基准协议。

**Conclusion:** 该研究为视觉编辑领域提供了统一的评估框架，通过模型蒸馏技术实现了高效且准确的自动评估，解决了跨范式比较和视频编辑评估的难题，为未来视觉编辑方法的发展提供了可靠的基准工具。
...
### [4] [Making Image Editing Easier via Adaptive Task Reformulation with Agentic Executions](https://arxiv.org/abs/2604.15917)
*Bo Zhao, Kairui Guo, Runnan Du, Haiyang Sun, Pengshan Wang, Huan Yang, Kun Gai, Yixin Cao, Wei Ji*

#### 🧩 TL;DR
本文提出了一种自适应任务重构框架，通过将原始图像编辑指令转化为由多模态大语言模型代理动态确定和执行的序列化操作，显著提升了指令引导图像编辑的可靠性，而无需修改底层生成模型。

---

#### 📘 Detailed Summary
**Motivation:** 当前指令引导图像编辑在许多看似简单的情况下仍无法产生可靠结果，研究发现大量失败并非源于模型容量不足，而是由于任务表述不当，如涉及小目标、隐式空间关系或指令不明确等问题，这揭示了任务表述作为关键但未被充分探索的因素。

**Method:** 该研究提出自适应任务重构框架，将原始图像-指令对转化为由多模态大语言模型代理通过分析、路由、重构和反馈驱动优化动态确定并执行的序列化操作，该框架不修改底层编辑模型，而是通过任务重构来匹配模型的有效操作区间。

**Result:** 在多个基准测试（包括ImgEdit、PICA和RePlan）上，使用不同编辑骨干模型（如Qwen Image Edit和Nano Banana）的实验显示了一致的性能改进，特别是在涉及小目标、复杂空间关系等挑战性案例上取得了显著增益，证明了任务重构对提升编辑可靠性的有效性。

**Conclusion:** 研究表明任务重构是影响图像编辑性能的关键因素，通过将编辑任务更好地匹配到现有模型的有效操作区间，可以在不修改底层模型的情况下实现实质性改进，这为未来图像编辑系统设计提供了新方向，强调了任务表述优化的重要性。
...
### [5] [Mind's Eye: A Benchmark of Visual Abstraction, Transformation and Composition for Multimodal LLMs](https://arxiv.org/abs/2604.16054)
*Rohit Sinha, Aditya Kanade, Sai Srinivas Kancheti, Vineeth N Balasubramanian, Tanuja Ganu*

#### 🧩 TL;DR
本文提出了"Mind's Eye"基准测试，用于评估多模态大语言模型的视觉认知与空间推理能力，发现当前模型在八项视觉认知任务上的表现显著低于人类水平，揭示了模型在视觉注意力分配和概念抽象方面的局限性。

---

#### 📘 Detailed Summary
**Motivation:** 尽管多模态大语言模型在视觉语言基准测试上取得了显著进展，但其视觉认知和空间推理能力尚未得到充分理解。现有研究缺乏对模型流体智力核心过程（如模式归纳、类比关系映射和心理转换）的系统评估，这构成了当前研究的重要空白。

**Method:** 研究引入了"Mind's Eye"基准测试，包含八个视觉认知任务，这些任务灵感来源于经典人类智力测试，并按照新颖的"A-R-T"分类法组织：抽象、关系和转换。该基准评估了闭源和开源的多模态大语言模型，并与人类参与者的表现进行比较，同时进行了详细的错误分析以识别模型失败的具体模式。

**Result:** 人类参与者在基准测试中达到了80%的准确率，而表现最佳的多模态大语言模型准确率仍低于50%。错误分析揭示了模型在三个关键方面的失败：视觉注意力分配不当、内部感知操作能力不足以及对底层视觉概念的抽象能力薄弱。

**Conclusion:** 研究结果表明，与人类相比，当前的多模态大语言模型在视觉空间推理能力上存在显著局限。这一发现强调了需要开发更具认知基础的评估框架，以更好地理解和提升模型的高级视觉认知能力，为未来模型设计提供了重要的方向性指导。
...
### [6] [HyperGVL: Benchmarking and Improving Large Vision-Language Models in Hypergraph Understanding and Reasoning](https://arxiv.org/abs/2604.15648)
...
#### 🧩 TL;DR
本文提出了首个用于评估大型视觉语言模型在超图理解与推理方面能力的基准测试HyperGVL，包含84,000个视觉语言问答样本，涵盖12项任务，并引入了可学习的自适应表示路由器WiseHyGR来提升模型性能。

---

#### 📘 Detailed Summary
**Motivation:** 尽管超图在生命科学和社会网络等现实领域具有重要应用价值，且大型视觉语言模型在复杂拓扑理解方面展现出潜力，但目前缺乏专门评估LVLMs在超图理解与推理能力的基准测试，导致其能力边界不明确。

**Method:** 研究提出了HyperGVL基准测试，包含84,000个视觉语言问答样本，涵盖12项从基础组件计数到复杂NP难问题推理的任务，涉及多尺度合成结构及真实引文和蛋白质网络。同时研究了12种文本和视觉超图表示方法的影响，并引入了可泛化的路由器WiseHyGR，通过学习自适应表示来提升LVLMs在超图任务上的性能。

**Result:** 研究对12种先进的大型视觉语言模型进行了全面评估，涵盖了广泛的超图理解任务。实验结果表明，不同表示方法对模型性能有显著影响，而提出的WiseHyGR路由器能够有效提升模型在超图任务上的表现，为模型选择最优表示提供了机制。

**Conclusion:** 这项工作在连接超图与大型视觉语言模型方面迈出了重要一步，为评估和提升LVLMs在复杂超图结构上的理解能力提供了系统框架。提出的基准测试和自适应表示学习机制为未来研究提供了基础，有助于明确模型能力边界并推动超图相关应用的发展。
...
### [7] [ReactBench: A Benchmark for Topological Reasoning in MLLMs on Chemical Reaction Diagrams](https://arxiv.org/abs/2604.15994)
*Qiang Xu, Shengyuan Bai, Yu Wang, He Cao, Leqing Chen, Yuanyuan Liu, Bin Feng, Zijing Liu, Yu Li*

#### 🧩 TL;DR
该研究揭示了多模态大语言模型在复杂拓扑结构推理方面的根本性缺陷，并提出了ReactBench基准测试来量化这一差距，发现MLLMs在整体结构推理任务上的性能比基于锚点的任务低30%以上。

---

#### 📘 Detailed Summary
**Motivation:** 当前多模态大语言模型在识别单个视觉元素和简单线性图推理方面表现出色，但在处理涉及分支路径、汇聚流和循环依赖的复杂拓扑结构时，其推理能力急剧下降，现有基准测试主要关注语义理解而非结构推理，无法有效探测这一能力差距。

**Method:** 研究引入了ReactBench基准测试，该基准利用化学反应图作为理想测试平台，包含1,618个专家标注的问答对，涵盖四个层次化的任务维度，通过控制消融实验区分感知与推理瓶颈，对17个MLLMs进行了广泛评估。

**Result:** 评估结果显示，MLLMs在基于锚点的任务和整体结构推理任务之间存在超过30%的显著性能差距，控制消融实验证实这一瓶颈在于推理而非感知能力，表明模型在复杂拓扑结构理解方面存在根本性缺陷。

**Conclusion:** 该研究揭示了多模态大语言模型在结构理解方面的根本性不足，为推进视觉推理能力的发展确立了明确方向，强调了开发能够处理复杂拓扑关系的模型架构和训练方法的重要性。
